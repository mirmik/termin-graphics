...
namespace termin {
    namespace animation {
        ...
        public:
            ...
            explicit TcAnimationClip(tc_animation_handle h)
                : handle(h) {
            ...
            TcAnimationClip(const TcAnimationClip& other)
                : handle(other.handle) {
                if (tc_animation* a = tc_animation_get(handle)) {
                    tc_animation_add_ref(a);
                }
            ...
            TcAnimationClip(TcAnimationClip&& other) noexcept
                : handle(other.handle) {
            ...
            tc_animation* get() const {
                return tc_animation_get(handle);
            }

            // Query
            bool is_valid() const {
                return tc_animation_is_valid(handle);
            }
            ...
            void deserialize_from(const tc_value* data, void* = nullptr) {
                ...
                if (uuid_val && uuid_val->type == TC_VALUE_STRING && uuid_val->data.s) {
                    ...
                    tc::Log::warn(
                        "tc_animation_clip deserialize: animation not found, uuid=%s name=%s", uuid_val->data.s, name);
                }
            ...
            static TcAnimationClip create(const std::string& name = "", const std::string& uuid_hint = "") {
                tc_animation_handle h =
                    uuid_hint.empty() ? tc_animation_create(nullptr) : tc_animation_get_or_create(uuid_hint.c_str());
                if (tc_animation_handle_is_invalid(h)) {
                    return TcAnimationClip();
                }
