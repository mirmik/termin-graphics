...
public:
    ...
    tc_plot_item3d_handle add_line(
        ...
        return require_handle(tc_retained_chart3d_add_line(
            chart_, x.data(), y.data(), z.data(), x.size(), &style));
    }
    ...
    tc_plot_item3d_handle add_scatter(
        ...
        return require_handle(tc_retained_chart3d_add_scatter(
            chart_, x.data(), y.data(), z.data(), x.size(), &style));
    }
    ...
    tc_plot_item3d_handle add_surface(
        ...
        if (x.size() != static_cast<std::size_t>(rows) * columns) {
            throw std::invalid_argument(
                "surface dimensions do not match the flat arrays");
        }
        ...
        return require_handle(tc_retained_chart3d_add_surface(
            chart_, x.data(), y.data(), z.data(), rows, columns, &style));
    }

    bool set_surface_grid(
    ...
    void set_axis_labels(
        ...
        const std::string& y,
        const std::string& z) {
        tc_retained_chart3d_set_axis_labels(
            chart_, x.c_str(), y.c_str(), z.c_str());
    }

    void set_axis_scale(float x, float y, float z) {
        require_success(
            tc_retained_chart3d_set_axis_scale(chart_, x, y, z),
            "set_axis_scale");
    }
    ...
    void set_surface_shading(bool enabled, float strength) {
        require_success(
            tc_retained_chart3d_set_surface_shading(
                chart_, enabled ? 1 : 0, strength),
            "set_surface_shading");
    }

    void set_light_direction(float x, float y, float z) {
        require_success(
            tc_retained_chart3d_set_light_direction(chart_, x, y, z),
            "set_light_direction");
    }
...
private:
    ...
    static tcplot::Color4 resolve_color(nb::object color, std::uint32_t index) {
        if (!color.is_none()) return color_from_seq(color);
    ...
    static void require_success(int result, const char* operation) {
        if (!result) {
            throw std::runtime_error(
                std::string("RetainedChart3D ") + operation + " failed");
        }
    }

    void attach(tgfx::GraphicsHost& graphics, tgfx::FontAtlas& font) {
        if (graphics_ == &graphics && font_ == &font) return;
        auto next_host = std::make_unique<tcplot::GpuHost>(graphics, font);
        require_success(
            tc_retained_chart3d_attach_gpu_host(chart_, next_host.get()),
            "attach_gpu_host");
...
void bind_retained_chart3d(nb::module_& m) {
    nb::class_<tc_plot_item3d_handle>(m, "PlotItem3DHandle")
        .def_prop_ro("scene_id", [](const tc_plot_item3d_handle& value) {
            return value.scene_id;
        })
        .def_prop_ro("index", [](const tc_plot_item3d_handle& value) {
            return value.index;
        })
        .def_prop_ro("generation", [](const tc_plot_item3d_handle& value) {
            return value.generation;
        });

    nb::class_<PythonRetainedChart3D>(m, "RetainedChart3D")
        .def(nb::init<>())
        .def("add_line", &PythonRetainedChart3D::add_line,
             nb::arg("x"), nb::arg("y"), nb::arg("z"),
             nb::arg("color") = nb::none(), nb::arg("thickness") = 1.5f)
        .def("add_scatter", &PythonRetainedChart3D::add_scatter,
             nb::arg("x"), nb::arg("y"), nb::arg("z"),
             nb::arg("color") = nb::none(), nb::arg("size") = 4.0f)
        .def("add_surface", &PythonRetainedChart3D::add_surface,
             nb::arg("x"), nb::arg("y"), nb::arg("z"),
             nb::arg("rows"), nb::arg("columns"),
             nb::arg("color") = nb::none(),
             nb::arg("colormap") = tcplot::SurfaceColorMap::Jet,
             nb::arg("wireframe") = false)
        .def("set_surface_grid", &PythonRetainedChart3D::set_surface_grid,
             nb::arg("item"), nb::arg("visible"), nb::arg("row_step"),
             nb::arg("column_step"), nb::arg("color"),
             nb::arg("width_px") = 1.5f)
        .def("set_surface_wireframe",
             &PythonRetainedChart3D::set_surface_wireframe)
        ...
        .def("set_light_direction",
             &PythonRetainedChart3D::set_light_direction)
        ...
        .def("render", &PythonRetainedChart3D::render,
             nb::arg("graphics_host"), nb::arg("font"),
