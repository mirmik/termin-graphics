...
fit_plot_range2d(PlotRange2D data_bounds, double padding_fraction = 0.05);
...
TCPLOT_API std::optional<PlotTicks2D>
make_plot_ticks2d(const PlotFrame2D &frame, float x_spacing_logical_px = 80.0f,
                  float y_spacing_logical_px = 50.0f, int minimum_ticks = 3);

// font_size_logical_px is scaled by pixel_scale. The result is in physical
// pixels and owns no font/backend pointer.
TCPLOT_API std::optional<PlotTextMetrics2D>
measure_plot_text2d(tgfx::FontAtlas &font, std::string_view text,
                    float font_size_logical_px, float pixel_scale = 1.0f);
