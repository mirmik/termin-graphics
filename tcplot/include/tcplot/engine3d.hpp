...
namespace tcplot {
    ...
    public:
        ...
        void plot(std::vector<double> x, std::vector<double> y, std::vector<double> z, LinePlotOptions options = {});

        void
        scatter(std::vector<double> x, std::vector<double> y, std::vector<double> z, ScatterPlotOptions options = {});

        void surface(std::vector<double> X,
                     std::vector<double> Y,
                     std::vector<double> Z,
                     uint32_t rows,
                     uint32_t cols,
        ...
        void compute_mvp_(float aspect, float out16[16], bool apply_axis_scale) const;

        static std::optional<MeshGpu> make_mesh_(tgfx::IRenderDevice& device,
        ...
        void build_grid_mesh_(tgfx::IRenderDevice& device, const double bounds_min[3], const double bounds_max[3]);
        void build_surface_mesh_(tgfx::IRenderDevice& device, const SurfaceSeries& surf);
