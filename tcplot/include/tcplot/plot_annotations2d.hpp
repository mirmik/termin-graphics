...
inline constexpr std::string_view kPlotDefaultFontResource2D =
    "tcplot://default-font";
...
struct PlotAnnotationHandle {
    ...
    friend bool operator==(
        const PlotAnnotationHandle&,
        const PlotAnnotationHandle&) = default;
};
...
using PlotAnchor2D = std::variant<
    DataAnchor2D,
    SeriesPointRef2D,
    AxesFractionAnchor2D,
    ViewportPixelAnchor2D>;
...
public:
    ...
    using ActionHandler =
        std::function<void(const PlotAnnotationAction2D&)>;
    ...
    std::optional<PlotAnnotationSnapshot2D> snapshot(
    ...
    std::optional<PlotPoint2D> snap_data(
        PlotAnnotationHandle handle,
    ...
    bool route_pointer(
        ...
        const termin::visual::PointerEvent2D& event);
