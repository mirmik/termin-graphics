...
из C++ через имя виджета. Интерактивный `termin.gui.Plot3D` оборачивает
`RetainedChart3D`, готовит его GPU-текстуру перед UI render pass и может
работать обычным дочерним виджетом или portal content, включая body узла
`termin-nodegraph`. Сам `tcplot` при этом не зависит от UI: вся integration
policy остаётся в leaf bridge.
...
Surface, scatter и grid payloads дополнительно кэшируют encoder-ready vertex
