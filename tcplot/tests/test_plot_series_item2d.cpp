...
int main() {
  ...
  const auto other_projection =
      tcplot::create_plot_projection2d(other_scene, frame());
  ...
  const auto line = tc_plot_line_series_item2d_create(
      scene_handle, projection->handle(), x.data(), y.data(), nullptr, x.size(),
      line_style);
  const auto scatter = tc_plot_scatter_series_item2d_create(
      scene_handle, projection->handle(), x.data(), y.data(), x.size(),
      scatter_style);
  ...
  assert(tc_graphic_item_handle_is_invalid(tc_plot_line_series_item2d_create(
      other_scene_handle, projection->handle(), x.data(), y.data(), nullptr,
      x.size(), line_style)));

  tc_plot_series_snapshot2d snapshot{};
  tc_plot_line_style_state2d copied_line_style{};
  assert(tc_plot_line_series_item2d_snapshot(scene_handle, line, &snapshot,
                                             &copied_line_style));
  ...
  assert(tc_plot_line_series_item2d_append(scene_handle, line, append_x,
                                           append_y, nullptr, 2));
  assert(tc_plot_line_series_item2d_snapshot(scene_handle, line, &snapshot,
                                             &copied_line_style));
  ...
  assert(tc_plot_line_series_item2d_copy_data(
             scene_handle, line, copied_x.data(), copied_y.data(), nullptr,
             copied_x.size()) == copied_x.size());
  assert(copied_x.back() == 12.0 && copied_y.back() == 3.0);

  tc_plot_nearest_point2d nearest{};
  assert(tc_plot_line_series_item2d_nearest(scene_handle, line, 350.0f, 230.0f,
                                            2.0f, &nearest));
  assert(nearest.index == 2 && nearest.data_x == 5.0);
  assert(!tc_plot_line_series_item2d_set_projection(
      scene_handle, line, other_projection->handle()));
  ...
  assert(tc_plot_line_series_item2d_set_data(scene_handle, line, large_x.data(),
                                             large_y.data(), nullptr,
                                             large_count));
  ...
  std::printf("retained-series paint: 200000 points, 25 snapshots, %lld us, "
              "%zu commands\n",
              static_cast<long long>(elapsed.count()), command_count);
