...
int main() {
  ...
  auto projection = create_plot_projection2d(
      scene, frame(5.0f, 7.0f, 400.0f, 300.0f, 0.0, 10.0, -5.0, 5.0));
  auto foreign_projection = create_plot_projection2d(
      other_scene, frame(0.0f, 0.0f, 100.0f, 100.0f, 0.0, 1.0, 0.0, 1.0));
  ...
  const auto item = tc_plot_grid_item2d_create(
      scene_handle, projection->handle(), x_ticks.data(), x_ticks.size(),
      y_ticks.data(), y_ticks.size(), style);
  assert(!tc_graphic_item_handle_is_invalid(item));
  assert(tc_visual_scene_item_count(scene_handle) == 1);

  const auto cross_scene_item = tc_plot_grid_item2d_create(
      other_scene_handle, projection->handle(), nullptr, 0, nullptr, 0, style);
  ...
  assert(tc_plot_grid_item2d_copy_ticks(scene_handle, item, nullptr, 0, nullptr,
                                        0) == x_ticks.size() + y_ticks.size());
  std::vector<double> copied_x(x_ticks.size());
  std::vector<double> copied_y(y_ticks.size());
  assert(tc_plot_grid_item2d_copy_ticks(scene_handle, item, copied_x.data(),
                                        copied_x.size(), copied_y.data(),
                                        copied_y.size()) ==
  ...
  assert(projection->update(
      frame(100.0f, 50.0f, 220.0f, 160.0f, 0.0, 20.0, -10.0, 10.0)));
  builder.clear();
  assert(scene.paint(builder, resolver));
  ...
  assert(!tc_plot_grid_item2d_set_ticks(scene_handle, item, &invalid_tick, 1,
                                        nullptr, 0));
  assert(!tc_plot_grid_item2d_set_projection(scene_handle, item,
                                             foreign_projection->handle()));
