...
namespace {
    ...
    tcplot::PlotFrame2D frame(float x,
                              float y,
                              float width,
                              float height,
                              double x_min,
...
int main() {
    ...
    auto created = create_plot_projection2d(scene, frame(5.0f, 7.0f, 640.0f, 480.0f, -4.0, 8.0, -2.0, 6.0, 1.5f));
    ...
    assert(projection.owner_scene().handle().generation == scene_handle.generation);
    ...
    assert(tc_plot_projection_state2d_data_to_visual(&c_state, {2.0, 2.0}, &visual));
    tc_plot_point2d round_trip{};
    assert(tc_plot_projection_state2d_visual_to_data(&c_state, visual, &round_trip));
    assert(near(round_trip.x, 2.0));
    assert(near(round_trip.y, 2.0));

    const PlotFrame2D resized = frame(100.0f, 200.0f, 320.0f, 240.0f, 20.0, 40.0, 30.0, 50.0, 2.0f);
    ...
    auto reused = create_plot_projection2d(scene, frame(0.0f, 0.0f, 100.0f, 100.0f, 0.0, 1.0, 0.0, 1.0));
    ...
    const PlotProjectionHandle2D stale_scene_create = tc_plot_projection2d_create(scene_handle, &valid_desc);
