...
int main() {
    ...
    assert(!fit_plot_range2d({0.0, std::numeric_limits<double>::infinity(), 0.0, 1.0}));
    ...
    const tc_plot_axis_ticks_desc2d tick_desc{-4.0, 8.0, 800.0f, 80.0f, 1.0f, 3};
    const size_t c_tick_count = tc_plot_axis_ticks2d_copy(&tick_desc, nullptr, 0);
    assert(c_tick_count > 0);
    std::vector<double> c_ticks(c_tick_count);
    assert(tc_plot_axis_ticks2d_copy(&tick_desc, c_ticks.data(), c_ticks.size()) == c_tick_count);
    const size_t label_size = tc_plot_format_tick2d(c_ticks.front(), nullptr, 0);
    assert(label_size > 1);
    std::string label(label_size, '\0');
    assert(tc_plot_format_tick2d(c_ticks.front(), label.data(), label.size()) == label_size);
    ...
    const auto alternate = measure_plot_text2d(alternate_font, "Axis label", 14.0f);
    assert(alternate);
