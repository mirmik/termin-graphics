...
bool valid_style(PlotGridStyle2D style) {
  return style.color.is_finite() && std::isfinite(style.width_px) &&
         style.width_px > 0.0f;
}
...
PlotGridItem2D::PlotGridItem2D(PlotProjection2D projection,
    ...
    : NativeGraphicItem2D(kPlotGridItemType), projection_(projection),
      x_ticks_(std::move(x_ticks)), y_ticks_(std::move(y_ticks)),
      style_(style) {
...
bool PlotGridItem2D::projection_matches_item_scene() const {
  ...
  return tc_graphic_item_handle_is_invalid(item_handle) ||
         (projection_.valid() &&
          item_handle.scene_id == projection_.handle().scene_id);
}
...
bool PlotGridItem2D::set_projection(PlotProjection2D projection) {
  ...
  const auto item_handle = handle();
  if (!tc_graphic_item_handle_is_invalid(item_handle) &&
      item_handle.scene_id != projection.handle().scene_id) {
    tc::Log::error(
        "PlotGridItem2D::set_projection rejected cross-scene projection");
    return false;
  }
...
bool PlotGridItem2D::set_ticks(std::vector<double> x_ticks,
                               std::vector<double> y_ticks) {
...
PlotGridSnapshot2D PlotGridItem2D::snapshot() const {
  return {
      projection_, style_, x_ticks_, y_ticks_, revision_,
  };
}
...
bool PlotGridItem2D::paint(
  ...
  if (!has_lines)
    return true;
  if (!context.push_clip_rect({clip_area->x(), clip_area->y(),
                               clip_area->width(), clip_area->height()})) {
    tc::Log::error("PlotGridItem2D::paint failed to push plot clip");
    return false;
  }
  const bool painted =
      context.path(std::move(path), std::nullopt,
                   tgfx::StrokePaint{style_.color, style_.width_px});
...
bool PlotGridItem2D::hit_test(termin::Vec2f, float) const { return false; }

std::optional<termin::visual::GraphicItemHandle>
adopt_plot_grid_item2d(termin::visual::TcVisualScene scene,
                       PlotProjection2D projection, std::vector<double> x_ticks,
                       std::vector<double> y_ticks, PlotGridStyle2D style) {
  if (!scene.valid() || !projection.matches_scene(scene)) {
    tc::Log::error(
        "adopt_plot_grid_item2d requires a live matching scene and projection");
    return std::nullopt;
  }
  try {
    return scene.adopt(std::make_unique<PlotGridItem2D>(
        projection, std::move(x_ticks), std::move(y_ticks), style));
...
PlotGridItem2D *
resolve_plot_grid_item2d(termin::visual::TcVisualScene &scene,
...
const PlotGridItem2D *
resolve_plot_grid_item2d(const termin::visual::TcVisualScene &scene,
                         termin::visual::GraphicItemHandle handle) {
...
tc_graphic_item_handle tc_plot_grid_item2d_create(
...
bool tc_plot_grid_item2d_set_projection(
  ...
  if (grid == nullptr) {
    tc::Log::error(
        "tc_plot_grid_item2d_set_projection received stale or wrong item");
    return false;
  }
...
bool tc_plot_grid_item2d_set_ticks(tc_visual_scene_handle owner_scene,
  ...
  if (grid == nullptr) {
    tc::Log::error(
        "tc_plot_grid_item2d_set_ticks received stale or wrong item");
    return false;
  }
  return grid->set_ticks(tcplot::copy_ticks(x_ticks, x_tick_count),
...
bool tc_plot_grid_item2d_set_style(tc_visual_scene_handle owner_scene,
  ...
  if (grid == nullptr) {
    tc::Log::error(
        "tc_plot_grid_item2d_set_style received stale or wrong item");
    return false;
  }
...
size_t tc_plot_grid_item2d_copy_ticks(tc_visual_scene_handle owner_scene,
                                      tc_graphic_item_handle item,
                                      double *out_x_ticks, size_t x_capacity,
                                      double *out_y_ticks, size_t y_capacity) {
  const termin::visual::TcVisualScene scene{owner_scene};
  const auto *grid = tcplot::resolve_plot_grid_item2d(scene, item);
  if (grid == nullptr) {
    tc::Log::error(
        "tc_plot_grid_item2d_copy_ticks received stale or wrong item");
    return 0;
  }
  const tcplot::PlotGridSnapshot2D snapshot = grid->snapshot();
  ...
  if (out_x_ticks == nullptr && out_y_ticks == nullptr)
    return total;
  if ((snapshot.x_ticks.size() > 0 &&
       (out_x_ticks == nullptr || x_capacity < snapshot.x_ticks.size())) ||
      (snapshot.y_ticks.size() > 0 &&
       (out_y_ticks == nullptr || y_capacity < snapshot.y_ticks.size()))) {
