...
namespace tcplot
{
    ...
    bool ChartInteraction2D::pointer_down(const PlotFrame2D& frame,
                                          float x,
                                          float y,
                                          int button)
    {
        if (button != kMiddleMouseButton || !finite_pointer(x, y) ||
    ...
    std::optional<PlotRange2D> ChartInteraction2D::pointer_move(float x,
                                                                float y) const
    {
        ...
        return PlotRange2D{start.x_min() + dx,
                           start.x_max() + dx,
                           start.y_min() + dy,
                           start.y_max() + dy};
    }
    ...
    ChartInteraction2D::wheel(const PlotFrame2D& frame,
                              float x,
                              float y,
                              float steps,
                              bool x_only) const
    {
        if (!finite_pointer(x, y) || !std::isfinite(steps) || steps == 0.0f ||
            !frame.contains_plot_pixel(x, y))
            return std::nullopt;
        ...
        return PlotRange2D{
            center.x + (range.x_min() - center.x) * factor,
            center.x + (range.x_max() - center.x) * factor,
            x_only ? range.y_min()
                   : center.y + (range.y_min() - center.y) * factor,
            x_only ? range.y_max()
                   : center.y + (range.y_max() - center.y) * factor};
    }
