...
namespace tcplot {
    namespace {
        ...
        bool valid_line_style(PlotLineSeriesStyle2D style) {
            ...
            return line_style >= static_cast<int>(LineStyle::Solid) && line_style <= static_cast<int>(LineStyle::Dot) &&
                   colormap >= static_cast<int>(SurfaceColorMap::Jet) &&
                   colormap <= static_cast<int>(SurfaceColorMap::Solid) && finite_color(style.color) &&
                   std::isfinite(style.thickness_px) && style.thickness_px > 0.0f && std::isfinite(style.dash_px) &&
                   style.dash_px > 0.0f && std::isfinite(style.gap_px) && style.gap_px > 0.0f &&
                   std::isfinite(style.scalar_min) && std::isfinite(style.scalar_max) &&
                   style.scalar_max > style.scalar_min;
        }

        bool valid_scatter_style(PlotScatterSeriesStyle2D style) {
            return finite_color(style.color) && std::isfinite(style.diameter_px) && style.diameter_px > 0.0f;
        }

        bool valid_line_data(std::span<const double> x, std::span<const double> y, std::span<const double> scalar) {
            return x.size() == y.size() && (scalar.empty() || scalar.size() == x.size()) && finite_values(x) &&
        ...
        std::optional<termin::Rect2f> intersect_clip(std::optional<termin::Rect2f> requested,
            ...
            const float x1 = std::min(requested->x + requested->width, state.clip_rect.x + state.clip_rect.width);
            const float y1 = std::min(requested->y + requested->height, state.clip_rect.y + state.clip_rect.height);
            return termin::Rect2f{x0, y0, std::max(0.0f, x1 - x0), std::max(0.0f, y1 - y0)};
        }
        ...
        void data_to_clip(const PlotFrame2D& frame,
                          const termin::Affine2f& transform,
                          termin::Rect2f viewport,
                          float out[16]) {
        ...
        float projected_length_scale(const PlotFrame2D& frame, const termin::Affine2f& transform) {
            const auto& area = frame.plot_area();
            const auto& range = frame.range();
            const auto x = transform.transform_vector({static_cast<float>(area.width() / range.x_span()), 0.0f});
            const auto y = transform.transform_vector({0.0f, static_cast<float>(area.height() / range.y_span())});
            return std::sqrt((x.x * x.x + x.y * x.y + y.x * y.x + y.y * y.y) * 0.5f);
        }

        termin::Rect2f transformed_viewport(const PlotFrame2D& frame, const termin::Affine2f& transform) {
            const auto& viewport = frame.viewport();
            const auto bounds =
                transform.transform_bounds({viewport.x(), viewport.y(), viewport.right(), viewport.bottom()});
            return {bounds.x0, bounds.y0, bounds.x1 - bounds.x0, bounds.y1 - bounds.y0};
        }

        std::optional<PlotNearestPoint2D> nearest_point(const PlotProjection2D& projection,
            ...
            for (std::size_t index = 0; index < x.size(); ++index) {
                ...
                result = PlotNearestPoint2D{index, x[index], y[index], {projected.x, projected.y}, std::sqrt(squared)};
            }
            return result;
        }
        ...
        std::vector<double> copy_array(const double* values, std::size_t count) {
            return count == 0 ? std::vector<double>{} : std::vector<double>{values, values + count};
        }
    ...
    struct PlotLineSeriesGpu2D::Impl {
        ...
        bool ensure_capacity(tgfx::IRenderDevice& target, std::uint32_t wanted, std::uint32_t stride_floats) {
            ...
            desc.size = static_cast<std::uint64_t>(next) * stride_floats * sizeof(float);
    ...
    PlotLineSeriesGpu2D::PlotLineSeriesGpu2D()
    ...
    bool PlotLineSeriesGpu2D::render(tgfx::RenderContext2& context,
        ...
        if (!valid_line_data(x, y, scalar) || !valid_line_style(style) ||
            x.size() > std::numeric_limits<std::uint32_t>::max() || !std::isfinite(opacity) || opacity < 0.0f ||
            opacity > 1.0f) {
        ...
        if (!styled) {
            ...
            const std::uint32_t first = impl_->full_dirty ? 0 : std::min(impl_->gpu_count, point_count);
        ...
        if (!impl_->styled_uploaded || impl_->full_dirty || impl_->gpu_count != vertex_count) {
            const bool tail_update = impl_->append_only && impl_->styled_uploaded && !impl_->full_dirty &&
                                     impl_->gpu_count >= 2 && impl_->gpu_count < vertex_count &&
                                     (impl_->gpu_count % 2) == 0;
            const std::uint32_t first_point = tail_update ? impl_->gpu_count / 2 - 1 : 0;
            std::vector<float> vertices;
            vertices.reserve(static_cast<std::size_t>(point_count - first_point) * 2 * 10);
            double cumulative = tail_update ? impl_->styled_cumulative_end : 0.0;
            for (std::uint32_t index = first_point; index < point_count; ++index) {
                ...
                const float scalar_value = scalar.empty() ? 0.0f : static_cast<float>(scalar[index]);
            ...
            context.device().upload_buffer(
                impl_->vbo,
                std::span<const std::uint8_t>{reinterpret_cast<const std::uint8_t*>(vertices.data()),
        ...
        push.params[2] =
            scalar.empty() ? static_cast<float>(SurfaceColorMap::Solid) : static_cast<float>(style.colormap);
        ...
        push.viewport[2] = style.line_style == LineStyle::Dot ? style.thickness_px : style.dash_px;
    ...
    void PlotScatterSeriesGpu2D::invalidate_data(bool) {
        impl_->dirty = true;
    }
    ...
    bool PlotScatterSeriesGpu2D::render(tgfx::RenderContext2& context,
                                        const PlotFrame2D& frame,
                                        const termin::Affine2f& transform,
                                        float opacity,
                                        std::optional<termin::Rect2f> clip_rect,
        ...
        if (!impl_->corners) {
            ...
            impl_->device->upload_buffer(
                impl_->corners,
                std::span<const std::uint8_t>{reinterpret_cast<const std::uint8_t*>(corners.data()), sizeof(corners)});
        }
        ...
        corners_layout.attributes.push_back({0, tgfx::VertexFormat::Float2, 0, "POSITION"});
        tgfx::VertexBufferLayout instance_layout;
        instance_layout.stride = 2 * sizeof(float);
        instance_layout.per_instance = true;
        instance_layout.attributes.push_back({1, tgfx::VertexFormat::Float2, 0, "INSTANCEPOS"});
        context.set_vertex_layouts(
            std::vector<tgfx::VertexBufferLayout>{std::move(corners_layout), std::move(instance_layout)});
        ...
        context.draw_arrays_instanced(impl_->corners, impl_->instances, 6, impl_->gpu_count);
        return true;
    }

    struct PlotLineSeriesItem2D::State {
        State(PlotProjection2D projection_value,
    ...
    struct PlotScatterSeriesItem2D::State {
        State(PlotProjection2D projection_value,
              std::vector<double> x_value,
              std::vector<double> y_value,
              PlotScatterSeriesStyle2D style_value)
            : projection(projection_value),
              x(std::move(x_value)),
              y(std::move(y_value)),
              style(style_value) {}
    ...
    namespace {
        ...
        public:
            ...
            bool draw(tgfx::RenderContext2& context, const tgfx::RetainedDrawState2D& draw_state) override {
                ...
                return state_->gpu.render(context,
                                          projection->frame,
        ...
        public:
            explicit ScatterRetainedBatch(std::shared_ptr<PlotScatterSeriesItem2D::State> state)
                : state_(std::move(state)) {}

            bool draw(tgfx::RenderContext2& context, const tgfx::RetainedDrawState2D& draw_state) override {
                if (draw_state.unsupported_clip) {
                    tc::Log::error("PlotScatterSeriesItem2D requires rectangular retained clip");
                    return false;
                }
                const auto projection = state_->projection.snapshot();
                if (!projection) {
        ...
        bool matching_scene(tc_graphic_item_handle item, const PlotProjection2D& projection) {
    ...
    PlotLineSeriesItem2D::PlotLineSeriesItem2D(PlotProjection2D projection,
        ...
        : NativeGraphicItem2D(kLineItemType),
          state_(std::make_shared<State>(projection, std::move(x), std::move(y), std::move(scalar), style)) {
        if (!projection.valid() || !valid_line_data(state_->x, state_->y, state_->scalar) || !valid_line_style(style)) {
    ...
    bool PlotLineSeriesItem2D::set_projection(PlotProjection2D projection) {
        if (!projection.valid() || !matching_scene(handle(), projection)) {
            tc::Log::error("PlotLineSeriesItem2D rejected stale or cross-scene projection");
            return false;
        }
    ...
    bool PlotLineSeriesItem2D::set_data(std::vector<double> x, std::vector<double> y, std::vector<double> scalar) {
    ...
    bool
    PlotLineSeriesItem2D::append(std::span<const double> x, std::span<const double> y, std::span<const double> scalar) {
        if (!valid_line_data(x, y, scalar) || (!state_->scalar.empty() && scalar.size() != x.size()) ||
    ...
    std::span<const double> PlotLineSeriesItem2D::x() const {
        return state_->x;
    }
    std::span<const double> PlotLineSeriesItem2D::y() const {
        return state_->y;
    }
    ...
    std::optional<PlotNearestPoint2D> PlotLineSeriesItem2D::nearest(termin::Vec2f pixel, float max_distance_px) const {
        return nearest_point(state_->projection, state_->x, state_->y, pixel, max_distance_px);
    }
    ...
    bool PlotLineSeriesItem2D::paint(termin::visual::GraphicItemPaintContext2D& context) const {
        if (!matching_scene(handle(), state_->projection)) {
        ...
        if (!area || state_->x.size() < 2)
            return true;
        if (!context.push_clip_rect({area->x(), area->y(), area->width(), area->height()})) {
            return false;
        }
    ...
    bool PlotLineSeriesItem2D::hit_test(termin::Vec2f point, float tolerance) const {
    ...
    PlotScatterSeriesItem2D::PlotScatterSeriesItem2D(PlotProjection2D projection,
        ...
        : NativeGraphicItem2D(kScatterItemType),
          state_(std::make_shared<State>(projection, std::move(x), std::move(y), style)) {
        if (!projection.valid() || !valid_scatter_data(state_->x, state_->y) || !valid_scatter_style(style)) {
    ...
    bool PlotScatterSeriesItem2D::set_projection(PlotProjection2D projection) {
        if (!projection.valid() || !matching_scene(handle(), projection)) {
            tc::Log::error("PlotScatterSeriesItem2D rejected stale or cross-scene projection");
            return false;
        }
        state_->projection = projection;
        bump(state_->revision);
    ...
    bool PlotScatterSeriesItem2D::set_data(std::vector<double> x, std::vector<double> y) {
    ...
    std::span<const double> PlotScatterSeriesItem2D::x() const {
        return state_->x;
    }
    std::span<const double> PlotScatterSeriesItem2D::y() const {
        return state_->y;
    }
    std::uint64_t PlotScatterSeriesItem2D::revision() const {
        return state_->revision;
    }
    std::optional<PlotNearestPoint2D> PlotScatterSeriesItem2D::nearest(termin::Vec2f pixel,
    ...
    bool PlotScatterSeriesItem2D::paint(termin::visual::GraphicItemPaintContext2D& context) const {
        if (!matching_scene(handle(), state_->projection)) {
    ...
    bool PlotScatterSeriesItem2D::hit_test(termin::Vec2f point, float tolerance) const {
        return nearest(point, tolerance + state_->style.diameter_px * 0.5f).has_value();
    }

    std::optional<termin::visual::GraphicItemHandle> adopt_plot_line_series_item2d(termin::visual::TcVisualScene scene,
                                                                                   PlotProjection2D projection,
                                                                                   std::vector<double> x,
                                                                                   std::vector<double> y,
                                                                                   std::vector<double> scalar,
    ...
    adopt_plot_scatter_series_item2d(termin::visual::TcVisualScene scene,
        ...
        try {
            return scene.adopt(
                std::make_unique<PlotScatterSeriesItem2D>(projection, std::move(x), std::move(y), style));
        } catch (const std::exception& error) {
    ...
    template <typename T>
    T* resolve_item(termin::visual::TcVisualScene& scene, termin::visual::GraphicItemHandle handle, const char* type) {
        auto* item = scene.resolve(handle);
        if (!item || !item->body || std::strcmp(tc_graphic_item_type_name(item), type) != 0) {
            return nullptr;
        }
    ...
    PlotLineSeriesItem2D* resolve_plot_line_series_item2d(termin::visual::TcVisualScene& scene,
                                                          termin::visual::GraphicItemHandle handle) {
        return resolve_item<PlotLineSeriesItem2D>(scene, handle, kLineItemType);
    }
    const PlotLineSeriesItem2D* resolve_plot_line_series_item2d(const termin::visual::TcVisualScene& scene,
                                                                termin::visual::GraphicItemHandle handle) {
        return resolve_item<PlotLineSeriesItem2D>(scene, handle, kLineItemType);
    }
    PlotScatterSeriesItem2D* resolve_plot_scatter_series_item2d(termin::visual::TcVisualScene& scene,
                                                                termin::visual::GraphicItemHandle handle) {
        return resolve_item<PlotScatterSeriesItem2D>(scene, handle, kScatterItemType);
    }
    const PlotScatterSeriesItem2D* resolve_plot_scatter_series_item2d(const termin::visual::TcVisualScene& scene,
                                                                      termin::visual::GraphicItemHandle handle) {
        return resolve_item<PlotScatterSeriesItem2D>(scene, handle, kScatterItemType);
    }
...
tc_graphic_item_handle tc_plot_line_series_item2d_create(tc_visual_scene_handle owner_scene,
    ...
    auto result =
        tcplot::adopt_plot_line_series_item2d(termin::visual::TcVisualScene{owner_scene},
                                              tcplot::PlotProjection2D{projection},
                                              tcplot::copy_array(x, point_count),
...
tc_graphic_item_handle tc_plot_scatter_series_item2d_create(tc_visual_scene_handle owner_scene,
                                                            tc_plot_projection_handle2d projection,
                                                            const double* x,
                                                            const double* y,
                                                            size_t point_count,
    ...
    auto result = tcplot::adopt_plot_scatter_series_item2d(termin::visual::TcVisualScene{owner_scene},
                                                           tcplot::PlotProjection2D{projection},
                                                           tcplot::copy_array(x, point_count),
                                                           tcplot::copy_array(y, point_count),
                                                           tcplot::from_c(style));
...
bool tc_plot_line_series_item2d_set_projection(tc_visual_scene_handle owner_scene,
                                               tc_graphic_item_handle item,
                                               tc_plot_projection_handle2d projection) {
    TCPLOT_RESOLVE_MUTABLE("line set_projection", resolve_plot_line_series_item2d, series);
    return series->set_projection(tcplot::PlotProjection2D{projection});
}

bool tc_plot_scatter_series_item2d_set_projection(tc_visual_scene_handle owner_scene,
                                                  tc_graphic_item_handle item,
                                                  tc_plot_projection_handle2d projection) {
    TCPLOT_RESOLVE_MUTABLE("scatter set_projection", resolve_plot_scatter_series_item2d, series);
    return series->set_projection(tcplot::PlotProjection2D{projection});
}
...
bool tc_plot_line_series_item2d_set_data(tc_visual_scene_handle owner_scene,
    ...
    if (point_count > 0 && (!x || !y))
        return false;
    TCPLOT_RESOLVE_MUTABLE("line set_data", resolve_plot_line_series_item2d, series);
...
bool tc_plot_line_series_item2d_append(tc_visual_scene_handle owner_scene,
    ...
    if (point_count > 0 && (!x || !y))
        return false;
    TCPLOT_RESOLVE_MUTABLE("line append", resolve_plot_line_series_item2d, series);
    return series->append(std::span<const double>{x, point_count},
                          std::span<const double>{y, point_count},
                          scalar ? std::span<const double>{scalar, point_count} : std::span<const double>{});
}
...
bool tc_plot_scatter_series_item2d_set_data(tc_visual_scene_handle owner_scene,
    ...
    if (point_count > 0 && (!x || !y))
        return false;
    TCPLOT_RESOLVE_MUTABLE("scatter set_data", resolve_plot_scatter_series_item2d, series);
    return series->set_data(tcplot::copy_array(x, point_count), tcplot::copy_array(y, point_count));
}

bool tc_plot_line_series_item2d_set_style(tc_visual_scene_handle owner_scene,
                                          tc_graphic_item_handle item,
                                          tc_plot_line_style_state2d style) {
    TCPLOT_RESOLVE_MUTABLE("line set_style", resolve_plot_line_series_item2d, series);
    return series->set_style(tcplot::from_c(style));
}

bool tc_plot_scatter_series_item2d_set_style(tc_visual_scene_handle owner_scene,
                                             tc_graphic_item_handle item,
                                             tc_plot_scatter_style_state2d style) {
    TCPLOT_RESOLVE_MUTABLE("scatter set_style", resolve_plot_scatter_series_item2d, series);
    return series->set_style(tcplot::from_c(style));
}

#undef TCPLOT_RESOLVE_MUTABLE

bool tc_plot_line_series_item2d_snapshot(tc_visual_scene_handle owner_scene,
                                         tc_graphic_item_handle item,
    ...
    if (!series)
        return false;
    *out_snapshot = {series->projection().handle(), series->x().size(), !series->scalar().empty(), series->revision()};
    *out_style = tcplot::to_c(series->style());
    return true;
}

bool tc_plot_scatter_series_item2d_snapshot(tc_visual_scene_handle owner_scene,
                                            tc_graphic_item_handle item,
                                            tc_plot_series_snapshot2d* out_snapshot,
    ...
    if (!series)
        return false;
    *out_snapshot = {series->projection().handle(), series->x().size(), false, series->revision()};
    *out_style = tcplot::to_c(series->style());
    return true;
}
...
size_t tc_plot_line_series_item2d_copy_data(tc_visual_scene_handle owner_scene,
    ...
    if (!out_x && !out_y && !out_scalar)
        return count;
    if (capacity < count || !out_x || !out_y || (!series->scalar().empty() && !out_scalar)) {
...
size_t tc_plot_scatter_series_item2d_copy_data(
    tc_visual_scene_handle owner_scene, tc_graphic_item_handle item, double* out_x, double* out_y, size_t capacity) {
    const termin::visual::TcVisualScene scene{owner_scene};
    const auto* series = tcplot::resolve_plot_scatter_series_item2d(scene, item);
    if (!series)
        return 0;
...
bool tc_plot_line_series_item2d_nearest(tc_visual_scene_handle owner_scene,
    ...
    if (!nearest)
        return false;
    *out_point = {
        nearest->index, nearest->data_x, nearest->data_y, nearest->pixel.x, nearest->pixel.y, nearest->distance_px};
    return true;
}
