...
bool ensure_pool() {
  ...
  if (!tc_pool_init_ex(&g_projection_pool, sizeof(ProjectionSlot), 16,
                       &config)) {
...
ProjectionSlot *raw_slot(tc_plot_projection_handle2d handle) {
  if (!g_projection_pool_initialized ||
      tc_plot_projection_handle2d_is_invalid(handle)) {
    return nullptr;
  }
  auto *slot = static_cast<ProjectionSlot *>(
      tc_pool_get(&g_projection_pool, local_handle(handle)));
  return slot != nullptr && slot->owner_scene_id == handle.scene_id ? slot
                                                                    : nullptr;
}
...
ProjectionSlot *checked_live_slot(tc_plot_projection_handle2d handle,
                                  const char *operation) {
  ProjectionSlot *slot = raw_slot(handle);
  if (slot == nullptr) {
    tc_log_error("%s: invalid or stale projection handle "
                 "(scene=%llu index=%u generation=%u)",
                 operation, static_cast<unsigned long long>(handle.scene_id),
...
tc_plot_projection_desc2d to_c(const tcplot::PlotFrame2D &frame) {
  return {
      to_c(frame.viewport()),  to_c(frame.plot_area()), to_c(frame.range()),
      to_c(frame.clip_rect()), frame.pixel_scale(),
  };
}
...
tcplot::PlotFrame2D from_c(const tc_plot_projection_desc2d &value) {
  return {
      from_c(value.viewport),  from_c(value.plot_area), from_c(value.range),
      from_c(value.clip_rect), value.pixel_scale,
  };
}
...
bool tc_plot_projection_desc2d_is_valid(const tc_plot_projection_desc2d *desc) {
  if (desc == nullptr || !finite_rect(desc->viewport) ||
      !finite_rect(desc->plot_area) || !finite_rect(desc->clip_rect) ||
      !std::isfinite(desc->range.x_min) || !std::isfinite(desc->range.x_max) ||
      !std::isfinite(desc->range.y_min) || !std::isfinite(desc->range.y_max) ||
      !std::isfinite(desc->pixel_scale)) {
    return false;
  }
  return desc->viewport.width >= 0.0f && desc->viewport.height >= 0.0f &&
         desc->plot_area.width > 0.0f && desc->plot_area.height > 0.0f &&
         desc->clip_rect.width >= 0.0f && desc->clip_rect.height >= 0.0f &&
         desc->range.x_max > desc->range.x_min &&
         desc->range.y_max > desc->range.y_min && desc->pixel_scale > 0.0f;
}

tc_plot_projection_handle2d
tc_plot_projection2d_create(tc_visual_scene_handle owner_scene,
  ...
  auto *slot =
      static_cast<ProjectionSlot *>(tc_pool_get(&g_projection_pool, local));
...
bool tc_plot_projection2d_destroy(tc_plot_projection_handle2d handle) {
  ...
  if (slot == nullptr) {
    tc_log_error("tc_plot_projection2d_destroy: invalid or stale projection "
                 ...
                 static_cast<unsigned long long>(handle.scene_id), handle.index,
                 handle.generation);
    return false;
  }
  *slot = ProjectionSlot{};
...
bool tc_plot_projection2d_matches_scene(tc_plot_projection_handle2d handle,
                                        tc_visual_scene_handle scene) {
  const ProjectionSlot *slot = live_slot(handle);
  return slot != nullptr && tc_visual_scene_is_valid(scene) &&
         tc_visual_scene_id(scene) == slot->owner_scene_id &&
...
bool tc_plot_projection2d_update(tc_plot_projection_handle2d handle,
                                 const tc_plot_projection_desc2d *desc) {
  ProjectionSlot *slot =
      checked_live_slot(handle, "tc_plot_projection2d_update");
  if (slot == nullptr)
    return false;
  if (!tc_plot_projection_desc2d_is_valid(desc)) {
...
bool tc_plot_projection2d_snapshot(tc_plot_projection_handle2d handle,
                                   tc_plot_projection_state2d *out_state) {
  if (out_state == nullptr) {
    tc_log_error("tc_plot_projection2d_snapshot: output is required");
    return false;
  }
  const ProjectionSlot *slot =
      checked_live_slot(handle, "tc_plot_projection2d_snapshot");
  if (slot == nullptr)
    return false;
...
bool tc_plot_projection_state2d_data_to_visual(
    const tc_plot_projection_state2d *state, tc_plot_point2d data,
    tc_plot_visual_point2d *out_visual) {
  if (state == nullptr || out_visual == nullptr ||
      !tc_plot_projection_desc2d_is_valid(&state->projection) ||
  ...
  const double sx =
      (data.x - value.range.x_min) / (value.range.x_max - value.range.x_min);
  const double sy =
      (data.y - value.range.y_min) / (value.range.y_max - value.range.y_min);
  out_visual->x =
      value.plot_area.x + static_cast<float>(sx) * value.plot_area.width;
  out_visual->y = value.plot_area.y +
                  (1.0f - static_cast<float>(sy)) * value.plot_area.height;
  return true;
}

bool tc_plot_projection_state2d_visual_to_data(
    const tc_plot_projection_state2d *state, tc_plot_visual_point2d visual,
    tc_plot_point2d *out_data) {
  if (state == nullptr || out_data == nullptr ||
      !tc_plot_projection_desc2d_is_valid(&state->projection) ||
  ...
  const double sy =
      1.0 - (visual.y - value.plot_area.y) / value.plot_area.height;
  out_data->x =
      value.range.x_min + sx * (value.range.x_max - value.range.x_min);
  out_data->y =
      value.range.y_min + sy * (value.range.y_max - value.range.y_min);
  return true;
}
...
bool PlotProjection2D::matches_scene(
    const termin::visual::TcVisualScene &scene) const {
...
termin::visual::TcVisualScene PlotProjection2D::owner_scene() const {
  return termin::visual::TcVisualScene{
      tc_plot_projection2d_owner_scene(handle_)};
}
...
std::optional<PlotProjection2D>
create_plot_projection2d(const termin::visual::TcVisualScene &owner_scene,
                         const PlotFrame2D &frame) {
  const tc_plot_projection_desc2d desc = to_c(frame);
  const PlotProjectionHandle2D handle =
      tc_plot_projection2d_create(owner_scene.handle(), &desc);
