...
namespace tcplot {
    ...
    void PlotView3D::ensure_offscreen_(int w, int h) {
        if (offscreen_w_ == w && offscreen_h_ == h && offscreen_color_.id != 0 && offscreen_depth_.id != 0) {
            return;
        }
        // Drop prior attachments — device.destroy is safe on zero ids.
        if (offscreen_color_.id != 0)
            device_->destroy(offscreen_color_);
        if (offscreen_depth_.id != 0)
            device_->destroy(offscreen_depth_);
    ...
    void PlotView3D::plot(SeriesData3DView series, LinePlotOptions options) {
        ...
        engine_->plot(plot3d_copy_array(series.x, series.count),
    ...
    void PlotView3D::scatter(SeriesData3DView series, ScatterPlotOptions options) {
        ...
        engine_->scatter(plot3d_copy_array(series.x, series.count),
                         plot3d_copy_array(series.y, series.count),
                         plot3d_copy_array(series.z, series.count),
                         std::move(options));
    }
    ...
    void PlotView3D::surface_colormap(SurfaceDataView surface, SurfacePlotOptions options) {
        ...
        engine_->surface(plot3d_copy_array(surface.x, n),
    ...
    void PlotView3D::set_axis_labels(const char* x_label, const char* y_label, const char* z_label) {
        set_x_label(x_label);
        set_y_label(y_label);
        set_z_label(z_label);
    }
    ...
    void PlotView3D::toggle_wireframe() {
        engine_->toggle_wireframe();
    }
    void PlotView3D::toggle_marker_mode() {
        engine_->toggle_marker_mode();
    }
    void PlotView3D::set_z_scale(float s) {
        engine_->z_scale = s;
    }
    float PlotView3D::get_z_scale() const {
        return engine_->z_scale;
    }
    ...
    float PlotView3D::get_x_scale() const {
        return engine_->x_scale;
    }
    float PlotView3D::get_y_scale() const {
        return engine_->y_scale;
    }
    ...
    OrbitCamera& PlotView3D::camera() {
        return engine_->camera;
    }
    ...
    bool PlotView3D::pick(float mx, float my, double* out_x, double* out_y, double* out_z, double* out_screen_dist_px) {
        ...
        if (!r.has_value()) {
            ...
            if (out_z)
                *out_z = 0;
            if (out_screen_dist_px)
                *out_screen_dist_px = 0;
            return false;
        }
        if (out_x)
            *out_x = r->x;
        ...
        if (out_z)
            *out_z = r->z;
        if (out_screen_dist_px)
            *out_screen_dist_px = r->screen_dist_px;
        return true;
    }
