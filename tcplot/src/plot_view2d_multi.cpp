...
namespace tcplot {
    ...
    PlotView2DMulti::PlotView2DMulti(GpuHost& host, int panel_count)
        : PlotView2DMulti(host.device(), host.cache(), host.ctx(), host.font(), panel_count) {}
    ...
    const PlotAnnotationLayer2D* PlotView2DMulti::annotations(int panel_idx) const {
        if (panel_idx < 0 || panel_idx >= panel_count())
            return nullptr;
        return &panels_[panel_idx]->annotations();
    }
    ...
    void PlotView2DMulti::set_panel_count(int n) {
        ...
        if ((int)panels_.size() > n) {
            ...
            for (size_t i = static_cast<size_t>(n); i < panels_.size(); ++i) {
                if (panels_[i])
                    panels_[i]->release_gpu_resources();
            }
    ...
    int PlotView2DMulti::add_line(int panel_idx, SeriesData2DView series, LinePlotOptions options) {
    ...
    int PlotView2DMulti::add_line_colormap(int panel_idx,
        ...
        eng.plot_colormap(to_vec(series.x, series.count),
    ...
    int PlotView2DMulti::add_scatter(int panel_idx, SeriesData2DView series, ScatterPlotOptions options) {
        if (panel_idx < 0 || panel_idx >= (int)panels_.size())
            return -1;
        PlotEngine2D& eng = *panels_[panel_idx];
    ...
    void PlotView2DMulti::append_to_line(int panel_idx, int series_idx, const double* x, const double* y, size_t n) {
        if (panel_idx < 0 || panel_idx >= (int)panels_.size())
            return;
        if (n == 0 || !x || !y)
            return;
    ...
    void PlotView2DMulti::set_autoscroll(bool on, double window_size) {
        autoscroll_ = on;
        if (window_size > 0)
            autoscroll_window_ = window_size;
    }
    ...
    void PlotView2DMulti::set_panel_view_y(int panel_idx, double y_min, double y_max) {
        if (panel_idx < 0 || panel_idx >= (int)panels_.size())
            return;
    ...
    void PlotView2DMulti::set_line_color(int panel_idx, int series_idx, float r, float g, float b, float a) {
        ...
        if (series_idx < 0)
            return;
        panels_[panel_idx]->set_line_color(static_cast<size_t>(series_idx), Color4{r, g, b, a});
    }
    void PlotView2DMulti::set_scatter_color(int panel_idx, int series_idx, float r, float g, float b, float a) {
        ...
        if (series_idx < 0)
            return;
        panels_[panel_idx]->set_scatter_color(static_cast<size_t>(series_idx), Color4{r, g, b, a});
    }
    void PlotView2DMulti::set_line_style(int panel_idx, int series_idx, LineStyle style, float dash_px, float gap_px) {
        ...
        if (series_idx < 0)
            return;
        panels_[panel_idx]->set_line_style(static_cast<size_t>(series_idx), style, dash_px, gap_px);
    }
    void PlotView2DMulti::set_line_colormap_reversed(int panel_idx, int series_idx, bool reversed) {
        if (panel_idx < 0 || panel_idx >= (int)panels_.size())
            return;
        if (series_idx < 0)
            return;
        panels_[panel_idx]->set_line_colormap_reversed(static_cast<size_t>(series_idx), reversed);
    }
    ...
    void PlotView2DMulti::ensure_offscreen_(int w, int h) {
        if (offscreen_color_.id != 0 && offscreen_w_ == w && offscreen_h_ == h)
            return;
    ...
    void PlotView2DMulti::layout_panels_(int w, int h) {
        ...
        if (panel_height_ > 0.0f) {
            ...
            for (size_t i = 0; i < panels_.size(); ++i) {
                ...
                r.y = std::floor(y);
                r.w = fw;
                r.h = (i + 1 == panels_.size()) ? (fh - r.y) : std::floor(y + panel_h) - r.y;
                panel_rects_[i] = r;
                visible_panels_.push_back(static_cast<int>(i));
                y += panel_h;
            }
    ...
    tgfx::TextureHandle PlotView2DMulti::render_to_texture(int width, int height) {
        if (width <= 0 || height <= 0 || panels_.empty())
        ...
        ctx_->begin_pass(offscreen_color_, tgfx::TextureHandle{}, clear_col, 1.0f, /*clear_depth_enabled=*/false);
    ...
    void PlotView2DMulti::on_mouse_move(float x, float y) {
        ...
        if (std::abs(x_min - shared_x_min_) > 1e-12 || std::abs(x_max - shared_x_max_) > 1e-12) {
            shared_x_min_ = x_min;
            shared_x_max_ = x_max;
            have_shared_x_ = true;
    ...
    void PlotView2DMulti::on_mouse_up(float x, float y, int button) {
        const int target = (dragging_panel_ >= 0) ? dragging_panel_ : panel_at_(y);
        if (target >= 0) {
            panels_[target]->on_mouse_up(x, y, static_cast<tcbase::MouseButton>(button));
        }
        dragging_panel_ = -1;
    }
    ...
    bool PlotView2DMulti::on_mouse_wheel(float x, float y, float dy) {
        ...
        if (idx < 0)
            return false;
        PlotEngine2D& eng = *panels_[idx];
        const PlotInputResult2D result = eng.on_mouse_wheel_result(x, y, dy);
        if (result == PlotInputResult2D::Unhandled)
            return false;
        if (result == PlotInputResult2D::Annotation)
            return true;
    ...
    bool PlotView2DMulti::on_mouse_wheel_x(float x, float y, float dy) {
        ...
        if (idx < 0)
            return false;
        PlotEngine2D& eng = *panels_[idx];
        const PlotInputResult2D result = eng.on_mouse_wheel_x_result(x, y, dy);
        if (result == PlotInputResult2D::Unhandled)
            return false;
        if (result == PlotInputResult2D::Annotation)
            return true;
