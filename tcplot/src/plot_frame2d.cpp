...
namespace tcplot {
    ...
    PlotFrame2D::PlotFrame2D(
        ...
        : viewport_(viewport),
          ...
          pixel_scale_(std::isfinite(pixel_scale) && pixel_scale > 0.0f ? pixel_scale : 1.0f) {}
    ...
    PlotPixelPoint2D PlotFrame2D::data_to_pixel(double x, double y) const {
        ...
        const double sx = x_span != 0.0 ? (x - range_.x_min()) / x_span : 0.5;
        const double sy = y_span != 0.0 ? (y - range_.y_min()) / y_span : 0.5;
        return {
            plot_area_.x() + static_cast<float>(sx) * plot_area_.width(),
            plot_area_.y() + (1.0f - static_cast<float>(sy)) * plot_area_.height(),
        };
    }
