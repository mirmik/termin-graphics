...
void PlotEngine2D::scatter(std::vector<double> x, std::vector<double> y,
    ...
    data.add_scatter(
        ...
        options.size,
        std::move(options.label));
    scatter_renderers_.push_back(
        std::make_unique<PlotScatterSeriesGpu2D>());
    if (!view_x_min_.has_value()) fit();
}
...
void PlotEngine2D::render(tgfx::RenderContext2* ctx, tgfx::FontAtlas* font) {
    ...
    while (line_renderers_.size() < data.lines.size()) {
        line_renderers_.push_back(
            std::make_unique<PlotLineSeriesGpu2D>());
    }
    ...
    const termin::Rect2f series_clip{
        pa.x(), pa.y(), pa.width(), pa.height()};
    for (std::size_t index = 0; index < data.lines.size(); ++index) {
        const auto& series = data.lines[index];
        const Color4 color = series.color.value_or(
            styles::cycle_color(static_cast<std::uint32_t>(index)));
        ...
        if (!line_renderers_[index]->render(
                *ctx, frame, termin::Affine2f::identity(), 1.0f,
                series_clip, series.x, series.y, series.scalar, style)) {
            tc::Log::error(
                "PlotEngine2D failed to render line series %zu", index);
            return;
        }
    }
    for (std::size_t index = 0; index < data.scatters.size(); ++index) {
        const auto& series = data.scatters[index];
        const Color4 color = series.color.value_or(styles::cycle_color(
            static_cast<std::uint32_t>(data.lines.size() + index)));
        if (!scatter_renderers_[index]->render(
                *ctx, frame, termin::Affine2f::identity(), 1.0f,
                series_clip, series.x, series.y,
                {color, static_cast<float>(series.size)})) {
            tc::Log::error(
                "PlotEngine2D failed to render scatter series %zu", index);
            return;
        }
    }
