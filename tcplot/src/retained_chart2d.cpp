...
namespace {
    ...
    bool valid_rect(tc_plot_rect2d rect) {
        return std::isfinite(rect.x) && std::isfinite(rect.y) && std::isfinite(rect.width) && rect.width > 0.0f &&
               std::isfinite(rect.height) && rect.height > 0.0f;
    }

    bool valid_range(tc_plot_range2d range) {
        return std::isfinite(range.x_min) && std::isfinite(range.x_max) && std::isfinite(range.y_min) &&
               std::isfinite(range.y_max) && range.x_max > range.x_min && range.y_max > range.y_min;
    }

    bool valid_theme(const tc_chart2d_theme& theme) {
        return finite_color(theme.background_color) && finite_color(theme.plot_background_color) &&
               finite_color(theme.foreground_color) && finite_color(theme.axis_color) &&
               std::isfinite(theme.grid_style.r) && std::isfinite(theme.grid_style.g) &&
               std::isfinite(theme.grid_style.b) && std::isfinite(theme.grid_style.a) &&
               std::isfinite(theme.grid_style.width_px) && theme.grid_style.width_px > 0.0f &&
               std::isfinite(theme.axis_width_logical_px) && theme.axis_width_logical_px > 0.0f &&
               std::isfinite(theme.font_size_logical_px) && theme.font_size_logical_px > 0.0f &&
               std::isfinite(theme.title_font_size_logical_px) && theme.title_font_size_logical_px > 0.0f &&
               std::isfinite(theme.tick_length_logical_px) && theme.tick_length_logical_px >= 0.0f &&
               std::isfinite(theme.gap_logical_px) && theme.gap_logical_px >= 0.0f &&
               std::isfinite(theme.outer_padding_logical_px) && theme.outer_padding_logical_px >= 0.0f &&
               std::isfinite(theme.x_tick_spacing_logical_px) && theme.x_tick_spacing_logical_px > 0.0f &&
               std::isfinite(theme.y_tick_spacing_logical_px) && theme.y_tick_spacing_logical_px > 0.0f;
    }
    ...
    tc_visual_stroke_paint2d stroke(tc_visual_color4f color, float width) {
        return {color, width, TC_VISUAL_STROKE_JOIN_MITER, TC_VISUAL_STROKE_CAP_SQUARE, 4.0f, nullptr, 0, 0.0f};
    }
    ...
    public:
        RetainedChart2D(tcplot::GpuHost& host,
            ...
            if (!tc_visual_scene_is_valid(scene_))
                throw std::invalid_argument("RetainedChart2D requires a live scene");
            if (!valid_rect(viewport_) || !valid_range(range_) || !std::isfinite(pixel_scale_) ||
                pixel_scale_ <= 0.0f || font_uri_.empty() || !valid_theme(theme_)) {
                throw std::invalid_argument("RetainedChart2D configuration is invalid");
            }
            try {
                create_tree();
                apply_layout();
        ...
        void set_viewport(tc_plot_rect2d viewport, float pixel_scale) {
            if (!valid_rect(viewport) || !std::isfinite(pixel_scale) || pixel_scale <= 0.0f)
        ...
        void set_frame(tc_plot_rect2d viewport, float pixel_scale, tc_plot_range2d range) {
            if (!valid_rect(viewport) || !std::isfinite(pixel_scale) || pixel_scale <= 0.0f || !valid_range(range))
        ...
        void fit(double padding_fraction, bool fit_x, bool fit_y) {
            ...
            const auto fitted = tcplot::fit_optional_plot_range2d(bounds, padding_fraction);
            if (!fitted)
        ...
        bool replace_part(tc_chart2d_part part_id, tc_graphic_item_handle replacement) {
            ...
            if (same(*slot.handle, replacement))
                return true;
            const size_t index = tc_visual_scene_item_child_count(scene_, *slot.parent);
            if (!tc_visual_scene_item_set_parent(scene_, replacement, *slot.parent, index) ||
                !tc_visual_scene_item_set_z_order(scene_, replacement, slot.z_order))
                return false;
        ...
        tc_graphic_item_handle add_line(
            const double* x, const double* y, const double* scalar, size_t count, tc_plot_line_style_state2d style) {
            const tc_chart_series_handle2d series = add_named_line("", false, x, y, scalar, count, style);
            const SeriesSlot* slot = resolve_series(series);
            return slot ? slot->item : tc_graphic_item_handle_invalid();
        }

        tc_graphic_item_handle
        add_scatter(const double* x, const double* y, size_t count, tc_plot_scatter_style_state2d style) {
            const tc_chart_series_handle2d series = add_named_scatter("", false, x, y, count, style);
            const SeriesSlot* slot = resolve_series(series);
            return slot ? slot->item : tc_graphic_item_handle_invalid();
        }
        ...
        tc_chart_series_handle2d add_named_line(std::string name,
                                                bool show_in_legend,
                                                const double* x,
                                                const double* y,
                                                const double* scalar,
            ...
            tc_graphic_item_handle item =
                adopt_series(tc_plot_line_series_item2d_create(scene_, projection_, x, y, scalar, count, style));
        ...
        tc_chart_series_handle2d add_named_scatter(std::string name,
                                                   bool show_in_legend,
                                                   const double* x,
                                                   const double* y,
                                                   size_t count,
            ...
            tc_graphic_item_handle item =
                adopt_series(tc_plot_scatter_series_item2d_create(scene_, projection_, x, y, count, style));
            if (tc_graphic_item_handle_is_invalid(item))
                throw std::runtime_error("failed to create chart scatter series");
            SeriesSlot* allocated = nullptr;
            try {
                SeriesSlot& slot = allocate_series(TC_CHART_SERIES_SCATTER_2D);
                allocated = &slot;
                slot.item = item;
                slot.name = std::move(name);
                slot.show_in_legend = show_in_legend;
        ...
        bool series_snapshot(tc_chart_series_handle2d series, tc_chart_series_snapshot2d& snapshot) const {
            const SeriesSlot* slot = resolve_series(series);
            if (!slot)
                return false;
            const auto bounds = series_item_bounds(slot->item);
            bool visible = false;
            if (!tc_visual_scene_item_get_visible(scene_, slot->item, &visible))
                return false;
            snapshot = {
                slot->kind,
                slot->item,
        ...
        bool set_series_name(tc_chart_series_handle2d series, std::string name) {
        ...
        bool set_series_visible(tc_chart_series_handle2d series, bool visible) {
            SeriesSlot* slot = resolve_series(series);
            if (!slot || !tc_visual_scene_item_set_visible(scene_, slot->item, visible))
                return false;
            update_legend();
            return true;
        }

        bool set_series_legend_visible(tc_chart_series_handle2d series, bool show) {
            SeriesSlot* slot = resolve_series(series);
            if (!slot)
                return false;
        ...
        bool set_line_series_style(tc_chart_series_handle2d series, tc_plot_line_style_state2d style) {
            SeriesSlot* slot = resolve_series(series);
            if (!slot || slot->kind != TC_CHART_SERIES_LINE_2D ||
                !tc_plot_line_series_item2d_set_style(scene_, slot->item, style))
                return false;
            update_legend();
            return true;
        }

        bool set_scatter_series_style(tc_chart_series_handle2d series, tc_plot_scatter_style_state2d style) {
            SeriesSlot* slot = resolve_series(series);
            if (!slot || slot->kind != TC_CHART_SERIES_SCATTER_2D ||
                !tc_plot_scatter_series_item2d_set_style(scene_, slot->item, style))
                return false;
            update_legend();
            return true;
        }
    ...
    private:
        tcplot::PlotFrame2D frame() const {
            return {
                ...
                {plot_area_.x, plot_area_.y, plot_area_.width, plot_area_.height},
        ...
        struct SeriesSlot {
            ...
            tc_graphic_item_handle legend_group = tc_graphic_item_handle_invalid();
            tc_graphic_item_handle legend_swatch = tc_graphic_item_handle_invalid();
            tc_graphic_item_handle legend_label = tc_graphic_item_handle_invalid();
        };
        ...
        SeriesSlot* resolve_series_slot(tc_chart_series_handle2d series) {
            if (series.chart_id != chart_id_ || series.index >= series_.size())
                return nullptr;
        ...
        SeriesSlot* resolve_series(tc_chart_series_handle2d series) {
            ...
            if (!slot)
                return nullptr;
            const char* expected = slot->kind == TC_CHART_SERIES_LINE_2D      ? kLineType
                                   : slot->kind == TC_CHART_SERIES_SCATTER_2D ? kScatterType
                                                                              : nullptr;
            return expected && tc_visual_scene_item_is_type(scene_, slot->item, expected) ? slot : nullptr;
        }

        const SeriesSlot* resolve_series(tc_chart_series_handle2d series) const {
        ...
        void set_parent(tc_graphic_item_handle child, tc_graphic_item_handle parent) {
            const size_t index = tc_visual_scene_item_child_count(scene_, parent);
        ...
        tc_graphic_item_handle create_group(tc_graphic_item_handle parent) {
            tc_graphic_item_handle result = tc_visual_group_item2d_create(scene_, parent);
            require(result, "failed to create chart group");
            return result;
        }

        tc_graphic_item_handle create_rect(tc_graphic_item_handle parent, tc_visual_color4f color) {
            tc_graphic_item_handle result =
                tc_visual_rect_item2d_create(scene_, parent, visual_rect(viewport_), fill(color), nullptr);
            require(result, "failed to create chart rectangle");
            return result;
        }

        tc_graphic_item_handle create_path(tc_graphic_item_handle parent) {
            const std::vector<tc_visual_path_verb2d> verbs = {TC_VISUAL_PATH_MOVE_TO, TC_VISUAL_PATH_LINE_TO};
            const std::vector<tc_vec2f> points = {{0, 0}, {1, 0}};
            tc_visual_stroke_paint2d paint = stroke(theme_.axis_color, 1.0f);
            tc_graphic_item_handle result =
                tc_visual_path_item2d_create(scene_, parent, path_view(verbs, points), nullptr, &paint);
            require(result, "failed to create chart path");
            return result;
        }

        tc_graphic_item_handle create_text(tc_graphic_item_handle parent, tc_visual_text_anchor2d anchor) {
            ...
            tc_graphic_item_handle result = tc_visual_text_item2d_create(scene_, parent, &desc);
        ...
        void create_legend_items(SeriesSlot& slot) {
            ...
            slot.legend_label = create_text(slot.legend_group, TC_VISUAL_TEXT_ANCHOR_LEFT);
            tc_visual_scene_item_set_z_order(scene_, slot.legend_group, 1);
        }
        ...
        tc_visual_stroke_paint2d legend_stroke(const SeriesSlot& slot) const {
            ...
            if (slot.kind == TC_CHART_SERIES_LINE_2D) {
                ...
                tc_plot_line_style_state2d style{};
                if (tc_plot_line_series_item2d_snapshot(scene_, slot.item, &snapshot, &style)) {
                ...
                tc_plot_series_snapshot2d snapshot{};
                tc_plot_scatter_style_state2d style{};
                if (tc_plot_scatter_series_item2d_snapshot(scene_, slot.item, &snapshot, &style)) {
                    color = {style.color.r, style.color.g, style.color.b, style.color.a};
        ...
        void update_legend() {
            ...
            const auto metrics = tcplot::measure_plot_text2d(host_->font(), "Mg", theme_.font_size_logical_px, scale);
            if (!metrics)
            ...
            for (SeriesSlot& slot : series_) {
                const bool participates = slot.alive && slot.show_in_legend && !slot.name.empty();
                if (!tc_graphic_item_handle_is_invalid(slot.legend_group) &&
                    !tc_visual_scene_item_set_visible(scene_, slot.legend_group, participates))
                    throw std::runtime_error("failed to update legend entry visibility");
                if (!participates)
                    continue;
                const auto measured =
                    tcplot::measure_plot_text2d(host_->font(), slot.name, theme_.font_size_logical_px, scale);
                if (!measured)
            ...
            if (!tc_visual_scene_item_set_visible(scene_, legend_root_, visible) ||
                !tc_visual_scene_item_set_visible(scene_, legend_background_, visible))
                throw std::runtime_error("failed to update legend visibility");
            if (!visible)
                return;

            const float swatch_width = 24.0f * scale;
            const float row_height = std::max(metrics->line_height, 12.0f * scale) + gap;
            const float width = padding * 2.0f + swatch_width + gap + widest_label;
            const float height = padding * 2.0f + row_height * visible_count - gap;
            ...
            for (SeriesSlot& slot : series_) {
                if (!slot.alive || !slot.show_in_legend || slot.name.empty())
                    continue;
                const float center_y = top + padding + row * row_height + metrics->line_height * 0.5f;
                const std::vector<tc_visual_path_verb2d> verbs = {TC_VISUAL_PATH_MOVE_TO, TC_VISUAL_PATH_LINE_TO};
                const std::vector<tc_vec2f> points = {{left + padding, center_y},
                                                      {left + padding + swatch_width, center_y}};
                tc_visual_stroke_paint2d paint = legend_stroke(slot);
                if (!tc_visual_path_item2d_set(scene_, slot.legend_swatch, path_view(verbs, points), nullptr, &paint))
                    throw std::runtime_error("failed to update legend swatch");
                set_text(slot.legend_label,
                         slot.name,
                         {left + padding + swatch_width + gap, center_y + metrics->ascent * 0.5f},
                         font_size,
        ...
        void create_tree() {
            ...
            plot_background_ = create_rect(plot_area_root_, theme_.plot_background_color);

            const tc_plot_projection_desc2d provisional = {viewport_, viewport_, range_, viewport_, pixel_scale_};
            ...
            grid_ = tc_plot_grid_item2d_create(scene_, projection_, nullptr, 0, nullptr, 0, theme_.grid_style);
            ...
            title_item_ = create_text(chrome_root_, TC_VISUAL_TEXT_ANCHOR_CENTER);
            x_axis_label_item_ = create_text(chrome_root_, TC_VISUAL_TEXT_ANCHOR_CENTER);
            y_axis_label_item_ = create_text(chrome_root_, TC_VISUAL_TEXT_ANCHOR_LEFT);
            legend_root_ = create_group(root_);
            legend_background_ = create_rect(legend_root_, theme_.plot_background_color);
        ...
        void apply_layout() {
            ...
            const float title_font_px = theme_.title_font_size_logical_px * scale;
            const float provisional_width = std::max(1.0f, viewport_.width - padding * 2.0f);
            const float provisional_height = std::max(1.0f, viewport_.height - padding * 2.0f);

            const tcplot::PlotFrame2D provisional(
                {viewport_.x, viewport_.y, viewport_.width, viewport_.height},
                {viewport_.x + padding, viewport_.y + padding, provisional_width, provisional_height},
                {range_.x_min, range_.x_max, range_.y_min, range_.y_max},
                {viewport_.x, viewport_.y, viewport_.width, viewport_.height},
                scale);
            const auto ticks = tcplot::make_plot_ticks2d(
                provisional, theme_.x_tick_spacing_logical_px, theme_.y_tick_spacing_logical_px);
            const auto tick_metrics =
                tcplot::measure_plot_text2d(host_->font(), "Mg", theme_.font_size_logical_px, scale);
            ...
            for (const std::string& label : ticks->y.labels) {
                const auto measured =
                    tcplot::measure_plot_text2d(host_->font(), label, theme_.font_size_logical_px, scale);
                if (!measured)
            ...
            if (!title_.empty()) {
                const auto measured =
                    tcplot::measure_plot_text2d(host_->font(), title_, theme_.title_font_size_logical_px, scale);
                if (!measured)
                    throw std::runtime_error("failed to measure chart title");
                title_height = measured->line_height;
            }
            const float x_label_height = x_label_.empty() ? 0.0f : tick_metrics->line_height + gap;
            float y_label_width = 0.0f;
            if (!y_label_.empty()) {
                const auto measured =
                    tcplot::measure_plot_text2d(host_->font(), y_label_, theme_.font_size_logical_px, scale);
                if (!measured)
                    throw std::runtime_error("failed to measure Y axis label");
                y_label_width = measured->width + gap;
            }

            const float left = viewport_.x + padding + y_label_width + widest_y + tick_length + gap * 2.0f;
            const float top = viewport_.y + padding + title_height + (title_height > 0.0f ? gap : 0.0f);
            const float right = padding;
            const float bottom = padding + tick_length + gap + tick_metrics->line_height + x_label_height;
            plot_area_ = {left,
                          top,
                          std::max(1.0f, viewport_.x + viewport_.width - left - right),
                          std::max(1.0f, viewport_.y + viewport_.height - top - bottom)};

            const tc_plot_projection_desc2d projection_desc = {viewport_, plot_area_, range_, plot_area_, scale};
            if (!tc_plot_projection2d_update(projection_, &projection_desc))
                throw std::runtime_error("failed to update chart projection");
            if (!tc_visual_scene_item_set_clip_rect(scene_, root_, visual_rect(viewport_)) ||
                !tc_visual_scene_item_set_clip_rect(scene_, plot_area_root_, visual_rect(plot_area_)))
                throw std::runtime_error("failed to update chart clipping");
            set_rect(background_, viewport_, theme_.background_color);
            set_rect(plot_background_, plot_area_, theme_.plot_background_color);

            const tcplot::PlotFrame2D exact_frame({viewport_.x, viewport_.y, viewport_.width, viewport_.height},
                                                  {plot_area_.x, plot_area_.y, plot_area_.width, plot_area_.height},
                                                  {range_.x_min, range_.x_max, range_.y_min, range_.y_max},
                                                  {plot_area_.x, plot_area_.y, plot_area_.width, plot_area_.height},
                                                  scale);
            const auto exact_ticks = tcplot::make_plot_ticks2d(
                exact_frame, theme_.x_tick_spacing_logical_px, theme_.y_tick_spacing_logical_px);
            if (!exact_ticks)
                throw std::runtime_error("failed to generate exact chart ticks");
            y_ticks_ = exact_ticks->y;
            if (!tc_graphic_item_handle_is_invalid(grid_)) {
                if (!tc_plot_grid_item2d_set_projection(scene_, grid_, projection_) ||
                    !tc_plot_grid_item2d_set_style(scene_, grid_, theme_.grid_style) ||
                    !tc_plot_grid_item2d_set_ticks(scene_,
                                                   grid_,
            ...
            set_text(title_item_,
                     title_,
                     {viewport_.x + viewport_.width * 0.5f, viewport_.y + padding + title_font_px},
                     title_font_px,
                     TC_VISUAL_TEXT_ANCHOR_CENTER);
            set_text(x_axis_label_item_,
                     x_label_,
                     {plot_area_.x + plot_area_.width * 0.5f, viewport_.y + viewport_.height - padding},
                     tick_font_px,
                     TC_VISUAL_TEXT_ANCHOR_CENTER);
            set_text(y_axis_label_item_,
                     y_label_,
                     {viewport_.x + padding, plot_area_.y + plot_area_.height * 0.5f},
                     tick_font_px,
                     TC_VISUAL_TEXT_ANCHOR_LEFT);
            update_legend();
            ++layout_revision_;
        ...
        void apply_x_range() {
            ...
            const tc_plot_projection_desc2d projection_desc = {viewport_, plot_area_, range_, plot_area_, pixel_scale_};
            if (!tc_plot_projection2d_update(projection_, &projection_desc))
                throw std::runtime_error("failed to update chart X projection");

            const tcplot::PlotFrame2D frame({viewport_.x, viewport_.y, viewport_.width, viewport_.height},
                                            {plot_area_.x, plot_area_.y, plot_area_.width, plot_area_.height},
                                            {range_.x_min, range_.x_max, range_.y_min, range_.y_max},
                                            {plot_area_.x, plot_area_.y, plot_area_.width, plot_area_.height},
                                            pixel_scale_);
            const auto x_ticks = tcplot::make_plot_axis_ticks2d(
                range_.x_min, range_.x_max, plot_area_.width, theme_.x_tick_spacing_logical_px, pixel_scale_);
            if (!x_ticks)
                throw std::runtime_error("failed to generate chart X ticks");

            if (!tc_graphic_item_handle_is_invalid(grid_) &&
                (!tc_plot_grid_item2d_set_projection(scene_, grid_, projection_) ||
                 !tc_plot_grid_item2d_set_ticks(scene_,
                                                grid_,
                                                x_ticks->values.data(),
                                                x_ticks->values.size(),
            ...
            const float tick_length = theme_.tick_length_logical_px * pixel_scale_;
            set_x_axis(*x_ticks, tick_length, frame);
            update_x_tick_labels(*x_ticks, tick_metrics_.ascent, frame);
            ++layout_revision_;
        }

        void set_rect(tc_graphic_item_handle item, tc_plot_rect2d rect, tc_visual_color4f color) {
            if (tc_graphic_item_handle_is_invalid(item))
                return;
            if (!tc_visual_rect_item2d_set(scene_, item, visual_rect(rect), fill(color), nullptr))
        ...
        void set_axes(const tcplot::PlotTicks2D& ticks, float tick_length, const tcplot::PlotFrame2D& frame) {
            set_x_axis(ticks.x, tick_length, frame);
            set_y_axis(ticks.y, tick_length, frame);
        }

        void set_x_axis(const tcplot::PlotAxisTicks2D& ticks, float tick_length, const tcplot::PlotFrame2D& frame) {
            tc_visual_stroke_paint2d paint = stroke(theme_.axis_color, theme_.axis_width_logical_px * pixel_scale_);
            if (!tc_graphic_item_handle_is_invalid(x_axis_)) {
                ...
                for (double value : ticks.values) {
                    ...
                    append_path_line(verbs,
                                     ...
                                     plot_area_.y + plot_area_.height,
                                     p.x,
                                     plot_area_.y + plot_area_.height + tick_length);
                }
                if (!tc_visual_path_item2d_set(scene_, x_axis_, path_view(verbs, points), nullptr, &paint))
                    throw std::runtime_error("failed to update X axis");
            }
        }

        void set_y_axis(const tcplot::PlotAxisTicks2D& ticks, float tick_length, const tcplot::PlotFrame2D& frame) {
            tc_visual_stroke_paint2d paint = stroke(theme_.axis_color, theme_.axis_width_logical_px * pixel_scale_);
            if (!tc_graphic_item_handle_is_invalid(y_axis_)) {
                std::vector<tc_visual_path_verb2d> verbs;
                std::vector<tc_vec2f> points;
                append_path_line(
                    verbs, points, plot_area_.x, plot_area_.y, plot_area_.x, plot_area_.y + plot_area_.height);
                for (double value : ticks.values) {
                    const auto p = frame.data_to_pixel(range_.x_min, value);
                    append_path_line(verbs, points, plot_area_.x - tick_length, p.y, plot_area_.x, p.y);
                }
                if (!tc_visual_path_item2d_set(scene_, y_axis_, path_view(verbs, points), nullptr, &paint))
        ...
        void ensure_tick_pool(std::vector<tc_graphic_item_handle>& pool,
            ...
            for (size_t index = 0; index < pool.size(); ++index)
                tc_visual_scene_item_set_visible(scene_, pool[index], index < count);
        }
        ...
        void update_x_tick_labels(const tcplot::PlotAxisTicks2D& ticks,
                                  float text_ascent,
                                  const tcplot::PlotFrame2D& frame) {
            ensure_tick_pool(x_tick_labels_, ticks.values.size(), x_tick_labels_root_, TC_VISUAL_TEXT_ANCHOR_CENTER);
            const float gap = theme_.gap_logical_px * pixel_scale_;
            const float tick_length = theme_.tick_length_logical_px * pixel_scale_;
            const float font_size = theme_.font_size_logical_px * pixel_scale_;
            for (size_t index = 0; index < ticks.values.size(); ++index) {
                const auto p = frame.data_to_pixel(ticks.values[index], range_.y_min);
                set_text(x_tick_labels_[index],
                         ticks.labels[index],
                         {p.x, plot_area_.y + plot_area_.height + tick_length + gap + text_ascent},
                         font_size,
                         TC_VISUAL_TEXT_ANCHOR_CENTER);
            }
        }
        ...
        void update_y_tick_labels(const tcplot::PlotAxisTicks2D& ticks,
                                  float text_ascent,
                                  const tcplot::PlotFrame2D& frame) {
            ensure_tick_pool(y_tick_labels_, ticks.values.size(), y_tick_labels_root_, TC_VISUAL_TEXT_ANCHOR_RIGHT);
            const float gap = theme_.gap_logical_px * pixel_scale_;
            const float tick_length = theme_.tick_length_logical_px * pixel_scale_;
            const float font_size = theme_.font_size_logical_px * pixel_scale_;
            for (size_t index = 0; index < ticks.values.size(); ++index) {
                const auto p = frame.data_to_pixel(range_.x_min, ticks.values[index]);
                set_text(y_tick_labels_[index],
                         ticks.labels[index],
                         {plot_area_.x - tick_length - gap, p.y + text_ascent * 0.5f},
                         font_size,
        ...
        tc_graphic_item_handle adopt_series(tc_graphic_item_handle item) {
            if (tc_graphic_item_handle_is_invalid(item))
                return item;
            const size_t index = tc_visual_scene_item_child_count(scene_, series_root_);
            if (!tc_visual_scene_item_set_parent(scene_, item, series_root_, index)) {
                tc_visual_scene_destroy_item(scene_, item);
                return tc_graphic_item_handle_invalid();
            }
            return item;
        ...
        std::optional<tcplot::PlotRange2D> series_item_bounds(tc_graphic_item_handle handle) const {
            ...
            if (const auto* line = tcplot::resolve_plot_line_series_item2d(scene, handle)) {
                x = line->x();
                y = line->y();
            } else if (const auto* scatter = tcplot::resolve_plot_scatter_series_item2d(scene, handle)) {
                x = scatter->x();
                y = scatter->y();
            } else
                return std::nullopt;
        ...
        std::optional<tcplot::PlotRange2D> series_bounds() const {
            ...
            for (const SeriesSlot& slot : series_) {
                ...
                if (!slot.alive || item == nullptr || !scene.effective_visible(*item))
                    continue;
                const auto bounds = series_item_bounds(slot.item);
                if (!bounds)
                    continue;
            ...
            return found ? std::optional<tcplot::PlotRange2D>{tcplot::PlotRange2D{x_min, x_max, y_min, y_max}}
                         : std::nullopt;
        }
        ...
        tc_plot_projection_handle2d projection_ = tc_plot_projection_handle2d_invalid();
        ...
        tc_graphic_item_handle plot_area_root_ = tc_graphic_item_handle_invalid();
        tc_graphic_item_handle plot_background_ = tc_graphic_item_handle_invalid();
        ...
        tc_graphic_item_handle annotations_root_ = tc_graphic_item_handle_invalid();
        ...
        tc_graphic_item_handle x_tick_labels_root_ = tc_graphic_item_handle_invalid();
        tc_graphic_item_handle y_tick_labels_root_ = tc_graphic_item_handle_invalid();
        tc_graphic_item_handle title_item_ = tc_graphic_item_handle_invalid();
        tc_graphic_item_handle x_axis_label_item_ = tc_graphic_item_handle_invalid();
        tc_graphic_item_handle y_axis_label_item_ = tc_graphic_item_handle_invalid();
        tc_graphic_item_handle legend_root_ = tc_graphic_item_handle_invalid();
        tc_graphic_item_handle legend_background_ = tc_graphic_item_handle_invalid();
    ...
    Result logged(const char* operation, Result failure, Function&& function) {
        try {
            ...
            tc::Log::error("RetainedChart2D: %s failed: %s", operation, error.what());
        } catch (...) {
            tc::Log::error("RetainedChart2D: %s failed with unknown error", operation);
        }
        return failure;
    }
...
struct tc_retained_chart2d {
    ...
    tc_retained_chart2d(tcplot::GpuHost& host,
                        ...
                        float pixel_scale,
                        tc_chart2d_theme theme)
        : value(host, scene, owns_scene, viewport, range, std::move(font_uri), pixel_scale, theme) {}
};
...
tc_retained_chart2d* tc_retained_chart2d_create(void* gpu_host,
                                                tc_plot_rect2d viewport,
                                                tc_plot_range2d range,
                                                const char* font_uri,
                                                float pixel_scale,
    ...
    if (!tc_visual_scene_is_valid(scene))
        return nullptr;
    tc_retained_chart2d* result = logged("create", static_cast<tc_retained_chart2d*>(nullptr), [&] {
        return new tc_retained_chart2d(*static_cast<tcplot::GpuHost*>(gpu_host),
                                       scene,
                                       true,
                                       viewport,
                                       range,
...
tc_retained_chart2d* tc_retained_chart2d_create_in_scene(void* gpu_host,
                                                         tc_visual_scene_handle scene,
                                                         tc_plot_rect2d viewport,
                                                         tc_plot_range2d range,
                                                         const char* font_uri,
                                                         ...
                                                         const tc_chart2d_theme* theme) {
    if (!gpu_host) {
        tc::Log::error("RetainedChart2D: create_in_scene requires a live GpuHost");
        return nullptr;
    }
    return logged("create_in_scene", static_cast<tc_retained_chart2d*>(nullptr), [&] {
        return new tc_retained_chart2d(*static_cast<tcplot::GpuHost*>(gpu_host),
                                       scene,
                                       false,
                                       viewport,
...
tc_visual_scene_handle tc_retained_chart2d_scene(const tc_retained_chart2d* chart) {
    return chart ? chart->value.scene() : tc_visual_scene_handle{};
}

tc_plot_projection_handle2d tc_retained_chart2d_projection(const tc_retained_chart2d* chart) {
    return chart ? chart->value.projection() : tc_plot_projection_handle2d_invalid();
}

tc_graphic_item_handle tc_retained_chart2d_part_handle(const tc_retained_chart2d* chart, tc_chart2d_part part) {
    return chart ? chart->value.part(part) : tc_graphic_item_handle_invalid();
}

bool tc_retained_chart2d_layout(const tc_retained_chart2d* chart, tc_chart2d_layout* out_layout) {
    if (!chart || !out_layout)
        return false;
...
bool tc_retained_chart2d_range(const tc_retained_chart2d* chart, tc_plot_range2d* out_range) {
    if (!chart || !out_range)
        return false;
...
bool tc_retained_chart2d_theme(const tc_retained_chart2d* chart, tc_chart2d_theme* out_theme) {
    if (!chart || !out_theme)
        return false;
...
bool tc_retained_chart2d_set_viewport(tc_retained_chart2d* chart, tc_plot_rect2d viewport, float pixel_scale) {
    return chart && logged("set_viewport", false, [&] {
               chart->value.set_viewport(viewport, pixel_scale);
               return true;
           });
}

bool tc_retained_chart2d_set_range(tc_retained_chart2d* chart, tc_plot_range2d range) {
    return chart && logged("set_range", false, [&] {
               chart->value.set_range(range);
               return true;
           });
}

bool tc_retained_chart2d_set_x_range(tc_retained_chart2d* chart, double x_min, double x_max) {
    return chart && logged("set_x_range", false, [&] {
...
bool tc_retained_chart2d_set_frame(tc_retained_chart2d* chart,
                                   ...
                                   float pixel_scale,
                                   tc_plot_range2d range) {
    return chart && logged("set_frame", false, [&] {
               chart->value.set_frame(viewport, pixel_scale, range);
               return true;
           });
}

bool tc_retained_chart2d_fit(tc_retained_chart2d* chart, double padding_fraction) {
    return chart && logged("fit", false, [&] {
               chart->value.fit(padding_fraction, true, true);
               return true;
           });
}

bool tc_retained_chart2d_fit_x(tc_retained_chart2d* chart, double padding_fraction) {
    return chart && logged("fit_x", false, [&] {
               chart->value.fit(padding_fraction, true, false);
               return true;
           });
}

bool tc_retained_chart2d_fit_y(tc_retained_chart2d* chart, double padding_fraction) {
    return chart && logged("fit_y", false, [&] {
               chart->value.fit(padding_fraction, false, true);
               return true;
           });
}

bool tc_retained_chart2d_pointer_down(tc_retained_chart2d* chart, float x, float y, int button) {
    return chart && logged("pointer_down", false, [&] { return chart->value.pointer_down(x, y, button); });
}

bool tc_retained_chart2d_pointer_move(tc_retained_chart2d* chart, float x, float y) {
    return chart && logged("pointer_move", false, [&] { return chart->value.pointer_move(x, y); });
}

bool tc_retained_chart2d_pointer_up(tc_retained_chart2d* chart, float, float, int button) {
    return chart && logged("pointer_up", false, [&] { return chart->value.pointer_up(button); });
}

bool tc_retained_chart2d_wheel(tc_retained_chart2d* chart, float x, float y, float steps, bool x_only) {
    return chart && logged("wheel", false, [&] { return chart->value.wheel(x, y, steps, x_only); });
}
...
bool tc_retained_chart2d_set_theme(tc_retained_chart2d* chart, const tc_chart2d_theme* theme) {
    return chart && theme && logged("set_theme", false, [&] {
               chart->value.set_theme(*theme);
               return true;
           });
}

bool tc_retained_chart2d_set_title(tc_retained_chart2d* chart, const char* utf8) {
    return chart && logged("set_title", false, [&] {
               chart->value.set_title(utf8 ? utf8 : "");
               return true;
           });
}

bool tc_retained_chart2d_set_x_axis_label(tc_retained_chart2d* chart, const char* utf8) {
    return chart && logged("set_x_axis_label", false, [&] {
               chart->value.set_x_label(utf8 ? utf8 : "");
               return true;
           });
}

bool tc_retained_chart2d_set_y_axis_label(tc_retained_chart2d* chart, const char* utf8) {
    return chart && logged("set_y_axis_label", false, [&] {
...
bool tc_retained_chart2d_replace_part(tc_retained_chart2d* chart,
    ...
    return chart && logged("replace_part", false, [&] { return chart->value.replace_part(part, replacement); });
}

bool tc_retained_chart2d_remove_part(tc_retained_chart2d* chart, tc_chart2d_part part) {
    return chart && logged("remove_part", false, [&] { return chart->value.remove_part(part); });
}

tc_graphic_item_handle tc_retained_chart2d_add_line(tc_retained_chart2d* chart,
                                                    const double* x,
                                                    const double* y,
                                                    const double* scalar,
                                                    size_t point_count,
    ...
    return chart ? logged("add_line",
                          ...
                          [&] { return chart->value.add_line(x, y, scalar, point_count, style); })
                 : tc_graphic_item_handle_invalid();
}

tc_graphic_item_handle tc_retained_chart2d_add_scatter(tc_retained_chart2d* chart,
                                                       const double* x,
                                                       const double* y,
                                                       size_t point_count,
                                                       tc_plot_scatter_style_state2d style) {
    return chart ? logged("add_scatter",
                          tc_graphic_item_handle_invalid(),
                          [&] { return chart->value.add_scatter(x, y, point_count, style); })
                 : tc_graphic_item_handle_invalid();
}

bool tc_retained_chart2d_remove_series(tc_retained_chart2d* chart, tc_graphic_item_handle series) {
    return chart && logged("remove_series", false, [&] { return chart->value.remove_series(series); });
}

tc_chart_series_handle2d tc_retained_chart2d_add_named_line(tc_retained_chart2d* chart,
    ...
    return chart ? logged("add_named_line",
                          ...
                          [&] {
                              return chart->value.add_named_line(
                                  name_utf8 ? name_utf8 : "", show_in_legend, x, y, scalar, point_count, style);
                          })
                 : invalid_series();
}

tc_chart_series_handle2d tc_retained_chart2d_add_named_scatter(tc_retained_chart2d* chart,
                                                               const char* name_utf8,
                                                               bool show_in_legend,
                                                               const double* x,
                                                               const double* y,
    ...
    return chart ? logged("add_named_scatter",
                          ...
                          [&] {
                              return chart->value.add_named_scatter(
                                  name_utf8 ? name_utf8 : "", show_in_legend, x, y, point_count, style);
                          })
                 : invalid_series();
}

bool tc_retained_chart2d_series_is_valid(const tc_retained_chart2d* chart, tc_chart_series_handle2d series) {
    return chart && chart->value.series_valid(series);
}

bool tc_retained_chart2d_series_snapshot(const tc_retained_chart2d* chart,
                                         tc_chart_series_handle2d series,
                                         tc_chart_series_snapshot2d* out_snapshot) {
    return chart && out_snapshot && chart->value.series_snapshot(series, *out_snapshot);
}

size_t tc_retained_chart2d_series_name_copy(const tc_retained_chart2d* chart,
                                            tc_chart_series_handle2d series,
                                            char* out_utf8,
                                            size_t capacity) {
    if (!chart)
    ...
    if (capacity < required) {
        tc::Log::error("RetainedChart2D: series_name_copy output is too small");
        return 0;
    }
...
bool tc_retained_chart2d_series_set_name(tc_retained_chart2d* chart,
                                         tc_chart_series_handle2d series,
                                         const char* name_utf8) {
    return chart && logged("series_set_name", false, [&] {
               return chart->value.set_series_name(series, name_utf8 ? name_utf8 : "");
           });
}

bool tc_retained_chart2d_series_set_visible(tc_retained_chart2d* chart, tc_chart_series_handle2d series, bool visible) {
    return chart &&
           logged("series_set_visible", false, [&] { return chart->value.set_series_visible(series, visible); });
}

bool tc_retained_chart2d_series_set_legend_visible(tc_retained_chart2d* chart,
                                                   tc_chart_series_handle2d series,
                                                   bool show_in_legend) {
    return chart && logged("series_set_legend_visible", false, [&] {
               return chart->value.set_series_legend_visible(series, show_in_legend);
           });
}

bool tc_retained_chart2d_line_series_set_style(tc_retained_chart2d* chart,
                                               tc_chart_series_handle2d series,
                                               tc_plot_line_style_state2d style) {
    return chart &&
           logged("line_series_set_style", false, [&] { return chart->value.set_line_series_style(series, style); });
}

bool tc_retained_chart2d_scatter_series_set_style(tc_retained_chart2d* chart,
                                                  tc_chart_series_handle2d series,
                                                  tc_plot_scatter_style_state2d style) {
    return chart && logged("scatter_series_set_style", false, [&] {
               return chart->value.set_scatter_series_style(series, style);
           });
}

bool tc_retained_chart2d_remove_semantic_series(tc_retained_chart2d* chart, tc_chart_series_handle2d series) {
    return chart &&
           logged("remove_semantic_series", false, [&] { return chart->value.remove_semantic_series(series); });
}
