...
namespace tcplot {
    namespace {
        ...
        tc_shader_handle plot3d_shader_handle() {
            ...
            if (!tc_shader_is_valid(handle)) {
                handle = tgfx::register_builtin_shader_from_catalog(kPlot3DShaderUuid);
            }
            return handle;
        }
        ...
        bool is_encoded_plot_item_kind(uint32_t kind) {
            return kind == PLOT_RENDER_ITEM_KIND_SURFACE || kind == PLOT_RENDER_ITEM_KIND_SCATTER ||
                   kind == PLOT_RENDER_ITEM_KIND_GRID || kind == PLOT_RENDER_ITEM_KIND_LINE;
        }

        bool payload_kind_matches_item(const tc_render_item& item, const PlotScene3DRenderItemPayload& payload) {
            if (!payload.item) {
                return false;
            }
            return (item.kind == PLOT_RENDER_ITEM_KIND_SURFACE && payload.item->kind == TC_PLOT_ITEM3D_SURFACE) ||
        ...
        termin::RenderItemTaskRejection plan_plot_scene3d_shader(const termin::RenderItemTaskPlanningRequest& request,
                                                                 termin::RenderItemTaskShaderPlan& out_plan,
                                                                 const char*& out_detail,
                                                                 void*) {
            const PlotScene3DRenderItemPayload* payload =
                request.item ? plot_scene3d_render_item_payload(*request.item) : nullptr;
            if (!request.item || !is_encoded_plot_item_kind(request.item->kind) || !payload ||
                !payload_kind_matches_item(*request.item, *payload)) {
                out_detail = "item has no matching immutable PlotScene3D payload";
        ...
        bool prepare_plot_scene3d_draw(tgfx::RenderContext2& context,
            ...
            if (!termin::ensure_material_pipeline_shader(
                    context, *request.device, request.shader_handle, pass_name, shader_binding)) {
                tc::Log::error("[%s] failed to prepare PlotScene3D shader", pass_name);
                return false;
            }
            ...
            if (surface_mode) {
                ...
                draw.light_strength[3] = std::clamp(payload.frame.surface_shading_strength, 0.0f, 1.0f);
            }
            ...
            out_layout.attributes[0] = {0, tgfx::VertexFormat::Float3, 0, nullptr};
            out_layout.attributes[1] = {1, tgfx::VertexFormat::Float4, 3 * sizeof(float), nullptr};
            out_layout.attributes[2] = {2, tgfx::VertexFormat::Float4, 7 * sizeof(float), nullptr};
            out_layout.attributes[3] = {3, tgfx::VertexFormat::Float4, 11 * sizeof(float), nullptr};
            out_layout.attributes[4] = {4, tgfx::VertexFormat::Float4, 15 * sizeof(float), nullptr};
            return true;
        }
        ...
        bool encode_surface(tgfx::RenderContext2& context,
                            ...
                            const termin::RenderItemDrawSubmitRequest& request,
                            void*) {
            const char* pass_name = request.debug_pass_name ? request.debug_pass_name : "PlotScene3DSurface";
            if (item.kind != PLOT_RENDER_ITEM_KIND_SURFACE) {
                tc::Log::error("[%s] PlotScene3D surface encoder received item kind %u", pass_name, item.kind);
                return false;
            }
            ...
            if (!request.device || !request.draw_context || request.draw_context->viewport_width <= 0 ||
            ...
            const PlotScene3DRenderItemPayload* payload = plot_scene3d_render_item_payload(item);
            if (!payload || !payload->item || payload->item->kind != TC_PLOT_ITEM3D_SURFACE) {
            ...
            if (!prepare_plot_scene3d_draw(context, request, *payload, data, true, pass_name, layout)) {
                return false;
            }
            ...
            context.draw_transient_arrays(data.draw_vertices.data(),
                                          static_cast<uint32_t>(data.draw_vertices.size() * sizeof(float)),
        ...
        bool encode_scatter(tgfx::RenderContext2& context,
                            ...
                            const termin::RenderItemDrawSubmitRequest& request,
                            void*) {
            const char* pass_name = request.debug_pass_name ? request.debug_pass_name : "PlotScene3DScatter";
            if (item.kind != PLOT_RENDER_ITEM_KIND_SCATTER) {
                tc::Log::error("[%s] PlotScene3D scatter encoder received item kind %u", pass_name, item.kind);
                return false;
            }
            if (request.phase != TC_PHASE_OPAQUE || request.material_phase) {
            ...
            const PlotScene3DRenderItemPayload* payload = plot_scene3d_render_item_payload(item);
            if (!payload || !payload->item || payload->item->kind != TC_PLOT_ITEM3D_SCATTER) {
            ...
            tgfx::VertexLayoutDesc layout{};
            if (!prepare_plot_scene3d_draw(context, request, *payload, data, false, pass_name, layout)) {
                return false;
            }
        ...
        bool encode_grid(tgfx::RenderContext2& context,
                         ...
                         const termin::RenderItemDrawSubmitRequest& request,
                         void*) {
            const char* pass_name = request.debug_pass_name ? request.debug_pass_name : "PlotScene3DGrid";
            if (item.kind != PLOT_RENDER_ITEM_KIND_GRID) {
                tc::Log::error("[%s] PlotScene3D grid encoder received item kind %u", pass_name, item.kind);
                return false;
            }
            if (request.phase != TC_PHASE_OPAQUE || request.material_phase) {
            ...
            const PlotScene3DRenderItemPayload* payload = plot_scene3d_render_item_payload(item);
            if (!payload || !payload->item || payload->item->kind != TC_PLOT_ITEM3D_GRID) {
        ...
        bool encode_line(tgfx::RenderContext2& context,
                         ...
                         const termin::RenderItemDrawSubmitRequest& request,
                         void*) {
            const char* pass_name = request.debug_pass_name ? request.debug_pass_name : "PlotScene3DLine";
            if (item.kind != PLOT_RENDER_ITEM_KIND_LINE || request.phase != TC_PHASE_OPAQUE || request.material_phase ||
                !request.device || !request.draw_context || request.draw_context->viewport_width <= 0 ||
                request.draw_context->viewport_height <= 0) {
                tc::Log::error("[%s] invalid retained line submission", pass_name);
                return false;
            }
            const PlotScene3DRenderItemPayload* payload = plot_scene3d_render_item_payload(item);
            if (!payload || !payload->item || payload->item->kind != TC_PLOT_ITEM3D_LINE) {
                tc::Log::error("[%s] malformed retained line payload", pass_name);
                return false;
            }
            const PlotScene3DItemRenderData& data = *payload->item;
            if (data.draw_vertex_count == 0 ||
                data.draw_vertices.size() != static_cast<size_t>(data.draw_vertex_count) * kPlot3DFloatsPerVertex ||
                data.draw_vertices.size() > std::numeric_limits<uint32_t>::max() / sizeof(float)) {
                tc::Log::error("[%s] invalid retained line draw stream", pass_name);
                return false;
            }
            tgfx::VertexLayoutDesc layout{};
            if (!prepare_plot_scene3d_draw(context, request, *payload, data, false, pass_name, layout)) {
    ...
    void build_plot_scene3d_surface_draw_stream(PlotScene3DItemRenderData& data) {
        ...
        if (data.kind != TC_PLOT_ITEM3D_SURFACE || data.rows < 2 || data.columns < 2) {
            return;
        }
        const size_t point_count = static_cast<size_t>(data.rows) * data.columns;
        if (data.x.size() != point_count || data.y.size() != point_count || data.z.size() != point_count) {
            tc::Log::error("[PlotScene3DSurface] cannot build draw stream from "
                           "mismatched data arrays");
            return;
        }

        const size_t vertices_per_cell = data.surface_style.wireframe != 0 ? 12u : 6u;
        const size_t cell_count = static_cast<size_t>(data.rows - 1) * (data.columns - 1);
        const size_t max_float_count = std::numeric_limits<uint32_t>::max() / sizeof(float);
        if (cell_count > max_float_count / (vertices_per_cell * kPlot3DFloatsPerVertex)) {
    ...
    void build_plot_scene3d_scatter_draw_stream(PlotScene3DItemRenderData& data) {
        data.draw_vertices.clear();
        data.draw_vertex_count = 0;
        if (data.kind != TC_PLOT_ITEM3D_SCATTER || data.x.empty() || data.x.size() != data.y.size() ||
            data.x.size() != data.z.size()) {
        ...
        const size_t max_float_count = std::numeric_limits<uint32_t>::max() / sizeof(float);
        if (data.x.size() > max_float_count / (kVerticesPerPoint * kPlot3DFloatsPerVertex)) {
        ...
        for (size_t index = 1; index < data.x.size(); ++index) {
            const double point[3] = {data.x[index], data.y[index], data.z[index]};
        ...
        const float cross_size = static_cast<float>(std::sqrt(dx * dx + dy * dy + dz * dz) * 0.008 *
                                                    (std::max(data.scatter_style.size, 0.1f) / 4.0f));

        data.draw_vertices.reserve(data.x.size() * kVerticesPerPoint * kPlot3DFloatsPerVertex);
        for (size_t index = 0; index < data.x.size(); ++index) {
            ...
            append_scatter_vertex(data.draw_vertices, x - cross_size, y, z, data.scatter_style);
            append_scatter_vertex(data.draw_vertices, x + cross_size, y, z, data.scatter_style);
            append_scatter_vertex(data.draw_vertices, x, y - cross_size, z, data.scatter_style);
            append_scatter_vertex(data.draw_vertices, x, y + cross_size, z, data.scatter_style);
            append_scatter_vertex(data.draw_vertices, x, y, z - cross_size, data.scatter_style);
            append_scatter_vertex(data.draw_vertices, x, y, z + cross_size, data.scatter_style);
        }
        data.draw_vertex_count = static_cast<uint32_t>(data.draw_vertices.size() / kPlot3DFloatsPerVertex);
    }
    ...
    void build_plot_scene3d_line_draw_stream(PlotScene3DItemRenderData& data) {
        data.draw_vertices.clear();
        data.draw_vertex_count = 0;
        if (data.kind != TC_PLOT_ITEM3D_LINE || data.x.size() < 2 || data.x.size() != data.y.size() ||
            data.x.size() != data.z.size()) {
        ...
        const size_t max_float_count = std::numeric_limits<uint32_t>::max() / sizeof(float);
        if (segment_count > max_float_count / (kVerticesPerSegment * kPlot3DFloatsPerVertex)) {
            tc::Log::error("[PlotScene3DLine] draw stream exceeds the "
                           "transient upload ABI");
            return;
        }
        const tc_line_item3d_style& style = data.line_style;
        data.draw_vertices.reserve(segment_count * kVerticesPerSegment * kPlot3DFloatsPerVertex);
        for (size_t index = 0; index < segment_count; ++index) {
            for (size_t endpoint = index; endpoint <= index + 1; ++endpoint) {
                append_line_vertex(data.draw_vertices,
    ...
    void build_plot_scene3d_grid_draw_stream(PlotScene3DItemRenderData& data,
        ...
        data.draw_vertices.reserve((kAxisCount * kMaximumTickCount + kAxisCount) * kVerticesPerLine *
        ...
        for (size_t axis = 0; axis < kAxisCount; ++axis) {
            ...
            for (double tick : axes::nice_ticks(frame.bounds_min[axis], frame.bounds_max[axis], 8)) {
    ...
    void ensure_plot_scene3d_render_item_encoders_registered() {
        ...
        std::call_once(once, [] {
            ...
            descriptor.plan_task_shader = plan_plot_scene3d_shader;
            descriptor.debug_name = "PlotScene3DSurface";
            descriptor.capabilities = capabilities;
            if (!termin::register_render_item_draw_encoder(PLOT_RENDER_ITEM_KIND_SURFACE, descriptor)) {
            ...
            if (!termin::register_render_item_draw_encoder(PLOT_RENDER_ITEM_KIND_SCATTER, descriptor)) {
            ...
            if (!termin::register_render_item_draw_encoder(PLOT_RENDER_ITEM_KIND_GRID, descriptor)) {
            ...
            if (!termin::register_render_item_draw_encoder(PLOT_RENDER_ITEM_KIND_LINE, descriptor)) {
