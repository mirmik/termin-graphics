...
namespace tcplot {
    ...
    std::optional<PlotRange2D> fit_plot_range2d(PlotRange2D data_bounds, double padding_fraction) {
        if (!finite_range(data_bounds) || data_bounds.x_max() < data_bounds.x_min() ||
            data_bounds.y_max() < data_bounds.y_min() || !std::isfinite(padding_fraction) || padding_fraction < 0.0) {
    ...
    std::optional<PlotRange2D> fit_optional_plot_range2d(std::optional<PlotRange2D> data_bounds,
                                                         double padding_fraction) {
        return fit_plot_range2d(data_bounds.value_or(PlotRange2D{0.0, 1.0, 0.0, 1.0}), padding_fraction);
    }

    std::optional<PlotAxisTicks2D> make_plot_axis_ticks2d(double minimum,
                                                          double maximum,
                                                          float extent_px,
                                                          float spacing_logical_px,
                                                          float pixel_scale,
                                                          int minimum_ticks) {
        if (!std::isfinite(minimum) || !std::isfinite(maximum) || maximum <= minimum || !std::isfinite(extent_px) ||
            extent_px <= 0.0f || !std::isfinite(spacing_logical_px) || spacing_logical_px <= 0.0f ||
        ...
        const int maximum_tick_count = std::max(static_cast<int>(extent_px / spacing), minimum_ticks);
    ...
    std::optional<PlotTicks2D> make_plot_ticks2d(const PlotFrame2D& frame,
        ...
        if (!finite_range(range) || range.x_span() <= 0.0 || range.y_span() <= 0.0 || !std::isfinite(area.width()) ||
            !std::isfinite(area.height()) || area.width() <= 0.0f || area.height() <= 0.0f ||
            !std::isfinite(frame.pixel_scale()) || frame.pixel_scale() <= 0.0f ||
            !std::isfinite(x_spacing_logical_px) || !std::isfinite(y_spacing_logical_px) ||
            x_spacing_logical_px <= 0.0f || y_spacing_logical_px <= 0.0f || minimum_ticks < 1) {
            tc::Log::error("make_plot_ticks2d rejected invalid layout input");
            return std::nullopt;
        }
        const auto x = make_plot_axis_ticks2d(
            range.x_min(), range.x_max(), area.width(), x_spacing_logical_px, frame.pixel_scale(), minimum_ticks);
        const auto y = make_plot_axis_ticks2d(
            range.y_min(), range.y_max(), area.height(), y_spacing_logical_px, frame.pixel_scale(), minimum_ticks);
        if (!x || !y)
            return std::nullopt;
        return PlotTicks2D{*x, *y};
    }

    std::optional<PlotTextMetrics2D>
    measure_plot_text2d(tgfx::FontAtlas& font, std::string_view text, float font_size_logical_px, float pixel_scale) {
        if (!std::isfinite(font_size_logical_px) || font_size_logical_px <= 0.0f || !std::isfinite(pixel_scale) ||
            pixel_scale <= 0.0f) {
...
bool tc_plot_fit_range2d(tc_plot_range2d data_bounds, double padding_fraction, tc_plot_range2d* out_range) {
    if (out_range == nullptr) {
        tc::Log::error("tc_plot_fit_range2d requires output");
        return false;
    }
    const auto fitted = tcplot::fit_plot_range2d(
        {data_bounds.x_min, data_bounds.x_max, data_bounds.y_min, data_bounds.y_max}, padding_fraction);
    if (!fitted)
        return false;
    *out_range = {
        fitted->x_min(),
...
size_t tc_plot_axis_ticks2d_copy(const tc_plot_axis_ticks_desc2d* desc, double* out_values, size_t capacity) {
    if (desc == nullptr || !std::isfinite(desc->minimum) || !std::isfinite(desc->maximum) ||
        desc->maximum <= desc->minimum || !std::isfinite(desc->extent_px) || desc->extent_px <= 0.0f ||
        !std::isfinite(desc->spacing_logical_px) || desc->spacing_logical_px <= 0.0f ||
        !std::isfinite(desc->pixel_scale) || desc->pixel_scale <= 0.0f || desc->minimum_tick_count < 1) {
        tc::Log::error("tc_plot_axis_ticks2d_copy rejected invalid input");
        return 0;
    }
    const float physical_spacing = desc->spacing_logical_px * desc->pixel_scale;
    ...
    const int maximum_tick_count =
        std::max(static_cast<int>(desc->extent_px / physical_spacing), static_cast<int>(desc->minimum_tick_count));
    const std::vector<double> values = tcplot::axes::nice_ticks(desc->minimum, desc->maximum, maximum_tick_count);
