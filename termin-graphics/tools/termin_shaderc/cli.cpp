...
namespace termin_shaderc {
    ...
    namespace {
        ...
        void print_help(std::ostream& out) {
            out << "termin_shaderc - Termin shader artifact compiler\n"
                << "\n"
                << "Usage:\n"
                << "  termin_shaderc --help\n"
    ...
    ParsedCommandLine parse_command_line(int argc, char** argv) {
        ...
        if (options.language.empty() || options.target.empty() || options.stage.empty() || options.input.empty() ||
            options.output.empty()) {
            usage();
            parsed.exit_code = 2;
            return parsed;
        }
        if (!options.default_scope.empty() && options.default_scope != "frame" && options.default_scope != "pass" &&
            options.default_scope != "material" && options.default_scope != "draw" &&
            options.default_scope != "transient") {
            std::cerr << "termin_shaderc: invalid --default-scope value '" << options.default_scope
