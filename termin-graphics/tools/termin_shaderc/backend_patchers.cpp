...
namespace termin_shaderc::internal {
    ...
    void annotate_slang_glsl_symbols(std::vector<ShaderResourceBinding>& resources, const std::string& source) {
        for (ShaderResourceBinding& resource : resources) {
            ...
            if (resource.kind == "constant_buffer") {
                const std::regex cbuffer_re("ConstantBuffer\\s*<\\s*([A-Za-z_][A-Za-z0-9_]*)\\s*>\\s*" +
                                            regex_escape(resource.name) + "\\b");
                std::smatch match;
    ...
    static bool slang_opengl_resource_present_in_glsl(const std::string& glsl, const ShaderResourceBinding& resource) {
        if (!resource.slang_glsl_symbol.empty() && glsl.find(resource.slang_glsl_symbol) != std::string::npos) {
            return true;
        }
    ...
    bool filter_slang_opengl_resources_for_glsl(const CompileOptions& options,
    ...
    static std::vector<std::string> slang_opengl_resource_symbols(const ShaderResourceBinding& resource) {
    ...
    static bool
    patch_slang_opengl_glsl_resource_binding(std::string& glsl, const ShaderResourceBinding& resource, bool& changed) {
        ...
        if (symbol_pos == std::string::npos) {
            std::cerr << "termin_shaderc: OpenGL GLSL is missing reflected resource '" << resource.name
                      << "'; cannot patch binding placement\n";
            return false;
        }

        const std::regex binding_layout_re(R"(layout\s*\(([^\)]*?\bbinding\s*=\s*)([0-9]+)([^\)]*)\))");
        ...
        while (true) {
            ...
            if (std::regex_search(declaration_prefix, match, binding_layout_re)) {
                const size_t value_begin = layout_pos + static_cast<size_t>(match.position(2));
        ...
        std::cerr << "termin_shaderc: OpenGL GLSL declaration for resource '" << resource.name
                  << "' has no layout(binding=...) to patch\n";
        return false;
    }

    bool patch_slang_opengl_glsl_resource_bindings(const CompileOptions& options,
    ...
    static bool patch_slang_wgsl_symbol_binding(std::string& wgsl,
                                                const std::string& symbol_pattern,
                                                uint32_t group,
                                                uint32_t binding,
                                                bool& changed,
                                                bool& found) {
        const std::regex declaration_re("(@binding\\()([0-9]+)(\\)\\s*@group\\()([0-9]+)"
                                        "(\\)\\s*var(?:<[^>]+>)?\\s+)(" +
                                        symbol_pattern + ")(\\s*:)");
        std::smatch match;
    ...
    static bool slang_wgsl_has_symbol_declaration(const std::string& wgsl, const std::string& symbol_pattern) {
    ...
    static bool slang_wgsl_resource_present(const std::string& wgsl, const ShaderResourceBinding& resource) {
        ...
        if (resource.kind == "texture") {
            return slang_wgsl_has_symbol_declaration(wgsl, escaped_name + "_texture_[0-9]+") ||
                   slang_wgsl_has_symbol_declaration(wgsl, escaped_name + "_[0-9]+");
        }
        if (resource.kind == "sampler") {
            return slang_wgsl_has_symbol_declaration(wgsl, escaped_name + "_sampler_[0-9]+") ||
                   slang_wgsl_has_symbol_declaration(wgsl, escaped_name + "_[0-9]+");
        }
        return slang_wgsl_has_symbol_declaration(wgsl, escaped_name + "_[0-9]+");
    }

    static bool
    patch_slang_wgsl_resource_binding(std::string& wgsl, const ShaderResourceBinding& resource, bool& changed) {
        const std::string escaped_name = regex_escape(resource.name);
        auto patch_candidates =
            [&](const std::vector<std::string>& patterns, uint32_t binding, const char* role) -> bool {
            for (const std::string& pattern : patterns) {
                bool found = false;
                if (!patch_slang_wgsl_symbol_binding(wgsl, pattern, resource.set, binding, changed, found)) {
                    return false;
                }
                if (found) {
                    return true;
            ...
            std::cerr << "termin_shaderc: WGSL is missing " << role << " declaration for reflected resource '"
                      << resource.name << "'; cannot patch WebGPU binding placement\n";
            return false;
        };
        ...
        if (resource.kind == "texture") {
            ...
            if (resource.webgpu_has_sampler_binding) {
                return patch_candidates({escaped_name + "_sampler_[0-9]+"}, resource.webgpu_sampler_binding, "sampler");
            }
            return true;
        }
        ...
        return patch_candidates({escaped_name + "_[0-9]+"}, resource.binding, "resource");
    }

    bool assign_and_patch_slang_webgpu_resource_bindings(const CompileOptions& options,
                                                         std::vector<ShaderResourceBinding>& resources) {
        std::string wgsl;
        ...
        for (ShaderResourceBinding& resource : resources) {
            ...
            const tgfx::ShaderResourceScope scope = resource.scope == "frame"      ? tgfx::ShaderResourceScope::Frame
            ...
            const tgfx::BackendBindingRange range = tgfx::transitional_backend_binding_range(
                tgfx::BackendType::Vulkan, tgfx::ShaderResourceKind::Sampler, scope);
            if (range.size == 0) {
                std::cerr << "termin_shaderc: combined texture resource '" << resource.name
                          << "' has no WebGPU sampler binding range for scope '" << resource.scope << "'\n";
                return false;
            }

            const uint32_t hash = tgfx::stable_shader_resource_name_hash(resource.name + ":sampler");
            uint32_t candidate = range.base + (hash % range.size);
            bool assigned = false;
            for (uint32_t attempt = 0; attempt < range.size; ++attempt) {
                const bool conflict = std::any_of(occupied.begin(), occupied.end(), [&](const OccupiedBinding& slot) {
            ...
            if (!assigned) {
                std::cerr << "termin_shaderc: no free WebGPU sampler binding for combined " << "texture resource '"
                          << resource.name << "'\n";
                return false;
            }
        }
    ...
    static bool replace_all_literal(std::string& text, const std::string& needle, const std::string& replacement) {
        bool changed = false;
        size_t pos = 0;
    ...
    static bool patch_slang_d3d11_hlsl_symbol_register(std::string& hlsl,
        ...
        while (true) {
            ...
            const bool left_ok = symbol_pos == 0 || !is_identifier_char(hlsl[symbol_pos - 1]);
            const size_t symbol_end = symbol_pos + symbol.size();
            const bool right_ok = symbol_end >= hlsl.size() || !is_identifier_char(hlsl[symbol_end]);
            if (!left_ok || !right_ok) {
            ...
            const size_t search_limit = line_end == std::string::npos ? hlsl.size() : line_end;
            ...
            while (class_pos < search_limit && (hlsl[class_pos] == ' ' || hlsl[class_pos] == '\t')) {
                ++class_pos;
            }
            ...
            while (value_begin < search_limit && (hlsl[value_begin] == ' ' || hlsl[value_begin] == '\t')) {
                ++value_begin;
            }
            size_t value_end = value_begin;
            while (value_end < search_limit && hlsl[value_end] >= '0' && hlsl[value_end] <= '9') {
                ++value_end;
            }
            ...
            if (hlsl.compare(value_begin, value_end - value_begin, normalized_register) == 0) {
                return true;
            }
    ...
    static bool
    patch_slang_d3d11_hlsl_resource(std::string& hlsl, const ShaderResourceBinding& resource, bool& changed) {
        ...
        auto patch_any_symbol = [&](const std::vector<std::string>& symbols, char cls, bool required) -> bool {
            bool any_found = false;
            for (const std::string& symbol : symbols) {
                bool found = false;
                if (!patch_slang_d3d11_hlsl_symbol_register(hlsl, symbol, cls, register_index, changed, found)) {
                    return false;
                }
                any_found = any_found || found;
            }
            if (!any_found && required) {
                std::cerr << "termin_shaderc: D3D11 HLSL is missing register declaration " << "for reflected resource '"
                          << resource.name << "'\n";
                return false;
            }
            return true;
        ...
        if (resource.kind == "texture") {
            const bool texture_found_in_hlsl = hlsl.find(resource.name + "_texture_0") != std::string::npos ||
                                               hlsl.find(resource.name + "_0") != std::string::npos;
            if (!patch_any_symbol(
                    {resource.name + "_texture_0", resource.name + "_0"}, register_class, texture_found_in_hlsl)) {
                return false;
            }
            if (resource.slang_combined_texture) {
                const bool sampler_found_in_hlsl = hlsl.find(resource.name + "_sampler_0") != std::string::npos;
                if (!patch_any_symbol({resource.name + "_sampler_0"}, 's', sampler_found_in_hlsl)) {
                    return false;
                }
            }
            return true;
        ...
        return patch_any_symbol({resource.name + "_0", resource.name}, register_class, false);
    }
    ...
    static std::string d3d11_hlsl_resource_name_from_symbol(const std::string& symbol) {
    ...
    static std::optional<D3D11HlslResourceDecl> parse_d3d11_hlsl_resource_decl(const std::string& line) {
        ...
        size_t class_pos = open_paren + 1;
        while (class_pos < line.size() && (line[class_pos] == ' ' || line[class_pos] == '\t')) {
            ++class_pos;
        }
        ...
        if (register_class != 'b' && register_class != 't' && register_class != 's' && register_class != 'u') {
            return std::nullopt;
        }
        size_t value_begin = class_pos + 1;
        while (value_begin < line.size() && (line[value_begin] == ' ' || line[value_begin] == '\t')) {
            ++value_begin;
        }
        size_t value_end = value_begin;
        ...
        decl.register_index = static_cast<uint32_t>(std::stoul(line.substr(value_begin, value_end - value_begin)));
        decl.is_sampler = prefix.find("SamplerState") != std::string::npos ||
        ...
        decl.is_comparison_sampler = prefix.find("SamplerComparisonState") != std::string::npos;
    ...
    static std::vector<D3D11HlslResourceDecl> collect_d3d11_hlsl_resource_decls(const std::string& hlsl) {
        std::vector<D3D11HlslResourceDecl> decls;
        std::istringstream stream(hlsl);
        std::string line;
        while (std::getline(stream, line)) {
            std::optional<D3D11HlslResourceDecl> decl = parse_d3d11_hlsl_resource_decl(line);
            if (decl) {
    ...
    static bool d3d11_hlsl_has_decl_for_resource(const std::vector<D3D11HlslResourceDecl>& decls,
                                                 const std::string& resource_name,
                                                 bool sampler) {
        return std::any_of(decls.begin(), decls.end(), [&](const D3D11HlslResourceDecl& decl) {
            return decl.resource_name == resource_name && decl.is_sampler == sampler;
        });
    }

    static bool d3d11_hlsl_has_comparison_sampler_array_for_resource(const std::vector<D3D11HlslResourceDecl>& decls,
                                                                     const std::string& resource_name) {
        return std::any_of(decls.begin(), decls.end(), [&](const D3D11HlslResourceDecl& decl) {
            return decl.resource_name == resource_name && decl.is_sampler && decl.is_comparison_sampler &&
                   decl.is_array;
        });
    }

    static const ShaderResourceBinding*
    find_declared_d3d11_hlsl_resource(const std::vector<ShaderResourceBinding>& declared_resources,
    ...
    static std::string infer_d3d11_hlsl_resource_scope(const std::string& name, const std::string& kind) {
        (void)name;
        (void)kind;
        return "unscoped";
    }

    static void append_missing_d3d11_hlsl_resources(std::vector<ShaderResourceBinding>& resources,
                                                    const std::vector<ShaderResourceBinding>& declared_resources,
                                                    const std::string& hlsl,
                                                    uint32_t stage_mask) {
        const std::vector<D3D11HlslResourceDecl> decls = collect_d3d11_hlsl_resource_decls(hlsl);
        for (ShaderResourceBinding& binding : resources) {
            if (binding.kind == "texture" &&
                d3d11_hlsl_has_comparison_sampler_array_for_resource(decls, binding.name)) {
                binding.d3d11_scalar_sampler_for_texture_array = true;
            }
        }
        for (const D3D11HlslResourceDecl& decl : decls) {
            if (decl.is_sampler && d3d11_hlsl_has_decl_for_resource(decls, decl.resource_name, false)) {
                continue;
            }
            ...
            if (const ShaderResourceBinding* declared =
                    find_declared_d3d11_hlsl_resource(declared_resources, decl.resource_name, decl.kind)) {
                binding = *declared;
            } else {
                binding.name = decl.resource_name;
                binding.kind = decl.kind;
                binding.scope = infer_d3d11_hlsl_resource_scope(binding.name, binding.kind);
            }
            binding.name = decl.resource_name;
            binding.kind = decl.kind;
            ...
            binding.slang_combined_texture =
                decl.kind == "texture" && d3d11_hlsl_has_decl_for_resource(decls, decl.resource_name, true);
            binding.d3d11_scalar_sampler_for_texture_array =
                decl.kind == "texture" &&
                d3d11_hlsl_has_comparison_sampler_array_for_resource(decls, decl.resource_name);
            binding.slang_split_texture = decl.kind == "texture" && !binding.slang_combined_texture;
    ...
    bool augment_d3d11_resource_bindings_from_hlsl(const CompileOptions& options,
        ...
        append_missing_d3d11_hlsl_resources(resources, declared_resources, hlsl, stage_mask_for_stage(options.stage));
    ...
    static bool legalize_slang_d3d11_comparison_sampler_arrays(std::string& hlsl, bool& changed) {
        ...
        hlsl = std::regex_replace(hlsl, comparison_sampler_decl_re, "$1$2");
        hlsl = std::regex_replace(hlsl, comparison_sampler_use_re, "$1");
        if (hlsl != before) {
            changed = true;
        }
        return true;
    ...
    static bool
    patch_slang_d3d11_fragment_entry_input_position(std::string& hlsl, const std::string& entry, bool& changed) {
        if (entry.empty()) {
            return true;
        }

        const std::regex entry_re("\\b" + regex_escape(entry) +
        ...
        const std::regex struct_re("\\bstruct\\s+" + regex_escape(input_type) + R"(\s*\{)");
        ...
        if (body.find("SV_Position") != std::string::npos || body.find(':') == std::string::npos) {
            return true;
        }
    ...
    bool patch_slang_d3d11_hlsl_resource_bindings(const CompileOptions& options,
                                                  const std::filesystem::path& hlsl_path,
                                                  const std::vector<ShaderResourceBinding>& resources) {
        std::string hlsl;
        if (!read_file(hlsl_path.string(), hlsl)) {
        ...
        if (std::getenv("TERMIN_SHADERC_DEBUG_LAYOUT")) {
            for (const ShaderResourceBinding& resource : resources) {
                std::cerr << "termin_shaderc: D3D11 layout resource '" << resource.name << "' kind=" << resource.kind
                          << " binding=" << resource.binding << " register=" << resource.d3d11_register_class
    ...
    bool legalize_slang_opengl_glsl_builtins(const CompileOptions& options) {
        ...
        bool changed = false;
        changed |= replace_all_literal(glsl, "uint(gl_InstanceIndex - gl_BaseInstance)", "uint(gl_InstanceID)");
        changed |= replace_all_literal(glsl, "gl_InstanceIndex - gl_BaseInstance", "gl_InstanceID");
        changed |= replace_all_literal(glsl, "gl_InstanceIndex", "gl_InstanceID");
        ...
        static const std::regex version_re(R"(^#version[ \t]+([0-9]+)(?:[ \t]+core)?[ \t]*\r?\n)");
        ...
        if (std::regex_search(glsl, version_match, version_re)) {
            ...
            if (version > 450) {
                const bool has_draw_parameters_extension =
                    glsl.find("#extension GL_ARB_shader_draw_parameters : require") != std::string::npos;
                if (version == 460 && has_draw_parameters_extension) {
                    glsl.replace(static_cast<size_t>(version_match.position(1)),
                                 static_cast<size_t>(version_match.length(1)),
                                 "450");
                    changed = true;
                } else {
                    std::cerr << "termin_shaderc: Slang emitted unsupported GLSL " << version
    ...
    static bool read_spirv_words(const std::filesystem::path& path, std::vector<uint32_t>& out) {
    ...
    static std::string spirv_string_operand(const std::vector<uint32_t>& words, size_t begin, size_t end) {
        std::string out;
    ...
    static bool
    spirv_resource_id_for_name(const std::vector<uint32_t>& words, const std::string& name, uint32_t& out_id) {
    ...
    static bool spirv_resource_has_descriptor_decorations(const std::vector<uint32_t>& words, uint32_t resource_id) {
    ...
    bool strip_slang_vulkan_spirv_source_metadata(const CompileOptions& options) {
        ...
        for (size_t i = 5; i < words.size();) {
            ...
            const uint16_t opcode = static_cast<uint16_t>(instruction & 0xffffu);
            if (word_count == 0 || i + word_count > words.size()) {
                std::cerr << "termin_shaderc: invalid SPIR-V instruction stream while "
            ...
            if (opcode == OP_SOURCE) {
                ...
                filtered.insert(filtered.end(), words.begin() + i, words.begin() + i + word_count);
            }
            i += word_count;
        }
    ...
    bool filter_slang_vulkan_resources_for_spirv(const CompileOptions& options,
                                                 std::vector<ShaderResourceBinding>& resources) {
        std::vector<uint32_t> words;
        if (!read_spirv_words(options.output, words)) {
    ...
    bool patch_slang_vulkan_spirv_descriptor_decorations(const CompileOptions& options,
                                                         const std::vector<ShaderResourceBinding>& resources) {
        std::vector<uint32_t> words;
        if (!read_spirv_words(options.output, words)) {
            // Fake/offline compiler tests may produce placeholder bytes while still
        ...
        for (const ShaderResourceBinding& resource : resources) {
            uint32_t resource_id = 0;
            if (!spirv_resource_id_for_name(words, resource.name, resource_id)) {
                std::cerr << "termin_shaderc: reflected SPIR-V resource '" << resource.name
