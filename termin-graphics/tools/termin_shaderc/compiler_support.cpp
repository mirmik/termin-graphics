...
namespace termin_shaderc::internal {
    ...
    static std::optional<std::string> find_windows_sdk_fxc() {
        ...
        if (const char* program_files_x86 = std::getenv("ProgramFiles(x86)")) {
            if (program_files_x86[0] != '\0') {
                roots.emplace_back(std::filesystem::path(program_files_x86) / "Windows Kits" / "10" / "bin");
            }
        }
    ...
    std::optional<std::string> resolve_slangc(const CompileOptions& options, const char* argv0) {
        ...
        if (!configured.is_nil()) {
            if (!configured.is_string()) {
                std::cerr << "termin_shaderc: Shader/slangCompiler must be a string\n";
                return std::nullopt;
            }
            if (!configured.as_string().empty() && !is_existing_file(configured.as_string())) {
                std::cerr << "termin_shaderc: Shader/slangCompiler points to missing slangc: " << configured.as_string()
                          << "\n";
                return std::nullopt;
            }
        ...
        std::cerr << "termin_shaderc: slangc not found. Configure Shader/slangCompiler, "
                  << "set TERMIN_SLANGC, add slangc to PATH, or install it under " << "TERMIN_SDK/bin.\n";
        return std::nullopt;
    }
    ...
    std::optional<std::string> resolve_fxc(const CompileOptions& options, const char* argv0) {
        ...
        std::cerr << "termin_shaderc: fxc not found. Set TERMIN_FXC, add fxc to PATH, "
                  << "install it under TERMIN_SDK/bin, or install the Windows SDK.\n";
        return std::nullopt;
    }

    std::optional<std::string> resolve_wgsl_validator(const CompileOptions& options, const char* argv0) {
        if (!options.wgsl_validator.empty()) {
            if (!is_existing_file(options.wgsl_validator)) {
                std::cerr << "termin_shaderc: WGSL validator does not exist: " << options.wgsl_validator << "\n";
                return std::nullopt;
            }
        ...
        if (const char* env = std::getenv("TERMIN_WGSL_VALIDATOR")) {
            if (env[0] != '\0') {
                if (!is_existing_file(env)) {
                    std::cerr << "termin_shaderc: TERMIN_WGSL_VALIDATOR points to missing validator: " << env << "\n";
                    return std::nullopt;
                }
                return std::string(env);
            }
        ...
        std::cerr << "termin_shaderc: WGSL validator not found. Pass --wgsl-validator, "
                  << "set TERMIN_WGSL_VALIDATOR, add naga to PATH, or install it under " << "TERMIN_SDK/bin.\n";
        return std::nullopt;
    }

    static void append_unique_existing_dir(std::vector<std::string>& dirs, const std::filesystem::path& path) {
        std::error_code ec;
    ...
    std::vector<std::string> slang_include_dirs(const CompileOptions& options, const char* argv0) {
        ...
        if (const char* sdk = std::getenv("TERMIN_SDK")) {
            if (sdk[0] != '\0') {
                append_unique_existing_dir(dirs, std::filesystem::path(sdk) / "share" / "termin" / "builtin_shaders");
            }
        }
        ...
        if (argv0 && argv0[0] != '\0') {
            ...
            std::filesystem::path tool_dir = std::filesystem::absolute(argv0, ec).parent_path();
            if (!ec) {
                append_unique_existing_dir(dirs, tool_dir.parent_path() / "share" / "termin" / "builtin_shaders");
                append_unique_existing_dir(dirs, tool_dir / "share" / "termin" / "builtin_shaders");
            }
        }
    ...
    int run_command(const std::vector<std::string>& args) {
        ...
        if (!result.start_error.empty()) {
            std::cerr << "termin_shaderc: failed to run " << (args.empty() ? "<empty>" : args[0]) << ": "
                      << result.start_error << "\n";
        }
