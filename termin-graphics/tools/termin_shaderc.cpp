...
namespace termin_shaderc::internal {
    ...
    static bool is_valid_explicit_resource_scope(const std::string& scope) {
        return scope == "frame" || scope == "pass" || scope == "material" || scope == "draw" || scope == "transient";
    }
    ...
    void apply_default_resource_scope(std::vector<ShaderResourceBinding>& resources, const std::string& default_scope) {
    ...
    static tgfx::ShaderResourceKind shader_resource_kind_for_layout(const std::string& kind) {
        if (kind == "constant_buffer" || kind == "uniform_buffer") {
            return tgfx::ShaderResourceKind::ConstantBuffer;
        }
        if (kind == "texture")
            return tgfx::ShaderResourceKind::Texture;
        if (kind == "sampler")
            return tgfx::ShaderResourceKind::Sampler;
        if (kind == "storage_buffer")
            return tgfx::ShaderResourceKind::StorageBuffer;
        if (kind == "storage_texture")
            return tgfx::ShaderResourceKind::StorageTexture;
        return tgfx::ShaderResourceKind::None;
    }

    static uint32_t shader_resource_c_kind_for_layout(const std::string& kind) {
        if (kind == "constant_buffer" || kind == "uniform_buffer") {
    ...
    static tgfx::ShaderResourceScope shader_resource_scope_for_layout(const std::string& scope) {
        if (scope == "frame")
            return tgfx::ShaderResourceScope::Frame;
        if (scope == "pass")
            return tgfx::ShaderResourceScope::Pass;
        if (scope == "material")
            return tgfx::ShaderResourceScope::Material;
        if (scope == "draw")
            return tgfx::ShaderResourceScope::Draw;
        if (scope == "transient")
            return tgfx::ShaderResourceScope::Transient;
        if (scope == "unscoped")
            return tgfx::ShaderResourceScope::Unscoped;
        return tgfx::ShaderResourceScope::Unknown;
    }

    static bool resource_binding_conflicts(const ShaderResourceBinding& a,
        ...
        const tgfx::BackendBindingConflictClass a_class =
            tgfx::backend_binding_conflict_class(backend, shader_resource_kind_for_layout(a.kind));
        const tgfx::BackendBindingConflictClass b_class =
            tgfx::backend_binding_conflict_class(backend, shader_resource_kind_for_layout(b.kind));
        if (a_class != b_class) {
            return false;
        }
    ...
    void normalize_scope_first_binding_slots(std::vector<ShaderResourceBinding>& resources,
        ...
        for (size_t index = 0; index < resources.size(); ++index) {
            ...
            resource.set = 0;
            const tgfx::ShaderResourceKind kind = shader_resource_kind_for_layout(resource.kind);
            const tgfx::ShaderResourceScope scope = shader_resource_scope_for_layout(resource.scope);
            ...
            const tgfx::BackendBindingRange range = tgfx::transitional_backend_binding_range(backend, kind, scope);
            if (range.size == 0) {
                continue;
            }
            ...
            for (uint32_t attempt = 0; attempt < range.size; ++attempt) {
                ...
                for (size_t previous = 0; previous < index; ++previous) {
                    if (resource_binding_conflicts(resource, resources[previous], target)) {
                        conflict = true;
                        break;
                    }
                }
    ...
    static void append_unique_resource(std::vector<ShaderResourceBinding>& resources, ShaderResourceBinding binding) {
        for (ShaderResourceBinding& existing : resources) {
            if (existing.name == binding.name) {
                existing.kind = binding.kind;
                if (!binding.scope.empty() && binding.scope != "unknown" && binding.scope != "unscoped") {
                ...
                existing.slang_combined_texture = existing.slang_combined_texture || binding.slang_combined_texture;
                existing.slang_split_texture = existing.slang_split_texture || binding.slang_split_texture;
                existing.slang_separate_sampler = existing.slang_separate_sampler || binding.slang_separate_sampler;
                existing.slang_storage_texture = existing.slang_storage_texture || binding.slang_storage_texture;
                existing.d3d11_scalar_sampler_for_texture_array =
                    existing.d3d11_scalar_sampler_for_texture_array || binding.d3d11_scalar_sampler_for_texture_array;
                return;
            }
        }
    ...
    static bool canonicalize_shader_abi_resources(std::vector<ShaderResourceBinding>& resources) {
        std::vector<ShaderResourceBinding> canonical;
        canonical.reserve(resources.size());
        for (ShaderResourceBinding& resource : resources) {
            const tc_shader_abi_resource_decl* abi = tc_shader_abi_find_resource(resource.name.c_str());
            if (abi) {
                const uint32_t kind = shader_resource_c_kind_for_layout(resource.kind);
                if (kind != abi->kind) {
                    std::cerr << "termin_shaderc: shader ABI resource '" << resource.name << "' has kind '"
                              << resource.kind << "', expected kind " << abi->kind << "\n";
                    return false;
                }

                const std::string expected_scope = shader_resource_scope_layout_name(abi->scope);
                if (is_unscoped_resource_scope(resource.scope) || resource.scope == "unknown") {
                    std::cerr << "termin_shaderc: shader ABI resource '" << resource.name
                              << "' has no explicit scope, expected '" << expected_scope
                              << "'; preserve [[TerminScope]] metadata or provide an " << "explicit imported scope\n";
                    return false;
                } else if (resource.scope != expected_scope) {
                    std::cerr << "termin_shaderc: shader ABI resource '" << resource.name << "' has scope '"
                              << resource.scope << "', expected '" << expected_scope << "'\n";
                    return false;
                }
    ...
    bool has_resource_named(const std::vector<ShaderResourceBinding>& resources, const std::string& name) {
        return std::any_of(resources.begin(), resources.end(), [&](const ShaderResourceBinding& resource) {
            return resource.name == name;
        });
    }

    static void append_missing_resources(std::vector<ShaderResourceBinding>& resources,
    ...
    static std::optional<std::string_view> entry_function_body(const std::string& source, const std::string& entry) {
        if (entry.empty()) {
            return std::nullopt;
        }
        ...
        while (true) {
            ...
            const bool left_ok = name_pos == 0 || !is_identifier_char(source[name_pos - 1]);
            const size_t name_end = name_pos + entry.size();
            const bool right_ok = name_end >= source.size() || !is_identifier_char(source[name_end]);
            if (!left_ok || !right_ok) {
            ...
            for (size_t i = body_pos; i < source.size(); ++i) {
                if (source[i] == '{') {
                    ...
                    --depth;
                    if (depth == 0) {
                        return std::string_view(source).substr(body_pos + 1, i - body_pos - 1);
                    }
                }
            }
            return std::nullopt;
    ...
    static bool entry_body_references_resource(std::string_view body, const std::string& name) {
        if (name.empty()) {
            return false;
        }
    ...
    static void append_missing_resources_used_by_entry(std::vector<ShaderResourceBinding>& resources,
                                                       std::vector<ShaderResourceBinding> candidates,
                                                       const std::string& source,
                                                       const std::string& entry) {
        const std::optional<std::string_view> body = entry_function_body(source, entry);
        if (!body) {
    ...
    static bool trent_string_field(const nos::trent& value, const char* key, std::string& out) {
    ...
    static bool trent_uint_field(const nos::trent& value, const char* key, uint32_t& out) {
        const nos::trent* field = trent_dict_get(value, key);
        if (!field || !field->is_numer()) {
            return false;
        }
        const int64_t number = field->as_integer();
        if (number < 0 || number > static_cast<int64_t>(std::numeric_limits<uint32_t>::max())) {
            return false;
        }
    ...
    static bool trent_bool_field(const nos::trent& value, const char* key, bool& out) {
        const nos::trent* field = trent_dict_get(value, key);
    ...
    static bool slang_parameter_binding(const nos::trent& parameter,
    ...
    static std::string material_property_type_from_slang_type(const nos::trent& type) {
        ...
        if (kind == "scalar") {
            ...
            if (scalar_type == "float32" || scalar_type == "float16")
                return "Float";
            if (scalar_type == "int32" || scalar_type == "int16" || scalar_type == "int8")
                return "Int";
            if (scalar_type == "uint32" || scalar_type == "uint16" || scalar_type == "uint8")
                return "Int";
            if (scalar_type == "bool")
                return "Bool";
            return {};
        ...
        if (kind == "vector") {
            ...
            std::string scalar_type;
            if (!trent_string_field(*element_type, "kind", element_kind) || element_kind != "scalar" ||
                !trent_string_field(*element_type, "scalarType", scalar_type) || scalar_type != "float32") {
                return {};
            }
            if (element_count == 2)
                return "Vec2";
    ...
    static bool is_slang_storage_buffer_base_shape(const std::string& base_shape) {
        return base_shape == "structuredBuffer" || base_shape == "byteAddressBuffer" ||
               base_shape == "rwStructuredBuffer" || base_shape == "rwByteAddressBuffer";
    }

    static bool slang_resource_kind_from_parameter(const nos::trent& parameter,
                                                   const std::string& binding_kind,
                                                   ShaderResourceBinding& out_binding) {
        const nos::trent* type = trent_dict_get(parameter, "type");
        std::string type_kind;
        if (!type || !type->is_dict() || !trent_string_field(*type, "kind", type_kind)) {
            return false;
        }

        if (type_kind == "constantBuffer") {
            if (binding_kind != "constantBuffer" && binding_kind != "descriptorTableSlot") {
                return false;
            }
                ...
                if (el_type && el_type->is_dict()) {
                    ...
                    if (fields && fields->is_list()) {
                        for (const nos::trent& field : fields->as_list()) {
                            ...
                            if (!trent_string_field(field, "name", f.name))
                                continue;
        ...
        auto resource_kind_from_type = [&](const nos::trent& resource_type, ShaderResourceBinding& binding) -> bool {
            ...
            if (is_slang_storage_buffer_base_shape(base_shape)) {
                if (binding_kind != "descriptorTableSlot" && binding_kind != "shaderResource") {
                    return false;
                }
            ...
            if (has_access && access == "readWrite") {
                ...
                if (binding_kind != "shaderResource" && binding_kind != "descriptorTableSlot") {
                    return false;
                }
        ...
        if (type_kind == "array") {
            const nos::trent* element_type = trent_dict_get(*type, "elementType");
            std::string element_kind;
            if (!element_type || !element_type->is_dict() || !trent_string_field(*element_type, "kind", element_kind) ||
    ...
    slang_scope_attribute_from_parameter(const nos::trent& parameter, const std::string& resource_name, bool& out_ok) {
        out_ok = true;
        ...
        if (!user_attribs->is_list()) {
            std::cerr << "termin_shaderc: malformed userAttribs for resource '" << resource_name << "'\n";
            out_ok = false;
            return std::nullopt;
        }
        ...
        for (const nos::trent& attrib : user_attribs->as_list()) {
            if (!attrib.is_dict()) {
                std::cerr << "termin_shaderc: malformed user attribute for resource '" << resource_name << "'\n";
                out_ok = false;
                return std::nullopt;
            }
            ...
            if (!trent_string_field(attrib, "name", attrib_name)) {
                std::cerr << "termin_shaderc: unnamed user attribute for resource '" << resource_name << "'\n";
                out_ok = false;
                return std::nullopt;
            }
            ...
            if (!args || !args->is_list()) {
                std::cerr << "termin_shaderc: " << attrib_name << " on resource '" << resource_name
            ...
            if (!is_valid_explicit_resource_scope(candidate)) {
                std::cerr << "termin_shaderc: invalid " << attrib_name << " value '" << candidate << "' on resource '"
                          << resource_name << "' (expected frame, pass, material, draw, or transient)\n";
                out_ok = false;
                return std::nullopt;
            }

            if (scope && *scope != candidate) {
                std::cerr << "termin_shaderc: conflicting scope attributes on resource '" << resource_name << "': '"
                          << *scope << "' and '" << candidate << "'\n";
                out_ok = false;
                return std::nullopt;
            }
    ...
    static std::vector<ShaderResourceBinding> infer_resource_bindings_from_slang_reflection(
        const std::string& reflection_text, const CompileOptions& options, bool& out_ok) {
        out_ok = true;
        ...
        for (const nos::trent& parameter : parameters->as_list()) {
            ...
            if (!slang_parameter_binding(parameter, binding_kind, binding.binding, binding.set)) {
                continue;
            }
            if (!slang_resource_kind_from_parameter(parameter, binding_kind, binding)) {
                continue;
            }
            bool scope_ok = true;
            std::optional<std::string> explicit_scope = slang_scope_attribute_from_parameter(parameter, name, scope_ok);
            if (!scope_ok) {
                out_ok = false;
                return {};
            }
    ...
    static std::vector<ShaderResourceBinding> infer_resource_bindings(const std::string& source,
        ...
        if (options.language == "slang") {
            ...
            for (std::sregex_iterator it(source.begin(), source.end(), slang_constant_buffer_re), end; it != end;
                 ++it) {
            ...
            for (std::sregex_iterator it(source.begin(), source.end(), texture_register_re), end; it != end; ++it) {
                ...
                binding.name = match[2].str();
                binding.kind = (reg_space == "u" || type.rfind("RWTexture", 0) == 0) ? "storage_texture" : "texture";
            ...
            for (std::sregex_iterator it(source.begin(), source.end(), bare_resource_re), end; it != end; ++it) {
                ...
                } else if (type.rfind("StructuredBuffer", 0) == 0 || type.rfind("RWStructuredBuffer", 0) == 0 ||
                           type == "ByteAddressBuffer" || type == "RWByteAddressBuffer") {
                    binding.kind = "storage_buffer";
            ...
            for (std::sregex_iterator it(source.begin(), source.end(), sampler_register_re), end; it != end; ++it) {
                const std::smatch& match = *it;
                ShaderResourceBinding binding;
                binding.name = match[2].str();
                binding.kind = "sampler";
        ...
        if (options.language == "glsl") {
            ...
            for (std::sregex_iterator it(source.begin(), source.end(), sampler_re), end; it != end; ++it) {
                const std::smatch& match = *it;
                ShaderResourceBinding binding;
                binding.name = match[2].str();
                binding.kind = "texture";
    ...
    resolve_slang_import_path(const std::string& module_name, const std::vector<std::string>& include_dirs) {
    ...
    static void collect_slang_import_resource_bindings(const std::string& source,
                                                       const CompileOptions& options,
                                                       const std::vector<std::string>& include_dirs,
                                                       std::unordered_set<std::string>& visited_imports,
                                                       std::vector<ShaderResourceBinding>& out_resources) {
        static const std::regex import_re(R"(\bimport\s+([A-Za-z_][A-Za-z0-9_\.]*)\s*;)");
        for (std::sregex_iterator it(source.begin(), source.end(), import_re), end; it != end; ++it) {
            const std::string module_name = (*it)[1].str();
            std::optional<std::filesystem::path> import_path = resolve_slang_import_path(module_name, include_dirs);
            if (!import_path) {
                continue;
            }
            ...
            append_missing_resources(out_resources, infer_resource_bindings(import_source, options));
            collect_slang_import_resource_bindings(
                import_source, options, include_dirs, visited_imports, out_resources);
        }
    }

    static std::vector<ShaderResourceBinding> collect_declared_slang_resource_bindings(
        const std::string& source, const CompileOptions& options, const std::vector<std::string>& include_dirs) {
        std::vector<ShaderResourceBinding> resources = infer_resource_bindings(source, options);
        std::unordered_set<std::string> visited_imports;
        collect_slang_import_resource_bindings(source, options, include_dirs, visited_imports, resources);
        return resources;
    }
    ...
    static bool assign_program_resource_placement(const CompileOptions& options,
        ...
        for (const std::string& source_path : options.program_sources) {
            ...
            if (!read_file(source_path, source)) {
                std::cerr << "termin_shaderc: failed to read program source: " << source_path << "\n";
                return false;
            }
        ...
        std::sort(program_resources.begin(),
                  program_resources.end(),
                  [](const ShaderResourceBinding& a, const ShaderResourceBinding& b) {
                      return std::tie(a.scope, a.kind, a.name) < std::tie(b.scope, b.kind, b.name);
                  });
        normalize_scope_first_binding_slots(program_resources, true, options.target);
        std::sort(program_resources.begin(),
                  program_resources.end(),
                  [](const ShaderResourceBinding& a, const ShaderResourceBinding& b) {
    ...
    static bool collect_resource_bindings(const CompileOptions& options,
                                          const std::string& source,
        ...
        if (reflection_path && is_existing_file(*reflection_path)) {
            ...
            if (read_file(reflection_path->string(), reflection_text)) {
                ...
                resources = infer_resource_bindings_from_slang_reflection(reflection_text, options, reflection_ok);
                if (!reflection_ok) {
                    return false;
                }
            }
        }
        std::vector<ShaderResourceBinding> source_resources = infer_resource_bindings(source, options);
        if (resources.empty()) {
            ...
            if (options.target == "d3d11") {
                ...
                append_missing_resources_used_by_entry(resources, std::move(source_resources), source, options.entry);
            }
        }
        ...
        normalize_scope_first_binding_slots(resources, options.language == "slang", options.target);
        if (options.target == "d3d11") {
        ...
        if (options.language == "slang" && options.target == "vulkan") {
            for (const ShaderResourceBinding& resource : resources) {
                if (resource.slang_split_texture || resource.slang_separate_sampler) {
                    std::cerr << "termin_shaderc: Vulkan path does not support split Slang "
    ...
    static bool write_resource_layout_sidecar(const CompileOptions& options,
        ...
        for (const auto& res : resources) {
            ...
            if (res.kind == "uniform_buffer" && res.size > max_ubo_size) {
                std::cerr << "termin_shaderc: UBO '" << res.name << "' size " << res.size << " exceeds limit "
                          << max_ubo_size << " bytes; shader will likely fail validation on GPU\n";
                return false;
            }
            // Binding range
            if (res.binding > max_binding) {
                std::cerr << "termin_shaderc: resource '" << res.name << "' binding=" << res.binding
            ...
            if (res.scope.empty() || res.scope == "unknown" || res.scope == "unscoped") {
                ...
                std::cerr << "termin_shaderc: WARNING — resource '" << res.name << "' has "
                          << (res.scope == "unscoped" ? "no scope" : "unknown scope") << "; consider adding " << attr
        ...
        if (options.target == "d3d11") {
            for (const ShaderResourceBinding& res : resources) {
                if (res.d3d11_register_class.empty()) {
                    std::cerr << "termin_shaderc: resource '" << res.name << "' kind '" << res.kind
            ...
            for (size_t i = 0; i < resources.size(); ++i) {
                for (size_t j = i + 1; j < resources.size(); ++j) {
                    ...
                    if (a.d3d11_register_class != b.d3d11_register_class ||
                        a.d3d11_register_index != b.d3d11_register_index || (a.stage_mask & b.stage_mask) == 0) {
                        continue;
                    }
                    if (a.name != b.name || a.kind != b.kind || a.scope != b.scope) {
                        std::cerr << "termin_shaderc: conflicting D3D11 resources at register("
                                  << a.d3d11_register_class << a.d3d11_register_index << "): '" << a.name << "' ("
                                  << a.kind << ", " << a.scope << ") vs '" << b.name << "' (" << b.kind << ", "
                                  << b.scope << ")\n";
                        return false;
                    }
                }
            ...
            for (size_t i = 0; i < resources.size(); ++i) {
                for (size_t j = i + 1; j < resources.size(); ++j) {
                    ...
                    std::cerr << "termin_shaderc: conflicting resources at set=" << a.set << " binding=" << a.binding
                              << ": '" << a.name << "' (" << a.kind << ", " << a.scope << ") vs '" << b.name << "' ("
                              << b.kind << ", " << b.scope << ")\n";
                    return false;
                }
            }
        ...
        const uint32_t sidecar_version = options.target == "webgpu" ? 3u : (options.target == "d3d11" ? 2u : 1u);
        out << "{\n";
        ...
        for (size_t i = 0; i < resources.size(); ++i) {
            ...
            out << "    {" << "\"name\": \"" << json_escape(binding.name) << "\", " << "\"kind\": \""
                << json_escape(binding.kind) << "\", " << "\"scope\": \"" << json_escape(binding.scope) << "\", "
                << "\"set\": " << binding.set << ", " << "\"binding\": " << binding.binding << ", "
                << "\"stage_mask\": " << binding.stage_mask << ", " << "\"size\": " << binding.size;
            if (options.target == "d3d11") {
                out << ", \"d3d11\": {" << "\"register_class\": \"" << json_escape(binding.d3d11_register_class)
            ...
            if (options.target == "webgpu") {
                out << ", \"webgpu\": {" << "\"group\": " << binding.set << ", " << "\"binding\": " << binding.binding;
            ...
            if (!binding.fields.empty()) {
                ...
                for (size_t fi = 0; fi < binding.fields.size(); ++fi) {
                    ...
                    out << "{\"name\":\"" << json_escape(f.name) << "\"," << "\"type\":\"" << json_escape(f.type)
                        << "\"," << "\"offset\":" << f.offset << "," << "\"size\":" << f.size << "}";
                    if (fi + 1 < binding.fields.size())
                        out << ", ";
                }
                out << "]";
            }
    ...
    static bool
    write_resource_layout_sidecar(const CompileOptions& options,
                                  const std::string& source,
    ...
    static bool compile_glsl_to_vulkan(const CompileOptions& options) {
        ...
        std::cerr << "termin_shaderc: GLSL to Vulkan compilation requires shaderc support, "
        ...
        auto module =
            compiler.CompileGlslToSpv(source, kind, options.debug_name.c_str(), options.entry.c_str(), shader_options);

        if (module.GetCompilationStatus() != shaderc_compilation_status_success) {
            std::cerr << "termin_shaderc: shader compilation failed for " << options.debug_name << ": "
    ...
    static bool compile_slang(const CompileOptions& options, const char* argv0) {
        ...
        if (options.target == "vulkan") {
            ...
            if (options.stage == "geometry") {
                std::cerr << "termin_shaderc: WebGPU does not support geometry shaders\n";
                return false;
            }
            slang_target = "wgsl";
            native_clip_y_up_define = "-DTERMIN_NATIVE_CLIP_Y_UP=1";
        ...
        const std::vector<ShaderResourceBinding> declared_slang_resources =
            is_d3d11 ? collect_declared_slang_resource_bindings(source, options, include_dirs)
        ...
        if (!is_existing_file(slang_output_path)) {
            std::cerr << "termin_shaderc: slangc did not produce expected output: " << slang_output_path.string()
                      << "\n";
            return false;
        }
        if (is_cross_glsl) {
        ...
        if (is_d3d11) {
            if (!augment_d3d11_resource_bindings_from_hlsl(
                    options, slang_output_path, declared_slang_resources, resources)) {
                std::error_code ec;
                std::filesystem::remove(options.output, ec);
                std::filesystem::remove(options.output + ".layout.json", ec);
                if (!std::getenv("TERMIN_SHADERC_KEEP_INTERMEDIATE")) {
            ...
            if (!patch_slang_d3d11_hlsl_resource_bindings(options, slang_output_path, resources)) {
                std::error_code ec;
                std::filesystem::remove(options.output, ec);
                std::filesystem::remove(options.output + ".layout.json", ec);
                if (!std::getenv("TERMIN_SHADERC_KEEP_INTERMEDIATE")) {
            ...
            if (!is_existing_file(options.output)) {
                std::cerr << "termin_shaderc: fxc did not produce expected output: " << options.output << "\n";
                std::error_code ec;
                if (!std::getenv("TERMIN_SHADERC_KEEP_INTERMEDIATE")) {
                    std::filesystem::remove(slang_output_path, ec);
                }
        ...
        if (options.target == "vulkan") {
            ...
            wrote_layout = strip_slang_vulkan_spirv_source_metadata(options) &&
            ...
            wrote_layout = legalize_slang_opengl_glsl_builtins(options) &&
            ...
            wrote_layout = assign_and_patch_slang_webgpu_resource_bindings(options, resources) &&
                           run_command({
                               *wgsl_validator,
                               "--input-kind",
                               "wgsl",
...
int main(int argc, char** argv) {
    termin_shaderc::ParsedCommandLine parsed = termin_shaderc::parse_command_line(argc, argv);
