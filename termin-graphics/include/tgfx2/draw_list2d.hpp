...
public:
    ...
    bool ellipse(termin::Rect2f bounds,
                 FillPaint paint,
                 std::optional<StrokePaint> stroke = std::nullopt);
    bool path(Path2f path,
              std::optional<FillPaint> fill,
              std::optional<StrokePaint> stroke = std::nullopt);
    bool polyline(std::span<const termin::Vec2f> points,
                  StrokePaint stroke,
                  bool closed = false);
...
private:
    enum class Scope : std::uint8_t { Transform, Opacity, Clip };
