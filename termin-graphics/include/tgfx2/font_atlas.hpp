...
namespace tgfx {
    ...
    public:
        ...
        // texture immediately.
        void ensure_glyphs(std::string_view text_utf8, float display_px, RenderContext2* ctx = nullptr);
        ...
        int default_preload_size() const {
            return default_preload_size_;
        }
        ...
        const uint8_t* cpu_atlas_data() const {
            return atlas_.data();
        }

        // --- SDF configuration ---

        bool sdf_enabled() const {
            return sdf_enabled_;
        }
        ...
        int sdf_threshold_px() const {
            return sdf_threshold_px_;
        }
        void set_sdf_threshold_px(int v) {
            sdf_threshold_px_ = v;
        }

        int sdf_reference_px() const {
            return sdf_reference_px_;
        }
        int sdf_spread() const {
            return sdf_spread_;
        }
        ...
        const uint8_t* sdf_atlas_data() const {
            return sdf_atlas_.data();
        }
    ...
    private:
        ...
        static uint64_t make_key_(uint32_t codepoint, int px_size) {
            return (static_cast<uint64_t>(codepoint) << 32) | static_cast<uint32_t>(px_size);
        }
        ...
        void compute_sdf_(const uint8_t* bitmap, int w, int h, uint8_t* sdf, int sdf_stride);
    };
