...
namespace tgfx {
    ...
    private:
        ...
        bool begin_pass_impl(const RenderPassDesc& pass,
                             uint32_t view_count,
                             bool multiview,
        ...
        std::array<ResourceBindingBucket, static_cast<size_t>(ResourceScope::Count)> pending_binding_buckets_;
        ...
        void upsert_pending_planned_binding(ResourceScope scope,
                                            const BackendBoundResourceSlot& slot,
                                            const BoundResourceValue& value);
        BoundResourceSetDesc build_pending_bound_resource_set(uintptr_t resource_layout_token,
    ...
    public:
        RenderContext2(IRenderDevice& device, PipelineCache& cache, GraphicsHost* graphics_host = nullptr);
        ...
        // --- Shader binding ---
        void bind_shader(ShaderHandle vs, ShaderHandle fs, ShaderHandle gs = {});
        ...
        void bind_uniform(std::string_view name, BufferHandle buffer, uint64_t offset = 0, uint64_t range = 0);
        void bind_uniform(const struct ::tc_shader_resource_binding* rb,
                          BufferHandle buffer,
                          uint64_t offset = 0,
                          uint64_t range = 0);
        void bind_storage_buffer(std::string_view name, BufferHandle buffer, uint64_t offset = 0, uint64_t range = 0);
        void bind_storage_buffer(const struct ::tc_shader_resource_binding* rb,
                                 BufferHandle buffer,
                                 uint64_t offset = 0,
                                 uint64_t range = 0);
        void bind_texture(std::string_view name, TextureHandle texture, SamplerHandle sampler = {});
        void
        bind_texture(const struct ::tc_shader_resource_binding* rb, TextureHandle texture, SamplerHandle sampler = {});
        ...
        void bind_uniform_data(const struct ::tc_shader_resource_binding* rb, const void* data, uint32_t size);
        ...
        void draw(BufferHandle vbo, BufferHandle ibo, uint32_t index_count, IndexType idx_type = IndexType::Uint32);
        ...
        void draw_arrays_instanced(BufferHandle vbo, uint32_t vertex_count, uint32_t instance_count);
        ...
        GraphicsHost* graphics_host() const {
            return graphics_host_;
        }
