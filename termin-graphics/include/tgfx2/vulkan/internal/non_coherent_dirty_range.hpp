...
namespace tgfx::vulkan_detail {
    ...
    struct NonCoherentDirtyRange {
        ...
        [[nodiscard]] bool include(uint64_t offset, uint64_t size, uint64_t allocation_size) noexcept {
            if (size == 0) {
        ...
        [[nodiscard]] NonCoherentDirtyRange aligned(uint64_t non_coherent_atom_size,
            ...
            result.end = end == rounded_end ? end : std::min(allocation_size, rounded_end + atom);
            return result;
        }
    };
