...
namespace tgfx {
    ...
    inline termin::Bounds2i aspect_fit_rect(uint32_t source_width,
        ...
        if (static_cast<uint64_t>(source_width) * destination_height >
            ...
            height = std::max(
                1u, static_cast<uint32_t>(static_cast<uint64_t>(destination_width) * source_height / source_width));
        } else {
            width = std::max(
                1u, static_cast<uint32_t>(static_cast<uint64_t>(destination_height) * source_width / source_height));
        }
