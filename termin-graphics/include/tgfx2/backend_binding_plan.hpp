...
namespace tgfx {
    ...
    TGFX2_API bool shader_resource_scope_has_transitional_binding_range(ShaderResourceScope scope);
    TGFX2_API uint32_t stable_shader_resource_name_hash(std::string_view value);
    TGFX2_API BackendBindingRange transitional_backend_binding_range(BackendType backend,
                                                                     ShaderResourceKind kind,
                                                                     ShaderResourceScope scope);
    TGFX2_API BackendBindingConflictClass backend_binding_conflict_class(BackendType backend, ShaderResourceKind kind);
    ...
    public:
        ...
        void append_group(ShaderResourceScope scope,
                          bool dirty,
    ...
    template <typename Fn> void for_each_bound_resource_binding(const BoundResourceSetDesc& desc, Fn&& fn) {
    ...
    template <typename Fn> void for_each_dirty_bound_resource_binding(const BoundResourceSetDesc& desc, Fn&& fn) {
        for (uint32_t group_index = 0; group_index < desc.group_count; ++group_index) {
            const BoundResourceGroupView& group = desc.groups[group_index];
            if (!group.dirty) {
                continue;
    ...
    TGFX2_API bool build_backend_binding_plan(BackendType backend,
                                              const tc_shader_resource_binding* bindings,
                                              uint32_t binding_count,
                                              BackendBindingPlan& out_plan,
                                              std::string* error = nullptr);

    TGFX2_API BackendBoundResourceSlot bound_resource_slot_from_plan_entry(const BackendBindingPlanEntry& entry);
