...
namespace tgfx {
    ...
    TGFX2_API Tgfx2MeshBinding wrap_mesh_as_tgfx2(IRenderDevice& device, tc_mesh* mesh);

    TGFX2_API void release_mesh_binding(IRenderDevice& device, const Tgfx2MeshBinding& binding);

    TGFX2_API VertexLayoutDesc filter_vertex_layout_to_locations(const VertexLayoutDesc& layout,
                                                                 std::initializer_list<uint32_t> used_locations,
                                                                 bool use_shader_input_locations = false);

    TGFX2_API VertexLayoutDesc filter_vertex_layout_to_semantics(const VertexLayoutDesc& layout,
    ...
    TGFX2_API bool try_filter_vertex_layout_to_semantics(const VertexLayoutDesc& layout,
                                                         std::initializer_list<std::string_view> used_semantics,
    ...
    TGFX2_API bool vertex_layout_has_semantic(const VertexLayoutDesc& layout, std::string_view semantic);
    ...
    TGFX2_API bool
    draw_tc_mesh(RenderContext2& ctx, tc_mesh* mesh, const VertexBufferLayout* layout_override = nullptr);

    TGFX2_API bool draw_tc_mesh(RenderContext2& ctx, tc_mesh* mesh, const VertexLayoutDesc* layout_override);

    TGFX2_API bool draw_tc_submesh(RenderContext2& ctx,
                                   tc_mesh* mesh,
    ...
    TGFX2_API bool draw_tc_mesh(RenderContext2& ctx,
                                tc_mesh* mesh,
                                std::initializer_list<uint32_t> used_locations,
                                bool use_shader_input_locations = false);
