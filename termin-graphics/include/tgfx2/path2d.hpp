...
namespace tgfx {
    ...
    struct FlattenedPath2f {
        ...
        TGFX2_API bool stroke_contains(termin::Vec2f point, const StrokePaint& stroke) const noexcept;
    };
    ...
    public:
        ...
        bool try_assign(std::span<const Path2Verb> verbs, std::span<const termin::Vec2f> points);

        const std::vector<Path2Verb>& verbs() const noexcept {
            return verbs_;
        }
        const std::vector<termin::Vec2f>& points() const noexcept {
            return points_;
        }
