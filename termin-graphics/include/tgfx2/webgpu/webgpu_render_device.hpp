...
namespace tgfx {
    ...
    public:
        static void request_async(const WebGpuDeviceRequest& request, WebGpuDeviceCallback callback);
        ...
        bool has_device_error() const {
            return error_state_->failed;
        }
        const std::string& device_error_message() const {
    ...
    private:
        ...
        WebGpuRenderDevice(wgpu::Instance instance,
        ...
        PipelineHandle texture_op_pipeline(PixelFormat format, uint32_t sample_count, bool blit);
        ResourceSetHandle texture_op_resource_set(PipelineHandle pipeline, TextureHandle source, bool blit);
