...
namespace termin {
    ...
    public:
        ...
        explicit TcShaderProgram(tc_shader_program_handle value)
            : handle(value) {
            tc_shader_program_retain(get());
        }
        TcShaderProgram(const TcShaderProgram& other)
            : handle(other.handle) {
            tc_shader_program_retain(get());
        }
        TcShaderProgram(TcShaderProgram&& other) noexcept
            : handle(other.handle) {
        ...
        tc_shader_program* get() const {
            return tc_shader_program_get(handle);
        }
        bool is_valid() const {
            return tc_shader_program_is_valid(handle);
        }
        ...
        uint32_t version() const {
            return tc_shader_program_version(get());
        }
    };
