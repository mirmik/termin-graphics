...
typedef struct tc_gpu_context {
    ...
    // Human-readable name for debug introspection
    char name[32];
} tc_gpu_context;
...
// Set human-readable name for debug introspection.
TGFX_API void tc_gpu_context_set_name(tc_gpu_context* ctx, const char* name);
...
#define TC_MAX_GPU_CONTEXTS 16

// Per-context VAO info for a single mesh
typedef struct tc_mesh_context_vao_info {
    ...
    uint32_t bound_ebo;
    uint8_t  vao_stale;
} tc_mesh_context_vao_info;

// Full GPU state for one mesh across all contexts
typedef struct tc_mesh_gpu_info {
    uint32_t pool_index;
    // Shared (from share_group)
    uint32_t vbo;
    uint32_t ebo;
    int32_t  shared_version;
    // Per-context VAO info
    tc_mesh_context_vao_info contexts[TC_MAX_GPU_CONTEXTS];
    int context_count;
} tc_mesh_gpu_info;

// Get GPU state for mesh by pool_index (shared + all contexts' VAOs).
TGFX_API bool tc_gpu_get_mesh_info(uint32_t pool_index, tc_mesh_gpu_info* out);

// Number of registered GPU contexts.
TGFX_API int tc_gpu_context_count(void);

// Get context key by index (0..count-1).
TGFX_API uintptr_t tc_gpu_context_key_at(int index);
