...
namespace termin {
    ...
    public:
        ...
        explicit TcTexture(tc_texture_handle h)
            : handle(h) {
        ...
        TcTexture(const TcTexture& other)
            : handle(other.handle) {
            if (tc_texture* t = tc_texture_get(handle)) {
                tc_texture_add_ref(t);
            }
        ...
        TcTexture(TcTexture&& other) noexcept
            : handle(other.handle) {
        ...
        tc_texture* get() const {
            return tc_texture_get(handle);
        }

        // Query (safe - returns defaults if handle is stale)
        bool is_valid() const {
            return tc_texture_is_valid(handle);
        }
        ...
        tgfx::TextureEncoding encoding() const {
            tc_texture* t = get();
            return t ? tgfx::from_tc_texture_encoding(static_cast<tc_texture_encoding>(t->encoding))
        ...
        bool set_encoding(tgfx::TextureEncoding value) {
            tc_texture* t = get();
            return t && tc_texture_set_encoding(t, tgfx::to_tc_texture_encoding(value));
        }
