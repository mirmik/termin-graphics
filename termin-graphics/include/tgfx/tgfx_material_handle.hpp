...
namespace termin {
    ...
    public:
        ...
        explicit TcMaterial(tc_material_handle h)
            : handle(h) {
        ...
        TcMaterial(const TcMaterial& other)
            : handle(other.handle) {
            if (tc_material* m = tc_material_get(handle)) {
                tc_material_add_ref(m);
            }
        ...
        TcMaterial(TcMaterial&& other) noexcept
            : handle(other.handle) {
        ...
        tc_material* get() const {
            return tc_material_get(handle);
        }

        bool is_valid() const {
            return tc_material_is_valid(handle);
        }
        ...
        tc_material_phase* add_phase_from_sources(const TcMaterialPhaseFromSourcesInfo& info) {
            ...
            tc_material_phase* phase = tc_material_add_phase(m, shader.handle, info.phase_mark.c_str(), info.priority);
            if (phase) {
        ...
        void set_uniform_vec3(const char* name, const Vec3& v) {
            tc_material* m = get();
            if (m) {
                const Vec3f value{static_cast<float>(v.x), static_cast<float>(v.y), static_cast<float>(v.z)};
        ...
        TcShader get_phase_shader(size_t phase_index) const {
            tc_material* m = get();
            if (!m || phase_index >= m->phase_count)
                return TcShader();
        ...
        tc_value serialize_to_value() const {
            ...
            tc_value_dict_set(&d, "kind", tc_value_string("tc_material"));
            return d;
        }
