...
int main() {
    ...
    const auto identity_only = select_swapchain_pre_transform(
    ...
    const auto rotated_with_identity = select_swapchain_pre_transform(
    ...
    const auto rotated_without_identity = select_swapchain_pre_transform(
        VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR);
    ...
    if (swapchain_result_requires_recreate(VK_SUCCESS) ||
        swapchain_result_requires_recreate(VK_SUBOPTIMAL_KHR) ||
