...
static tc_shader_resource_binding
make_plan_test_binding(const char* name, uint32_t kind, uint32_t scope, uint32_t binding, uint32_t stage_mask) {
...
TEST_CASE("tgfx2 backend names and compiled availability are queryable") {
    ...
    const bool any_backend_compiled =
        tgfx::backend_is_compiled(tgfx::BackendType::OpenGL) || tgfx::backend_is_compiled(tgfx::BackendType::Vulkan) ||
        tgfx::backend_is_compiled(tgfx::BackendType::D3D11) || tgfx::backend_is_compiled(tgfx::BackendType::WebGPU);
...
TEST_CASE("tgfx2 adapter classes identify software devices") {
    ...
    CHECK(std::string(tgfx::adapter_class_name(hardware.hardware_class)) == "integrated-gpu");
}

#ifdef TGFX2_HAS_VULKAN
TEST_CASE("tgfx2 Vulkan device types map to stable adapter classes") {
    CHECK(tgfx::classify_vulkan_adapter(VK_PHYSICAL_DEVICE_TYPE_CPU) == tgfx::AdapterClass::Cpu);
    CHECK(tgfx::classify_vulkan_adapter(VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) == tgfx::AdapterClass::DiscreteGpu);
    CHECK(tgfx::classify_vulkan_adapter(VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU) == tgfx::AdapterClass::IntegratedGpu);
    CHECK(tgfx::classify_vulkan_adapter(VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU) == tgfx::AdapterClass::VirtualGpu);
    CHECK(tgfx::classify_vulkan_adapter(VK_PHYSICAL_DEVICE_TYPE_OTHER) == tgfx::AdapterClass::Unknown);
}
#endif
...
TEST_CASE("tgfx2 shader artifact paths are backend aware") {
    ...
    fs::path root = fs::temp_directory_path() / ("termin_tgfx2_artifacts_" + std::to_string(unique));
    ...
    std::string path;
    CHECK(
        termin::tgfx2_shader_artifact_path("shader-uuid", tgfx::BackendType::Vulkan, tgfx::ShaderStage::Vertex, path));
    CHECK(path == root_str + "/shaders/vulkan/shader-uuid.vert.spv");

    CHECK(termin::tgfx2_shader_artifact_path(
        "shader-uuid", tgfx::BackendType::OpenGL, tgfx::ShaderStage::Fragment, path));
    CHECK(path == root_str + "/shaders/opengl/shader-uuid.frag.glsl");

    CHECK(termin::tgfx2_shader_artifact_path("shader-uuid", tgfx::BackendType::D3D11, tgfx::ShaderStage::Vertex, path));
    ...
    CHECK(termin::tgfx2_load_shader_artifact_for_backend(
        "shader-uuid", tgfx::BackendType::Vulkan, tgfx::ShaderStage::Fragment, bytes));
    CHECK(bytes.size() == 3);
...
TEST_CASE("legacy shader resolver discovers installed artifacts beside built-in sources") {
    ...
    const auto unique = std::chrono::steady_clock::now().time_since_epoch().count();
    const fs::path root = fs::temp_directory_path() / ("termin_tgfx2_installed_artifacts_" + std::to_string(unique));
    ...
    ScopedTestEnvironmentVariable explicit_artifact_root("TERMIN_SHADER_ARTIFACT_ROOT");
    ScopedTestEnvironmentVariable explicit_builtin_root("TERMIN_BUILTIN_SHADER_ROOT");
    ...
    std::string path;
    CHECK(termin::tgfx2_shader_artifact_path(
        "installed-shader", tgfx::BackendType::D3D11, tgfx::ShaderStage::Vertex, path));
    CHECK(fs::path(path) == artifact_root / "shaders" / "d3d11" / "installed-shader.vs.cso");
...
TEST_CASE("shader artifact resolvers isolate runtime roots") {
    ...
    REQUIRE(termin::tgfx2_shader_artifact_path(
        first, "shared-shader", tgfx::BackendType::Vulkan, tgfx::ShaderStage::Fragment, first_path));
    REQUIRE(termin::tgfx2_shader_artifact_path(
        second, "shared-shader", tgfx::BackendType::Vulkan, tgfx::ShaderStage::Fragment, second_path));
...
TEST_CASE("backend binding plan separates semantic resources from backend placement") {
    tc_shader_resource_binding binding = make_plan_test_binding(
        "material", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 1, TC_SHADER_STAGE_FRAGMENT);
    ...
    std::string error;
    REQUIRE(tgfx::build_backend_binding_plan(tgfx::BackendType::Vulkan, &binding, 1, vulkan_plan, &error));
    ...
    CHECK(vulkan_plan.entries[0].placement.vulkan.descriptor_kind == tgfx::BackendDescriptorKind::UniformBuffer);

    tgfx::BackendBindingPlan d3d11_plan;
    REQUIRE(tgfx::build_backend_binding_plan(tgfx::BackendType::D3D11, &binding, 1, d3d11_plan, &error));
    ...
    REQUIRE(tgfx::build_backend_binding_plan(tgfx::BackendType::OpenGL, &binding, 1, opengl_plan, &error));
    ...
    CHECK(opengl_plan.entries[0].placement.opengl.binding_class == tgfx::OpenGLBindingClass::UniformBuffer);
    ...
    REQUIRE(tgfx::build_backend_binding_plan(tgfx::BackendType::WebGPU, &binding, 1, webgpu_plan, &error));
...
TEST_CASE("bound resource group views preserve dirty scopes and owned storage") {
    ...
    grouped_entry.placement.opengl.binding_class = tgfx::OpenGLBindingClass::UniformBuffer;
    ...
    storage.append_group(tgfx::ShaderResourceScope::Frame, true, &frame_binding, 1);
    storage.append_group(tgfx::ShaderResourceScope::Material, false, &clean_binding, 1);
    ...
    tgfx::for_each_dirty_bound_resource_binding(grouped_desc, [&](const tgfx::BoundResourceBinding& binding) {
    ...
    const tgfx::BoundResourceGroupView source_group = {
        tgfx::ShaderResourceScope::Frame,
        true,
        &mutable_source,
        1,
    };
...
TEST_CASE("backend binding plan validates D3D11 register class and stage conflicts") {
    ...
    bindings[1] = make_plan_test_binding(
        "draw_data", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_DRAW, 2, TC_SHADER_STAGE_FRAGMENT);
    ...
    std::string error;
    CHECK(!tgfx::build_backend_binding_plan(tgfx::BackendType::D3D11, bindings, 2, plan, &error));
...
TEST_CASE("backend binding plan validates missing D3D11 placement and class-separated slots") {
    tc_shader_resource_binding bindings[2]{};
    bindings[0] = make_plan_test_binding(
        "material", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 2, TC_SHADER_STAGE_FRAGMENT);

    tgfx::BackendBindingPlan plan;
    std::string error;
    CHECK(!tgfx::build_backend_binding_plan(tgfx::BackendType::D3D11, bindings, 1, plan, &error));
    ...
    bindings[1] = make_plan_test_binding(
        "albedo", TC_SHADER_RESOURCE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 2, TC_SHADER_STAGE_FRAGMENT);
    bindings[1].has_d3d11_placement = 1;
    bindings[1].d3d11.register_class = TC_SHADER_D3D11_REGISTER_T;
...
TEST_CASE("backend binding plan maps D3D11 storage buffers to shader-resource registers") {
    tc_shader_resource_binding binding = make_plan_test_binding("foliage_instances",
    ...
    tgfx::BackendBindingPlan plan;
    std::string error;
    REQUIRE(tgfx::build_backend_binding_plan(tgfx::BackendType::D3D11, &binding, 1, plan, &error));
    ...
    CHECK(!tgfx::build_backend_binding_plan(tgfx::BackendType::D3D11, &binding, 1, plan, &error));
    CHECK(error.find("register class") != std::string::npos);
}

TEST_CASE("backend binding plan rejects D3D11 storage textures until UAV binding exists") {
    tc_shader_resource_binding binding = make_plan_test_binding(
        "output_image", TC_SHADER_RESOURCE_STORAGE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_PASS, 0, TC_SHADER_STAGE_FRAGMENT);
    binding.has_d3d11_placement = 1;
    binding.d3d11.register_class = TC_SHADER_D3D11_REGISTER_U;
...
TEST_CASE("backend binding plan rejects unsupported Vulkan descriptor sets") {
    tc_shader_resource_binding binding = make_plan_test_binding(
        "per_frame", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_FRAME, 2, TC_SHADER_STAGE_VERTEX);
    binding.set = 1;

    tgfx::BackendBindingPlan plan;
    std::string error;
    CHECK(!tgfx::build_backend_binding_plan(tgfx::BackendType::Vulkan, &binding, 1, plan, &error));
...
TEST_CASE("backend binding plan validates Vulkan and OpenGL placement conflicts") {
    ...
    bindings[0] = make_plan_test_binding(
        "material", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 1, TC_SHADER_STAGE_FRAGMENT);
    bindings[1] = make_plan_test_binding(
        "draw_data", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_DRAW, 1, TC_SHADER_STAGE_FRAGMENT);

    tgfx::BackendBindingPlan plan;
    std::string error;
    CHECK(!tgfx::build_backend_binding_plan(tgfx::BackendType::OpenGL, bindings, 2, plan, &error));
    CHECK(error.find("OpenGL binding plan conflict") != std::string::npos);

    bindings[1] = make_plan_test_binding(
        "normal_map", TC_SHADER_RESOURCE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 1, TC_SHADER_STAGE_FRAGMENT);
    CHECK(!tgfx::build_backend_binding_plan(tgfx::BackendType::Vulkan, bindings, 2, plan, &error));
    CHECK(error.find("Vulkan binding plan conflict") != std::string::npos);

    bindings[0] = make_plan_test_binding(
        "albedo", TC_SHADER_RESOURCE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 3, TC_SHADER_STAGE_FRAGMENT);
    bindings[1] = make_plan_test_binding(
        "normal_map", TC_SHADER_RESOURCE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 3, TC_SHADER_STAGE_FRAGMENT);
    CHECK(!tgfx::build_backend_binding_plan(tgfx::BackendType::OpenGL, bindings, 2, plan, &error));
    CHECK(error.find("OpenGL binding plan conflict") != std::string::npos);
}
...
TEST_CASE("backend binding plan rejects incompatible duplicate semantic names") {
    ...
    bindings[0] = make_plan_test_binding(
        "material", TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 1, TC_SHADER_STAGE_FRAGMENT);
    bindings[1] = make_plan_test_binding(
        "material", TC_SHADER_RESOURCE_TEXTURE, TC_SHADER_RESOURCE_SCOPE_MATERIAL, 2, TC_SHADER_STAGE_FRAGMENT);
...
TEST_CASE("static uuid registration preserves non-default shader identity") {
    ...
    tc_shader_handle first = tc_shader_register_static_uuid_ex(vs,
                                                               fs,
                                                               nullptr,
    ...
    tc_shader_handle second = tc_shader_register_static_uuid_ex(vs,
                                                                fs,
                                                                nullptr,
                                                                "static_slang_identity_test",
                                                                "static-slang-identity-test",
...
TEST_CASE("tc_shader identity hash separates source languages") {
    ...
    const tc_shader_create_desc glsl_desc = {{vs, fs, nullptr, "identity_glsl", nullptr, nullptr, nullptr, nullptr},
                                             nullptr,
                                             TC_SHADER_LANGUAGE_GLSL,
                                             TC_SHADER_ARTIFACT_OPTIONAL};
    tc_shader_handle glsl = tc_shader_from_sources_desc(&glsl_desc);
    const tc_shader_create_desc slang_desc = {{vs, fs, nullptr, "identity_slang", nullptr, nullptr, nullptr, nullptr},
                                              nullptr,
                                              TC_SHADER_LANGUAGE_SLANG,
                                              TC_SHADER_ARTIFACT_REQUIRED};
    tc_shader_handle slang = tc_shader_from_sources_desc(&slang_desc);
...
TEST_CASE("tgfx2 shader runtime lazily compiles stale artifacts in dev mode") {
    ...
    const fs::path root = fs::temp_directory_path() / ("termin_tgfx2_lazy_shader_" + std::to_string(unique));
...
float4 main() : SV_Target {
    ...
    CHECK(termin::tgfx2_load_or_compile_shader_artifact_for_backend(
        shader, tgfx::BackendType::Vulkan, tgfx::ShaderStage::Vertex, bytes));
    CHECK(bytes == std::vector<uint8_t>({'S', 'P', 'I', 'R', 'V'}));

    const std::string actual_shader_uuid = shader->uuid;
    const fs::path artifact = artifact_root / "shaders" / "vulkan" / (actual_shader_uuid + ".vert.spv");
    const fs::path metadata = fs::path(artifact.string() + ".artifact");
    const fs::path legacy_metadata = fs::path(artifact.string() + ".meta");
    const fs::path source = cache_root / "source" / (actual_shader_uuid + ".vert.slang");
    ...
    CHECK(read_test_text_file(fs::path(artifact.string() + ".layout.json")).find("\"compile_count\":2") !=
          std::string::npos);
...
TEST_CASE("tgfx2 engine shader artifact metadata invalidates stale layout schema") {
    ...
    const auto unique = std::chrono::steady_clock::now().time_since_epoch().count();
    const fs::path root = fs::temp_directory_path() / ("termin_tgfx2_engine_schema_" + std::to_string(unique));
    const fs::path artifact_root = root / "assets";
    const fs::path cache_root = root / "cache";
    const fs::path include_root = root / "include";
    const fs::path compiler = root / "fake_termin_shaderc.sh";
    ...
    const tgfx::EngineShaderStageSource& shader = tgfx::engine_fullscreen_quad_vertex_shader();
    ...
    setenv("TERMIN_BUILTIN_SHADER_ROOT", include_root.string().c_str(), 1);
    const fs::path artifact = artifact_root / "shaders" / "vulkan" / (std::string(shader.uuid) + ".vert.spv");
    const fs::path metadata = fs::path(artifact.string() + ".artifact");
    ...
    REQUIRE(termin::tgfx2_load_or_compile_engine_shader_stage_artifact_for_backend(
        shader, tgfx::BackendType::Vulkan, bytes));
...
TEST_CASE("tgfx2 shader runtime loads D3D11 resource placement sidecar") {
    ...
    const fs::path root = fs::temp_directory_path() / ("termin_tgfx2_d3d11_layout_sidecar_" + std::to_string(unique));
    const fs::path artifact_root = root / "assets";
    ...
    }
    {
        ...
        out << "{\n"
            ...
            << "\"scope\": \"material\", \"set\": 0, \"binding\": 1, " << "\"stage_mask\": 2, \"size\": 16, "
            << "\"d3d11\": {\"register_class\": \"b\", \"register_index\": 1}},\n"
            << "    {\"name\": \"albedo_texture\", \"kind\": \"texture\", "
            << "\"scope\": \"material\", \"set\": 0, \"binding\": 4, " << "\"stage_mask\": 2, \"size\": 0, "
    ...
    REQUIRE(termin::tgfx2_load_or_compile_shader_artifact_for_backend(
        shader, tgfx::BackendType::D3D11, tgfx::ShaderStage::Fragment, bytes));

    const tc_shader_resource_binding* material = tc_shader_find_resource_binding(shader, TC_SHADER_RESOURCE_MATERIAL);
    ...
    const tc_shader_resource_binding* texture = tc_shader_find_resource_binding(shader, "albedo_texture");
    ...
    tgfx::BackendBindingPlan plan;
    std::string error;
    REQUIRE(tgfx::build_backend_binding_plan(tgfx::BackendType::D3D11,
    ...
    const auto texture_entry =
        std::find_if(plan.entries.begin(), plan.entries.end(), [](const tgfx::BackendBindingPlanEntry& entry) {
...
TEST_CASE("tgfx2 shader runtime loads resource layout sidecar") {
    ...
    const fs::path root = fs::temp_directory_path() / ("termin_tgfx2_layout_sidecar_" + std::to_string(unique));
    const fs::path artifact_root = root / "assets";
    const fs::path cache_root = root / "cache";
    const fs::path compiler = root / "fake_termin_shaderc.sh";
...

    {
        ...
        out << "#!/bin/sh\n"
            ...
            << "  \"stage\": \"fragment\",\n"
            << "  \"resources\": [\n"
            << "    {\"name\": \"material\", \"kind\": \"constant_buffer\", \"scope\": \"material\", \"set\": 2, "
               "\"binding\": 7, \"stage_mask\": 2, \"size\": 16}\n"
            << "  ]\n"
            << "}\n"
            << "JSON\n";
    }
...
float4 main() : SV_Target {
    ...
    std::vector<uint8_t> bytes;
    CHECK(termin::tgfx2_load_or_compile_shader_artifact_for_backend(
        shader, tgfx::BackendType::Vulkan, tgfx::ShaderStage::Fragment, bytes));

    const tc_shader_resource_binding* binding = tc_shader_find_resource_binding(shader, TC_SHADER_RESOURCE_MATERIAL);
