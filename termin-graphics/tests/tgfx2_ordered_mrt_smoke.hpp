...
namespace tgfx::tests
{
    ...
    bool run_ordered_mrt_smoke(IRenderDevice& device,
                               ...
                               Draw&& draw)
    {
        ...
        depth_desc.usage =
            TextureUsage::DepthStencilAttachment | TextureUsage::CopySrc;
        ...
        for (size_t i = 0; i < targets.size(); ++i)
        {
            ...
            std::memcpy(pass.colors[i].clear_color,
                        clear_colors[i],
        ...
        auto read_expected = [&](const float expected[3][4])
        {
            ...
            for (size_t i = 0; i < targets.size(); ++i)
            {
                matches = device.read_pixel_rgba8(
                              targets[i], kWidth / 2, kHeight / 2, pixels[i]) &&
                          pixel_matches(pixels[i],
                                        expected[i][0],
                                        expected[i][1],
                                        expected[i][2]) &&
                          matches;
        ...
        const bool clear_ok =
            run_pass(false, false) && read_expected(clear_colors);
        for (ColorAttachmentDesc& color : pass.colors)
        {
            color.load = LoadOp::Load;
        }
        pass.depth.load = LoadOp::Load;
        const bool preserved_loads =
            run_pass(true, true) && read_expected(clear_colors);
        ...
        const bool draw_ok =
            run_pass(true, false) && read_expected(draw_colors);

        std::printf(
            "%.*s ordered MRT clear: %s, shared-depth preserved load: %s, "
            "SV_Target routing: %s\n",
            static_cast<int>(backend.size()),
            backend.data(),
            clear_ok ? "ok" : "failed",
