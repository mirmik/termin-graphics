...
namespace {
    ...
    public:
        bool draw(tgfx::RenderContext2&, const tgfx::RetainedDrawState2D&) override {
            return true;
        }
    };
    ...
    tgfx::DrawList2D make_list() {
        ...
        assert(builder.push_transform(termin::Affine2f::translation(10, 20) * termin::Affine2f::shear(0.25f, -0.1f)));
        ...
        const tgfx::DrawVertex2D triangle[] = {{{0, 0}, {0, 0}}, {{10, 0}, {1, 0}}, {{0, 10}, {0, 1}}};
        assert(builder.custom_batch(triangle, {1, 0, 0, 1}, tgfx::TextureHandle{29}));
    ...
    std::string fingerprint(const tgfx::DrawList2D& list) {
        ...
        for (const auto& command : list.commands()) {
            out << command.index() << ':';
            if (const auto* transform = std::get_if<tgfx::PushTransform2D>(&command)) {
                out << transform->transform.m00 << ',' << transform->transform.m01 << ',' << transform->transform.m10
                    << ',' << transform->transform.m11 << ',' << transform->transform.tx << ','
                    << transform->transform.ty;
            } else if (const auto* clip = std::get_if<tgfx::PushClip2D>(&command)) {
                out << clip->path.verbs().size() << ',' << clip->path.points().size() << ','
                    << static_cast<int>(clip->rule);
            } else if (const auto* text = std::get_if<tgfx::DrawText2D>(&command)) {
                out << text->text << ',' << text->font.id;
            } else if (const auto* image = std::get_if<tgfx::DrawImage2D>(&command)) {
                out << image->texture.id << ',' << image->rect.width;
            } else if (const auto* custom = std::get_if<tgfx::DrawCustomBatch2D>(&command)) {
                out << custom->vertices.size() << ',' << custom->texture.id;
            } else if (const auto* retained = std::get_if<tgfx::DrawRetainedBatch2D>(&command)) {
...
int main() {
    ...
    const auto* transform = std::get_if<tgfx::PushTransform2D>(&first.commands()[0]);
    assert(transform && transform->transform.m01 == 0.25f);
    const auto* outer_clip = std::get_if<tgfx::PushClip2D>(&first.commands()[2]);
    const auto* inner_clip = std::get_if<tgfx::PushClip2D>(&first.commands()[4]);
    assert(outer_clip && inner_clip);
    assert(outer_clip->path.points().size() == 4);
    assert(inner_clip->rule == tgfx::FillRule::EvenOdd);

    const auto* text = std::get_if<tgfx::DrawText2D>(&first.commands()[10]);
    const auto* image = std::get_if<tgfx::DrawImage2D>(&first.commands()[11]);
    ...
    assert(std::holds_alternative<tgfx::PushTransform2D>(frozen_composed->commands().front()));
    assert(std::holds_alternative<tgfx::PopTransform2D>(frozen_composed->commands().back()));
    assert(fingerprint(first) == fingerprint(second));
