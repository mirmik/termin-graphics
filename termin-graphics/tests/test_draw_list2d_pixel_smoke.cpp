...
void configure_shader_tools(const char* executable) {
    ...
    if (!error) {
        const auto compiler =
            executable_path.parent_path() / "termin_shaderc";
        if (std::filesystem::is_regular_file(compiler, error)) {
            termin::tgfx2_set_shader_compiler_path(
                compiler.string().c_str());
        }
    }
}

bool red(const float pixel[4]) {
    return pixel[0] > 0.8f && pixel[1] < 0.1f &&
           pixel[2] < 0.1f && pixel[3] > 0.9f;
}

bool clear(const float pixel[4]) {
    return pixel[0] < 0.05f && pixel[1] < 0.05f &&
           pixel[2] < 0.05f && pixel[3] > 0.9f;
}
...
int run(const char* executable) {
    ...
    termin::tgfx2_set_shader_cache_root(
        (artifacts.path / "cache").string().c_str());
    ...
    assert(builder.push_transform(
        termin::Affine2f::translation(32, 32) *
        termin::Affine2f::rotation(pi * 0.25f)));
    ...
    const bool read =
        device->read_pixel_rgba8(target, 32, 32, center) &&
    ...
    const bool blue_control =
        immediate_control[0] < 0.1f && immediate_control[1] < 0.1f &&
    ...
    if (!result) {
        std::fprintf(
            ...
            inside_tip[0], inside_tip[1], inside_tip[2], inside_tip[3],
            outside_corner[0], outside_corner[1],
            outside_corner[2], outside_corner[3]);
        std::fprintf(
            stderr, "immediate=(%.2f %.2f %.2f %.2f)\n",
            immediate_control[0], immediate_control[1],
            immediate_control[2], immediate_control[3]);
    }
    canvas.release_gpu();
