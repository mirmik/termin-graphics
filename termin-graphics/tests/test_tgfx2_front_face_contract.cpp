...
int main() {
    ...
    if (tgfx::gl::to_gl_front_face(tgfx::FrontFace::CCW) != GL_CCW ||
        tgfx::gl::to_gl_front_face(tgfx::FrontFace::CW) != GL_CW) {
        std::fprintf(stderr, "OpenGL logical front-face mapping regressed\n");
        return 1;
    }

#ifdef TGFX2_HAS_VULKAN
    if (tgfx::vk::to_vk_front_face(tgfx::FrontFace::CCW) != VK_FRONT_FACE_CLOCKWISE ||
        tgfx::vk::to_vk_front_face(tgfx::FrontFace::CW) != VK_FRONT_FACE_COUNTER_CLOCKWISE) {
