...
// Global registry of all GPU contexts (for debug introspection)
static tc_gpu_context* g_all_contexts[TC_MAX_GPU_CONTEXTS];
static int g_all_context_count = 0;
...
tc_gpu_context* tc_gpu_context_new(uintptr_t key, tc_gpu_share_group* group) {
    ...
    if (g_all_context_count < TC_MAX_GPU_CONTEXTS) {
        g_all_contexts[g_all_context_count++] = ctx;
    } else {
        tc_log(TC_LOG_ERROR, "tc_gpu_context_new: context registry full (%d)", TC_MAX_GPU_CONTEXTS);
    }
...
void tc_gpu_context_free(tc_gpu_context* ctx) {
    ...
    for (int i = 0; i < g_all_context_count; i++) {
        if (g_all_contexts[i] == ctx) {
            g_all_contexts[i] = g_all_contexts[g_all_context_count - 1];
            g_all_contexts[g_all_context_count - 1] = NULL;
            g_all_context_count--;
            break;
        }
    }
...
void tc_ensure_default_gpu_context(void) {
    if (!g_current_gpu_context) {
        if (!g_default_gpu_context) {
            ...
            tc_gpu_context_set_name(g_default_gpu_context, "default");
        }
...
void tc_gpu_context_set_name(tc_gpu_context* ctx, const char* name) {
    if (!ctx) return;
    if (name) {
...
bool tc_gpu_get_mesh_info(uint32_t pool_index, tc_mesh_gpu_info* out) {
    if (!out) return false;
    memset(out, 0, sizeof(*out));
    out->pool_index = pool_index;
    out->shared_version = -1;

    // Find share_group from any context
    tc_gpu_share_group* sg = NULL;
    if (g_all_context_count > 0 && g_all_contexts[0]) {
        sg = g_all_contexts[0]->share_group;
    }
    if (!sg) return false;

    // Shared data
    if (pool_index < sg->mesh_data_capacity) {
        tc_gpu_mesh_data_slot* slot = &sg->mesh_data[pool_index];
        out->vbo = slot->vbo;
        out->ebo = slot->ebo;
        out->shared_version = slot->version;
    }

    // Per-context VAOs
    out->context_count = 0;
    for (int i = 0; i < g_all_context_count && i < TC_MAX_GPU_CONTEXTS; i++) {
        tc_gpu_context* ctx = g_all_contexts[i];
        if (!ctx) continue;

        tc_mesh_context_vao_info* ci = &out->contexts[out->context_count];
        ci->context_key = ctx->key;
        ci->context_name = ctx->name[0] ? ctx->name : NULL;
        ci->vao = 0;
        ci->bound_vbo = 0;
        ci->bound_ebo = 0;
        ci->vao_stale = 0;

        if (pool_index < ctx->mesh_vao_capacity) {
            tc_gpu_vao_slot* vs = &ctx->mesh_vaos[pool_index];
            ci->vao = vs->vao;
            ci->bound_vbo = vs->bound_vbo;
            ci->bound_ebo = vs->bound_ebo;
            ci->vao_stale = (vs->bound_vbo != out->vbo || vs->bound_ebo != out->ebo) ? 1 : 0;
        }
...
int tc_gpu_context_count(void) {
    return g_all_context_count;
}

uintptr_t tc_gpu_context_key_at(int index) {
    if (index < 0 || index >= g_all_context_count) return 0;
    return g_all_contexts[index] ? g_all_contexts[index]->key : 0;
}
