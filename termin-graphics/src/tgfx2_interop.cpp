...
tc_shader_handle tgfx2_interop_register_builtin_shader(const char* shader_uuid) {
    if (!shader_uuid || shader_uuid[0] == '\0') {
        tc_log(TC_LOG_ERROR, "tgfx2_interop_register_builtin_shader: shader UUID is required");
        return tc_shader_handle_invalid();
    }
    if (!tgfx2_interop_get_device()) {
        tc_log(TC_LOG_ERROR, "tgfx2_interop_register_builtin_shader: no current graphics device");
        return tc_shader_handle_invalid();
    }
...
namespace {
    ...
    bool
    tgfx2_read_pixel(tgfx::D3D11RenderDevice& device, tgfx::TextureHandle texture, int x, int y, float out_rgba[4]) {
        out_rgba[0] = out_rgba[1] = out_rgba[2] = out_rgba[3] = -1.0f;
        return texture && device.read_pixel_rgba8(texture, x, y, out_rgba);
    }

    void tgfx2_log_pixel_probe(tgfx::D3D11RenderDevice& device,
                               uintptr_t bridge_id,
                               uint64_t frame,
                               const char* phase,
    ...
    public:
        D3D11D3DImageBridge(tgfx::D3D11RenderDevice& device, uint32_t width, uint32_t height)
            : device_(device),
              d3d9_runtime_(shared_d3d9_image_runtime()) {
            resize(width, height);
        }
        ...
        bool resize(uint32_t width, uint32_t height) {
            ...
            HRESULT hr = device_.native_device()->CreateTexture2D(&d3d11_desc, nullptr, d3d11_texture_.GetAddressOf());
            if (FAILED(hr)) {
            ...
            hr = d3d9_runtime_->device()->CreateTexture(width_,
                                                        height_,
                                                        1,
        ...
        bool present(tgfx::TextureHandle source) {
            ...
            if (log_frame) {
                tc_log(TC_LOG_INFO,
                       "[D3DImage:%p] frame=%llu present src=%u src_size=%ux%u src_fmt=%d src_samples=%u dst=%u "
                       "dst_size=%ux%u",
                       this,
            ...
            if (probe && log_frame) {
                tgfx2_log_pixel_probe(
                    device_, reinterpret_cast<uintptr_t>(this), present_count_, "src-before", source, src_desc);
            }

            device_.blit_to_texture(
                shared_texture_handle_,
                source,
                termin::Bounds2i::from_size(static_cast<int>(src_desc.width), static_cast<int>(src_desc.height)),
...
int tgfx2_interop_claim_device(void* device, const void* owner) {
    ...
    tc_log_error("[tgfx2_interop] refusing a second application graphics device "
...
int tgfx2_interop_release_device(void* device, const void* owner) {
    ...
    std::lock_guard<std::mutex> lock(g_tgfx2_device_mutex);
    if (g_tgfx2_device != device || g_tgfx2_device_owner != owner) {
        tc_log_error("[tgfx2_interop] refusing device release by a non-owner "
                     "(active=%p owner=%p, requested=%p owner=%p)",
                     g_tgfx2_device,
                     g_tgfx2_device_owner,
                     device,
...
uint32_t tgfx2_interop_register_external_gl_texture(
    ...
    if (gl_tex_id == 0 || width == 0 || height == 0) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] invalid external GL texture: id=%u size=%ux%u", gl_tex_id, width, height);
        return 0;
    }

    auto* device = static_cast<tgfx::IRenderDevice*>(tgfx2_interop_get_device());
    if (!device) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] cannot register external GL texture: no active tgfx2 device");
        return 0;
    }
    ...
    try {
        tgfx::TextureHandle handle = device->register_external_texture(static_cast<uintptr_t>(gl_tex_id), desc);
        return handle.id;
    } catch (const std::exception& e) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to register external GL texture: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to register external GL texture: unknown error");
    }
    return 0;
}
...
void tgfx2_interop_destroy_texture_handle(uint32_t handle_id) {
    ...
    auto* device = static_cast<tgfx::IRenderDevice*>(tgfx2_interop_get_device());
    if (!device) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] cannot destroy texture handle %u: no active tgfx2 device", handle_id);
        return;
    }
    ...
    try {
        ...
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to destroy texture handle %u: %s", handle_id, e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to destroy texture handle %u: unknown error", handle_id);
    }
}

void tgfx2_interop_blit_texture(uint32_t src_handle_id, uint32_t dst_handle_id, int width, int height) {
    ...
    auto* device = static_cast<tgfx::IRenderDevice*>(tgfx2_interop_get_device());
    if (!device) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] cannot blit texture: no active tgfx2 device");
        return;
    }

    try {
        device->blit_to_texture(tgfx::TextureHandle{dst_handle_id},
                                tgfx::TextureHandle{src_handle_id},
                                termin::Bounds2i::from_size(width, height),
                                termin::Bounds2i::from_size(width, height));
    } catch (const std::exception& e) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to blit texture: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to blit texture: unknown error");
    }
}

void* tgfx2_interop_create_d3d11_swapchain(void* hwnd, uint32_t width, uint32_t height) {
#ifdef _WIN32
    if (!hwnd || width == 0 || height == 0) {
        tc_log(
            TC_LOG_ERROR, "[tgfx2_interop] invalid D3D11 swapchain request: hwnd=%p size=%ux%u", hwnd, width, height);
        return nullptr;
    }

    auto* device = static_cast<tgfx::IRenderDevice*>(tgfx2_interop_get_device());
    if (!device) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] cannot create D3D11 swapchain: no active tgfx2 device");
        return nullptr;
    }
    if (device->backend_type() != tgfx::BackendType::D3D11) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] cannot create D3D11 swapchain: active backend is not D3D11");
        return nullptr;
    }

    auto* d3d_device = dynamic_cast<tgfx::D3D11RenderDevice*>(device);
    if (!d3d_device) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] active tgfx2 device reports D3D11 but is not D3D11RenderDevice");
        return nullptr;
    }

    try {
        return new tgfx::D3D11Swapchain(*d3d_device, static_cast<HWND>(hwnd), width, height);
    } catch (const std::exception& e) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to create D3D11 swapchain: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to create D3D11 swapchain: unknown error");
    }
    return nullptr;
#else
    (void)hwnd;
    (void)width;
    (void)height;
    tc_log(TC_LOG_ERROR, "[tgfx2_interop] D3D11 swapchain creation is only supported on Windows");
    return nullptr;
#endif
}
...
int tgfx2_interop_resize_d3d11_swapchain(void* swapchain, uint32_t width, uint32_t height) {
#ifdef _WIN32
    ...
    try {
        ...
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to resize D3D11 swapchain: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to resize D3D11 swapchain: unknown error");
    }
    return 0;
#else
    (void)swapchain;
    (void)width;
    (void)height;
    tc_log(TC_LOG_ERROR, "[tgfx2_interop] D3D11 swapchain resize is only supported on Windows");
    return 0;
#endif
}

int tgfx2_interop_present_d3d11_swapchain(void* swapchain, uint32_t source_handle_id, uint32_t sync_interval) {
#ifdef _WIN32
    ...
    try {
        return static_cast<tgfx::D3D11Swapchain*>(swapchain)->compose_and_present(tgfx::TextureHandle{source_handle_id},
                                                                                  sync_interval)
                   ? 1
                   : 0;
    } catch (const std::exception& e) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to present D3D11 swapchain: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to present D3D11 swapchain: unknown error");
    }
    return 0;
#else
    (void)swapchain;
    (void)source_handle_id;
    (void)sync_interval;
    tc_log(TC_LOG_ERROR, "[tgfx2_interop] D3D11 swapchain present is only supported on Windows");
    return 0;
#endif
}
void* tgfx2_interop_create_d3d11_d3dimage_bridge(uint32_t width, uint32_t height) {
#ifdef _WIN32
    if (width == 0 || height == 0) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] invalid D3DImage bridge request: size=%ux%u", width, height);
        return nullptr;
    }

    auto* device = static_cast<tgfx::IRenderDevice*>(tgfx2_interop_get_device());
    if (!device) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] cannot create D3DImage bridge: no active tgfx2 device");
        return nullptr;
    }
    if (device->backend_type() != tgfx::BackendType::D3D11) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] cannot create D3DImage bridge: active backend is not D3D11");
        return nullptr;
    }
    ...
    try {
        ...
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to create D3DImage bridge: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to create D3DImage bridge: unknown error");
    }
    return nullptr;
#else
    (void)width;
    (void)height;
    tc_log(TC_LOG_ERROR, "[tgfx2_interop] D3DImage bridge creation is only supported on Windows");
    return nullptr;
#endif
}
...
int tgfx2_interop_resize_d3d11_d3dimage_bridge(void* bridge, uint32_t width, uint32_t height) {
#ifdef _WIN32
    ...
    try {
        ...
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to resize D3DImage bridge: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to resize D3DImage bridge: unknown error");
    }
    return 0;
#else
    (void)bridge;
...
int tgfx2_interop_present_d3d11_d3dimage_bridge(void* bridge, uint32_t source_handle_id) {
#ifdef _WIN32
    ...
    try {
        ...
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to present D3DImage bridge: %s", e.what());
    } catch (...) {
        tc_log(TC_LOG_ERROR, "[tgfx2_interop] failed to present D3DImage bridge: unknown error");
    }
    return 0;
#else
    (void)bridge;
