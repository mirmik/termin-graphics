...
namespace tgfx::vk {
    ...
    VkBufferUsageFlags to_vk_buffer_usage(BufferUsage usage) {
        ...
        if (has_flag(usage, BufferUsage::CopySrc))
            flags |= VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
        if (has_flag(usage, BufferUsage::CopyDst))
    ...
    VkImageUsageFlags to_vk_image_usage(TextureUsage usage) {
        ...
        if (has_flag(usage, TextureUsage::DepthStencilAttachment))
            flags |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        if (has_flag(usage, TextureUsage::CopySrc))
            flags |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
        if (has_flag(usage, TextureUsage::CopyDst))
