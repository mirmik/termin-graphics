...
namespace tgfx {
    ...
    TextureHandle VulkanRenderDevice::ensure_tc_texture(tc_texture* tex) {
        ...
        if (gpu_first) {
            if (tex->mipmap) {
                tc_log(TC_LOG_ERROR,
                       "VulkanRenderDevice::ensure_tc_texture: GPU-first texture '%s' requests mipmaps without a "
                       "source chain",
                       tex->header.name ? tex->header.name : tex->header.uuid);
                return {};
            }
    ...
    std::pair<BufferHandle, BufferHandle> VulkanRenderDevice::ensure_tc_mesh(tc_mesh* mesh) {
        ...
        const size_t vb_size = mesh->vertex_count * static_cast<size_t>(mesh->layout.stride);
    ...
    bool VulkanRenderDevice::ensure_tc_shader(tc_shader* shader, ShaderHandle* out_vs, ShaderHandle* out_fs) {
        if (!shader) {
