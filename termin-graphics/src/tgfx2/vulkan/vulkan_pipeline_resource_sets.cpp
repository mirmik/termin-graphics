...
namespace tgfx {
    ...
    static uint64_t hash_descriptor_bindings(const std::vector<VkShaderResource::DescriptorBinding>& bindings) {
        ...
        std::sort(sorted.begin(),
                  sorted.end(),
                  [](const VkShaderResource::DescriptorBinding& a, const VkShaderResource::DescriptorBinding& b) {
    ...
    VkPipelineLayout VulkanRenderDevice::get_or_create_pipeline_layout(VkDescriptorSetLayout set_layout) {
        ...
        VkDescriptorSetLayout cache_key =
            set_layout ? set_layout : reinterpret_cast<VkDescriptorSetLayout>(static_cast<uintptr_t>(1));
        {
    ...
    PipelineHandle VulkanRenderDevice::create_pipeline(const PipelineDesc& desc) {
        if (desc.view_count == 0 || desc.view_count >= 32 ||
            (desc.view_count > 1 && (!multiview_enabled_ || desc.view_count > max_multiview_views_))) {
            tc_log(TC_LOG_ERROR,
        ...
        if (vs) {
            merged_bindings.insert(
                merged_bindings.end(), vs->descriptor_bindings.begin(), vs->descriptor_bindings.end());
        }
        if (fs) {
            merged_bindings.insert(
        ...
        color_fmts.erase(std::remove(color_fmts.begin(), color_fmts.end(), PixelFormat::Undefined), color_fmts.end());
        bool needs_depth = desc.depth_format != PixelFormat::Undefined;
        {
            VulkanStatsTimer timer(g_pipeline_renderpass_us);
            const std::vector<LoadOp> color_loads(color_fmts.size(), LoadOp::Clear);
            const std::vector<StoreOp> color_stores(color_fmts.size(), StoreOp::Store);
            const uint32_t view_mask = desc.view_count > 1 ? ((1u << desc.view_count) - 1u) : 0u;
            res.render_pass = get_or_create_render_pass(color_fmts,
                                                        color_loads,
                                                        color_stores,
                                                        desc.depth_format,
                                                        needs_depth,
        ...
        if (desc.geometry_shader)
            add_stage(desc.geometry_shader, VK_SHADER_STAGE_GEOMETRY_BIT);
        ...
        for (uint32_t i = 0; i < desc.vertex_layouts.size(); ++i) {
            ...
            const uint32_t attribute_count = std::min(vl.attribute_count, TGFX2_VERTEX_ATTRIBUTE_MAX);
        ...
        if (vertex_shader && vertex_shader->vertex_input_locations_known) {
            ...
            provided_locations.erase(std::unique(provided_locations.begin(), provided_locations.end()),
            ...
            const std::string shader_name =
                vertex_shader->debug_name.empty() ? std::string("<unnamed vertex shader>") : vertex_shader->debug_name;
            tc_log(TC_LOG_WARN,
    ...
    namespace {
        ...
        static bool descriptor_kind_matches_layout(BackendDescriptorKind descriptor_kind,
        ...
        static VkDescriptorType expected_descriptor_type_from_layout(BackendDescriptorKind descriptor_kind,
                                                                     VkDescriptorType layout_type) {
        ...
        static bool bound_value_kind_matches_descriptor_kind(BoundResourceKind value_kind,
        ...
        find_layout_binding(const std::vector<VkDescriptorSetLayoutBinding>& layout_bindings, uint32_t binding) {
    ...
    ResourceSetHandle
    VulkanRenderDevice::create_resolved_resource_set(VkDescriptorSetLayout layout,
        ...
        for (const VkDescriptorSetLayoutBinding& lb : layout_bindings) {
            ...
            for (uint32_t element = 0; element < descriptor_count; ++element) {
                const VulkanResolvedResourceBinding* provided = find_resolved_binding(lb.binding, element);
                ...
                case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER: {
                    ...
                    if (!buf)
                        continue;
                    buf_infos.push_back(
                        {buf->buffer, provided->offset, provided->range ? provided->range : VK_WHOLE_SIZE});
                    w.pBufferInfo = &buf_infos.back();
                    break;
                }
                ...
                case VK_DESCRIPTOR_TYPE_SAMPLER: {
                    ...
                    img_infos.push_back({samp_vk, VK_NULL_HANDLE, VK_IMAGE_LAYOUT_UNDEFINED});
                    w.pImageInfo = &img_infos.back();
                    break;
                }
                default:
    ...
    ResourceSetHandle VulkanRenderDevice::create_bound_resource_set(const BoundResourceSetDesc& desc) {
        VkDescriptorSetLayout layout = VK_NULL_HANDLE;
        if (desc.resource_layout_token != 0) {
            layout = reinterpret_cast<VkDescriptorSetLayout>(desc.resource_layout_token);
        }
        if (!layout) {
            tc_log(TC_LOG_ERROR, "VulkanRenderDevice: create_bound_resource_set called without resource_layout_token");
            return {};
        }
        ...
        for_each_bound_resource_binding(desc, [&](const BoundResourceBinding& b) {
            ...
            const VkDescriptorSetLayoutBinding* lb = find_layout_binding(layout_bindings, binding);
            if (!lb) {
                tc_log(TC_LOG_ERROR,
            ...
            const VkDescriptorType expected =
                expected_descriptor_type_from_layout(slot.placement.vulkan.descriptor_kind, lb->descriptorType);
            if (expected == VK_DESCRIPTOR_TYPE_MAX_ENUM) {
                tc_log(TC_LOG_ERROR,
                       "VulkanRenderDevice: resource '%s' descriptor kind=%u does not match layout descriptor type=%u "
                       "at binding=%u",
                       bound_resource_debug_name(b),
            ...
            if (!bound_value_kind_matches_descriptor_kind(b.value.kind, slot.placement.vulkan.descriptor_kind)) {
                tc_log(
                    TC_LOG_ERROR,
        ...
        return create_resolved_resource_set(
            layout, layout_bindings, desc.resource_layout_token, resolved_bindings, {}, 0x626f756e64000001ull);
    }
