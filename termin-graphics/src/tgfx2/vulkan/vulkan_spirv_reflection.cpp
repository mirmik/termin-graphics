...
namespace tgfx {
    ...
    std::string reflect_spirv_stage_entry_point(const std::vector<uint32_t>& spirv, ShaderStage stage) {
        if (spirv.size() < 5)
            return {};

        static constexpr uint32_t OP_ENTRY_POINT = 15;
        const uint32_t execution_model = spirv_execution_model_for_stage(stage);
        if (execution_model == UINT32_MAX)
            return {};
    ...
    SpirvVertexInputs reflect_spirv_vertex_inputs(const std::vector<uint32_t>& spirv, const std::string& entry_point) {
    ...
    std::vector<VkShaderResource::DescriptorBinding>
    reflect_spirv_descriptor_bindings(const std::vector<uint32_t>& spirv) {
        ...
        result.erase(std::unique(result.begin(),
                                 ...
                                 [](const VkShaderResource::DescriptorBinding& a,
                                    const VkShaderResource::DescriptorBinding& b) { return a.binding == b.binding; }),
                     result.end());
    ...
    bool vertex_shader_uses_location(const VkShaderResource* shader, uint32_t location) {
        if (!shader || !shader->vertex_input_locations_known)
            return true;
        return std::find(shader->vertex_input_locations.begin(), shader->vertex_input_locations.end(), location) !=
               shader->vertex_input_locations.end();
    }

    bool vertex_attributes_have_location(const std::vector<VkVertexInputAttributeDescription>& attributes,
                                         uint32_t location) {
        return std::any_of(
            attributes.begin(), attributes.end(), [location](const VkVertexInputAttributeDescription& attr) {
    ...
    std::string describe_vk_vertex_attributes(const std::vector<VkVertexInputAttributeDescription>& attributes) {
        std::ostringstream out;
        for (size_t i = 0; i < attributes.size(); ++i) {
            const auto& attr = attributes[i];
            if (i)
                out << "; ";
            out << "loc=" << attr.location << " binding=" << attr.binding << " offset=" << attr.offset
                << " vkfmt=" << attr.format;
        }
        return out.str();
    }

    std::string describe_vertex_layouts(const std::vector<VertexLayoutDesc>& layouts) {
        std::ostringstream out;
        for (size_t i = 0; i < layouts.size(); ++i) {
            const auto& layout = layouts[i];
            if (i)
                out << " | ";
            out << "binding " << i << " stride=" << layout.stride
                << " rate=" << (layout.per_instance ? "instance" : "vertex") << " attrs=[";
            ...
            for (uint32_t j = 0; j < attribute_count; ++j) {
                ...
                if (j)
                    out << "; ";
                out << "loc=" << attr.location << " offset=" << attr.offset << " fmt=" << static_cast<int>(attr.format);
            }
            out << "]";
        }
        return out.str();
