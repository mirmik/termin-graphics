...
namespace tgfx {
    ...
    std::unique_ptr<GraphicsHost> GraphicsHost::adopt_application_device(std::unique_ptr<IRenderDevice> device) {
    ...
    std::unique_ptr<GraphicsHost> GraphicsHost::adopt_isolated_device(std::unique_ptr<IRenderDevice> device) {
    ...
    void GraphicsHost::claim_application_domain() {
        ...
        if (!tgfx2_interop_claim_device(device_, this)) {
            throw std::runtime_error("GraphicsHost: another application graphics device is already installed");
        }
    ...
    void GraphicsHost::configure_shader_artifacts(const termin::ShaderArtifactResolver& resolver) {
        if (!device_) {
            throw std::runtime_error("GraphicsHost: cannot configure shader artifacts without a device");
        }
    ...
    const termin::ShaderArtifactResolver& GraphicsHost::shader_artifact_resolver() const {
        if (!device_) {
            throw std::runtime_error("GraphicsHost: shader artifact resolver is unavailable without a device");
        }
    ...
    void GraphicsHost::ensure_context_() {
        ...
        if (!owned_ctx_) {
            owned_ctx_ = std::make_unique<RenderContext2>(*device_, *owned_cache_, this);
        }
    }
