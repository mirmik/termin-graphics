...
namespace tgfx::line_renderer {
    ...
    BufferHandle create_static_vertex_buffer(IRenderDevice& device, const void* data, size_t byte_size) {
        if (!data || byte_size == 0) {
            return {};
        }
    ...
    UploadedInstanceStream upload_instance_stream(RenderContext2& ctx, const void* data, size_t byte_size) {
        ...
        if (byte_size <= std::numeric_limits<uint32_t>::max()) {
            const uint64_t ring_offset = device.transient_vertex_write(data, static_cast<uint32_t>(byte_size));
