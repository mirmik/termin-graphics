...
namespace tgfx {
    ...
    bool shader_resource_kind_is_texture_like(ShaderResourceKind kind) {
        return kind == ShaderResourceKind::Texture || kind == ShaderResourceKind::Sampler ||
    ...
    bool shader_resource_scope_has_transitional_binding_range(ShaderResourceScope scope) {
        return scope == ShaderResourceScope::Frame || scope == ShaderResourceScope::Pass ||
               scope == ShaderResourceScope::Material || scope == ShaderResourceScope::Draw ||
    ...
    BackendBindingRange
    transitional_backend_binding_range(BackendType backend, ShaderResourceKind kind, ShaderResourceScope scope) {
        if (shader_resource_kind_is_constant_buffer(kind)) {
            if (scope == ShaderResourceScope::Frame)
                return {0, 4};
            if (scope == ShaderResourceScope::Pass)
                return {4, 4};
            if (scope == ShaderResourceScope::Material)
                return {8, 8};
            if (scope == ShaderResourceScope::Draw)
                return {16, 16};
            if (scope == ShaderResourceScope::Transient)
                return {32, 8};
        }
    ...
    BackendBindingConflictClass backend_binding_conflict_class(BackendType backend, ShaderResourceKind kind) {
        if (backend != BackendType::OpenGL) {
    ...
    namespace {
        ...
        bool validate_resource_binding(const tc_shader_resource_binding& binding,
        ...
        bool build_vulkan_entry(const tc_shader_resource_binding& binding,
                                BackendBindingPlanEntry& entry,
                                std::string* error) {
            if (binding.set != 0) {
                set_error(error,
                          "shader resource " + resource_label(binding) + " uses Vulkan descriptor set " +
                              std::to_string(binding.set) + ", but tgfx2 currently supports set 0 only");
                return false;
            }
            ...
            entry.placement.vulkan.descriptor_kind = descriptor_kind_for_resource(entry.resource.kind);
            return true;
        }

        bool build_d3d11_entry(const tc_shader_resource_binding& binding,
                               BackendBindingPlanEntry& entry,
                               std::string* error) {
            ...
            if (!binding.has_d3d11_placement) {
                set_error(error, "shader resource " + resource_label(binding) + " is missing D3D11 placement");
                return false;
            }
            const D3D11RegisterClass register_class = d3d11_register_class_from_shader(binding.d3d11.register_class);
            const D3D11RegisterClass expected = expected_d3d11_register_class(entry.resource.kind);
            if (register_class == D3D11RegisterClass::None || register_class != expected) {
                set_error(error,
                          "shader resource " + resource_label(binding) + " has incompatible D3D11 register class");
                return false;
            }
        ...
        bool build_opengl_entry(const tc_shader_resource_binding& binding,
                                BackendBindingPlanEntry& entry,
                                std::string* error) {
            const OpenGLBindingClass binding_class = opengl_binding_class_for_resource(entry.resource.kind);
            if (binding_class == OpenGLBindingClass::None) {
                set_error(error, "shader resource " + resource_label(binding) + " has no OpenGL binding class");
                return false;
            }
            entry.placement.kind = BackendPlacementKind::OpenGLBinding;
            entry.placement.opengl.binding_class = binding_class;
            if (binding_class == OpenGLBindingClass::TextureUnit || binding_class == OpenGLBindingClass::SamplerUnit) {
        ...
        bool build_webgpu_entry(const tc_shader_resource_binding& binding,
                                BackendBindingPlanEntry& entry,
                                std::string* error) {
            ...
            if (binding.webgpu.group != 0) {
                set_error(error,
                          "shader resource " + resource_label(binding) + " uses WebGPU bind group " +
                              std::to_string(binding.webgpu.group) + ", but tgfx2 currently supports group 0 only");
                return false;
            }
            if (binding.webgpu.has_sampler_binding && entry.resource.kind != ShaderResourceKind::Texture) {
                set_error(error,
                          "shader resource " + resource_label(binding) +
                              " declares sampler_binding but is not a sampled texture");
                return false;
            }
            if (binding.webgpu.has_sampler_binding && binding.webgpu.sampler_binding == binding.webgpu.binding) {
                set_error(error,
                          "shader resource " + resource_label(binding) +
            ...
            entry.placement.webgpu.has_sampler_binding = binding.webgpu.has_sampler_binding != 0;
            entry.placement.webgpu.sampler_binding = binding.webgpu.sampler_binding;
            return true;
        }

        bool d3d11_conflicts(const BackendBindingPlanEntry& a, const BackendBindingPlanEntry& b) {
        ...
        bool same_semantic_resource(const BackendBindingPlanEntry& a, const BackendBindingPlanEntry& b) {
            return a.resource.name == b.resource.name && a.resource.kind == b.resource.kind &&
                   a.resource.scope == b.resource.scope;
        }

        bool semantic_conflicts(const BackendBindingPlanEntry& a, const BackendBindingPlanEntry& b) {
            return a.resource.name == b.resource.name && !same_semantic_resource(a, b);
        }

        bool vulkan_conflicts(const BackendBindingPlanEntry& a, const BackendBindingPlanEntry& b) {
        ...
        bool opengl_conflicts(const BackendBindingPlanEntry& a, const BackendBindingPlanEntry& b) {
        ...
        bool webgpu_conflicts(const BackendBindingPlanEntry& a, const BackendBindingPlanEntry& b) {
            if (a.placement.kind != BackendPlacementKind::WebGPU || b.placement.kind != BackendPlacementKind::WebGPU ||
                a.placement.webgpu.group != b.placement.webgpu.group) {
                return false;
            }
            const auto occupies = [](const BackendBindingPlanEntry& entry, uint32_t binding) {
                return entry.placement.webgpu.binding == binding || (entry.placement.webgpu.has_sampler_binding &&
                                                                     entry.placement.webgpu.sampler_binding == binding);
            };
            return occupies(a, b.placement.webgpu.binding) ||
                   (b.placement.webgpu.has_sampler_binding && occupies(a, b.placement.webgpu.sampler_binding));
        }

        bool validate_plan_conflicts(const BackendBindingPlan& plan, std::string* error) {
            for (size_t i = 0; i < plan.entries.size(); ++i) {
                for (size_t j = i + 1; j < plan.entries.size(); ++j) {
                    ...
                    if (semantic_conflicts(a, b)) {
                        set_error(error, "binding plan semantic conflict for resource '" + a.resource.name + "'");
                        return false;
                    }
                    ...
                    case BackendType::Vulkan:
                        if (vulkan_conflicts(a, b)) {
                            set_error(error,
                                      "Vulkan binding plan conflict between resource '" + a.resource.name + "' and '" +
                                          b.resource.name + "'");
                            return false;
                        }
                        break;
                    case BackendType::D3D11:
                        if (d3d11_conflicts(a, b)) {
                            set_error(error,
                                      "D3D11 binding plan conflict between resource '" + a.resource.name + "' and '" +
                                          b.resource.name + "'");
                            return false;
                        }
                        break;
                    case BackendType::OpenGL:
                        if (opengl_conflicts(a, b)) {
                            set_error(error,
                                      "OpenGL binding plan conflict between resource '" + a.resource.name + "' and '" +
                                          b.resource.name + "'");
                            return false;
                        }
                        break;
                    case BackendType::WebGPU:
                        if (webgpu_conflicts(a, b)) {
                            set_error(error,
                                      "WebGPU binding plan conflict between resource '" + a.resource.name + "' and '" +
                                          b.resource.name + "'");
                            return false;
                        }
                        break;
    ...
    bool build_backend_binding_plan(BackendType backend,
    ...
    BackendBoundResourceSlot bound_resource_slot_from_plan_entry(const BackendBindingPlanEntry& entry) {
    ...
    void BoundResourceSetStorage::assign(const BoundResourceSetDesc& desc) {
        ...
        for (uint32_t group_index = 0; group_index < desc.group_count; ++group_index) {
            ...
            if (source.binding_count != 0) {
                destination.bindings.assign(source.bindings, source.bindings + source.binding_count);
            }
        }
        rebuild_views();
    }

    void BoundResourceSetStorage::append_group(ShaderResourceScope scope,
                                               bool dirty,
