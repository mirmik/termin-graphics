...
namespace tgfx {
    namespace {
        ...
        bool fill_binding_from_mesh(Tgfx2MeshBinding& out, tc_mesh* mesh) {
            ...
            out.layout_desc.use_shader_input_locations = mesh->layout.use_shader_input_locations != 0;
            out.layout_desc.attribute_count = std::min<uint32_t>(mesh->layout.attrib_count, TGFX2_VERTEX_ATTRIBUTE_MAX);
            if (mesh->layout.attrib_count > TGFX2_VERTEX_ATTRIBUTE_MAX) {
                tc::Log::error("wrap_mesh_as_tgfx2: mesh '%s' has %u attributes, max is %u; truncating",
            ...
            for (uint8_t i = 0; i < mesh->layout.attrib_count; i++) {
                ...
                case TGFX_ATTRIB_FLOAT32:
                    switch (a.size) {
                    case 1:
                        va.format = VertexFormat::Float;
                        break;
                    case 2:
                        va.format = VertexFormat::Float2;
                        break;
                    case 3:
                        va.format = VertexFormat::Float3;
                        break;
                    case 4:
                        va.format = VertexFormat::Float4;
                        break;
                    default:
                        ok = false;
                        break;
                ...
                case TGFX_ATTRIB_INT32:
                    switch (a.size) {
                    case 1:
                        va.format = VertexFormat::Int;
                        break;
                    case 2:
                        va.format = VertexFormat::Int2;
                        break;
                    case 3:
                        va.format = VertexFormat::Int3;
                        break;
                    case 4:
                        va.format = VertexFormat::Int4;
                        break;
                    default:
                        ok = false;
                        break;
                ...
                case TGFX_ATTRIB_UINT32:
                    switch (a.size) {
                    case 1:
                        va.format = VertexFormat::UInt;
                        break;
                    case 2:
                        va.format = VertexFormat::UInt2;
                        break;
                    case 3:
                        va.format = VertexFormat::UInt3;
                        break;
                    case 4:
                        va.format = VertexFormat::UInt4;
                        break;
                    default:
                        ok = false;
                        break;
                ...
                case TGFX_ATTRIB_INT16:
                    switch (a.size) {
                    case 1:
                        va.format = VertexFormat::Short;
                        break;
                    case 2:
                        va.format = VertexFormat::Short2;
                        break;
                    case 3:
                        va.format = VertexFormat::Short3;
                        break;
                    case 4:
                        va.format = VertexFormat::Short4;
                        break;
                    default:
                        ok = false;
                        break;
                ...
                case TGFX_ATTRIB_UINT16:
                    switch (a.size) {
                    case 1:
                        va.format = VertexFormat::UShort;
                        break;
                    case 2:
                        va.format = VertexFormat::UShort2;
                        break;
                    case 3:
                        va.format = VertexFormat::UShort3;
                        break;
                    case 4:
                        va.format = VertexFormat::UShort4;
                        break;
                    default:
                        ok = false;
                        break;
                ...
                case TGFX_ATTRIB_INT8:
                    if (a.size == 4)
                        va.format = VertexFormat::Byte4;
                    else
                        ok = false;
                    break;
                case TGFX_ATTRIB_UINT8:
                    if (a.size == 4)
                        va.format = VertexFormat::UByte4;
                    else
                        ok = false;
                    break;
                default:
                ...
                if (!ok) {
                    tc::Log::error("wrap_mesh_as_tgfx2: unsupported attrib (type=%u, size=%u) "
            ...
            out.topology =
                (mesh->draw_mode == TC_DRAW_LINES) ? PrimitiveTopology::LineList : PrimitiveTopology::TriangleList;
            return true;
        }

        bool semantic_in_list(std::string_view semantic, std::initializer_list<std::string_view> used_semantics) {
        ...
        const tc_submesh* validate_submesh_range(tc_mesh* mesh, size_t submesh_index) {
            ...
            if (!submesh) {
                tc::Log::error("draw_tc_submesh: mesh '%s' has no submesh %zu (count=%zu)",
                               mesh->header.name ? mesh->header.name : mesh->header.uuid,
                               submesh_index,
                               mesh->submesh_count);
                return nullptr;
            }
            if (submesh->index_count == 0 || static_cast<size_t>(submesh->first_index) > mesh->index_count ||
        ...
        bool draw_tc_submesh_binding(RenderContext2& ctx,
            ...
            ctx.set_topology(submesh.draw_mode == TC_DRAW_LINES ? PrimitiveTopology::LineList
                                                                : PrimitiveTopology::TriangleList);
            ctx.draw(binding.vertex_buffer,
    ...
    bool vertex_layout_has_semantic(const VertexLayoutDesc& layout, std::string_view semantic) {
    ...
    VertexLayoutDesc filter_vertex_layout_to_locations(const VertexLayoutDesc& layout,
            ...
            for (uint32_t loc : used_locations) {
                if (attr.location == loc) {
                    if (out.attribute_count >= TGFX2_VERTEX_ATTRIBUTE_MAX) {
                        tc::Log::error("filter_vertex_layout_to_locations: output attribute overflow");
                        return out;
                    }
    ...
    bool try_filter_vertex_layout_to_semantics(const VertexLayoutDesc& layout,
        ...
        for (uint32_t i = 0; i < attribute_count; ++i) {
            ...
            if (semantic_in_list(vertex_attribute_semantic(attr), used_semantics)) {
                if (out.attribute_count >= TGFX2_VERTEX_ATTRIBUTE_MAX) {
                    tc::Log::error("filter_vertex_layout_to_semantics: output attribute overflow");
                    return false;
                }
                out.attributes[out.attribute_count++] = attr;
            }
        ...
        for (std::string_view semantic : used_semantics) {
            if (!vertex_layout_has_semantic(out, semantic)) {
                tc::Log::error("filter_vertex_layout_to_semantics: layout has no '%.*s' attribute",
    ...
    VertexLayoutDesc filter_vertex_layout_to_semantics(const VertexLayoutDesc& layout,
                                                       std::initializer_list<std::string_view> used_semantics,
                                                       bool use_shader_input_locations) {
        VertexLayoutDesc out;
        try_filter_vertex_layout_to_semantics(layout, used_semantics, out, use_shader_input_locations);
        return out;
    }

    bool draw_tc_mesh(RenderContext2& ctx, tc_mesh* mesh, const VertexBufferLayout* layout_override) {
        ...
        ctx.draw(binding.vertex_buffer, binding.index_buffer, binding.index_count, binding.index_type);

        release_mesh_binding(ctx.device(), binding);
        return true;
    }

    bool draw_tc_mesh(RenderContext2& ctx, tc_mesh* mesh, const VertexLayoutDesc* layout_override) {
        Tgfx2MeshBinding binding = wrap_mesh_as_tgfx2(ctx.device(), mesh);
        if (binding.index_count == 0)
            return false;
    ...
    bool draw_tc_submesh(RenderContext2& ctx,
                         tc_mesh* mesh,
        ...
        const VertexLayoutDesc override_desc =
            layout_override ? make_vertex_layout_desc(*layout_override) : VertexLayoutDesc{};
        draw_tc_submesh_binding(ctx, binding, *submesh, layout_override ? &override_desc : nullptr);
        release_mesh_binding(ctx.device(), binding);
        return true;
    }

    bool
    draw_tc_submesh(RenderContext2& ctx, tc_mesh* mesh, size_t submesh_index, const VertexLayoutDesc* layout_override) {
        const tc_submesh* submesh = validate_submesh_range(mesh, submesh_index);
        if (!submesh) {
            return false;
        }
    ...
    bool draw_tc_mesh(RenderContext2& ctx,
                      tc_mesh* mesh,
                      std::initializer_list<uint32_t> used_locations,
                      bool use_shader_input_locations) {
        Tgfx2MeshBinding binding = wrap_mesh_as_tgfx2(ctx.device(), mesh);
        ...
        VertexLayoutDesc filtered =
            filter_vertex_layout_to_locations(binding.layout_desc, used_locations, use_shader_input_locations);
    ...
    bool draw_tc_mesh(RenderContext2& ctx,
        ...
        if (!try_filter_vertex_layout_to_semantics(
                binding.layout_desc, used_semantics, filtered, use_shader_input_locations)) {
            release_mesh_binding(ctx.device(), binding);
            return false;
        }
        ctx.set_vertex_layout(filtered);
