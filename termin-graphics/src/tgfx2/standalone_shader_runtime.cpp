...
std::optional<fs::path> configured_tool(
    ...
    const char* tool_name)
{
    if (const char* sdk = std::getenv("TERMIN_SDK");
        ...
        candidates.push_back(
            executable_path(fs::path(sdk) / "bin" / tool_name));
    }

    for (const fs::path& builtin_root : builtin_shader_roots()) {
        // <sdk>/share/termin/builtin_shaders -> <sdk>/bin
        const fs::path sdk_root =
            builtin_root.parent_path().parent_path().parent_path();
        candidates.push_back(
            executable_path(sdk_root / "bin" / tool_name));
    }
    ...
    const char* settings_key)
{
    ...
    tc_log_error(
        "[StandaloneShaderRuntime] %s not found; set %s, install it under "
...
std::optional<fs::path> platform_cache_root(const char* label) {
    ...
    if (const char* local_app_data = std::getenv("LOCALAPPDATA");
        local_app_data && local_app_data[0] != '\0') {
        return fs::path(local_app_data) / "Termin" / "Cache" /
            (std::string(label) + "-shaders");
    }
    if (const char* user_profile = std::getenv("USERPROFILE");
        user_profile && user_profile[0] != '\0') {
        return fs::path(user_profile) / "AppData" / "Local" / "Termin" /
            "Cache" / (std::string(label) + "-shaders");
    }
#else
    if (const char* xdg_cache = std::getenv("XDG_CACHE_HOME");
        xdg_cache && xdg_cache[0] != '\0') {
        return fs::path(xdg_cache) / "termin" /
            (std::string(label) + "-shaders");
    }
    if (const char* home = std::getenv("HOME");
        home && home[0] != '\0') {
        return fs::path(home) / ".cache" / "termin" /
            (std::string(label) + "-shaders");
    }
#endif

    tc_log_error(
        "[StandaloneShaderRuntime] user cache directory is unavailable; "
...
bool create_directory(const fs::path& path, const char* label) {
    ...
    if (error) {
        tc_log_error(
            "[StandaloneShaderRuntime] failed to create %s '%s': %s",
            label,
            path.string().c_str(),
            error.message().c_str());
        return false;
...
#endif
        tc_log_error(
            "[StandaloneShaderRuntime] failed to configure TERMIN_SLANGC='%s'",
            path.string().c_str());
        return false;
    }
    return true;
...
bool configure_default_standalone_shader_runtime(
    GraphicsHost& host,
    const char* label)
{
    ...
    if (!valid_label(label_view)) {
        tc_log_error(
            "[StandaloneShaderRuntime] cache label must contain only letters, "
            "digits, '.', '_' or '-': '%s'",
            label ? label : "<null>");
        return false;
    }
    if (host.is_closed()) {
        tc_log_error(
            "[StandaloneShaderRuntime] cannot configure a closed GraphicsHost");
        return false;
    }
    ...
    const std::optional<fs::path> runtime_root =
        platform_cache_root(label);
    if (!runtime_root) {
        return false;
    }
    ...
    host.configure_shader_artifacts(termin::ShaderArtifactResolver(
        normalized_path(artifact_root).string(),
        normalized_path(cache_root).string(),
        shader_compiler->string(),
        true));
    tc_log_info(
        "[StandaloneShaderRuntime] %s configured: artifact_root='%s', "
