...
namespace tgfx {
    ...
    namespace {
        ...
        void add_share_roots_from_ancestors(std::vector<std::filesystem::path>& roots, std::filesystem::path base) {
    ...
    namespace {
        ...
        std::optional<BuiltinLocatedSource> try_load_builtin_shader_source(const char* filename,
        ...
        bool validate_builtin_shader_catalog(const nos::trent& catalog) {
            ...
            for (const nos::trent& entry : shaders->as_list()) {
                ...
                if (language != "slang" && language != "glsl") {
                    tc::Log::error("[BuiltInShaderCatalog] Shader '%s' has unsupported language '%s'",
                                   uuid.c_str(),
                                   language.c_str());
                    valid = false;
                    continue;
                ...
                for (const char* stage_name : {"vertex", "fragment", "geometry", "compute"}) {
                    ...
                    has_stage = true;
                    if (stage->get_type() != nos::trent_type::dict || string_field(*stage, "path").empty() ||
                        string_field(*stage, "entry").empty()) {
                        tc::Log::error(
                            "[BuiltInShaderCatalog] Shader '%s' has invalid '%s' stage", uuid.c_str(), stage_name);
                        valid = false;
                    }
                }
                if (!has_stage) {
    ...
    std::string load_builtin_shader_source(const char* filename, const char* debug_name) {
        if (!filename || filename[0] == '\0') {
            tc::Log::error("[BuiltInShaderSource] Missing filename for '%s'", debug_name ? debug_name : "<unnamed>");
            return {};
        }

        if (std::optional<BuiltinLocatedSource> source = try_load_builtin_shader_source(filename, debug_name)) {
            return source->source;
        }

        const auto roots = builtin_shader_roots();
        tc::Log::error("[BuiltInShaderSource] Failed to load '%s' for '%s'. Searched: %s",
                       filename,
                       debug_name ? debug_name : "<unnamed>",
    ...
    tc_shader_handle register_builtin_fragment_shader(const char* filename, const char* name, const char* uuid) {
        ...
        return tc_shader_register_static_uuid_ex(
            nullptr, source.c_str(), nullptr, name, uuid, TC_SHADER_LANGUAGE_GLSL, TC_SHADER_ARTIFACT_OPTIONAL);
    }

    tc_shader_handle register_builtin_vertex_fragment_shader(const char* vertex_filename,
        ...
        return tc_shader_register_static_uuid_ex(vertex_source.c_str(),
    ...
    std::string load_builtin_shader_stage_source_from_catalog(const char* uuid, const char* stage_name) {
        if (!stage_name || stage_name[0] == '\0') {
            tc::Log::error("[BuiltInShaderCatalog] Shader '%s' requested with missing stage name",
                           uuid ? uuid : "<null>");
            return {};
        }

        BuiltinShaderStageMetadata metadata = load_builtin_shader_stage_metadata_from_catalog(uuid, stage_name);
    ...
    BuiltinShaderStageMetadata load_builtin_shader_stage_metadata_from_catalog(const char* uuid,
        ...
        if (language != "glsl" && language != "slang") {
            tc::Log::error("[BuiltInShaderCatalog] Shader '%s' has unsupported live stage language '%s'",
                           uuid ? uuid : "<null>",
                           language.c_str());
            return {};
        }
        ...
        if (metadata.path.empty()) {
            tc::Log::error("[BuiltInShaderCatalog] Shader '%s' has no '%s' stage", uuid ? uuid : "<null>", stage_name);
            return {};
        }
        const nos::trent* stages = dict_get(*entry, "stages");
    ...
    tc_shader_handle register_builtin_shader_from_catalog(const char* uuid) {
        ...
        const tc_shader_language shader_language = is_slang ? TC_SHADER_LANGUAGE_SLANG : TC_SHADER_LANGUAGE_GLSL;
        ...
        auto load_catalog_stage_source = [&](const char* stage_name, const std::string& path) {
            ...
            tc::Log::error("[BuiltInShaderCatalog] Shader '%s' failed to load %s stage '%s'. Roots: %s",
                           uuid ? uuid : "<null>",
        ...
        auto stage_entry = [&](const char* stage_name) -> std::string {
            const nos::trent* stages = dict_get(*entry, "stages");
            if (!stages || !stages->is_dict())
                return {};
            const nos::trent* stage_obj = dict_get(*stages, stage_name);
            if (!stage_obj || !stage_obj->is_dict())
                return {};
        ...
        const tc_shader_create_desc shader_desc = {{vertex_source.empty() ? nullptr : vertex_source.c_str(),
        ...
        if (tc_shader_handle_is_invalid(handle)) {
            tc::Log::error("[BuiltInShaderCatalog] Failed to register shader '%s' "
...

        {
            ...
            if (!shader) {
                tc::Log::error("[BuiltInShaderCatalog] Registered shader '%s' but registry lookup returned null",
                               uuid ? uuid : "<null>");
                return tc_shader_handle_invalid();
            }

            if (!fragment_path.empty() && (!shader->fragment_source || shader->fragment_source[0] == '\0')) {
                tc::Log::error("[BuiltInShaderCatalog] Registered shader '%s' has empty fragment source "
    ...
    BuiltinShaderProgramSource load_builtin_shader_program_from_catalog(const char* uuid) {
        ...
        const std::string language = string_field(*entry, "language");
        if (language != "shader") {
            tc::Log::error("[BuiltInShaderCatalog] Shader '%s' is not a shader program entry", uuid ? uuid : "<null>");
            return {};
        }
