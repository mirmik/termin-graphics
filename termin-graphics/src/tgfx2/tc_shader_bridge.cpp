...
namespace termin {
    ...
    static const char* stage_extension(tgfx::ShaderStage stage) {
        ...
        case tgfx::ShaderStage::Vertex:
            return "vert";
        case tgfx::ShaderStage::Fragment:
            return "frag";
        case tgfx::ShaderStage::Geometry:
            return "geom";
    ...
    static bool shader_language_target_supported(tc_shader_language language, tgfx::BackendType backend) {
        if (language == TC_SHADER_LANGUAGE_GLSL) {
            return backend == tgfx::BackendType::Vulkan;
        }
        if (language == TC_SHADER_LANGUAGE_SLANG) {
            return backend == tgfx::BackendType::Vulkan || backend == tgfx::BackendType::OpenGL ||
                   backend == tgfx::BackendType::D3D11 || backend == tgfx::BackendType::WebGPU;
        }
    ...
    static bool collect_slang_dependency_hash(std::string_view source,
        ...
        for (std::sregex_iterator it(source_text.begin(), source_text.end(), import_re), end; it != end; ++it) {
            ...
            for (const auto& root : include_roots) {
                for (const std::string& candidate_name : {module_name + ".slang", module_path + ".slang"}) {
                    std::error_code ec;
            ...
            if (!in) {
                error = "failed to read Slang import '" + module_name + "' at '" + resolved.string() + "'";
                return false;
            }
            const std::string dependency{std::istreambuf_iterator<char>(in), std::istreambuf_iterator<char>()};
            fnv1a_update(hash, module_name);
            fnv1a_update(hash, "\0");
            fnv1a_update(hash, dependency);
            fnv1a_update(hash, "\0");
            if (!collect_slang_dependency_hash(dependency, include_roots, visited, hash, error)) {
                return false;
            }
        }
        return true;
    ...
    static bool shader_dependency_fingerprint(tc_shader_language language, std::string_view source, std::string& out) {
        if (language != TC_SHADER_LANGUAGE_SLANG) {
            out = "none";
            return true;
        }
        ...
        if (!collect_slang_dependency_hash(source, roots, visited, hash, error)) {
            tc_log(TC_LOG_ERROR, "tgfx2 shader dependency scan: %s; include roots=%zu", error.c_str(), roots.size());
            return false;
        }
        char encoded[17];
        std::snprintf(encoded, sizeof(encoded), "%016llx", static_cast<unsigned long long>(hash));
        out = encoded;
        return true;
    }

    static bool shader_program_dependency_fingerprint(const tc_shader* shader,
        ...
        if (backend != tgfx::BackendType::D3D11 || language != TC_SHADER_LANGUAGE_SLANG) {
            ...
            return shader_dependency_fingerprint(language, source ? std::string_view(source) : std::string_view(), out);
        }
        ...
        for (const tgfx::ShaderStage program_stage :
            ...
            if (!shader_dependency_fingerprint(language, source, stage_fingerprint)) {
                return false;
            }
    ...
    shader_cache_source_path(const ShaderArtifactResolver& resolver, const tc_shader* shader, tgfx::ShaderStage stage) {
    ...
    static std::string artifact_metadata_text(const tc_shader* shader,
                                              tgfx::BackendType backend,
                                              tgfx::ShaderStage stage,
    ...
    static std::string engine_shader_artifact_metadata_text(const tgfx::EngineShaderStageSource& shader,
    ...
    static std::filesystem::path shader_artifact_metadata_path(const std::filesystem::path& artifact_path) {
        return std::filesystem::path(artifact_path.string() + kShaderArtifactMetadataSuffix);
    }

    static std::filesystem::path legacy_shader_artifact_metadata_path(const std::filesystem::path& artifact_path) {
        return std::filesystem::path(artifact_path.string() + kLegacyShaderArtifactMetadataSuffix);
    }

    static std::string shader_compiler_fingerprint(const std::filesystem::path& compiler_path) {
        std::error_code ec;
        const std::filesystem::path canonical_path = std::filesystem::weakly_canonical(compiler_path, ec);
        ...
        const int64_t mtime_ticks =
            ec ? 0 : std::chrono::duration_cast<std::chrono::nanoseconds>(mtime.time_since_epoch()).count();

        ec.clear();
        const uintmax_t size = std::filesystem::file_size(stable_path, ec);

        std::ostringstream out;
        out << stable_path.string() << "|mtime=" << mtime_ticks << "|size=" << (ec ? 0 : size);
        return out.str();
    }

    static bool artifact_metadata_current(const std::filesystem::path& artifact_path,
        ...
        return text == artifact_metadata_text(shader, backend, stage, compiler_fingerprint, dependency_fingerprint);
    }

    static bool engine_shader_artifact_metadata_current(const std::filesystem::path& artifact_path,
    ...
    static bool parse_shader_resource_layout_sidecar(const std::string& text,
        ...
        for (const nos::trent& object : resources->as_list()) {
            ...
            uint32_t size = 0;
            if (!trent_string_field(object, "name", name) || !trent_string_field(object, "kind", kind_name) ||
                !trent_uint_field(object, "set", set) || !trent_uint_field(object, "binding", binding) ||
            ...
            if (d3d11) {
                ...
                if (!d3d11->is_dict() || !trent_string_field(*d3d11, "register_class", register_class_name) ||
                    !trent_uint_field(*d3d11, "register_index", register_index)) {
                    return false;
                }
                const uint32_t register_class = shader_d3d11_register_class_from_name(register_class_name);
                ...
                if (trent_bool_field(*d3d11, "scalar_sampler_for_texture_array", scalar_sampler_for_texture_array)) {
                    resource.d3d11_scalar_sampler_for_texture_array = scalar_sampler_for_texture_array ? 1 : 0;
                }
            ...
            if (webgpu) {
                ...
                if (!webgpu->is_dict() || !trent_uint_field(*webgpu, "group", group) ||
                    !trent_uint_field(*webgpu, "binding", webgpu_binding) || group != 0) {
                    return false;
                }
                ...
                if (trent_uint_field(*webgpu, "sampler_binding", sampler_binding)) {
                    if (kind != TC_SHADER_RESOURCE_TEXTURE || sampler_binding == webgpu_binding) {
                        return false;
                    }
    ...
    static void merge_shader_resource_binding(std::vector<tc_shader_resource_binding>& bindings,
        ...
        auto clone_binding = [](const tc_shader_resource_binding& src) {
            ...
            if (src.field_count > 0 && src.fields) {
                ...
                if (dst.fields) {
                    std::memcpy(dst.fields, src.fields, src.field_count * sizeof(tc_shader_resource_field));
                } else {
                    dst.field_count = 0;
                }
            }
        ...
        for (tc_shader_resource_binding& existing : bindings) {
            if (std::strcmp(existing.name, incoming.name) == 0) {
                ...
                const uint8_t previous_has_d3d11_placement = existing.has_d3d11_placement;
                ...
                const uint8_t previous_has_webgpu_placement = existing.has_webgpu_placement;
                ...
                if (previous_has_webgpu_placement && incoming.has_webgpu_placement &&
                    (previous_webgpu.group != incoming.webgpu.group ||
                     ...
                     previous_webgpu.has_sampler_binding != incoming.webgpu.has_sampler_binding ||
                     (previous_webgpu.has_sampler_binding &&
                      previous_webgpu.sampler_binding != incoming.webgpu.sampler_binding))) {
                    tc_log(TC_LOG_ERROR,
                           "merge_shader_resource_binding: '%s' has conflicting "
                           "WebGPU placements across stages",
                           incoming.name);
                }
                if (!incoming.has_webgpu_placement && previous_has_webgpu_placement) {
                ...
                if (incoming.field_count == 0 && previous_field_count != 0 && previous_fields) {
    ...
    static void free_shader_resource_binding_fields(std::vector<tc_shader_resource_binding>& bindings) {
    ...
    static bool attach_shader_contract_from_resource_layout(tc_shader* shader, const char* source_debug_name) {
        ...
        for (uint32_t i = 0; i < binding_count; ++i) {
            ...
            std::snprintf(requirement.name, sizeof(requirement.name), "%s", binding.name);
        ...
        if (!tc_shader_set_contract(shader, &desc)) {
            tc_log(TC_LOG_ERROR, "tgfx2 shader contract: failed to attach reflected contract for '%s'", shader->uuid);
            return false;
        }
        return true;
    }

    static bool apply_shader_resource_layout_sidecar(const ShaderArtifactResolver& resolver,
                                                     tc_shader* shader,
        ...
        if (!resolver.read_artifact(sidecar.generic_string(), bytes)) {
            if (resolver.has_read_callback())
                return true;
            std::error_code ec;
            if (!std::filesystem::exists(sidecar, ec) || std::filesystem::is_directory(sidecar, ec))
                return true;
            tc_log(TC_LOG_ERROR, "tgfx2 shader resource layout: failed to read '%s'", sidecar.string().c_str());
            return false;
        }
        ...
        if (!parse_shader_resource_layout_sidecar(text, incoming)) {
            tc_log(TC_LOG_ERROR, "tgfx2 shader resource layout: malformed sidecar '%s'", sidecar.string().c_str());
            return false;
        }
        if (incoming.empty()) {
            tc_shader_mark_resource_layout_known(shader);
            return attach_shader_contract_from_resource_layout(shader, "shader resource layout sidecar");
        }
        ...
        tc_shader_set_resource_layout(shader, merged.data(), static_cast<uint32_t>(merged.size()));
        const bool contract_attached =
            attach_shader_contract_from_resource_layout(shader, "shader resource layout sidecar");
    ...
    static std::optional<std::filesystem::path> resolve_shader_compiler(const ShaderArtifactResolver& resolver) {
        ...
        for (const std::string& dir : split_paths(std::getenv("PATH"))) {
            ...
            if (is_existing_file(exe_candidate))
        ...
        tc_log(TC_LOG_ERROR, "tgfx2 shader dev compile: termin_shaderc not found; set TERMIN_SHADERC or PATH");
        return std::nullopt;
    }
    ...
    static std::filesystem::path engine_shader_cache_source_path(const ShaderArtifactResolver& resolver,
                                                                 const tgfx::EngineShaderStageSource& shader,
    ...
    bool tgfx2_load_shader_artifact_for_backend(const char* shader_uuid,
                                                tgfx::BackendType backend,
                                                tgfx::ShaderStage stage,
                                                std::vector<uint8_t>& out) {
    ...
    bool tgfx2_load_shader_artifact(const char* shader_uuid, tgfx::ShaderStage stage, std::vector<uint8_t>& out) {
        return tgfx2_load_shader_artifact_for_backend(shader_uuid, tgfx::BackendType::Vulkan, stage, out);
    }

    bool tc_shader_ensure_tgfx2(::tc_shader* shader,
        ...
        if (tc_shader_has_resource_layout(shader) && !tc_shader_sync_reflected_contract_resources(shader)) {
            tc_log(TC_LOG_ERROR,
