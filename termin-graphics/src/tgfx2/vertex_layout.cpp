...
namespace tgfx {
    ...
    VertexLayoutDesc make_vertex_layout_desc(const VertexBufferLayout& layout) {
        ...
        const size_t count = std::min<size_t>(layout.attributes.size(), TGFX2_VERTEX_ATTRIBUTE_MAX);
        desc.attribute_count = static_cast<uint32_t>(count);
        if (layout.attributes.size() > TGFX2_VERTEX_ATTRIBUTE_MAX) {
            tc::Log::error("make_vertex_layout_desc: layout has %zu attributes, max is %u; truncating",
    ...
    VertexLayoutDesc make_vertex_layout_desc(const VertexLayoutDesc& layout) {
        ...
        desc.per_instance = layout.per_instance;
        desc.use_shader_input_locations = layout.use_shader_input_locations;
        const uint32_t count = std::min<uint32_t>(layout.attribute_count, TGFX2_VERTEX_ATTRIBUTE_MAX);
        desc.attribute_count = count;
        if (layout.attribute_count > TGFX2_VERTEX_ATTRIBUTE_MAX) {
            tc::Log::error("make_vertex_layout_desc: desc has %u attributes, max is %u; truncating",
                           layout.attribute_count,
                           TGFX2_VERTEX_ATTRIBUTE_MAX);
        }
        for (uint32_t i = 0; i < count; ++i) {
            desc.attributes[i] = layout.attributes[i];
            desc.attributes[i].semantic = intern_vertex_semantic(layout.attributes[i].semantic);
        }
        return desc;
    }

    bool vertex_layout_desc_equal(const VertexLayoutDesc& a, const VertexLayoutDesc& b) {
        if (a.stride != b.stride || a.per_instance != b.per_instance ||
            a.use_shader_input_locations != b.use_shader_input_locations || a.attribute_count != b.attribute_count) {
            return false;
        }
        const uint32_t count = std::min<uint32_t>(a.attribute_count, TGFX2_VERTEX_ATTRIBUTE_MAX);
        for (uint32_t i = 0; i < count; ++i) {
            ...
            if (aa.format != bb.format || aa.offset != bb.offset || !semantic_equal(aa.semantic, bb.semantic)) {
                return false;
            }
        }
        return true;
    ...
    size_t hash_vertex_layout_descs(const VertexLayoutDesc* layouts, uint32_t count) {
        size_t hash = 0;
