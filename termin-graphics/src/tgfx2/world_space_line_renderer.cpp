...
namespace tgfx {
    namespace {
        ...
        static_assert(sizeof(CapInstance) == 11 * sizeof(float), "CapInstance layout drift - shader and C++ disagree");
        ...
        constexpr const char* WORLD_LINE_ROUND_JOIN_SHADER_UUID = "termin-engine-world-line-round-join";
        ...
        std::vector<CapCornerVertex> build_round_cap_template(int round_segments) {
            ...
            auto rim = [round_segments](int i) {
                const float t = -kPi * 0.5f + kPi * static_cast<float>(i) / static_cast<float>(round_segments);
        ...
        std::vector<CapCornerVertex> build_round_join_template(int round_segments) {
            ...
            auto rim = [round_segments](int i) {
                const float t = kPi * 2.0f * static_cast<float>(i) / static_cast<float>(round_segments);
                return CapCornerVertex{std::cos(t), std::sin(t)};
            };
        ...
        void bind_world_line_shader(RenderContext2& ctx,
            ...
            ctx.bind_uniform_data(WORLD_LINE_DRAW_RESOURCE, &draw_data, static_cast<uint32_t>(sizeof(draw_data)));
        }
    ...
    void WorldSpaceLineRenderer::ensure_resources(RenderContext2& ctx) {
        ...
        if (!corner_vbo_) {
            corner_vbo_ = line_renderer::create_static_vertex_buffer(
                device, kSegmentCorners.data(), sizeof(CornerVertex) * kSegmentCorners.size());
        }

        if (!join_corner_vbo_) {
            join_corner_vbo_ = line_renderer::create_static_vertex_buffer(
                device, kBevelJoinCorners.data(), sizeof(JoinCornerVertex) * kBevelJoinCorners.size());
        }

        line_renderer::ensure_shader_pair(device,
                                          shader_handle_,
        ...
        line_renderer::ensure_fragment_shader(device,
                                              lit_shader_handle_,
    ...
    void WorldSpaceLineRenderer::ensure_cap_template(RenderContext2& ctx, int round_segments) {
        round_segments = std::clamp(round_segments, 4, 64);
        ...
        cap_corner_vbo_ = line_renderer::create_static_vertex_buffer(
            device, corners.data(), sizeof(CapCornerVertex) * corners.size());
    }

    void WorldSpaceLineRenderer::ensure_round_join_template(RenderContext2& ctx, int round_segments) {
        round_segments = std::clamp(round_segments, 4, 64);
    ...
    void WorldSpaceLineRenderer::draw_polyline(RenderContext2& ctx,
        ...
        if (style.join == LineJoinStyle::Round) {
            round_join_instances.reserve(clean_points.size() > 2 ? clean_points.size() - 2 : 0);
        ...
        const ShaderHandle segment_fragment_shader =
            params.fragment_shader ? params.fragment_shader
        ...
        const line_renderer::UploadedInstanceStream segment_stream =
            line_renderer::upload_instance_stream(ctx, instances.data(), instances.size() * sizeof(SegmentInstance));
        if (!segment_stream.buffer) {
            return;
        }

        WorldLinePush push{};
        std::memcpy(push.view_projection, params.view_projection.data(), sizeof(push.view_projection));
        ...
        ctx.draw_arrays_instanced(corner_vbo_,
                                  0,
        ...
        if (!cap_instances.empty()) {
            ...
            const line_renderer::UploadedInstanceStream cap_stream = line_renderer::upload_instance_stream(
                ctx, cap_instances.data(), cap_instances.size() * sizeof(CapInstance));
            ...
            const ShaderHandle cap_selected_fragment_shader =
                params.fragment_shader ? params.fragment_shader
            ...
            bind_world_line_shader(ctx, cap_shader_handle_, cap_vertex_shader_, cap_selected_fragment_shader, push);
        ...
        if (!round_join_instances.empty()) {
            ...
            const line_renderer::UploadedInstanceStream round_join_stream = line_renderer::upload_instance_stream(
                ctx, round_join_instances.data(), round_join_instances.size() * sizeof(CapInstance));
            ...
            const ShaderHandle round_join_selected_fragment_shader =
                params.fragment_shader ? params.fragment_shader
            ...
            bind_world_line_shader(
                ctx, round_join_shader_handle_, round_join_vertex_shader_, round_join_selected_fragment_shader, push);
            ctx.set_vertex_layouts(round_join_layouts, 2);
            ctx.set_topology(PrimitiveTopology::TriangleList);
            ctx.draw_arrays_instanced(round_join_corner_vbo_,
                                      0,
        ...
        if (!join_instances.empty()) {
            const line_renderer::UploadedInstanceStream join_stream = line_renderer::upload_instance_stream(
                ctx, join_instances.data(), join_instances.size() * sizeof(JoinInstance));
            ...
            const ShaderHandle join_selected_fragment_shader =
                params.fragment_shader ? params.fragment_shader
            ...
            bind_world_line_shader(ctx, join_shader_handle_, join_vertex_shader_, join_selected_fragment_shader, push);
            ctx.set_vertex_layouts(join_layouts, 2);
            ctx.set_topology(PrimitiveTopology::TriangleList);
            ctx.draw_arrays_instanced(join_corner_vbo_,
                                      0,
                                      join_stream.buffer,
                                      join_stream.offset,
