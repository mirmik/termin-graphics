...
namespace tgfx {
    ...
    D3D11Swapchain::D3D11Swapchain(
        D3D11RenderDevice& device, HWND hwnd, uint32_t width, uint32_t height, PresentationMode presentation_mode)
        : device_(device),
    ...
    void D3D11Swapchain::create_swapchain(HWND hwnd) {
        ...
        throw_if_failed(device_.native_device()->QueryInterface(__uuidof(IDXGIDevice),
        ...
        throw_if_failed(adapter->GetParent(__uuidof(IDXGIFactory2), reinterpret_cast<void**>(factory.GetAddressOf())),
        ...
        if (SUCCEEDED(factory.As(&factory5)) && factory5) {
            ...
            const HRESULT feature_hr = factory5->CheckFeatureSupport(
                DXGI_FEATURE_PRESENT_ALLOW_TEARING, &native_tearing_supported, sizeof(native_tearing_supported));
            if (SUCCEEDED(feature_hr)) {
                tearing_supported = native_tearing_supported == TRUE;
            } else {
                tc::Log::warn("D3D11Swapchain: DXGI tearing capability query failed: HRESULT=0x%08X",
                              static_cast<unsigned>(feature_hr));
            }
        }

        presentation_ = resolve_d3d11_presentation(presentation_.requested_mode, tearing_supported);
        if (!presentation_.supported) {
            tc::Log::error("D3D11Swapchain: requested presentation mode 'immediate' requires "
                           "DXGI_FEATURE_PRESENT_ALLOW_TEARING support");
            throw std::runtime_error("D3D11Swapchain: requested presentation mode 'immediate' is unsupported: "
                                     "DXGI_FEATURE_PRESENT_ALLOW_TEARING is unavailable");
        }
        swapchain_flags_ = presentation_.allow_tearing ? static_cast<uint32_t>(DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING) : 0u;
        ...
        throw_if_failed(
            factory->CreateSwapChainForHwnd(device_.native_device(), hwnd, &desc, nullptr, nullptr, &swapchain_),
    ...
    void D3D11Swapchain::refresh_backbuffer_texture() {
        ...
        backbuffer_texture_ = device_.register_external_texture(reinterpret_cast<uintptr_t>(backbuffer.Get()), desc);
        if (!backbuffer_texture_) {
    ...
    bool D3D11Swapchain::present(uint32_t sync_interval) {
        const bool valid_interval = presentation_.effective_mode == PresentationMode::VSync
                                        ...
                                        : sync_interval == 0;
        if (!valid_interval) {
            tc::Log::error("D3D11Swapchain::present: sync interval %u conflicts with construction-time "
                           "presentation mode '%s'",
                           sync_interval,
                           presentation_.effective_mode == PresentationMode::VSync ? "vsync" : "immediate");
            return false;
        }
        const UINT present_flags = presentation_.allow_tearing ? DXGI_PRESENT_ALLOW_TEARING : 0u;
    ...
    bool D3D11Swapchain::compose_and_present(TextureHandle color_texture, uint32_t sync_interval) {
        ...
        if (src_desc.width == 0 || src_desc.height == 0) {
            tc::Log::error("D3D11Swapchain::compose_and_present: invalid source texture id=%u", color_texture.id);
            return false;
        }
        const termin::Bounds2i destination = aspect_fit_rect(src_desc.width, src_desc.height, width_, height_);
        ...
        if (!source || !source->texture || !target || !target->texture || !target->rtv) {
            tc::Log::error("D3D11Swapchain::compose_and_present: stale native texture source=%u backbuffer=%u",
                           color_texture.id,
    ...
    void D3D11Swapchain::resize(uint32_t width, uint32_t height) {
        ...
        device_.wait_idle();
        release_backbuffer_texture();
        throw_if_failed(swapchain_->ResizeBuffers(0, width_, height_, DXGI_FORMAT_UNKNOWN, swapchain_flags_),
