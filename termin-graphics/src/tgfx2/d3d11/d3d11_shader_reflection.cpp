...
namespace tgfx::d3d11_internal {
    ...
    std::vector<D3D11InputSemantic> reflect_d3d11_vertex_inputs(const D3D11ShaderModule& vs) {
        ...
        HRESULT hr = D3DReflect(vs.bytecode.data(),
                                vs.bytecode.size(),
                                __uuidof(ID3D11ShaderReflection),
                                reinterpret_cast<void**>(reflection.GetAddressOf()));
        if (FAILED(hr) || !reflection) {
            tc::Log::error("D3D11RenderDevice::create_pipeline: D3DReflect failed for vertex shader: HRESULT=0x%08X",
        ...
        if (FAILED(hr)) {
            tc::Log::error("D3D11RenderDevice::create_pipeline: shader reflection GetDesc failed: HRESULT=0x%08X",
                           static_cast<unsigned>(hr));
            return out;
        }
    ...
    std::vector<D3D11SignatureParam> reflect_d3d11_signature(const D3D11ShaderModule& shader, bool output_signature) {
        ...
        Microsoft::WRL::ComPtr<ID3D11ShaderReflection> reflection;
        HRESULT hr = D3DReflect(shader.bytecode.data(),
                                shader.bytecode.size(),
                                __uuidof(ID3D11ShaderReflection),
                                reinterpret_cast<void**>(reflection.GetAddressOf()));
        if (FAILED(hr) || !reflection) {
            tc::Log::error("D3D11RenderDevice::create_pipeline: D3DReflect failed for shader '%s': HRESULT=0x%08X",
        ...
        const UINT count = output_signature ? shader_desc.OutputParameters : shader_desc.InputParameters;
        out.reserve(count);
        for (UINT i = 0; i < count; ++i) {
            D3D11_SIGNATURE_PARAMETER_DESC param{};
            hr = output_signature ? reflection->GetOutputParameterDesc(i, &param)
    ...
    static const D3D11SignatureParam* find_signature_param(const std::vector<D3D11SignatureParam>& params,
    ...
    void log_d3d11_shader_signatures(const char* reason,
                                     ...
                                     const std::vector<D3D11SignatureParam>& ps_inputs) {
        tc::Log::warn("D3D11RenderDevice::create_pipeline: shader signature diagnostic (%s): vs='%s' ps='%s'",
                      reason,
                      vs.debug_name.c_str(),
                      ps.debug_name.c_str());
        for (size_t i = 0; i < vs_outputs.size(); ++i) {
            const D3D11SignatureParam& p = vs_outputs[i];
            tc::Log::warn("  vs_out[%zu]: %s%u reg=%u mask=0x%02X sys=%u",
                          i,
                          p.semantic.c_str(),
                          p.semantic_index,
                          p.register_index,
        ...
        for (size_t i = 0; i < ps_inputs.size(); ++i) {
            ...
            tc::Log::warn("  ps_in[%zu]: %s%u reg=%u mask=0x%02X sys=%u",
                          i,
                          p.semantic.c_str(),
                          p.semantic_index,
                          p.register_index,
    ...
    void log_d3d11_input_layout_failure(HRESULT hr,
        ...
        tc::Log::error("D3D11RenderDevice::create_pipeline: CreateInputLayout failed: HRESULT=0x%08X "
