...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
  ...
  "resources": [
    ...
    },
    {
      ...
      "webgpu": {"group": 0, "binding": 1, "sampler_binding": 2}
        ...
        wgpu::BufferUsage buffer_usage(const BufferDesc& desc) {
            ...
            if (has_flag(desc.usage, BufferUsage::CopySrc))
                usage |= wgpu::BufferUsage::CopySrc;
            if (has_flag(desc.usage, BufferUsage::CopyDst))
        ...
        wgpu::TextureUsage texture_usage(TextureUsage value) {
            ...
            if (has_flag(value, TextureUsage::Sampled))
                usage |= wgpu::TextureUsage::TextureBinding;
            if (has_flag(value, TextureUsage::Storage))
            ...
            if (has_flag(value, TextureUsage::CopySrc))
                usage |= wgpu::TextureUsage::CopySrc;
            if (has_flag(value, TextureUsage::CopyDst))
        ...
        uint32_t bytes_per_pixel(PixelFormat format) {
            ...
            case PixelFormat::RGBA32F:
                return 16;
            default:
                fail("texture upload format is unsupported");
            }
        }
        ...
        wgpu::AddressMode address(AddressMode mode) {
            ...
            case AddressMode::ClampToBorder:
                fail("ClampToBorder samplers are unsupported by WebGPU");
            }
        ...
        bool rect_inside(termin::Bounds2i rect, uint32_t width, uint32_t height) {
            return rect.width() > 0 && rect.height() > 0 && rect.x0 >= 0 && rect.y0 >= 0 &&
                   rect.x1 <= static_cast<int>(width) && rect.y1 <= static_cast<int>(height);
        }
    ...
    void WebGpuRenderDevice::request_async(const WebGpuDeviceRequest& request, WebGpuDeviceCallback callback) {
        if (!callback)
            fail("request_async requires a callback");
        ...
        instance.RequestAdapter(
            ...
            [instance, surface, request, callback = std::move(callback)](
                wgpu::RequestAdapterStatus status, wgpu::Adapter adapter, wgpu::StringView message) mutable {
                if (status != wgpu::RequestAdapterStatus::Success) {
                    callback(nullptr, "WebGPU adapter request failed: " + std::string(message.data, message.length));
                    return;
                }
                wgpu::DeviceDescriptor device_desc;
                auto error_state = std::make_shared<WebGpuErrorState>();
                device_desc.SetUncapturedErrorCallback(
                    [](const wgpu::Device&, wgpu::ErrorType type, wgpu::StringView error, WebGpuErrorState* state) {
                        state->failed = true;
                ...
                device_desc.SetDeviceLostCallback(
                    ...
                    [](const wgpu::Device&,
                       ...
                       WebGpuErrorState* state) {
                        if (reason == wgpu::DeviceLostReason::Destroyed)
                            return;
                        state->failed = true;
                        state->message = "device lost: " + std::string(error.data, error.length);
                ...
                adapter.RequestDevice(
                    &device_desc,
                    wgpu::CallbackMode::AllowSpontaneous,
                    [instance, surface, adapter, request, error_state, callback = std::move(callback)](
                        wgpu::RequestDeviceStatus device_status,
                        wgpu::Device device,
                        wgpu::StringView device_message) mutable {
                        ...
                        try {
                            callback(
                                std::unique_ptr<WebGpuRenderDevice>(new WebGpuRenderDevice(
                                    instance, adapter, device, surface, request.width, request.height, error_state)),
                                {});
                        } catch (const std::exception& error) {
    ...
    WebGpuRenderDevice::WebGpuRenderDevice(wgpu::Instance instance,
    ...
    void WebGpuRenderDevice::ensure_texture_op_state() {
        ...
        auto create_op_shader =
            [this](ShaderStage stage, std::string_view source, std::string_view layout, const char* name) {
                ShaderDesc desc;
                desc.stage = stage;
                desc.source = source;
        ...
        texture_ops_->clear_vertex = create_op_shader(
            ShaderStage::Vertex, texture_op_clear_wgsl, texture_op_clear_layout, "webgpu-texture-clear-vs");
        texture_ops_->clear_fragment = create_op_shader(
            ShaderStage::Fragment, texture_op_clear_wgsl, texture_op_clear_layout, "webgpu-texture-clear-fs");
        texture_ops_->blit_vertex = create_op_shader(
            ShaderStage::Vertex, texture_op_blit_wgsl, texture_op_blit_layout, "webgpu-texture-blit-vs");
        texture_ops_->blit_fragment = create_op_shader(
            ShaderStage::Fragment, texture_op_blit_wgsl, texture_op_blit_layout, "webgpu-texture-blit-fs");
    ...
    PipelineHandle WebGpuRenderDevice::texture_op_pipeline(PixelFormat format, uint32_t sample_count, bool blit) {
        ensure_texture_op_state();
        const uint64_t key = (static_cast<uint64_t>(static_cast<uint32_t>(format)) << 32) | sample_count;
        auto& cache = blit ? texture_ops_->blit_pipelines : texture_ops_->clear_pipelines;
        const auto found = cache.find(key);
        if (found != cache.end())
            return found->second;

        PipelineDesc desc;
        desc.vertex_shader = blit ? texture_ops_->blit_vertex : texture_ops_->clear_vertex;
        desc.fragment_shader = blit ? texture_ops_->blit_fragment : texture_ops_->clear_fragment;
    ...
    ResourceSetHandle
    WebGpuRenderDevice::texture_op_resource_set(PipelineHandle pipeline, TextureHandle source, bool blit) {
        ensure_texture_op_state();
        if (!blit) {
            const auto found = texture_ops_->clear_sets.find(pipeline.id);
            if (found != texture_ops_->clear_sets.end())
                return found->second;
        } else {
            const uint64_t key = (static_cast<uint64_t>(pipeline.id) << 32) | source.id;
            const auto found = texture_ops_->blit_sets.find(key);
            if (found != texture_ops_->blit_sets.end())
                return found->second;
        }
        ...
        bindings[0].value.buffer = blit ? texture_ops_->blit_params : texture_ops_->clear_params;
    ...
    void WebGpuRenderDevice::release_texture_op_source(TextureHandle source) {
        if (!texture_ops_)
            return;
        for (auto it = texture_ops_->blit_sets.begin(); it != texture_ops_->blit_sets.end();) {
    ...
    void WebGpuRenderDevice::release_texture_op_state() {
        if (!texture_ops_)
            return;
        for (const auto& [_, set] : texture_ops_->blit_sets)
            destroy(set);
        for (const auto& [_, set] : texture_ops_->clear_sets)
            destroy(set);
        for (const auto& [_, pipeline] : texture_ops_->blit_pipelines)
            destroy(pipeline);
        for (const auto& [_, pipeline] : texture_ops_->clear_pipelines)
            destroy(pipeline);
        if (texture_ops_->clear_vertex)
            destroy(texture_ops_->clear_vertex);
        if (texture_ops_->clear_fragment)
            destroy(texture_ops_->clear_fragment);
        if (texture_ops_->blit_vertex)
            destroy(texture_ops_->blit_vertex);
        if (texture_ops_->blit_fragment)
            destroy(texture_ops_->blit_fragment);
        if (texture_ops_->clear_params)
            destroy(texture_ops_->clear_params);
        if (texture_ops_->blit_params)
            destroy(texture_ops_->blit_params);
        if (texture_ops_->linear_sampler)
            destroy(texture_ops_->linear_sampler);
        texture_ops_.reset();
    }
    ...
    void WebGpuRenderDevice::clear_texture(TextureHandle dst_handle, termin::Color4 color, termin::Bounds2i viewport) {
        ...
        if (!color_format(dst->desc.format) || !has_flag(dst->desc.usage, TextureUsage::ColorAttachment)) {
    ...
    void WebGpuRenderDevice::blit_to_texture(TextureHandle dst_handle,
        ...
        const WebGpuTexture* dst = textures_.get(dst_handle.id);
        if (!src || !dst)
            fail("blit_to_texture requires valid textures");
        if (src_handle == dst_handle)
            fail("blit_to_texture does not support self-blits");
        ...
        const bool fast_copy = src_rect.width() == dst_rect.width() && src_rect.height() == dst_rect.height() &&
                               src->desc.format == dst->desc.format && src->desc.sample_count == 1 &&
                               dst->desc.sample_count == 1 && has_flag(src->desc.usage, TextureUsage::CopySrc) &&
                               has_flag(dst->desc.usage, TextureUsage::CopyDst);
        if (fast_copy) {
            wgpu::CommandEncoder encoder = device_.CreateCommandEncoder();
            wgpu::TexelCopyTextureInfo source;
            source.texture = src->object;
            source.origin = {static_cast<uint32_t>(src_rect.x0), static_cast<uint32_t>(src_rect.y0), 0};
            wgpu::TexelCopyTextureInfo destination;
            destination.texture = dst->object;
            destination.origin = {static_cast<uint32_t>(dst_rect.x0), static_cast<uint32_t>(dst_rect.y0), 0};
            const wgpu::Extent3D extent{
                static_cast<uint32_t>(src_rect.width()), static_cast<uint32_t>(src_rect.height()), 1};
        ...
        ensure_texture_op_state();
        upload_buffer(texture_ops_->blit_params,
                      std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(params.data()), sizeof(params)));
        const PipelineHandle pipeline = texture_op_pipeline(dst->desc.format, dst->desc.sample_count, true);
        RenderPassDesc pass;
        ColorAttachmentDesc attachment;
        attachment.texture = dst_handle;
        ...
        commands->begin();
        commands->begin_render_pass(pass);
        commands->set_viewport(dst_rect.x0, dst_rect.y0, dst_rect.width(), dst_rect.height());
        commands->set_scissor(dst_rect.x0, dst_rect.y0, dst_rect.width(), dst_rect.height());
        commands->bind_pipeline(pipeline);
        commands->bind_resource_set(texture_op_resource_set(pipeline, src_handle, true));
        commands->draw(3);
        commands->end_render_pass();
        commands->end();
        submit(*commands);
    ...
    TextureHandle WebGpuRenderDevice::create_texture(const TextureDesc& desc) {
        if (desc.array_layers != 1) {
            tc_log(TC_LOG_ERROR, "WebGpuRenderDevice::create_texture: layered textures are not supported");
            return {};
        }
        ...
        if (native.usage == wgpu::TextureUsage::None)
            fail("create_texture requires usage flags");
    ...
    ShaderHandle WebGpuRenderDevice::create_shader(const ShaderDesc& desc) {
        if (desc.stage == ShaderStage::Geometry)
            fail("geometry shaders are unsupported");
        if (desc.source.empty())
            fail("WebGPU shader requires prebuilt WGSL source");
        ...
        if (!object)
            fail("CreateShaderModule failed for '" + desc.debug_name + "'");
    ...
    PipelineHandle WebGpuRenderDevice::create_pipeline(const PipelineDesc& desc) {
        ...
        if (!vertex || !fragment)
            fail("pipeline references invalid shaders");
        ...
        std::sort(native_layout.begin(), native_layout.end(), [](const auto& a, const auto& b) {
        ...
        for (size_t i = 0; i < desc.vertex_layouts.size(); ++i) {
            ...
            buffers[i].stepMode = source.per_instance ? wgpu::VertexStepMode::Instance : wgpu::VertexStepMode::Vertex;
        ...
        if (!object)
            fail("CreateRenderPipeline failed");
        WebGpuPipeline pipeline{std::move(object), std::move(bind_group_layout), desc, 0, std::move(layout)};
        const uint32_t id = pipelines_.add(std::move(pipeline));
        pipelines_.get(id)->layout_token = id;
        return {id};
    }

    ResourceSetHandle WebGpuRenderDevice::create_bound_resource_set(const BoundResourceSetDesc& desc) {
        ...
        for (uint32_t group_index = 0; group_index < desc.group_count; ++group_index) {
            ...
            for (uint32_t binding_index = 0; binding_index < group.binding_count; ++binding_index) {
                ...
                const auto layout_item =
                    std::find_if(pipeline->layout.begin(), pipeline->layout.end(), [&](const WebGpuLayoutEntry& item) {
                        return item.binding == binding.slot.placement.webgpu.binding;
                    });
                if (layout_item == pipeline->layout.end() || layout_item->kind != binding.slot.kind) {
                    fail(std::string("resource '") + bound_resource_debug_name(binding) +
                         "' does not match the pipeline layout");
                }
                const bool value_kind_matches = (layout_item->kind == ShaderResourceKind::ConstantBuffer &&
                ...
                case BoundResourceKind::StorageBuffer: {
                    ...
                    if (!buffer)
                        fail("resource set references an invalid buffer");
                    native.buffer = buffer->object;
                    native.offset = binding.value.offset;
                    native.size =
                        binding.value.range == 0 ? buffer->desc.size - binding.value.offset : binding.value.range;
        ...
        if (!object)
            fail("CreateBindGroup failed");
        return {resource_sets_.add({std::move(object), desc.resource_layout_token})};
    }

    void WebGpuRenderDevice::destroy(BufferHandle handle) {
        buffers_.remove(handle.id);
    }
    void WebGpuRenderDevice::destroy(TextureHandle handle) {
        release_texture_op_source(handle);
        if (handle == acquired_surface_texture_)
            acquired_surface_texture_ = {};
        textures_.remove(handle.id);
    }
    void WebGpuRenderDevice::destroy(SamplerHandle handle) {
        samplers_.remove(handle.id);
    }
    void WebGpuRenderDevice::destroy(ShaderHandle handle) {
        shaders_.remove(handle.id);
    }
    void WebGpuRenderDevice::destroy(PipelineHandle handle) {
        pipelines_.remove(handle.id);
    }
    void WebGpuRenderDevice::destroy(ResourceSetHandle handle) {
        resource_sets_.remove(handle.id);
    }

    void WebGpuRenderDevice::upload_buffer(BufferHandle dst, std::span<const uint8_t> data, uint64_t offset) {
        const WebGpuBuffer* buffer = buffers_.get(dst.id);
        if (!buffer || offset + data.size() > buffer->desc.size)
            fail("buffer upload is out of bounds");
        queue_.WriteBuffer(buffer->object, offset, data.data(), data.size());
    }

    void WebGpuRenderDevice::upload_texture(TextureHandle dst, std::span<const uint8_t> data, uint32_t mip) {
        const WebGpuTexture* texture = textures_.get(dst.id);
        if (!texture)
            fail("texture upload references an invalid texture");
    ...
    void WebGpuRenderDevice::upload_texture_region(TextureHandle dst,
                                                   uint32_t x,
                                                   uint32_t y,
                                                   uint32_t width,
                                                   uint32_t height,
                                                   ...
                                                   uint32_t mip) {
        const WebGpuTexture* texture = textures_.get(dst.id);
        if (!texture || mip >= texture->desc.mip_levels)
            fail("invalid texture upload target");
        ...
        if (x + width > mip_width || y + height > mip_height || data.size() != static_cast<size_t>(stride) * height) {
    ...
    void WebGpuRenderDevice::read_buffer(BufferHandle, std::span<uint8_t>, uint64_t) {
        fail("synchronous buffer readback is unsupported by WebGPU; use an async API");
    }

    uintptr_t WebGpuRenderDevice::pipeline_resource_layout_token(PipelineHandle handle) const {
    ...
    std::unique_ptr<ICommandList> WebGpuRenderDevice::create_command_list(QueueType queue) {
        if (queue != QueueType::Graphics)
            fail("only the WebGPU graphics queue is exposed");
    ...
    void WebGpuRenderDevice::submit(ICommandList& command_list) {
        ...
        if (!webgpu)
            fail("command list belongs to another backend");
        wgpu::CommandBuffer command = webgpu->take_command_buffer();
        if (!command)
            fail("command list was not ended before submit");
        queue_.Submit(1, &command);
    }

    void WebGpuRenderDevice::present() {
        if (!acquired_surface_texture_)
            fail("present called without an acquired surface texture");
    ...
    void WebGpuRenderDevice::configure_surface(uint32_t width, uint32_t height) {
        if (width == 0 || height == 0)
            fail("surface extent must be non-zero");
        if (acquired_surface_texture_) {
            textures_.remove(acquired_surface_texture_.id);
            acquired_surface_texture_ = {};
        }
    ...
    PixelFormat WebGpuRenderDevice::surface_pixel_format() const {
        if (surface_format_ == wgpu::TextureFormat::BGRA8Unorm)
            return PixelFormat::BGRA8_UNorm;
        if (surface_format_ == wgpu::TextureFormat::RGBA8Unorm)
            return PixelFormat::RGBA8_UNorm;
        if (surface_format_ == wgpu::TextureFormat::BGRA8UnormSrgb)
            return PixelFormat::BGRA8_sRGB;
        if (surface_format_ == wgpu::TextureFormat::RGBA8UnormSrgb)
