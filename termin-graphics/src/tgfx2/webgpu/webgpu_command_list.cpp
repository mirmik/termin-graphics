...
namespace tgfx {
    ...
    WebGpuCommandList::WebGpuCommandList(WebGpuRenderDevice& device)
        : device_(device) {}

    void WebGpuCommandList::begin() {
        if (recording_)
            command_fail("begin called while recording");
    ...
    void WebGpuCommandList::end() {
        if (!recording_ || in_render_pass_)
            command_fail("end requires an ended render pass");
    ...
    void WebGpuCommandList::begin_render_pass(const RenderPassDesc& pass) {
        if (!recording_ || in_render_pass_)
            command_fail("invalid begin_render_pass state");
    ...
    void WebGpuCommandList::end_render_pass() {
        if (!in_render_pass_)
            command_fail("no active render pass");
        render_pass_.End();
    ...
    void WebGpuCommandList::bind_pipeline(PipelineHandle handle) {
        if (!in_render_pass_)
            command_fail("bind_pipeline requires a render pass");
    ...
    void WebGpuCommandList::bind_resource_set(ResourceSetHandle handle,
        ...
        if (!in_render_pass_)
            command_fail("bind_resource_set requires a render pass");
        if (set_index != 0)
            command_fail("only WebGPU bind group 0 is supported");
        ...
        if (!set)
            command_fail("invalid resource set handle");
    ...
    void WebGpuCommandList::bind_vertex_buffer(uint32_t slot, BufferHandle handle, uint64_t offset) {
        if (!in_render_pass_)
            command_fail("bind_vertex_buffer requires a render pass");
        const WebGpuBuffer* buffer = device_.buffers_.get(handle.id);
        if (!buffer || offset >= buffer->desc.size)
            command_fail("invalid vertex buffer");
        render_pass_.SetVertexBuffer(slot, buffer->object, offset, buffer->desc.size - offset);
    }

    void WebGpuCommandList::bind_index_buffer(BufferHandle handle, IndexType type, uint64_t offset) {
        if (!in_render_pass_)
            command_fail("bind_index_buffer requires a render pass");
        const WebGpuBuffer* buffer = device_.buffers_.get(handle.id);
        if (!buffer || offset >= buffer->desc.size)
            command_fail("invalid index buffer");
        render_pass_.SetIndexBuffer(buffer->object,
    ...
    void
    WebGpuCommandList::draw_instanced(uint32_t count, uint32_t instances, uint32_t first, uint32_t first_instance) {
        if (!in_render_pass_)
            command_fail("draw requires a render pass");
        render_pass_.Draw(count, instances, first, first_instance);
    }

    void WebGpuCommandList::draw_indexed(uint32_t count, uint32_t first, int32_t vertex_offset) {
    ...
    void WebGpuCommandList::copy_buffer(
        BufferHandle src_handle, BufferHandle dst_handle, uint64_t size, uint64_t src_offset, uint64_t dst_offset) {
        if (!recording_ || in_render_pass_)
            command_fail("copy_buffer requires recording outside a pass");
        const WebGpuBuffer* src = device_.buffers_.get(src_handle.id);
        const WebGpuBuffer* dst = device_.buffers_.get(dst_handle.id);
        if (!src || !dst || src_offset + size > src->desc.size || dst_offset + size > dst->desc.size)
            command_fail("invalid buffer copy");
    ...
    void WebGpuCommandList::copy_texture(TextureHandle src_handle, TextureHandle dst_handle) {
        if (!recording_ || in_render_pass_)
            command_fail("copy_texture requires recording outside a pass");
        ...
        wgpu::Extent3D extent{
            std::min(src->desc.width, dst->desc.width), std::min(src->desc.height, dst->desc.height), 1};
    ...
    void WebGpuCommandList::set_viewport(int x, int y, int width, int height) {
        ...
        render_pass_.SetViewport(static_cast<float>(x),
                                 static_cast<float>(y),
