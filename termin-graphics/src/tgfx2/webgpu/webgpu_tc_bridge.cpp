...
namespace tgfx {
    namespace {
        ...
        bool
        read_text_file(const termin::ShaderArtifactResolver& resolver, const std::string& path, std::string& result) {
        ...
        bool load_webgpu_stage(const termin::ShaderArtifactResolver& resolver,
                               tc_shader* shader,
                               ShaderStage stage,
                               ShaderDesc& desc) {
            ...
            return read_text_file(resolver, artifact_path + ".layout.json", desc.resource_layout_json);
        }

    } // namespace

    bool WebGpuRenderDevice::ensure_tc_shader(tc_shader* shader, ShaderHandle* out_vs, ShaderHandle* out_fs) {
        if (!shader || !out_fs) {
        ...
        if (cached != tc_shader_cache_.end() && cached->second.version == shader->version &&
            ...
            cached->second.fs && (!has_vs || cached->second.vs)) {
            if (out_vs)
                *out_vs = cached->second.vs;
        ...
        try {
            if (has_vs) {
                ...
                if (shader->vertex_entry && shader->vertex_entry[0])
                    desc.entry_point = shader->vertex_entry;
            ...
            if (shader->fragment_entry && shader->fragment_entry[0])
                desc.entry_point = shader->fragment_entry;
            if (!load_webgpu_stage(resolver, shader, desc.stage, desc)) {
                if (vs)
                    destroy(vs);
                return false;
            ...
            tc_shader_cache_.emplace(shader->pool_index,
            ...
            *out_fs = fs;
            ...
            if (vs)
                destroy(vs);
            tc::Log::error("WebGpuRenderDevice::ensure_tc_shader: failed for '%s': %s",
    ...
    TextureHandle WebGpuRenderDevice::ensure_tc_texture(tc_texture* texture) {
        ...
        if (cached != tc_texture_cache_.end()) {
            if (cached->second.handle)
                destroy(cached->second.handle);
        ...
        if (gpu_first) {
            ...
            desc.format = pixel_format_for_tc_texture(static_cast<tc_texture_format>(texture->format),
            ...
            if (!prepare_tc_texture_upload(texture, upload))
                return {};
            ...
            tc_texture_cache_.emplace(texture->header.pool_index,
    ...
    std::pair<BufferHandle, BufferHandle> WebGpuRenderDevice::ensure_tc_mesh(tc_mesh* mesh) {
        ...
        if (cached != tc_mesh_cache_.end()) {
            if (cached->second.vbo)
                destroy(cached->second.vbo);
            if (cached->second.ebo)
                destroy(cached->second.ebo);
        ...
        if (!vbo)
            return {};
        upload_buffer(vbo, std::span<const uint8_t>(static_cast<const uint8_t*>(mesh->vertices), vertex_bytes));
        BufferHandle ebo = create_buffer({index_bytes, BufferUsage::Index | BufferUsage::CopyDst});
        if (!ebo) {
            destroy(vbo);
            return {};
        }
        upload_buffer(ebo, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(mesh->indices), index_bytes));
        tc_mesh_cache_.emplace(mesh->header.pool_index, CachedTcMeshEntry{vbo, ebo, mesh->header.version});
        return {vbo, ebo};
    }

    void WebGpuRenderDevice::invalidate_tc_mesh_cache(uint32_t pool_index) {
        auto entry = tc_mesh_cache_.find(pool_index);
        if (entry == tc_mesh_cache_.end())
            return;
        if (entry->second.vbo)
            destroy(entry->second.vbo);
        if (entry->second.ebo)
            destroy(entry->second.ebo);
