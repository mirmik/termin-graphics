...
namespace tgfx {
    ...
    void OpenGLCommandList::begin_render_pass(const RenderPassDesc& pass) {
        ...
        device_.enforce_clip_space_contract();
    ...
    void OpenGLCommandList::bind_pipeline(PipelineHandle pipeline) {
        ...
        if (blend.enabled) {
            glEnable(GL_BLEND);
            glBlendFuncSeparate(gl::to_gl_blend_factor(blend.src_color),
                                gl::to_gl_blend_factor(blend.dst_color),
                                gl::to_gl_blend_factor(blend.src_alpha),
                                gl::to_gl_blend_factor(blend.dst_alpha));
            glBlendEquationSeparate(gl::to_gl_blend_op(blend.color_op), gl::to_gl_blend_op(blend.alpha_op));
        } else {
            glDisable(GL_BLEND);
        }
    ...
    void OpenGLCommandList::apply_pending_push_constants() {
        if (pending_push_size_ == 0)
            return;
        GLuint ubo = device_.push_constants_ring_buffer();
        if (ubo == 0)
            return;
        glBindBufferRange(
            GL_UNIFORM_BUFFER, TGFX2_PUSH_CONSTANTS_BINDING, ubo, pending_push_offset_, pending_push_size_);
    }
    ...
    void bind_bound_resource_binding(OpenGLRenderDevice& device, const BoundResourceBinding& binding) {
        ...
        case OpenGLBindingClass::StorageBuffer: {
            ...
            if (buf) {
                if (binding.value.range > 0) {
                    glBindBufferRange(0x90D2 /*GL_SHADER_STORAGE_BUFFER*/,
                                      placement.binding_point,
                                      buf->gl_id,
                                      static_cast<GLintptr>(binding.value.offset),
                                      static_cast<GLsizeiptr>(binding.value.range));
                } else {
                    glBindBufferBase(0x90D2 /*GL_SHADER_STORAGE_BUFFER*/, placement.binding_point, buf->gl_id);
                }
            }
            break;
        }
        ...
        case OpenGLBindingClass::ImageUnit:
            tc::Log::error("OpenGLCommandList::bind_resource_set: image bindings are not implemented");
            break;
        case OpenGLBindingClass::None:
        default:
            tc::Log::error("OpenGLCommandList::bind_resource_set: missing OpenGL placement for resource '%s'",
    ...
    void OpenGLCommandList::bind_resource_set(ResourceSetHandle set,
        ...
        if (dynamic_offsets != nullptr || dynamic_offset_count != 0) {
            tc::Log::error("OpenGLCommandList::bind_resource_set: dynamic uniform offsets are "
            ...
            for (uint32_t i = 0; i < attribute_count; ++i) {
                ...
                if (gl::vertex_format_is_integer(attr.format)) {
                    glVertexAttribIPointer(
                        attr.location, count, gl_type, layout.stride, reinterpret_cast<const void*>(ptr_offset));
                } else {
    ...
    void OpenGLCommandList::draw_instanced(uint32_t vertex_count,
        ...
        glDrawArraysInstanced(current_topology_, first_vertex, vertex_count, instance_count);
    }
    ...
    void OpenGLCommandList::draw_indexed_instanced(uint32_t index_count,
        ...
        if (first_instance != 0) {
            static bool warned = false;
            if (!warned) {
                tc::Log::error("OpenGLCommandList::draw_indexed_instanced: first_instance is not supported by the "
                               "OpenGL path yet");
                warned = true;
            }
        }
    ...
    void OpenGLCommandList::copy_buffer(
    ...
    void OpenGLCommandList::copy_texture(TextureHandle src, TextureHandle dst) {
        ...
        auto is_depth = [](PixelFormat f) {
            return f == PixelFormat::D24_UNorm || f == PixelFormat::D24_UNorm_S8_UInt || f == PixelFormat::D32F;
        };
        ...
        glBlitFramebuffer(0, 0, s->desc.width, s->desc.height, 0, 0, d->desc.width, d->desc.height, bit, filter);
