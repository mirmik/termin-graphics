...
namespace tgfx::gl {
    ...
    GLenum to_gl_min_filter(FilterMode min, FilterMode mip) {
        if (min == FilterMode::Nearest && mip == FilterMode::Nearest)
            return GL_NEAREST_MIPMAP_NEAREST;
        if (min == FilterMode::Linear && mip == FilterMode::Nearest)
            return GL_LINEAR_MIPMAP_NEAREST;
        if (min == FilterMode::Nearest && mip == FilterMode::Linear)
    ...
    GLenum to_gl_shader_stage(ShaderStage stage) {
        ...
        case ShaderStage::Compute:
            return GL_VERTEX_SHADER; // Compute not available in GL 3.3
        }
        return GL_VERTEX_SHADER;
    }
