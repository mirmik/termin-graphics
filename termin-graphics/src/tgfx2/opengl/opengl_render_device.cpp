...
uint64_t OpenGLRenderDevice::request_pixel_rgba8(
    TextureHandle texture, int x, int y
) {
    return request_pixel_readback(texture, x, y, PixelReadbackKind::Rgba8);
}

bool OpenGLRenderDevice::poll_pixel_rgba8(
    uint64_t request_id, float out_rgba[4]
) {
    ...
    if (slot->kind != PixelReadbackKind::Rgba8) {
        tc_log_error(
            "OpenGLRenderDevice::poll_pixel_rgba8: request %llu has the wrong kind",
    ...
    if (!mapped) {
        tc_log_error(
            "OpenGLRenderDevice::poll_pixel_rgba8: failed to map request %llu",
            static_cast<unsigned long long>(request_id));
    ...
    if (unmap_ok != GL_TRUE) {
        tc_log_error(
            "OpenGLRenderDevice::poll_pixel_rgba8: PBO data became invalid for request %llu",
            static_cast<unsigned long long>(request_id));
        return false;
    }
    return true;
...
uint64_t OpenGLRenderDevice::request_pixel_depth_float(
    TextureHandle texture, int x, int y
) {
    return request_pixel_readback(texture, x, y, PixelReadbackKind::DepthF32);
}

bool OpenGLRenderDevice::poll_pixel_depth_float(
    uint64_t request_id, float* out_depth
) {
    ...
    if (slot->kind != PixelReadbackKind::DepthF32) {
        tc_log_error(
            "OpenGLRenderDevice::poll_pixel_depth_float: request %llu has the wrong kind",
            static_cast<unsigned long long>(request_id));
        release_pixel_readback_slot(*slot);
        return false;
    }
    ...
    glBindBuffer(GL_PIXEL_PACK_BUFFER, slot->pbo);
    const void* mapped = glMapBufferRange(
        GL_PIXEL_PACK_BUFFER, 0, sizeof(float), GL_MAP_READ_BIT);
    if (!mapped) {
        tc_log_error(
            "OpenGLRenderDevice::poll_pixel_depth_float: failed to map request %llu",
            static_cast<unsigned long long>(request_id));
        glBindBuffer(GL_PIXEL_PACK_BUFFER, static_cast<GLuint>(previous_pbo));
        release_pixel_readback_slot(*slot);
        return false;
    ...
    if (unmap_ok != GL_TRUE) {
        tc_log_error(
            "OpenGLRenderDevice::poll_pixel_depth_float: PBO data became invalid for request %llu",
            static_cast<unsigned long long>(request_id));
        return false;
    }
    return true;
...
uint64_t OpenGLRenderDevice::request_pixel_readback(
    ...
    if (texture->desc.sample_count != 1) {
        tc_log_error(
            "OpenGLRenderDevice::request_pixel_readback: multisampled texture %u is unsupported",
            handle.id);
        return 0;
    }
    ...
    if (kind == PixelReadbackKind::DepthF32 &&
        ...
        tc_log_error(
            "OpenGLRenderDevice::request_pixel_readback: texture %u is not D32F",
            handle.id);
        return 0;
    }
    ...
    if (!slot) {
        tc_log_error(
            "OpenGLRenderDevice::request_pixel_readback: all %zu slots are pending",
    ...
    if (!slot->pbo) {
        ...
        if (!slot->pbo) {
            tc_log_error(
                "OpenGLRenderDevice::request_pixel_readback: PBO creation failed");
            return 0;
        }
        GLint previous_pbo = 0;
        glGetIntegerv(GL_PIXEL_PACK_BUFFER_BINDING, &previous_pbo);
    ...
    if (!pixel_readback_fbo_) {
        ...
        if (!pixel_readback_fbo_) {
            tc_log_error(
                "OpenGLRenderDevice::request_pixel_readback: framebuffer creation failed");
            return 0;
        }
    }
    ...
    if (kind == PixelReadbackKind::Rgba8) {
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, texture->target, 0, 0);
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
            texture->target, texture->gl_id, 0);
        glDrawBuffer(GL_COLOR_ATTACHMENT0);
        glReadBuffer(GL_COLOR_ATTACHMENT0);
    } else {
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, texture->target, 0, 0);
        glFramebufferTexture2D(
    ...
    if (framebuffer_status != GL_FRAMEBUFFER_COMPLETE) {
        tc_log_error(
            "OpenGLRenderDevice::request_pixel_readback: framebuffer incomplete for texture %u: 0x%04x",
            handle.id, static_cast<unsigned>(framebuffer_status));
        glBindFramebuffer(
            GL_READ_FRAMEBUFFER, static_cast<GLuint>(previous_read_fbo));
        glBindFramebuffer(
            GL_DRAW_FRAMEBUFFER, static_cast<GLuint>(previous_draw_fbo));
        return 0;
    }
    ...
    if (error != GL_NO_ERROR || !slot->fence) {
        tc_log_error(
            ...
            handle.id, static_cast<unsigned>(error));
        release_pixel_readback_slot(*slot);
        return 0;
    }
...
OpenGLRenderDevice::PixelReadbackSlot*
OpenGLRenderDevice::acquire_pixel_readback_slot(PixelReadbackKind kind) {
    (void)kind;
    ...
    tc_log_warn(
        "OpenGLRenderDevice::request_pixel_readback: reclaiming unpolled completed request %llu",
...
OpenGLRenderDevice::PixelReadbackSlot*
OpenGLRenderDevice::find_pixel_readback_slot(uint64_t request_id) {
    for (PixelReadbackSlot& slot : pixel_readback_slots_) {
...
bool OpenGLRenderDevice::pixel_readback_ready(
    PixelReadbackSlot& slot, bool log_failure
) {
    if (!slot.active || !slot.fence) return false;
    ...
    if (log_failure) {
        tc_log_error(
            "OpenGLRenderDevice::pixel_readback_ready: fence wait failed for request %llu",
