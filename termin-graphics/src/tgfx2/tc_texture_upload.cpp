...
namespace tgfx {
    namespace {
        ...
        uint16_t float_to_half(float value) {
            ...
            if (remainder > 0x1000u || (remainder == 0x1000u && (rounded_mantissa & 1u))) {
                ...
                if (rounded_mantissa == 0x0400u) {
                    ...
                    return static_cast<uint16_t>(sign | (static_cast<uint32_t>(half_exponent + 1) << 10u));
                }
            }
            return static_cast<uint16_t>(sign | (static_cast<uint32_t>(half_exponent) << 10u) | rounded_mantissa);
        }
        ...
        bool is_f16_format(PixelFormat format) {
            return format == PixelFormat::R16F || format == PixelFormat::RG16F || format == PixelFormat::RGBA16F;
        }

        bool is_f32_format(PixelFormat format) {
            return format == PixelFormat::R32F || format == PixelFormat::RG32F || format == PixelFormat::RGBA32F;
        }

        float read_channel(const uint8_t* pixel, PixelFormat format, uint32_t channel) {
        ...
        void write_channel(uint8_t* pixel, PixelFormat format, uint32_t channel, float value) {
            if (is_u8_format(format)) {
                if (is_srgb_format(format) && channel < 3u) {
                    value = linear_to_srgb(value);
                }
                pixel[channel] = static_cast<uint8_t>(std::lround(std::clamp(value, 0.0f, 1.0f) * 255.0f));
                return;
            }
            if (is_f16_format(format)) {
        ...
        bool downsample_level(PixelFormat format,
            ...
            if (channels == 0 || bytes_per_pixel == 0 ||
                (!is_u8_format(format) && !is_f16_format(format) && !is_f32_format(format))) {
                tc_log_error("build_texture_mip_chain: format '%.*s' does not support CPU mip filtering",
            ...
            destination.resize(static_cast<size_t>(destination_width) * destination_height * bytes_per_pixel);

            for (uint32_t y = 0; y < destination_height; ++y) {
                for (uint32_t x = 0; x < destination_width; ++x) {
                    float sums[4] = {};
                    uint32_t sample_count = 0;
                    const uint32_t source_y_begin = y * source_height / destination_height;
                    const uint32_t source_y_end = (y + 1u) * source_height / destination_height;
                    const uint32_t source_x_begin = x * source_width / destination_width;
                    const uint32_t source_x_end = (x + 1u) * source_width / destination_width;
                    for (uint32_t sy = source_y_begin; sy < source_y_end; ++sy) {
                        for (uint32_t sx = source_x_begin; sx < source_x_end; ++sx) {
                            const uint8_t* source_pixel =
                                source.data() + (static_cast<size_t>(sy) * source_width + sx) * bytes_per_pixel;
                            for (uint32_t channel = 0; channel < channels; ++channel) {
                                sums[channel] += read_channel(source_pixel, format, channel);
                            }
                            ++sample_count;
                        }
                    }
                    uint8_t* destination_pixel =
                        destination.data() + (static_cast<size_t>(y) * destination_width + x) * bytes_per_pixel;
                    for (uint32_t channel = 0; channel < channels; ++channel) {
                        write_channel(
                            destination_pixel, format, channel, sums[channel] / static_cast<float>(sample_count));
                    }
                }
            }
            return true;
        ...
        bool
        normalize_base_level(const tc_texture* texture, PixelFormat& out_format, std::vector<uint8_t>& out_pixels) {
            ...
            if (source_format == TC_TEXTURE_RGB8) {
                out_format = pixel_format_for_tc_texture(TC_TEXTURE_RGBA8, encoding);
    ...
    bool build_texture_mip_chain(PixelFormat format,
                                 uint32_t width,
                                 uint32_t height,
        ...
        if (static_cast<size_t>(width) >
            std::numeric_limits<size_t>::max() / static_cast<size_t>(height) / bytes_per_pixel) {
        ...
        if (base_level.size() != expected_size) {
            tc_log_error(
                "build_texture_mip_chain: base level has %zu bytes, expected %zu", base_level.size(), expected_size);
            return false;
        }
        ...
        while (width > 1 || height > 1) {
            ...
            if (!downsample_level(format,
                                  width,
                                  height,
                                  std::span<const uint8_t>(out_levels.back().data(), out_levels.back().size()),
                                  next)) {
                out_levels.clear();
                return false;
            }
    ...
    bool prepare_tc_texture_upload(const tc_texture* texture, TcTextureUpload& out_upload) {
        out_upload = {};
        if (!texture || !texture->data || texture->width == 0 || texture->height == 0) {
            tc_log_error("prepare_tc_texture_upload: texture '%s' has no valid base level", texture_name(texture));
            return false;
        }

        std::vector<uint8_t> base_level;
        if (!normalize_base_level(texture, out_upload.format, base_level)) {
            tc_log_error("prepare_tc_texture_upload: texture '%s' has unsupported format/encoding %u/%u",
        ...
        if (!build_texture_mip_chain(out_upload.format,
                                     texture->width,
                                     texture->height,
                                     std::span<const uint8_t>(base_level.data(), base_level.size()),
                                     out_upload.levels)) {
            tc_log_error("prepare_tc_texture_upload: failed to build mip chain for texture '%s'",
                         texture_name(texture));
            out_upload = {};
            return false;
        }
