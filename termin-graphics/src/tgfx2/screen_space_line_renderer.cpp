...
namespace tgfx {
    namespace {
        ...
        static_assert(sizeof(CapInstance) == 12 * sizeof(float), "CapInstance layout drift - shader and C++ disagree");
        ...
        void bind_screen_line_shader(RenderContext2& ctx,
            ...
            ctx.bind_uniform_data(SCREEN_LINE_DRAW_RESOURCE, &draw_data, static_cast<uint32_t>(sizeof(draw_data)));
        }
    ...
    void ScreenSpaceLineRenderer::ensure_cap_template(RenderContext2& ctx, int round_segments) {
        round_segments = std::clamp(round_segments, 4, 64);
    ...
    void ScreenSpaceLineRenderer::ensure_round_join_template(RenderContext2& ctx, int round_segments) {
        round_segments = std::clamp(round_segments, 4, 64);
    ...
    void ScreenSpaceLineRenderer::draw_polyline(RenderContext2& ctx,
        ...
        if (!cap_instances.empty()) {
            ...
            bind_screen_line_shader(ctx, cap_shader_handle_, cap_vertex_shader_, cap_fragment_shader_, push);
        ...
        if (!round_join_instances.empty()) {
            ...
            bind_screen_line_shader(
                ctx, round_join_shader_handle_, round_join_vertex_shader_, round_join_fragment_shader_, push);
        ...
        if (!join_instances.empty()) {
            ...
            bind_screen_line_shader(ctx, join_shader_handle_, join_vertex_shader_, join_fragment_shader_, push);
