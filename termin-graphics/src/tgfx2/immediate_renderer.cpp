...
namespace termin {
    ...
    namespace {
        ...
        static_assert(sizeof(ImmediatePushData) == 64, "ImmediatePushData must be 64 bytes (mat4)");
    ...
    void ImmediateRenderer::triangles(const float* vertices,
                                      size_t vertex_count,
    ...
    void ImmediateRenderer::torus_solid(const TorusSolidSpec& spec, const SrgbColor& color, bool depth_test) {
        ...
        for (int i = 0; i < spec.major_segments; ++i) {
            ...
            Vec3 ring_center =
                spec.center + (tangent * std::cos(theta) + bitangent * std::sin(theta)) * spec.major_radius;
    ...
    void ImmediateRenderer::_flush_buffers(tgfx::RenderContext2* ctx2,
        ...
        if (blend) {
            ctx2->set_blend_func(tgfx::BlendFactor::SrcAlpha, tgfx::BlendFactor::OneMinusSrcAlpha);
        }
    ...
    void ImmediateRenderer::flush(
        tgfx::RenderContext2* ctx2, const Mat44& view_matrix, const Mat44& proj_matrix, bool depth_test, bool blend) {
        if (line_vertices.empty() && tri_vertices.empty())
            return;

        bool profile = tc_profiler_enabled();
        if (profile)
            tc_profiler_begin_section("Immediate:Flush");
        _flush_buffers(ctx2, line_vertices, tri_vertices, view_matrix, proj_matrix, depth_test, blend);
        if (profile)
            tc_profiler_end_section();
    }

    void ImmediateRenderer::flush_depth(tgfx::RenderContext2* ctx2,
                                        const Mat44& view_matrix,
                                        const Mat44& proj_matrix,
                                        bool blend) {
        if (line_vertices_depth.empty() && tri_vertices_depth.empty())
            return;

        bool profile = tc_profiler_enabled();
        if (profile)
            tc_profiler_begin_section("Immediate:FlushDepth");
