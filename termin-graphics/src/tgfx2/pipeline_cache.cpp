...
namespace tgfx {
    ...
    static bool layout_ranges_equal(std::span<const VertexLayoutDesc> a, std::span<const VertexLayoutDesc> b) {
        if (a.size() != b.size()) {
            return false;
        }
    ...
    static bool pipeline_cache_key_state_equal(const PipelineCacheKeyState& a, const PipelineCacheKeyState& b) {
        ...
        return a.vertex_shader == b.vertex_shader && a.fragment_shader == b.fragment_shader &&
               ...
               a.depth_stencil.depth_compare == b.depth_stencil.depth_compare && a.blend.enabled == b.blend.enabled &&
               a.blend.src_color == b.blend.src_color && a.blend.dst_color == b.blend.dst_color &&
               a.blend.color_op == b.blend.color_op && a.blend.src_alpha == b.blend.src_alpha &&
    ...
    bool PipelineCacheKeyEqual::operator()(const PipelineCacheKey& a, const PipelineCacheKey& b) const {
        return pipeline_cache_key_state_equal(a, b) && layout_ranges_equal(a.vertex_layouts, b.vertex_layouts);
    }

    bool PipelineCacheKeyEqual::operator()(const PipelineCacheKey& a, const PipelineCacheLookupKey& b) const {
        return pipeline_cache_key_state_equal(a, b) && layout_ranges_equal(a.vertex_layouts, b.vertex_layouts);
    }

    bool PipelineCacheKeyEqual::operator()(const PipelineCacheLookupKey& a, const PipelineCacheKey& b) const {
        return (*this)(b, a);
    }
    ...
    PipelineHandle PipelineCache::get(const PipelineCacheLookupKey& key) {
        ...
        for (uint32_t i = 0; i < key.color_format_count; ++i) {
            if (key.color_formats[i] == PixelFormat::Undefined) {
                tc_log(TC_LOG_ERROR, "PipelineCache: color format %u is undefined; backend call skipped", i);
                return {};
            }
        }
        ...
        ++miss_count_;
        const bool new_vertex_layout_signature = observed_vertex_layout_hashes_.insert(key.vertex_layouts_hash).second;
#ifdef TGFX2_HAS_VULKAN
        ...
        desc.color_formats.assign(key.color_formats.begin(), key.color_formats.begin() + key.color_format_count);
    ...
    PipelineCacheStats PipelineCache::stats() const {
        ...
        out.unique_vertex_layout_signature_count = static_cast<uint64_t>(observed_vertex_layout_hashes_.size());
        out.vertex_layout_signature_hashes.assign(observed_vertex_layout_hashes_.begin(),
                                                  observed_vertex_layout_hashes_.end());
        std::sort(out.vertex_layout_signature_hashes.begin(), out.vertex_layout_signature_hashes.end());
        return out;
    }
