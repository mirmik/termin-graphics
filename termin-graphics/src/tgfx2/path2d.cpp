...
namespace tgfx {
    ...
    namespace {
        ...
        void flatten_quad(termin::Vec2f a,
                          termin::Vec2f b,
                          termin::Vec2f c,
                          float tolerance,
                          unsigned depth,
        ...
        void flatten_cubic(termin::Vec2f a,
                           termin::Vec2f b,
                           termin::Vec2f c,
                           termin::Vec2f d,
                           float tolerance,
                           unsigned depth,
                           std::vector<termin::Vec2f>& out) {
            if (depth == 16 && std::max(distance_to_line(b, a, d), distance_to_line(c, a, d)) > tolerance) {
    ...
    bool Path2f::quadratic_to(termin::Vec2f control, termin::Vec2f end) {
        if (!finite(control) || !finite(end) || verbs_.empty() || verbs_.back() == Path2Verb::Close) {
    ...
    bool Path2f::cubic_to(termin::Vec2f c1, termin::Vec2f c2, termin::Vec2f end) {
        if (!finite(c1) || !finite(c2) || !finite(end) || verbs_.empty() || verbs_.back() == Path2Verb::Close) {
    ...
    bool Path2f::try_assign(std::span<const Path2Verb> verbs, std::span<const termin::Vec2f> points) {
        Path2f candidate;
        std::size_t point_index = 0;
        for (Path2Verb verb : verbs) {
            const unsigned count = point_count(verb);
            if (count == std::numeric_limits<unsigned>::max() || point_index + count > points.size()) {
            ...
            switch (verb) {
            case Path2Verb::MoveTo:
                ok = candidate.move_to(points[point_index]);
                break;
            case Path2Verb::LineTo:
                ok = candidate.line_to(points[point_index]);
                break;
            case Path2Verb::QuadraticTo:
                ok = candidate.quadratic_to(points[point_index], points[point_index + 1]);
                break;
            case Path2Verb::CubicTo:
                ok = candidate.cubic_to(points[point_index], points[point_index + 1], points[point_index + 2]);
                break;
            case Path2Verb::Close:
                ok = candidate.close();
                break;
    ...
    FlattenedPath2f Path2f::flatten(float tolerance, const termin::Affine2f& transform) const {
        ...
        for (Path2Verb verb : verbs_) {
            if (verb == Path2Verb::MoveTo) {
                ...
                current = end;
                pi += 2;
                result.contours.back().points.insert(
                    result.contours.back().points.end(), transformed.begin(), transformed.end());
    ...
    termin::Bounds2f Path2f::stroke_bounds(const StrokePaint& stroke, const termin::Affine2f& transform) const {
    ...
    bool FlattenedPath2f::stroke_contains(termin::Vec2f p, const StrokePaint& stroke) const noexcept {
