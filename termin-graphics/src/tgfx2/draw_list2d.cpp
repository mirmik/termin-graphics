...
bool valid_sampling(DrawTextureSampling2D sampling) {
    return sampling == DrawTextureSampling2D::Linear ||
           sampling == DrawTextureSampling2D::Nearest;
}

bool valid_anchor(TextAnchor2D anchor) {
    return anchor == TextAnchor2D::Left ||
           anchor == TextAnchor2D::Center ||
           anchor == TextAnchor2D::Right;
}
...
bool DrawList2DBuilder::pop_scope_(Scope expected, const char* operation) {
    if (scopes_.empty() || scopes_.back() != expected) {
        tc::Log::error("[DrawList2DBuilder] %s has no matching active scope",
                       operation);
        return false;
    }
    scopes_.pop_back();
...
bool DrawList2DBuilder::pop_transform() {
    if (!pop_scope_(Scope::Transform, "pop_transform")) return false;
...
bool DrawList2DBuilder::pop_opacity() {
    if (!pop_scope_(Scope::Opacity, "pop_opacity")) return false;
...
bool DrawList2DBuilder::pop_clip() {
    if (!pop_scope_(Scope::Clip, "pop_clip")) return false;
...
bool DrawList2DBuilder::rounded_rect(
    termin::Rect2f rect,
    float radius,
    FillPaint paint,
    ...
    commands_.emplace_back(
        DrawRoundedRect2D{rect, radius, paint, std::move(stroke)});
    return true;
}

bool DrawList2DBuilder::ellipse(
...
bool DrawList2DBuilder::path(
    Path2f path,
    std::optional<FillPaint> fill,
    std::optional<StrokePaint> stroke) {
    if (path.empty() || (!fill && !stroke) ||
        (fill && !fill->validate()) || (stroke && !stroke->validate())) {
        tc::Log::error("[DrawList2DBuilder] invalid path draw rejected");
        return false;
    }
    commands_.emplace_back(
        DrawPath2D{std::move(path), std::move(fill), std::move(stroke)});
    return true;
}

bool DrawList2DBuilder::polyline(
    ...
    commands_.emplace_back(DrawPolyline2D{
        std::vector<termin::Vec2f>(points.begin(), points.end()),
        std::move(stroke),
        closed});
    return true;
}

bool DrawList2DBuilder::text(
    ...
    FontHandle font,
    TextAnchor2D anchor) {
    if (text_value.empty() || !finite(origin) || !finite(size_px) ||
        size_px <= 0.0f || !color.is_finite() || !font ||
        !valid_anchor(anchor)) {
        tc::Log::error("[DrawList2DBuilder] invalid text draw rejected");
        return false;
    }
    commands_.emplace_back(DrawText2D{
        std::move(text_value), origin, size_px, color, font, anchor});
    return true;
}
...
bool DrawList2DBuilder::image(
    ...
    if (!texture || !valid_rect(rect) ||
        rect.width <= 0.0f || rect.height <= 0.0f ||
        !valid_rect(uv) || !tint.is_finite() || !valid_sampling(sampling)) {
...
bool DrawList2DBuilder::custom_batch(
    std::span<const DrawVertex2D> vertices,
    Color4f color,
    TextureHandle texture,
    DrawTextureSampling2D sampling) {
    if (vertices.empty() || vertices.size() % 3 != 0 ||
        !color.is_finite() || !valid_sampling(sampling)) {
    ...
    commands_.emplace_back(DrawCustomBatch2D{
        std::vector<DrawVertex2D>(vertices.begin(), vertices.end()),
        color,
        texture,
        sampling});
    return true;
