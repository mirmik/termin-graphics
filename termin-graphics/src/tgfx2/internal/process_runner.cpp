...
#ifndef _WIN32
std::string shell_quote_arg(const std::string &text) {
    std::string out = "'";
    for (char ch : text) {
        if (ch == '\'') {
            out += "'\\''";
...
#else
std::wstring widen_windows_arg(const std::string &text) {
    if (text.empty()) {
        return {};
    }

    int len = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, text.c_str(), -1, nullptr, 0);
...
std::string narrow_windows_text(const wchar_t *text) {
    if (!text || text[0] == L'\0') {
        return {};
    }
...
bool windows_arg_ends_with_case_insensitive(const std::wstring &arg, const wchar_t *suffix) {
...
ProcessResult run_process(const std::vector<std::string> &args, bool capture_output) {
    ...
    } else if (windows_arg_ends_with_case_insensitive(wide_args[0], L".cmd") ||
    ...
    BOOL ok = CreateProcessW(application_name.empty() ? nullptr : application_name.c_str(),
                             command_line.data(), nullptr, nullptr, TRUE, 0, nullptr, nullptr,
