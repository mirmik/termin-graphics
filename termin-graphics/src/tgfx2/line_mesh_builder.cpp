...
namespace tgfx {
    namespace {
        ...
        void append_round_disk(
            LineMesh& mesh, LinePoint3 center, LinePoint3 axis_a, LinePoint3 axis_b, float radius, int segments) {
        ...
        void append_round_cap(
            LineMesh& mesh, LinePoint3 center, LinePoint3 side, LinePoint3 outward, float radius, int segments) {
    ...
    LineMesh build_line_mesh(std::span<const LinePoint3> points, const LineStyle& style) {
        LineMesh mesh;
        ...
        for (size_t i = 0; i < segments.size(); ++i) {
            ...
            append_quad(mesh, seg.p0 + offset, seg.p0 - offset, seg.p1 - offset, seg.p1 + offset);
        }

        if (!style.closed && style.cap == LineCapStyle::Round) {
            const Segment& first = segments.front();
            append_round_cap(mesh, first.p0, first.side, -first.dir, half_width, style.round_segments);

            const Segment& last = segments.back();
            append_round_cap(mesh, last.p1, -last.side, last.dir, half_width, style.round_segments);
        }
        ...
        if (style.join == LineJoinStyle::Round) {
            for (size_t i = 1; i < clean_points.size() - 1; ++i) {
                ...
                append_round_disk(mesh, clean_points[i], seg.side, seg.dir, half_width, style.round_segments);
            }
        }
