...
namespace tgfx {
    ...
    RenderContext2::RenderContext2(IRenderDevice& device, PipelineCache& cache, GraphicsHost* graphics_host)
        : device_(device),
          cache_(cache),
    ...
    RenderContext2::~RenderContext2() {
        ...
        if (fsq_vbo_)
            device_.destroy(fsq_vbo_);
        if (fsq_ibo_)
            device_.destroy(fsq_ibo_);
        if (fsq_vs_)
            device_.destroy(fsq_vs_);
    }
    ...
    void RenderContext2::end_frame() {
        ...
        if (profile)
            tc_profiler_begin_section("RenderContext2::destroy_transients");
        if (current_resource_set_) {
            device_.destroy(current_resource_set_);
        ...
        for (auto h : deferred_destroy_textures_)
            device_.destroy(h);
        for (auto h : deferred_destroy_buffers_)
            device_.destroy(h);
        for (auto h : deferred_destroy_resource_sets_)
            device_.destroy(h);
    ...
    bool RenderContext2::begin_pass(const RenderPassDesc& pass) {
        return begin_pass_impl(pass, 1, false, MultiviewColorFinalState::ShaderRead);
    }
    ...
    bool RenderContext2::begin_multiview_pass(const MultiviewRenderPassDesc& pass) {
        ...
        if (!caps.supports_multiview) {
            tc_log(TC_LOG_ERROR, "RenderContext2::begin_multiview_pass: backend does not support multiview");
            return false;
        }
        if (pass.view_count < 2 || pass.view_count > caps.max_multiview_views || pass.view_count >= 32) {
            tc_log(TC_LOG_ERROR,
                   "RenderContext2::begin_multiview_pass: invalid view_count=%u (backend max=%u)",
                   pass.view_count,
                   caps.max_multiview_views);
            return false;
        }
        RenderPassDesc base;
        ...
        return begin_pass_impl(base, pass.view_count, true, pass.color_final_state);
    }

    bool RenderContext2::begin_pass_impl(const RenderPassDesc& pass,
                                         uint32_t view_count,
                                         bool multiview,
        ...
        const BackendCapabilities caps = device_.capabilities();
        if (pass.colors.size() > TGFX2_MAX_COLOR_ATTACHMENTS || pass.colors.size() > caps.max_color_attachments) {
            tc_log(TC_LOG_ERROR,
        ...
        if (pass.colors.empty() && (!pass.has_depth || !pass.depth.texture)) {
            tc_log(TC_LOG_ERROR, "RenderContext2::begin_pass: pass has no color or depth attachment");
            return false;
        }
        ...
        for (size_t i = 0; i < pass.colors.size(); ++i) {
            ...
            if (!color.texture) {
                tc_log(TC_LOG_ERROR, "RenderContext2::begin_pass: color attachment %zu has a null texture", i);
                return false;
            }
            ...
            if ((!multiview && desc.array_layers != 1) || (multiview && desc.array_layers < view_count)) {
                tc_log(TC_LOG_ERROR,
            ...
            if (!has_flag(desc.usage, TextureUsage::ColorAttachment)) {
                tc_log(TC_LOG_ERROR, "RenderContext2::begin_pass: color attachment %zu lacks ColorAttachment usage", i);
                return false;
            }
        ...
        if (pass.has_depth) {
            if (!pass.depth.texture) {
                tc_log(TC_LOG_ERROR, "RenderContext2::begin_pass: depth attachment is enabled but has a null texture");
                return false;
            }
            for (size_t i = 0; i < pass.colors.size(); ++i) {
            ...
            if (!has_flag(desc.usage, TextureUsage::DepthStencilAttachment)) {
                tc_log(TC_LOG_ERROR, "RenderContext2::begin_pass: depth attachment lacks DepthStencilAttachment usage");
                return false;
            }
            ...
            tc_log(TC_LOG_ERROR, "RenderContext2::begin_pass: depth texture is set while has_depth is false");
            return false;
        }
    ...
    void RenderContext2::framebuffer_local_barrier() {
        if (!in_pass_ || !cmd_) {
            tc_log(TC_LOG_ERROR, "RenderContext2::framebuffer_local_barrier requires an active render pass");
            return;
        }
    ...
    void RenderContext2::set_blend_func(BlendFactor src, BlendFactor dst) {
        if (blend_.src_color != src || blend_.dst_color != dst || blend_.src_alpha != src || blend_.dst_alpha != dst) {
    ...
    void RenderContext2::set_depth_bias(bool enabled, float constant, float slope, float clamp) {
        if (raster_.depth_bias_enabled != enabled || raster_.depth_bias_constant != constant ||
            raster_.depth_bias_slope != slope || raster_.depth_bias_clamp != clamp) {
    ...
    void RenderContext2::set_color_mask(bool r, bool g, bool b, bool a) {
        if (color_mask_.r != r || color_mask_.g != g || color_mask_.b != b || color_mask_.a != a) {
            color_mask_ = {r, g, b, a};
            pipeline_dirty_ = true;
        }
    }
    ...
    void RenderContext2::set_vertex_layouts(const std::vector<VertexBufferLayout>& layouts) {
        ...
        set_vertex_layouts(descs.data(), static_cast<uint32_t>(descs.size()));
    }
    ...
    void RenderContext2::set_vertex_layouts(const VertexLayoutDesc* layouts, uint32_t count) {
        vertex_layouts_.clear();
        vertex_layouts_.reserve(count);
        for (uint32_t i = 0; i < count; ++i) {
            vertex_layouts_.push_back(make_vertex_layout_desc(layouts[i]));
        }
        vertex_layouts_hash_ =
            hash_vertex_layout_descs(vertex_layouts_.data(), static_cast<uint32_t>(vertex_layouts_.size()));
        pipeline_dirty_ = true;
    }
    ...
    RenderContext2::ResourceScope RenderContext2::scope_from_shader_resource(uint32_t shader_scope) {
        switch (shader_scope) {
        case TC_SHADER_RESOURCE_SCOPE_FRAME:
            return ResourceScope::Frame;
        case TC_SHADER_RESOURCE_SCOPE_PASS:
            return ResourceScope::Pass;
        case TC_SHADER_RESOURCE_SCOPE_MATERIAL:
            return ResourceScope::Material;
        case TC_SHADER_RESOURCE_SCOPE_DRAW:
            return ResourceScope::Draw;
        case TC_SHADER_RESOURCE_SCOPE_TRANSIENT:
            return ResourceScope::Transient;
    ...
    ShaderResourceScope RenderContext2::shader_scope_from_resource_scope(ResourceScope scope) {
        switch (scope) {
        case ResourceScope::Frame:
            return ShaderResourceScope::Frame;
        case ResourceScope::Pass:
            return ShaderResourceScope::Pass;
        case ResourceScope::Material:
            return ShaderResourceScope::Material;
        case ResourceScope::Draw:
            return ShaderResourceScope::Draw;
        case ResourceScope::Transient:
    ...
    bool same_backend_placement(const BackendPlacement& a, const BackendPlacement& b) {
        return a.kind == b.kind && a.vulkan.set == b.vulkan.set && a.vulkan.binding == b.vulkan.binding &&
               a.vulkan.descriptor_kind == b.vulkan.descriptor_kind &&
               a.d3d11.register_class == b.d3d11.register_class && a.d3d11.register_index == b.d3d11.register_index &&
               a.opengl.binding_class == b.opengl.binding_class && a.opengl.binding_point == b.opengl.binding_point &&
               a.opengl.texture_unit == b.opengl.texture_unit;
    }

    bool same_backend_bound_resource_slot(const BackendBoundResourceSlot& a, const BackendBoundResourceSlot& b) {
        return a.kind == b.kind && a.scope == b.scope && a.stage_mask == b.stage_mask &&
               a.array_count == b.array_count && a.size == b.size && same_backend_placement(a.placement, b.placement) &&
               a.debug_name == b.debug_name;
    }

    bool same_bound_resource_value(const BoundResourceValue& a, const BoundResourceValue& b) {
        return a.kind == b.kind && a.buffer == b.buffer && a.texture == b.texture && a.sampler == b.sampler &&
               a.offset == b.offset && a.range == b.range && a.array_element == b.array_element;
    }

    BoundResourceBinding* RenderContext2::find_planned_binding(std::vector<BoundResourceBinding>& bindings,
                                                               const BackendBoundResourceSlot& slot,
                                                               const BoundResourceValue& value) {
        for (BoundResourceBinding& binding : bindings) {
            if (same_backend_placement(binding.slot.placement, slot.placement) && binding.slot.kind == slot.kind &&
    ...
    void RenderContext2::upsert_pending_planned_binding(ResourceScope scope,
                                                        const BackendBoundResourceSlot& slot,
                                                        const BoundResourceValue& value) {
        auto& bucket = pending_binding_buckets_[static_cast<size_t>(scope)];
        BoundResourceBinding* existing = find_planned_binding(bucket.planned, slot, value);
        if (existing) {
    ...
    RenderContext2::build_pending_bound_resource_set(uintptr_t resource_layout_token,
        ...
        for (size_t i = 0; i < pending_binding_buckets_.size(); ++i) {
            ...
            if (group_count >= group_views.size()) {
                tc_log(
                    TC_LOG_ERROR, "RenderContext2: resource-group view capacity %zu is too small", group_views.size());
                break;
            }
    ...
    const tc_shader_resource_binding* RenderContext2::active_resource_binding_by_name(std::string_view name,
        ...
        const tc_shader_resource_binding* bindings = tc_shader_resource_bindings(active_shader_layout_);
    ...
    const BackendBindingPlanEntry* RenderContext2::active_backend_binding_for(const tc_shader_resource_binding* rb,
                                                                              const char* action) const {
        if (!rb) {
        ...
        if (!active_shader_layout_) {
            tc_log(TC_LOG_WARN, "RenderContext2: %s('%s') called without active shader layout", action, rb->name);
            return nullptr;
        }
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            if (entry.resource.name != rb->name || static_cast<uint32_t>(entry.resource.kind) != rb->kind ||
    ...
    void RenderContext2::use_shader_resource_layout(const struct ::tc_shader* shader) {
        ...
        active_backend_binding_plan_valid_ = build_backend_binding_plan(device_.backend_type(),
    ...
    void RenderContext2::bind_uniform(std::string_view name, BufferHandle buffer, uint64_t offset, uint64_t range) {
    ...
    void RenderContext2::bind_uniform(const tc_shader_resource_binding* rb,
        ...
        const BackendBindingPlanEntry* plan_entry = active_backend_binding_for(rb, "bind_uniform");
        if (!plan_entry)
            return;
    ...
    void
    RenderContext2::bind_storage_buffer(std::string_view name, BufferHandle buffer, uint64_t offset, uint64_t range) {
        bind_storage_buffer(active_resource_binding_by_name(name, "bind_storage_buffer"), buffer, offset, range);
    }
    ...
    void RenderContext2::bind_storage_buffer(const tc_shader_resource_binding* rb,
        ...
        const BackendBindingPlanEntry* plan_entry = active_backend_binding_for(rb, "bind_storage_buffer");
        if (!plan_entry)
            return;
    ...
    void RenderContext2::bind_texture(std::string_view name, TextureHandle texture, SamplerHandle sampler) {
        bind_texture_array_element(name, 0, texture, sampler);
    }

    void
    RenderContext2::bind_texture(const tc_shader_resource_binding* rb, TextureHandle texture, SamplerHandle sampler) {
    ...
    void RenderContext2::bind_texture_array_element(std::string_view name,
        ...
        bind_texture_array_element(
            active_resource_binding_by_name(name, "bind_texture"), array_element, texture, sampler);
    }
    ...
    void RenderContext2::bind_texture_array_element(const tc_shader_resource_binding* rb,
        ...
        const BackendBindingPlanEntry* plan_entry = active_backend_binding_for(rb, "bind_texture");
        if (!plan_entry)
            return;
    ...
    void RenderContext2::bind_uniform_data(const tc_shader_resource_binding* rb, const void* data, uint32_t size) {
        if (!data || size == 0 || !rb)
            return;
        if (rb->kind != TC_SHADER_RESOURCE_CONSTANT_BUFFER) {
            tc_log(TC_LOG_WARN,
        ...
        const BackendBindingPlanEntry* plan_entry = active_backend_binding_for(rb, "bind_uniform_data");
        if (!plan_entry)
            return;
        ...
        if (!ring_write_succeeded) {
            ...
            if (device_.transient_uniform_write(data, size, buffer, transient_offset)) {
    ...
    void RenderContext2::defer_destroy(TextureHandle handle) {
        if (handle)
            deferred_destroy_textures_.push_back(handle);
    }

    void RenderContext2::defer_destroy(BufferHandle handle) {
        if (handle)
            deferred_destroy_buffers_.push_back(handle);
    }
    ...
    bool RenderContext2::flush_pipeline() {
        if (!pipeline_dirty_) {
            ...
            if (push_constants_dirty_ && pending_push_constants_size_ != 0) {
                cmd_->set_push_constants(pending_push_constants_.data(), pending_push_constants_size_);
                push_constants_dirty_ = false;
            }
            return true;
        }
        ...
        if (!pipeline) {
            tc_log(TC_LOG_ERROR, "RenderContext2: pipeline creation failed; skipping draw until the backend recovers");
            return false;
        }
        ...
        if (pipeline_changed) {
            ...
            const uint64_t resource_layout_token = device_.pipeline_resource_layout_token(pipeline);
    ...
    void RenderContext2::flush_resource_set() {
        ...
        if (last_bound_pipeline_) {
            resource_layout_token = device_.pipeline_resource_layout_token(last_bound_pipeline_);
        }

        std::array<BoundResourceGroupView, static_cast<size_t>(ResourceScope::Count)> group_views{};
        const BoundResourceSetDesc bound_desc = build_pending_bound_resource_set(resource_layout_token, group_views);
    ...
    void RenderContext2::draw(BufferHandle vbo, BufferHandle ibo, uint32_t index_count, IndexType idx_type) {
        ...
        if (ibo != last_bound_ibo_ || last_bound_ibo_offset_ != 0 || idx_type != last_bound_index_type_) {
    ...
    void RenderContext2::draw(BufferHandle vbo,
                              BufferHandle ibo,
        ...
        if (ibo != last_bound_ibo_ || last_bound_ibo_offset_ != index_offset || idx_type != last_bound_index_type_) {
    ...
    void RenderContext2::draw_indexed_instanced(const IndexedInstancedDraw& draw) {
        ...
        if (draw.vertex_buffer != last_bound_vbos_[0] || draw.vertex_offset != last_bound_vbo_offsets_[0]) {
        ...
        if (draw.instance_buffer &&
            (draw.instance_buffer != last_bound_vbos_[1] || draw.instance_offset != last_bound_vbo_offsets_[1])) {
        ...
        if (draw.index_buffer != last_bound_ibo_ || draw.index_offset != last_bound_ibo_offset_ ||
    ...
    void RenderContext2::draw_arrays_instanced(BufferHandle vertex_vbo,
        ...
        flush_resource_set();
        ensure_cached_vertex_buffer_slots(2);
        if (vertex_vbo != last_bound_vbos_[0] || vertex_offset != last_bound_vbo_offsets_[0]) {
            cmd_->bind_vertex_buffer(0, vertex_vbo, vertex_offset);
            last_bound_vbos_[0] = vertex_vbo;
            last_bound_vbo_offsets_[0] = vertex_offset;
        }
        if (instance_vbo != last_bound_vbos_[1] || instance_offset != last_bound_vbo_offsets_[1]) {
    ...
    void RenderContext2::draw_transient_arrays(const void* data,
        ...
        draw_transient_arrays(data, byte_size, vertex_count, make_vertex_layout_desc(layout), topology);
    }
    ...
    void RenderContext2::draw_transient_arrays(const void* data,
                                               ...
                                               PrimitiveTopology topology) {
        if (!data || byte_size == 0 || vertex_count == 0)
            return;
        ...
        if (profile)
            tc_profiler_begin_section("transient.upload");
        ...
        if (ring_offset != UINT64_MAX) {
            ...
            device_.upload_buffer(buf, std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(data), byte_size));
        }
        if (profile)
            tc_profiler_end_section();

        if (profile)
            tc_profiler_begin_section("transient.draw");
    ...
    void RenderContext2::draw_immediate_generic(const float* data, uint32_t vertex_count, PrimitiveTopology topo) {
        ...
        draw_transient_arrays(
            data, vertex_count * 7 * static_cast<uint32_t>(sizeof(float)), vertex_count, layout, topo);
    }
