...
namespace tgfx_bindings {
    ...
    void bind_texture(nb::module_& m) {
        ...
        nb::class_<TcTexture>(m, "TcTexture")
            ...
            .def_prop_ro("data",
                         [](const TcTexture& self) -> nb::object {
                             ...
                             size_t shape[3] = {static_cast<size_t>(self.height()),
                             ...
                             auto arr = nb::ndarray<nb::numpy, uint8_t, nb::shape<-1, -1, -1>>(buf, 3, shape, owner);
                             return nb::cast(arr);
                         })
            ...
            .def("get_upload_data",
                 [](const TcTexture& self) {
                     ...
                     size_t shape[3] = {
                         static_cast<size_t>(h), static_cast<size_t>(w), static_cast<size_t>(self.channels())};
        ...
        m.def(
            ...
            [](TcTexture& handle) { return tc_texture_is_loaded(handle.handle); },
            nb::arg("handle"),
            "Check if texture data is loaded");

        m.def(
            "tc_texture_ensure_loaded",
            [](TcTexture& handle) { return tc_texture_ensure_loaded(handle.handle); },
            nb::arg("handle"),
            "Ensure texture is loaded (triggers callback if needed)");
    }
