...
namespace tgfx_bindings {
    ...
    namespace {
        ...
        nb::dict shader_resource_requirement_to_dict(const tc_shader_resource_requirement& requirement) {
            nb::dict result;
        ...
        tc_render_state shader_program_render_state_from_dict(const nb::dict& values) {
            ...
            if (values.contains("cull"))
                result.cull = nb::cast<bool>(values["cull"]);
            if (values.contains("depth_test"))
                result.depth_test = nb::cast<bool>(values["depth_test"]);
            if (values.contains("depth_write"))
                result.depth_write = nb::cast<bool>(values["depth_write"]);
            if (values.contains("blend"))
                result.blend = nb::cast<bool>(values["blend"]);
        ...
        [[noreturn]] void throw_property_default_error(const std::string& name,
                                                       const std::string& property_type,
                                                       const std::string& detail) {
            throw std::invalid_argument("TcShaderProgram property '" + name + "' (" + property_type + ") " + detail);
        }

        void set_shader_program_property_default(const std::string& name,
                                                 const std::string& property_type,
                                                 nb::handle object,
            ...
            if (size != expected_size) {
                throw_property_default_error(name,
                                             property_type,
                                             "default requires exactly " + std::to_string(expected_size) +
                                                 " components; got " + std::to_string(size));
            }
            ...
            case TC_UNIFORM_VEC2:
                destination = default_value.data.v2;
                break;
            case TC_UNIFORM_VEC3:
                destination = default_value.data.v3;
                break;
            case TC_UNIFORM_VEC4:
                destination = default_value.data.v4;
                break;
            case TC_UNIFORM_MAT4:
                destination = default_value.data.m4;
                break;
            default:
                break;
            }
    ...
    void bind_shader(nb::module_& m) {
        ...
        nb::class_<tc_shader_handle>(m, "TcShaderHandle")
            ...
            .def("is_invalid", [](const tc_shader_handle& h) { return tc_shader_handle_is_invalid(h); })
            .def("__repr__", [](const tc_shader_handle& h) {
                return "<TcShaderHandle index=" + std::to_string(h.index) + " gen=" + std::to_string(h.generation) +
                       ">";
            });
        ...
        nb::enum_<tc_shader_variant_op>(m, "ShaderVariantOp")
            ...
            .value("LINE_TUBE", TC_SHADER_VARIANT_LINE_TUBE);
        ...
        nb::class_<TcShader>(m, "TcShader")
            ...
            .def_prop_ro("surface_producer",
                         [](const TcShader& s) -> nb::object {
                             ...
                             for (uint32_t i = 0; i < view.fragment_input_count; ++i) {
                                 nb::dict input;
                                 input["semantic"] = std::string(view.fragment_inputs[i].semantic);
            ...
            .def_prop_ro("has_contract", [](const TcShader& s) { return tc_shader_has_contract(s.get()); })
            .def_prop_ro("contract",
                         [](const TcShader& s) -> nb::object {
                             ...
                             result["debug_name"] = view.debug_name ? std::string(view.debug_name) : std::string();
                             result["source_debug_name"] =
                                 view.source_debug_name ? std::string(view.source_debug_name) : std::string();
            ...
            .def_prop_ro("_raw_ptr", [](const TcShader& s) -> const ::tc_shader* { return s.get(); })
            ...
            .def(
                ...
                [](const TcShader& self, const std::string& name) -> nb::object {
                    const tc_shader_resource_binding* binding = self.find_resource_binding(name.c_str());
                    if (!binding)
                        return nb::none();
            ...
            .def(
                ...
                [](TcShader& self,
                   const std::vector<std::tuple<std::string,
                    ...
                    tc_shader_set_resource_layout(shader, bindings.data(), static_cast<uint32_t>(bindings.size()));
                },
                nb::arg("entries"),
                ...
                [](const std::string& vertex,
                   ...
                   const std::string& geometry_entry) {
                    if (!language.has_value()) {
                        throw std::invalid_argument("TcShader.from_sources requires an explicit shader language");
                    }
            ...
            .def_static(
                "from_builtin_catalog",
                [](const std::string& uuid) {
                    const tc_shader_handle h = tgfx::register_builtin_shader_from_catalog(uuid.c_str());
            ...
            .def_static("get_or_create",
                        &TcShader::get_or_create,
                        nb::arg("uuid"),
            ...
            .def(
                ...
                [](TcShader& self,
                   ...
                   uint32_t block_size) {
                    ...
                    tc_shader_set_material_ubo_layout(s, buf.data(), static_cast<uint32_t>(buf.size()), block_size);
                },
                nb::arg("entries"),
                nb::arg("block_size"),
            ...
            .def("__repr__", [](const TcShader& s) {
                if (!s.is_valid())
                    return std::string("<TcShader invalid>");
        ...
        nb::class_<TcShaderProgram>(m, "TcShaderProgram")
                             ...
                             for (uint32_t i = 0; i < program->property_count; ++i) {
                                 ...
                                 if (property.has_expected_encoding) {
                                     item["expected_encoding"] = std::string(
                                         property.expected_encoding == TC_TEXTURE_ENCODING_SRGB ? "srgb" : "linear");
                                 } else {
                                 ...
                                 if (property.has_default) {
                                     if (property.default_text[0] != '\0') {
                                         ...
                                         case TC_UNIFORM_FLOAT:
                                             ...
                                             break;
                                         case TC_UNIFORM_VEC2:
                                             item["default"] = nb::make_tuple(property.default_value.data.v2[0],
                                                                              property.default_value.data.v2[1]);
                                             break;
                                         case TC_UNIFORM_VEC3:
                                             item["default"] = nb::make_tuple(property.default_value.data.v3[0],
                                                                              property.default_value.data.v3[1],
                                                                              property.default_value.data.v3[2]);
                                             break;
                                         case TC_UNIFORM_VEC4:
                                             item["default"] = nb::make_tuple(property.default_value.data.v4[0],
                                 ...
                                 item["range_min"] = property.has_range_min ? nb::cast(property.range_min) : nb::none();
                                 item["range_max"] = property.has_range_max ? nb::cast(property.range_max) : nb::none();
                                 result.append(item);
                             }
                             return result;
                         })
            ...
            .def(
                ...
                [](TcShaderProgram& value,
                   ...
                   nb::list phase_values) {
                    ...
                    if (!program)
                        throw std::runtime_error("TcShaderProgram is stale");
                    ...
                    for (size_t i = 0; i < property_count; ++i) {
                        ...
                        property_labels.push_back(item.contains("label") ? nb::cast<std::string>(item["label"])
                                                                         : std::string());
                        ...
                        if (item.contains("default") && !item["default"].is_none()) {
                            ...
                            set_shader_program_property_default(
                                property_names.back(), property_types.back(), object, default_value, default_texts[i]);
                        }
                        ...
                        if (item.contains("expected_encoding") && !item["expected_encoding"].is_none()) {
                            const std::string encoding = nb::cast<std::string>(item["expected_encoding"]);
                            if (encoding == "srgb") {
                        ...
                        desc.default_value = has_defaults[i] && default_texts[i].empty() ? &defaults[i] : nullptr;
            ...
            .def_static(
                "make_phase_uuid",
                [](const std::string& program_uuid, const std::string& phase_mark) {
                    char result[TC_UUID_SIZE];
                    tc_shader_program_make_phase_uuid(result, sizeof(result), program_uuid.c_str(), phase_mark.c_str());
                    return std::string(result);
                },
                nb::arg("program_uuid"),
                nb::arg("phase_mark"))
            .def("find_phase",
                 [](const TcShaderProgram& value, const std::string& mark) -> nb::object {
            ...
            .def("__repr__", [](const TcShaderProgram& value) {
                if (!value.is_valid())
                    return std::string("<TcShaderProgram invalid>");
                return std::string("<TcShaderProgram ") + value.uuid() + " v" + std::to_string(value.version()) + ">";
            });
