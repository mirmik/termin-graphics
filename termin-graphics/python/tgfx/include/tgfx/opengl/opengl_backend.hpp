...
private:
    ...
    // UI and immediate mode resources are stored per-context in
    // tc_gpu_context (VAO) and tc_gpu_share_group (VBO).
...
public:
    ...
    void draw_ui_vertices(const float* vertices, int vertex_count) override {
        ...
        glBindVertexArray(ctx->backend_ui_vao);
        glBindBuffer(GL_ARRAY_BUFFER, ctx->share_group->backend_ui_vbo);
...
private:
    ...
    void ensure_ui_buffers() {
        tc_gpu_context* ctx = tc_gpu_get_context();
        if (!ctx) {
            tc_log_warn("[ensure_ui_buffers] no GPU context set");
            return;
        }
        tc_gpu_share_group* group = ctx->share_group;

        if (group->backend_ui_vbo == 0) {
            glGenBuffers(1, &group->backend_ui_vbo);
        }
        if (ctx->backend_ui_vao == 0) {
            glGenVertexArrays(1, &ctx->backend_ui_vao);
        }
    }
    ...
    void draw_immediate_impl(const float* vertices, int vertex_count, GLenum mode) {
        ...
        glBindVertexArray(ctx->backend_immediate_vao);
        glBindBuffer(GL_ARRAY_BUFFER, ctx->share_group->backend_immediate_vbo);
    ...
    void ensure_immediate_buffers() {
        tc_gpu_context* ctx = tc_gpu_get_context();
        if (!ctx) {
            tc_log_warn("[ensure_immediate_buffers] no GPU context set");
            return;
        }
        tc_gpu_share_group* group = ctx->share_group;

        if (group->backend_immediate_vbo == 0) {
            glGenBuffers(1, &group->backend_immediate_vbo);
        }
        if (ctx->backend_immediate_vao == 0) {
            glGenVertexArrays(1, &ctx->backend_immediate_vao);

            glBindVertexArray(ctx->backend_immediate_vao);
            glBindBuffer(GL_ARRAY_BUFFER, group->backend_immediate_vbo);
