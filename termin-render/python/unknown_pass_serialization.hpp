...
namespace termin {
    ...
    inline nb::dict serialize_unknown_pass_envelope(tc_pass* raw) {
        ...
        if (!unknown)
            throw std::runtime_error("incompatible UnknownPass object");
        ...
        nb::dict graph;
        nb::list reads;
        for (const std::string& value : unknown->original_reads)
            reads.append(value);
        graph["reads"] = reads;
        nb::list writes;
        for (const std::string& value : unknown->original_writes)
            writes.append(value);
        graph["writes"] = writes;
        nb::list aliases;
        ...
        nb::list symbols;
        for (const std::string& value : unknown->original_internal_symbols)
