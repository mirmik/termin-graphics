...
namespace termin {
    ...
    collect_pass_strings(tc_pass* pass, PassStringCollector collect, size_t values_per_item = 1) {
    ...
    static std::vector<const char*> collect_frame_graph_alias_names(tc_frame_graph* fg, const char* canonical) {
    ...
    constexpr const char* kPythonPassClassProjectionBinding = "termin.python.frame_pass_class_projection";
    ...
    static bool adopt_pass_from_python_api(tc_pipeline_handle pipeline, tc_pass* pass, tc_pass* before = nullptr) {
        if (!pass)
            return false;
        if (pass->native_language == TC_LANGUAGE_PYTHON && pass->body) {
            Py_INCREF(reinterpret_cast<PyObject*>(pass->body));
            const bool adopted = before ? tc_pipeline_adopt_pass_before(pipeline, pass, &py_owned_pass_deleter, before)
                                        : tc_pipeline_adopt_pass(pipeline, pass, &py_owned_pass_deleter);
            if (!adopted) {
                Py_DECREF(reinterpret_cast<PyObject*>(pass->body));
            }
            return adopted;
        }
        return before ? tc_pipeline_adopt_pass_before(pipeline, pass, pass->deleter, before)
    ...
    static bool python_pass_factory(void* context, const void*, void* out_result) {
        ...
        *static_cast<tc_pass**>(out_result) = nullptr;
        try {
            ...
            tc_log(TC_LOG_ERROR, "python_pass_factory: %s has no valid _tc_pass", factory->type_name.c_str());
        } catch (const nb::python_error& e) {
            tc_log(TC_LOG_ERROR, "python_pass_factory: failed to create %s: %s", factory->type_name.c_str(), e.what());
            PyErr_Clear();
        }
    ...
    static size_t cache_string_iterable(
        nb::handle py_pass, nb::handle iterable, const char* cache_attr, const char** out, size_t max) {
        nb::list cached;
    ...
    static size_t cache_string_pairs(
        nb::handle py_pass, nb::handle iterable, const char* cache_attr, const char** out, size_t max_pairs) {
        nb::list cached;
        size_t pair_count = 0;
        for (nb::handle item : iterable) {
    ...
    static size_t py_pass_string_callback(void* wrapper,
    ...
    static size_t py_pass_get_internal_symbols(void* wrapper, const char** out, size_t max) {
        return py_pass_string_callback(
            wrapper, "get_internal_symbols", "_cached_tc_symbols", "get_internal_symbols", out, max);
    }
    ...
    void bind_tc_pass_runtime(nb::module_& m) {
        ...
        nb::class_<TcPassRef>(m, "TcPassRef")
            ...
            .def("get_inplace_aliases",
                 [](TcPassRef& self) {
                     std::vector<std::tuple<std::string, std::string>> result;
                     tc_pass* p = self.ptr();
                     if (p) {
                         std::vector<const char*> pairs = collect_pass_strings(p, tc_pass_get_inplace_aliases, 2);
                         size_t count = pairs.size() / 2;
                         for (size_t i = 0; i < count; i++) {
                             result.emplace_back(pairs[i * 2] ? pairs[i * 2] : "",
            ...
            .def("get_internal_symbols",
                 [](TcPassRef& self) {
                     ...
                     tc_pass* p = self.ptr();
                     if (p) {
                         std::vector<const char*> symbols = collect_pass_strings(p, tc_pass_get_internal_symbols);
            ...
            .def("get_internal_symbols_with_timing",
                 [](TcPassRef& self) {
            ...
            .def(
                "set_debugger_window",
                [](TcPassRef& self, nb::object window, nb::object depth_callback, nb::object error_callback) {
            ...
            .def("to_python", [](TcPassRef& self) { return tc_pass_to_python(self.ptr()); })
            .def_prop_ro("viewport_name",
                         [](TcPassRef& self) {
                             tc_pass* p = self.ptr();
            ...
            .def(
                "get_field",
                [](TcPassRef& self, const std::string& field_name) -> nb::object {
                    tc::init_cpp_inspect_vtable();
                    tc_pass* p = self.ptr();
                    if (!p) {
                        return nb::none();
                    ...
                    try {
                        return tc::InspectRegistry_get(
                            tc::InspectRegistry::instance(), obj_ptr, tc_pass_type_name(p), field_name);
                    } catch (const std::exception& e) {
                        tc_log(TC_LOG_ERROR, "[tc_pass] get_field(%s) failed: %s", field_name.c_str(), e.what());
                        return nb::none();
                    } catch (...) {
                        tc_log(
                            TC_LOG_ERROR, "[tc_pass] get_field(%s) failed with unknown exception", field_name.c_str());
                        return nb::none();
                    }
                },
                nb::arg("field_name"))
            .def(
                "set_field",
                [](TcPassRef& self, const std::string& field_name, nb::object value) {
                    tc::init_cpp_inspect_vtable();
                    tc_pass* p = self.ptr();
                    if (!p || value.is_none()) {
                        return;
                    ...
                    try {
                        tc::InspectRegistry_set(
                            tc::InspectRegistry::instance(), obj_ptr, tc_pass_type_name(p), field_name, value, nullptr);
                    } catch (const std::exception& e) {
                        tc_log(TC_LOG_ERROR, "[tc_pass] set_field(%s) failed: %s", field_name.c_str(), e.what());
                    } catch (...) {
                        tc_log(
                            TC_LOG_ERROR, "[tc_pass] set_field(%s) failed with unknown exception", field_name.c_str());
                    }
                },
                nb::arg("field_name"),
                nb::arg("value"))
            .def(
                "set_viewport_name",
                [](TcPassRef& self, const std::string& name) {
                    tc_pass* p = self.ptr();
                    if (!p) {
                        return;
                    }
        ...
        nb::class_<TcPass>(m, "TcPass")
            .def(
                "__init__",
                [](TcPass* self, nb::object py_self, const std::string& type_name) {
                    ensure_py_callbacks_registered();
        ...
        m.def(
            "tc_pipeline_create",
            [](const std::string& name) -> std::tuple<uint32_t, uint32_t> {
        ...
        m.def("tc_pipeline_insert_pass_before",
              [](std::tuple<uint32_t, uint32_t> h, TcPassRef pass_ref, TcPassRef before_ref) {
                  tc_pipeline_handle handle = {std::get<0>(h), std::get<1>(h)};
                  if (pass_ref.valid()) {
        ...
        m.def("tc_pipeline_insert_pass_before",
              [](std::tuple<uint32_t, uint32_t> h, TcPass* pass, TcPassRef before_ref) {
                  tc_pipeline_handle handle = {std::get<0>(h), std::get<1>(h)};
                  if (pass && pass->ptr()) {
        ...
        m.def("tc_frame_graph_destroy",
              [](intptr_t fg_ptr) { tc_frame_graph_destroy(reinterpret_cast<tc_frame_graph*>(fg_ptr)); });

        m.def("tc_frame_graph_get_error",
              [](intptr_t fg_ptr) { return tc_frame_graph_get_error(reinterpret_cast<tc_frame_graph*>(fg_ptr)); });
        ...
        m.def("tc_frame_graph_schedule_count",
              [](intptr_t fg_ptr) { return tc_frame_graph_schedule_count(reinterpret_cast<tc_frame_graph*>(fg_ptr)); });
        ...
        m.def("tc_frame_graph_canonical_resource", [](intptr_t fg_ptr, const std::string& name) {
            return std::string(
                tc_frame_graph_canonical_resource(reinterpret_cast<tc_frame_graph*>(fg_ptr), name.c_str()));
        });

        m.def("tc_frame_graph_dump",
              [](intptr_t fg_ptr) { tc_frame_graph_dump(reinterpret_cast<tc_frame_graph*>(fg_ptr)); });
        ...
        m.def("tc_resources_allocate_dict",
              [](intptr_t fg_ptr, nb::dict specs_dict, nb::object target_fbo, int width, int height) -> nb::dict {
                  tc_frame_graph* fg = reinterpret_cast<tc_frame_graph*>(fg_ptr);
                  nb::dict result;
        ...
        m.def("tc_pipeline_registry_count", []() { return tc_pipeline_registry_count(); });
            ...
            if (infos) {
                for (size_t i = 0; i < count; i++) {
                    ...
                    if (infos[i].ptr && std::string(tc_pass_type_name(infos[i].ptr)) == "UnknownPass") {
                        ...
                        if (unknown)
                            info["original_type"] = unknown->original_type;
                    }
                    info["pipeline_handle"] =
                        nb::make_tuple(infos[i].pipeline_handle.index, infos[i].pipeline_handle.generation);
        ...
        m.def("tc_pass_registry_type_count", []() { return tc_pass_registry_type_count(); });
        ...
        m.def(
            ...
            [](const std::string& type_name) {
                void* context =
                    tc_runtime_type_registry_get_binding(type_name.c_str(), kPythonPassClassProjectionBinding);
                return context ? static_cast<PythonPassFactoryContext*>(context)->cls : nb::object(nb::none());
            },
            nb::arg("type_name"));

        m.def(
            "tc_pass_registry_bind_class_projection",
            [](const std::string& type_name, nb::object cls) {
                ...
                if (!PyType_Check(cls.ptr())) {
                    tc_log(TC_LOG_ERROR, "[PassRegistry] Python projection for '%s' is not a class", type_name.c_str());
                    return false;
                }
                auto* projection = new PythonPassFactoryContext{std::move(cls), type_name};
                return tc_runtime_type_registry_set_binding(type_name.c_str(),
        ...
        m.def(
            ...
            [](const std::string& type_name,
               ...
               nb::dict metadata) {
                ...
                const bool allow_same_owner_replacement = existing_owner && owner == existing_owner;
                ...
                auto* factory_context = new PythonPassFactoryContext{std::move(cls), type_name};
                tc_runtime_owned_factory factory = tc_runtime_owned_factory_make(
                    python_pass_factory, factory_context, destroy_python_pass_factory_context);
                auto descriptor = FramePassTypeDescriptorBuilder(type_name.c_str(),
                                                                 owner.c_str(),
                                                                 parent_name,
                                                                 factory,
                                                                 TC_EXTERNAL_PASS,
                                                                 allow_same_owner_replacement);
                descriptor.runtime_binding(kPythonPassClassProjectionBinding, factory_context, nullptr);
        ...
        m.def("tc_pass_registry_clear_python", []() {
            ...
            for (const auto& type_name : type_names) {
                if (tc_pass_registry_has(type_name.c_str()) &&
                    ...
                    tc_runtime_type_registry_remove_binding(type_name.c_str(), kPythonPassClassProjectionBinding);
                }
            }
        });

        m.def("tc_pass_registry_has",
              [](const std::string& type_name) { return tc_pass_registry_has(type_name.c_str()); });
