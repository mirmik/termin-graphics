...
namespace termin {
    ...
    template <typename T> void init_render_pass_from_python(T* self, const char* type_name) {
    ...
    static Mat44f mat44f_from_buffer_compatible_object(nb::object value, const char* field_name) {
        try {
            ...
            tc::Log::error("RenderContext %s expects a 4x4 C-contiguous float32 buffer-compatible matrix: %s",
                           field_name,
                           e.what());
            throw std::runtime_error(std::string("RenderContext ") + field_name +
...
namespace termin {
    ...
    void bind_render_framework(nb::module_& m) {
        ...
        nb::class_<ResourceSpec>(m, "ResourceSpec")
            .def(nb::init<>())
            .def(nb::init<std::string, std::string>(), nb::arg("resource"), nb::arg("resource_type") = "fbo")
            .def(
                "__init__",
                [](ResourceSpec* self,
        ...
        nb::class_<Rect2i>(m, "Rect2i")
            .def(nb::init<>())
            .def(nb::init<int, int, int, int>(), nb::arg("x"), nb::arg("y"), nb::arg("width"), nb::arg("height"))
            .def_rw("x", &Rect2i::x)
            .def_rw("y", &Rect2i::y)
            .def_rw("width", &Rect2i::width)
            .def_rw("height", &Rect2i::height)
            .def("__len__", [](const Rect2i&) { return 4; })
            .def("__getitem__",
                 [](const Rect2i& self, size_t index) {
                     switch (index) {
                     case 0:
                         return self.x;
                     case 1:
            ...
            .def(
                "__iter__",
                [](const Rect2i& self) {
                    return nb::make_iterator(nb::type<Rect2i>(), "rect2i_iter", &self.x, &self.x + 4);
                },
                nb::keep_alive<0, 1>())
            .def("__repr__", [](const Rect2i& self) {
                return "Rect2i(" + std::to_string(self.x) + ", " + std::to_string(self.y) + ", " +
        ...
        nb::class_<SceneRenderServices>(m, "SceneRenderServices")
            .def_prop_ro("scene", [](const SceneRenderServices& services) { return services.scene; })
        ...
        nb::class_<ExecuteContext>(m, "ExecuteContext")
            .def(nb::init<>())
            .def("__init__",
                 [](ExecuteContext* self, nb::kwargs kwargs) {
            ...
            .def_prop_rw(
                "view",
                [](const ExecuteContext& ctx) -> const RenderCamera* { return ctx.view.primary_view(); },
            ...
            .def_prop_ro(
                ...
                [](const ExecuteContext& ctx) -> const SceneRenderServices* {
                    return ctx.capabilities ? ctx.capabilities->find<SceneRenderServices>() : nullptr;
                },
            ...
            .def_prop_ro(
                "ctx2",
                [](const ExecuteContext& ctx) -> tgfx::RenderContext2* { return ctx.ctx2; },
                nb::rv_policy::reference)
            ...
            .def_prop_ro("tex2_depth_writes", [](const ExecuteContext& ctx) -> nb::dict {
                nb::dict result;
        ...
        nb::class_<CxxFramePass>(m, "FramePass")
            ...
            .def_prop_ro("_tc_pass", [](CxxFramePass& p) { return TcPassRef(p.tc_pass_ptr()); });
                ...
                [](GraphAliasPass* self,
                   ...
                   const std::string& pass_name) {
                    new (self) GraphAliasPass(
                        std::move(read_resources), std::move(write_resources), std::move(alias_resources), pass_name);
        ...
        nb::class_<RenderContext>(m, "RenderContext")
            .def(nb::init<>())
            .def("__init__",
                 [](RenderContext* self, nb::kwargs kwargs) {
                     new (self) RenderContext();

                     if (kwargs.contains("scene") || kwargs.contains("camera")) {
                         throw nb::type_error("RenderContext is scene-neutral and no longer accepts scene or camera; "
                     ...
                     if (kwargs.contains("phase")) {
                         ...
                         if (nb::isinstance<nb::str>(value)) {
                             ...
                             if (phase == TC_PHASE_NONE) {
                                 throw nb::value_error(("RenderContext phase '" + name +
        ...
        nb::class_<FrameGraphPresenter>(m, "FrameGraphPresenter")
            ...
            .def(
                ...
                nb::arg("channel_mode"),
                nb::arg("highlight_hdr"))
            .def("compute_hdr_stats", &FrameGraphPresenter::compute_hdr_stats, nb::arg("device"), nb::arg("tex"))
            .def(
                "read_depth_normalized",
                [](FrameGraphPresenter& self, tgfx::IRenderDevice* device, tgfx::TextureHandle tex) -> nb::object {
                    int w = 0, h = 0;
                    auto data = self.read_depth_normalized(device, tex, &w, &h);
                    if (data.empty()) {
                        return nb::none();
                    }
                    nb::bytes bytes_obj(reinterpret_cast<const char*>(data.data()), data.size());
                    return nb::make_tuple(bytes_obj, w, h);
                },
                nb::arg("device"),
                nb::arg("tex"))
            .def_static("get_texture_info", &FrameGraphPresenter::get_texture_info, nb::arg("device"), nb::arg("tex"));
