...
void bind_tc_render_target(nb::module_& m) {
    ...
    nb::class_<tc_render_target_handle>(m, "RenderTargetHandle")
        ...
        .def_prop_ro("valid", [](const tc_render_target_handle& h) { return tc_render_target_handle_valid(h); })
        .def_prop_ro("alive", [](const tc_render_target_handle& h) { return tc_render_target_alive(h); })
        .def_prop_rw(
            "name",
            ...
            [](tc_render_target_handle& h, const std::string& name) { tc_render_target_set_name(h, name.c_str()); })
        .def_prop_rw(
            "kind",
            [](const tc_render_target_handle& h) -> std::string {
        ...
        .def_prop_rw(
            "clear_depth_enabled",
            [](const tc_render_target_handle& h) { return tc_render_target_get_clear_depth_enabled(h); },
            [](tc_render_target_handle& h, bool v) { tc_render_target_set_clear_depth_enabled(h, v); })
        .def_prop_rw(
            "clear_depth_value",
            [](const tc_render_target_handle& h) { return tc_render_target_get_clear_depth_value(h); },
            [](tc_render_target_handle& h, float v) { tc_render_target_set_clear_depth_value(h, v); })
        .def_prop_rw(
            "enabled",
        ...
        .def("ensure_textures", [](tc_render_target_handle& h) { tc_render_target_ensure_textures(h); })
        ...
        .def(
            ...
            [](tc_render_target_handle& h, const std::string& resource_name) -> nb::object {
                const bool fbo_resource =
                    resource_name == "OUTPUT" || resource_name == "DISPLAY" || resource_name == "RT_COLOR";
                const bool color_texture_resource = resource_name == "RT_COLOR.color";
                const bool depth_texture_resource = resource_name == "RT_DEPTH" || resource_name == "RT_COLOR.depth";
                ...
                if (depth_texture_resource) {
                    ...
                    d["has_depth"] = false;
                    d["color_format_name"] =
                        tc_render_target_format_to_string(static_cast<tc_texture_format>(depth->format));
                } else if (color_texture_resource) {
                    d["resource_type"] = "color_texture";
                    d["has_depth"] = false;
                    d["color_format_name"] =
                        tc_render_target_format_to_string(static_cast<tc_texture_format>(color->format));
                } else {
                    d["resource_type"] = "fbo";
    ...
    m.def(
        "render_target_new",
        [](const std::string& name) { return tc_render_target_new(name.c_str()); },
        nb::arg("name"));

    m.def("render_target_pool_count", []() { return tc_render_target_pool_count(); });

    m.def("render_target_pool_list", []() {
        std::vector<tc_render_target_handle> result;
        tc_render_target_pool_foreach(
            [](tc_render_target_handle h, void* ud) -> bool {
