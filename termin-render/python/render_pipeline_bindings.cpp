...
namespace termin {
    ...
    namespace {
        ...
        void
        adopt_python_pass(tc_pipeline_handle pipeline, tc_pass* pass, nb::handle owner, tc_pass* before = nullptr) {
            if (!pass || owner.is_none()) {
                throw std::runtime_error("cannot adopt an invalid Python pass");
            }
            Py_INCREF(owner.ptr());
            const bool adopted =
                before ? tc_pipeline_adopt_pass_before(pipeline, pass, &python_pipeline_pass_deleter, before)
                       : tc_pipeline_adopt_pass(pipeline, pass, &python_pipeline_pass_deleter);
            if (!adopted) {
                Py_DECREF(owner.ptr());
        ...
        tc_pass* make_unknown_pass_from_serialized(const std::string& original_type, nb::dict data) {
            if (!tc_pass_registry_has("UnknownPass"))
                register_builtin_render_pass_types();
            tc_pass* raw = tc_pass_registry_create("UnknownPass");
            if (!raw)
                throw std::runtime_error("failed to create UnknownPass");
            ...
            unknown->original_data = data.contains("data") ? py_to_tc_value(data["data"]) : tc_value_dict_new();
            if (data.contains("pass_name"))
                tc_pass_set_name(raw, nb::cast<std::string>(data["pass_name"]).c_str());
            if (data.contains("enabled"))
                raw->enabled = nb::cast<bool>(data["enabled"]);
            if (data.contains("passthrough"))
                raw->passthrough = nb::cast<bool>(data["passthrough"]);
            ...
            if (data.contains("_unknown_graph")) {
                ...
                if (graph.contains("reads"))
                    unknown->original_reads = nb::cast<std::vector<std::string>>(graph["reads"]);
                if (graph.contains("writes"))
                    unknown->original_writes = nb::cast<std::vector<std::string>>(graph["writes"]);
                ...
                if (graph.contains("internal_symbols")) {
                    unknown->original_internal_symbols = nb::cast<std::vector<std::string>>(graph["internal_symbols"]);
                }
                if (graph.contains("resource_specs")) {
                    for (nb::handle value : nb::cast<nb::list>(graph["resource_specs"])) {
                        ...
                        if (item.contains("resource"))
                            spec.resource = nb::cast<std::string>(item["resource"]);
                        if (item.contains("resource_type"))
                        ...
                        if (item.contains("format"))
                            spec.format = nb::cast<std::string>(item["format"]);
                        ...
                        if (item.contains("scale"))
                            spec.scale = nb::cast<float>(item["scale"]);
                        if (item.contains("filter"))
                            spec.filter = static_cast<TextureFilter>(nb::cast<int>(item["filter"]));
    ...
    void bind_render_pipeline(nb::module_& m) {
        nb::class_<TcPipelineTemplate>(m, "TcPipelineTemplate")
            ...
            .def_prop_ro("uuid", [](const TcPipelineTemplate& value) { return std::string(value.uuid()); })
            .def_prop_ro("name", [](const TcPipelineTemplate& value) { return std::string(value.name()); })
            .def_prop_ro("version", &TcPipelineTemplate::version)
            .def_prop_ro("execution_model",
                         [](const TcPipelineTemplate& value) {
            ...
            .def("serialize",
                 [](const TcPipelineTemplate& value) {
                     ...
                     const size_t size = tc_pipeline_template_serialize(pipeline_template, nullptr, 0);
                     if (size == 0) {
                         throw std::runtime_error("failed to size pipeline template descriptor");
                     }
                     std::vector<uint8_t> bytes(size);
                     if (tc_pipeline_template_serialize(pipeline_template, bytes.data(), bytes.size()) != size) {
                         ...
                         [](const TcPipelineTemplate& value) {
                             ...
                             for (uint32_t i = 0; i < pipeline->pass_count; ++i) {
                                 ...
                                 item["parameters"] = pipeline->passes[i].parameters
                                                          ? nb::cast(pipeline->passes[i].parameters)
                                                          : nb::none();
                                 item["viewport_name"] = pipeline->passes[i].viewport_name
                                                             ? nb::cast(pipeline->passes[i].viewport_name)
                                                             : nb::none();
            ...
            .def_prop_ro("resources",
                         [](const TcPipelineTemplate& value) {
                             ...
                             for (uint32_t i = 0; i < pipeline->resource_count; ++i) {
                                 ...
                                 item["viewport_name"] =
                                     resource.viewport_name ? nb::cast(resource.viewport_name) : nb::none();
                         ...
                         [](const TcPipelineTemplate& value) {
                             ...
                             for (uint32_t i = 0; i < pipeline->target_count; ++i) {
                                 ...
                                 item["viewport_name"] = pipeline->targets[i].viewport_name
                                                             ? nb::cast(pipeline->targets[i].viewport_name)
                                                             : nb::none();
            ...
            .def_prop_ro("resource_views",
                         [](const TcPipelineTemplate& value) {
                             ...
                             for (uint32_t i = 0; i < pipeline->resource_view_count; ++i) {
                                 const tc_pipeline_template_resource_view_desc& view = pipeline->resource_views[i];
                                 nb::dict item;
                                 item["parent"] = view.parent;
                                 item["attachment"] =
                                     view.attachment == TC_PIPELINE_ATTACHMENT_DEPTH ? "depth" : "color";
            ...
            .def_prop_ro("fbo_compositions", [](const TcPipelineTemplate& value) {
                ...
                for (uint32_t i = 0; i < pipeline->fbo_composition_count; ++i) {
                    const tc_pipeline_template_fbo_composition_desc& composition = pipeline->fbo_compositions[i];
                    nb::dict item;
        ...
        nb::class_<RenderPipeline>(m, "RenderPipeline")
            ...
            .def_prop_ro(
                "_tc_pipeline", [](RenderPipeline& self) { return self.ptr(); }, nb::rv_policy::reference)

            .def_prop_ro("_pipeline_handle", [](RenderPipeline& self) -> tc_pipeline_handle { return self.handle(); })

            .def_prop_ro("pipeline_template",
                         [](RenderPipeline& self) { return TcPipelineTemplate(self.template_handle()); })

            .def_prop_ro("pass_count", &RenderPipeline::pass_count)

            .def("add_pass",
                 [](RenderPipeline& self, TcPassRef pass_ref) {
                     if (!pass_ref.valid())
                         return;
                     tc_pass* pass = pass_ref.ptr();
                     if (pass->native_language == TC_LANGUAGE_PYTHON && pass->body) {
                         adopt_python_pass(self.handle(), pass, nb::handle(reinterpret_cast<PyObject*>(pass->body)));
                     } else {
                         self.add_pass(pass);
                     }
                 })
            .def("add_pass",
                 [](RenderPipeline& self, TcPass* pass) {
                     if (pass && pass->ptr()) {
                         tc_pass* raw = pass->ptr();
                         adopt_python_pass(self.handle(), raw, nb::handle(reinterpret_cast<PyObject*>(raw->body)));
                     }
                 })
            .def("add_pass",
                 [](RenderPipeline& self, nb::object pass_obj) {
                     adopt_python_pass(
                         self.handle(), tc_pass_ptr_from_python_pass(pass_obj, "RenderPipeline.add_pass"), pass_obj);
                 })
            .def("remove_pass",
                 [](RenderPipeline& self, TcPassRef pass_ref) {
                     if (pass_ref.valid())
            ...
            .def("insert_pass_before",
                 [](RenderPipeline& self, TcPassRef pass_ref, TcPassRef before_ref) {
                     if (pass_ref.valid()) {
                         tc_pass* pass = pass_ref.ptr();
                         tc_pass* before = before_ref.valid() ? before_ref.ptr() : nullptr;
                         if (pass->native_language == TC_LANGUAGE_PYTHON && pass->body) {
                             adopt_python_pass(
                                 self.handle(), pass, nb::handle(reinterpret_cast<PyObject*>(pass->body)), before);
                         } else {
            ...
            .def("insert_pass_before",
                 [](RenderPipeline& self, nb::object pass_obj, nb::object before_obj) {
                     tc_pass* pass_ptr =
                         tc_pass_ptr_from_python_pass(pass_obj, "RenderPipeline.insert_pass_before(pass)");
                     tc_pass* before_ptr = nullptr;
                     if (!before_obj.is_none()) {
                         before_ptr =
                             tc_pass_ptr_from_python_pass(before_obj, "RenderPipeline.insert_pass_before(before)");
                     }
            ...
            .def(
                "move_pass_before",
                [](RenderPipeline& self, nb::object pass_obj, nb::object before_obj) {
                    tc_pass* pass_ptr = tc_pass_ptr_from_python_pass(pass_obj, "RenderPipeline.move_pass_before(pass)");
                    tc_pass* before_ptr = nullptr;
                    if (!before_obj.is_none()) {
                        before_ptr =
                            tc_pass_ptr_from_python_pass(before_obj, "RenderPipeline.move_pass_before(before)");
                    }
                ...
                nb::arg("pass_obj"),
                nb::arg("before_obj") = nb::none())
            .def("get_pass",
                 [](RenderPipeline& self, const std::string& name) { return TcPassRef(self.get_pass(name)); })
            .def("get_pass_by_name",
                 [](RenderPipeline& self, const std::string& name) { return TcPassRef(self.get_pass(name)); })
            .def("get_pass_at", [](RenderPipeline& self, size_t index) { return TcPassRef(self.get_pass_at(index)); })
            ...
            .def("get_spec_at",
                 [](RenderPipeline& self, size_t index) -> nb::object {
            ...
            .def("get_fbo",
                 [](RenderPipeline& self, const std::string& key) -> nb::object { return fbo_info(self, key); })

            .def("get_resource_info",
                 [](RenderPipeline& self, const std::string& key) -> nb::object {
            ...
            .def_static(
                "deserialize",
                [](nb::dict data, nb::object resource_manager) -> RenderPipeline* {
                    ...
                    std::unique_ptr<RenderPipeline, RenderPipelineCandidateDeleter> pipeline(new RenderPipeline(name));
                        ...
                        for (size_t i = 0; i < nb::len(passes); i++) {
                            try {
                                ...
                                if (!pass_type.empty() && !tc_pass_registry_has(pass_type.c_str())) {
                                    ...
                                    if (!tc_pipeline_adopt_pass(pipeline->handle(), unknown, unknown->deleter)) {
                                ...
                                tc::Log::error(
                                    "RenderPipeline::deserialize: failed to deserialize pass %zu: %s", i, e.what());
                                throw;
                            }
                        }
                    }
                    ...
                    if (data.contains("resource_views")) {
                        ...
                        for (auto [key, value] : values) {
                            ...
                            if (!item.contains("parent") || !item.contains("attachment")) {
                                throw std::runtime_error("resource view requires 'parent' and 'attachment'");
                            }
                            ResourceView view;
                            view.parent = nb::cast<std::string>(item["parent"]);
                            const std::string attachment = nb::cast<std::string>(item["attachment"]);
                            ...
                            if (attachment == "depth") {
                                ...
                                throw std::runtime_error("resource view attachment must be 'color' or 'depth'");
                            }
                    ...
                    if (data.contains("fbo_compositions")) {
                        ...
                        for (auto [key, value] : values) {
                            ...
                            if (composition.color.empty() && composition.depth.empty()) {
                                throw std::runtime_error("FBO composition requires a color or depth attachment");
                            }
            ...
            .def(
                "copy",
                [](RenderPipeline& self, nb::object resource_manager) -> nb::object {
        ...
        m.def(
            ...
            [](const std::string& json_str, const std::string& template_uuid, const std::string& template_name) {
               ...
               const std::string& execution_model_name) {
                ...
                for (nb::handle value : target_values) {
                    ...
                    if (item.contains("height"))
                        target.height = nb::cast<int32_t>(item["height"]);
                ...
                if (execution_model_name == "single_view") {
                    ...
                    throw std::invalid_argument("unsupported pipeline execution model '" + execution_model_name + "'");
                }
