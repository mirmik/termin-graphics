...
namespace termin {
    ...
    bool Drawable::collect_render_items(const tc_render_item_collect_context& context, tc_render_item_sink& sink) {
        (void)context;
        if (!sink.emit) {
        ...
        tc::Log::error("[Drawable] component '%s' does not implement collect_render_items",
    ...
    namespace {
        ...
        bool validate_render_item(const tc_render_item& item,
            ...
            tc_component* component =
                item.source.adapter_data ? reinterpret_cast<tc_component*>(item.source.adapter_data) : source_component;
            ...
            if (item.kind == TC_RENDER_ITEM_KIND_MESH && tc_mesh_handle_is_invalid(item.payload.mesh.mesh_handle)) {
                tc::Log::error("[RenderItemSink] malformed Mesh item: pass='%s' phase='%s' component='%s' geometry=%d "
                               "has no stable mesh handle",
                               pass_name,
                               phase_mark,
                               component_type,
                               item.geometry_id);
            ...
            if (item.kind == TC_RENDER_ITEM_KIND_LINE_BATCH &&
                ...
                tc::Log::error("[RenderItemSink] malformed LineBatch item: pass='%s' phase='%s' component='%s' "
                               "geometry=%d has no drawable points",
                               pass_name,
                               phase_mark,
                               component_type,
                               item.geometry_id);
            ...
            if (item.kind == TC_RENDER_ITEM_KIND_TEXT_BATCH &&
                ...
                tc::Log::error("[RenderItemSink] malformed TextBatch item: pass='%s' phase='%s' component='%s' "
                               "geometry=%d has empty text",
                               pass_name,
                               phase_mark,
                               component_type,
                               item.geometry_id);
            ...
            if (item.kind == TC_RENDER_ITEM_KIND_FOLIAGE_BATCH &&
                (tc_mesh_handle_is_invalid(item.payload.foliage_batch.prototype_mesh_handle) ||
                 !item.payload.foliage_batch.foliage_uuid || item.payload.foliage_batch.foliage_uuid[0] == '\0')) {
                tc::Log::error("[RenderItemSink] malformed FoliageBatch item: pass='%s' phase='%s' component='%s' "
                               "geometry=%d has no stable prototype mesh handle or foliage asset",
                               pass_name,
                               phase_mark,
                               component_type,
                               item.geometry_id);
            ...
            if ((item.flags & TC_RENDER_ITEM_FLAG_HAS_MATERIAL_PHASE) && !item.material_phase) {
                tc::Log::error("[RenderItemSink] malformed %s item: pass='%s' phase='%s' component='%s' geometry=%d "
                               "has material flag without material phase",
            ...
            if ((item.flags & TC_RENDER_ITEM_FLAG_HAS_INLINE_UNIFORM) != 0u &&
                (item.inline_uniform.name[0] == '\0' ||
                 std::memchr(item.inline_uniform.name, '\0', TC_RENDER_ITEM_INLINE_UNIFORM_NAME_CAPACITY) == nullptr ||
                 item.inline_uniform.size == 0u ||
                 item.inline_uniform.size > TC_RENDER_ITEM_INLINE_UNIFORM_DATA_CAPACITY)) {
                tc::Log::error("[RenderItemSink] malformed %s item: pass='%s' phase='%s' component='%s' geometry=%d "
                               "has invalid inline uniform name/size (%u)",
                               render_item_kind_name(item.kind),
                               pass_name,
                               phase_mark,
                               component_type,
        ...
        tc_render_item_source scene_item_source(tc_component* component) {
            ...
            source.namespace_id = (static_cast<uint64_t>(owner.pool.generation) << 32u) | owner.pool.index;
            ...
            const size_t component_index = tc_entity_pool_component_index(pool, owner.id, component);
            source.subobject_id = component_index <= UINT32_MAX ? static_cast<uint32_t>(component_index) : UINT32_MAX;
            return source;
        }
        ...
        bool emit_render_item_to_vector(const tc_render_item* item, void* user_data) {
            ...
            if (copy.kind == TC_RENDER_ITEM_KIND_LINE_BATCH && copy.payload.line_batch.points &&
                ...
                if (copy.payload.text_batch.text) {
                    ...
                    if (source == data->last_text_source) {
                        ...
                        if (index == data->collection->text_batch_strings.size()) {
                            data->collection->text_batch_strings.push_back(std::make_unique<std::string>());
                        }
            ...
            if (copy.kind == TC_RENDER_ITEM_KIND_FOLIAGE_BATCH && copy.payload.foliage_batch.foliage_uuid) {
                ...
                if (source == data->last_foliage_source) {
                    ...
                    if (index == data->collection->foliage_batch_strings.size()) {
                        data->collection->foliage_batch_strings.push_back(std::make_unique<std::string>());
                    }
    ...
    bool collect_drawable_render_items(tc_component* component,
                                       const tc_render_item_collect_context& context,
    ...
    bool Drawable::_cb_collect_render_items(tc_component* c,
                                            const tc_render_item_collect_context* context,
                                            tc_render_item_sink* sink) {
