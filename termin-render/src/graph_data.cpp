...
namespace tc {
    ...
    static std::string target_socket_type_for(const NodeData& node, const std::string& target_socket_name) {
    ...
    GraphData GraphData::from_trent(const nos::trent& t) {
        ...
        if (graph.execution_model != "single_view" && graph.execution_model != "xr_multiview") {
            tc::Log::error("GraphData: invalid execution_model '%s'", graph.execution_model.c_str());
        ...
        for (const auto& conn : graph.connections) {
            ...
            if (to_node && is_pass_node_type(to_node->node_type) && !conn.to_socket.empty()) {
                if (!has_socket(to_node->inputs, conn.to_socket)) {
                    ...
                    to_node->inputs.push_back({conn.to_socket, socket_type, true});
                }
            }
        }
