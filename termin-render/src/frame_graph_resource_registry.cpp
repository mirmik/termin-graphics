...
bool register_frame_graph_resource_type(
    const FrameGraphResourceTypeDescriptor& descriptor)
{
    if (!valid_resource_type(descriptor.resource_type) || !descriptor.create) {
        tc::Log::error(
            "[FrameGraphResourceRegistry] cannot register a resource type "
            "without a non-empty name and create function");
        return false;
    }

    const bool inserted = resource_type_registry().emplace(
        descriptor.resource_type,
        ResourceTypeRecord{descriptor.create, descriptor.sampled_texture}).second;
    if (!inserted) {
        tc::Log::error(
            "[FrameGraphResourceRegistry] resource type '%s' is already registered",
            descriptor.resource_type);
        return false;
    }
    return true;
...
bool unregister_frame_graph_resource_type(const char* resource_type)
{
    if (!valid_resource_type(resource_type)) {
        tc::Log::error(
            "[FrameGraphResourceRegistry] cannot unregister an empty resource type");
        return false;
    }
    if (resource_type_registry().erase(resource_type) == 0) {
        tc::Log::error(
            "[FrameGraphResourceRegistry] resource type '%s' is not registered",
            resource_type);
        return false;
    }
    return true;
...
bool has_frame_graph_resource_type(const char* resource_type)
{
    return valid_resource_type(resource_type)
        && resource_type_registry().find(resource_type)
            != resource_type_registry().end();
}

bool frame_graph_resource_type_matches(
    const FrameGraphResourceTypeDescriptor& descriptor)
{
    ...
    return it != resource_type_registry().end()
        && it->second.create == descriptor.create
...
FrameGraphResource* create_frame_graph_resource(const ResourceSpec& spec)
{
    ...
    if (it == resource_type_registry().end()) {
        tc::Log::error(
            "[FrameGraphResourceRegistry] resource '%s' has unknown type '%s'",
            spec.resource.c_str(),
    ...
    if (!resource) {
        tc::Log::error(
            "[FrameGraphResourceRegistry] factory for type '%s' failed to create resource '%s'",
            spec.resource_type.c_str(),
            spec.resource.c_str());
        return nullptr;
    }
    ...
    if (!actual_type || std::strcmp(actual_type, spec.resource_type.c_str()) != 0) {
        tc::Log::error(
            "[FrameGraphResourceRegistry] factory for type '%s' returned mismatched type '%s'",
            spec.resource_type.c_str(),
...
FrameGraphResourceSampledTexture frame_graph_resource_sampled_texture(
