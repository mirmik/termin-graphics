...
namespace termin {
    namespace {
        ...
        void* pass_object_ptr(tc_pass* pass) {
            if (!pass)
                return nullptr;
            return pass->kind == TC_NATIVE_PASS ? static_cast<void*>(CxxFramePass::from_tc(pass)) : pass->body;
        }

        std::vector<std::string> collect_names(tc_pass* pass, size_t (*collector)(tc_pass*, const char**, size_t)) {
        ...
        std::vector<std::pair<std::string, std::string>> collect_aliases(tc_pass* pass) {
            ...
            while (count > 0) {
                ...
                const size_t actual = tc_pass_get_inplace_aliases(pass, values.data(), count);
                if (actual <= count) {
            ...
            result.reserve(count);
            for (size_t i = 0; i < count; ++i) {
                result.emplace_back(values[i * 2] ? values[i * 2] : "", values[i * 2 + 1] ? values[i * 2 + 1] : "");
            }
            return result;
        }
        ...
        std::vector<ResourceSpec> collect_specs(tc_pass* pass) {
            ...
            while (count > 0) {
                values.resize(count);
                const size_t actual = tc_pass_get_resource_specs(pass, values.data(), values.size());
                if (actual <= count) {
                    values.resize(actual);
                    break;
                }
        ...
        bool type_selected(const std::unordered_set<std::string>& filter, const std::string& type_name) {
            return filter.empty() || filter.count(type_name) != 0;
        }

        bool prepare_replacement(tc_pass* pass,
            ...
            tc_pass* unknown_tc =
                hooks.create_replacement ? hooks.create_replacement() : tc_pass_registry_create("UnknownPass");
            if (!unknown_tc) {
            ...
            unknown->original_data =
                hooks.serialize ? hooks.serialize(object, raw_type) : tc_inspect_serialize(object, raw_type);
        ...
        bool upgrade_at(tc_pipeline_handle pipeline, size_t index, tc_pass* pass, std::string& error) {
            ...
            const tc_inspect_apply_result applied = tc_inspect_deserialize_checked(
                object, unknown->original_type.c_str(), &unknown->original_data, nullptr);
            if (applied.status != TC_INSPECT_APPLY_OK) {
                const std::string field =
                    applied.field_path ? " at field '" + std::string(applied.field_path) + "'" : std::string();
                tc_pass_delete_unowned(restored);
    ...
    UnknownPassDegradationPlan::UnknownPassDegradationPlan(UnknownPassDegradationPlan&&) noexcept = default;

    UnknownPassDegradationPlan& UnknownPassDegradationPlan::operator=(UnknownPassDegradationPlan&&) noexcept = default;
    ...
    bool UnknownPassDegradationPlan::validate(std::string* error) const {
        ...
        if (!_impl) {
            if (error)
                *error = "Pass degradation plan has no state";
            return false;
        }
        if (_impl->is_committed) {
            if (error)
                *error = "Pass degradation plan is already committed";
            return false;
        }
        ...
        for (const Impl::Entry& entry : _impl->entries) {
            if (!tc_pipeline_pool_alive(entry.pipeline) ||
                ...
                if (error)
                    *error = "Prepared pipeline/pass identity changed before commit";
                return false;
            }
            if (!entry.replacement || tc_pipeline_handle_valid(entry.replacement->owner_pipeline)) {
                if (error)
                    *error = "Prepared UnknownPass is no longer unowned";
                return false;
            }
        }
        ...
        for (const auto& [type_name, expected_count] : _impl->expected_type_counts) {
            ...
            if (live_count != expected_count) {
                if (error) {
                    *error = "Pass type '" + type_name + "' has " + std::to_string(live_count) +
                             " live instance(s), but the prepared " + "pipeline batch accounts for " +
    ...
    bool UnknownPassDegradationPlan::commit(std::string* error) {
        ...
        for (; published < _impl->entries.size(); ++published) {
            ...
            if (!tc_pipeline_exchange_pass_at_checked(entry.pipeline,
                                                      entry.index,
                                                      entry.source,
                                                      entry.replacement,
                ...
                for (size_t rollback = published; rollback > 0; --rollback) {
                    ...
                    if (!tc_pipeline_exchange_pass_at_checked(restored.pipeline,
                                                              restored.index,
                                                              restored.replacement,
                                                              restored.source,
                                                              restored.source_deleter,
                                                              &replacement_deleter)) {
                        tc::Log::error("[UnknownPass] Failed to roll back an unexpected publication failure");
                    }
                    restored.source_deleter = nullptr;
                }
                if (error)
                    *error = "Prepared pass publication failed after validation";
    ...
    bool prepare_passes_to_unknown(const std::vector<std::string>& type_names,
        ...
        try {
            ...
            for (size_t pipeline_index = 0; pipeline_index < pipeline_count; ++pipeline_index) {
                ...
                for (size_t index = 0; index < pass_count; ++index) {
                    ...
                    if (!pass || is_unknown_pass(pass))
                        continue;
                    const std::string type_name = tc_pass_type_name(pass);
                    if (!type_selected(filter, type_name))
                        continue;

                    tc_pass* replacement = nullptr;
                    std::string entry_error;
                    if (!prepare_replacement(pass, hooks, &replacement, entry_error)) {
                        if (error) {
                            *error = "Failed to prepare pass '" +
                                     std::string(pass->pass_name ? pass->pass_name : "<unnamed>") + "' of type '" +
                                     type_name + "': " + entry_error;
                        }
                        tc::Log::error("[UnknownPass] %s", error ? error->c_str() : entry_error.c_str());
                        return false;
                    }
                    prepared._impl->entries.push_back({pipeline, index, pass, replacement, nullptr, type_name});
                    ++prepared._impl->expected_type_counts[type_name];
                }
            }
        } catch (const std::exception& exception) {
            if (error) {
                *error = "Exception while preparing pass degradation: " + std::string(exception.what());
            }
            tc::Log::error("[UnknownPass] Exception while preparing degradation: %s", exception.what());
            return false;
        } catch (...) {
            if (error)
                *error = "Unknown exception while preparing pass degradation";
            tc::Log::error("[UnknownPass] Unknown exception while preparing degradation");
            return false;
        }

        if (!prepared.validate(error)) {
            tc::Log::error("[UnknownPass] Prepared plan rejected: %s",
    ...
    UnknownPassStats degrade_passes_to_unknown(const std::vector<std::string>& type_names) {
        UnknownPassStats stats;
        UnknownPassDegradationPlan plan;
        std::string error;
        if (!prepare_passes_to_unknown(type_names, plan, &error)) {
            stats.failed = 1;
            tc::Log::error("[UnknownPass] Failed to prepare pass degradation: %s", error.c_str());
            return stats;
        }
        ...
        if (!plan.commit(&error)) {
            ...
            stats.degraded = 0;
            tc::Log::error("[UnknownPass] Failed to commit pass degradation: %s", error.c_str());
        }
        return stats;
    }

    UnknownPassStats upgrade_unknown_passes(const std::vector<std::string>& type_names) {
        UnknownPassStats stats;
        const std::unordered_set<std::string> filter(type_names.begin(), type_names.end());
        const size_t pipeline_count = tc_pipeline_registry_count();
        for (size_t pipeline_index = 0; pipeline_index < pipeline_count; ++pipeline_index) {
            ...
            for (size_t index = 0; index < pass_count; ++index) {
                ...
                if (upgrade_at(pipeline, index, pass, error)) {
                    ...
                    ++stats.failed;
                    tc::Log::error("[UnknownPass] Failed to upgrade pass '%s': %s",
