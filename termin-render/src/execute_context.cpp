...
namespace termin {
    ...
    bool ExecuteContext::should_capture_internal(const char* symbol) const {
        ...
        for (const FrameGraphCaptureRequest* request : debug_internal_capture_requests) {
            if (request && request->kind == FrameGraphCaptureRequestKind::InternalSymbol && !request->paused &&
                request->capture && request->internal_symbol == symbol) {
                return true;
            }
        }
        return false;
    ...
    bool ExecuteContext::capture_internal(
        const char* symbol, tgfx::TextureHandle texture, int width, int height, tgfx::PixelFormat format) {
        bool captured = false;
        if (!symbol || !texture)
            return false;
        for (FrameGraphCaptureRequest* request : debug_internal_capture_requests) {
            ...
            request->capture->capture_direct_via_ctx2(ctx2, texture, width, height, format, request->max_long_edge);
    ...
    bool ExecuteContext::build_render_pass(std::span<const FrameGraphColorAttachment> colors,
                                           const FrameGraphDepthAttachment* depth,
                                           tgfx::RenderPassDesc& out_pass) const {
        out_pass = {};
        if (colors.size() > tgfx::TGFX2_MAX_COLOR_ATTACHMENTS) {
            tc::Log::error("ExecuteContext::build_render_pass: %zu color attachments exceed engine limit %u",
                           colors.size(),
        ...
        for (size_t index = 0; index < colors.size(); ++index) {
            ...
            if (!source.resource_name || source.resource_name[0] == '\0') {
                tc::Log::error("ExecuteContext::build_render_pass: color attachment %zu has no resource name", index);
                out_pass = {};
                return false;
            }
            const auto resource = tex2_writes.find(source.resource_name);
            if (resource == tex2_writes.end() || !resource->second) {
                tc::Log::error("ExecuteContext::build_render_pass: color output '%s' is unavailable",
            ...
            for (const tgfx::ColorAttachmentDesc& existing : out_pass.colors) {
                if (existing.texture == resource->second) {
                    tc::Log::error("ExecuteContext::build_render_pass: color output '%s' aliases an earlier MRT slot",
                                   source.resource_name);
                    out_pass = {};
                    return false;
                }
        ...
        if (depth) {
            if (!depth->resource_name || depth->resource_name[0] == '\0') {
                tc::Log::error("ExecuteContext::build_render_pass: depth attachment has no resource name");
                out_pass = {};
                return false;
            }
            const auto resource = tex2_depth_writes.find(depth->resource_name);
            if (resource == tex2_depth_writes.end() || !resource->second) {
                tc::Log::error("ExecuteContext::build_render_pass: depth output '%s' is unavailable",
                               depth->resource_name);
                out_pass = {};
                return false;
            }
            for (const tgfx::ColorAttachmentDesc& color : out_pass.colors) {
                if (color.texture == resource->second) {
                    tc::Log::error("ExecuteContext::build_render_pass: depth output '%s' aliases a color MRT slot",
                                   depth->resource_name);
                    out_pass = {};
                    return false;
                }
        ...
        if (out_pass.colors.empty() && !out_pass.has_depth) {
            tc::Log::error("ExecuteContext::build_render_pass: pass has no attachments");
            return false;
        }
        return true;
    }
