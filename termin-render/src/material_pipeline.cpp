...
namespace termin {
    namespace {
        ...
        bool should_log_material_shader_override_error(const MaterialShaderOverrideRequest& request,
                                                       const char* reason) {
        ...
        void append_semantics_to_hash(const std::vector<MaterialPipelineSemantic>& semantics, uint64_t& hash) {
        ...
        void append_resources_to_hash(const std::vector<MaterialPipelineResourceDecl>& resources, uint64_t& hash) {
        ...
        void append_vertex_transform_to_hash(const VertexTransformContract& vertex_transform, uint64_t& hash) {
        ...
        void append_vertex_output_adapter_to_hash(const std::optional<VertexOutputAdapter>& adapter, uint64_t& hash) {
        ...
        void append_surface_composition_to_hash(TcShader original_shader,
                                                const MaterialPipelinePassContract& pass_contract,
                                                uint64_t& hash) {
            MaterialFragmentComposition fragment_composition = pass_contract.fragment_composition;
            ...
            hash = fnv1a_append_u32(static_cast<uint32_t>(fragment_composition), hash);
            if (fragment_composition != MaterialFragmentComposition::SurfaceConsumer) {
                return;
            }
            ...
            const MaterialSurfaceConsumerContract& consumer = *pass_contract.surface_consumer;
            ...
            append_semantics_to_hash(consumer.required_fragment_input.semantics, hash);
            ...
            hash = fnv1a_append(surface ? surface->source_identity : "<unregistered>", hash);
        }
        ...
        std::string shader_override_name_for_request(TcShader original_shader,
        ...
        void log_material_pipeline_assembly_diagnostics(const MaterialShaderOverrideRequest& request,
            ...
            for (const MaterialPipelineDiagnostic& diagnostic : assembly.diagnostics) {
                tc::Log::error("[%s] shader contract assembly failed: %s: %s",
                               context,
        ...
        bool contract_has_vertex_input(const tc_shader_contract_view& contract, const char* semantic) {
            if (!semantic || semantic[0] == '\0') {
                return false;
            }
            for (uint32_t i = 0; i < contract.vertex_input_count; ++i) {
                if (std::strncmp(contract.vertex_inputs[i].semantic, semantic, TC_SHADER_RESOURCE_NAME_MAX) == 0) {
                    return true;
                }
            }
            return false;
        ...
        bool shader_requires_material_pipeline_contract(const tc_shader* shader) {
            ...
            return shader->variant_op == TC_SHADER_VARIANT_NONE || shader->variant_op == TC_SHADER_VARIANT_SKINNING ||
                   shader->variant_op == TC_SHADER_VARIANT_FOLIAGE ||
                   shader->variant_op == TC_SHADER_VARIANT_FOLIAGE_SHADOW;
        }

        const std::optional<VertexTransformContract>&
        vertex_transform_for_request(const MaterialPipelinePassContract& pass_contract, VertexTransformKind kind) {
            switch (kind) {
            case VertexTransformKind::SkinnedMesh:
    ...
    std::string material_pipeline_shader_intent_fingerprint(TcShader original_shader,
        ...
        append_surface_composition_to_hash(original_shader, pass_contract, hash);
        ...
        append_semantics_to_hash(pass_contract.required_material_fragment_input.semantics, hash);
        hash = fnv1a_append(":resources:", hash);
        ...
        char buffer[32];
        std::snprintf(buffer, sizeof(buffer), "shv_%016llx", static_cast<unsigned long long>(hash));
        return buffer;
    }
    ...
    TcShader assemble_material_shader_override(const MaterialShaderOverrideRequest& request) {
        ...
        if (shader_variant_op != TC_SHADER_VARIANT_NONE && original_shader.variant_op() == shader_variant_op) {
        ...
        if (!material_shader_override_supported(request.vertex_transform_kind)) {
            if (should_log_material_shader_override_error(request, "unsupported_transform_kind")) {
                tc::Log::error("[%s] cannot create material shader override for '%s': unsupported transform kind %u",
                               context,
        ...
        if (!request.pass_contract.has_value()) {
            if (should_log_material_shader_override_error(request, "missing_pass_contract")) {
                tc::Log::error("[%s] cannot create material shader override for '%s': pass contract is required",
                               context,
                               original_shader.name());
            }
            return TcShader();
        ...
        if (pass_contract.fragment_composition == MaterialFragmentComposition::SurfaceConsumerOrFinalColor) {
            pass_contract.fragment_composition = original_shader.has_surface_producer()
                                                     ? MaterialFragmentComposition::SurfaceConsumer
                                                     : MaterialFragmentComposition::FinalColor;
        }
        ...
        std::optional<VertexTransformContract> vertex_transform_contract_opt =
            request.vertex_transform_contract.has_value()
                ...
                : vertex_transform_for_request(pass_contract, request.vertex_transform_kind);
        if (!vertex_transform_contract_opt.has_value()) {
            if (should_log_material_shader_override_error(request, "missing_vertex_transform_contract")) {
                tc::Log::error("[%s] cannot create material shader override for '%s': pass '%s' has no %s vertex "
                               "transform contract",
                               context,
                               original_shader.name(),
        ...
        VertexTransformContract vertex_transform_contract = std::move(*vertex_transform_contract_opt);

        const std::string variant_uuid = material_pipeline_shader_intent_fingerprint(
            original_shader, shader_variant_op, vertex_transform_contract, pass_contract);
        ...
        if (!tc_shader_handle_is_invalid(cached_handle)) {
            ...
            const bool matches_request =
                cached && cached->is_variant && cached->variant_op == static_cast<uint8_t>(shader_variant_op) &&
                tc_shader_handle_eq(cached->original_handle, original_shader.handle) && tc_shader_has_contract(cached);
            ...
            if (matches_request && !stale) {
                if (!tc_shader_retain_static(cached_handle)) {
                    tc::Log::error("[%s] failed to retain cached material shader override for '%s'",
                                   context,
                                   original_shader.name());
                    return TcShader();
                }
            ...
            if (!stale) {
                tc::Log::error("[%s] material shader override fingerprint collision for '%s' (%s)",
                               context,
                               original_shader.name(),
                               variant_uuid.c_str());
                return TcShader();
        ...
        assembly_request.material = material_pipeline_material_contract_from_shader(
            original_shader, pass_contract.required_material_fragment_input);
        assembly_request.vertex_transform = vertex_transform_contract;
        assembly_request.pass = pass_contract;
        assembly_request.shader_name =
            shader_override_name_for_request(original_shader, request, vertex_transform_contract, pass_contract);
        ...
        MaterialPipelineShaderAssemblyResult assembly = material_pipeline_assemble_shader(assembly_request);
        if (!assembly.ok()) {
        ...
        if (!tc_shader_has_contract(assembly.shader.get())) {
            tc::Log::error("[%s] material shader override for '%s' was created without tc_shader_contract",
                           context,
                           original_shader.name());
            return TcShader();
        }
        if (!tc_shader_retain_static(assembly.shader.handle)) {
            tc::Log::error(
                "[%s] failed to retain canonical material shader override for '%s'", context, original_shader.name());
            return TcShader();
        }
    ...
    MaterialMeshVertexInput material_mesh_vertex_input_for_shader(const tc_shader* shader,
        ...
        for (uint32_t i = 0; i < contract.vertex_input_count; ++i) {
            ...
            if (std::strcmp(semantic, "position") != 0 && std::strcmp(semantic, "normal") != 0 &&
                std::strcmp(semantic, "uv") != 0 && std::strcmp(semantic, "tangent") != 0 &&
                std::strcmp(semantic, "joints") != 0 && std::strcmp(semantic, "weights") != 0) {
    ...
    bool draw_material_pipeline_mesh(tgfx::RenderContext2& ctx, tc_mesh* mesh, MaterialMeshVertexInput input) {
        switch (input) {
        ...
        case MaterialMeshVertexInput::SkinnedFullMaterial:
            return ::termin::draw_tc_mesh(
                ctx, mesh, {"position", "normal", "uv", "tangent", "joints", "weights"}, true);
        case MaterialMeshVertexInput::SkinnedPositionJointsWeights:
            return ::termin::draw_tc_mesh(ctx, mesh, {"position", "joints", "weights"}, true);
        case MaterialMeshVertexInput::SkinnedPositionNormalJointsWeights:
            return ::termin::draw_tc_mesh(ctx, mesh, {"position", "normal", "joints", "weights"}, true);
        }
        return false;
    }

    bool draw_material_pipeline_submesh(tgfx::RenderContext2& ctx,
                                        tc_mesh* mesh,
        ...
        case MaterialMeshVertexInput::SkinnedFullMaterial:
            return ::termin::draw_tc_submesh(
                ctx, mesh, submesh_index, {"position", "normal", "uv", "tangent", "joints", "weights"}, true);
        case MaterialMeshVertexInput::SkinnedPositionJointsWeights:
            return ::termin::draw_tc_submesh(ctx, mesh, submesh_index, {"position", "joints", "weights"}, true);
        case MaterialMeshVertexInput::SkinnedPositionNormalJointsWeights:
            return ::termin::draw_tc_submesh(
                ctx, mesh, submesh_index, {"position", "normal", "joints", "weights"}, true);
        }
        return false;
    }

    bool ensure_material_pipeline_shader(tgfx::RenderContext2& ctx,
        ...
        if (!shader) {
            tc::Log::error("[MaterialPipeline] %s shader handle is stale (index=%u gen=%u)",
        ...
        if (!tc_shader_ensure_tgfx2(shader, &device, &vs, &fs)) {
            tc::Log::error("[MaterialPipeline] %s tc_shader_ensure_tgfx2 failed for '%s'",
                           debug_context ? debug_context : "material",
        ...
        if (!validate_shader_contract(
                ...
                ShaderContractValidationOptions{
                    debug_context ? debug_context : "MaterialPipeline", requires_contract, true})) {
            return false;
        }
    ...
    bool prepare_material_pipeline_resources(tgfx::RenderContext2& ctx,
                                             tgfx::IRenderDevice& device,
        ...
        if (resources.per_frame) {
            bind_engine_frame_uniform_data(ctx, resources.per_frame, resources.per_frame_size, shader);
            bound_any = true;
        }
        ...
        if (resources.textures) {
            for (uint32_t i = 0; i < resources.texture_count; ++i) {
                ...
                ctx.bind_texture_array_element(
                    texture.binding, texture.array_element, texture.texture, texture.sampler);
                bound_any = true;
            }
        }

        if (resources.shadow_block && resources.shadow_block_size > 0) {
            bound_any |= bind_shadow_block_for_shader(ctx, shader, resources.shadow_block, resources.shadow_block_size);
        }

        if (resources.lighting_ubo) {
            bound_any |= bind_lighting_ubo_for_shader(ctx, shader, resources.lighting_ubo);
        }
        ...
        if (resources.shadow_sampler) {
            bound_any |= bind_shadow_maps_for_shader(ctx,
                                                     shader,
    ...
    bool prepare_material_pipeline_resources(tgfx::RenderContext2& ctx,
        ...
        const size_t shadow_count = resources.max_shadow_maps == 0
                                        ? resources.shadow_maps.size()
                                        : std::min(resources.shadow_maps.size(), resources.max_shadow_maps);
        view.shadow_map_count = static_cast<uint32_t>(std::min<size_t>(shadow_count, UINT32_MAX));
        view.shadow_sampler = resources.shadow_sampler;

        bool bound_any = prepare_material_pipeline_resources(ctx, device, shader, phase, view);
