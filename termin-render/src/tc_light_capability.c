...
tc_component_cap_id tc_light_capability_id(void) {
    ...
    if (light_cap == TC_COMPONENT_CAPABILITY_INVALID_ID) {
        light_cap = tc_component_capability_register_with_destructor("light", tc_light_capability_destroy);
    }
    return light_cap;
}

bool tc_light_capability_attach(tc_component* c, const tc_light_vtable* vtable, void* userdata) {
    if (!c || !vtable)
        return false;

    tc_light_capability* cap = (tc_light_capability*)tc_component_get_capability(c, tc_light_capability_id());
    if (!cap) {
...
const tc_light_capability* tc_light_capability_get(const tc_component* c) {
    if (!c)
        return NULL;
    return (const tc_light_capability*)tc_component_get_capability(c, tc_light_capability_id());
}
