...
namespace termin {
    namespace {
        ...
        bool collect_drawable_items_callback(tc_component* component, void* user_data) {
            ...
            if (!component) {
                tc::Log::error("[%s] scene drawable iteration returned null component", safe_pass_name(*data->request));
                data->ok = false;
                return true;
            }

            if (data->request->phase != TC_PHASE_NONE &&
                !tc_phase_mask_contains(tc_component_phase_mask(component), data->request->phase)) {
                return true;
            }
    ...
    bool RenderSceneItemCollector::collect_into(const RenderSceneItemCollectRequest& request,
        ...
        tc_scene_foreach_drawable(
            request.scene, collect_drawable_items_callback, &data, request.scene_filter_flags, request.layer_mask);
