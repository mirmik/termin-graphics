...
namespace termin {
    ...
    bool resolve_render_camera(tc_component* component, double aspect_override, RenderCameraSnapshot& out) {
        ...
        tc_camera_data data{};
        if (!capability->vtable->get_camera_data(component, aspect_override, &data)) {
            return false;
        }

        std::memcpy(out.camera.view.data, data.view, sizeof(data.view));
        std::memcpy(out.camera.projection.data, data.projection, sizeof(data.projection));
        out.camera.position = Vec3(data.position[0], data.position[1], data.position[2]);
    ...
    NamedCameraResolveError resolve_named_render_camera(tc_scene_handle scene,
        ...
        const tc_entity_id entity_id = tc_scene_find_entity_by_name(scene, entity_name);
        if (!tc_entity_id_valid(entity_id)) {
            return NamedCameraResolveError::EntityNotFound;
        }

        tc_entity_pool* pool = tc_scene_entity_pool(scene);
        const size_t component_count = tc_entity_pool_component_count(pool, entity_id);
        for (size_t i = 0; i < component_count; ++i) {
            tc_component* component = tc_entity_pool_component_at(pool, entity_id, i);
    ...
    bool resolve_named_render_camera_for_pass(tc_scene_handle scene,
                                              const char* entity_name,
                                              double aspect_override,
                                              const char* pass_name,
                                              RenderCameraSnapshot& out) {
        const NamedCameraResolveError error = resolve_named_render_camera(scene, entity_name, aspect_override, out);
        if (error == NamedCameraResolveError::None) {
            return true;
        }
        tc::Log::error("[%s] cannot use named camera entity '%s': %s",
