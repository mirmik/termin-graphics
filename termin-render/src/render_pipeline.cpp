        ...
        try {
            ...
            for (uint32_t offset = 0; offset < definition->pass_count; ++offset) {
                ...
                if (!pass) {
                    ...
                    auto* unknown = pass ? dynamic_cast<UnknownPass*>(CxxFramePass::from_tc(pass)) : nullptr;
                    if (!unknown) {
                        throw std::runtime_error(std::string("cannot instantiate pass type '") + desc.type_name + "'");
                    }
                ...
                if (desc.viewport_name)
                    pass_ref.set_viewport_name(desc.viewport_name);
                if (desc.parameters && desc.parameters[0]) {
                    ...
                    if (!parameters.is_dict()) {
                        ...
                        throw std::runtime_error(std::string("parameters for pass '") + desc.name +
                                                 "' are not an object");
                    }
                    if (auto* unknown = dynamic_cast<UnknownPass*>(CxxFramePass::from_tc(pass))) {
                        ...
                        for (const auto& [field_name, value] : parameters.as_dict()) {
                            ...
                            if (!assigned) {
                                tc::Log::warn("RenderPipeline: failed to restore field '%s.%s'",
                                              desc.type_name,
                                              field_name.c_str());
                            }
                        }
                ...
                if (!tc_pipeline_adopt_pass(handle_, pass, pass->deleter)) {
                    tc_pass_delete_unowned(pass);
                    throw std::runtime_error(std::string("cannot adopt pass '") + desc.name + "'");
                }
            }
            ...
            for (uint32_t i = 0; i < definition->resource_count; ++i) {
                ...
                if (resource_type.starts_with("external"))
                    continue;
                ResourceSpec spec;
                ...
                if (desc.viewport_name)
                    spec.viewport_name = desc.viewport_name;
                if (desc.width > 0 && desc.height > 0)
                    spec.size = {desc.width, desc.height};
            ...
            for (uint32_t i = 0; i < definition->resource_view_count; ++i) {
                ...
                render_cache.resource_views.emplace(
                    ...
                    ResourceView{
                        desc.parent,
                        desc.attachment == TC_PIPELINE_ATTACHMENT_DEPTH ? AttachmentKind::Depth : AttachmentKind::Color,
                    });
            }
            for (uint32_t i = 0; i < definition->fbo_composition_count; ++i) {
                const tc_pipeline_template_fbo_composition_desc& desc = definition->fbo_compositions[i];
                render_cache.fbo_compositions.emplace(desc.name,
                                                      FboComposition{
    ...
    std::vector<ResourceSpec> RenderPipeline::collect_specs() const {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (pass && pass->enabled) {
                const size_t pass_spec_count = tc_pass_get_resource_specs(pass, nullptr, 0);
                std::vector<ResourceSpec> pass_specs(pass_spec_count);
                const size_t actual = tc_pass_get_resource_specs(pass, pass_specs.data(), pass_specs.size());
                if (actual != pass_spec_count) {
                    tc::Log::error("RenderPipeline::collect_specs: pass '%s' changed its resource "
