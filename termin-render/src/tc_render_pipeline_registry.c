...
static void clear_payload(tc_render_pipeline* pipeline) {
    if (!pipeline) return;
    ...
    pipeline->resources = NULL;
    pipeline->dependencies = NULL;
    pipeline->targets = NULL;
    pipeline->pass_count = 0;
    pipeline->resource_count = 0;
    pipeline->dependency_count = 0;
    pipeline->target_count = 0;
}

void tc_render_pipeline_init(void) {
    if (g_initialized) return;
    if (!tc_pool_init(&g_pool, sizeof(tc_render_pipeline), 32)) {
        tc_log_error("tc_render_pipeline_init: failed to initialize pool");
        return;
    }
    g_uuid_to_index = tc_resource_map_new(NULL);
    if (!g_uuid_to_index) {
        tc_log_error("tc_render_pipeline_init: failed to create UUID map");
        tc_pool_free(&g_pool);
        return;
    }
    g_initialized = true;
...
void tc_render_pipeline_shutdown(void) {
    if (!g_initialized) return;
    for (uint32_t i = 0; i < g_pool.capacity; ++i) {
        if (g_pool.states[i] == TC_SLOT_OCCUPIED) {
            clear_payload((tc_render_pipeline*)tc_pool_get_unchecked(&g_pool, i));
        }
    }
    tc_pool_free(&g_pool);
...
tc_render_pipeline_handle tc_render_pipeline_create(const char* uuid, const char* name) {
    if (!g_initialized) tc_render_pipeline_init();
    if (!g_initialized || !uuid || !uuid[0] || !name || !name[0]) {
        tc_log_error("tc_render_pipeline_create: UUID and name are required");
        return tc_render_pipeline_handle_invalid();
    }
    if (strlen(uuid) >= TC_UUID_SIZE) {
        tc_log_error("tc_render_pipeline_create: UUID '%s' is too long", uuid);
        return tc_render_pipeline_handle_invalid();
    }
    if (!tc_render_pipeline_handle_is_invalid(tc_render_pipeline_find(uuid))) {
        tc_log_warn("tc_render_pipeline_create: UUID '%s' already exists", uuid);
        return tc_render_pipeline_handle_invalid();
    }
    tc_handle raw = tc_pool_alloc(&g_pool);
    if (tc_handle_is_invalid(raw)) {
        tc_log_error("tc_render_pipeline_create: pool allocation failed");
        return tc_render_pipeline_handle_invalid();
    }
    tc_render_pipeline* pipeline = (tc_render_pipeline*)tc_pool_get(&g_pool, raw);
    memset(pipeline, 0, sizeof(*pipeline));
    tc_resource_header_init(&pipeline->header, uuid);
    pipeline->header.name = tc_intern_string(name);
    pipeline->header.is_loaded = 1;
    pipeline->header.pool_index = raw.index;
    pipeline->self_handle = raw;
    pipeline->descriptor_version = TC_RENDER_PIPELINE_DESCRIPTOR_VERSION;
    if (!tc_resource_map_add(g_uuid_to_index, uuid, tc_pack_index(raw.index))) {
        tc_log_error("tc_render_pipeline_create: failed to publish UUID '%s'", uuid);
...
tc_render_pipeline_handle tc_render_pipeline_find(const char* uuid) {
    if (!g_initialized || !uuid) return tc_render_pipeline_handle_invalid();
    void* value = tc_resource_map_get(g_uuid_to_index, uuid);
    if (!tc_has_index(value)) return tc_render_pipeline_handle_invalid();
    uint32_t index = tc_unpack_index(value);
    if (index >= g_pool.capacity || g_pool.states[index] != TC_SLOT_OCCUPIED) {
        return tc_render_pipeline_handle_invalid();
    }
    tc_render_pipeline_handle result = {index, g_pool.generations[index]};
    return result;
}

tc_render_pipeline_handle tc_render_pipeline_declare(const char* uuid, const char* name) {
    tc_render_pipeline_handle existing = tc_render_pipeline_find(uuid);
    return tc_render_pipeline_handle_is_invalid(existing)
        ? tc_render_pipeline_create(uuid, name) : existing;
}

tc_render_pipeline* tc_render_pipeline_get(tc_render_pipeline_handle handle) {
    if (!g_initialized || tc_render_pipeline_handle_is_invalid(handle)) return NULL;
    return (tc_render_pipeline*)tc_pool_get(&g_pool, handle);
}

bool tc_render_pipeline_is_valid(tc_render_pipeline_handle handle) {
    return g_initialized && tc_pool_is_valid(&g_pool, handle);
}

size_t tc_render_pipeline_count(void) {
    return g_initialized ? tc_pool_count(&g_pool) : 0;
}

static bool destroy_pipeline(tc_render_pipeline_handle handle) {
    tc_render_pipeline* pipeline = tc_render_pipeline_get(handle);
    if (!pipeline) return false;
    tc_resource_map_remove(g_uuid_to_index, pipeline->header.uuid);
    clear_payload(pipeline);
    return tc_pool_free_slot(&g_pool, handle);
}

void tc_render_pipeline_retain(tc_render_pipeline* pipeline) {
    if (pipeline) ++pipeline->header.ref_count;
}

bool tc_render_pipeline_release(tc_render_pipeline* pipeline) {
    if (!pipeline || pipeline->header.ref_count == 0) return false;
    --pipeline->header.ref_count;
    return pipeline->header.ref_count == 0 && destroy_pipeline(pipeline->self_handle);
}

bool tc_render_pipeline_remove(tc_render_pipeline_handle handle) {
    tc_render_pipeline* pipeline = tc_render_pipeline_get(handle);
    if (!pipeline) return false;
    if (pipeline->header.ref_count != 0) {
        tc_log_error(
            "tc_render_pipeline_remove: '%s' still has %u retained handle(s)",
            pipeline->header.uuid,
            pipeline->header.ref_count);
        return false;
    }
    return destroy_pipeline(handle);
}

static bool validate_payload(const tc_render_pipeline_payload_desc* desc) {
    if (!desc || desc->descriptor_version != TC_RENDER_PIPELINE_DESCRIPTOR_VERSION
        || !desc->name || !desc->name[0]) {
        tc_log_error("tc_render_pipeline_set_payload: invalid descriptor version or name");
        return false;
    }
    if ((desc->pass_count && !desc->passes)
        ...
        tc_log_error("tc_render_pipeline_set_payload: array pointer is NULL");
        return false;
    }
    for (uint32_t i = 0; i < desc->pass_count; ++i) {
        if (!desc->passes[i].type_name || !desc->passes[i].type_name[0]
            ...
            tc_log_error("tc_render_pipeline_set_payload: pass %u lacks type or name", i);
            return false;
        }
    }
    for (uint32_t i = 0; i < desc->resource_count; ++i) {
        if (!desc->resources[i].name || !desc->resources[i].name[0]
            ...
            tc_log_error("tc_render_pipeline_set_payload: resource %u lacks name or type", i);
            return false;
        }
    }
    for (uint32_t i = 0; i < desc->dependency_count; ++i) {
        if (desc->dependencies[i].pass_index >= desc->pass_count
            ...
            tc_log_error("tc_render_pipeline_set_payload: invalid dependency %u", i);
            return false;
        }
    }
    return true;
...
bool tc_render_pipeline_set_payload(
    tc_render_pipeline* pipeline,
    const tc_render_pipeline_payload_desc* desc
) {
    if (!pipeline || !validate_payload(desc)) return false;
    tc_render_pipeline_pass_desc* passes = NULL;
    tc_render_pipeline_resource_desc* resources = NULL;
    tc_render_pipeline_dependency_desc* dependencies = NULL;
    tc_render_pipeline_target_desc* targets = NULL;
    ...
    clear_payload(pipeline);
    pipeline->header.name = tc_intern_string(desc->name);
    pipeline->descriptor_version = desc->descriptor_version;
    pipeline->passes = passes;
    pipeline->pass_count = desc->pass_count;
    pipeline->resources = resources;
    pipeline->resource_count = desc->resource_count;
    pipeline->dependencies = dependencies;
    pipeline->dependency_count = desc->dependency_count;
    pipeline->targets = targets;
    pipeline->target_count = desc->target_count;
    tc_resource_header_bump_version(&pipeline->header);
    return true;

allocation_failed:
    tc_log_error("tc_render_pipeline_set_payload: allocation failed");
    free(passes);
    free(resources);
    free(dependencies);
    free(targets);
...
uint32_t tc_render_pipeline_version(const tc_render_pipeline* pipeline) {
    return pipeline ? pipeline->header.version : 0;
}
...
size_t tc_render_pipeline_serialize(
    ...
    if (output) {
        const size_t required = tc_render_pipeline_serialize(pipeline, NULL, 0);
        if (capacity < required) return required;
    }
    byte_writer writer = {output, capacity, 0, true};
    const uint8_t magic[4] = {'T', 'R', 'P', 'L'};
    write_bytes(&writer, magic, sizeof(magic));
    write_u32(&writer, TC_RENDER_PIPELINE_BINARY_VERSION);
    write_u32(&writer, pipeline->descriptor_version);
    write_string(&writer, pipeline->header.name);
    write_u32(&writer, pipeline->pass_count);
    write_u32(&writer, pipeline->resource_count);
    write_u32(&writer, pipeline->dependency_count);
    write_u32(&writer, pipeline->target_count);
    for (uint32_t i = 0; i < pipeline->pass_count; ++i) {
        write_string(&writer, pipeline->passes[i].type_name);
        write_string(&writer, pipeline->passes[i].name);
        write_string(&writer, pipeline->passes[i].parameters);
        write_string(&writer, pipeline->passes[i].viewport_name);
    }
    for (uint32_t i = 0; i < pipeline->resource_count; ++i) {
        const tc_render_pipeline_resource_desc* value = &pipeline->resources[i];
    ...
    for (uint32_t i = 0; i < pipeline->dependency_count; ++i) {
        write_u32(&writer, pipeline->dependencies[i].pass_index);
        write_string(&writer, pipeline->dependencies[i].resource);
        write_u32(&writer, (uint32_t)pipeline->dependencies[i].access);
    }
    for (uint32_t i = 0; i < pipeline->target_count; ++i) {
        write_string(&writer, pipeline->targets[i].viewport_name);
        write_string(&writer, pipeline->targets[i].export_name);
        write_i32(&writer, pipeline->targets[i].width);
        write_i32(&writer, pipeline->targets[i].height);
    }
...
static void free_decoded_payload(tc_render_pipeline_payload_desc* desc) {
    if (!desc) return;
    free((void*)desc->name);
...
tc_render_pipeline_handle tc_render_pipeline_deserialize(
    const char* uuid,
    const uint8_t* data,
    size_t size
) {
    tc_render_pipeline_payload_desc desc;
    memset(&desc, 0, sizeof(desc));
    if (!uuid || !uuid[0] || !data) {
        tc_log_error("tc_render_pipeline_deserialize: UUID and data are required");
        return tc_render_pipeline_handle_invalid();
    }
    ...
    if (!reader.valid || memcmp(magic, "TRPL", 4) != 0
        || binary_version != TC_RENDER_PIPELINE_BINARY_VERSION
        || desc.descriptor_version != TC_RENDER_PIPELINE_DESCRIPTOR_VERSION
        || desc.pass_count > 65536 || desc.resource_count > 65536
        || desc.dependency_count > 262144 || desc.target_count > 65536) {
        tc_log_error("tc_render_pipeline_deserialize: invalid header");
        free_decoded_payload(&desc);
        return tc_render_pipeline_handle_invalid();
    }
    desc.passes = (tc_render_pipeline_pass_desc*)calloc(desc.pass_count, sizeof(*desc.passes));
    desc.resources = (tc_render_pipeline_resource_desc*)calloc(desc.resource_count, sizeof(*desc.resources));
    desc.dependencies = (tc_render_pipeline_dependency_desc*)calloc(desc.dependency_count, sizeof(*desc.dependencies));
    desc.targets = (tc_render_pipeline_target_desc*)calloc(desc.target_count, sizeof(*desc.targets));
    ...
    for (uint32_t i = 0; reader.valid && i < desc.pass_count; ++i) {
        tc_render_pipeline_pass_desc* value = (tc_render_pipeline_pass_desc*)&desc.passes[i];
    ...
    for (uint32_t i = 0; reader.valid && i < desc.resource_count; ++i) {
        tc_render_pipeline_resource_desc* value = (tc_render_pipeline_resource_desc*)&desc.resources[i];
        value->name = read_string(&reader);
    ...
    for (uint32_t i = 0; reader.valid && i < desc.dependency_count; ++i) {
        tc_render_pipeline_dependency_desc* value = (tc_render_pipeline_dependency_desc*)&desc.dependencies[i];
    ...
    for (uint32_t i = 0; reader.valid && i < desc.target_count; ++i) {
        tc_render_pipeline_target_desc* value = (tc_render_pipeline_target_desc*)&desc.targets[i];
        value->viewport_name = read_string(&reader);
    ...
    if (!reader.valid || reader.offset != reader.size) {
        tc_log_error("tc_render_pipeline_deserialize: truncated or trailing data");
        free_decoded_payload(&desc);
        return tc_render_pipeline_handle_invalid();
    }
    tc_render_pipeline_handle handle = tc_render_pipeline_create(uuid, desc.name);
    tc_render_pipeline* pipeline = tc_render_pipeline_get(handle);
    if (!pipeline || !tc_render_pipeline_set_payload(pipeline, &desc)) {
        if (pipeline) tc_render_pipeline_remove(handle);
        handle = tc_render_pipeline_handle_invalid();
    }
    free_decoded_payload(&desc);
    return handle;
}
