...
namespace termin {
    ...
    namespace {
        ...
        static void append_shader_usage(TcShader shader, ShaderUsageCollectContext* ctx) {
            ...
            if (!uuid || uuid[0] == '\0') {
                tc::Log::warn("%s: shader '%s' has empty uuid", ctx->log_context, shader.name());
                return;
            }

            if (!shader.fragment_source() || shader.fragment_source()[0] == '\0') {
                tc::Log::warn("%s: shader '%s' [%s] has no fragment source", ctx->log_context, shader.name(), uuid);
                return;
            }
    ...
    std::vector<TcShader> collect_scene_shader_usages(tc_scene_handle scene) {
        ...
        tc::Log::error("collect_scene_shader_usages: scene-only drawable shader collection was removed; "
                       "use collect_shader_usages_for_pipeline(scene, pipeline) so passes provide RenderItem phases "
                       "and shader contracts");
        return shaders;
    }

    std::vector<TcShader> collect_shader_usages_for_pipeline(tc_scene_handle scene, const RenderPipeline& pipeline) {
        std::vector<TcShader> shaders;
        ...
        for (size_t i = 0; i < pass_count; ++i) {
            ...
            if (!cxx_pass) {
                tc::Log::warn("collect_shader_usages_for_pipeline: skipping non-C++ pass '%s'",
                              tc_pass_type_name(pass));
                continue;
            }

            const auto* scene_provider = dynamic_cast<const SceneShaderUsageProvider*>(cxx_pass);
            if (!scene_provider) {
                continue;
            }
            scene_provider->collect_scene_shader_usages(scene,
                                                        [&](TcShader shader) { append_shader_usage(shader, &ctx); });
        }
