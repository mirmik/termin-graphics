...
namespace termin {
    namespace {
        ...
        bool same_resource_identity(const MaterialPipelineResourceDecl& a, const MaterialPipelineResourceDecl& b) {
            return a.requirement.name == b.requirement.name && a.requirement.kind == b.requirement.kind &&
                   a.requirement.scope == b.requirement.scope;
        }

        bool same_resource_name_different_contract(const MaterialPipelineResourceDecl& a,
        ...
        void add_diagnostic(std::vector<MaterialPipelineDiagnostic>& diagnostics,
            ...
            diagnostic.message = std::string(reason) + ": existing " + resource_label(existing) + " vs incoming " +
                                 resource_label(incoming);
            diagnostics.push_back(std::move(diagnostic));
        }

        MaterialPipelineResourceDecl abi_expected_resource_decl(const ShaderAbiResourceDecl& abi,
                                                                const MaterialPipelineResourceDecl& incoming) {
        ...
        bool canonicalize_and_validate_abi_resource(MaterialPipelineResourceDecl& resource,
                                                    std::vector<MaterialPipelineDiagnostic>& diagnostics) {
            const ShaderAbiResourceDecl* abi = find_shader_abi_resource(resource.requirement.name);
            if (!abi) {
                return true;
            }

            if (resource.requirement.kind != abi->kind || resource.requirement.scope != abi->scope) {
    ...
    const char* material_pipeline_resource_owner_name(MaterialPipelineResourceOwner owner) {
        switch (owner) {
    ...
    const char* material_pipeline_diagnostic_code_name(MaterialPipelineDiagnosticCode code) {
        switch (code) {
    ...
    MaterialPipelineResourceDecl material_pipeline_abi_resource_decl(ShaderAbiResourceId id,
                                                                     uint32_t stage_mask,
    ...
    bool material_pipeline_merge_resource(std::vector<MaterialPipelineResourceDecl>& resources,
    ...
    MaterialPipelineResourceMergeResult
    material_pipeline_merge_resources(std::span<const MaterialPipelineResourceDecl> resources) {
        ...
        for (const MaterialPipelineResourceDecl& resource : resources) {
            material_pipeline_merge_resource(result.resources, resource, result.diagnostics);
        }
        return result;
    }
