...
tc_component_cap_id tc_camera_capability_id(void) {
    ...
    if (camera_cap == TC_COMPONENT_CAPABILITY_INVALID_ID) {
        camera_cap = tc_component_capability_register_with_destructor("camera", tc_camera_capability_destroy);
    }
    return camera_cap;
}

bool tc_camera_capability_attach(tc_component* c, const tc_camera_vtable* vtable, void* userdata) {
    if (!c || !vtable)
        return false;

    tc_camera_capability* cap = (tc_camera_capability*)tc_component_get_capability(c, tc_camera_capability_id());
    if (!cap) {
...
const tc_camera_capability* tc_camera_capability_get(const tc_component* c) {
    if (!c)
        return NULL;
    return (const tc_camera_capability*)tc_component_get_capability(c, tc_camera_capability_id());
}
