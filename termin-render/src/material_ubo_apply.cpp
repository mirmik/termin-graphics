...
namespace termin {
    ...
    namespace {
        ...
        void
        log_missing_material_resource_once(const tc_shader* shader, const char* resource_name, const char* reason) {
            ...
            tc::Log::error("[MaterialPipeline] shader '%s' is missing reflected material resource '%s': %s",
        ...
        const tc_uniform_value* find_phase_uniform(const tc_material_phase* phase, const char* name) {
        ...
        bool field_range_is_valid(const tc_shader* shader,
            ...
            tc::Log::error("Material UBO field '%s' in shader '%s' exceeds block size "
    ...
    pack_material_uniform_value_to_std140_field(const tc_uniform_value& uniform, const char* field_type, uint8_t* dst) {
        if (!field_type || !dst) {
            tc::Log::error("Cannot pack material uniform '%s': field type or destination is null",
                           uniform.name[0] != '\0' ? uniform.name : "<unnamed>");
            return false;
        }
        const auto incompatible_type = [&]() {
            tc::Log::error("Cannot pack material uniform '%s': uniform type %s is incompatible with "
    ...
    namespace {
        ...
        bool pack_material_ubo_entry(const tc_material_phase* phase,
                                     ...
                                     uint32_t block_size,
                                     uint8_t* out_buffer) {
            if (!field_range_is_valid(shader, field_name, field_type, field_offset, block_size)) {
                return false;
            }
            ...
            return pack_material_uniform_value_to_std140_field(*uniform, field_type, out_buffer + field_offset);
        }

        bool pack_material_ubo_from_legacy_entries(const tc_material_phase* phase,
                                                   const tc_shader* shader,
                                                   uint8_t* out_buffer,
                                                   uint32_t block_size) {
            ...
            for (uint32_t i = 0; i < shader->material_ubo_entry_count; ++i) {
                ...
                if (!pack_material_ubo_entry(
                        phase, shader, entry.name, entry.property_type, entry.offset, block_size, out_buffer)) {
                    return false;
                }
            }
            return true;
        ...
        bool validate_reflected_material_fields(const tc_shader* shader,
            ...
            for (uint32_t i = 0; i < material_rb->field_count; ++i) {
                ...
                if (field.type[0] == '\0') {
                    tc::Log::error("Material UBO field '%s' in shader '%s' has no reflected type",
                                   field.name,
                                   shader_debug_name(shader));
                    return false;
                }
        ...
        bool pack_material_ubo_from_reflected_fields(const tc_material_phase* phase,
                                                     const tc_shader* shader,
                                                     const tc_shader_resource_binding* material_rb,
                                                     uint8_t* out_buffer,
                                                     uint32_t block_size) {
            ...
            for (uint32_t i = 0; i < material_rb->field_count; ++i) {
                const tc_shader_resource_field& field = material_rb->fields[i];
                if (!pack_material_ubo_entry(
                        phase, shader, field.name, field.type, field.offset, block_size, out_buffer)) {
                    return false;
                }
            }
            return true;
    ...
    bool apply_material_phase_ubo(tc_material_phase* phase,
        ...
        } else if (phase->uniform_count > 0 && material_rb) {
