...
namespace termin {
    namespace {
        ...
        ShaderAbiResourceDecl make_cpp_decl(const tc_shader_abi_resource_decl& decl) {
            return {
    ...
    const ShaderAbiResourceDecl& shader_abi_resource(ShaderAbiResourceId id) {
        ...
        tc::Log::error("[ShaderABI] unknown shader ABI resource id %u", static_cast<unsigned>(id));
        std::abort();
    }

    bool shader_abi_name_is_legacy_alias(const ShaderAbiResourceDecl& decl, std::string_view name) {
    ...
    bool shader_abi_name_matches(const ShaderAbiResourceDecl& decl, std::string_view name) {
        return decl.canonical_name == name || shader_abi_name_is_legacy_alias(decl, name);
    }

    const ShaderAbiResourceDecl* find_shader_abi_resource(std::string_view name) {
        const tc_shader_abi_resource_decl* c_decl = tc_shader_abi_find_resource(std::string(name).c_str());
        if (!c_decl) {
            return nullptr;
        }
    ...
    bool shader_abi_binding_matches(const ShaderAbiResourceDecl& decl, const tc_shader_resource_binding& binding) {
        return shader_abi_name_matches(decl, binding.name) && binding.kind == decl.kind && binding.scope == decl.scope;
    }

    const tc_shader_resource_binding* find_shader_abi_resource_binding(const tc_shader* shader,
                                                                       ShaderAbiResourceId id) {
        if (!shader) {
            return nullptr;
        }
        const ShaderAbiResourceDecl& decl = shader_abi_resource(id);
        const tc_shader_resource_binding* rb = tc_shader_abi_find_resource_binding(shader, shader_abi_c_id(id));
        if (rb) {
            return rb;
        }
        const tc_shader_abi_resource_decl* c_decl = tc_shader_abi_resource(shader_abi_c_id(id));
        if (c_decl && tc_shader_abi_find_resource_binding(shader, c_decl->id) == nullptr) {
            const tc_shader_resource_binding* invalid = tc_shader_find_resource_binding(shader, c_decl->canonical_name);
            ...
            if (invalid) {
                tc::Log::error("[ShaderABI] shader '%s' declares ABI resource '%s' with "
