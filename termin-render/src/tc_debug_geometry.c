...
static debug_type_entry* find_type(tc_debug_geometry_type_id type_id) {
    ...
    for (size_t i = 0; i < g_type_count; ++i) {
        if (g_types[i].desc.type_id == type_id)
            return &g_types[i];
    }
    return NULL;
}
...
static bool ensure_type_capacity(size_t needed) {
    if (g_type_capacity >= needed)
        return true;
    size_t capacity = g_type_capacity == 0 ? 8 : g_type_capacity * 2;
    while (capacity < needed)
        capacity *= 2;
    debug_type_entry* replacement = (debug_type_entry*)realloc(g_types, capacity * sizeof(debug_type_entry));
    if (!replacement) {
...
tc_debug_geometry_type_id tc_debug_geometry_type_register(const char* stable_id,
...
bool tc_debug_geometry_type_unregister(tc_debug_geometry_type_id type_id) {
    for (size_t i = 0; i < g_type_count; ++i) {
        ...
        if (i + 1 < g_type_count)
            g_types[i] = g_types[g_type_count - 1];
        g_type_count--;
        return true;
    }
...
tc_debug_geometry_type_id tc_debug_geometry_type_find(const char* stable_id) {
    ...
    return entry && strcmp(entry->desc.stable_id, stable_id) == 0 ? type_id : TC_DEBUG_GEOMETRY_TYPE_INVALID;
}
...
bool tc_debug_geometry_type_at(size_t index, tc_debug_geometry_type_desc* out_desc) {
    if (!out_desc || index >= g_type_count)
        return false;
...
static tc_debug_geometry_setting* find_setting(tc_scene_render_mount* mount, tc_debug_geometry_type_id type_id) {
    if (!mount)
        return NULL;
...
bool tc_scene_debug_geometry_enabled(tc_scene_handle scene, tc_debug_geometry_type_id type_id) {
...
bool tc_scene_debug_geometry_set_enabled(tc_scene_handle scene, tc_debug_geometry_type_id type_id, bool enabled) {
    ...
    if (mount->debug_geometry_setting_count == mount->debug_geometry_setting_capacity) {
        size_t capacity = mount->debug_geometry_setting_capacity == 0 ? 8 : mount->debug_geometry_setting_capacity * 2;
        tc_debug_geometry_setting* replacement = (tc_debug_geometry_setting*)realloc(
            mount->debug_geometry_settings, capacity * sizeof(tc_debug_geometry_setting));
        if (!replacement) {
...
bool tc_debug_geometry_drawer_valid(const tc_debug_geometry_drawer* drawer) {
    if (!drawer || !find_type(drawer->type_id))
        return false;
...
static tc_debug_geometry_primitive* append_primitive(const tc_debug_geometry_drawer* drawer) {
    if (!tc_debug_geometry_drawer_valid(drawer))
        return NULL;
    tc_scene_render_mount* mount = tc_scene_render_mount_get(drawer->scene);
    if (mount->debug_geometry_primitive_count == mount->debug_geometry_primitive_capacity) {
        size_t capacity =
            mount->debug_geometry_primitive_capacity == 0 ? 64 : mount->debug_geometry_primitive_capacity * 2;
        tc_debug_geometry_primitive* replacement = (tc_debug_geometry_primitive*)realloc(
            mount->debug_geometry_primitives, capacity * sizeof(tc_debug_geometry_primitive));
        if (!replacement) {
    ...
    tc_debug_geometry_primitive* result = &mount->debug_geometry_primitives[mount->debug_geometry_primitive_count++];
    *result = (tc_debug_geometry_primitive){0};
    result->type_id = drawer->type_id;
    return result;
}

bool tc_debug_geometry_drawer_line(const tc_debug_geometry_drawer* drawer,
                                   const float start[3],
                                   const float end[3],
                                   const float color[4],
                                   bool depth_test) {
    if (!finite_vector(start, 3) || !finite_vector(end, 3) || !finite_vector(color, 4))
        return false;
...
bool tc_debug_geometry_drawer_wire_sphere(const tc_debug_geometry_drawer* drawer,
                                          const float center[3],
                                          float radius,
                                          const float color[4],
                                          uint16_t segments,
                                          bool depth_test) {
    if (!finite_vector(center, 3) || !finite_vector(color, 4) || !isfinite(radius) || radius <= 0.0f)
        return false;
    tc_debug_geometry_primitive* primitive = append_primitive(drawer);
    if (!primitive)
        return false;
...
bool tc_debug_geometry_drawer_wire_box(const tc_debug_geometry_drawer* drawer,
                                       const float center[3],
                                       ...
                                       const float color[4],
                                       bool depth_test) {
    if (!finite_vector(center, 3) || !finite_vector(half_axis_x, 3) || !finite_vector(half_axis_y, 3) ||
        !finite_vector(half_axis_z, 3) || !finite_vector(color, 4))
        return false;
    tc_debug_geometry_primitive* primitive = append_primitive(drawer);
    if (!primitive)
        return false;
    ...
    memcpy(primitive->data.box.half_axis_x, half_axis_x, sizeof(primitive->data.box.half_axis_x));
    memcpy(primitive->data.box.half_axis_y, half_axis_y, sizeof(primitive->data.box.half_axis_y));
    memcpy(primitive->data.box.half_axis_z, half_axis_z, sizeof(primitive->data.box.half_axis_z));
    return true;
}

bool tc_debug_geometry_drawer_wire_capsule(const tc_debug_geometry_drawer* drawer,
                                           const float start[3],
                                           const float end[3],
                                           float radius,
                                           const float color[4],
                                           uint16_t segments,
                                           bool depth_test) {
    if (!finite_vector(start, 3) || !finite_vector(end, 3) || !finite_vector(color, 4) || !isfinite(radius) ||
        radius <= 0.0f) {
        return false;
    }
    tc_debug_geometry_primitive* primitive = append_primitive(drawer);
    ...
    memcpy(primitive->color, color, sizeof(primitive->color));
    memcpy(primitive->data.capsule.start, start, sizeof(primitive->data.capsule.start));
    memcpy(primitive->data.capsule.end, end, sizeof(primitive->data.capsule.end));
...
const tc_debug_geometry_primitive* tc_scene_debug_geometry_primitive_at(tc_scene_handle scene, size_t index) {
    tc_scene_render_mount* mount = tc_scene_render_mount_get(scene);
    if (!mount || index >= mount->debug_geometry_primitive_count)
        return NULL;
    const tc_debug_geometry_primitive* primitive = &mount->debug_geometry_primitives[index];
