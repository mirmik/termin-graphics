...
namespace termin {
    ...
    FramePassTypeDescriptorBuilder::FramePassTypeDescriptorBuilder(const char* type_name,
                                                                   const char* owner,
                                                                   const char* parent,
        ...
        if (!_descriptor || !tc_pass_type_descriptor_add_facet(_descriptor, &factory, kind)) {
    ...
    FramePassTypeDescriptorBuilder::FramePassTypeDescriptorBuilder(FramePassTypeDescriptorBuilder&& other) noexcept
    ...
    bool FramePassTypeDescriptorBuilder::commit() {
        ...
        if (!_inspect.attach_to(_descriptor)) {
            tc_log(
                TC_LOG_ERROR, "[FramePassTypeDescriptor] failed to stage inspect facet for '%s'", _type_name.c_str());
            return false;
        }
