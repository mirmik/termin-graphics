...
bool DebugGeometryDrawer::wire_box(
    const Vec3& center,
    ...
    return tc_debug_geometry_drawer_wire_box(
        &drawer_, center_data, axis_x_data, axis_y_data, axis_z_data,
        color_data, depth_test);
}
...
bool DebugGeometryDrawer::wire_capsule(
    ...
    const float color_data[4] = {color.r, color.g, color.b, color.a};
    const uint16_t segment_count = static_cast<uint16_t>(
        std::clamp(segments, 4, 65535));
    return tc_debug_geometry_drawer_wire_capsule(
        &drawer_, start_data, end_data, static_cast<float>(radius),
        color_data, segment_count, depth_test);
}
