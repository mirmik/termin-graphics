...
namespace termin {
    namespace {
        ...
        MaterialPipelineSemantic semantic(std::string name, MaterialPipelineValueType type) {
            return {std::move(name), type};
        }

        MaterialPipelineResourceDecl
        resource(std::string name, uint32_t kind, uint32_t scope, uint32_t stage_mask, uint32_t size = 0) {
        ...
        void append_bone_block(VertexTransformContract& contract) {
            contract.resources.push_back(
                material_pipeline_abi_resource_decl(ShaderAbiResourceId::BoneBlock,
        ...
        MaterialFragmentInterface world_position_interface() {
            ...
            interface.semantics.push_back(semantic("world_pos", MaterialPipelineValueType::Float3));
            return interface;
        }

        MaterialFragmentInterface world_position_normal_interface() {
            MaterialFragmentInterface interface = world_position_interface();
            interface.semantics.push_back(semantic("normal_world", MaterialPipelineValueType::Float3));
            return interface;
        }

        VertexTransformProvider make_mesh_vertex_transform_provider(VertexTransformKind kind,
            ...
            provider.source_module = {"termin_vertex_transform", "builtin_shaders/termin_vertex_transform.slang"};

            const bool skinned = kind == VertexTransformKind::SkinnedMesh;
            switch (profile) {
            case MeshVertexTransformProfile::Material:
                provider.vertex_inputs = skinned ? material_pipeline_skinned_material_mesh_input()
...
struct VertexInput {
    ...
    float4 tangent : TANGENT;
                ...
                provider.adapter_input_expression =
                    skinned ? "termin_skinned_world_vertex(input.position, input.normal, input.uv, input.tangent, "
                              "input.joints, input.weights, " +
                                  model_expression + ", bone_block)"
                            : "termin_static_world_vertex(input.position, input.normal, input.uv, input.tangent, " +
                                  model_expression + ")";
                break;
            case MeshVertexTransformProfile::Position:
                provider.vertex_inputs =
                    skinned ? material_pipeline_skinned_position_mesh_input() : material_pipeline_position_mesh_input();
...
struct VertexInput {
    float3 position : POSITION;
                ...
                provider.adapter_input_expression =
                    skinned ? "termin_skinned_world_position(input.position, input.joints, input.weights, " +
                                  model_expression + ", bone_block)"
                            : "termin_static_world_position(input.position, " + model_expression + ")";
                break;
            case MeshVertexTransformProfile::PositionNormal:
                provider.vertex_inputs = skinned ? material_pipeline_skinned_position_normal_mesh_input()
...
struct VertexInput {
    ...
    float3 normal : NORMAL;
                ...
                provider.adapter_input_expression =
                    skinned
                        ? "termin_skinned_world_position_normal(input.position, input.normal, input.joints, "
                          "input.weights, " +
                              model_expression + ", bone_block)"
    ...
    bool vertex_transform_provider_is_modular(const VertexTransformProvider& provider) {
        return !provider.source_module.module_name.empty() && !provider.source_module.source_identity.empty() &&
               !provider.entry_input_declaration.empty() && !provider.adapter_input_expression.empty();
    }
    ...
    VertexInputContract material_pipeline_position_normal_mesh_input() {
        ...
        input.mesh_attributes.push_back(semantic("normal", MaterialPipelineValueType::Float3));
        return input;
    }
    ...
    VertexInputContract material_pipeline_skinned_material_mesh_input() {
        ...
        input.mesh_attributes.push_back(semantic("joints", MaterialPipelineValueType::Float4));
        input.mesh_attributes.push_back(semantic("weights", MaterialPipelineValueType::Float4));
        return input;
    }
    ...
    MaterialPipelineResourceDecl
    material_pipeline_draw_resource_decl(std::string name, uint32_t stage_mask, uint32_t size) {
        return resource(
            std::move(name), TC_SHADER_RESOURCE_CONSTANT_BUFFER, TC_SHADER_RESOURCE_SCOPE_DRAW, stage_mask, size);
    }

    std::vector<MaterialPipelineResourceDecl> material_pipeline_common_vertex_resources(std::string draw_resource_name,
        ...
        resources.push_back(material_pipeline_abi_resource_decl(
            ShaderAbiResourceId::PerFrame, TC_SHADER_STAGE_VERTEX, MaterialPipelineResourceOwner::VertexTransform));
        resources.push_back(material_pipeline_draw_resource_decl(
            std::move(draw_resource_name), TC_SHADER_STAGE_VERTEX, draw_resource_size));
        return resources;
    }

    std::vector<MaterialPipelineResourceDecl> material_pipeline_pass_vertex_resources(std::string draw_resource_name,
                                                                                      uint32_t draw_resource_size) {
        std::vector<MaterialPipelineResourceDecl> resources;
        resources.push_back(material_pipeline_abi_resource_decl(
    ...
    std::vector<MaterialPipelineResourceDecl> material_pipeline_foliage_vertex_resources() {
        ...
        resources.push_back(material_pipeline_abi_resource_decl(
            ShaderAbiResourceId::PerFrame, TC_SHADER_STAGE_VERTEX, MaterialPipelineResourceOwner::VertexTransform));
        resources.push_back(resource("foliage_draw",
    ...
    VertexTransformProvider material_pipeline_make_static_mesh_vertex_transform_provider(
        std::string debug_name, MeshVertexTransformProfile profile, std::string model_expression) {
        return make_mesh_vertex_transform_provider(
            VertexTransformKind::StaticMesh, std::move(debug_name), profile, std::move(model_expression));
    }

    VertexTransformProvider material_pipeline_make_skinned_mesh_vertex_transform_provider(
        std::string debug_name, MeshVertexTransformProfile profile, std::string model_expression) {
        return make_mesh_vertex_transform_provider(
            VertexTransformKind::SkinnedMesh, std::move(debug_name), profile, std::move(model_expression));
    }
    ...
    VertexOutputAdapter material_pipeline_standard_material_vertex_output_adapter() {
        ...
        adapter.source_module = {"termin_material_vertex_output_adapter",
        ...
        adapter.consumed_world_semantics = material_pipeline_standard_material_fragment_interface();
        adapter.produced_output_semantics = material_pipeline_standard_material_fragment_interface();
    ...
    VertexOutputAdapter material_pipeline_multiview_material_vertex_output_adapter() {
        ...
        adapter.source_module = {"termin_multiview_material_vertex_output_adapter",
    ...
    VertexTransformProvider material_pipeline_make_foliage_material_vertex_transform_provider(std::string debug_name) {
        return material_pipeline_make_foliage_vertex_transform_provider(std::move(debug_name),
                                                                        MeshVertexTransformProfile::Material);
    }

    VertexTransformProvider
    material_pipeline_make_foliage_vertex_transform_provider(std::string debug_name,
                                                             MeshVertexTransformProfile profile) {
        VertexTransformProvider provider;
        ...
        provider.source_module = {"termin_foliage_material_transform",
...
struct VertexInput {
    ...
    uint instance_id : SV_InstanceID;
            ...
            provider.adapter_input_expression = "termin_foliage_material_world_vertex("
...
struct VertexInput {
    ...
    uint instance_id : SV_InstanceID;
            ...
            provider.adapter_input_expression = "termin_foliage_world_position(input.position, input.instance_id)";
            break;
        case MeshVertexTransformProfile::PositionNormal:
            provider.entry_input_declaration = R"(
struct VertexInput {
    ...
    uint instance_id : SV_InstanceID;
            ...
            provider.adapter_input_expression = "termin_foliage_world_position_normal("
    ...
    bool material_pipeline_interface_produces(const MaterialFragmentInterface& interface,
                                              const std::string& semantic_name,
                                              MaterialPipelineValueType type) {
        return std::any_of(
            interface.semantics.begin(), interface.semantics.end(), [&](const MaterialPipelineSemantic& semantic) {
