...
DebugGeometryDrawer RenderPrepareContext::debug_geometry(
    ...
    tc_render_prepare_context_debug_geometry(
        reinterpret_cast<const tc_render_prepare_context*>(this),
        type_id,
        &drawer);
...
extern "C" bool tc_render_prepare_context_debug_geometry(
    const tc_render_prepare_context* context,
    tc_debug_geometry_type_id type_id,
    tc_debug_geometry_drawer* out_drawer
) {
    if (!context || !out_drawer ||
        !tc_debug_geometry_type_registered(type_id)) return false;
    const auto* typed =
