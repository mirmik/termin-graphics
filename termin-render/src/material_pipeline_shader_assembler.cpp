...
namespace termin {
    namespace {
        ...
        MaterialPipelineDiagnostic diagnostic(MaterialPipelineDiagnosticCode code, std::string message) {
        ...
        MaterialPipelineResourceDecl resource_decl_from_binding(const tc_shader_resource_binding& binding,
        ...
        MaterialPipelineResourceDecl resource_decl_from_requirement(const tc_shader_resource_requirement& requirement,
                                                                    MaterialPipelineResourceOwner owner) {
            MaterialPipelineResourceDecl result{};
        ...
        void append_resources(std::vector<MaterialPipelineResourceDecl>& out,
                              const std::vector<MaterialPipelineResourceDecl>& resources) {
            out.insert(out.end(), resources.begin(), resources.end());
        }

        bool validate_fragment_interface(const MaterialFragmentInterface& fragment_input,
                                         const char* input_owner,
                                         const VertexTransformContract& vertex_transform,
                                         std::vector<MaterialPipelineDiagnostic>& diagnostics) {
            bool ok = true;
            for (const MaterialPipelineSemantic& semantic : fragment_input.semantics) {
                if (!material_pipeline_interface_produces(
                        vertex_transform.produced_fragment_input, semantic.name, semantic.type)) {
                    ok = false;
                    diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::MissingVertexOutputSemantic,
        ...
        bool validate_adapter_interface(const VertexTransformProvider& provider,
                                        const VertexOutputAdapter& adapter,
                                        std::vector<MaterialPipelineDiagnostic>& diagnostics) {
            bool ok = true;
            for (const MaterialPipelineSemantic& semantic : adapter.consumed_world_semantics.semantics) {
                if (!material_pipeline_interface_produces(
                        provider.produced_world_semantics, semantic.name, semantic.type)) {
                    ok = false;
                    diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::MissingVertexOutputSemantic,
                                                     "vertex transform provider '" + provider.debug_name +
                                                         "' does not produce adapter semantic '" + semantic.name +
                                                         "' required by '" + adapter.debug_name + "'"));
                }
            }
            return ok;
        }

        std::string compose_modular_vertex_source(const VertexTransformProvider& provider,
                                                  const VertexOutputAdapter& adapter,
                                                  const std::string& entry_point) {
            std::string source;
            source.reserve(provider.source_module.module_name.size() + adapter.source_module.module_name.size() +
                           provider.entry_input_declaration.size() + provider.adapter_input_expression.size() + 320u);
        ...
        bool append_contract_inputs(const VertexTransformContract& vertex_transform,
                                    std::vector<tc_shader_contract_vertex_input>& out) {
            out.reserve(vertex_transform.vertex_inputs.mesh_attributes.size());
            for (const MaterialPipelineSemantic& semantic : vertex_transform.vertex_inputs.mesh_attributes) {
                tc_shader_contract_vertex_input input{};
                std::snprintf(input.semantic, sizeof(input.semantic), "%s", semantic.name.c_str());
        ...
        void apply_instance_stream_strides(const VertexTransformContract& vertex_transform,
            ...
            for (const InstanceStreamDecl& stream : vertex_transform.instance_streams) {
                for (tc_shader_resource_requirement& resource : resources) {
                    if (std::strncmp(resource.name, stream.name.c_str(), TC_SHADER_RESOURCE_NAME_MAX) == 0) {
        ...
        bool source_defines_entry(const std::string& source, const std::string& entry) {
            if (source.empty() || entry.empty()) {
                return false;
            }
            size_t position = 0;
            while ((position = source.find(entry, position)) != std::string::npos) {
                const bool has_identifier_before = position > 0 && shader_identifier_character(source[position - 1]);
                size_t after = position + entry.size();
                const bool has_identifier_after = after < source.size() && shader_identifier_character(source[after]);
        ...
        std::string compose_surface_fragment_source(const tc_surface_contract_desc& surface,
                                                    const tc_shader_surface_producer_view& producer,
                                                    const MaterialSurfaceConsumerContract& consumer) {
            std::string source;
            source.reserve(std::char_traits<char>::length(surface.interface_source) +
                           std::char_traits<char>::length(producer.evaluator_source) + consumer.consumer_source.size() +
                           256u);
    ...
    MaterialPipelineMaterialContract
    material_pipeline_material_contract_from_shader(TcShader shader,
        ...
        contract.required_fragment_input = std::move(final_color_required_fragment_input);
        ...
        if (tc_shader* raw = shader.get()) {
            ...
            if (tc_shader_get_surface_producer_view(raw, &producer)) {
                ...
                contract.required_fragment_input.semantics.reserve(producer.fragment_input_count);
                for (uint32_t i = 0; i < producer.fragment_input_count; ++i) {
                    ...
                    if (!type.has_value()) {
                        tc::Log::error("Surface producer '%s@%u' has unsupported fragment input type %u for '%s'",
            ...
            for (uint32_t i = 0; i < count; ++i) {
                const uint32_t fragment_stage_mask = bindings[i].stage_mask & TC_SHADER_STAGE_FRAGMENT;
    ...
    material_pipeline_assemble_shader(const MaterialPipelineShaderAssemblyRequest& request) {
        ...
        const bool has_modular_provider = vertex_transform_provider_is_modular(request.vertex_transform);
        ...
        if (vertex_source.empty() && has_modular_provider && has_output_adapter) {
            const std::string& entry = request.vertex_entry_override.empty() ? request.vertex_transform.vertex_entry
                                                                             : request.vertex_entry_override;
            vertex_source =
                compose_modular_vertex_source(request.vertex_transform, *request.pass.vertex_output_adapter, entry);
        }
        if (vertex_source.empty()) {
            result.diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::MissingVertexTransformTemplate,
        ...
        MaterialFragmentComposition fragment_composition = request.pass.fragment_composition;
        if (fragment_composition == MaterialFragmentComposition::SurfaceConsumerOrFinalColor) {
        ...
        case MaterialFragmentComposition::FinalColor: {
            if (!request.material.shader.is_valid() || request.material.shader.fragment_source()[0] == '\0') {
                result.diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::MissingFragmentSource,
                                                        "final-color material fragment source is empty"));
                break;
            }
            if (!request.material.shader.is_executable()) {
                result.diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::FragmentCompositionMismatch,
            ...
            fragment_entry = raw && raw->fragment_entry ? raw->fragment_entry : "main";
            if (!source_defines_entry(fragment_source, fragment_entry)) {
                result.diagnostics.push_back(
                    diagnostic(MaterialPipelineDiagnosticCode::MissingFragmentEntry,
                               "final-color entry '" + fragment_entry + "' was not found in material fragment source"));
            }
            validate_fragment_interface(
                request.material.required_fragment_input, "material", request.vertex_transform, result.diagnostics);
        ...
        case MaterialFragmentComposition::SurfaceConsumer: {
            ...
            tc_shader_surface_producer_view producer{};
            if (!tc_shader_get_surface_producer_view(request.material.shader.get(), &producer)) {
                result.diagnostics.push_back(
                    diagnostic(MaterialPipelineDiagnosticCode::FragmentCompositionMismatch,
                               "pass '" + request.pass.debug_name +
            ...
            if (!request.pass.surface_consumer.has_value()) {
                result.diagnostics.push_back(diagnostic(
                    ...
                    "pass '" + request.pass.debug_name + "' selected SurfaceConsumer without a consumer descriptor"));
                break;
            }
            const MaterialSurfaceConsumerContract& consumer = *request.pass.surface_consumer;
            if (consumer.accepted_surface.id != producer.contract_id ||
                consumer.accepted_surface.version != producer.contract_version) {
                result.diagnostics.push_back(
                    diagnostic(MaterialPipelineDiagnosticCode::SurfaceContractMismatch,
                               "pass '" + request.pass.debug_name + "' accepts '" + consumer.accepted_surface.id + "@" +
                                   std::to_string(consumer.accepted_surface.version) + "' but material produces '" +
                                   producer.contract_id + "@" + std::to_string(producer.contract_version) + "'"));
                break;
            }
            const tc_surface_contract_desc* surface = tc_surface_contract_registry_find({
                producer.contract_id,
                producer.contract_version,
            });
            if (!surface) {
                result.diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::UnknownSurfaceContract,
                                                        "surface contract '" + std::string(producer.contract_id) + "@" +
                                                            std::to_string(producer.contract_version) +
                                                            "' is not registered"));
                break;
            }
            if (std::string(surface->surface_type_name) != producer.surface_type_name) {
                result.diagnostics.push_back(
                    diagnostic(MaterialPipelineDiagnosticCode::SurfaceContractMismatch,
                               "registered surface type '" + std::string(surface->surface_type_name) +
                                   "' does not match producer type '" + producer.surface_type_name + "'"));
                break;
            }
            if (!source_defines_entry(producer.evaluator_source, producer.evaluator_entry)) {
                result.diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::MissingFragmentEntry,
                                                        "surface evaluator entry '" +
                                                            std::string(producer.evaluator_entry) +
                                                            "' was not found in evaluator source"));
            }
            if (!source_defines_entry(consumer.consumer_source, consumer.fragment_entry)) {
                result.diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::MissingFragmentEntry,
            ...
            validate_fragment_interface(request.material.required_fragment_input,
                                        "surface producer",
                                        request.vertex_transform,
                                        result.diagnostics);
            validate_fragment_interface(
                consumer.required_fragment_input, "surface consumer", request.vertex_transform, result.diagnostics);
            fragment_source = compose_surface_fragment_source(*surface, producer, consumer);
        ...
        case MaterialFragmentComposition::PassOwned:
            if (request.pass.fragment_source_override.empty()) {
                result.diagnostics.push_back(
                    diagnostic(MaterialPipelineDiagnosticCode::MissingFragmentSource,
                               "pass '" + request.pass.debug_name + "' selected PassOwned without a fragment source"));
                break;
            }
            ...
            if (!source_defines_entry(fragment_source, fragment_entry)) {
                result.diagnostics.push_back(
                    diagnostic(MaterialPipelineDiagnosticCode::MissingFragmentEntry,
                               "pass-owned entry '" + fragment_entry + "' was not found in fragment source"));
            }
            validate_fragment_interface(request.pass.required_material_fragment_input,
                                        "pass-owned",
                                        request.vertex_transform,
                                        result.diagnostics);
            break;
        ...
        if (has_modular_provider && has_output_adapter) {
            validate_adapter_interface(
                request.vertex_transform, *request.pass.vertex_output_adapter, result.diagnostics);
        }
        ...
        MaterialPipelineResourceMergeResult merged = material_pipeline_merge_resources(resource_decls);
        ...
        const std::string vertex_entry = request.vertex_entry_override.empty() ? request.vertex_transform.vertex_entry
                                                                               : request.vertex_entry_override;
        ...
        const tc_shader_create_desc shader_desc = {
            {vertex_source.c_str(),
             ...
             request.geometry_source_override.empty() ? nullptr : request.geometry_source_override.c_str(),
             shader_name.c_str(),
             nullptr,
             vertex_entry.empty() ? nullptr : vertex_entry.c_str(),
             fragment_entry.empty() ? nullptr : fragment_entry.c_str(),
             request.geometry_entry_override.empty() ? nullptr : request.geometry_entry_override.c_str()},
        ...
        if (tc_shader_handle_is_invalid(handle)) {
            result.diagnostics.push_back(diagnostic(MaterialPipelineDiagnosticCode::ShaderCreationFailed,
                                                    "tc_shader_from_sources_desc failed for '" + shader_name + "'"));
            return result;
        }
