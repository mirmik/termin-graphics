...
TEST_CASE("compiled pipeline template round-trips without execution state") {
    ...
    tc_pipeline_template_handle original_handle = tc_pipeline_template_create("pipeline-roundtrip-a", "placeholder");
    ...
    CHECK_EQ(tc_pipeline_template_serialize(original, undersized.data(), undersized.size()), byte_count);
    ...
    tc_pipeline_template_handle decoded_handle =
        tc_pipeline_template_deserialize("pipeline-roundtrip-b", bytes.data(), bytes.size());
...
TEST_CASE("instances share definition but isolate mutable execution state") {
    ...
    tc_pipeline_template_handle template_handle = tc_pipeline_template_create("pipeline-shared", "shared");
    ...
    CHECK(tc_pipeline_template_handle_eq(tc_pipeline_get_template(first), tc_pipeline_get_template(second)));
    ...
    tc_pipeline_template_handle replacement = tc_pipeline_template_create("pipeline-shared", "replacement");
...
TEST_CASE("runtime instance restores resource views and FBO compositions from template") {
    ...
    const tc_pipeline_template_handle handle = tc_pipeline_template_create("pipeline-recipe-instance", "placeholder");
...

    {
        ...
        CHECK(instance.cache().resource_views.at("scene-depth").attachment == termin::AttachmentKind::Depth);
