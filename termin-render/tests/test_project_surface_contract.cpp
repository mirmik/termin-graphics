...
struct ScopedArtifactConfiguration {
    ...
    ScopedArtifactConfiguration()
    {
        ...
        root = std::filesystem::temp_directory_path() /
            ("termin-project-surface-contract-" + std::to_string(unique));
        ...
        termin::tgfx2_set_shader_compiler_path(
            TERMIN_PROJECT_SURFACE_TEST_SHADERC);
#endif
...
termin::TcShader make_weathered_producer()
{
    ...
    std::snprintf(
        fragment_inputs[0].semantic,
        sizeof(fragment_inputs[0].semantic),
        "%s",
        "world_pos");
    fragment_inputs[0].type = TC_SHADER_CONTRACT_VALUE_FLOAT3;
    std::snprintf(
        fragment_inputs[1].semantic,
        sizeof(fragment_inputs[1].semantic),
        "%s",
        "normal_world");
...
TEST_CASE("project-owned surface contract composes compiles and unloads safely")
{
    ...
    const tc_surface_contract_desc* standard_before =
    ...
    const tc_surface_contract_desc* project =
        tc_surface_contract_registry_find(project_key);
    REQUIRE(project != nullptr);
    CHECK(
        std::strcmp(
            tc_surface_contract_registry_owner(project_key),
            termin_project_surface_test_plugin_owner()) == 0);
    CHECK(
        std::strstr(project->interface_source, "float weathering") != nullptr);
    ...
    termin::TcShader variant =
        termin::assemble_material_shader_override(request);
    REQUIRE(variant.is_valid());
    REQUIRE(variant.is_executable());
    CHECK(
        std::strstr(
            variant.fragment_source(),
            "TestWeatheredSurfaceV1") != nullptr);
    CHECK(
        std::strstr(
            variant.fragment_source(),
            "surface.weathering") != nullptr);
    ...
    CHECK_EQ(
        termin_project_surface_test_plugin_unregister_owner(),
        1u);
    CHECK(tc_surface_contract_registry_find(project_key) == nullptr);

    const tc_surface_contract_desc* standard_after =
        tc_surface_contract_registry_find(standard_key);
