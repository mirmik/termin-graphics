...
namespace {
    ...
    tc_phase_mask phase_mask(tc_component*) {
        return TC_PHASE_EDITOR_DEBUG;
    }

    bool
    collect_items(tc_component* component, const tc_render_item_collect_context* context, tc_render_item_sink* sink) {
        assert(context);
...
int main() {
    ...
    termin::TcSceneRenderItemSource source(scene, nullptr, TC_SCENE_FILTER_NONE);
    ...
    assert(first_view.item(0)->source.domain_id == TC_RENDER_ITEM_SOURCE_DOMAIN_SCENE);
    assert(first_view.item(0)->source.object_id != 0);
    assert(first_view.item(0)->source.generation == entity.generation);
    assert(termin::render_scene_item_component(*first_view.item(0)) == &component);
    assert(first_view.item(0)->payload.line_batch.points == first_view.item(1)->payload.line_batch.points);
