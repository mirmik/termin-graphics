...
namespace {
    ...
    bool has_attribute(const termin::VertexInputContract& input,
                       const std::string& name,
                       termin::MaterialPipelineValueType type) {
        return std::any_of(input.mesh_attributes.begin(),
    ...
    const termin::MaterialPipelineResourceDecl* find_resource(const termin::VertexTransformContract& contract,
                                                              const std::string& name) {
        auto it = std::find_if(
            contract.resources.begin(),
            contract.resources.end(),
            [&](const termin::MaterialPipelineResourceDecl& resource) { return resource.requirement.name == name; });
    ...
    termin::VertexTransformContract compact_static_transform(const char* draw_resource) {
        return termin::material_pipeline_make_static_mesh_vertex_transform_provider(
            "static_compact", termin::MeshVertexTransformProfile::Position, std::string(draw_resource) + ".u_model");
    }

    termin::VertexTransformContract normal_static_transform() {
        return termin::material_pipeline_make_static_mesh_vertex_transform_provider(
            "static_position_normal", termin::MeshVertexTransformProfile::PositionNormal, "normal_draw.u_model");
    }

    termin::VertexTransformContract foliage_material_transform() {
        return termin::material_pipeline_make_foliage_material_vertex_transform_provider("foliage");
    }

    termin::VertexTransformContract foliage_shadow_transform() {
        termin::VertexTransformContract contract = termin::material_pipeline_make_foliage_vertex_transform_provider(
            "foliage_shadow", termin::MeshVertexTransformProfile::Position);
...
TEST_CASE("Skinned material provider is modular and owns deformation resources") {
    termin::VertexTransformContract contract = termin::material_pipeline_make_skinned_mesh_vertex_transform_provider(
    ...
    const termin::MaterialPipelineResourceDecl* bone_block = find_resource(contract, TC_SHADER_RESOURCE_BONE_BLOCK);
    ...
    CHECK(termin::material_pipeline_interface_produces(
        contract.produced_fragment_input, "world_pos", termin::MaterialPipelineValueType::Float3));
}

TEST_CASE("Skinned compact provider keeps position-only mesh input") {
    termin::VertexTransformContract contract = termin::material_pipeline_make_skinned_mesh_vertex_transform_provider(
        "skinned_shadow", termin::MeshVertexTransformProfile::Position, "shadow_draw.u_model");
...
TEST_CASE("Foliage material provider is modular and declares instanced resources") {
    ...
    CHECK(termin::vertex_transform_provider_is_modular(contract));
    CHECK_EQ(contract.source_module.module_name, std::string("termin_foliage_material_transform"));
    CHECK(has_attribute(contract.vertex_inputs, "position", termin::MaterialPipelineValueType::Float3));
    CHECK(has_attribute(contract.vertex_inputs, "normal", termin::MaterialPipelineValueType::Float3));
    CHECK(has_attribute(contract.vertex_inputs, "uv", termin::MaterialPipelineValueType::Float2));

    const termin::MaterialPipelineResourceDecl* foliage_draw = find_resource(contract, "foliage_draw");
    ...
    const termin::MaterialPipelineResourceDecl* foliage_instances = find_resource(contract, "foliage_instances");
...
TEST_CASE("Foliage auxiliary providers expose only the pass-required mesh ABI") {
    termin::VertexTransformContract position = termin::material_pipeline_make_foliage_vertex_transform_provider(
        "foliage_position", termin::MeshVertexTransformProfile::Position);
    ...
    termin::VertexTransformContract position_normal = termin::material_pipeline_make_foliage_vertex_transform_provider(
        "foliage_position_normal", termin::MeshVertexTransformProfile::PositionNormal);
...
TEST_CASE("Position-normal transform is data, not pass kind") {
    ...
    CHECK(has_attribute(contract.vertex_inputs, "position", termin::MaterialPipelineValueType::Float3));
    CHECK(has_attribute(contract.vertex_inputs, "normal", termin::MaterialPipelineValueType::Float3));
    CHECK(contract.adapter_input_expression.find("normal_draw.u_model") != std::string::npos);
}
...
TEST_CASE("Pass contracts carry fragment input intent explicitly") {
    ...
    material_pass.required_material_fragment_input = termin::material_pipeline_standard_material_fragment_interface();
    material_pass.static_vertex_transform = material_static_transform();

    CHECK(termin::material_pipeline_interface_produces(
        material_pass.required_material_fragment_input, "world_pos", termin::MaterialPipelineValueType::Float3));
    CHECK(termin::material_pipeline_interface_produces(
        material_pass.required_material_fragment_input, "normal_world", termin::MaterialPipelineValueType::Float3));
    ...
    CHECK(auxiliary_pass.static_vertex_transform->adapter_input_expression.find("custom_draw.u_model") !=
          std::string::npos);
}

TEST_CASE("Custom geometry pass labels do not select vertex contracts") {
    CustomGeometryPassDescriptor material_pass{"actor_attribute", termin::MaterialPipelinePassContract{}};
    ...
    CustomGeometryPassDescriptor compact_pass{"actor_attribute", termin::MaterialPipelinePassContract{}};
    compact_pass.shader_contract.debug_name = "actor_attribute_compact";
    compact_pass.shader_contract.required_material_fragment_input = termin::MaterialFragmentInterface{};
    compact_pass.shader_contract.static_vertex_transform = compact_static_transform("actor_attribute_draw");

    CHECK_EQ(material_pass.phase_mark, compact_pass.phase_mark);
    CHECK(termin::material_pipeline_interface_produces(material_pass.shader_contract.required_material_fragment_input,
                                                       "world_pos",
                                                       termin::MaterialPipelineValueType::Float3));
    CHECK(has_attribute(material_pass.shader_contract.static_vertex_transform->vertex_inputs,
                        "normal",
                        termin::MaterialPipelineValueType::Float3));
    CHECK(compact_pass.shader_contract.required_material_fragment_input.semantics.empty());
    CHECK(has_attribute(compact_pass.shader_contract.static_vertex_transform->vertex_inputs,
                        "position",
                        termin::MaterialPipelineValueType::Float3));
    CHECK(!has_attribute(compact_pass.shader_contract.static_vertex_transform->vertex_inputs,
                         "normal",
