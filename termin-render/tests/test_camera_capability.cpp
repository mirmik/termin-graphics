...
namespace {
    ...
    bool get_camera_data(tc_component* component, double aspect_override, tc_camera_data* out) {
...
TEST_CASE("camera capability resolves a provider without CameraComponent") {
    ...
    REQUIRE(tc_camera_capability_attach(&provider.component, &kAlternateCameraVtable, &provider));

    termin::RenderCameraSnapshot snapshot;
    REQUIRE(termin::resolve_render_camera(&provider.component, 16.0 / 9.0, snapshot));
...
TEST_CASE("named camera lookup uses capability and reports missing capability") {
    ...
    termin::RenderCameraSnapshot snapshot;
    CHECK(termin::resolve_named_render_camera(scene, "alternate-camera", 2.0, snapshot) ==
    ...
    CHECK(termin::resolve_named_render_camera(scene, "plain-entity", 0.0, snapshot) ==
          termin::NamedCameraResolveError::CapabilityNotFound);
    CHECK(termin::resolve_named_render_camera(scene, "missing", 0.0, snapshot) ==
