...
namespace termin {
    ...
    RENDER_CORE_API bool vertex_transform_provider_is_modular(const VertexTransformProvider& provider);
    ...
    RENDER_CORE_API MaterialPipelineResourceDecl material_pipeline_draw_resource_decl(std::string name,
                                                                                      uint32_t stage_mask,
                                                                                      uint32_t size = 64u);

    RENDER_CORE_API std::vector<MaterialPipelineResourceDecl>
    material_pipeline_common_vertex_resources(std::string draw_resource_name, uint32_t draw_resource_size = 64u);

    RENDER_CORE_API std::vector<MaterialPipelineResourceDecl>
    material_pipeline_pass_vertex_resources(std::string draw_resource_name, uint32_t draw_resource_size = 64u);
    ...
    RENDER_CORE_API VertexTransformProvider
    material_pipeline_make_foliage_material_vertex_transform_provider(std::string debug_name);

    RENDER_CORE_API VertexTransformProvider material_pipeline_make_foliage_vertex_transform_provider(
        std::string debug_name, MeshVertexTransformProfile profile);

    RENDER_CORE_API bool material_pipeline_interface_produces(const MaterialFragmentInterface& interface,
