...
namespace termin {
    ...
    RENDER_API bool resolve_render_camera(tc_component* component, double aspect_override, RenderCameraSnapshot& out);
    ...
    RENDER_API NamedCameraResolveError resolve_named_render_camera(tc_scene_handle scene,
                                                                   const char* entity_name,
                                                                   double aspect_override,
                                                                   RenderCameraSnapshot& out);

    RENDER_API bool resolve_named_render_camera_for_pass(tc_scene_handle scene,
                                                         const char* entity_name,
                                                         double aspect_override,
                                                         const char* pass_name,
                                                         RenderCameraSnapshot& out);

    RENDER_API const char* named_camera_resolve_error_name(NamedCameraResolveError error);
