...
namespace termin {
    ...
    public:
        ...
        explicit TcPipelineTemplate(tc_pipeline_template_handle value)
            : handle(value) {
            tc_pipeline_template_retain(get());
        }
        TcPipelineTemplate(const TcPipelineTemplate& other)
            : handle(other.handle) {
            tc_pipeline_template_retain(get());
        }
        TcPipelineTemplate(TcPipelineTemplate&& other) noexcept
            : handle(other.handle) {
        ...
        tc_pipeline_template* get() const {
            return tc_pipeline_template_get(handle);
        }
        bool is_valid() const {
            return tc_pipeline_template_is_valid(handle);
        }
        const char* uuid() const {
            return get() ? get()->header.uuid : "";
        }
        const char* name() const {
            return get() && get()->header.name ? get()->header.name : "";
        }
        uint32_t version() const {
            return tc_pipeline_template_version(get());
        }
    };
