...
namespace termin {
    ...
    draw_material_pipeline_mesh(tgfx::RenderContext2& ctx, tc_mesh* mesh, MaterialMeshVertexInput input);

    RENDER_CORE_API bool draw_material_pipeline_submesh(tgfx::RenderContext2& ctx,
                                                        tc_mesh* mesh,
                                                        size_t submesh_index,
                                                        MaterialMeshVertexInput input);

    RENDER_CORE_API bool ensure_material_pipeline_shader(tgfx::RenderContext2& ctx,
                                                         tgfx::IRenderDevice& device,
                                                         tc_shader_handle shader_handle,
                                                         const char* debug_context,
                                                         MaterialPipelineShaderBinding& out);

    RENDER_CORE_API bool prepare_material_pipeline_resources(tgfx::RenderContext2& ctx,
                                                             tgfx::IRenderDevice& device,
