...
namespace termin {
    ...
    public:
        ...
        FrameGraphResource* get_frame_graph_resource(const std::string& name) const {
            auto it = frame_graph_resources.find(name);
            return it == frame_graph_resources.end() ? nullptr : it->second;
        }

        template <typename ResourceT> ResourceT* get_frame_graph_resource_as(const std::string& name) const {
        ...
        RENDER_CORE_API bool capture_internal(const char* symbol,
        ...
        // the pass invocation.
        RENDER_CORE_API bool build_render_pass(std::span<const FrameGraphColorAttachment> colors,
