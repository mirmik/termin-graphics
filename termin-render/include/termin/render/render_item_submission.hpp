...
namespace termin {
    ...
    using RenderItemDrawEncoderFn = bool (*)(tgfx::RenderContext2& ctx,
    ...
    struct RenderItemTaskPlanningContract {
        tc_phase_mask phase = TC_PHASE_NONE;
        RenderItemMaterialPhasePolicy material_phase_policy = RenderItemMaterialPhasePolicy::Optional;
    ...
    using RenderItemTaskShaderPlannerFn = RenderItemTaskRejection (*)(const RenderItemTaskPlanningRequest& request,
    ...
    set_render_item_inline_uniform(tc_render_item& item, const char* name, const void* data, uint32_t size);
    ...
    RENDER_CORE_API bool register_render_item_draw_encoder(uint32_t item_kind, const RenderItemDrawEncoderDesc& desc);

    RENDER_CORE_API bool
    unregister_render_item_draw_encoder(uint32_t item_kind, RenderItemDrawEncoderFn encode, void* user_data = nullptr);

    RENDER_CORE_API bool get_render_item_encoder_capabilities(uint32_t item_kind, RenderItemEncoderCapabilities& out);

    RENDER_CORE_API bool render_item_encoder_supports_phase(uint32_t item_kind, tc_phase_mask phase);
    ...
    RENDER_CORE_API RenderItemTaskRejection
    plan_render_item_passthrough_shader(const RenderItemTaskPlanningRequest& request,
                                        RenderItemTaskShaderPlan& out_plan,
                                        const char*& out_detail,
                                        void* user_data);
    ...
    RENDER_CORE_API RenderItemTaskPlanningResult plan_render_item_task(const RenderItemTaskPlanningRequest& request,
                                                                       RenderTaskList& out_tasks);

    RENDER_CORE_API const char* render_item_task_rejection_name(RenderItemTaskRejection rejection);

    RENDER_CORE_API bool bind_render_item_common_resources(tgfx::RenderContext2& ctx,
    ...
    RENDER_CORE_API bool bind_render_item_inline_uniform(tgfx::RenderContext2& ctx,
                                                         const tc_render_item& item,
                                                         const tc_shader* shader,
                                                         const char* debug_pass_name,
                                                         const char* debug_entity_name);

    RENDER_CORE_API bool submit_render_item_draw(tgfx::RenderContext2& ctx,
                                                 const tc_render_item& item,
                                                 const RenderItemDrawSubmitRequest& request);
