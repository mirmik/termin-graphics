...
namespace termin {
    ...
    RENDER_CORE_API const char* material_pipeline_resource_owner_name(MaterialPipelineResourceOwner owner);

    RENDER_CORE_API const char* material_pipeline_diagnostic_code_name(MaterialPipelineDiagnosticCode code);

    RENDER_CORE_API MaterialPipelineResourceDecl material_pipeline_abi_resource_decl(
        ShaderAbiResourceId id, uint32_t stage_mask, MaterialPipelineResourceOwner owner, uint32_t size = 0);

    RENDER_CORE_API bool material_pipeline_merge_resource(std::vector<MaterialPipelineResourceDecl>& resources,
                                                          const MaterialPipelineResourceDecl& incoming,
                                                          std::vector<MaterialPipelineDiagnostic>& diagnostics);

    RENDER_CORE_API MaterialPipelineResourceMergeResult
