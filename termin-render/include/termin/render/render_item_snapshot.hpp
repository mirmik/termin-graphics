...
namespace termin {
    ...
    public:
        ...
        const RenderItemSnapshotCounters& counters() const {
            return counters_;
        }
        bool valid() const {
            return valid_;
    ...
    RENDER_CORE_API bool render_item_matches_phase(const tc_render_item& item, tc_phase_mask phase);

    RENDER_CORE_API const RenderItemSnapshot* require_render_item_snapshot(const ExecuteContext& context,
                                                                           const char* consumer);
