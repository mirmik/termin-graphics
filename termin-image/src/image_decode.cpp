...
namespace termin::image {
    namespace {
        ...
        DecodedImage decode_jpeg_rgba8(std::span<const std::uint8_t> bytes) {
            ...
            if (setjmp(state->error.jump) != 0) {
                ...
                throw std::runtime_error(std::string("libjpeg failed to decode image: ") + state->error.message);
            }
            ...
            state->result.pixels.resize(static_cast<std::size_t>(width) * static_cast<std::size_t>(height) * 4);
        ...
        DecodedImage decode_webp_rgba8(std::span<const std::uint8_t> bytes) {
            ...
            std::uint8_t* decoded =
                WebPDecodeRGBAInto(bytes.data(), bytes.size(), result.pixels.data(), result.pixels.size(), width * 4);
            if (decoded == nullptr) {
    ...
    void encode_png_rgba8_to(
        std::span<const std::uint8_t> rgba, int width, int height, PngWriteCallback write, void* context) {
        if (width <= 0 || height <= 0) {
        ...
        for (int y = 0; y < height; ++y) {
            (*rows)[static_cast<std::size_t>(y)] =
                const_cast<png_bytep>(rgba.data() + static_cast<std::size_t>(y) * static_cast<std::size_t>(width) * 4);
        }
