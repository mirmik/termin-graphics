...
namespace termin::image {
    ...
    TERMIN_IMAGE_API DecodedImage decode_rgba8(std::span<const std::uint8_t> bytes,
    ...
    TERMIN_IMAGE_API std::vector<std::uint8_t>
    encode_png_rgba8(std::span<const std::uint8_t> rgba, int width, int height);
