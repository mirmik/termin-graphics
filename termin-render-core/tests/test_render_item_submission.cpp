...
FragmentOutput consume_planner_surface(FragmentInput input) {
    ...
    termin::RenderItemTaskRejection owning_test_shader_planner(const termin::RenderItemTaskPlanningRequest& request,
        ...
        create_info.uuid = "termin-render-core-task-owned-shader-test";
        create_info.language = TC_SHADER_LANGUAGE_SLANG;
        create_info.artifact_policy = TC_SHADER_ARTIFACT_REQUIRED;
...
TEST_CASE("RenderItem planner assembles surface consumers without fallback") {
    ...
    REQUIRE(tc_surface_contract_registry_register("termin-render-core-item-tests", &descriptor));
...
TEST_CASE("RenderItem task shaders can be retained by pass draw-call caches") {
    ...
    constexpr const char* shader_uuid = "termin-render-core-task-owned-shader-test";
    termin::unregister_render_item_draw_encoder(test_kind, test_encoder, nullptr);
