...
FragmentOutput consume_test_surface_b(FragmentInput input) {
    ...
    bool contract_has_vertex_input(const tc_shader_contract_view& view, const char* semantic) {
    ...
    const tc_shader_resource_requirement* contract_resource(const tc_shader_contract_view& view, const char* name) {
    ...
    termin::MaterialPipelineMaterialContract material_contract() {
        ...
        return termin::material_pipeline_material_contract_from_shader(
            shader, termin::material_pipeline_standard_material_fragment_interface());
    }

    termin::MaterialPipelineMaterialContract material_contract_from_fragment(const char* fragment_source,
                                                                             const char* name) {
        termin::TcShaderCreateInfo create_info{};
        create_info.sources.vertex = "";
    ...
    void register_test_surface_contract(const char* source_identity) {
        ...
        REQUIRE(tc_surface_contract_registry_register("termin-render-core-tests", &descriptor));
    }

    termin::MaterialPipelineMaterialContract
    surface_material_contract(const char* uuid = "assembler-test-surface-producer") {
    ...
    surface_consumer_pass(const char* debug_name, const char* source, const char* entry, const char* source_identity) {
        ...
        consumer.required_fragment_input.semantics.push_back({"world_pos", termin::MaterialPipelineValueType::Float3});
    ...
    termin::MaterialPipelinePassContract material_pass_contract() {
        ...
        contract.skinned_vertex_transform = termin::material_pipeline_make_skinned_mesh_vertex_transform_provider(
            "skinned", termin::MeshVertexTransformProfile::Material, "draw_data.u_model");
        ...
        contract.foliage_vertex_transform =
            termin::material_pipeline_make_foliage_material_vertex_transform_provider("foliage");
        return contract;
    }
    ...
    termin::MaterialPipelinePassContract compact_auxiliary_pass_contract() {
        ...
        contract.static_vertex_transform = termin::material_pipeline_make_static_mesh_vertex_transform_provider(
            "static_compact", termin::MeshVertexTransformProfile::Position, "compact_draw.u_model");
        contract.skinned_vertex_transform = termin::material_pipeline_make_skinned_mesh_vertex_transform_provider(
            "skinned_compact", termin::MeshVertexTransformProfile::Position, "compact_draw.u_model");
        contract.static_vertex_transform->resources.push_back(
            termin::material_pipeline_draw_resource_decl("compact_draw", TC_SHADER_STAGE_VERTEX, 64u));
        contract.skinned_vertex_transform->resources.push_back(
            termin::material_pipeline_draw_resource_decl("compact_draw", TC_SHADER_STAGE_VERTEX, 64u));
        return contract;
    }
    ...
    termin::MaterialFragmentInterface world_position_interface() {
        ...
        interface.semantics.push_back({"world_pos", termin::MaterialPipelineValueType::Float3});
        return interface;
    }
    ...
    termin::MaterialFragmentInterface world_position_normal_interface() {
        ...
        interface.semantics.push_back({"normal_world", termin::MaterialPipelineValueType::Float3});
        return interface;
    }

    termin::VertexOutputAdapter auxiliary_output_adapter(const char* module,
                                                         const char* function,
        ...
        adapter.source_module = {module, std::string("builtin_shaders/") + module + ".slang"};
        ...
        adapter.resources = termin::material_pipeline_pass_vertex_resources(draw_resource, draw_size);
        return adapter;
    }
    ...
    termin::VertexTransformProvider modular_shadow_provider(bool skinned) {
        return skinned
                   ? termin::material_pipeline_make_skinned_mesh_vertex_transform_provider(
                         "skinned_shadow_provider", termin::MeshVertexTransformProfile::Position, "shadow_draw.u_model")
                   : termin::material_pipeline_make_static_mesh_vertex_transform_provider(
                         "static_shadow_provider", termin::MeshVertexTransformProfile::Position, "shadow_draw.u_model");
    }

    termin::VertexOutputAdapter modular_shadow_adapter() {
        termin::VertexOutputAdapter adapter;
        adapter.debug_name = "shadow_adapter";
        adapter.source_module = {"termin_shadow_vertex_output_adapter",
        ...
        adapter.produced_output_semantics.semantics.push_back(
            {"clip_position", termin::MaterialPipelineValueType::Float4});
        adapter.resources.push_back(
            termin::material_pipeline_abi_resource_decl(termin::ShaderAbiResourceId::PerFrame,
    ...
    termin::MaterialPipelinePassContract modular_shadow_pass_contract() {
        ...
        pass.static_vertex_transform->resources.push_back(
            termin::material_pipeline_draw_resource_decl("shadow_draw", TC_SHADER_STAGE_VERTEX, 64u));
        pass.skinned_vertex_transform = modular_shadow_provider(true);
        pass.skinned_vertex_transform->resources.push_back(
            termin::material_pipeline_draw_resource_decl("shadow_draw", TC_SHADER_STAGE_VERTEX, 64u));
        pass.foliage_vertex_transform = termin::material_pipeline_make_foliage_vertex_transform_provider(
            "foliage_shadow_provider", termin::MeshVertexTransformProfile::Position);
        pass.foliage_vertex_transform->kind = termin::VertexTransformKind::FoliageShadow;
        std::erase_if(pass.foliage_vertex_transform->resources,
                      [](const termin::MaterialPipelineResourceDecl& resource) {
                          return resource.requirement.name == "per_frame";
                      });
        pass.foliage_vertex_transform->source_module = {"termin_shadow_foliage_transform",
...
struct VertexInput {
    ...
    uint instance_id : SV_InstanceID;
        ...
        pass.foliage_vertex_transform->produced_world_semantics = world_position_interface();
        return pass;
    }
...
TEST_CASE("material contract projects reflected resources to fragment stage") {
    ...
    CHECK_EQ(material.resources[0].requirement.stage_mask, static_cast<uint32_t>(TC_SHADER_STAGE_FRAGMENT));
...
TEST_CASE("surface producer composes with distinct pass consumers") {
    ...
    termin::MaterialPipelineMaterialContract material = surface_material_contract();
    REQUIRE(material.required_fragment_input.semantics.size() == 2u);
    REQUIRE(material.resources.size() == 1u);
    CHECK(material.resources[0].requirement.name == TC_SHADER_RESOURCE_MATERIAL);

    termin::MaterialPipelinePassContract pass_a =
        surface_consumer_pass("surface_consumer_a", kTestSurfaceConsumerA, "consume_test_surface_a", "consumer:a:v1");
    termin::MaterialPipelinePassContract pass_b =
        surface_consumer_pass("surface_consumer_b", kTestSurfaceConsumerB, "consume_test_surface_b", "consumer:b:v1");
    ...
    termin::MaterialPipelineShaderAssemblyResult result_a = termin::material_pipeline_assemble_shader(request);
    REQUIRE(result_a.ok());
    REQUIRE(result_a.shader.is_executable());
    CHECK_FALSE(result_a.shader.has_surface_producer());
    CHECK(std::string(result_a.shader.fragment_source()).find("struct TerminTestSurface") != std::string::npos);
    CHECK(std::string(result_a.shader.fragment_source()).find("evaluate_test_surface") != std::string::npos);
    CHECK(std::string(result_a.shader.fragment_source()).find("consume_test_surface_a") != std::string::npos);
    ...
    termin::MaterialPipelineShaderAssemblyResult result_b = termin::material_pipeline_assemble_shader(request);
    REQUIRE(result_b.ok());
    CHECK(std::string(result_b.shader.fragment_source()).find("consume_test_surface_b") != std::string::npos);
    CHECK(std::string(result_a.shader.source_hash()) != std::string(result_b.shader.source_hash()));

    const std::string fingerprint_a = termin::material_pipeline_shader_intent_fingerprint(
        material.shader, TC_SHADER_VARIANT_NONE, *pass_a.static_vertex_transform, pass_a);
    const std::string fingerprint_b = termin::material_pipeline_shader_intent_fingerprint(
        material.shader, TC_SHADER_VARIANT_NONE, *pass_b.static_vertex_transform, pass_b);
    ...
    variant_request.vertex_transform_kind = termin::VertexTransformKind::StaticMesh;
    variant_request.pass_contract = pass_a;
    variant_request.debug_context = "surface-consumer-variant-test";
    termin::TcShader variant_a = termin::assemble_material_shader_override(variant_request);
    REQUIRE(variant_a.is_valid());
    REQUIRE(variant_a.is_executable());
    termin::TcShader cached_a = termin::assemble_material_shader_override(variant_request);
    REQUIRE(cached_a.is_valid());
    CHECK(tc_shader_handle_eq(variant_a.handle, cached_a.handle));

    variant_request.pass_contract = pass_b;
    termin::TcShader variant_b = termin::assemble_material_shader_override(variant_request);
...
TEST_CASE("hybrid color composition keeps final-color and surface materials compatible") {
    ...
    termin::MaterialPipelinePassContract pass =
        surface_consumer_pass("hybrid_color", kTestSurfaceConsumerA, "consume_test_surface_a", "consumer:a:v1");
    pass.fragment_composition = termin::MaterialFragmentComposition::SurfaceConsumerOrFinalColor;

    termin::MaterialPipelineShaderAssemblyRequest request{};
    request.pass = pass;
    request.vertex_transform = *pass.static_vertex_transform;

    termin::MaterialPipelineMaterialContract final_material = material_contract();
    request.material = final_material;
    request.shader_name = "hybrid-final-color";
    request.shader_uuid = "hybrid-final-color";
    termin::MaterialPipelineShaderAssemblyResult final_color = termin::material_pipeline_assemble_shader(request);
    REQUIRE(final_color.ok());
    CHECK(std::string(final_color.shader.fragment_source()).find("fs_main") != std::string::npos);

    termin::MaterialPipelineMaterialContract surface_material = surface_material_contract("hybrid-surface-producer");
    request.material = surface_material;
    request.shader_name = "hybrid-surface";
    request.shader_uuid = "hybrid-surface";
    termin::MaterialPipelineShaderAssemblyResult surface = termin::material_pipeline_assemble_shader(request);
    REQUIRE(surface.ok());
    CHECK(std::string(surface.shader.fragment_source()).find("consume_test_surface_a") != std::string::npos);

    termin::MaterialPipelinePassContract explicit_pass = pass;
    explicit_pass.fragment_composition = termin::MaterialFragmentComposition::FinalColor;
    CHECK(termin::material_pipeline_shader_intent_fingerprint(
              final_material.shader, TC_SHADER_VARIANT_NONE, *pass.static_vertex_transform, pass) ==
          termin::material_pipeline_shader_intent_fingerprint(
              final_material.shader, TC_SHADER_VARIANT_NONE, *explicit_pass.static_vertex_transform, explicit_pass));

    explicit_pass.fragment_composition = termin::MaterialFragmentComposition::SurfaceConsumer;
    CHECK(termin::material_pipeline_shader_intent_fingerprint(
              surface_material.shader, TC_SHADER_VARIANT_NONE, *pass.static_vertex_transform, pass) ==
          termin::material_pipeline_shader_intent_fingerprint(
              surface_material.shader, TC_SHADER_VARIANT_NONE, *explicit_pass.static_vertex_transform, explicit_pass));
...
TEST_CASE("surface composition rejects incompatible fragment roles and contracts") {
    ...
    termin::MaterialPipelineMaterialContract surface = surface_material_contract();
    termin::MaterialPipelineMaterialContract final_color = material_contract();
    termin::MaterialPipelinePassContract surface_pass =
        surface_consumer_pass("surface_consumer", kTestSurfaceConsumerA, "consume_test_surface_a", "consumer:a:v1");
    ...
    termin::MaterialPipelineShaderAssemblyResult final_as_surface = termin::material_pipeline_assemble_shader(request);
    REQUIRE_FALSE(final_as_surface.ok());
    REQUIRE_FALSE(final_as_surface.diagnostics.empty());
    CHECK(final_as_surface.diagnostics[0].code == termin::MaterialPipelineDiagnosticCode::FragmentCompositionMismatch);
    ...
    termin::MaterialPipelineShaderAssemblyResult surface_as_final = termin::material_pipeline_assemble_shader(request);
    REQUIRE_FALSE(surface_as_final.ok());
    REQUIRE_FALSE(surface_as_final.diagnostics.empty());
    CHECK(surface_as_final.diagnostics[0].code == termin::MaterialPipelineDiagnosticCode::FragmentCompositionMismatch);
    ...
    termin::MaterialPipelineShaderAssemblyResult mismatch = termin::material_pipeline_assemble_shader(request);
    REQUIRE_FALSE(mismatch.ok());
    REQUIRE_FALSE(mismatch.diagnostics.empty());
    CHECK(mismatch.diagnostics[0].code == termin::MaterialPipelineDiagnosticCode::SurfaceContractMismatch);
    ...
    termin::MaterialPipelineShaderAssemblyResult missing_entry = termin::material_pipeline_assemble_shader(request);
    REQUIRE_FALSE(missing_entry.ok());
    CHECK(std::any_of(missing_entry.diagnostics.begin(),
                      missing_entry.diagnostics.end(),
                      [](const termin::MaterialPipelineDiagnostic& item) {
                          return item.code == termin::MaterialPipelineDiagnosticCode::MissingFragmentEntry;
                      }));

    request.pass = surface_pass;
    request.vertex_transform = termin::material_pipeline_make_static_mesh_vertex_transform_provider(
        "surface_position_only", termin::MeshVertexTransformProfile::Position, "draw_data.u_model");
    request.pass.vertex_output_adapter = modular_shadow_adapter();
    request.shader_name = "surface-missing-semantic";
    request.shader_uuid = "surface-missing-semantic";
    termin::MaterialPipelineShaderAssemblyResult missing_semantic = termin::material_pipeline_assemble_shader(request);
    REQUIRE_FALSE(missing_semantic.ok());
    CHECK(std::any_of(missing_semantic.diagnostics.begin(),
                      missing_semantic.diagnostics.end(),
                      [](const termin::MaterialPipelineDiagnostic& item) {
                          return item.code == termin::MaterialPipelineDiagnosticCode::MissingVertexOutputSemantic;
                      }));
...
TEST_CASE("surface composition preserves static skinned and foliage resources") {
    ...
    termin::MaterialPipelinePassContract pass = surface_consumer_pass(
        "surface_transform_variants", kTestSurfaceConsumerA, "consume_test_surface_a", "consumer:a:v1");
    ...
    for (const TransformCase& transform_case : cases) {
        ...
        request.shader_name = std::string("surface-transform-") + transform_case.name;
        request.shader_uuid = std::string("surface-transform-") + transform_case.name;
        termin::MaterialPipelineShaderAssemblyResult result = termin::material_pipeline_assemble_shader(request);
        REQUIRE(result.ok());
        tc_shader_contract_view view{};
        REQUIRE(tc_shader_get_contract_view(result.shader.get(), &view));
...
TEST_CASE("surface fingerprint tracks producer consumer and interface identities") {
    ...
    termin::MaterialPipelineMaterialContract material = surface_material_contract();
    termin::MaterialPipelinePassContract pass =
        surface_consumer_pass("surface_identity", kTestSurfaceConsumerA, "consume_test_surface_a", "consumer:a:v1");
    const termin::VertexTransformContract& transform = *pass.static_vertex_transform;
    const std::string original =
        termin::material_pipeline_shader_intent_fingerprint(material.shader, TC_SHADER_VARIANT_NONE, transform, pass);

    termin::MaterialPipelinePassContract changed_consumer = pass;
    changed_consumer.surface_consumer->source_identity = "consumer:a:v2";
    CHECK(original != termin::material_pipeline_shader_intent_fingerprint(
                          material.shader, TC_SHADER_VARIANT_NONE, transform, changed_consumer));

    REQUIRE(tc_surface_contract_registry_unregister("termin-render-core-tests", {"test.surface.synthetic", 1u}));
    ...
    REQUIRE(tc_shader_set_surface_producer(material.shader.get(), &changed_producer));
...
TEST_CASE("pass-owned fragment is explicit and ignores material resources") {
    ...
    pass.fragment_composition = termin::MaterialFragmentComposition::PassOwned;
...
TEST_CASE("material pipeline assembler attaches skinned shader contract") {
    ...
    const tc_shader_resource_requirement* bone = contract_resource(view, TC_SHADER_RESOURCE_BONE_BLOCK);
    ...
    CHECK(tc_shader_find_resource_binding(result.shader.get(), TC_SHADER_RESOURCE_BONE_BLOCK) == nullptr);
...
TEST_CASE("material pipeline assembler keeps skinned debug normal material semantics linkable") {
    ...
    termin::MaterialPipelineShaderAssemblyRequest request{};
    request.material =
        material_contract_from_fragment(kStandardNormalFragmentSource, "assembler-standard-normal-fragment");
    request.pass = material_pass_contract();
    request.vertex_transform = *request.pass.skinned_vertex_transform;
    ...
    CHECK(vertex_source.find("import termin_material_vertex_output_adapter;") != std::string::npos);
...
TEST_CASE("material mesh input selection follows static compact shader contract") {
    ...
    REQUIRE(result.ok());
    CHECK(termin::material_mesh_vertex_input_for_shader(result.shader.get(),
...
TEST_CASE("material mesh input selection preserves authored vertex attributes") {
    ...
    termin::TcShader shader = termin::TcShader::from_sources(create_info);
    REQUIRE(shader.is_valid());
    CHECK(termin::material_mesh_vertex_input_for_shader(shader.get(), termin::MaterialMeshVertexInput::FullMaterial) ==
          termin::MaterialMeshVertexInput::FullMaterial);
...
TEST_CASE("material pipeline composes one skinned provider across pass adapters") {
    ...
    for (const AdapterCase& adapter_case : cases) {
        ...
        pass.vertex_output_adapter =
            adapter_case.material
                ? termin::material_pipeline_standard_material_vertex_output_adapter()
                : auxiliary_output_adapter(adapter_case.module,
                                           adapter_case.function,
                                           adapter_case.draw_resource,
                                           adapter_case.normal_input ? world_position_normal_interface()
                                                                     : world_position_interface(),
                                           adapter_case.draw_size);
        pass.skinned_vertex_transform = termin::material_pipeline_make_skinned_mesh_vertex_transform_provider(
            std::string("skinned_") + adapter_case.name, adapter_case.profile, adapter_case.model_expression);
        if (adapter_case.material) {
            pass.skinned_vertex_transform->resources.push_back(termin::material_pipeline_draw_resource_decl(
                adapter_case.draw_resource, TC_SHADER_STAGE_VERTEX, adapter_case.draw_size));
        }
        ...
        CHECK(source.find(std::string("import ") + adapter_case.module + ";") != std::string::npos);
        ...
        REQUIRE(tc_shader_get_contract_view(result.shader.get(), &view));
        CHECK(contract_has_vertex_input(view, "joints"));
        CHECK(contract_has_vertex_input(view, "normal") == adapter_case.normal_input);
...
TEST_CASE("material pipeline assembler composes shadow providers with one output adapter") {
    ...
    termin::MaterialPipelineShaderAssemblyResult static_result = termin::material_pipeline_assemble_shader(request);
    REQUIRE(static_result.ok());
    const std::string static_source = static_result.shader.vertex_source();
    CHECK(static_source.find("import termin_vertex_transform;") != std::string::npos);
    CHECK(static_source.find("import termin_shadow_vertex_output_adapter;") != std::string::npos);
    CHECK(static_source.find("termin_static_world_position") != std::string::npos);
    ...
    termin::MaterialPipelineShaderAssemblyResult skinned_result = termin::material_pipeline_assemble_shader(request);
    REQUIRE(skinned_result.ok());
    const std::string skinned_source = skinned_result.shader.vertex_source();
    CHECK(skinned_source.find("import termin_vertex_transform;") != std::string::npos);
    CHECK(skinned_source.find("termin_skinned_world_position") != std::string::npos);

    tc_shader_contract_view skinned_view{};
    REQUIRE(tc_shader_get_contract_view(skinned_result.shader.get(), &skinned_view));
    const tc_shader_resource_requirement* bone = contract_resource(skinned_view, TC_SHADER_RESOURCE_BONE_BLOCK);
    REQUIRE(bone != nullptr);
    CHECK_EQ(bone->scope, TC_SHADER_RESOURCE_SCOPE_DRAW);
    ...
    termin::MaterialPipelineShaderAssemblyResult foliage_result = termin::material_pipeline_assemble_shader(request);
    REQUIRE(foliage_result.ok());
    const std::string foliage_source = foliage_result.shader.vertex_source();
    CHECK(foliage_source.find("import termin_shadow_foliage_transform;") != std::string::npos);
    CHECK(foliage_source.find("termin_shadow_foliage_world_position(input.position, input.instance_id)") !=
          std::string::npos);
...
TEST_CASE("material shader intent fingerprint includes skinned vertex transform") {
    ...
    const std::string fingerprint_a = termin::material_pipeline_shader_intent_fingerprint(
        material.shader, TC_SHADER_VARIANT_SKINNING, *pass_a.skinned_vertex_transform, pass_a);
    const std::string fingerprint_b = termin::material_pipeline_shader_intent_fingerprint(
        material.shader, TC_SHADER_VARIANT_SKINNING, *pass_b.skinned_vertex_transform, pass_b);
...
TEST_CASE("material shader intent fingerprint includes modular source identities") {
    ...
    termin::VertexTransformContract changed_provider = *pass_a.static_vertex_transform;
    changed_provider.source_module.source_identity = "builtin_shaders/termin_vertex_transform.changed.slang";
    const std::string provider_fingerprint = termin::material_pipeline_shader_intent_fingerprint(
        material.shader, TC_SHADER_VARIANT_NONE, changed_provider, pass_a);
...
TEST_CASE("destroyed variant originals are quiet cache invalidation probes") {
    ...
    tc_shader_set_variant_info(tc_shader_get(variant), original, TC_SHADER_VARIANT_SKINNING);
...
TEST_CASE("static material shader overrides are canonical derived shaders") {
    ...
    const tc_shader_resource_requirement* material_resource = contract_resource(view, TC_SHADER_RESOURCE_MATERIAL);
    REQUIRE(material_resource != nullptr);
    CHECK_EQ(material_resource->stage_mask, static_cast<uint32_t>(TC_SHADER_STAGE_FRAGMENT));
...
TEST_CASE("static authored vertex interface is not widened by material pipeline planning") {
    ...
    CHECK(termin::material_mesh_vertex_input_for_shader(planned.get(), termin::MaterialMeshVertexInput::FullMaterial) ==
          termin::MaterialMeshVertexInput::Position);
