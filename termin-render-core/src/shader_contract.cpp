...
namespace termin {
    namespace {
        ...
        bool validate_resource_requirement(const tc_shader* shader,
                                           const tc_shader_resource_requirement& requirement,
                                           const ShaderContractValidationOptions& options) {
            const tc_shader_resource_binding* binding = tc_shader_find_resource_binding(shader, requirement.name);
            if (!binding) {
                tc::Log::error(
            ...
            if (binding->kind != requirement.kind) {
                tc::Log::error("[%s] shader '%s' resource '%s' kind mismatch: contract=%u layout=%u",
                               context_name(options),
                               shader_name(shader),
                               requirement.name,
                               requirement.kind,
            ...
            if (binding->scope != requirement.scope) {
                tc::Log::error("[%s] shader '%s' resource '%s' scope mismatch: contract=%u layout=%u",
                               context_name(options),
                               shader_name(shader),
                               requirement.name,
                               requirement.scope,
            ...
            if (!stage_mask_covers(binding->stage_mask, requirement.stage_mask)) {
                tc::Log::error("[%s] shader '%s' resource '%s' stage mismatch: contract=0x%x layout=0x%x",
                               context_name(options),
                               shader_name(shader),
                               requirement.name,
            ...
            if (requirement.size > 0 && binding->size > 0 && binding->size < requirement.size) {
                tc::Log::error("[%s] shader '%s' resource '%s' layout size %u is smaller than contract size %u",
                               context_name(options),
                               shader_name(shader),
                               requirement.name,
                               binding->size,
    ...
    bool validate_shader_contract(const tc_shader* shader, const ShaderContractValidationOptions& options) {
        if (!shader) {
            tc::Log::error("[%s] cannot validate shader contract: shader is null", context_name(options));
            return false;
        }
        ...
        if (!tc_shader_get_contract_view(shader, &contract)) {
            if (options.require_contract) {
                tc::Log::error("[%s] shader '%s' is on a migrated render path but has no tc_shader_contract",
                               context_name(options),
                               shader_name(shader));
                return false;
            }
        ...
        if (!tc_shader_has_resource_layout(shader)) {
            tc::Log::error("[%s] shader '%s' has tc_shader_contract resources but no known resource layout",
                           context_name(options),
                           shader_name(shader));
            return false;
        }
    ...
    bool validate_shader_contract(tc_shader_handle shader_handle, const ShaderContractValidationOptions& options) {
        tc_shader* shader = tc_shader_get(shader_handle);
        if (!shader) {
            tc::Log::error("[%s] shader handle is stale (index=%u gen=%u)",
                           context_name(options),
                           shader_handle.index,
