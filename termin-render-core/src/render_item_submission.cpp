...
namespace termin {
    ...
    bool set_render_item_inline_uniform(tc_render_item& item, const char* name, const void* data, uint32_t size) {
        if (!name || name[0] == '\0' || !data || size == 0u || size > TC_RENDER_ITEM_INLINE_UNIFORM_DATA_CAPACITY) {
            return false;
        }
    ...
    namespace {
        ...
        using RegisteredRenderItemDrawEncoderPtr = std::shared_ptr<const RegisteredRenderItemDrawEncoder>;

        std::unordered_map<uint32_t, RegisteredRenderItemDrawEncoderPtr>& render_item_draw_encoder_registry() {
            static std::unordered_map<uint32_t, RegisteredRenderItemDrawEncoderPtr> registry;
            return registry;
        }

        bool find_registered_render_item_draw_encoder(uint32_t item_kind,
        ...
        bool resolve_request_shader(tgfx::RenderContext2& ctx,
            ...
            if (!request.device) {
                tc::Log::error("[%s] skip RenderItem draw for '%s': shader_handle was provided without render device",
                               pass_name,
                               entity_name);
                return false;
            }

            if (!ensure_material_pipeline_shader(ctx, *request.device, request.shader_handle, pass_name, out_binding)) {
                return false;
            }
        ...
        bool bind_request_resources(tgfx::RenderContext2& ctx,
                                    const RenderItemDrawSubmitRequest& request,
            ...
            if (resources.material_resources) {
                prepare_material_pipeline_resources(
                    ctx, *request.device, shader, request.material_phase, *resources.material_resources);
            }
            ...
            if (resources.named_textures) {
                for (uint32_t i = 0; i < resources.named_texture_count; ++i) {
                    ...
                    const tc_shader_resource_binding* rb = tc_shader_find_resource_binding(shader, texture.name);
                    if (!rb || rb->kind != TC_SHADER_RESOURCE_TEXTURE) {
                        if (texture.only_if_shader_has_resource && !rb) {
                            continue;
                        }
                        tc::Log::error("[%s] skip extra texture '%s' for '%s': shader does not declare a Texture2D "
        ...
        bool mesh_render_item_draw_encoder(tgfx::RenderContext2& ctx,
                                           const tc_render_item& item,
                                           const RenderItemDrawSubmitRequest& request,
                                           void* user_data) {
            (void)user_data;

            const char* pass_name = request.debug_pass_name ? request.debug_pass_name : "RenderItemSubmit";
            ...
            if (!resolve_request_shader(ctx, request, pass_name, entity_name, binding, shader)) {
                tc::Log::error(
                    "[%s] skip RenderItem mesh draw for '%s': request has no shader", pass_name, entity_name);
                return false;
            }
            if (!bind_request_resources(ctx, request, shader, pass_name, entity_name)) {
                return false;
            }
            if (!bind_render_item_inline_uniform(ctx, item, shader, pass_name, entity_name)) {
                return false;
            }
        ...
        RenderItemEncoderCapabilities mesh_render_item_capabilities() {
            ...
            capabilities.supported_task_input_mask = render_item_task_input_bit(RenderItemTaskInput::DrawContext) |
                                                     render_item_task_input_bit(RenderItemTaskInput::ModelMatrix) |
                                                     render_item_task_input_bit(RenderItemTaskInput::InlineUniform);
            capabilities.required_task_input_mask = render_item_task_input_bit(RenderItemTaskInput::DrawContext);
            capabilities.requires_draw_context = true;
            capabilities.consumes_common_resources = true;
            return capabilities;
        }

        RenderItemTaskRejection mesh_render_item_task_shader_planner(const RenderItemTaskPlanningRequest& request,
            ...
            out_plan.vertex_transform_kind = (request.item->flags & TC_RENDER_ITEM_FLAG_HAS_SKINNING_MATRICES)
                                                 ? VertexTransformKind::SkinnedMesh
                                                 : VertexTransformKind::StaticMesh;
            if (request.contract) {
                const uint64_t transform_bit = render_item_vertex_transform_kind_bit(out_plan.vertex_transform_kind);
            ...
            if (tc_mesh_handle_is_invalid(mesh_handle)) {
                out_detail = "mesh item has no stable mesh handle";
                return RenderItemTaskRejection::ShaderPlanningRejected;
            }
            tc_mesh* mesh = tc_mesh_get(mesh_handle);
            if (!mesh) {
                out_detail = "mesh handle is stale or invalid";
                return RenderItemTaskRejection::ShaderPlanningRejected;
            }

            const MaterialPipelinePassContract& pass_contract = *request.contract->shader_contract;
            ...
            auto reject_missing_semantic = [&](const char* contract_name, const char* semantic) {
                ...
                bool should_log = false;
                {
                    ...
                    auto [it, inserted] = diagnosed_versions.emplace(diagnostic_key, mesh->header.version);
                    should_log = inserted || it->second != mesh->header.version;
                    it->second = mesh->header.version;
                }
                if (should_log) {
                    tc::Log::error("[RenderItemPlan] skip mesh '%s' in pass '%s': phase contract "
                                   "requires vertex semantic '%s', but the mesh layout does not provide it",
                                   mesh->header.name ? mesh->header.name : mesh->header.uuid,
                                   request.contract->debug_pass_name ? request.contract->debug_pass_name : "<unknown>",
                                   semantic);
                }
            };
            ...
            if (planned.is_valid()) {
                ...
                if (tc_shader_get_contract_view(planned.get(), &shader_contract)) {
                    ...
                    for (uint32_t i = 0; i < shader_contract.vertex_input_count; ++i) {
                        const tc_shader_contract_vertex_input& required = shader_contract.vertex_inputs[i];
                        if (!required.required || tc_vertex_layout_find(&mesh->layout, required.semantic)) {
                            continue;
                        }
                        reject_missing_semantic(planned.uuid(), required.semantic);
                        out_detail = "mesh layout does not satisfy the planned shader vertex-input contract";
                        return RenderItemTaskRejection::MeshVertexInputMismatch;
                    }
                }
            }
            if (!checked_planned_shader && transform && transform->has_value()) {
                for (const MaterialPipelineSemantic& required : transform->value().vertex_inputs.mesh_attributes) {
                    if (tc_vertex_layout_find(&mesh->layout, required.name.c_str())) {
                        continue;
                    }
                    reject_missing_semantic(transform->value().debug_name.c_str(), required.name.c_str());
        ...
        void ensure_builtin_render_item_draw_encoders() {
            ...
            std::call_once(once, []() {
                ...
                std::lock_guard<std::mutex> lock(render_item_draw_encoder_mutex());
                render_item_draw_encoder_registry().emplace(TC_RENDER_ITEM_KIND_MESH, mesh_encoder);
            });
        }

    } // namespace

    bool register_render_item_draw_encoder(uint32_t item_kind, const RenderItemDrawEncoderDesc& desc) {
        if (item_kind == TC_RENDER_ITEM_KIND_INVALID) {
            tc::Log::error("[RenderItemSubmit] cannot register draw encoder for invalid item kind");
            return false;
        }
        if (!desc.encode) {
            tc::Log::error("[RenderItemSubmit] cannot register null draw encoder for item kind %u", item_kind);
            return false;
        }
        if (!desc.plan_task_shader) {
            tc::Log::error("[RenderItemSubmit] cannot register encoder without task shader planner for item kind %u",
                           item_kind);
            return false;
        }
        ...
        if (registry.find(item_kind) != registry.end()) {
            tc::Log::error("[RenderItemSubmit] draw encoder for item kind %u is already registered", item_kind);
            return false;
        }
    ...
    bool unregister_render_item_draw_encoder(uint32_t item_kind, RenderItemDrawEncoderFn encode, void* user_data) {
        ensure_builtin_render_item_draw_encoders();

        if (item_kind == TC_RENDER_ITEM_KIND_MESH) {
            tc::Log::error("[RenderItemSubmit] cannot unregister built-in mesh draw encoder");
            return false;
        }
        ...
        auto it = registry.find(item_kind);
        if (it == registry.end()) {
            tc::Log::error("[RenderItemSubmit] cannot unregister draw encoder for item kind %u: no encoder registered",
                           item_kind);
            return false;
        }
        if (it->second->encode != encode || it->second->user_data != user_data) {
            tc::Log::error("[RenderItemSubmit] cannot unregister draw encoder for item kind %u: callback mismatch",
                           item_kind);
            return false;
        }
        registry.erase(it);
    ...
    bool get_render_item_encoder_capabilities(uint32_t item_kind, RenderItemEncoderCapabilities& out) {
        ensure_builtin_render_item_draw_encoders();
    ...
    bool render_item_encoder_supports_phase(uint32_t item_kind, tc_phase_mask phase) {
        RenderItemEncoderCapabilities capabilities{};
    ...
    const char* render_item_task_rejection_name(RenderItemTaskRejection rejection) {
        switch (rejection) {
        case RenderItemTaskRejection::None:
            return "none";
        case RenderItemTaskRejection::InvalidRequest:
            return "invalid_request";
        case RenderItemTaskRejection::EncoderNotFound:
            return "encoder_not_found";
        case RenderItemTaskRejection::PassOutputUnsupported:
            return "pass_output_unsupported";
        case RenderItemTaskRejection::MaterialPhaseRequired:
            return "material_phase_required";
        case RenderItemTaskRejection::MaterialPhaseForbidden:
            return "material_phase_forbidden";
        case RenderItemTaskRejection::RequiredInputUnsupported:
            return "required_input_unsupported";
        case RenderItemTaskRejection::RequiredInputMissing:
            return "required_input_missing";
        case RenderItemTaskRejection::VertexTransformUnsupported:
            return "vertex_transform_unsupported";
        case RenderItemTaskRejection::PassVertexTransformUnsupported:
            return "pass_vertex_transform_unsupported";
        case RenderItemTaskRejection::MeshVertexInputMismatch:
            return "mesh_vertex_input_mismatch";
        case RenderItemTaskRejection::ShaderPlanningRejected:
            return "shader_planning_rejected";
        }
        return "unknown";
    }

    RenderItemTaskRejection plan_render_item_passthrough_shader(const RenderItemTaskPlanningRequest& request,
                                                                RenderItemTaskShaderPlan& out_plan,
                                                                const char*& out_detail,
                                                                void* user_data) {
        (void)user_data;
    ...
    namespace {
        ...
        uint32_t render_item_available_task_inputs(const tc_render_item& item,
        ...
        void log_task_rejection(const RenderItemTaskPlanningRequest& request,
            ...
            const char* pass_name =
                request.contract && request.contract->debug_pass_name ? request.contract->debug_pass_name : "<unknown>";
            const char* encoder_name =
                encoder && !encoder->debug_name.empty() ? encoder->debug_name.c_str() : "<unregistered>";
            tc::Log::error("[RenderItemPlan] reject task: pass='%s' item_kind=%u encoder='%s' reason='%s' detail='%s'",
                           pass_name,
                           request.item ? request.item->kind : static_cast<uint32_t>(TC_RENDER_ITEM_KIND_INVALID),
                           encoder_name,
    ...
    RenderItemTaskPlanningResult plan_render_item_task(const RenderItemTaskPlanningRequest& request,
        ...
        if (contract.material_phase_policy == RenderItemMaterialPhasePolicy::Required && !request.material_phase) {
            result.rejection = RenderItemTaskRejection::MaterialPhaseRequired;
            result.detail = "pass requires a resolved material phase";
            log_task_rejection(request, encoder.get(), result);
            return result;
        }
        if (contract.material_phase_policy == RenderItemMaterialPhasePolicy::Forbidden && request.material_phase) {
        ...
        const uint32_t available_inputs = render_item_available_task_inputs(*request.item, contract);
        const uint32_t required_inputs = encoder->capabilities.required_task_input_mask | contract.required_input_mask;
        ...
        RenderItemTaskRejection planner_rejection =
            encoder->plan_task_shader(request, shader_plan, planner_detail, encoder->user_data);
        ...
        if (shader_plan.has_vertex_transform_kind) {
            const uint64_t transform_bit = render_item_vertex_transform_kind_bit(shader_plan.vertex_transform_kind);
        ...
        for (uint32_t i = 0; i < shader_plan.shader_usage_count; ++i) {
            ...
            task.owned_shader_usages[i] = std::move(shader_plan.owned_shader_usages[i]);
        }
    ...
    bool bind_render_item_common_resources(tgfx::RenderContext2& ctx,
                                           const tc_shader* shader,
                                           const RenderItemDrawSubmitRequest& request) {
        const char* pass_name = request.debug_pass_name ? request.debug_pass_name : "RenderItemSubmit";
        const char* entity_name = request.debug_entity_name ? request.debug_entity_name : "<unnamed>";
        if (!shader) {
            tc::Log::error(
                "[%s] cannot bind RenderItem common resources for '%s': shader layout is null", pass_name, entity_name);
            return false;
        }
        return bind_request_resources(ctx, request, shader, pass_name, entity_name);
    }

    bool bind_render_item_inline_uniform(tgfx::RenderContext2& ctx,
                                         const tc_render_item& item,
                                         const tc_shader* shader,
        ...
        if (!shader) {
            tc::Log::error("[%s] cannot bind inline uniform '%s' for '%s': shader layout is null",
                           pass_name,
                           uniform.name,
                           entity_name);
            return false;
        }
        const tc_shader_resource_binding* binding = tc_shader_find_resource_binding(shader, uniform.name);
        if (!binding || binding->kind != TC_SHADER_RESOURCE_CONSTANT_BUFFER) {
            tc::Log::error("[%s] cannot bind inline uniform '%s' for '%s': shader has no matching constant buffer",
                           pass_name,
                           uniform.name,
                           entity_name);
            return false;
    ...
    bool submit_render_item_draw(tgfx::RenderContext2& ctx,
                                 const tc_render_item& item,
                                 const RenderItemDrawSubmitRequest& request) {
        const char* pass_name = request.debug_pass_name ? request.debug_pass_name : "RenderItemSubmit";
        const char* entity_name = request.debug_entity_name ? request.debug_entity_name : "<unnamed>";
        ...
        tc::Log::error(
            "[%s] skip RenderItem draw for '%s': unsupported item kind %u", pass_name, entity_name, item.kind);
        return false;
    }
