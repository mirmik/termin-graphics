...
namespace termin {
    ...
    tgfx::TextureHandle wrap_tc_texture_as_tgfx2(tgfx::IRenderDevice& device, tc_texture_handle handle) {
    ...
    Tgfx2MeshBinding wrap_mesh_as_tgfx2(tgfx::IRenderDevice& device, tc_mesh* mesh) {
        return tgfx::wrap_mesh_as_tgfx2(device, mesh);
    }

    void release_mesh_binding(tgfx::IRenderDevice& device, const Tgfx2MeshBinding& binding) {
        tgfx::release_mesh_binding(device, binding);
    }

    bool draw_tc_mesh(tgfx::RenderContext2& ctx, tc_mesh* mesh, const tgfx::VertexBufferLayout* layout_override) {
        return tgfx::draw_tc_mesh(ctx, mesh, layout_override);
    }

    bool draw_tc_submesh(tgfx::RenderContext2& ctx,
                         tc_mesh* mesh,
    ...
    bool draw_tc_mesh(tgfx::RenderContext2& ctx,
                      tc_mesh* mesh,
    ...
    bool draw_tc_submesh(tgfx::RenderContext2& ctx,
                         ...
                         std::initializer_list<uint32_t> used_locations,
                         bool use_shader_input_locations) {
        return tgfx::draw_tc_submesh(ctx, mesh, submesh_index, used_locations, use_shader_input_locations);
    }
    ...
    bool draw_tc_submesh(tgfx::RenderContext2& ctx,
                         ...
                         std::initializer_list<std::string_view> used_semantics,
                         bool use_shader_input_locations) {
        return tgfx::draw_tc_submesh(ctx, mesh, submesh_index, used_semantics, use_shader_input_locations);
    }
