...
namespace termin {
    ...
    namespace {
        ...
        static_assert(sizeof(PresenterPushData) == 16, "PresenterPushData must match the shader push block");

    } // namespace

    std::pair<int, int> bounded_frame_graph_capture_dimensions(int width, int height, std::uint32_t max_long_edge) {
        ...
        const double scale = static_cast<double>(max_long_edge) / static_cast<double>(long_edge);
        return {
    ...
    void FrameGraphCapture::ensure_capture_tex(tgfx::IRenderDevice& device, int w, int h, tgfx::PixelFormat fmt) {
        if (device_ == &device && capture_tex_ && width_ == w && height_ == h && format_ == fmt) {
            return;
        }
        ...
        if (tgfx::is_depth_format(fmt)) {
            desc.usage = tgfx::TextureUsage::Sampled | tgfx::TextureUsage::DepthStencilAttachment |
    ...
    bool FrameGraphCapture::read_color_rgba_float(std::vector<float>& out) const {
        out.clear();
        if (!device_ || !capture_tex_ || !captured_ || is_depth() || width_ <= 0 || height_ <= 0) {
            return false;
        }
        ...
        if (pixel_count > std::numeric_limits<size_t>::max() / 4) {
            tc_log(TC_LOG_ERROR, "[FrameGraphCapture] color readback dimensions overflow: %dx%d", width_, height_);
            return false;
        }
        ...
        try {
            ...
            tc_log(TC_LOG_ERROR, "[FrameGraphCapture] exact color readback threw: %s", error.what());
        } catch (...) {
            tc_log(TC_LOG_ERROR, "[FrameGraphCapture] exact color readback threw an unknown exception");
        }
        tc_log(TC_LOG_ERROR,
    ...
    bool FrameGraphCapture::read_depth_float(std::vector<float>& out) const {
        ...
        try {
            ...
            tc_log(TC_LOG_ERROR, "[FrameGraphCapture] exact depth readback threw: %s", error.what());
        } catch (...) {
            tc_log(TC_LOG_ERROR, "[FrameGraphCapture] exact depth readback threw an unknown exception");
        }
        tc_log(TC_LOG_ERROR,
    ...
    void FrameGraphCapture::capture_direct_via_ctx2(tgfx::RenderContext2* ctx2,
        ...
        (void)format;
        if (ctx2->device().backend_type() == tgfx::BackendType::D3D11 && tgfx::is_depth_format(effective) &&
        ...
        std::tie(width, height) = bounded_frame_graph_capture_dimensions(width, height, max_long_edge);
        if (width <= 0 || height <= 0) {
            tc_log(TC_LOG_ERROR,
...
void main() {
    ...
    void FrameGraphPresenter::render_in_current_pass(tgfx::RenderContext2* ctx2, const FrameGraphPresenterDraw& draw) {
    ...
    void FrameGraphPresenter::render(tgfx::RenderContext2* ctx2,
    ...
    HDRStats FrameGraphPresenter::compute_hdr_stats(tgfx::IRenderDevice* device, tgfx::TextureHandle tex) {
        HDRStats stats{};
    ...
    std::vector<uint8_t> FrameGraphPresenter::read_depth_normalized(tgfx::IRenderDevice* device,
    ...
    TextureInfo FrameGraphPresenter::get_texture_info(tgfx::IRenderDevice* device, tgfx::TextureHandle tex) {
        TextureInfo info;
        if (!device || !tex) {
            return info;
        }
