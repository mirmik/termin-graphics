...
namespace termin {
    ...
    std::set<const char*> UnknownPass::compute_reads() const {
        ...
        for (const std::string& value : original_reads)
            result.insert(value.c_str());
        return result;
    }

    std::set<const char*> UnknownPass::compute_writes() const {
        std::set<const char*> result;
        for (const std::string& value : original_writes)
            result.insert(value.c_str());
        return result;
    }
    ...
    void ensure_unknown_pass_registered() {
        auto descriptor = FramePassTypeDescriptorBuilder::native<UnknownPass>("UnknownPass", "termin-render-core");
