...
Во избежание, лучше собирать sdk штатным скриптом (лучше с флагом `--no-wheels`). Оно не так долго собирается, чтобы вручную копировать либы.
...
Также воздержись от использхования `thread_local` переменных в C/C++ коде.
...
Если видишь какую-то регулярно возникающую проблемы, если работа каких-то систем не соответствует твоим ожиданиям - сообщи. 

## Доска
Если доступна проектная доска задач, а она доступна на машинах Aurora и AionShad, (скилл kanboard-taskboard), 
фиксируй найденные баги, регрессы, неоконченные миграции и крупные технические хвосты в тикеты. 
...
Доска задач не заменяет локальную проверку кода и тесты, но должна отражать актуальное состояние известных проблем.
...
Кроме того, стоит агрессивней закрывать, или хотябы переводить во взятые в работу/тестирование, обмозгованные карточки. (А то у нас вечная проблема, что задача по факту сделана, а карточка как висела в бэклоге, так и висит)
