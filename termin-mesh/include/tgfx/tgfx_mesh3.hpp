    ...
    public:
        ...
        void compute_tangents() {
            ...
            if (!has_normals())
                compute_normals();
            if (!has_uvs())
                return; // Can't compute tangents without UVs
            ...
            for (size_t t = 0; t < num_tris; t++) {
                ...
                Vec3f sdir = {
                    (dv2 * e1.x - dv1 * e2.x) * r, (dv2 * e1.y - dv1 * e2.y) * r, (dv2 * e1.z - dv1 * e2.z) * r};
                Vec3f tdir = {
                    (du1 * e2.x - du2 * e1.x) * r, (du1 * e2.y - du2 * e1.y) * r, (du1 * e2.z - du2 * e1.z) * r};
