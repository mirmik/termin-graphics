...
namespace {
    ...
    template <typename T> nb::object make_array_2d(const T* data, size_t rows, size_t cols) {
    ...
    template <typename T> nb::object make_array_1d(const T* data, size_t size) {
        T* buf = new T[size];
...
namespace tmesh_bindings {
    ...
    void bind_mesh(nb::module_& m) {
        ...
        nb::class_<Mesh3>(m, "Mesh3")
                ...
                [](Mesh3* self,
                   ...
                   std::string uuid) {
                    ...
                    if (!normals_obj.is_none()) {
                        auto n_arr =
                            nb::cast<nb::ndarray<float, nb::shape<-1, 3>, nb::c_contig, nb::device::cpu>>(normals_obj);
                    ...
                    if (!uvs_obj.is_none()) {
                        auto uv_arr =
                            nb::cast<nb::ndarray<float, nb::shape<-1, 2>, nb::c_contig, nb::device::cpu>>(uvs_obj);
            ...
            .def_prop_rw(
                "name", [](const Mesh3& m) { return m.name; }, [](Mesh3& m, const std::string& n) { m.name = n; })

            .def_prop_rw(
                "uuid", [](const Mesh3& m) { return m.uuid; }, [](Mesh3& m, const std::string& u) { m.uuid = u; })
            ...
            .def("__repr__", [](const Mesh3& m) {
                return "<Mesh3 vertices=" + std::to_string(m.vertex_count()) +
                       " triangles=" + std::to_string(m.triangle_count()) + " name=\"" + m.name + "\"" + " uuid=\"" +
                       m.uuid + "\">";
            });
        ...
        nb::enum_<tc_draw_mode>(m, "TcDrawMode").value("TRIANGLES", TC_DRAW_TRIANGLES).value("LINES", TC_DRAW_LINES);
        ...
        nb::class_<tc_submesh>(m, "TcSubmesh")
            ...
            .def_prop_rw(
                "draw_mode",
                [](const tc_submesh& self) { return static_cast<tc_draw_mode>(self.draw_mode); },
                [](tc_submesh& self, tc_draw_mode mode) { self.draw_mode = static_cast<uint8_t>(mode); })
            .def_prop_rw(
                "name",
                [](const tc_submesh& self) { return std::string(self.name); },
            ...
            .def("__repr__", [](const tc_submesh& self) {
                return "<TcSubmesh first_index=" + std::to_string(self.first_index) +
                       ...
                       " material_slot=" + std::to_string(self.material_slot) + " name=" + std::string(self.name) + ">";
            });

        nb::class_<tc_vertex_layout>(m, "TcVertexLayout")
            .def("__init__",
                 [](tc_vertex_layout* self) {
            ...
            .def(
                ...
                [](tc_vertex_layout& self,
                   ...
                   uint8_t size,
                   tc_attrib_type type,
                   uint8_t location) { return tc_vertex_layout_add(&self, name.c_str(), size, type, location); },
                nb::arg("name"),
                nb::arg("size"),
                nb::arg("type"),
                nb::arg("location"))
            .def(
                "find",
                [](const tc_vertex_layout& self, const std::string& name) -> nb::object {
        ...
        nb::class_<tc_mesh>(m, "TcMeshData")
            ...
            .def("get_vertices_buffer",
                 [](const tc_mesh& m) -> nb::object {
        ...
        nb::class_<TcMesh>(m, "TcMesh")
            ...
            .def_prop_rw("draw_mode", &TcMesh::draw_mode, &TcMesh::set_draw_mode)
            .def_prop_ro("mesh",
                         [](const TcMesh& h) -> nb::object {
                             tc_mesh* m = h.get();
                             if (!m)
                                 return nb::none();
                             return nb::cast(m, nb::rv_policy::reference);
                         })
            .def_prop_ro("submeshes",
                         [](const TcMesh& h) {
            ...
            .def(
                "submesh_at",
                [](const TcMesh& h, size_t index) -> nb::object {
            ...
            .def_prop_ro("triangles",
                         [](const TcMesh& h) -> nb::object {
                             tc_mesh* m = h.get();
                             if (!m || !m->indices || m->index_count == 0)
                                 return nb::none();
            ...
            .def_static(
                "from_mesh3",
                [](const Mesh3& mesh, std::string name, std::string uuid) {
            ...
            .def_static(
                ...
                [](nb::ndarray<nb::c_contig, nb::device::cpu> vertices,
                   ...
                   tc_draw_mode draw_mode) {
                    ...
                    create_info.data = TcMeshInterleavedDataView{
                        vertices.data(), vertex_count, indices.data(), indices.size(), &layout};
                ...
                nb::arg("draw_mode") = TC_DRAW_TRIANGLES)
            .def_static(
                "from_interleaved_with_submeshes",
                [](nb::ndarray<nb::c_contig, nb::device::cpu> vertices,
                   size_t vertex_count,
                   nb::ndarray<uint32_t, nb::c_contig, nb::device::cpu> indices,
                   const tc_vertex_layout& layout,
            ...
            .def_static("list_all_names",
                        []() {
                            ...
                            tc_mesh_foreach(
                                [](tc_mesh_handle, tc_mesh* mesh, void* user_data) -> bool {
            ...
            .def("__repr__", [](const TcMesh& h) {
                tc_mesh* m = h.get();
                if (!m)
                    return std::string("<TcMesh invalid>");
                return "<TcMesh vertices=" + std::to_string(m->vertex_count) +
                       " triangles=" + std::to_string(m->index_count / 3) + " uuid=" + std::string(m->header.uuid) +
                       ">";
            });
        ...
        m.def(
            ...
            [](nb::ndarray<float, nb::c_contig, nb::device::cpu> vertices,
               nb::ndarray<uint32_t, nb::c_contig, nb::device::cpu> indices) {
                ...
                tc_mesh_compute_uuid(
                    vertices.data(), vertices.size() * sizeof(float), indices.data(), indices.size(), uuid);
        ...
        m.def(
            "tc_mesh_get",
            [](const std::string& uuid) -> std::optional<TcMesh> {
        ...
        m.def(
            ...
            [](TcMesh& handle, const std::vector<tc_submesh>& submeshes) {
                tc_mesh* m = handle.get();
                if (!m)
                    return false;
        ...
        m.def(
            "tc_mesh_contains",
            [](const std::string& uuid) { return tc_mesh_contains(uuid.c_str()); },
            nb::arg("uuid"),
            "Check if mesh exists in registry");

        m.def("tc_mesh_count", []() { return tc_mesh_count(); }, "Get number of meshes in registry");
        ...
        m.def(
            "tc_mesh_is_loaded",
            [](TcMesh& handle) { return tc_mesh_is_loaded(handle.handle); },
            nb::arg("handle"),
            "Check if mesh data is loaded");

        m.def(
            "tc_mesh_ensure_loaded",
            [](TcMesh& handle) { return tc_mesh_ensure_loaded(handle.handle); },
            nb::arg("handle"),
            "Ensure mesh is loaded (triggers callback if needed)");
    }
