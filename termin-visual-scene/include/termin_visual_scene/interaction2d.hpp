...
namespace termin::visual {
    ...
    public:
        ...
        PointerDispatch2D route(const TcVisualScene& scene, const PointerEvent2D& event);
        bool capture(const TcVisualScene& scene, PointerId2D pointer, GraphicItemHandle target);
    ...
    private:
        ...
        static GraphicItemHandle handle_(const std::unordered_map<PointerId2D, GraphicItemHandle>& values,
                                         PointerId2D pointer);
        void reconcile_(const TcVisualScene& scene);

        std::unordered_map<PointerId2D, GraphicItemHandle> hovered_;
        std::unordered_map<PointerId2D, GraphicItemHandle> pressed_;
        std::unordered_map<PointerId2D, GraphicItemHandle> captured_;
    ...
    public:
        void handle(const TcVisualScene& scene, const PointerDispatch2D& dispatch);
        void clear();
        bool select(const TcVisualScene& scene, GraphicItemHandle item);
        bool toggle(const TcVisualScene& scene, GraphicItemHandle item);
        void reconcile(const TcVisualScene& scene);
        const std::vector<GraphicItemHandle>& selection() const noexcept {
            return selection_;
        }
    ...
    public:
        bool handle(TcVisualScene& scene, const PointerDispatch2D& dispatch);
    ...
    private:
        ...
        termin::Affine2f start_transform_ = termin::Affine2f::identity();
