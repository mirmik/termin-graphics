...
namespace termin::visual {
    ...
    public:
        ...
        tc_graphic_item* c_item() noexcept {
            return &item_;
        }
        ...
        const tc_graphic_item* child_at(std::size_t index) const noexcept {
            return tc_graphic_item_child_at_const(&item_, index);
        }
        bool append_child(GraphicItem2D& child);
        bool insert_child(std::size_t index, GraphicItem2D& child);
    ...
    protected:
        GraphicItem2D(const tc_graphic_item_vtable* vtable, const char* declared_type_name);
