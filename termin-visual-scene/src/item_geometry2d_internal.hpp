...
namespace termin::visual::detail {
    ...
    inline bool valid_bounds(termin::Bounds2f bounds) {
        return std::isfinite(bounds.x0) && std::isfinite(bounds.y0) && std::isfinite(bounds.x1) &&
               std::isfinite(bounds.y1) && bounds.x1 >= bounds.x0 && bounds.y1 >= bounds.y0;
    }
    ...
    inline termin::Bounds2f expanded(termin::Bounds2f bounds, float amount) {
        return {
            bounds.x0 - amount,
            bounds.y0 - amount,
            bounds.x1 + amount,
    ...
    inline termin::Bounds2f merged(termin::Bounds2f a, termin::Bounds2f b) {
        return {
            std::min(a.x0, b.x0),
            std::min(a.y0, b.y0),
            std::max(a.x1, b.x1),
    ...
    inline bool bounds_contains(termin::Bounds2f bounds, termin::Vec2f point) {
        return point.x >= bounds.x0 && point.x <= bounds.x1 && point.y >= bounds.y0 && point.y <= bounds.y1;
    }

    inline bool rect_contains(termin::Rect2f rect, termin::Vec2f point) {
    ...
    inline bool ellipse_contains(termin::Rect2f bounds, termin::Vec2f point) {
        ...
        const float nx = (point.x - (bounds.x + bounds.width * 0.5f)) / (bounds.width * 0.5f);
        const float ny = (point.y - (bounds.y + bounds.height * 0.5f)) / (bounds.height * 0.5f);
