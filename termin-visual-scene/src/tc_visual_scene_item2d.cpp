...
namespace {
    ...
    tc_graphic_item* resolve(tc_visual_scene_handle scene, tc_graphic_item_handle item, const char* operation) {
    ...
    GraphicItem2D* resolve_object(tc_visual_scene_handle scene, tc_graphic_item_handle item, const char* operation) {
    ...
    bool make_path(tc_visual_path2d_view source, tgfx::Path2f& out) {
        ...
        const std::span<const termin::Vec2f> points{source.points, source.point_count};
...
bool tc_visual_scene_item_is_valid(tc_visual_scene_handle scene, tc_graphic_item_handle item) {
    return tc_visual_scene_resolve_item(scene, item) != nullptr;
}

const char* tc_visual_scene_item_type_name(tc_visual_scene_handle scene, tc_graphic_item_handle item) {
    const tc_graphic_item* value = tc_visual_scene_resolve_item_const(scene, item);
    return value != nullptr ? tc_graphic_item_type_name(value) : nullptr;
}

bool tc_visual_scene_item_is_type(tc_visual_scene_handle scene, tc_graphic_item_handle item, const char* type_name) {
    const char* actual = tc_visual_scene_item_type_name(scene, item);
    return actual != nullptr && type_name != nullptr && std::strcmp(actual, type_name) == 0;
}
...
#define TC_VISUAL_ITEM_BOOL_PROPERTY(name)                                                                             \
    bool tc_visual_scene_item_get_##name(tc_visual_scene_handle scene, tc_graphic_item_handle item, bool* out_value) { \
    ...
    bool tc_visual_scene_item_set_##name(tc_visual_scene_handle scene, tc_graphic_item_handle item, bool value) {      \
...
bool tc_visual_scene_item_get_opacity(tc_visual_scene_handle scene, tc_graphic_item_handle item, float* out_opacity) {
...
bool tc_visual_scene_item_set_opacity(tc_visual_scene_handle scene, tc_graphic_item_handle item, float opacity) {
    auto* object = resolve_object(scene, item, "set item opacity");
    if (object == nullptr || !std::isfinite(opacity) || opacity < 0.0f || opacity > 1.0f) {
...
bool tc_visual_scene_item_get_z_order(tc_visual_scene_handle scene, tc_graphic_item_handle item, int64_t* out_z_order) {
...
bool tc_visual_scene_item_set_z_order(tc_visual_scene_handle scene, tc_graphic_item_handle item, int64_t z_order) {
...
tc_graphic_item_handle tc_visual_scene_item_parent(tc_visual_scene_handle scene, tc_graphic_item_handle item) {
    tc_graphic_item* value = resolve(scene, item, "get item parent");
    return value != nullptr && value->parent != nullptr ? value->parent->handle : tc_graphic_item_handle_invalid();
}

size_t tc_visual_scene_item_child_count(tc_visual_scene_handle scene, tc_graphic_item_handle item) {
...
tc_visual_scene_item_child_at(tc_visual_scene_handle scene, tc_graphic_item_handle item, size_t index) {
...
bool tc_visual_scene_item_set_clip_path(tc_visual_scene_handle scene,
    ...
    if (object == nullptr || !owned_rule || !make_path(path, owned_path) || owned_path.empty()) {
...
bool tc_visual_scene_item_set_clip_rect(tc_visual_scene_handle scene, tc_graphic_item_handle item, tc_rect2f rect) {
    ...
    return tc_visual_scene_item_set_clip_path(scene, item, {verbs, 5, points, 4}, TC_VISUAL_FILL_RULE_NON_ZERO);
}

bool tc_visual_scene_item_clear_clip(tc_visual_scene_handle scene, tc_graphic_item_handle item) {
