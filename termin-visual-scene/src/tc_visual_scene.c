...
static bool ensure_scene_pool(void) {
    if (g_scene_pool_initialized)
        return true;
    if (!tc_pool_init(&g_scene_pool, sizeof(tc_visual_scene_slot), 16)) {
        tc_log_error("tc_visual_scene: scene pool initialization failed");
        return false;
    }
...
static tc_handle scene_base_handle(tc_visual_scene_handle handle) {
    return (tc_handle){handle.index, handle.generation};
}

static tc_visual_scene* resolve_scene(tc_visual_scene_handle handle) {
    if (!g_scene_pool_initialized || tc_visual_scene_handle_is_invalid(handle)) {
        return NULL;
    }
    tc_visual_scene_slot* slot = (tc_visual_scene_slot*)tc_pool_get(&g_scene_pool, scene_base_handle(handle));
...
static tc_graphic_item_slot* slot_for(tc_visual_scene* scene, tc_handle handle) {
    return (tc_graphic_item_slot*)tc_pool_get(&scene->items, handle);
}

static const tc_graphic_item_slot* slot_for_const(const tc_visual_scene* scene, tc_handle handle) {
    return (const tc_graphic_item_slot*)tc_pool_get((tc_pool*)&scene->items, handle);
}

static bool valid_handle(const tc_visual_scene* scene, tc_graphic_item_handle handle) {
    if (scene == NULL || tc_graphic_item_handle_is_invalid(handle) || handle.scene_id != scene->id ||
        !tc_pool_is_valid(&scene->items, local_handle(handle))) {
        return false;
    }
    const tc_graphic_item_slot* slot = slot_for_const(scene, local_handle(handle));
...
static bool release_slot(tc_visual_scene* scene, tc_handle handle) {
...
static void destroy_object(tc_visual_scene* scene, tc_graphic_item* item) {
...
tc_visual_scene_handle tc_visual_scene_create(void) {
    ...
    if (!tc_pool_init_ex(&scene->items, sizeof(tc_graphic_item_slot), 16, &config)) {
        tc_log_error("tc_visual_scene_create: pool initialization failed");
        free(scene);
        return tc_visual_scene_handle_invalid();
    }
    ...
    tc_visual_scene_slot* slot = (tc_visual_scene_slot*)tc_pool_get(&g_scene_pool, local);
    slot->scene = scene;
    return (tc_visual_scene_handle){local.index, local.generation};
}
...
uint64_t tc_visual_scene_id(tc_visual_scene_handle handle) {
    const tc_visual_scene* scene = resolve_scene(handle);
    return scene != NULL ? scene->id : 0;
}

size_t tc_visual_scene_item_count(tc_visual_scene_handle handle) {
    const tc_visual_scene* scene = resolve_scene(handle);
    return scene != NULL ? tc_pool_count(&scene->items) : 0;
}

uint64_t tc_visual_scene_order_revision(tc_visual_scene_handle handle) {
    const tc_visual_scene* scene = resolve_scene(handle);
    return scene != NULL ? scene->order_revision : 0;
}

tc_graphic_item_handle tc_visual_scene_adopt_item(tc_visual_scene_handle scene_handle,
                                                  tc_graphic_item* item,
                                                  tc_graphic_item_deleter deleter) {
    tc_visual_scene* scene = resolve_scene(scene_handle);
    if (scene == NULL || item == NULL || deleter == NULL) {
        tc_log_error("tc_visual_scene_adopt_item: scene, item and deleter are required");
        return tc_graphic_item_handle_invalid();
    }
    if (scene->clearing) {
        tc_log_error("tc_visual_scene_adopt_item: scene is clearing");
        deleter(item);
        return tc_graphic_item_handle_invalid();
    }
    if (tc_graphic_item_is_attached(item) || item->deleter != NULL) {
        tc_log_error("tc_visual_scene_adopt_item: item is already attached");
        return tc_graphic_item_handle_invalid();
    }
    if (item->vtable == NULL || item->body == NULL || item->parent != NULL || item->child_count != 0 ||
        item->children != NULL) {
        tc_log_error("tc_visual_scene_adopt_item: item is not an initialized tree root");
        deleter(item);
        return tc_graphic_item_handle_invalid();
    }
    ...
    if (tc_handle_is_invalid(local)) {
        tc_log_error("tc_visual_scene_adopt_item: pool allocation failed");
        deleter(item);
        return tc_graphic_item_handle_invalid();
    }
    ...
    if (!link_runtime_type(item)) {
        tc_log_error("tc_visual_scene_adopt_item: runtime type link failed");
        slot->item = NULL;
        item->deleter = NULL;
        item->scene = NULL;
        item->handle = tc_graphic_item_handle_invalid();
...
bool tc_visual_scene_replace_item(tc_visual_scene_handle scene_handle,
                                  tc_graphic_item_handle handle,
                                  tc_graphic_item* replacement,
                                  tc_graphic_item_deleter deleter) {
    tc_visual_scene* scene = resolve_scene(scene_handle);
    if (!valid_handle(scene, handle) || replacement == NULL || deleter == NULL) {
        if (replacement != NULL && deleter != NULL && !tc_graphic_item_is_attached(replacement)) {
            deleter(replacement);
        }
        return false;
    }
    if (tc_graphic_item_is_attached(replacement) || replacement->deleter != NULL) {
        tc_log_error("tc_visual_scene_replace_item: replacement is already attached");
        return false;
    }
    if (replacement->parent != NULL || replacement->child_count != 0 || replacement->children != NULL ||
        ...
        tc_log_error("tc_visual_scene_replace_item: replacement is not an unattached root");
        deleter(replacement);
        return false;
    }

    tc_graphic_item_slot* slot = slot_for(scene, local_handle(handle));
    ...
    const tc_graphic_item_deleter previous_deleter = previous->deleter;
    previous->scene = NULL;
...
tc_graphic_item* tc_visual_scene_resolve_item(tc_visual_scene_handle scene_handle, tc_graphic_item_handle handle) {
    tc_visual_scene* scene = resolve_scene(scene_handle);
...
const tc_graphic_item* tc_visual_scene_resolve_item_const(tc_visual_scene_handle scene_handle,
                                                          tc_graphic_item_handle handle) {
...
size_t tc_visual_scene_copy_items(tc_visual_scene_handle scene_handle, tc_graphic_item** out_items, size_t capacity) {
    const tc_visual_scene* scene = resolve_scene(scene_handle);
    ...
    while (written < capacity && written < count) {
        ...
        for (uint32_t index = 0; index < scene->items.capacity; ++index) {
            ...
            const tc_graphic_item_slot* slot = slot_for_const(scene, local);
            const tc_graphic_item* candidate = slot != NULL ? slot->item : NULL;
...
size_t tc_visual_scene_copy_item_handles(tc_visual_scene_handle scene_handle,
...
bool tc_visual_scene_destroy_item(tc_visual_scene_handle scene_handle, tc_graphic_item_handle handle) {
    tc_visual_scene* scene = resolve_scene(scene_handle);
    tc_graphic_item* item = valid_handle(scene, handle) ? slot_for(scene, local_handle(handle))->item : NULL;
    if (item == NULL)
        return false;
    tc_visual_scene_touch_order(scene);
