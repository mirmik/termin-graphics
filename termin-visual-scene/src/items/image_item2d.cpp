...
{
    if (uri.empty() ||
        !detail::valid_rect(rect) ||
        !detail::valid_rect(uv) ||
        !tint.is_finite() ||
        (sampling != tgfx::DrawTextureSampling2D::Linear &&
         sampling != tgfx::DrawTextureSampling2D::Nearest)) {
        throw std::invalid_argument(
            "invalid ImageItem2D state");
    }
}
...
ImageItem2D::local_bounds() const {
    return detail::rect_bounds(rect_);
}

bool ImageItem2D::hit_test(
    termin::Vec2f point,
    float) const
{
...
bool ImageItem2D::paint(
    GraphicItemPaintContext2D& context) const
{
    return context.image(
        image_uri_, rect_, uv_, tint_, sampling_);
}
