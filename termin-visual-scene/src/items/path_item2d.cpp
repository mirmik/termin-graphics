...
PathItem2D::local_bounds() const {
    auto result = path_.bounds();
    return stroke_
        ? detail::merged(
              result, path_.stroke_bounds(*stroke_))
        : result;
}

bool PathItem2D::hit_test(
    termin::Vec2f point,
    float) const
{
    const auto flat = path_.flatten();
    return (fill_ && flat.contains(point, fill_->rule)) ||
        (stroke_ && flat.stroke_contains(point, *stroke_));
}

bool PathItem2D::paint(
