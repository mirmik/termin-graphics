...
{
    if (text.empty() || font_uri.empty() ||
        !detail::valid_point(origin) ||
        !std::isfinite(size_px) || size_px <= 0.0f ||
        !color.is_finite() ||
        !detail::valid_bounds(layout_bounds)) {
        throw std::invalid_argument(
            "invalid TextItem2D state");
    }
}
...
tgfx::TextAnchor2D decode_anchor(
    const std::string& value)
{
    if (value == "left") return tgfx::TextAnchor2D::Left;
    if (value == "center") return tgfx::TextAnchor2D::Center;
    if (value == "right") return tgfx::TextAnchor2D::Right;
...
void TextItem2D::set_text(std::string text) {
    validate(
        text, font_uri_, origin_, size_px_,
        color_, layout_bounds_);
    text_ = std::move(text);
}

void TextItem2D::set_font_uri(std::string uri) {
    validate(
        text_, uri, origin_, size_px_,
        color_, layout_bounds_);
...
void TextItem2D::set_origin(termin::Vec2f origin) {
    validate(
        text_, font_uri_, origin, size_px_,
        color_, layout_bounds_);
    origin_ = origin;
}
...
void TextItem2D::set_color(tgfx::Color4f color) {
    validate(
        text_, font_uri_, origin_, size_px_,
        color, layout_bounds_);
    color_ = color;
}

void TextItem2D::set_anchor(
    tgfx::TextAnchor2D anchor)
{
    anchor_ = anchor;
}

void TextItem2D::set_layout_bounds(
...
TextItem2D::local_bounds() const {
    return layout_bounds_;
}

bool TextItem2D::hit_test(
    termin::Vec2f point,
    float) const
{
    return detail::bounds_contains(
        layout_bounds_, point);
}

bool TextItem2D::paint(
