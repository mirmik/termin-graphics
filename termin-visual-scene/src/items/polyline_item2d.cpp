...
void validate(
    const std::vector<termin::Vec2f>& points,
    const tgfx::StrokePaint& stroke)
{
    if (points.size() < 2 || !stroke.validate() ||
        ...
        throw std::invalid_argument(
            "invalid PolylineItem2D state");
    }
}

tgfx::Path2f hit_path(
    const std::vector<termin::Vec2f>& points,
    bool closed)
{
    tgfx::Path2f path;
...
void PolylineItem2D::set_points(
    std::vector<termin::Vec2f> points)
{
...
void PolylineItem2D::set_stroke(
    tgfx::StrokePaint stroke)
{
...
PolylineItem2D::local_bounds() const {
...
bool PolylineItem2D::hit_test(
    termin::Vec2f point,
    float) const
{
    return !points_.empty() &&
        hit_path(points_, closed_).flatten()
            .stroke_contains(point, stroke_);
}

bool PolylineItem2D::paint(
    GraphicItemPaintContext2D& context) const
{
    return context.polyline(
        points_, stroke_, closed_);
}
