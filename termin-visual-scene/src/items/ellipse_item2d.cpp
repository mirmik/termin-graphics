...
namespace termin::visual {
    ...
    EllipseItem2D::EllipseItem2D()
        : NativeGraphicItem2D("termin.visual.Ellipse2D") {}

    EllipseItem2D::EllipseItem2D(termin::Rect2f bounds, tgfx::FillPaint fill, std::optional<tgfx::StrokePaint> stroke)
        : EllipseItem2D() {
    ...
    void EllipseItem2D::set_bounds(termin::Rect2f bounds) {
        validate(bounds, fill_, stroke_);
        bounds_ = bounds;
    }

    void EllipseItem2D::set_fill(tgfx::FillPaint fill) {
        validate(bounds_, fill, stroke_);
        fill_ = std::move(fill);
    }

    void EllipseItem2D::set_stroke(std::optional<tgfx::StrokePaint> stroke) {
    ...
    bool EllipseItem2D::paint(GraphicItemPaintContext2D& context) const {
        return context.ellipse(bounds_, fill_, stroke_);
    }
