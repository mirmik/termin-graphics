...
namespace termin::visual {
    namespace {
        ...
        void validate(termin::Rect2f rect,
                      float radius,
    ...
    RoundedRectItem2D::RoundedRectItem2D()
        : NativeGraphicItem2D("termin.visual.RoundedRect2D") {}

    RoundedRectItem2D::RoundedRectItem2D(termin::Rect2f rect,
                                         float radius,
                                         tgfx::FillPaint fill,
    ...
    void RoundedRectItem2D::set_rect(termin::Rect2f rect) {
    ...
    void RoundedRectItem2D::set_fill(tgfx::FillPaint fill) {
        validate(rect_, radius_, fill, stroke_);
        fill_ = std::move(fill);
    }

    void RoundedRectItem2D::set_stroke(std::optional<tgfx::StrokePaint> stroke) {
    ...
    bool RoundedRectItem2D::hit_test(termin::Vec2f point, float) const {
        return detail::rounded_rect_contains(rect_, radius_, point);
    }

    bool RoundedRectItem2D::paint(GraphicItemPaintContext2D& context) const {
        return context.rounded_rect(rect_, radius_, fill_, stroke_);
    }
