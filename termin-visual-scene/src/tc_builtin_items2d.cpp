...
std::optional<tgfx::StrokePaint> stroke(const tc_visual_stroke_paint2d *value) {
  ...
  if (value->dash_count != 0) {
    dash_pattern.assign(value->dash_pattern,
                        value->dash_pattern + value->dash_count);
  }
...
bool make_path(tc_visual_path2d_view source, tgfx::Path2f &out) {
  ...
  return out.try_assign(
      verbs, std::span<const termin::Vec2f>{source.points, source.point_count});
}

tc_graphic_item *parent_object(tc_visual_scene_handle scene,
...
tc_graphic_item_handle adopt(tc_visual_scene_handle scene,
                             tc_graphic_item_handle parent,
                             std::unique_ptr<Item> object) {
  ...
  if (parent_item != nullptr &&
      (parent_item->native_language != TC_LANGUAGE_CXX ||
       parent_item->body == nullptr)) {
    tc::Log::error("built-in item factory parent has no native C++ body");
    return tc_graphic_item_handle_invalid();
  }
  const auto adopted = TcVisualScene{scene}.adopt(
      std::move(object), parent_item != nullptr
                             ? static_cast<GraphicItem2D *>(parent_item->body)
                             : nullptr);
  return adopted.value_or(tc_graphic_item_handle_invalid());
}

template <typename Item, typename Factory>
tc_graphic_item_handle create(tc_visual_scene_handle scene,
                              tc_graphic_item_handle parent,
                              const char *operation, Factory &&factory) {
  try {
...
template <typename Item, typename Factory>
bool replace(tc_visual_scene_handle scene, tc_graphic_item_handle item,
             const char *expected_type, const char *operation,
             Factory &&factory) {
  ...
  try {
    ...
    if (previous == nullptr || previous->body == nullptr ||
        previous->native_language != TC_LANGUAGE_CXX) {
    ...
    return tc_visual_scene_replace_item(scene, item, raw->c_item(),
...
std::optional<tgfx::DrawTextureSampling2D>
sampling(tc_visual_texture_sampling2d value) {
...
tc_visual_group_item2d_create(tc_visual_scene_handle scene,
                              tc_graphic_item_handle parent) {
  return create<termin::visual::GroupItem2D>(
      scene, parent, "create GroupItem2D",
      [] { return std::make_unique<termin::visual::GroupItem2D>(); });
}

tc_graphic_item_handle
tc_visual_rect_item2d_create(tc_visual_scene_handle scene,
                             tc_graphic_item_handle parent, tc_rect2f rect,
                             tc_visual_fill_paint2d fill_value,
                             const tc_visual_stroke_paint2d *stroke_value) {
  return create<termin::visual::RectItem2D>(
      scene, parent, "create RectItem2D", [&] {
        ...
        return std::make_unique<termin::visual::RectItem2D>(rect, *fill_paint,
                                                            stroke_paint);
      });
}

bool tc_visual_rect_item2d_set(tc_visual_scene_handle scene,
                               tc_graphic_item_handle item, tc_rect2f rect,
                               tc_visual_fill_paint2d fill_value,
                               const tc_visual_stroke_paint2d *stroke_value) {
  return replace<termin::visual::RectItem2D>(
      scene, item, "termin.visual.Rect2D", "set RectItem2D", [&] {
        const auto fill_paint = fill(&fill_value);
        const auto stroke_paint = stroke(stroke_value);
        if (!fill_paint || (stroke_value != nullptr && !stroke_paint)) {
          throw std::invalid_argument("invalid paint");
...
tc_graphic_item_handle tc_visual_path_item2d_create(
    tc_visual_scene_handle scene, tc_graphic_item_handle parent,
    tc_visual_path2d_view path_value, const tc_visual_fill_paint2d *fill_value,
    const tc_visual_stroke_paint2d *stroke_value) {
  return create<termin::visual::PathItem2D>(
      scene, parent, "create PathItem2D", [&] {
        tgfx::Path2f path;
        const auto fill_paint = fill(fill_value);
        const auto stroke_paint = stroke(stroke_value);
        if (!make_path(path_value, path) ||
            (fill_value != nullptr && !fill_paint) ||
            (stroke_value != nullptr && !stroke_paint)) {
          throw std::invalid_argument("invalid path or paint");
        }
        return std::make_unique<termin::visual::PathItem2D>(
            std::move(path), fill_paint, stroke_paint);
      });
}
...
bool tc_visual_path_item2d_set(tc_visual_scene_handle scene,
                               ...
                               const tc_visual_stroke_paint2d *stroke_value) {
  return replace<termin::visual::PathItem2D>(
      scene, item, "termin.visual.Path2D", "set PathItem2D", [&] {
        tgfx::Path2f path;
        const auto fill_paint = fill(fill_value);
        const auto stroke_paint = stroke(stroke_value);
        if (!make_path(path_value, path) ||
...
tc_graphic_item_handle
tc_visual_text_item2d_create(tc_visual_scene_handle scene,
                             tc_graphic_item_handle parent,
                             const tc_visual_text_desc2d *desc) {
  return create<termin::visual::TextItem2D>(
      scene, parent, "create TextItem2D", [&] {
        const auto anchor =
            desc != nullptr ? text_anchor(desc->anchor) : std::nullopt;
        if (desc == nullptr || desc->text == nullptr ||
            desc->font_uri == nullptr || !anchor) {
          throw std::invalid_argument("invalid text descriptor");
        }
        return std::make_unique<termin::visual::TextItem2D>(
            desc->text, desc->font_uri, desc->origin, desc->size_px,
            color(desc->color), *anchor, desc->layout_bounds);
      });
}
...
bool tc_visual_text_item2d_set(tc_visual_scene_handle scene,
                               tc_graphic_item_handle item,
                               const tc_visual_text_desc2d *desc) {
  return replace<termin::visual::TextItem2D>(
      scene, item, "termin.visual.Text2D", "set TextItem2D", [&] {
        const auto anchor =
            desc != nullptr ? text_anchor(desc->anchor) : std::nullopt;
        if (desc == nullptr || desc->text == nullptr ||
            desc->font_uri == nullptr || !anchor) {
...
tc_graphic_item_handle
tc_visual_image_item2d_create(tc_visual_scene_handle scene,
                              tc_graphic_item_handle parent,
                              const tc_visual_image_desc2d *desc) {
  return create<termin::visual::ImageItem2D>(
      scene, parent, "create ImageItem2D", [&] {
        const auto mode =
            desc != nullptr ? sampling(desc->sampling) : std::nullopt;
...
bool tc_visual_image_item2d_set(tc_visual_scene_handle scene,
                                tc_graphic_item_handle item,
                                const tc_visual_image_desc2d *desc) {
  return replace<termin::visual::ImageItem2D>(
      scene, item, "termin.visual.Image2D", "set ImageItem2D", [&] {
        const auto mode =
            desc != nullptr ? sampling(desc->sampling) : std::nullopt;
        if (desc == nullptr || desc->image_uri == nullptr || !mode) {
          throw std::invalid_argument("invalid image descriptor");
...
tc_graphic_item_handle tc_visual_hit_region_item2d_create(
    tc_visual_scene_handle scene, tc_graphic_item_handle parent,
    tc_visual_path2d_view path_value, tc_visual_fill_rule2d rule_value) {
  return create<termin::visual::HitRegionItem2D>(
      scene, parent, "create HitRegionItem2D", [&] {
        tgfx::Path2f path;
        ...
        return std::make_unique<termin::visual::HitRegionItem2D>(
            std::move(path), *rule);
      });
}
