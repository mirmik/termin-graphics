...
int main() {
    ...
    root_ptr->set_local_transform(
        termin::Affine2f::translation(10.0f, 20.0f)
        * termin::Affine2f::shear(0.3f, 0.2f));
    root_ptr->set_opacity(0.5f);
    root_ptr->set_clip(
        GeometricClip2D{box(100.0f), tgfx::FillRule::NonZero});

    auto rect = std::make_unique<RectItem2D>(
        termin::Rect2f{0.0f, 0.0f, 10.0f, 12.0f},
        tgfx::FillPaint{},
        std::nullopt);
    rect->set_clip(
        GeometricClip2D{box(30.0f), tgfx::FillRule::EvenOdd});
    rect->set_z_order(7);
    assert(scene.adopt(std::move(rect), root_ptr));
    assert(scene.adopt(
        std::make_unique<PathItem2D>(
            box(8.0f), tgfx::FillPaint{}, std::nullopt),
        root_ptr));
    assert(scene.adopt(
    ...
    assert(scene.adopt(
        std::make_unique<CustomBatchItem2D>(
            "batch://plot",
            termin::Bounds2f{0.0f, 0.0f, 2.0f, 2.0f}),
        root_ptr));
