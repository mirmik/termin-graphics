...
int main() {
  ...
  const auto group =
      tc_visual_group_item2d_create(scene, tc_graphic_item_handle_invalid());
  ...
  const auto rect = tc_visual_rect_item2d_create(
      scene, group, {1.0f, 2.0f, 40.0f, 30.0f}, white, &black);
  ...
  assert(tc_visual_scene_item_set_transform(
      scene, rect, tc_affine2f_translation(5.0f, 7.0f)));
  ...
  assert(tc_visual_scene_item_set_clip_rect(scene, rect,
  ...
  assert(tc_visual_rect_item2d_set(scene, rect, {0.0f, 0.0f, 10.0f, 10.0f},
                                   white, nullptr));
  const auto *rect_base = tc_visual_scene_resolve_item_const(scene, rect);
  assert(rect_base != nullptr);
  assert(static_cast<const termin::visual::GraphicItem2D *>(rect_base->body)
             ->clip()
             .has_value());
  ...
  const auto path =
      tc_visual_path_item2d_create(scene, group, triangle(), &white, &black);
  ...
  const auto hit = tc_visual_hit_region_item2d_create(
      scene, group, triangle(), TC_VISUAL_FILL_RULE_EVEN_ODD);
  ...
  assert(tc_visual_scene_item_set_parent(scene, rect,
                                         tc_graphic_item_handle_invalid(), 0));
  assert(tc_graphic_item_handle_is_invalid(
      tc_visual_scene_item_parent(scene, rect)));
