...
int main() {
    ...
    assert(tc_graphic_item_append_child(
    ...
    child->item.local_transform =
        tc_affine2f_translation(12.0f, -3.0f);
    child->item.visible = false;
    child->item.opacity = 0.5f;
    child->item.z_order = 17;

    auto* replacement =
        make_payload(20, deletes, destroys);
    ...
    const auto reused_result =
        scene.adopt(&reused->item, delete_payload);
