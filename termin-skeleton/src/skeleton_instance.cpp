...
namespace termin {
    ...
    SkeletonInstance::SkeletonInstance(tc_skeleton* skeleton, std::vector<Entity> bone_entities, Entity skeleton_root)
        : _skeleton(skeleton),
    ...
    void SkeletonInstance::update(Entity skinning_root) {
        ...
        for (size_t i = 0; i < _skeleton->bone_count; ++i) {
            ...
            if (bone.index >= static_cast<int>(_bone_entities.size()))
                continue;
