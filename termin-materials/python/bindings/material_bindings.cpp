...
namespace termin {
    ...
    namespace {
        ...
        bool source_contains_any(const std::string& source, std::initializer_list<const char*> tokens) {
        ...
        tc_shader_artifact_policy artifact_policy_for_language(tc_shader_language language) {
            return language == TC_SHADER_LANGUAGE_SLANG ? TC_SHADER_ARTIFACT_REQUIRED : TC_SHADER_ARTIFACT_OPTIONAL;
        }
        ...
        const tc_shader_abi_resource_decl& require_shader_abi_resource(uint32_t id, const char* context) {
        ...
        bool contract_has_vertex_input(const std::vector<tc_shader_contract_vertex_input>& inputs,
                                       const char* semantic) {
        ...
        void append_contract_vertex_input(std::vector<tc_shader_contract_vertex_input>& inputs,
                                          const char* semantic,
                                          uint32_t type) {
        ...
        std::vector<tc_shader_contract_vertex_input> infer_material_vertex_contract(const ShaderPhase& shader_phase) {
            ...
            if (source_contains_any(
                    source, {": TEXCOORD0", ":TEXCOORD0", ": TEXCOORD", ":TEXCOORD", "a_uv", "in_uv", "a_texcoord"})) {
        ...
        void append_resource_requirement(std::vector<tc_shader_resource_requirement>& requirements,
        ...
        void append_abi_resource_requirement(std::vector<tc_shader_resource_requirement>& requirements,
                                             uint32_t id,
                                             uint32_t stage_mask,
                                             uint32_t size = 0,
                                             const tc_shader_resource_field* fields = nullptr,
                                             uint32_t field_count = 0) {
            const tc_shader_abi_resource_decl& abi = require_shader_abi_resource(id, "append_abi_resource_requirement");
        ...
        std::vector<tc_shader_resource_field> material_fields_from_layout(const MaterialUboLayout& layout) {
        ...
        std::vector<tc_shader_resource_requirement>
        parser_shader_resource_requirements(const ShaderPhase& shader_phase,
            ...
            requirements.reserve((layout.empty() ? 0u : 1u) + (shader_phase.uses_engine_per_frame ? 1u : 0u) +
                                 (shader_phase.uses_engine_draw_data ? 1u : 0u) +
                                 shader_phase.material_texture_resources.size());

            if (!layout.empty()) {
                append_abi_resource_requirement(requirements,
            ...
            if (shader_phase.uses_engine_per_frame) {
                append_abi_resource_requirement(requirements, TC_SHADER_ABI_RESOURCE_PER_FRAME, STAGE_ALL_GRAPHICS);
            }
            if (shader_phase.uses_engine_draw_data) {
                append_abi_resource_requirement(requirements, TC_SHADER_ABI_RESOURCE_DRAW_DATA, STAGE_ALL_GRAPHICS, 64);
            }
        ...
        parser_surface_producer_desc(const ShaderPhase& shader_phase,
                                     const std::vector<tc_shader_resource_field>& material_fields,
            ...
            for (const SurfaceFragmentInput& parsed_input : producer.required_fragment_inputs) {
                tc_shader_fragment_input input{};
                std::strncpy(input.semantic, parsed_input.semantic.c_str(), TC_SHADER_RESOURCE_NAME_MAX - 1);
                input.semantic[TC_SHADER_RESOURCE_NAME_MAX - 1] = '\0';
            ...
            for (const SurfaceProducerResourceDecl& parsed_resource : producer.resources) {
                ResourceRequirementInfo info;
                ...
                if (parsed_resource.name == TC_SHADER_RESOURCE_MATERIAL) {
                    info.fields = material_fields.empty() ? nullptr : material_fields.data();
                    info.field_count = static_cast<uint32_t>(material_fields.size());
                }
            ...
            desc.fragment_inputs = fragment_inputs.empty() ? nullptr : fragment_inputs.data();
            desc.fragment_input_count = static_cast<uint32_t>(fragment_inputs.size());
            desc.resources = resources.empty() ? nullptr : resources.data();
            desc.resource_count = static_cast<uint32_t>(resources.size());
            return desc;
        }

        void apply_parser_shader_contract(tc_shader* shader,
            ...
            std::vector<tc_shader_contract_vertex_input> vertex_inputs = infer_material_vertex_contract(shader_phase);
            std::vector<tc_shader_resource_field> material_fields = material_fields_from_layout(layout);
            std::vector<tc_shader_resource_requirement> resources =
                parser_shader_resource_requirements(shader_phase, layout, material_fields);
            ...
            if (!tc_shader_set_contract(shader, &desc)) {
                tc::Log::error("Failed to attach shader parser contract to material shader '%s'",
                               shader->name ? shader->name : shader->uuid);
            }
        }

        void apply_parser_resource_layout(tc_shader* shader,
                                          const ShaderPhase& shader_phase,
                                          const MaterialUboLayout& layout) {
            if (!layout.empty()) {
                ...
                tc_shader_set_material_ubo_layout(
                    shader, entries.data(), static_cast<uint32_t>(entries.size()), layout.block_size);
            } else {
        ...
        TcMaterial create_material_from_parsed(const ShaderMultyPhaseProgramm& program,
            ...
            TcMaterial mat = TcMaterial::create(
                options.name.is_none() ? program.program : nb::cast<std::string>(options.name), options.shader_uuid);
            if (!mat.is_valid()) {
            ...
            TcTexture white_tex = optional_tc_texture(options.default_white_texture,
                                                      "create_material_from_parsed(default_white_texture)");
            TcTexture normal_tex = optional_tc_texture(options.default_normal_texture,
            ...
            for (const auto& shader_phase : program.phases) {
                ...
                if (shader_phase.surface_producer.has_value()) {
                    producer_desc = parser_surface_producer_desc(
                        shader_phase, producer_material_fields, producer_fragment_inputs, producer_resources);
                ...
                // Set available marks
                phase->available_mark_count =
                    std::min(shader_phase.available_marks.size(), (size_t)TC_MATERIAL_MAX_MARKS);
                ...
                apply_parser_resource_layout(tc_shader_get(phase->shader), shader_phase, layout);
                ...
                for (const auto& prop : shader_uniforms) {
                    if (std::holds_alternative<std::monostate>(prop.default_value))
                        continue;
                ...
                for (const auto& prop : shader_uniforms) {
                    if (prop.property_type == "Texture") {
                        const bool has_expected_encoding = prop.expected_texture_encoding.has_value();
                        const tc_texture_encoding expected =
                            has_expected_encoding ? tgfx::to_tc_texture_encoding(*prop.expected_texture_encoding)
                                                  : TC_TEXTURE_ENCODING_LINEAR;
                        const bool declared =
                            has_expected_encoding
                                ? tc_material_phase_declare_texture(phase, prop.name.c_str(), expected)
                                : tc_material_phase_declare_texture_slot(phase, prop.name.c_str());
                        if (!declared) {
                            throw std::runtime_error("Failed to declare texture property '" + prop.name + "'");
                        }
                        ...
                        if (default_tex_name != "white" && default_tex_name != "normal") {
                            throw std::runtime_error("Texture property '" + prop.name + "' has unsupported default '" +
                                                     default_tex_name + "'");
                        }
                        if (default_tex_name == "normal" && has_expected_encoding &&
                            expected != TC_TEXTURE_ENCODING_LINEAR) {
                            throw std::runtime_error("Texture property '" + prop.name +
                                                     "' cannot use normal default with sRGB encoding");
                        }

                        TcTexture& default_texture =
                            default_tex_name == "normal"
                                ? normal_tex
                                : (expected == TC_TEXTURE_ENCODING_SRGB ? srgb_white_tex : white_tex);
                        if (default_texture.is_valid() &&
                            !tc_material_phase_set_texture(phase, prop.name.c_str(), default_texture.handle)) {
                            throw std::runtime_error("Failed to bind default texture for '" + prop.name + "'");
                        }
                    }
                }
                    ...
                    for (auto item : tex_dict) {
                        ...
                        if (nb::isinstance<TcTexture>(val)) {
                            ...
                            if (!tc_material_phase_set_texture(phase, key.c_str(), tex.handle)) {
                                throw std::runtime_error("Failed to bind texture '" + key + "'");
                            }
                        } else {
        ...
        void configure_shader_from_parsed(TcShader& shader,
            ...
            if (vertex == phase.stages.end() || fragment == phase.stages.end()) {
                tc::Log::error("Shader phase '%s' has no complete vertex/fragment stage pair",
            ...
            if (std::string(shader.vertex_source()) != sources.vertex ||
                ...
                tc::Log::error("Failed to publish sources for shader phase '%s'", phase.phase_mark.c_str());
            ...
            if (phase.surface_producer.has_value()) {
                ...
                std::vector<tc_shader_fragment_input> producer_fragment_inputs;
                std::vector<tc_shader_resource_requirement> producer_resources;
                const tc_shader_surface_producer_desc producer_desc = parser_surface_producer_desc(
                    phase, producer_material_fields, producer_fragment_inputs, producer_resources);
                if (!tc_shader_set_surface_producer(raw, &producer_desc)) {
                    tc::Log::error("Failed to publish surface producer for shader phase '%s'",
                                   phase.phase_mark.c_str());
                    throw std::runtime_error("Failed to publish parsed surface producer");
                }
            } else {
                tc_shader_clear_surface_producer(raw);
            }
            if (!tc_shader_has_contract(raw)) {
                tc::Log::error("Failed to publish interface contract for shader phase '%s'", phase.phase_mark.c_str());
                throw std::runtime_error("Failed to publish parsed shader contract");
            }
        }

        // NOLINTNEXTLINE(readability-function-size): preserves the Python API; implementation uses
        // ParsedMaterialCreateOptions.
        TcMaterial create_material_from_parsed_py(const ShaderMultyPhaseProgramm& program,
                                                  nb::object color,
                                                  nb::object textures,
                                                  nb::object uniforms,
                                                  nb::object name,
    ...
    void bind_tc_material(nb::module_& m) {
        ...
        nb::class_<tc_material_phase>(m, "TcMaterialPhase")
            ...
            .def(
                "declare_texture",
                [](tc_material_phase& p, const char* name, nb::object encoding) {
                    if (encoding.is_none()) {
                        if (!tc_material_phase_declare_texture_slot(&p, name)) {
                            throw std::runtime_error("failed to declare texture slot '" + std::string(name) + "'");
                        }
                        return;
                    }
            ...
            .def("get_available_marks",
                 [](tc_material_phase& p) -> std::vector<std::string> {
            ...
            .def(
                "set_phase_mark",
                [](tc_material_phase& p, const std::string& mark) {
            ...
            .def(
                "set_param",
                [](tc_material_phase& p, const std::string& name, nb::object value) {
                    if (nb::isinstance<nb::bool_>(value)) {
                        ...
                        tc_material_phase_set_uniform(&p, name.c_str(), TC_UNIFORM_MAT4, matrix.data);
                        ...
                        if (size == 2) {
                            ...
                            tc::Log::error("tc_material_phase.set_param('%s') rejects raw 16-element buffers: "
        ...
        nb::class_<TcMaterial>(m, "TcMaterial")
            ...
            .def("__init__",
                 [](TcMaterial* self, nb::kwargs kwargs) {
            ...
            .def_static("create", &TcMaterial::create, nb::arg("name"), nb::arg("uuid_hint") = "")
            .def(
                "copy",
                [](const TcMaterial& self, const std::string& new_uuid) { return TcMaterial::copy(self, new_uuid); },
                nb::arg("new_uuid") = "")
            .def_prop_ro("is_valid", &TcMaterial::is_valid)
            .def_prop_ro("uuid", &TcMaterial::uuid)
            .def_prop_rw("name", &TcMaterial::name, &TcMaterial::set_name)
            .def_prop_ro("version", &TcMaterial::version)
            .def_prop_rw("shader_name", &TcMaterial::shader_name, &TcMaterial::set_shader_name)
            ...
            .def("set_shader_program_dependency",
                 &TcMaterial::set_shader_program_dependency,
                 nb::arg("uuid"),
                 nb::arg("version"))
            .def_prop_rw("source_path", &TcMaterial::source_path, &TcMaterial::set_source_path)
            .def_prop_ro("phase_count", &TcMaterial::phase_count)
            .def(
                "get_phase",
                [](TcMaterial& self, size_t index) -> tc_material_phase* { return self.get_phase(index); },
                nb::arg("index"),
            ...
            .def(
                ...
                [](TcMaterial& self, TcShader& shader, const std::string& phase_mark, int priority)
                    -> tc_material_phase* {
                    if (!shader.is_valid()) {
                        tc::Log::error("Failed to add phase '%s' to material '%s': shader is invalid",
                                       phase_mark.c_str(),
                                       self.name());
                        throw std::runtime_error("TcMaterial.add_phase requires a valid TcShader");
                    }

                    tc_material_phase* phase = self.add_phase(shader, phase_mark.c_str(), priority);
                    if (!phase) {
                        tc::Log::error("Failed to add phase '%s' to material '%s'", phase_mark.c_str(), self.name());
            ...
            .def(
                ...
                [](TcMaterial& self,
                   ...
                   const std::string& geometry_entry) -> tc_material_phase* {
            ...
            .def("set_uniform_mat4",
                 [](TcMaterial& self, const char* name, const Mat44f& mat) { self.set_uniform_mat4(name, mat); })
            .def("set_uniform_mat4",
                 [](TcMaterial& self, const char* name, const Mat44& mat) {
                     self.set_uniform_mat4(name, mat.to_float());
                 })
            .def(
                "set_texture",
                [](TcMaterial& self, const char* name, TcTexture& tex) -> size_t {
        ...
        m.def("create_material_from_parsed",
              &create_material_from_parsed_py,
              nb::arg("program"),
        ...
        m.def("configure_shader_from_parsed",
        ...
        m.def(
            "tc_material_count",
            []() -> size_t { return tc_material_count(); },
