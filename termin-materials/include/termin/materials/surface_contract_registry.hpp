...
namespace termin {
    ...
    public:
        static bool register_contract(const SurfaceContractDescriptor& descriptor, std::string_view owner);
        static std::optional<SurfaceContractDescriptor> find(const SurfaceContractKey& key);
        static std::optional<std::string> owner_of(const SurfaceContractKey& key);
        static bool unregister_contract(const SurfaceContractKey& key, std::string_view owner);
