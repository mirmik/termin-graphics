...
int main() {
    ...
    assert(SurfaceContractRegistry::register_contract(descriptor, "game-cpp-probe"));

    descriptor.debug_name = "mutated caller descriptor";
    const auto stored = SurfaceContractRegistry::find(SurfaceContractKey{"game.surface.cpp-probe", 7});
    assert(stored);
    ...
    assert(SurfaceContractRegistry::unregister_contract(stored->key, "game-cpp-probe"));
