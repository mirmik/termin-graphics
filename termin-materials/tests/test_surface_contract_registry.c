...
static tc_surface_contract_desc descriptor(
    const char* id,
    uint32_t version,
...
int main(void) {
    ...
    const tc_surface_contract_desc* standard =
        tc_surface_contract_registry_find(standard_key);
    assert(standard);
    ...
    assert(strcmp(
        tc_surface_contract_registry_owner(standard_key),
        "termin-materials"
    ) == 0);
    ...
    tc_surface_contract_desc version_one = descriptor(
        id,
        1,
        debug_name,
        type_name,
    ...
    const tc_surface_contract_desc conflict = descriptor(
        "game.surface.weathered",
        1,
        "Weathered v1",
        "GameWeatheredSurfaceV1",
    ...
    const tc_surface_contract_desc version_two = descriptor(
        "game.surface.weathered",
        2,
        "Weathered v2",
    ...
    assert(tc_surface_contract_registry_find(
        (tc_surface_contract_key){"game.surface.weathered", 2}
    ));

    assert(!tc_surface_contract_registry_unregister(
        "wrong-owner",
        version_one_key
    ));
    ...
    const tc_surface_contract_desc module_owned = descriptor(
        "game.surface.module-owned",
        1,
    ...
    assert(tc_surface_contract_registry_register(
        "synthetic-project-module",
        &module_owned
    ));
    assert(tc_runtime_type_registry_unregister_owner(
        "synthetic-project-module"
    ) == 1);
