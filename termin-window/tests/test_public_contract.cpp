...
namespace {
    ...
    template <typename T>
    concept ExposesGraphicsDevice = requires(T& value) { value.device(); };

    template <typename T>
    concept ExposesGraphicsContext = requires(T& value) { value.context(); };
...
int main() {
    ...
    if (termin::WindowHandle{})
        return 1;
    if (!(termin::WindowHandle{1, 2} == termin::WindowHandle{1, 2}))
        return 1;
    ...
    window.set_event_handler([&handled](const termin::WindowEvent& event) {
        handled = event.type == termin::WindowEventType::KeyPressed && event.key.key == termin::WindowKey::A;
    });
    window.poll_events();
    if (!handled)
        return 1;
