...
namespace {
    ...
    bool has_current_gl_bootstrap() {
        return SDL_GL_GetCurrentContext() != nullptr && SDL_GL_GetCurrentWindow() != nullptr;
    }
...
int main() {
    try {
        ...
        // and must release it before deleting the bootstrap context/window.
        {
            ...
            if (!check(has_current_gl_bootstrap(), "constructor did not install its GL bootstrap context"))
                return 1;
        }
        if (!check(has_no_current_gl_bootstrap(), "destructor left the prepared-device bootstrap current"))
            return 1;
        ...
        std::unique_ptr<tgfx::GraphicsHost> graphics;
        {
            ...
            graphics->close();
            if (!check(has_current_gl_bootstrap(), "closed GraphicsHost unexpectedly removed the platform bootstrap"))
                return 1;
        }
        if (!check(has_no_current_gl_bootstrap(), "destructor fallback left the adopted-device bootstrap current"))
            return 1;
        graphics.reset();
        ...
        // The regular close path shares the same idempotent resource release.
        {
            ...
            if (!check(has_no_current_gl_bootstrap(), "close left the GL bootstrap current"))
                return 1;
        }
        if (!check(has_no_current_gl_bootstrap(), "destructor repeated close incorrectly"))
            return 1;
        return 0;
