...
namespace termin {
    ...
    struct WindowManager::Impl {
        ...
        void require_open(const char* operation) const {
            if (!open || !session) {
                manager_logic_error(std::string("WindowManager::") + operation + " called after close");
            }
            if (session->is_closed()) {
                manager_logic_error(std::string("WindowManager::") + operation +
                                    " called after WindowedGraphicsSession close");
            }
        }

        Slot* find(WindowHandle handle) {
            if (!handle || handle.slot > slots.size())
                return nullptr;
            Slot& slot = slots[handle.slot - 1];
            if (!slot.window || slot.generation != handle.generation)
                return nullptr;
            return &slot;
        }
        ...
        Slot& require_slot(WindowHandle handle, const char* operation) {
            ...
            if (!slot) {
                manager_invalid_argument(std::string("WindowManager::") + operation +
    ...
    WindowHandle WindowManager::create_window(const WindowConfig& config) {
        ...
        if (!created) {
            manager_logic_error("WindowManager::create_window received a null BackendWindow");
        }
        if (&created->graphics_host() != &impl_->session->graphics()) {
            created->close();
            manager_logic_error("WindowManager::create_window received a window from another GraphicsHost");
        }
