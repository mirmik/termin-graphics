...
namespace termin {
    ...
    void BackendWindow::set_event_handler(std::function<void(const WindowEvent&)> handler) {
        event_handler_ = std::move(handler);
    }

    WindowedGraphicsSession::WindowedGraphicsSession(BackendWindowSystemPtr windows,
                                                     std::unique_ptr<tgfx::GraphicsHost> graphics)
        : windows_(std::move(windows)),
          graphics_(std::move(graphics)) {
        if (!windows_ || !graphics_) {
            throw std::invalid_argument("WindowedGraphicsSession requires a window system and GraphicsHost");
        }
    }
    ...
    WindowedGraphicsSession::~WindowedGraphicsSession() {
        ...
        if (active_window_managers_ != 0) {
            tc_log_error("[WindowedGraphicsSession] destroyed with %zu live WindowManager instance(s)",
                         active_window_managers_);
            return;
        }
        if (windows_ && windows_->live_window_count() != 0) {
            tc_log_error("[WindowedGraphicsSession] destroyed with %zu live presentation window(s)",
    ...
    void WindowedGraphicsSession::attach_window_manager() {
        if (closed_) {
            tc_log_error("[WindowedGraphicsSession] cannot attach WindowManager after close");
            throw std::logic_error("WindowedGraphicsSession cannot attach WindowManager after close");
        }
        if (active_window_managers_ != 0) {
            tc_log_error("[WindowedGraphicsSession] only one WindowManager may own the window collection");
            throw std::logic_error("WindowedGraphicsSession already has an active WindowManager");
        }
        ++active_window_managers_;
    }
    ...
    void WindowedGraphicsSession::detach_window_manager() noexcept {
        if (active_window_managers_ == 0) {
            tc_log_error("[WindowedGraphicsSession] WindowManager attachment count underflow");
            return;
        }
        --active_window_managers_;
    }
    ...
    void WindowedGraphicsSession::close() {
        if (closed_)
            return;
        if (active_window_managers_ != 0) {
            tc_log_error("[WindowedGraphicsSession] close requires WindowManager to close first");
            throw std::logic_error("WindowedGraphicsSession::close requires WindowManager to close first");
        }
        if (windows_->live_window_count() != 0) {
            throw std::logic_error("WindowedGraphicsSession::close requires all presentation windows to be closed");
        }
...
#else
        throw std::runtime_error("create_native_window_system: this termin-window build has no native backend");
#endif
    }
    ...
    std::unique_ptr<WindowedGraphicsSession> create_native_windowed_graphics() {
        ...
        return std::make_unique<WindowedGraphicsSession>(std::move(windows), std::move(graphics));
    }
