...
namespace termin {
    ...
    namespace {
        ...
        bool next_window_event(SDL_Window* window, SDLWindowEventQueue& own_queue, SDL_Event& out_event) {
            if (!own_queue.empty()) {
        ...
        tcbase::MouseButton translate_pointer_button(uint8_t button) {
            ...
            case SDL_BUTTON_LEFT:
                return tcbase::MouseButton::LEFT;
            case SDL_BUTTON_RIGHT:
                return tcbase::MouseButton::RIGHT;
            case SDL_BUTTON_MIDDLE:
                return tcbase::MouseButton::MIDDLE;
            default:
                return tcbase::MouseButton::OTHER;
            }
        }

        WindowKey translate_key(SDL_Scancode key) {
            if (key >= SDL_SCANCODE_A && key <= SDL_SCANCODE_Z) {
                return static_cast<WindowKey>(static_cast<uint16_t>(WindowKey::A) +
                                              static_cast<uint16_t>(key - SDL_SCANCODE_A));
            }
            if (key >= SDL_SCANCODE_1 && key <= SDL_SCANCODE_9) {
                return static_cast<WindowKey>(static_cast<uint16_t>(WindowKey::Digit1) +
                                              static_cast<uint16_t>(key - SDL_SCANCODE_1));
            }
            switch (key) {
            case SDL_SCANCODE_0:
                return WindowKey::Digit0;
            ...
            case SDL_SCANCODE_HOME:
                return WindowKey::Home;
            case SDL_SCANCODE_END:
                return WindowKey::End;
            case SDL_SCANCODE_INSERT:
                return WindowKey::Insert;
            case SDL_SCANCODE_PAGEUP:
                return WindowKey::PageUp;
            case SDL_SCANCODE_PAGEDOWN:
                return WindowKey::PageDown;
            ...
            case SDL_SCANCODE_F10:
                return WindowKey::F10;
            case SDL_SCANCODE_F11:
                return WindowKey::F11;
            case SDL_SCANCODE_F12:
                return WindowKey::F12;
            case SDL_SCANCODE_APOSTROPHE:
                return WindowKey::Apostrophe;
            case SDL_SCANCODE_COMMA:
                return WindowKey::Comma;
            case SDL_SCANCODE_MINUS:
                return WindowKey::Minus;
            case SDL_SCANCODE_PERIOD:
                return WindowKey::Period;
            case SDL_SCANCODE_SLASH:
                return WindowKey::Slash;
            case SDL_SCANCODE_SEMICOLON:
                return WindowKey::Semicolon;
            case SDL_SCANCODE_EQUALS:
                return WindowKey::Equals;
            case SDL_SCANCODE_LEFTBRACKET:
                return WindowKey::LeftBracket;
            case SDL_SCANCODE_BACKSLASH:
                return WindowKey::Backslash;
            case SDL_SCANCODE_RIGHTBRACKET:
                return WindowKey::RightBracket;
            case SDL_SCANCODE_GRAVE:
                return WindowKey::Grave;
            default:
    ...
    SDLWindowSystem::SDLWindowSystem()
        ...
        if (tgfx2_interop_get_device() != nullptr) {
            throw std::runtime_error("SDLWindowSystem: an application graphics device is already installed");
        }
        ...
        if (impl_->backend == tgfx::BackendType::OpenGL) {
            ...
            impl_->gl_bootstrap_window = SDL_CreateWindow(
                "Termin graphics runtime", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1, 1, flags);
            if (!impl_->gl_bootstrap_window) {
                throw std::runtime_error(std::string("SDLWindowSystem bootstrap window failed: ") + SDL_GetError());
            }
            ...
            if (!impl_->gl_context) {
                ...
                throw std::runtime_error(std::string("SDLWindowSystem GL context failed: ") + SDL_GetError());
            }
            ...
            std::unique_ptr<SDL_Window, decltype(&SDL_DestroyWindow)> bootstrap(
                SDL_CreateWindow("Termin Vulkan bootstrap",
                ...
                SDL_DestroyWindow);
            if (!bootstrap) {
                throw std::runtime_error(std::string("SDLWindowSystem Vulkan bootstrap failed: ") + SDL_GetError());
            }

            uint32_t ext_count = 0;
            if (!SDL_Vulkan_GetInstanceExtensions(bootstrap.get(), &ext_count, nullptr)) {
                throw std::runtime_error(std::string("SDL_Vulkan_GetInstanceExtensions(count) failed: ") +
                                         SDL_GetError());
            }
            std::vector<const char*> extensions(ext_count);
            if (!SDL_Vulkan_GetInstanceExtensions(bootstrap.get(), &ext_count, extensions.data())) {
                throw std::runtime_error(std::string("SDL_Vulkan_GetInstanceExtensions(list) failed: ") +
                                         SDL_GetError());
            }
            ...
            info.presentation_probe_surface_factory = [window = bootstrap.get()](VkInstance instance) {
                VkSurfaceKHR surface = VK_NULL_HANDLE;
                if (!SDL_Vulkan_CreateSurface(window, instance, &surface)) {
                    throw std::runtime_error(std::string("SDL_Vulkan_CreateSurface(probe) failed: ") + SDL_GetError());
                }
                return surface;
            };
    ...
    SDLWindowSystem::~SDLWindowSystem() {
        ...
        if (impl_->live_windows != 0) {
            tc_log_error("[SDLWindowSystem] destroyed with %zu live presentation window(s)", impl_->live_windows);
    ...
    std::unique_ptr<tgfx::GraphicsHost> SDLWindowSystem::create_graphics_host() {
        if (impl_->closed || !impl_->prepared_device || impl_->graphics_host) {
            throw std::logic_error("SDLWindowSystem::create_graphics_host requires one fresh prepared device");
        }
        auto graphics = tgfx::GraphicsHost::adopt_application_device(std::move(impl_->prepared_device));
        impl_->graphics_host = graphics.get();
        return graphics;
    }

    BackendWindowPtr SDLWindowSystem::create_window(tgfx::GraphicsHost& graphics, const WindowConfig& config) {
        ...
        return std::make_unique<SDLBackendWindow>(
            *this, graphics, config.title, config.width, config.height, config.presentation_mode);
    }
    ...
    void SDLWindowSystem::close(tgfx::GraphicsHost& graphics) {
        if (impl_->closed)
            return;
        if (impl_->live_windows != 0) {
            throw std::logic_error("SDLWindowSystem::close requires all presentation windows to be closed");
        }
        if (impl_->graphics_host != &graphics) {
            throw std::invalid_argument("SDLWindowSystem::close received a different GraphicsHost");
        }
        graphics.close();
    ...
    SDLBackendWindow::SDLBackendWindow(SDLWindowSystem& window_system,
        ...
        try {
            if (impl_->backend == tgfx::BackendType::OpenGL) {
                ...
                if (!window_) {
                    throw std::runtime_error(std::string("SDL_CreateWindow(OpenGL) failed: ") + SDL_GetError());
                }
                if (SDL_GL_MakeCurrent(window_, window_system.impl_->gl_context) != 0) {
                    throw std::runtime_error(std::string("SDL_GL_MakeCurrent(window) failed: ") + SDL_GetError());
                }
                const int swap_interval = presentation_mode == tgfx::PresentationMode::VSync ? 1 : 0;
                if (SDL_GL_SetSwapInterval(swap_interval) != 0) {
                    tc_log_warn("SDLBackendWindow: SDL_GL_SetSwapInterval(%d) is unsupported: %s",
                                swap_interval,
                                SDL_GetError());
                }
                const int applied_swap_interval = SDL_GL_GetSwapInterval();
                impl_->presentation_mode =
                    applied_swap_interval == 0 ? tgfx::PresentationMode::Immediate : tgfx::PresentationMode::VSync;
#else
                ...
                Uint32 flags = SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_SHOWN | SDL_WINDOW_ALLOW_HIGHDPI;
                window_ = SDL_CreateWindow(
                    title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, flags);
                if (!window_) {
                ...
                if (!SDL_Vulkan_CreateSurface(window_, vk_dev->instance(), &surf)) {
                    ...
                    window_ = nullptr;
                    throw std::runtime_error(std::string("SDL_Vulkan_CreateSurface(window) failed: ") + SDL_GetError());
                }
                impl_->surface = surf;
                ...
                impl_->swapchain = std::make_unique<tgfx::VulkanSwapchain>(
                    *vk_dev, surf, static_cast<uint32_t>(fb_w), static_cast<uint32_t>(fb_h), impl_->presentation_mode);
            }
#endif
#ifdef TGFX2_HAS_D3D11
            else if (impl_->backend == tgfx::BackendType::D3D11) {
                Uint32 flags = SDL_WINDOW_RESIZABLE | SDL_WINDOW_SHOWN | SDL_WINDOW_ALLOW_HIGHDPI;
                window_ = SDL_CreateWindow(
                    title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, flags);
                if (!window_) {
                    throw std::runtime_error(std::string("SDL_CreateWindow(D3D11) failed: ") + SDL_GetError());
                }
                ...
                impl_->d3d11_swapchain = std::make_unique<tgfx::D3D11Swapchain>(*d3d_dev,
    ...
    void SDLBackendWindow::set_icon_bmp(const std::string& path) {
        ...
        std::unique_ptr<SDL_Surface, decltype(&SDL_FreeSurface)> icon(SDL_LoadBMP(path.c_str()), SDL_FreeSurface);
        if (!icon) {
            const std::string message = "SDLBackendWindow::set_icon_bmp failed for '" + path + "': " + SDL_GetError();
    ...
    void SDLBackendWindow::set_text_input_enabled(bool enabled) {
        if (enabled == text_input_enabled_)
            return;
    ...
    void SDLBackendWindow::set_cursor(WindowCursor cursor) {
        ...
        if (!handle) {
            ...
            if (!handle) {
                tc_log_error("[SDLBackendWindow] SDL_CreateSystemCursor failed: %s", SDL_GetError());
                return;
            }
        }
    ...
    std::string SDLBackendWindow::clipboard_text() const {
        ...
        if (!text) {
            tc_log_error("[SDLBackendWindow] SDL_GetClipboardText failed: %s", SDL_GetError());
            return {};
        }
    ...
    bool SDLBackendWindow::set_clipboard_text(const std::string& text) {
        if (SDL_SetClipboardText(text.c_str()) == 0)
            return true;
        tc_log_error("[SDLBackendWindow] SDL_SetClipboardText failed: %s", SDL_GetError());
        return false;
    }
    ...
    float SDLBackendWindow::content_scale() const {
        ...
        const double horizontal = static_cast<double>(pixel_width) / static_cast<double>(logical_width);
        const double vertical = static_cast<double>(pixel_height) / static_cast<double>(logical_height);
    ...
    bool SDLBackendWindow::poll_event(WindowEvent& out_event) {
        ...
        while (next_window_event(window_, impl_->pending_events, ev)) {
            ...
            case SDL_MOUSEMOTION:
                ...
                out_event.pointer.logical_position = {static_cast<float>(ev.motion.x), static_cast<float>(ev.motion.y)};
                break;

            case SDL_MOUSEBUTTONDOWN:
            case SDL_MOUSEBUTTONUP:
                out_event.type = ev.type == SDL_MOUSEBUTTONDOWN ? WindowEventType::PointerButtonPressed
                                                                : WindowEventType::PointerButtonReleased;
                out_event.pointer.logical_position = {static_cast<float>(ev.button.x), static_cast<float>(ev.button.y)};
            ...
            case SDL_MOUSEWHEEL: {
                ...
                out_event.pointer.logical_position = {static_cast<float>(x), static_cast<float>(y)};
                const float direction = ev.wheel.direction == SDL_MOUSEWHEEL_FLIPPED ? -1.0f : 1.0f;
            ...
            case SDL_KEYDOWN:
            case SDL_KEYUP:
                out_event.type = ev.type == SDL_KEYDOWN ? WindowEventType::KeyPressed : WindowEventType::KeyReleased;
                ...
                out_event.key.modifiers = translate_modifiers(static_cast<SDL_Keymod>(ev.key.keysym.mod));
            ...
            case SDL_TEXTINPUT:
                ...
                {
                    const size_t length = std::min(std::strlen(ev.text.text), out_event.text.utf8.size() - 1);
                    std::copy_n(ev.text.text, length, out_event.text.utf8.data());
            ...
            case SDL_DROPFILE: {
                ...
                int y = 0;
                SDL_GetMouseState(&x, &y);
                out_event.file_drop.logical_position = {static_cast<float>(x), static_cast<float>(y)};
                out_event.file_drop.modifiers = translate_modifiers(SDL_GetModState());
                return true;
            }
    ...
    void SDLBackendWindow::present(tgfx::TextureHandle color_tex) {
        ...
        if (impl_->backend == tgfx::BackendType::OpenGL) {
            ...
            if (SDL_GL_MakeCurrent(window_, gl_ctx) != 0) {
                tc_log_error("[BackendWindow] present: SDL_GL_MakeCurrent failed: %s", SDL_GetError());
                return;
            }
