...
int main()
{
    ...
    require(tcplot::gui_native::register_plot_widget_types(),
            "Plot2D registration failed");
    require(tc_widget_registry_has("termin.gui.Plot2D"),
            "Plot2D is absent from the widget registry");

    const TcDocument loaded_document{tc_ui_document_create()};
    require(loaded_document.valid(), "UiScript document creation failed");
    require(loaded_document.set_presentation_metrics(
                tc_ui_presentation_metrics_identity({640.0f, 320.0f})),
    ...
    loaded_document.set_text_measurer(&measure_text, nullptr);
    {
        ...
        auto loaded = loader.load_string("uiscript: 2\n"
                                         ...
                                         "  auto_fit: true\n",
                                         loaded_document);
        tc_widget* loaded_widget = tc_ui_document_resolve_widget(
            loaded.document().handle(), loaded.named("chart").handle);
        require(loaded_widget != nullptr, "UiScript did not create Plot2D");
        auto* loaded_plot = dynamic_cast<Plot2D*>(
            static_cast<termin::gui_native::Widget*>(loaded_widget->body));
        require(loaded_plot != nullptr,
                "UiScript widget has the wrong C++ type");
        require(loaded_plot->title() == "Servo coordinate",
                "UiScript title was not applied");
        ...
        require(invalid_property_rejected,
                "UiScript accepted an invalid Plot2D property type");
    }
    ...
    require(plot->set_line_data(actual, time, coordinate),
            "setting actual line data failed");
    require(plot->set_line_data(target, time, target_coordinate),
            "setting target line data failed");
