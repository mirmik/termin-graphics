...
namespace tcplot::gui_native
{
    namespace
    {
        ...
        {
        public:
            std::optional<tgfx::FontHandle>
            resolve_font(std::string_view uri) override
            {
            ...
            std::optional<tgfx::TextureHandle>
            resolve_image(std::string_view) override
            {
                return std::nullopt;
            }

            std::optional<termin::visual::ResolvedCustomBatch2D>
            resolve_custom_batch(std::string_view, termin::Bounds2f) override
            {
                return std::nullopt;
            }
        };

        float text_width(tc_ui_document_handle document,
                         const std::string& text,
                         float font_size)
        {
            tc_ui_text_metrics metrics{};
            if (tc_ui_document_measure_text(
                    document, text.c_str(), text.size(), font_size, &metrics))
            {
    ...
    struct Plot2D::Impl
    {
        ...
        termin::visual::GraphicItemHandle grid_handle =
        ...
        Impl() : scene(tc_visual_scene_create())
        {
            if (!scene.valid())
            {
                throw std::runtime_error(
                    "Plot2D failed to create its visual scene");
            }
            frame = PlotFrame2D{
            ...
            const auto created_projection =
                create_plot_projection2d(scene, frame);
            if (!created_projection)
            {
                tc_visual_scene_destroy(scene.handle());
                scene = {};
                throw std::runtime_error(
                    "Plot2D failed to create its projection");
            }
            ...
            const auto grid = adopt_plot_grid_item2d(
                ...
                {},
                {},
                PlotGridStyle2D{visual_color(styles::grid_color()), 1.0f});
            if (!grid)
            {
    ...
    Plot2D::Plot2D() : NativeWidget("Plot2D"), impl_(std::make_unique<Impl>())
    {
    ...
    void
    Plot2D::set_view(double x_min, double x_max, double y_min, double y_max)
    {
    ...
    std::size_t Plot2D::add_line()
    {
        ...
        style.color = styles::cycle_color(
            static_cast<std::uint32_t>(impl_->line_handles.size()));
        return add_line(style);
    }

    std::size_t Plot2D::add_line(PlotLineSeriesStyle2D style)
    {
        const auto handle = adopt_plot_line_series_item2d(
            impl_->scene, impl_->projection, {}, {}, {}, style);
        if (!handle)
        {
        ...
        {
            line->set_z_order(
                10 + static_cast<std::int64_t>(impl_->line_handles.size()));
        }
        invalidate_chart();
        return impl_->line_handles.size() - 1;
    }

    bool Plot2D::set_line_data(std::size_t index,
                               ...
                               std::span<const double> y)
    {
        ...
        auto* line = resolve_plot_line_series_item2d(
            impl_->scene, impl_->line_handles[index]);
        if (line == nullptr ||
            !line->set_data(std::vector<double>(x.begin(), x.end()),
    ...
    bool Plot2D::append_line_data(std::size_t index,
                                  std::span<const double> x,
                                  std::span<const double> y)
    {
        if (index >= impl_->line_handles.size())
    ...
    tc_ui_size Plot2D::measure(tc_ui_document_handle,
    ...
    void Plot2D::paint(tc_ui_document_handle document,
                       tc_ui_paint_context* context)
    {
        ...
        tc_ui_painter_fill_rect(
            context, widget_bounds, ui_color(styles::bg_color()));
        tc_ui_painter_fill_rect(
            context, global_area, ui_color(styles::plot_area_bg()));
        ...
        const bool built = builder.push_transform(translation) &&
                           impl_->scene.paint(builder, resources) &&
                           builder.pop_transform();
        auto draw_list = built ? builder.freeze() : std::nullopt;
        if (!draw_list || !termin::gui_native::append_draw_list2d(
                              context, std::move(*draw_list)))
        {
        ...
        tc_ui_painter_draw_line(
            ...
            {global_area.x + global_area.width,
             global_area.y + global_area.height},
            axis,
            1.0f);
        tc_ui_painter_draw_line(
            context,
            {global_area.x, global_area.y},
            {global_area.x, global_area.y + global_area.height},
            axis,
            1.0f);

        for (std::size_t index = 0; index < impl_->ticks.x.values.size();
             ++index)
        {
            const float local_x =
                impl_->frame
                    .data_to_pixel(impl_->ticks.x.values[index],
                                   impl_->range.y_min())
                    .x;
        ...
        for (std::size_t index = 0; index < impl_->ticks.y.values.size();
             ++index)
        {
            const float local_y =
                impl_->frame
                    .data_to_pixel(impl_->range.x_min(),
                                   impl_->ticks.y.values[index])
                    .y;
            const float y = widget_bounds.y + local_y;
            tc_ui_painter_draw_line(context,
                                    {global_area.x - 4.0f, y},
                                    {global_area.x, y},
                                    axis,
                                    1.0f);
            ...
            tc_ui_painter_draw_text(context,
                                    text.c_str(),
                                    {global_area.x - width - 7.0f, y + 3.0f},
                                    tick_font_size,
                                    label);
        }
        ...
        {
            const float width =
                text_width(document, impl_->title, title_font_size);
            tc_ui_painter_draw_text(
                context,
                impl_->title.c_str(),
                {widget_bounds.x + (widget_bounds.width - width) * 0.5f,
                 widget_bounds.y + 17.0f},
                title_font_size,
                label);
        }
        ...
        {
            const float width =
                text_width(document, impl_->x_label, label_font_size);
            tc_ui_painter_draw_text(
                context,
                impl_->x_label.c_str(),
                {widget_bounds.x + (widget_bounds.width - width) * 0.5f,
                 widget_bounds.y + widget_bounds.height - 4.0f},
                label_font_size,
                label);
        }
    ...
    void Plot2D::update_chart_layout()
    {
        ...
        const tc_ui_rect widget_bounds = bounds();
        const float width =
            widget_bounds.width > 0.0f ? widget_bounds.width : preferred_width;
        const float height = widget_bounds.height > 0.0f ? widget_bounds.height
                                                         : preferred_height;
        ...
        impl_->frame =
            PlotFrame2D{viewport, plot_area, impl_->range, plot_area, 1.0f};
        ...
        if (ticks)
        {
            impl_->ticks = *ticks;
            if (auto* grid =
                    resolve_plot_grid_item2d(impl_->scene, impl_->grid_handle))
            {
    ...
    void Plot2D::update_auto_range()
    {
        ...
        {
            const auto* line =
                resolve_plot_line_series_item2d(impl_->scene, handle);
            if (line == nullptr)
            {
                continue;
            }
        ...
        const auto fitted = fit_optional_plot_range2d(
            has_data ? std::optional<PlotRange2D>{PlotRange2D{
