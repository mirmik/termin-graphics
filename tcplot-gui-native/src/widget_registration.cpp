...
namespace tcplot::gui_native
{
    namespace
    {
        ...
        bool create_widget(tc_ui_document_handle,
                           void*,
                           tc_widget_factory_result* result)
        {
            ...
            {
                tc_log_error("[tcplot-gui-native] failed to create Plot2D: %s",
                             error.what());
                return false;
            }
            catch (...)
            {
                tc_log_error(
                    "[tcplot-gui-native] failed to create Plot2D with an "
                    "unknown exception");
                return false;
            }
        }
        ...
        bool validate_property(const char* name, const tc_value* value)
        {
            ...
            if (property_name == "title" || property_name == "x_label" ||
                property_name == "y_label")
            {
        ...
        bool apply_properties(tc_widget* widget, const tc_value* properties)
        {
            ...
            if (const tc_value* auto_fit = property(properties, "auto_fit"))
            {
                ...
                {
                    tc_log_error(
                        "[tcplot-gui-native] Plot2D auto_fit must be bool");
                    return false;
                }
        ...
        {
            tc_log_error(
                "[tcplot-gui-native] Plot2D is declarative-only and has no "
        ...
        {
            tc_log_error(
                "[tcplot-gui-native] Plot2D cannot be restored through "
    ...
    bool register_plot_widget_types()
    {
        ...
        {
            tc_log_error(
                "[tcplot-gui-native] built-in widget registration failed");
            return false;
        }
        ...
        if (!tc_widget_registry_register(
                widget_type, module_owner, widget_parent, &descriptor))
        {
            tc_log_error(
                "[tcplot-gui-native] failed to register termin.gui.Plot2D");
            return false;
        }
        return true;
    }
