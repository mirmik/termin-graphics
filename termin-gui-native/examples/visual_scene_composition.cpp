...
namespace termin::gui_native::examples {
    namespace {
        ...
        Item* adopt(termin::visual::TcVisualScene& scene, termin::visual::GraphicItem2D* parent, Args&&... args) {
            auto item = std::make_unique<Item>(std::forward<Args>(args)...);
            auto* result = item.get();
            require(scene.adopt(std::move(item), parent).has_value(), "failed to adopt visual item");
            return result;
        }

        tc_ui_point position(const termin::visual::GraphicItem2D& item) {
            const auto& transform = item.local_transform();
            return {transform.tx, transform.ty};
        }

        void set_position(termin::visual::GraphicItem2D& item, tc_ui_point value) {
        ...
        bool measure_headless_text(
            void*, const char* text, std::size_t byte_length, float font_size, tc_ui_text_metrics* out) {
            ...
            while (offset < byte_length) {
                ...
                if (offset + codepoint_bytes > byte_length)
                    return false;
                for (std::size_t index = 1; index < codepoint_bytes; ++index) {
                    if ((static_cast<std::uint8_t>(text[offset + index]) & 0xc0u) != 0x80u) {
                        return false;
                    }
                }
        ...
        void click(TcDocument document, tc_ui_point point) {
            require(document.dispatch_pointer_event({TC_UI_POINTER_DOWN, point.x, point.y, 0, 1, 0, 0.0f, 0.0f}) ==
                        TC_UI_EVENT_HANDLED,
                    "pointer down was not handled");
            require(document.dispatch_pointer_event({TC_UI_POINTER_UP, point.x, point.y, 0, 1, 0, 0.0f, 0.0f}) ==
                        TC_UI_EVENT_HANDLED,
                    "pointer up was not handled");
        }

        std::size_t first_command(const tc_ui_draw_list* draw_list, tc_ui_draw_command_type type) {
    ...
    VisualSceneCompositionRefs build_visual_scene_composition(TcDocument document) {
        if (!document.valid()) {
            throw std::invalid_argument("visual scene composition requires a live document");
        }
        ...
        refs.scene = termin::visual::TcVisualScene{refs.state->scene};

        refs.root = &ui.make_root<BoxLayout>(Orientation::Horizontal, "VisualSceneCompositionRoot");
        refs.root->set_padding({14.0f, 14.0f, 14.0f, 14.0f})
            .set_spacing(14.0f)
            .set_background({0.045f, 0.052f, 0.070f, 1.0f});

        refs.controls = &ui.make<BoxLayout>(Orientation::Vertical, "CompositionControls");
        ...
        auto& hint = ui.make<Label>("Drag shapes. The green control is a widget portal.", 13.0f);
        ...
        refs.view->set_scene_colors(
            {0.075f, 0.082f, 0.105f, 1.0f}, {0.135f, 0.150f, 0.195f, 1.0f}, {0.28f, 0.34f, 0.45f, 1.0f});
        refs.root->add_fixed_child(*refs.controls, 230.0f);
        refs.root->add_flex_child(*refs.view);

        refs.card = adopt<termin::visual::RoundedRectItem2D>(refs.scene,
                                                             nullptr,
        ...
        adopt<termin::visual::TextItem2D>(refs.scene,
                                          refs.card,
                                          "shared scene item",
                                          "ui://default-font",
        ...
        refs.ellipse = adopt<termin::visual::EllipseItem2D>(refs.scene,
                                                            nullptr,
        ...
        auto* connector = adopt<termin::visual::PolylineItem2D>(
            refs.scene,
            nullptr,
            std::vector<termin::Vec2f>{{145.0f, 110.0f}, {255.0f, 110.0f}, {300.0f, 200.0f}},
            stroke(0.52f, 0.61f, 0.78f, 3.0f));
        connector->set_z_order(1);

        auto* clipped = adopt<termin::visual::RoundedRectItem2D>(refs.scene,
                                                                 nullptr,
        ...
        refs.portal = adopt<termin::visual::HitRegionItem2D>(refs.scene, nullptr, rectangle_path(175.0f, 42.0f));
        ...
        require(refs.view->set_widget_portal(refs.portal->handle(), refs.portal_button->handle()),
        ...
        auto* view = refs.view;
        refs.reset->clicked().connect([state, status, card, ellipse, view](Button&) {
        ...
        refs.portal_button->clicked().connect([state, status](Button& button) {
            ++state->portal_clicks;
            button.set_text("Portal clicked " + std::to_string(state->portal_clicks));
            status->set_text("Portal widget handled the pointer");
        });
        auto scene = refs.scene;
        refs.view->set_pointer_handler([state, status, scene, card, ellipse](
                                           SceneView& view, tc_ui_point world, const tc_ui_pointer_event& event) {
            if (event.type == TC_UI_POINTER_DOWN && event.button == 0) {
                const auto hit = termin::visual::hit_test(scene, {world.x, world.y});
                auto* item = hit ? scene.resolve(*hit) : nullptr;
                while (item && item != card->c_item() && item != ellipse->c_item()) {
                    item = item->parent;
                }
                state->drag_item = item == card->c_item()      ? static_cast<termin::visual::GraphicItem2D*>(card)
                                   : item == ellipse->c_item() ? static_cast<termin::visual::GraphicItem2D*>(ellipse)
                                                               : nullptr;
                if (!state->drag_item)
                    return false;
                state->drag_start_world = world;
                state->drag_start_position = position(*state->drag_item);
                return true;
            }
            if (event.type == TC_UI_POINTER_MOVE && state->drag_item) {
                set_position(*state->drag_item,
                             {
                                 state->drag_start_position.x + world.x - state->drag_start_world.x,
                                 state->drag_start_position.y + world.y - state->drag_start_world.y,
                             });
                ++state->item_moves;
            ...
            if ((event.type == TC_UI_POINTER_UP || event.type == TC_UI_POINTER_CANCEL) && state->drag_item) {
    ...
    int run_visual_scene_composition_headless_smoke() {
        ...
        try {
            ...
            require(document.set_presentation_metrics(tc_ui_presentation_metrics_identity(tc_ui_size{800.0f, 600.0f})),
            ...
            require(refs.view->bounds().width > 300.0f && refs.controls->bounds().width == 230.0f,
                    "mixed layout did not allocate widget panel and scene view");
            require(same_widget(document.hit_test(refs.reset->bounds().x + 5.0f, refs.reset->bounds().y + 5.0f),
                                refs.reset->handle()),
            ...
            const std::size_t canvas = first_command(draw_list, TC_UI_DRAW_CANVAS2D_LIST);
            const std::size_t count = tc_ui_draw_list_command_count(draw_list);
            ...
            require(canvas_is_clipped && balanced_clips && clip_depth == 0,
                    "scene and portal painting escaped the SceneView clip");
            require(first_command(draw_list, TC_UI_DRAW_TEXT) < canvas,
            ...
            require(portal_text < count, "portal widget was not painted at its scene item slot");
            ...
            const tc_ui_point portal_center = tc_ui_uniform_transform_map_point(
                ...
                {portal_bounds.x + portal_bounds.width * 0.5f, portal_bounds.y + portal_bounds.height * 0.5f});
            ...
            require(refs.state->portal_clicks == 1 && near(position(*refs.portal).x, portal_position.x) &&
                        near(position(*refs.portal).y, portal_position.y),
                    "portal click leaked into scene dragging");

            const tc_ui_point card_start = refs.view->world_to_screen({80.0f, 80.0f});
            const tc_ui_point card_end{card_start.x + 37.0f, card_start.y + 23.0f};
            require(document.dispatch_pointer_event(
                        {TC_UI_POINTER_DOWN, card_start.x, card_start.y, 0, 1, 0, 0.0f, 0.0f}) == TC_UI_EVENT_HANDLED,
                    "scene drag down was not handled");
            require(document.dispatch_pointer_event(
                        {TC_UI_POINTER_MOVE, card_end.x, card_end.y, 0, 0, 0, 0.0f, 0.0f}) == TC_UI_EVENT_HANDLED,
                    "scene drag move was not handled");
            require(document.dispatch_pointer_event({TC_UI_POINTER_UP, card_end.x, card_end.y, 0, 1, 0, 0.0f, 0.0f}) ==
                        TC_UI_EVENT_HANDLED,
                    "scene drag up was not handled");
            require(refs.state->item_moves == 1 && near(position(*refs.card).x, 92.0f) &&
            ...
            require(refs.view->has_dirty_flags(TC_WIDGET_DIRTY_LAYOUT | TC_WIDGET_DIRTY_PAINT),
            ...
            const tc_ui_rect resized_portal_screen =
                tc_ui_uniform_transform_map_rect(refs.portal_button->subtree_transform(), resized_portal);
            ...
            require(refs.state->reset_clicks == 1 && near(position(*refs.card).x, 55.0f) &&
            ...
            std::puts("visual scene composition smoke: widgets, scene and portal passed");
            return 0;
        } catch (const std::exception& error) {
            std::fprintf(stderr, "visual scene composition smoke failed: %s\n", error.what());
            if (context)
                tc_ui_paint_context_destroy(context);
            if (draw_list)
                tc_ui_draw_list_destroy(draw_list);
