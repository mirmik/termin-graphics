...
namespace termin::gui_native {
    ...
    public:
        ...
        BoxLayout& set_orientation(Orientation orientation);
        ...
        void add_fixed_child(const Widget& widget, float extent) {
            add_fixed_child(widget.handle(), extent);
        }
        void add_preferred_child(tc_widget_handle handle);
        void add_preferred_child(const Widget& widget) {
            add_preferred_child(widget.handle());
        }
        void add_flex_child(tc_widget_handle handle, float flex = 1.0f);
        void add_flex_child(const Widget& widget, float flex = 1.0f) {
            add_flex_child(widget.handle(), flex);
        }
        void add_stretch_child(tc_widget_handle handle);
        void add_stretch_child(const Widget& widget) {
            add_stretch_child(widget.handle());
        }
        bool set_child_placement(tc_widget_handle handle,
                                 LayoutPolicy policy,
                                 float basis,
                                 float grow,
                                 float shrink,
        ...
        bool set_child_placement(const Widget& widget,
                                 LayoutPolicy policy,
                                 float basis,
                                 float grow,
                                 float shrink,
                                 ...
                                 float max_extent,
                                 CrossAxisAlignment align_self = CrossAxisAlignment::Auto) {
            return set_child_placement(
                widget.handle(), policy, basis, grow, shrink, min_extent, max_extent, align_self);
        }
