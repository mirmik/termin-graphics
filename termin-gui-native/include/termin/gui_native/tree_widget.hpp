...
public:
    ...
    const std::shared_ptr<TreeExpansionModel>& expansion_model() const { return expansion_; }
    ...
    Signal<TreeWidget&, TreeNodeId>& selection_changed() { return selection_changed_; }
    Signal<TreeWidget&, TreeNodeId, bool>& expansion_changed() { return expansion_changed_; }
    Signal<TreeWidget&, TreeNodeId, const CollectionItem&>& activated() { return activated_; }
...
private:
    ...
    void append_visible(TreeNodeId node, size_t depth);
    void sync_models();
    void clamp_scroll();
