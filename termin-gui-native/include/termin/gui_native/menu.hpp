...
namespace termin::gui_native {
    ...
    public:
        ...
        const std::shared_ptr<CommandModel>& model() const {
            return model_;
        }
        void set_model(std::shared_ptr<CommandModel> model);
        bool
        show(tc_ui_document_handle document, tc_ui_point position, tc_ui_rect viewport, bool dismiss_on_outside = true);
        ...
        float max_visible_height() const {
            return max_visible_height_;
        }

        Signal<Menu&, size_t, CommandId, const CommandData&>& activated() {
            return activated_;
        }
        Signal<Menu&, tc_ui_overlay_dismiss_reason>& dismissed() {
            return dismissed_;
        }
        Signal<Menu&, int>& adjacent_requested() {
            return adjacent_requested_;
        }
    ...
    private:
        ...
        void set_anchor_owner(tc_widget_handle handle) {
            anchor_owner_ = handle;
        }
        Menu(std::shared_ptr<CommandModel> model, Menu* parent, std::unordered_set<const CommandModel*> ancestors);
        void connect_model();
