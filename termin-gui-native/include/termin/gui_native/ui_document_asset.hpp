...
namespace termin::gui_native {
    ...
    struct UiDocumentAssetHandle {
        ...
        bool valid() const;
        friend bool operator==(UiDocumentAssetHandle left, UiDocumentAssetHandle right) = default;
    };

    class TERMIN_GUI_NATIVE_API UiDocumentAsset {
    public:
        UiDocumentAsset(std::string uuid,
                        std::string name,
    ...
    public:
        ...
        UiDocumentAssetHandle handle() const {
            return handle_;
        }
        bool valid() const;
        bool is_valid() const {
        ...
        static TcUiDocumentAsset
        declare_source(std::string uuid, std::string name, std::string source_identity, const std::string& source);
        static TcUiDocumentAsset declare_compiled_json(const std::string& compiled_json,
                                                       const std::string& expected_uuid = {});
        static std::string compile_source_json(
            std::string uuid, std::string name, std::string source_identity, const std::string& source, int indent = 2);
