...
namespace termin::gui_native {
    ...
    public:
        ...
        TcDocument() = default;
        explicit TcDocument(tc_ui_document_handle handle)
            : handle_(handle) {}

        tc_ui_document_handle handle() const {
            return handle_;
        }
        ...
        bool valid() const {
            return tc_ui_document_is_valid(handle_);
        }
        ...
        void layout_roots(tc_ui_rect rect) const {
            tc_ui_document_layout_roots(handle_, rect);
        }
        void paint_roots(tc_ui_paint_context* context) const {
            tc_ui_document_paint_roots(handle_, context);
        }
        void paint(tc_ui_paint_context* context) const {
            tc_ui_document_paint(handle_, context);
        }
        ...
        bool show_overlay(const Widget& widget,
                          uint32_t flags,
                          const tc_ui_overlay_layout& layout,
                          tc_ui_rect viewport) const {
            return tc_ui_document_show_overlay_with_layout(handle_, widget.handle(), flags, &layout, viewport);
        }
        bool
        update_overlay_layout(const Widget& widget, const tc_ui_overlay_layout& layout, tc_ui_rect viewport) const {
            return tc_ui_document_update_overlay_layout(handle_, widget.handle(), &layout, viewport);
        }
        tc_ui_rect layout_rect() const {
            return tc_ui_document_layout_rect(handle_);
        }
        bool dismiss_overlay(const Widget& widget,
        ...
        size_t overlay_count() const {
            return tc_ui_document_overlay_count(handle_);
        }
        ...
        bool cancel_pointer_interaction(tc_ui_pointer_cancel_reason reason = TC_UI_POINTER_CANCEL_EXPLICIT) const {
        ...
        bool focus_next() const {
            return tc_ui_document_focus_next(handle_);
        }
        bool focus_previous() const {
            return tc_ui_document_focus_previous(handle_);
        }
        ...
        bool measure_text(const char* text_utf8,
                          size_t text_byte_length,
                          float font_size,
                          tc_ui_text_metrics& out_metrics) const {
            return tc_ui_document_measure_text(handle_, text_utf8, text_byte_length, font_size, &out_metrics);
        }
        const tc_ui_theme& theme() const {
            return *tc_ui_document_theme(handle_);
        }
        ...
        TERMIN_GUI_NATIVE_API tc_ui_style resolve_style(const Widget& widget, uint32_t extra_state_flags = 0) const;
    };
