...
namespace termin::gui_native {
    ...
    public:
        ...
        virtual std::string normalize(std::string_view path, std::string_view base, std::string& error) const = 0;
        virtual bool inspect(std::string_view path, FileDialogEntry& entry, std::string& error) const = 0;
        virtual std::vector<FileDialogEntry> list(std::string_view directory, std::string& error) const = 0;
    ...
    public:
        std::string normalize(std::string_view path, std::string_view base, std::string& error) const override;
        bool inspect(std::string_view path, FileDialogEntry& entry, std::string& error) const override;
        std::vector<FileDialogEntry> list(std::string_view directory, std::string& error) const override;
    ...
    public:
        explicit FileDialogModel(FileDialogMode mode, std::shared_ptr<FileDialogFileSystem> file_system = {});

        FileDialogMode mode() const {
            return mode_;
        }
        const std::string& current_directory() const {
            return current_directory_;
        }
        const std::vector<FileDialogEntry>& entries() const {
            return entries_;
        }
        const std::vector<FileDialogFilter>& filters() const {
            return filters_;
        }
        size_t selected_filter() const {
            return selected_filter_;
        }
