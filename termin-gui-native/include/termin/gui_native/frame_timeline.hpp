...
namespace termin::gui_native {
    ...
    public:
        const std::vector<FrameTimelineSample>& samples() const {
            return samples_;
        }
        ...
        void clear();
        Signal<FrameTimelineModel&>& changed() {
            return changed_;
        }
    };
    ...
    public:
        ...
        const std::shared_ptr<FrameTimelineModel>& model() const {
            return model_;
        }
        ...
        std::optional<int64_t> selected_id() const {
            return selected_id_;
        }
        bool select(int64_t stable_id);
        bool clear_selection();
        std::pair<size_t, size_t> visible_range() const;
        float warning_ratio() const {
            return warning_ratio_;
        }
        void set_warning_ratio(float ratio);
        Signal<FrameTimelineWidget&, int64_t>& selection_changed() {
