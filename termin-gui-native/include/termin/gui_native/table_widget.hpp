...
namespace termin::gui_native {
    ...
    public:
        explicit TableWidget(std::shared_ptr<TableModel> model = {}, std::shared_ptr<TableColumnModel> columns = {});
        ~TableWidget() override;

        const std::shared_ptr<TableModel>& model() const {
            return model_;
        }
        ...
        Signal<TableWidget&, size_t, const TableColumn&>& header_clicked() {
            return header_clicked_;
        }
        Signal<TableWidget&, size_t, float>& column_resized() {
