...
namespace termin::gui_native {
    ...
    public:
        ...
        const std::vector<MenuBarEntry>& entries() const {
            return entries_;
        }
        ...
        bool menu_open() const {
            return open_index_ != SIZE_MAX;
        }
        size_t open_index() const {
            return open_index_;
        }
        const std::vector<tc_ui_rect>& item_rects() const {
            return item_rects_;
        }
        bool dispatch_shortcut(int32_t key, int32_t modifiers);

        Signal<MenuBar&, size_t, CommandId, const CommandData&>& activated() {
            return activated_;
        }
    ...
    private:
        ...
        bool dispatch_shortcut_in(const std::shared_ptr<CommandModel>& model,
                                  int32_t key,
                                  int32_t modifiers,
