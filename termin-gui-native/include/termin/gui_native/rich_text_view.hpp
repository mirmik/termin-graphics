...
namespace termin::gui_native {
    ...
    public:
        ...
        const std::shared_ptr<RichTextModel>& model() const {
            return model_;
        }
        ...
        float content_height() const {
            return cached_line_height_ * static_cast<float>(rows_.size());
        }
        size_t visual_line_count() const {
            return rows_.size();
        }
        ...
        size_t source_offset_from_point(tc_ui_document_handle document, tc_ui_rect content, float x, float y) const;
        float
        row_x_for_offset(tc_ui_document_handle document, const VisualRow& row, size_t offset, float font_size) const;
        void append_run(VisualRow& row,
                        std::string text,
