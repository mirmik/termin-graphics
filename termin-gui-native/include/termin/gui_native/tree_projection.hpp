...
namespace termin::gui_native {
    ...
    build_tree_projection(const Roots& roots, Children&& children, Expanded&& expanded, size_t reserve = 0) {
        std::vector<TreeVisibleRow> visible;
        visible.reserve(reserve);
        const auto append = [&](const auto& self, TreeNodeId node, size_t depth) -> void {
