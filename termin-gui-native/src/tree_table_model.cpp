...
namespace termin::gui_native {
    ...
    const std::vector<TreeTableNodeId>& TreeTableModel::children(TreeTableNodeId parent) const {
    ...
    void TreeTableModel::validate_row(const TreeTableRowData& row) {
        if (row.stable_id.empty() || !valid_utf8(row.stable_id) || !valid_utf8(row.parent_stable_id)) {
            tc_log_error("[termin-gui-native] tree-table model rejected invalid stable id");
            throw std::invalid_argument("tree-table stable ids must be non-empty valid UTF-8 strings");
        }
        for (const std::string& cell : row.cells) {
            if (!valid_utf8(cell)) {
                tc_log_error("[termin-gui-native] tree-table model rejected invalid UTF-8 cell");
    ...
    void TreeTableModel::set_rows(std::vector<TreeTableRowData> rows) {
        ...
        for (size_t index = 0; index < rows.size(); ++index) {
            ...
            if (!input_indices.emplace(rows[index].stable_id, index).second) {
                tc_log_error("[termin-gui-native] tree-table model rejected duplicate stable id");
                throw std::invalid_argument("duplicate tree-table stable id");
            }
        }
        for (const TreeTableRowData& row : rows) {
            if (!row.parent_stable_id.empty() && input_indices.find(row.parent_stable_id) == input_indices.end()) {
                tc_log_error("[termin-gui-native] tree-table model rejected missing parent");
                throw std::invalid_argument("tree-table parent stable id is not present");
            }
            if (row.parent_stable_id == row.stable_id) {
                tc_log_error("[termin-gui-native] tree-table model rejected self-parent cycle");
        ...
        std::function<void(size_t)> visit = [&](size_t index) {
            ...
            if (visits[index] == Visit::Visiting) {
                tc_log_error("[termin-gui-native] tree-table model rejected parent cycle");
        ...
        for (TreeTableRowData& row : rows) {
            ...
            next_nodes.emplace(id, TreeTableNode{id, std::move(row), kInvalidTreeTableNodeId, {}});
        }
