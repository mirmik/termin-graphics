...
namespace termin::gui_native {
    ...
    Splitter::Splitter(Orientation orientation, const char* debug_name)
        ...
        set_preferred_size(orientation_ == Orientation::Horizontal ? tc_ui_size{320.0f, 180.0f}
    ...
    void Splitter::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        const bool active = tc_widget_handle_eq(tc_ui_document_pointer_capture(document), handle());
        const tc_ui_style style = computed_style(document, active ? TC_UI_STYLE_STATE_HOVERED : 0);
        tc_ui_painter_fill_rect(context, divider_line_rect(style.border_width), style.background);
    }
        ...
        if (event->type == TC_UI_POINTER_MOVE && captured) {
            ...
            if (axis > 0.0f) {
                const float position =
                    orientation_ == Orientation::Horizontal ? event->x - bounds().x : event->y - bounds().y;
    ...
    tc_widget_handle Splitter::hit_test(tc_ui_document_handle document, float x, float y) {
        ...
        c_widget()->cursor_intent = over_divider || captured
                                        ? (orientation_ == Orientation::Horizontal ? TC_UI_CURSOR_RESIZE_HORIZONTAL
    ...
    tc_ui_rect Splitter::divider_rect() const {
        ...
        if (orientation_ == Orientation::Horizontal) {
            const float first_extent = clamp_float(
                axis * split_fraction_, first_min_extent_, std::max(first_min_extent_, axis - second_min_extent_));
    ...
    tc_ui_rect Splitter::divider_hit_rect() const {
        ...
        const float expansion = std::max(0.0f, (divider_hit_thickness_ - divider_thickness_) * 0.5f);
        if (orientation_ == Orientation::Horizontal) {
            const float x = std::max(bounds().x, divider.x - expansion);
            const float right = std::min(bounds().x + bounds().width, divider.x + divider.width + expansion);
            return tc_ui_rect{x, divider.y, std::max(0.0f, right - x), divider.height};
        }
        const float y = std::max(bounds().y, divider.y - expansion);
        const float bottom = std::min(bounds().y + bounds().height, divider.y + divider.height + expansion);
    ...
    tc_ui_rect Splitter::divider_line_rect(float line_thickness) const {
        ...
        if (orientation_ == Orientation::Horizontal) {
            return tc_ui_rect{divider.x + (divider.width - thickness) * 0.5f, divider.y, thickness, divider.height};
        }
        return tc_ui_rect{divider.x, divider.y + (divider.height - thickness) * 0.5f, divider.width, thickness};
    }
    ...
    void Splitter::layout_children(tc_ui_document_handle document) {
        ...
        if (!tc_widget_handle_is_invalid(this->second())) {
            ...
            tc_ui_rect second_rect =
                orientation_ == Orientation::Horizontal
                    ? tc_ui_rect{bounds().x + first_extent + divider_thickness_,
                                 bounds().y,
                                 second_extent,
                                 bounds().height}
                    : tc_ui_rect{
                          bounds().x, bounds().y + first_extent + divider_thickness_, bounds().width, second_extent};
