...
FlowPlan build_plan(
    ...
    for (size_t index = 0; index < owner.child_count(); ++index) {
        ...
        if (primary_definite) {
            if (horizontal) {
                ...
                child_constraints.max_size.height =
                    std::max(0.0f, available_primary - vertical_margin(margin));
            }
        }
        if (cross_definite) {
            if (horizontal) {
                child_constraints.max_size.height =
                    std::max(0.0f, available_cross - vertical_margin(margin));
            } else {
                child_constraints.max_size.width =
                    std::max(0.0f, available_cross - horizontal_margin(margin));
            }
        }
        const tc_ui_size measured = measure_widget(
            child, document, child_constraints,
            horizontal
                ? tc_ui_size {available_primary, available_cross}
        ...
        line.primary +=
            (line.count > 0 ? owner.spacing() : 0.0f) + item_primary;
...
WrapLayout& WrapLayout::set_line_alignment(CrossAxisAlignment alignment) {
    line_alignment_ =
        alignment == CrossAxisAlignment::Auto
...
tc_ui_size WrapLayout::measure(
    tc_ui_document_handle document,
    tc_ui_constraints constraints
) {
    const bool horizontal = orientation_ == Orientation::Horizontal;
    const float primary_padding = horizontal
        ? padding_.left + padding_.right
        : padding_.top + padding_.bottom;
    const float cross_padding = horizontal
        ? padding_.top + padding_.bottom
        : padding_.left + padding_.right;
    const float maximum_primary = primary_size(
        constraints.max_size, orientation_);
    const float maximum_cross = cross_size(
        constraints.max_size, orientation_);
    const bool primary_definite =
        maximum_primary > 0.0f && maximum_primary < kHuge;
    const bool cross_definite =
        maximum_cross > 0.0f && maximum_cross < kHuge;
    const float available_primary = primary_definite
        ? std::max(0.0f, maximum_primary - primary_padding)
        : kHuge;
    const float available_cross = cross_definite
        ? std::max(0.0f, maximum_cross - cross_padding)
        : kHuge;
    const FlowPlan plan = build_plan(
        *this, document, available_primary, available_cross,
        primary_definite, cross_definite);
    const tc_ui_size measured = horizontal
        ? tc_ui_size {
              plan.primary + primary_padding,
              plan.cross + cross_padding}
        : tc_ui_size {
              plan.cross + cross_padding,
              plan.primary + primary_padding};
...
void WrapLayout::layout(
    ...
    const float available_primary =
        horizontal ? content.width : content.height;
    const float available_cross =
        horizontal ? content.height : content.width;
    const FlowPlan plan = build_plan(
        *this, document, available_primary, available_cross, true, true);
    ...
    for (const FlowLine& line : plan.lines) {
        ...
        for (size_t offset = 0; offset < line.count; ++offset) {
            ...
            const float item_primary =
                primary_size(item.measured, orientation_);
            float item_cross = cross_size(item.measured, orientation_);
            const float primary_before = horizontal
                ? item.margin.left
                : item.margin.top;
            const float primary_after = horizontal
                ? item.margin.right
                : item.margin.bottom;
            const float cross_before = horizontal
                ? item.margin.top
                : item.margin.left;
            const float cross_after = horizontal
                ? item.margin.bottom
                : item.margin.right;
            const float usable_cross =
                std::max(0.0f, line.cross - cross_before - cross_after);
            ...
            if (line_alignment_ == CrossAxisAlignment::Stretch) {
                ...
                cross_offset =
                    cross_before + std::max(0.0f, usable_cross - item_cross) * 0.5f;
            } else if (line_alignment_ == CrossAxisAlignment::End) {
                cross_offset =
                    cross_before + std::max(0.0f, usable_cross - item_cross);
            }
            const tc_ui_rect child_rect = horizontal
                ? tc_ui_rect {
                      primary_cursor + primary_before,
            ...
            primary_cursor +=
                primary_before + item_primary + primary_after + spacing_;
        }
        cross_cursor += line.cross + line_spacing_;
    }
}

void WrapLayout::paint(
    tc_ui_document_handle document,
...
tc_ui_event_result WrapLayout::pointer_event(
...
tc_widget_handle WrapLayout::hit_test(
    tc_ui_document_handle document,
    float x,
    float y
) {
