...
namespace termin::gui_native {
    ...
    namespace {
        ...
        std::string
        elide_text(tc_ui_document_handle document, std::string_view text, float font_size, float max_width) {
            ...
            if (!measure_text(document, ellipsis, font_size, ellipsis_metrics) || ellipsis_metrics.width >= max_width) {
            ...
            while (end > 0) {
                ...
                if (!measure_text(document, text.substr(0, end), font_size, metrics) || metrics.width <= available) {
                    break;
                }
            }
    ...
    namespace detail {
        ...
        void draw_semantic_file_icon(tc_ui_paint_context* context,
            ...
            if (icon == "folder") {
                ...
                tc_ui_painter_fill_rounded_rect(
                    context,
                    tc_ui_rect{rect.x, rect.y + tab_height, rect.width, rect.height - tab_height},
                    2.5f,
                    highlight);
                return;
            }
    ...
    size_t FileGridWidget::column_count() const {
        ...
        return std::max<size_t>(1, static_cast<size_t>((usable + tile_spacing_) / (tile_width_ + tile_spacing_)));
    }
    ...
    float FileGridWidget::content_height() const {
        ...
        return padding_ * 2.0f + static_cast<float>(rows) * tile_height_ + static_cast<float>(rows - 1) * tile_spacing_;
    }
    ...
    void FileGridWidget::clamp_scroll() {
        scroll_y_ = clamp_float(scroll_y_, 0.0f, max_scroll());
    }
    ...
    bool FileGridWidget::has_scrollbar() const {
        return show_scrollbar_ && max_scroll() > 0.0f;
    }
    ...
    tc_ui_rect FileGridWidget::scrollbar_thumb_rect() const {
        ...
        const float y = bounds().y + (max_scroll() > 0.0f ? track_height * scroll_y_ / max_scroll() : 0.0f);
        return tc_ui_rect{bounds().x + bounds().width - scrollbar_width_, y, scrollbar_width_, thumb_height};
    }

    bool FileGridWidget::scrollbar_hit(float x, float y) const {
        return has_scrollbar() && rect_contains(tc_ui_rect{bounds().x + bounds().width - scrollbar_width_,
                                                           bounds().y,
                                                           scrollbar_width_,
                                                           bounds().height},
                                                x,
    ...
    tc_ui_rect FileGridWidget::item_rect(size_t index) const {
        ...
        return tc_ui_rect{bounds().x + padding_ + static_cast<float>(column) * (tile_width_ + tile_spacing_),
                          bounds().y + padding_ + static_cast<float>(row) * (tile_height_ + tile_spacing_) - scroll_y_,
                          tile_width_,
                          tile_height_};
    }
    ...
    std::pair<size_t, size_t> FileGridWidget::visible_range() const {
        ...
        const size_t first_row = static_cast<size_t>(std::max(0.0f, std::floor((scroll_y_ - padding_) / stride)));
        const size_t last_row = static_cast<size_t>(std::ceil((scroll_y_ + bounds().height) / stride)) + 1;
        const size_t columns = column_count();
        return {std::min(model_->size(), first_row * columns), std::min(model_->size(), last_row * columns)};
    }
            ...
            for (size_t index = first; index < last; ++index) {
                ...
                if (icon_size_ > 0.0f) {
                    const tc_ui_rect icon_rect{
                        tile.x + (tile.width - icon_size_) * 0.5f, tile.y + 8.0f, icon_size_, icon_size_};
                    if (item.texture_id != 0) {
                        tc_ui_painter_draw_texture(
                            context, item.texture_id, icon_rect, foreground, TC_UI_TEXTURE_SAMPLING_LINEAR, false);
                    } else {
    ...
    size_t FileGridWidget::index_at(float x, float y) const {
        ...
        if (column >= column_count() || local_x - static_cast<float>(column) * column_stride > tile_width_ ||
    ...
    tc_ui_event_result FileGridWidget::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
            return TC_UI_EVENT_IGNORED;
        sync_model();
        if (event->type == TC_UI_POINTER_CANCEL) {
            const bool active = dragging_scrollbar_ || pressed_item_ != SelectionModel::npos || dragging_item_;
        ...
        if (dragging_scrollbar_) {
            if (event->type == TC_UI_POINTER_MOVE) {
                ...
                if (track > 0.0f) {
                    set_scroll_y(drag_start_scroll_ + (event->y - drag_start_y_) * max_scroll() / track);
                }
        ...
        if (event->button == tcbase::mouse_button_value(tcbase::MouseButton::RIGHT)) {
            ...
            context_menu_requested_.emit(
                *this, index == SelectionModel::npos ? -1 : static_cast<int64_t>(index), event->x, event->y);
            return TC_UI_EVENT_HANDLED;
        }
        if (event->button != tcbase::mouse_button_value(tcbase::MouseButton::LEFT))
            return TC_UI_EVENT_IGNORED;
    ...
    tc_ui_event_result FileGridWidget::key_event(tc_ui_document_handle, const tc_ui_key_event* event) {
        ...
        if (event->key == TC_UI_KEY_HOME)
            ...
            target = current == SelectionModel::npos || current < step ? next_enabled(SelectionModel::npos, 1)
