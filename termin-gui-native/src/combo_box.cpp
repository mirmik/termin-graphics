...
namespace termin::gui_native {
    ...
    public:
        explicit ComboBoxPopup(ComboBox& owner)
            : NativeWidget("ComboBoxPopup"),
              owner_(owner) {}

        tc_ui_size measure(tc_ui_document_handle, tc_ui_constraints constraints) override {
            const float height = std::min(owner_.items_.size(), owner_.max_visible_items_) * owner_.item_height_;
            return clamp_size(tc_ui_size{owner_.bounds().width, height}, constraints);
        }
        ...
        tc_ui_event_result pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) override {
            ...
            if (event->type == TC_UI_POINTER_WHEEL) {
                ...
                scroll_y_ = clamp_float(scroll_y_ - event->wheel_y * owner_.item_height_ * 2.0f,
                                        0.0f,
    ...
    std::string ComboBox::selected_text() const {
        return selected_index_ >= 0 && selected_index_ < static_cast<int>(items_.size()) ? items_[selected_index_]
                                                                                         : std::string{};
    }
    ...
    tc_ui_size ComboBox::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        for (const std::string& item : items_) {
            ...
            if (measure_text(document, item, style.font_size, metrics))
    ...
    void ComboBox::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        if (style.border_width > 0.0f) {
            tc_ui_painter_stroke_rounded_rect(
                context, bounds(), style.corner_radius, open_ ? style.accent : style.border, style.border_width);
        }
        tc_ui_painter_draw_text(context,
                                text.c_str(),
                                tc_ui_point{bounds().x + 8.0f, bounds().y + bounds().height * 0.68f},
                                style.font_size,
                                style.foreground);
        tc_ui_painter_draw_text(context,
                                open_ ? "^" : "v",
                                tc_ui_point{bounds().x + bounds().width - 18.0f, bounds().y + bounds().height * 0.68f},
                                style.font_size,
                                style.foreground);
    }
    ...
    bool ComboBox::show_popup(tc_ui_document_handle document) {
        ...
        if (tc_widget_handle_is_invalid(popup_handle_)) {
            ...
            popup_handle_ = tc_ui_document_adopt_widget(document, popup->c_widget(), &Widget::delete_owned_widget);
            if (tc_widget_handle_is_invalid(popup_handle_))
                return false;
            popup.release();
        }
        ...
        open_ = tc_ui_document_show_overlay_with_layout(
            document, popup_handle_, TC_UI_OVERLAY_DISMISS_ON_OUTSIDE, &overlay_layout, root->bounds);
        mark_dirty(TC_WIDGET_DIRTY_STATE | TC_WIDGET_DIRTY_PAINT);
        return open_;
    }
    ...
    void ComboBox::hide_popup(tc_ui_document_handle document) {
        if (open_)
            tc_ui_document_dismiss_overlay(document, popup_handle_, TC_UI_OVERLAY_DISMISS_PROGRAMMATIC);
    }
