...
void ScrollArea::set_scrollbar_policy(
    ScrollBarPolicy horizontal,
    ScrollBarPolicy vertical
) {
    if (horizontal_scrollbar_policy_ == horizontal &&
...
bool ScrollArea::horizontal_scrollbar_visible() const {
    if (!horizontal_scroll_enabled_ ||
        horizontal_scrollbar_policy_ == ScrollBarPolicy::Hidden) {
        return false;
    }
    return horizontal_scrollbar_policy_ == ScrollBarPolicy::Always ||
        max_scroll_x() > SCROLL_EPSILON;
}

bool ScrollArea::vertical_scrollbar_visible() const {
    if (!vertical_scroll_enabled_ ||
        vertical_scrollbar_policy_ == ScrollBarPolicy::Hidden) {
        return false;
    }
    return vertical_scrollbar_policy_ == ScrollBarPolicy::Always ||
        max_scroll_y() > SCROLL_EPSILON;
}
...
float ScrollArea::max_scroll_x() const {
    return horizontal_scroll_enabled_
        ? std::max(0.0f, content_size_.width - bounds().width)
        : 0.0f;
}

float ScrollArea::max_scroll_y() const {
    return vertical_scroll_enabled_
        ? std::max(0.0f, content_size_.height - bounds().height)
        : 0.0f;
}

bool ScrollArea::apply_scroll(
    tc_ui_document_handle document,
    float x,
    float y
) {
    const float next_x = horizontal_scroll_enabled_
        ? clamp_float(x, 0.0f, max_scroll_x())
        : 0.0f;
    const float next_y = vertical_scroll_enabled_
        ? clamp_float(y, 0.0f, max_scroll_y())
        : 0.0f;
    ...
    if (!tc_ui_document_handle_is_invalid(document) &&
        tc_ui_document_is_alive(document, handle())) {
...
void ScrollArea::layout_content(tc_ui_document_handle document) {
    ...
    tc_widget* content =
        resolve_child(document, c_widget(), this->content(), "ScrollArea::layout_content");
    if (!content) {
        return;
    }
    const tc_ui_insets margin = tc_widget_layout_spec(content).margin;
...
tc_ui_rect ScrollArea::horizontal_track_rect() const {
    const float vertical_reserve =
        vertical_scrollbar_visible() ? SCROLLBAR_THICKNESS + SCROLLBAR_INSET : 0.0f;
    return tc_ui_rect {
        bounds().x + SCROLLBAR_INSET,
...
tc_ui_rect ScrollArea::vertical_track_rect() const {
    const float horizontal_reserve =
        horizontal_scrollbar_visible() ? SCROLLBAR_THICKNESS + SCROLLBAR_INSET : 0.0f;
    return tc_ui_rect {
        bounds().x + bounds().width - SCROLLBAR_THICKNESS - SCROLLBAR_INSET,
...
tc_ui_rect ScrollArea::horizontal_thumb_rect() const {
    ...
    const float extent = content_size_.width > 0.0f
        ? std::min(
              track.width,
              std::max(MIN_THUMB_EXTENT, track.width * viewport / content_size_.width)
          )
        : track.width;
    const float travel = std::max(0.0f, track.width - extent);
    const float position = max_scroll_x() > 0.0f
        ? travel * scroll_x_ / max_scroll_x()
        : 0.0f;
...
tc_ui_rect ScrollArea::vertical_thumb_rect() const {
    ...
    const float extent = content_size_.height > 0.0f
        ? std::min(
              track.height,
              std::max(MIN_THUMB_EXTENT, track.height * viewport / content_size_.height)
          )
        : track.height;
    const float travel = std::max(0.0f, track.height - extent);
    const float position = max_scroll_y() > 0.0f
        ? travel * scroll_y_ / max_scroll_y()
        : 0.0f;
...
void ScrollArea::paint(
    ...
    if (!tc_widget_handle_is_invalid(this->content())) {
        tc_widget* content =
            resolve_child(document, c_widget(), this->content(), "ScrollArea::paint");
...
tc_ui_event_result ScrollArea::pointer_event(
    tc_ui_document_handle document,
    ...
    if (event->type == TC_UI_POINTER_MOVE &&
        (drag_axis_ != DragAxis::None || captured)) {
        if (drag_axis_ == DragAxis::Horizontal) {
            ...
            if (travel > 0.0f) {
                apply_scroll(
                    document,
                    drag_scroll_origin_ +
                        (event->x - drag_pointer_origin_) * max_scroll_x() / travel,
                    scroll_y_
                );
            }
            ...
            if (travel > 0.0f) {
                apply_scroll(
                    ...
                    scroll_x_,
                    drag_scroll_origin_ +
                        (event->y - drag_pointer_origin_) * max_scroll_y() / travel
                );
            }
        }
    ...
    if (event->type == TC_UI_POINTER_WHEEL) {
        const float delta_x =
            horizontal_scroll_enabled_ ? -event->wheel_x * wheel_step_ : 0.0f;
        const float delta_y =
            vertical_scroll_enabled_ ? -event->wheel_y * wheel_step_ : 0.0f;
        return apply_scroll(
            document,
            scroll_x_ + delta_x,
            scroll_y_ + delta_y
    ...
    if (horizontal_scrollbar_visible() &&
        rect_contains(horizontal_track_rect(), event->x, event->y)) {
        const tc_ui_rect thumb = horizontal_thumb_rect();
        if (rect_contains(thumb, event->x, event->y)) {
            ...
            apply_scroll(
                document,
                scroll_x_ + (event->x < thumb.x ? -bounds().width : bounds().width),
                scroll_y_
            );
        }
        return TC_UI_EVENT_HANDLED;
    ...
    if (vertical_scrollbar_visible() &&
        rect_contains(vertical_track_rect(), event->x, event->y)) {
        const tc_ui_rect thumb = vertical_thumb_rect();
        if (rect_contains(thumb, event->x, event->y)) {
            ...
            apply_scroll(
                document,
                scroll_x_,
                scroll_y_ + (event->y < thumb.y ? -bounds().height : bounds().height)
            );
        }
        return TC_UI_EVENT_HANDLED;
    }
...
tc_widget_handle ScrollArea::hit_test(
    tc_ui_document_handle document,
    float x,
    float y
) {
    ...
    if ((horizontal_scrollbar_visible() &&
         rect_contains(horizontal_track_rect(), x, y)) ||
        (vertical_scrollbar_visible() &&
         rect_contains(vertical_track_rect(), x, y))) {
        return handle();
    }
    if (!tc_widget_handle_is_invalid(this->content())) {
        tc_widget* content =
            resolve_child(document, c_widget(), this->content(), "ScrollArea::hit_test");
        if (content && content->vtable && content->vtable->hit_test) {
            const tc_widget_handle hit =
                content->vtable->hit_test(content, document, x, y);
...
tc_ui_event_result ScrollArea::key_event(
    tc_ui_document_handle document,
    ...
    return apply_scroll(document, next_x, next_y)
        ? TC_UI_EVENT_HANDLED
        : TC_UI_EVENT_IGNORED;
}
...
bool ScrollArea::ensure_visible(tc_widget_handle descendant) {
    ...
    if (tc_ui_document_handle_is_invalid(current_document) ||
        ...
        tc_log_error(
            "[termin-gui-native] ScrollArea cannot reveal invalid or stale descendant"
        );
        return false;
    }
    tc_widget* target =
        tc_ui_document_resolve_widget(current_document, descendant);
    bool belongs = false;
    ...
    if (!belongs || target == c_widget()) {
        tc_log_error(
            "[termin-gui-native] ScrollArea cannot reveal a non-descendant widget"
        );
        return false;
    }
    ...
    if (horizontal_scroll_enabled_) {
        if (target_rect.x < bounds().x) {
            ...
            next_x += target_rect.x + target_rect.width -
                (bounds().x + bounds().width);
        }
    }
    if (vertical_scroll_enabled_) {
        if (target_rect.y < bounds().y) {
            ...
            next_y += target_rect.y + target_rect.height -
                (bounds().y + bounds().height);
        }
    }
...
void ScrollArea::descendant_focused(
