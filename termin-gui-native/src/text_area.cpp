...
namespace termin::gui_native {
    ...
    std::string TextArea::selected_text() const {
        return has_selection() ? text_.substr(selection_start(), selection_end() - selection_start()) : std::string{};
    }
    ...
    bool TextArea::measure_range(
        tc_ui_document_handle document, size_t start, size_t end, float font_size, float& width) const {
    ...
    size_t TextArea::caret_from_line_x(tc_ui_document_handle document, const Line& line, float content_x) const {
        const tc_ui_style style = computed_style(document);
    ...
    size_t TextArea::caret_from_point(tc_ui_document_handle document, float x, float y) const {
        ...
        const size_t line_index = std::min(spans.size() - 1, static_cast<size_t>(content_y / height));
        return caret_from_line_x(document, spans[line_index], std::max(0.0f, x - clip.x + scroll_x_));
    }
    ...
    void TextArea::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        const size_t first_line = std::min(spans.size() - 1, static_cast<size_t>(std::max(0.0f, scroll_y_) / height));
        ...
        if (focused) {
            ...
            tc_ui_painter_draw_line(context, tc_ui_point{x, y}, tc_ui_point{x, y + height}, style.accent, 1.0f);
        }
        tc_ui_painter_pop_clip(context);
    }

    tc_ui_event_result TextArea::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event) {
        ...
        if (event->type == TC_UI_POINTER_WHEEL && rect_contains(bounds(), event->x, event->y)) {
            ...
            const float next_scroll = clamp_float(scroll_y_ - event->wheel_y * height * 3.0f, 0.0f, max_scroll);
    ...
    tc_ui_event_result TextArea::key_event(tc_ui_document_handle document, const tc_ui_key_event* event) {
        ...
        case TC_UI_KEY_LEFT:
            if (!extend && has_selection())
                ...
                move_caret(utf8_previous_boundary(text_, caret_), extend);
            break;
        case TC_UI_KEY_RIGHT:
            if (!extend && has_selection())
                move_caret(selection_end(), false);
            else
                move_caret(utf8_next_boundary(text_, caret_), extend);
            break;
    ...
    tc_ui_event_result TextArea::text_event(tc_ui_document_handle document, const tc_ui_text_event* event) {
    ...
    void TextArea::ensure_caret_visible(tc_ui_document_handle document) {
        ...
        scroll_y_ =
            clamp_float(scroll_y_, 0.0f, std::max(0.0f, static_cast<float>(spans.size()) * height - clip.height));
    }
    ...
    void TextArea::move_vertical(tc_ui_document_handle document, int direction, bool extend_selection) {
        const auto spans = lines();
        const size_t current = line_index_for_offset(spans, caret_);
        if ((direction < 0 && current == 0) || (direction > 0 && current + 1 >= spans.size())) {
            return;
        }
        const tc_ui_style style = computed_style(document);
        if (desired_x_ < 0.0f) {
