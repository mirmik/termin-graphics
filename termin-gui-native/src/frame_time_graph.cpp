...
namespace termin::gui_native {
    ...
    void FrameTimeGraph::set_thresholds(float target_frame_ms, float warning_frame_ms) {
        if (!std::isfinite(target_frame_ms) || !std::isfinite(warning_frame_ms) || target_frame_ms <= 0.0f ||
            warning_frame_ms <= target_frame_ms) {
    ...
    void FrameTimeGraph::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        if (model_->samples().empty()) {
            ...
            tc_ui_painter_draw_text(
                context,
                "No profiler data",
                tc_ui_point{rect.x + (rect.width - metrics.width) * 0.5f, rect.y + (rect.height + font_size) * 0.5f},
                font_size,
                style.foreground);
            return;
        }
        ...
        for (float threshold : {target_frame_ms_, warning_frame_ms_}) {
            if (threshold < scale) {
                ...
                tc_ui_painter_draw_line(
                    context, tc_ui_point{rect.x, y}, tc_ui_point{rect.x + rect.width, y}, grid, 1.0f);
            }
        }
        const float target_y = y_for(target_frame_ms_);
        tc_ui_painter_draw_line(
            context, tc_ui_point{rect.x, target_y}, tc_ui_point{rect.x + rect.width, target_y}, target, 1.0f);
        ...
        tc_ui_painter_draw_text(context, target_label, tc_ui_point{rect.x + 2.0f, target_y - 2.0f}, label_size, target);
        tc_ui_painter_draw_text(
            context, warning_label, tc_ui_point{rect.x + 2.0f, y_for(warning_frame_ms_) - 2.0f}, label_size, target);
