...
namespace termin::gui_native {
    ...
    TreeTableWidget::TreeTableWidget(std::shared_ptr<TreeTableModel> model,
        ...
        : NativeWidget("TreeTableWidget"),
          ...
          expansion_(expansion ? std::move(expansion) : std::make_shared<TreeExpansionModel>()) {
    ...
    void TreeTableWidget::connect_models() {
        if (model_ && model_connection_ == 0) {
            model_connection_ = model_->changed().connect([this](TreeTableModel&) { on_model_changed(); });
        }
        if (columns_ && column_connection_ == 0) {
            column_connection_ = columns_->changed().connect(
                [this](TableColumnModel&, const TableColumnChange&) { on_columns_changed(); });
        }
        if (expansion_ && expansion_connection_ == 0) {
            expansion_connection_ = expansion_->changed().connect(
                [this](TreeExpansionModel&, TreeNodeId node, bool value) { on_expansion_changed(node, value); });
        }
    }
    ...
    void TreeTableWidget::set_column_model(std::shared_ptr<TableColumnModel> columns) {
        disconnect_models();
        columns_ = columns ? std::move(columns) : std::make_shared<TableColumnModel>();
        observed_column_revision_ = columns_->revision();
    ...
    void TreeTableWidget::set_expansion_model(std::shared_ptr<TreeExpansionModel> expansion) {
        disconnect_models();
        expansion_ = expansion ? std::move(expansion) : std::make_shared<TreeExpansionModel>();
        observed_expansion_revision_ = expansion_->revision();
        connect_models();
    ...
    void TreeTableWidget::on_expansion_changed(TreeTableNodeId node, bool expanded) {
        observed_expansion_revision_ = expansion_->revision();
        rebuild_visible();
        clamp_scroll();
        mark_dirty(TC_WIDGET_DIRTY_LAYOUT | TC_WIDGET_DIRTY_STATE | TC_WIDGET_DIRTY_PAINT);
    ...
    void TreeTableWidget::rebuild_visible() {
        visible_ = build_tree_projection(
            model_->roots(),
            [this](TreeNodeId node) -> const std::vector<TreeNodeId>& { return model_->node(node).children; },
    ...
    void TreeTableWidget::set_row_height(float height) {
        if (!std::isfinite(height) || height <= 0.0f) {
            tc_log_error("[termin-gui-native] TreeTableWidget rejected invalid row height");
    ...
    void TreeTableWidget::set_header_height(float height) {
        if (!std::isfinite(height) || height <= 0.0f) {
            tc_log_error("[termin-gui-native] TreeTableWidget rejected invalid header height");
    ...
    void TreeTableWidget::set_indent_size(float size) {
        if (!std::isfinite(size) || size < 0.0f) {
            tc_log_error("[termin-gui-native] TreeTableWidget rejected invalid indentation");
    ...
    std::pair<size_t, size_t> TreeTableWidget::visible_range() {
        ...
        if (visible_.empty() || body_height() <= 0.0f)
            return {0, 0};
        const size_t first =
            std::min(visible_.size(), static_cast<size_t>(std::max(0.0f, std::floor(scroll_y_ / row_height_))));
        const size_t last =
            std::min(visible_.size(), static_cast<size_t>(std::ceil((scroll_y_ + body_height()) / row_height_)) + 1);
        return {first, last};
    }
    ...
    tc_ui_size TreeTableWidget::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        sync_models();
    ...
    void TreeTableWidget::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        sync_models();
        const tc_ui_style style = computed_style(document);
        ...
        tc_ui_painter_draw_line(context,
                                tc_ui_point{header.x, header.y + header.height},
        ...
        if (visible_.empty()) {
            ...
            for (size_t index = first; index < last; ++index) {
                ...
                const float y = body.y + static_cast<float>(index) * row_height_ - scroll_y_;
                const tc_ui_rect row{body.x, y, body.width, row_height_};
                tc_ui_srgb_color row_color = index % 2 == 0 ? style.background : style.border;
                ...
                if (!node.data.enabled)
                    foreground.a *= 0.45f;
                const size_t cell_count = std::min(node.data.cells.size(), column_layout_.size());
                for (size_t column = 0; column < cell_count; ++column) {
                    const TableColumnLayout layout = column_layout_[column];
                    const tc_ui_rect cell{row.x + layout.x, row.y, layout.width, row.height};
                    tc_ui_painter_push_clip(context, cell);
                    float text_x = cell.x + cell_padding_;
                    if (column == 0) {
                        const float toggle_x = text_x + static_cast<float>(row_info.depth) * indent_size_;
                    ...
                    tc_ui_painter_draw_text(context,
                                            node.data.cells[column].c_str(),
                                            tc_ui_point{text_x, cell.y + cell.height * 0.66f},
                                            style.font_size,
                                            foreground);
                    tc_ui_painter_pop_clip(context);
                }
    ...
    bool TreeTableWidget::point_in_toggle(const TreeVisibleRow& row, float x) const {
        if (column_layout_.empty())
            return false;
        const float left =
            bounds().x + column_layout_[0].x + cell_padding_ + static_cast<float>(row.depth) * indent_size_;
        return x >= left && x < left + toggle_size_;
    }

    tc_ui_event_result TreeTableWidget::pointer_event(tc_ui_document_handle document,
                                                      const tc_ui_pointer_event* event) {
        if (!event)
            return TC_UI_EVENT_IGNORED;
        sync_models();
        if (event->type == TC_UI_POINTER_CANCEL) {
            const bool was_resizing = resizing_column_ != SIZE_MAX;
            resizing_column_ = SIZE_MAX;
            if (was_resizing)
                mark_dirty(TC_WIDGET_DIRTY_STATE | TC_WIDGET_DIRTY_PAINT);
        ...
        if (resizing_column_ != SIZE_MAX) {
            if (event->type == TC_UI_POINTER_MOVE) {
                const float width =
                    columns_->resize(resizing_column_, resize_start_width_ + event->x - resize_start_x_);
        ...
        if (event->type == TC_UI_POINTER_MOVE) {
            ...
            const TreeTableNodeId next = index == SIZE_MAX ? kInvalidTreeTableNodeId : visible_[index].node;
            if (next != hovered_) {
                hovered_ = next;
                mark_dirty(TC_WIDGET_DIRTY_STATE | TC_WIDGET_DIRTY_PAINT);
            }
        ...
        const size_t index = row_index_at(event->x, event->y);
        const TreeTableNodeId node = index == SIZE_MAX ? kInvalidTreeTableNodeId : visible_[index].node;
        ...
        if (event->button != tcbase::mouse_button_value(tcbase::MouseButton::LEFT) || node == kInvalidTreeTableNodeId ||
            !model_->node(node).data.enabled) {
            activation_clicks_.clear();
            return TC_UI_EVENT_IGNORED;
        }
        if (!model_->node(node).children.empty() && point_in_toggle(visible_[index], event->x)) {
            toggle(node);
            return TC_UI_EVENT_HANDLED;
        }
    ...
    TreeTableNodeId TreeTableWidget::next_enabled(size_t from, int direction) const {
        if (visible_.empty())
            return kInvalidTreeTableNodeId;
        std::ptrdiff_t index = from == SIZE_MAX ? (direction > 0 ? 0 : static_cast<std::ptrdiff_t>(visible_.size()) - 1)
    ...
    TreeTableNodeId TreeTableWidget::first_enabled_child(TreeTableNodeId node) const {
    ...
    tc_ui_event_result TreeTableWidget::key_event(tc_ui_document_handle, const tc_ui_key_event* event) {
