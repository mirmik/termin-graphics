...
T* adopt(tc_ui_document_handle document, std::unique_ptr<T> widget, tc_widget_handle& handle) {
    handle =
        tc_ui_document_adopt_widget(document, widget->c_widget(), &Widget::delete_owned_widget);
...
bool FileDialogOverlay::ensure_content(tc_ui_document_handle document) {
    ...
    auto* places_panel =
        adopt(document, std::make_unique<VStack>("file-dialog-places"), places_panel_handle);
    auto* files_panel =
        adopt(document, std::make_unique<VStack>("file-dialog-files"), files_panel_handle);
    auto* places_title = adopt(document, std::make_unique<Label>("Places"), places_title_handle);
    auto* files_title =
        adopt(document, std::make_unique<Label>("Directory contents"), files_title_handle);
    ...
    auto* new_folder =
        adopt(document, std::make_unique<Button>("New Folder"), new_folder_button_handle_);
    auto* places =
        adopt(document, std::make_unique<ListWidget>(places_model_), places_list_handle_);
    auto* list = adopt(document, std::make_unique<ListWidget>(entries_model_), list_handle_);
    auto* selection =
        adopt(document, std::make_unique<Label>("Selection: none"), selection_label_handle_);
    auto* filter_row =
        adopt(document, std::make_unique<HStack>("file-dialog-filter-row"), filter_row_handle);
    auto* filter_label =
        adopt(document, std::make_unique<Label>("File type:"), filter_label_handle);
    ...
    if (!root || !navigation || !body || !places_panel || !files_panel || !places_title ||
        !files_title || !back || !forward || !up || !home || !path || !go || !new_folder ||
        !places || !list || !selection || !filter_row || !filter_label || !filter || !name_row ||
        !name_label || !name || !error) {
...
void FileDialogOverlay::sync_entries() {
    ...
    for (const FileDialogEntry& entry : model_.entries()) {
        items.emplace_back(entry.path, entry.name, entry_subtitle(entry), true, 0,
