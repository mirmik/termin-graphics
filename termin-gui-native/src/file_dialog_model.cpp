...
namespace termin::gui_native {
    ...
    std::string
    NativeFileDialogFileSystem::normalize(std::string_view path, std::string_view base, std::string& error) const {
        error.clear();
        ...
        try {
            ...
            tc_log_error("[termin-gui-native] file dialog failed to normalize path: %s", exception.what());
            return {};
        }
    }

    bool NativeFileDialogFileSystem::inspect(std::string_view path, FileDialogEntry& entry, std::string& error) const {
        error.clear();
        try {
            ...
            error = exception.what();
            tc_log_error("[termin-gui-native] file dialog failed to inspect path: %s", exception.what());
            return false;
        }
    }
    ...
    std::vector<FileDialogEntry> NativeFileDialogFileSystem::list(std::string_view directory,
        ...
        try {
            for (const fs::directory_entry& item : fs::directory_iterator(make_path(directory))) {
                ...
                if (!detail::valid_utf8(entry.name) || !detail::valid_utf8(entry.path)) {
                    tc_log_error("[termin-gui-native] file dialog skipped invalid UTF-8 directory entry");
                    continue;
                }
                entry.is_directory = fs::is_directory(status);
            ...
            error = exception.what();
            tc_log_error("[termin-gui-native] file dialog failed to list directory: %s", exception.what());
            result.clear();
        }
        return result;
    }
    ...
    bool NativeFileDialogFileSystem::create_directory(std::string_view path, std::string& error) {
        ...
        try {
            ...
            error = exception.what();
            tc_log_error("[termin-gui-native] file dialog failed to create directory: %s", exception.what());
            return false;
        }
    }

    FileDialogModel::FileDialogModel(FileDialogMode mode, std::shared_ptr<FileDialogFileSystem> file_system)
        : mode_(mode),
          file_system_(file_system ? std::move(file_system) : std::make_shared<NativeFileDialogFileSystem>()) {}
    ...
    std::vector<FileDialogFilter> FileDialogModel::parse_filter_string(std::string_view text) {
        ...
        while (part_start <= text.size()) {
            ...
            const std::string_view raw = text.substr(
                part_start, part_end == std::string_view::npos ? text.size() - part_start : part_end - part_start);
            ...
            if (first != std::string_view::npos) {
                ...
                const std::string_view label = separator == std::string_view::npos ? part : part.substr(0, separator);
                const size_t label_first = label.find_first_not_of(" \t");
                const size_t label_last = label.find_last_not_of(" \t");
                filter.label = label_first == std::string_view::npos
                                   ? "Files"
                ...
                if (separator == std::string_view::npos) {
                    ...
                    while (offset < patterns.size()) {
                        ...
                        filter.patterns.emplace_back(patterns.substr(
                            offset, end == std::string_view::npos ? patterns.size() - offset : end - offset));
    ...
    void FileDialogModel::set_filters(std::vector<FileDialogFilter> filters) {
        for (const FileDialogFilter& filter : filters) {
            const bool invalid_pattern =
                std::any_of(filter.patterns.begin(), filter.patterns.end(), [](const std::string& item) {
                    return item.empty() || !detail::valid_utf8(item);
                });
    ...
    bool FileDialogModel::refresh() {
        ...
        for (FileDialogEntry& entry : listed) {
            if (entry.is_directory || (mode_ != FileDialogMode::OpenDirectory && accepts_file(entry.name)))
                entries_.push_back(std::move(entry));
        }
        std::sort(entries_.begin(), entries_.end(), [](const FileDialogEntry& left, const FileDialogEntry& right) {
    ...
    bool FileDialogModel::create_unique_directory(std::string_view base_name) {
        if (!detail::valid_utf8(base_name) || base_name.empty() || make_path(base_name).is_absolute() ||
            make_path(base_name).has_parent_path()) {
            set_error("directory name must be a single path component");
            return false;
        }
        if (current_directory_.empty()) {
        ...
        for (size_t suffix = 1; suffix <= 10000; ++suffix) {
            const std::string name =
                suffix == 1 ? std::string(base_name) : std::string(base_name) + " " + std::to_string(suffix);
            const std::string path = path_string(make_path(current_directory_) / make_path(name));
            std::string error;
            if (file_system_->create_directory(path, error))
