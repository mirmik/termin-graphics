...
namespace termin::gui_native {
    namespace {
        ...
        RichTextStyle parse_style_attribute(std::string_view declaration, RichTextStyle style) {
            ...
            while (offset <= declaration.size()) {
                ...
                std::string_view part = declaration.substr(
                    offset, separator == std::string_view::npos ? declaration.size() - offset : separator - offset);
                ...
                if (colon != std::string_view::npos) {
                    ...
                    if (key == "color") {
                        ...
                        style.bold =
                            value == "bold" || value == "600" || value == "700" || value == "800" || value == "900";
        ...
        std::optional<std::string_view> find_style_attribute(std::string_view tag) {
            ...
            while (found != std::string::npos) {
                ...
                if (cursor < tag.size() && tag[cursor] == '=') {
                    ...
                    return tag.substr(cursor, end == std::string_view::npos ? tag.size() - cursor : end - cursor);
                }
    ...
    void RichTextModel::set_lines(std::vector<RichTextLine> lines) {
        replace_lines(std::move(lines));
    }
    ...
    void RichTextModel::clear() {
        replace_lines({RichTextLine{}});
    }
