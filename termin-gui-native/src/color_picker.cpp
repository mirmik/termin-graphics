...
namespace termin::gui_native {
    ...
    void ColorPicker::connect_model() {
        if (!model_ || model_connection_ != 0)
            return;
        model_connection_ =
            model_->changed().connect([this](ColorPickerModel&, uint32_t flags) { on_model_changed(flags); });
    }
    ...
    tc_ui_size ColorPicker::measure(tc_ui_document_handle, tc_ui_constraints constraints) {
        const float width = surface_size_ + gap_ + bar_width_ + (model_->show_alpha() ? gap_ + bar_width_ : 0.0f);
        return clamp_size(tc_ui_size{width, surface_size_ + gap_ + preview_height_ + label_height_}, constraints);
    }
    ...
    void ColorPicker::layout(tc_ui_document_handle document, tc_ui_rect rect) {
        ...
        surface_size_ = std::max(
            1.0f, std::min(180.0f, std::min(rect.width - bars, rect.height - gap_ - preview_height_ - label_height_)));
    }
    ...
    tc_ui_rect ColorPicker::hue_rect() const {
        return tc_ui_rect{content_rect_.x + surface_size_ + gap_, content_rect_.y, bar_width_, surface_size_};
    }
    ...
    tc_ui_rect ColorPicker::preview_rect() const {
        return tc_ui_rect{content_rect_.x, content_rect_.y + surface_size_ + gap_, surface_size_, preview_height_};
    }
    ...
    void ColorPicker::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        tc_ui_painter_stroke_rect(
            context, tc_ui_rect{hue.x - 1.0f, hue_y - 2.0f, hue.width + 2.0f, 4.0f}, style.foreground, 2.0f);
        if (model_->show_alpha()) {
            const float alpha_y = alpha.y + (1.0f - model_->alpha()) * alpha.height;
            tc_ui_painter_stroke_rect(
                context, tc_ui_rect{alpha.x - 1.0f, alpha_y - 2.0f, alpha.width + 2.0f, 4.0f}, style.foreground, 2.0f);
        }
        ...
        if (model_->show_alpha()) {
            ...
            std::snprintf(
                text.data(), text.size(), "#%02X%02X%02X", channel(current.r), channel(current.g), channel(current.b));
        }
    ...
    void ColorPicker::apply_pointer(float x, float y) {
        if (dragging_ == DragTarget::SaturationValue) {
            ...
            model_->set_hsv(model_->hue(),
                            clamp_float((x - rect.x) / rect.width, 0.0f, 1.0f),
    ...
    tc_ui_event_result ColorPicker::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
    ...
    tc_ui_event_result ColorPicker::key_event(tc_ui_document_handle, const tc_ui_key_event* event) {
        ...
        case TC_UI_KEY_PAGE_DOWN:
            model_->set_saturation(clamp_float(model_->saturation() - 0.1f, 0.0f, 1.0f));
            return TC_UI_EVENT_HANDLED;
