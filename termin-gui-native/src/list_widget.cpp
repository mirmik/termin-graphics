...
namespace termin::gui_native {
    ...
    float ListWidget::content_height() const {
        if (!model_ || model_->empty())
            return 0.0f;
        return static_cast<float>(model_->size()) * row_height_ + static_cast<float>(model_->size() - 1) * row_spacing_;
    }
    ...
    std::pair<size_t, size_t> ListWidget::visible_range() const {
        if (!model_ || model_->empty() || bounds().height <= 0.0f)
            return {0, 0};
        const float stride = row_height_ + row_spacing_;
        const size_t first =
            std::min(model_->size(), static_cast<size_t>(std::max(0.0f, std::floor(scroll_y_ / stride))));
        const size_t last =
            std::min(model_->size(), static_cast<size_t>(std::ceil((scroll_y_ + bounds().height) / stride)) + 1);
        return {first, last};
    }

    void ListWidget::ensure_visible(size_t index) {
        if (!model_ || index >= model_->size())
            return;
        const float stride = row_height_ + row_spacing_;
    ...
    bool ListWidget::select_index(size_t index, bool toggle, bool extend, bool additive) {
        sync_model();
        if (!model_ || index >= model_->size() || !model_->item(index).enabled)
            return false;
        bool changed = false;
        if (extend)
            ...
            for (size_t index = first; index < last; ++index) {
                ...
                if (item.texture_id != 0 || !item.icon.empty()) {
                    ...
                    const tc_ui_rect icon_rect{
                        text_x, row.y + (row.height - icon_extent) * 0.5f, icon_extent, icon_extent};
                ...
                if (item.subtitle.empty()) {
                    ...
                    tc_ui_painter_draw_text(context,
                                            ...
                                            std::max(9.0f, style.font_size - 2.0f),
                                            subtitle);
                }
            }
        }
    ...
    size_t ListWidget::index_at(float x, float y) const {
        if (!model_ || !rect_contains(bounds(), x, y))
    ...
    tc_ui_event_result ListWidget::key_event(tc_ui_document_handle, const tc_ui_key_event* event) {
        ...
        if (event->key == TC_UI_KEY_A && (event->modifiers & TC_UI_MOD_CTRL) != 0) {
            if (selection_.mode() != SelectionMode::Multiple)
                return TC_UI_EVENT_IGNORED;
            const std::vector<size_t> before = selection_.selected_indices();
            selection_.clear();
            for (size_t index = 0; index < model_->size(); ++index) {
                if (model_->item(index).enabled)
