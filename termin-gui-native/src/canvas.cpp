...
namespace termin::gui_native {
    ...
    void Canvas::set_texture_sampling(tc_ui_texture_sampling sampling) {
        if (sampling != TC_UI_TEXTURE_SAMPLING_LINEAR && sampling != TC_UI_TEXTURE_SAMPLING_NEAREST) {
            tc_log_error("[termin-gui-native] Canvas rejected invalid texture sampling mode");
            return;
        }
        if (texture_sampling_ == sampling)
            return;
    ...
    void Canvas::fit_in_view() {
        fit_mode_ = true;
        if (image_size_.width <= 0.0f || image_size_.height <= 0.0f || bounds().width <= 0.0f ||
            bounds().height <= 0.0f)
            return;
    ...
    tc_ui_point Canvas::widget_to_image(tc_ui_point point) const {
        return tc_ui_point{(point.x - bounds().x - offset_.x) / zoom_, (point.y - bounds().y - offset_.y) / zoom_};
    }

    tc_ui_point Canvas::image_to_widget(tc_ui_point point) const {
        return tc_ui_point{bounds().x + offset_.x + point.x * zoom_, bounds().y + offset_.y + point.y * zoom_};
    }
