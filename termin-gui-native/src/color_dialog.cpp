...
namespace termin::gui_native {
    ...
    bool ColorDialog::ensure_content(tc_ui_document_handle document) {
        if (!tc_widget_handle_is_invalid(picker_handle_) && tc_ui_document_is_alive(document, picker_handle_))
            return true;
        auto picker = std::make_unique<ColorPicker>(model_);
        picker_handle_ = tc_ui_document_adopt_widget(document, picker->c_widget(), &Widget::delete_owned_widget);
