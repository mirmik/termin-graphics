...
namespace termin::gui_native {
    ...
    void Dialog::set_content(NativeWidget& content) {
        ...
        if (tc_widget_handle_eq(content_handle_, content.handle()))
            return;
        if (!tc_widget_handle_is_invalid(content_handle_) && tc_ui_document_is_alive(document(), content_handle_)) {
    ...
    bool Dialog::ensure_buttons(tc_ui_document_handle document) {
        ...
        for (size_t index = 0; index < actions_.size(); ++index) {
            ...
            button_connections_.push_back(
                body->clicked().connect([this, stable_id](Button&) { activate(stable_id, this->document()); }));
        }
        return true;
    }
    ...
    tc_ui_size Dialog::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        const tc_ui_size wanted{std::max(min_width_, std::max(content_width, button_width) + padding_ * 2),
    ...
    void Dialog::layout(tc_ui_document_handle document, tc_ui_rect rect) {
        ...
        if (tc_widget* content = tc_ui_document_resolve_widget(document, content_handle_)) {
            layout_widget(content,
                          document,
                          tc_ui_rect{rect.x + padding_,
                                     content_y,
                                     rect.width - padding_ * 2,
        ...
        for (size_t index = 0; index < button_handles_.size(); ++index) {
            ...
            if (!button)
                continue;
            const float y = rect.y + rect.height - action_bar_height + (action_bar_height - sizes[index].height) * 0.5f;
    ...
    tc_ui_event_result Dialog::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
        ...
        if (event->type == TC_UI_POINTER_DOWN && draggable_ &&
            ...
            rect_contains(tc_ui_rect{bounds().x, bounds().y, bounds().width, title_height_}, event->x, event->y)) {
        ...
        if (event->type == TC_UI_POINTER_MOVE && dragging_ && captured) {
            ...
            overlay_layout.offset.x = wanted_x - (viewport.x + (viewport.width - bounds().width) * 0.5f);
            overlay_layout.offset.y = wanted_y - (viewport.y + (viewport.height - bounds().height) * 0.5f);
            if (!tc_ui_document_update_overlay_layout(document, handle(), &overlay_layout, viewport)) {
    ...
    bool Dialog::show(tc_ui_document_handle document, tc_ui_rect viewport) {
        ...
        dragging_ = false;
        open_ = tc_ui_document_show_overlay_with_layout(document, handle(), overlay_flags, &overlay_layout, viewport);
        if (!open_)
            return false;
    ...
    bool Dialog::before_action(const DialogAction&) {
        return true;
    }
    ...
    tc_ui_event_result Dialog::key_event(tc_ui_document_handle document, const tc_ui_key_event* event) {
        ...
        return action && activate(action->stable_id, document) ? TC_UI_EVENT_HANDLED : TC_UI_EVENT_IGNORED;
    }

    void Dialog::descendant_focused(tc_ui_document_handle, tc_widget_handle descendant) {
    ...
    void Dialog::overlay_dismissed(tc_ui_document_handle document, tc_ui_overlay_dismiss_reason reason) {
        ...
        if (has_pending_result_) {
            ...
            result = DialogResult{cancel ? cancel->stable_id : std::string{}, DialogDismissReason::Escape};
        } else {
