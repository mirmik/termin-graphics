...
namespace termin::gui_native {
    namespace {
        ...
        template <typename T> bool create_text_widget(tc_ui_document_handle, void*, tc_widget_factory_result* result) {
            if (!result) {
                return false;
            }
            auto* widget = new T();
            *result = tc_widget_factory_result{widget->c_widget(), &delete_native_widget, TC_WIDGET_OWNED};
            return true;
        }
        ...
        bool color_property(const tc_value* properties, const char* name, SrgbColor& out) {
            ...
            if (value->type != TC_VALUE_LIST || (value->data.list.count != 3 && value->data.list.count != 4)) {
                return false;
            }
        ...
        float number_property(const tc_value* properties, const char* name, float fallback = 0.0f) {
            ...
            return value->type == TC_VALUE_DOUBLE ? static_cast<float>(value->data.d) : fallback;
        }

        CrossAxisAlignment
        alignment_property(const tc_value* properties, const char* name, CrossAxisAlignment fallback) {
            const tc_value* value = property(properties, name);
            const std::string text = value && value->type == TC_VALUE_STRING && value->data.s ? value->data.s : "";
            if (text == "auto")
        ...
        EdgeInsets padding_property(const tc_value* properties) {
            ...
            if (value->type == TC_VALUE_INT || value->type == TC_VALUE_FLOAT || value->type == TC_VALUE_DOUBLE) {
        ...
        bool apply_box_properties(tc_widget* widget, const tc_value* properties) {
            ...
            if (const tc_value* orientation = property(properties, "orientation")) {
                ...
                box->set_orientation(text == "vertical" ? Orientation::Vertical : Orientation::Horizontal);
            }
            ...
            if (property(properties, "align_items")) {
                box->set_cross_axis_alignment(
                    alignment_property(properties, "align_items", CrossAxisAlignment::Stretch));
            }
            return true;
        }

        GridTrack grid_track_property(const tc_value* value) {
            const tc_value* policy_value = tc_value_dict_get(const_cast<tc_value*>(value), "policy");
            const std::string policy_text = policy_value && policy_value->data.s ? policy_value->data.s : "";
            GridTrack track;
        ...
        bool apply_wrap_layout_properties(tc_widget* widget, const tc_value* properties) {
            auto* layout = static_cast<WrapLayout*>(widget->body);
            if (const tc_value* orientation = property(properties, "orientation")) {
                const std::string text = orientation->data.s ? orientation->data.s : "";
                layout->set_orientation(text == "vertical" ? Orientation::Vertical : Orientation::Horizontal);
            }
            if (property(properties, "padding")) {
            ...
            if (property(properties, "line_spacing")) {
                layout->set_line_spacing(number_property(properties, "line_spacing"));
            }
            if (property(properties, "line_alignment")) {
                layout->set_line_alignment(alignment_property(properties, "line_alignment", CrossAxisAlignment::Start));
            }
            return true;
        }

        ScrollBarPolicy scrollbar_policy_property(const tc_value* properties, const char* name) {
            const tc_value* value = property(properties, name);
            const std::string text = value && value->data.s ? value->data.s : "auto";
            if (text == "always") {
        ...
        bool apply_scroll_area_properties(tc_widget* widget, const tc_value* properties) {
            ...
            scroll->set_scroll_axes(horizontal ? horizontal->data.b : true, vertical ? vertical->data.b : true);
            scroll->set_scrollbar_policy(scrollbar_policy_property(properties, "horizontal_scrollbar"),
        ...
        bool create_label(tc_ui_document_handle, void*, tc_widget_factory_result* result) {
            if (!result) {
                return false;
            }
        ...
        bool create_box_layout(tc_ui_document_handle, void*, tc_widget_factory_result* result) {
            if (!result) {
                return false;
            }
        ...
        bool apply_label_properties(tc_widget* widget, const tc_value* properties) {
            ...
            if (const tc_value* wrap = property(properties, "wrap")) {
                ...
                label->set_wrap_mode(text == "word"        ? TextWrapMode::Word
                                     : text == "character" ? TextWrapMode::Character
                                                           : TextWrapMode::None);
            }
            if (const tc_value* overflow = property(properties, "overflow")) {
                const std::string text = overflow->data.s ? overflow->data.s : "clip";
                label->set_overflow(text == "ellipsis" ? TextOverflow::Ellipsis : TextOverflow::Clip);
            }
        ...
        bool apply_checkbox_properties(tc_widget* widget, const tc_value* properties) {
        ...
        bool append_box_child(tc_widget* parent, tc_widget* child, const tc_value* properties) {
            ...
            if (!basis && (property(properties, "grow") || property(properties, "shrink"))) {
            ...
            return box->set_child_placement(child->handle,
                                            policy,
                                            basis_value,
                                            grow,
                                            shrink,
                                            ...
                                            alignment_property(properties, "align_self", CrossAxisAlignment::Auto));
        }

        bool append_overlay_child(tc_widget* parent, tc_widget* child, const tc_value* properties) {
            ...
            if (const tc_value* value = property(properties, "offset")) {
                ...
                auto number = [](const tc_value* item) {
                    ...
                    if (item->type == TC_VALUE_FLOAT)
                        return item->data.f;
                    return item->type == TC_VALUE_DOUBLE ? static_cast<float>(item->data.d) : 0.0f;
                };
                offset = {number(x), number(y)};
            }
            return static_cast<OverlayLayout*>(parent->body)->add_child(child->handle, anchor, offset);
        }

        bool append_grid_child(tc_widget* parent, tc_widget* child, const tc_value* properties) {
        ...
        bool append_scroll_content(tc_widget* parent, tc_widget* child, const tc_value*) {
            static_cast<ScrollArea*>(parent->body)->set_content(child->handle);
            return child->parent == parent;
        }

        bool append_wrap_child(tc_widget* parent, tc_widget* child, const tc_value*) {
        ...
        bool read_text_state(const tc_value* state, std::string& text, std::string& placeholder) {
            ...
            if (!text_value || !placeholder_value || text_value->type != TC_VALUE_STRING ||
                placeholder_value->type != TC_VALUE_STRING || !text_value->data.s || !placeholder_value->data.s ||
                !detail::valid_utf8(text_value->data.s) || !detail::valid_utf8(placeholder_value->data.s)) {
        ...
        template <typename T> bool serialize_text_widget(const tc_widget* widget, void*, tc_value* state) {
        ...
        template <typename T> bool deserialize_text_widget(tc_widget* widget, const tc_value* state, void*) {
            std::string text;
        ...
        template <typename T>
        bool register_declarative_widget(const char* type_name, const tc_uiscript_type_descriptor* uiscript) {
            if (tc_widget_registry_has(type_name)) {
                return true;
            }
            const tc_widget_factory_descriptor descriptor{
        ...
        constexpr const char* kBoxProperties[] = {"orientation", "padding", "spacing", "align_items"};
        constexpr const char* kGridProperties[] = {"padding", "column_spacing", "row_spacing", "columns", "rows"};
        ...
        constexpr const char* kScrollAreaProperties[] = {
            "horizontal_scroll", "vertical_scroll", "horizontal_scrollbar", "vertical_scrollbar"};
        ...
        constexpr const char* kGridChildProperties[] = {"row", "column", "row_span", "column_span"};
    ...
    bool register_builtin_widget_types() {
        return tc_widget_registry_initialize() &&
               ...
               register_declarative_widget<GridLayout>(NativeWidgetRuntimeType<GridLayout>::name, &kGridUiScript) &&
               register_declarative_widget<WrapLayout>(NativeWidgetRuntimeType<WrapLayout>::name, &kWrapUiScript) &&
               register_declarative_widget<ScrollArea>(NativeWidgetRuntimeType<ScrollArea>::name,
                                                       &kScrollAreaUiScript) &&
               register_declarative_widget<HStack>(NativeWidgetRuntimeType<HStack>::name, &kBoxUiScript) &&
               register_declarative_widget<VStack>(NativeWidgetRuntimeType<VStack>::name, &kBoxUiScript) &&
               register_label_widget() &&
               register_declarative_widget<Checkbox>(NativeWidgetRuntimeType<Checkbox>::name, &kCheckboxUiScript) &&
               register_declarative_widget<IconButton>(NativeWidgetRuntimeType<IconButton>::name, &kIconButtonUiScript);
    }
