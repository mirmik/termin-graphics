...
void UiDrawListRenderer::render(
    ...
    for (const UiDrawListBatch& batch : batches) {
        if (!tc_ui_presentation_metrics_is_valid(
                &batch.presentation_metrics) ||
            batch.first_command > count ||
            batch.command_count > count - batch.first_command) {
            tc_log_error(
                "[termin-gui-native] skipping invalid UI draw-list batch");
            continue;
        }
        ...
        for (size_t index = batch.first_command; index < batch_end; ++index) {
          ...
          case TC_UI_DRAW_FILL_RECT: {
            ...
            canvas_.draw_rect(rect.x, rect.y, rect.width, rect.height,
          ...
          case TC_UI_DRAW_FILL_CIRCLE: {
            ...
            canvas_.draw_circle(center.x, center.y,
                                physical_length(command->radius, scale),
                                canvas_color(command->color),
          ...
          case TC_UI_DRAW_STROKE_CIRCLE: {
            ...
            canvas_.draw_circle_outline(
                center.x, center.y, physical_length(command->radius, scale),
                canvas_color(command->color),
                physical_length(command->thickness, scale),
                command->segments > 0 ? command->segments : 24);
            break;
          ...
          case TC_UI_DRAW_LINE: {
            ...
            canvas_.draw_line(p0.x, p0.y, p1.x, p1.y,
                              canvas_color(command->color),
                              physical_length(command->thickness, scale));
            break;
          }
          case TC_UI_DRAW_POLYLINE:
            for (size_t point_index = 1; point_index < command->point_count;
                 ++point_index) {
              const tc_ui_point p0 =
                  physical_point(command->points[point_index - 1], scale);
              const tc_ui_point p1 =
                  physical_point(command->points[point_index], scale);
              canvas_.draw_line(p0.x, p0.y, p1.x, p1.y,
                                canvas_color(command->color),
                                physical_length(command->thickness, scale));
            }
          ...
          case TC_UI_DRAW_TEXTURE: {
            ...
            canvas_.draw_texture(tgfx::TextureHandle{command->texture_id},
                                 rect.x, rect.y, rect.width, rect.height,
                                 canvas_color(command->color), command->flip_v,
          ...
          case TC_UI_DRAW_POP_CLIP:
            if (clip_depth == 0) {
              tc_log_error(
                  "[termin-gui-native] ignoring unmatched UI pop-clip command");
            } else {
              canvas_.end_clip();
              clip_depth -= 1;
            }
          ...
          case TC_UI_DRAW_TEXT:
            if (canvas_.default_font()) {
              ...
              const float physical_font_size =
                  command->font_size * effective_font_scale;
              const float line_top =
                  baseline.y - static_cast<float>(
                                   owned_font_->ascent_px(physical_font_size));
              canvas_.draw_text(command->text ? command->text : "", baseline.x,
                                line_top, physical_font_size,
                                canvas_color(command->color));
          ...
          case TC_UI_DRAW_CANVAS2D_LIST: {
            const auto *nested =
                static_cast<const tgfx::DrawList2D *>(command->canvas2d_list);
            if (!nested) {
              tc_log_error(
            ...
            const bool transformed_ok = transformed_builder.push_transform(
                                            termin::Affine2f::scaling(scale)) &&
                                        transformed_builder.append(*nested) &&
                                        transformed_builder.pop_transform();
            auto transformed =
                transformed_ok ? transformed_builder.freeze() : std::nullopt;
            if (!transformed || !canvas_.execute(*transformed, resources)) {
              tc_log_error(
                  "[termin-gui-native] failed to transform or execute nested "
                  "DrawList2D");
            }
            break;
          }
