...
namespace termin::gui_native::detail {
    ...
    void set_style_color(Widget& widget, tc_ui_style_field_mask field, tc_ui_srgb_color color) {
        ...
        if (field == TC_UI_STYLE_BACKGROUND)
            style_override.value.background = color;
        else if (field == TC_UI_STYLE_FOREGROUND)
            style_override.value.foreground = color;
        else if (field == TC_UI_STYLE_BORDER)
            style_override.value.border = color;
        else if (field == TC_UI_STYLE_ACCENT)
            style_override.value.accent = color;
        else
            throw std::invalid_argument("unsupported native UI color style field");
    ...
    void set_style_metric(Widget& widget, tc_ui_style_field_mask field, float value) {
        tc_ui_style_override style_override = widget.style_override();
        style_override.fields |= field;
        if (field == TC_UI_STYLE_BORDER_WIDTH)
            style_override.value.border_width = value;
        else if (field == TC_UI_STYLE_CORNER_RADIUS)
            style_override.value.corner_radius = value;
        else if (field == TC_UI_STYLE_FONT_SIZE)
            style_override.value.font_size = value;
        else
            throw std::invalid_argument("unsupported native UI metric style field");
        if (!widget.set_style_override(style_override)) {
    ...
    bool key_matches_ascii(int32_t key, char lower) {
        return key == static_cast<int32_t>(lower) || key == static_cast<int32_t>(lower - ('a' - 'A'));
    }
    ...
    size_t utf8_floor_boundary(std::string_view text, size_t offset) {
        ...
        while (offset > 0 && offset < text.size() && (static_cast<uint8_t>(text[offset]) & 0xc0u) == 0x80u) {
            --offset;
        }
        return offset;
    }
    ...
    bool
    measure_text(tc_ui_document_handle document, std::string_view text, float font_size, tc_ui_text_metrics& metrics) {
        return tc_ui_document_measure_text(document, text.data(), text.size(), font_size, &metrics);
    }

    float
    centered_text_baseline(tc_ui_document_handle document, std::string_view text, float font_size, tc_ui_rect rect) {
    ...
    tc_ui_size clamp_size(tc_ui_size size, tc_ui_constraints constraints) {
        ...
        return tc_ui_size{clamp_float(size.width, constraints.min_size.width, max_width),
    ...
    tc_ui_constraints unconstrained() {
        return tc_ui_constraints{tc_ui_size{0.0f, 0.0f}, tc_ui_size{kHuge, kHuge}};
    }

    tc_widget* resolve_child(tc_ui_document_handle document,
    ...
    void detach_if_child(tc_widget* parent, tc_widget_handle child_handle) {
        if (!parent || tc_ui_document_handle_is_invalid(parent->document) ||
            tc_widget_handle_is_invalid(child_handle)) {
            return;
        }
        tc_widget* child = tc_ui_document_resolve_widget(parent->document, child_handle);
    ...
    tc_ui_size measure_widget(tc_widget* widget, tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        return measure_widget(widget, document, constraints, constraints.max_size, width_definite, height_definite);
    }

    tc_ui_size measure_widget(tc_widget* widget,
        ...
        if (!valid_extent(constraints.min_size.width) || !valid_extent(constraints.min_size.height) ||
            !valid_extent(constraints.max_size.width) || !valid_extent(constraints.max_size.height) ||
            (constraints.max_size.width > 0.0f && constraints.max_size.width < constraints.min_size.width) ||
            (constraints.max_size.height > 0.0f && constraints.max_size.height < constraints.min_size.height) ||
            (width_definite && !valid_extent(parent_extent.width)) ||
            (height_definite && !valid_extent(parent_extent.height))) {
            tc_log_error("[termin-gui-native] rejected invalid child measurement constraints");
        ...
        const bool height_exact =
            constraints.max_size.height > 0.0f && constraints.min_size.height == constraints.max_size.height;
        if (!width_exact) {
            measure_constraints.min_size.width = std::max(measure_constraints.min_size.width, spec.min_width);
        }
        if (!height_exact) {
            measure_constraints.min_size.height = std::max(measure_constraints.min_size.height, spec.min_height);
        }
        if (!width_exact && spec.max_width > 0.0f) {
            measure_constraints.max_size.width =
                std::min(effective_max(measure_constraints.max_size.width), spec.max_width);
        }
        if (!height_exact && spec.max_height > 0.0f) {
            measure_constraints.max_size.height =
                std::min(effective_max(measure_constraints.max_size.height), spec.max_height);
        }

        const auto resolved_axis = [](tc_ui_length length, float parent, bool definite, float& value) {
        ...
        if (!width_exact && resolved_axis(spec.width, parent_extent.width, width_definite, requested_width)) {
            requested_width = clamp_float(requested_width, spec.min_width, effective_max(spec.max_width));
            measure_constraints.min_size.width = requested_width;
            measure_constraints.max_size.width = requested_width;
        }
        if (!height_exact && resolved_axis(spec.height, parent_extent.height, height_definite, requested_height)) {
            requested_height = clamp_float(requested_height, spec.min_height, effective_max(spec.max_height));
            measure_constraints.min_size.height = requested_height;
            measure_constraints.max_size.height = requested_height;
        }
        measure_constraints.max_size.width =
            std::max(measure_constraints.min_size.width, measure_constraints.max_size.width);
        measure_constraints.max_size.height =
            std::max(measure_constraints.min_size.height, measure_constraints.max_size.height);

        const tc_ui_size intrinsic = widget->vtable->measure(widget, document, measure_constraints);
        if (!valid_extent(intrinsic.width) || !valid_extent(intrinsic.height)) {
            tc_log_error("[termin-gui-native] widget measurement returned a non-finite or "
                         "negative size");
            return measure_constraints.min_size;
        }
        tc_ui_size resolved{};
        if (!tc_ui_widget_layout_spec_resolve_size(
                &spec, intrinsic, parent_extent, width_definite, height_definite, &resolved)) {
            return measure_constraints.min_size;
        }
        resolved = clamp_size(resolved, constraints);
        if (!valid_extent(resolved.width) || !valid_extent(resolved.height)) {
            tc_log_error("[termin-gui-native] resolved widget measurement is non-finite or "
                         "negative");
            return measure_constraints.min_size;
        }
        return resolved;
    ...
    void distribute_grow(std::vector<float>& extents,
    ...
    void distribute_shrink(std::vector<float>& extents,
                           const std::vector<float>& min_extents,
                           const std::vector<float>& weights,
                           float deficit) {
    ...
    const LayoutItem* find_layout_item(const std::vector<LayoutItem>& items, tc_widget_handle handle) {
        const auto found = std::find_if(items.begin(), items.end(), [handle](const LayoutItem& item) {
            return tc_widget_handle_eq(item.handle, handle);
        });
        return found != items.end() ? &*found : nullptr;
    }

    const TabPage* find_tab_page(const std::vector<TabPage>& pages, tc_widget_handle handle) {
        const auto found = std::find_if(pages.begin(), pages.end(), [handle](const TabPage& page) {
            return tc_widget_handle_eq(page.handle, handle);
        });
    ...
    void apply_span_requirement(std::vector<float>& extents,
                                const std::vector<float>& max_extents,
                                size_t start,
                                size_t span,
                                float spacing,
    ...
    GridAxisLayout build_grid_axis(tc_ui_document_handle document,
                                   const tc_widget* expected_parent,
        ...
        for (const GridItem& item : items) {
            ...
            tc_widget* child = resolve_child(document,
                                             expected_parent,
                                             item.handle,
            ...
            if (!columns && column_extents && item.column < column_extents->size()) {
                const size_t column_end = std::min(column_extents->size(), item.column + item.column_span);
                float allocated_width = column_spacing * static_cast<float>(column_end - item.column - 1);
                for (size_t column = item.column; column < column_end; ++column) {
                    allocated_width += (*column_extents)[column];
                }
                allocated_width = std::max(0.0f, allocated_width - margin.left - margin.right);
                child_constraints.max_size.width = allocated_width;
                const tc_ui_length_mode width_mode = tc_widget_layout_spec(child).width.mode;
                if (width_mode == TC_UI_LENGTH_AUTO || width_mode == TC_UI_LENGTH_FILL) {
                    child_constraints.min_size.width = allocated_width;
                }
            }
            const tc_ui_size measured =
                measure_widget(child, document, child_constraints, parent_extent, width_definite, height_definite);
            const tc_ui_widget_layout_spec spec = tc_widget_layout_spec(child);
            const float explicit_min = columns ? std::max(spec.min_width,
                                                          spec.touch_target_policy == TC_UI_TOUCH_TARGET_LAYOUT_MINIMUM
                                                              ? spec.minimum_touch_target.width
                                                              : 0.0f) +
                                                     margin.left + margin.right
                                               : std::max(spec.min_height,
                                                          spec.touch_target_policy == TC_UI_TOUCH_TARGET_LAYOUT_MINIMUM
                                                              ? spec.minimum_touch_target.height
                                                              : 0.0f) +
                                                     margin.top + margin.bottom;
            apply_span_requirement(axis.min_extents, axis.max_extents, start, span, spacing, explicit_min);
            for (size_t index = start; index < std::min(axis.extents.size(), start + span); ++index) {
                axis.extents[index] = std::max(axis.extents[index], axis.min_extents[index]);
            }
            apply_span_requirement(axis.extents,
                                   axis.max_extents,
                                   start,
                                   span,
                                   spacing,
