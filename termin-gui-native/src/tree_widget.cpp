...
TreeWidget::TreeWidget(std::shared_ptr<TreeModel> model,
    ...
    : NativeWidget("TreeWidget"), model_(model ? std::move(model) : std::make_shared<TreeModel>()),
...
void TreeWidget::connect_models() {
    if (model_ && model_connection_ == 0) {
        model_connection_ = model_->changed().connect(
            [this](TreeModel&, const TreeChange& change) { on_model_changed(change); });
    }
    if (expansion_ && expansion_connection_ == 0) {
        expansion_connection_ =
            expansion_->changed().connect([this](TreeExpansionModel&, TreeNodeId node, bool value) {
...
void TreeWidget::append_visible(TreeNodeId node_id, size_t depth) {
    const TreeNode& node = model_->node(node_id);
    visible_.push_back(TreeVisibleRow{node_id, depth});
    if (!expansion_->expanded(node_id))
        return;
...
void TreeWidget::rebuild_visible() {
    visible_.clear();
    visible_.reserve(model_->size());
    for (TreeNodeId root : model_->roots())
        append_visible(root, 0);
...
bool TreeWidget::toggle(TreeNodeId node) { return set_expanded(node, !expansion_->expanded(node)); }
...
std::pair<size_t, size_t> TreeWidget::visible_range() {
    ...
    const size_t first = std::min(
        visible_.size(), static_cast<size_t>(std::max(0.0f, std::floor(scroll_y_ / stride))));
    const size_t last =
        std::min(visible_.size(),
...
tc_ui_size TreeWidget::measure(tc_ui_document* document, tc_ui_constraints constraints) {
    sync_models();
...
void TreeWidget::paint(tc_ui_document* document, tc_ui_paint_context* context) {
    ...
    if (visible_.empty()) {
        ...
        for (size_t index = first; index < last; ++index) {
            ...
            if (selected_ == node.id || hovered_ == node.id) {
                ...
                tc_ui_painter_fill_rounded_rect(context, row, std::min(4.0f, row_height_ * 0.2f),
                                                highlight);
            }
            if (dragging_ && drag_target_ == node.id) {
                ...
                if (drag_position_ == TreeDropPosition::Inside) {
                    ...
                    fill.a = 0.28f;
                    tc_ui_painter_fill_rounded_rect(context, row,
                                                    std::min(4.0f, row_height_ * 0.2f), fill);
                } else {
                    ...
                    tc_ui_painter_draw_line(context, tc_ui_point{row.x, line_y},
                                            tc_ui_point{row.x + row.width, line_y}, drop_color,
                                            2.0f);
                }
            }
            ...
            if (!node.children.empty()) {
                ...
                tc_ui_painter_draw_text(context, symbol,
    ...
    if (dragging_ && drag_position_ == TreeDropPosition::Root) {
        ...
        tc_ui_painter_draw_line(context, tc_ui_point{bounds().x, line_y},
                                tc_ui_point{bounds().x + bounds().width, line_y}, drop_color, 2.0f);
    }
...
TreeNodeId TreeWidget::next_enabled(size_t from, int direction) const {
    ...
    std::ptrdiff_t index =
        from == SIZE_MAX ? (direction > 0 ? 0 : static_cast<std::ptrdiff_t>(visible_.size()) - 1)
...
tc_ui_event_result TreeWidget::key_event(tc_ui_document*, const tc_ui_key_event* event) {
