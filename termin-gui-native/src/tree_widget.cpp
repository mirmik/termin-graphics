...
namespace termin::gui_native {
    ...
    TreeWidget::TreeWidget(std::shared_ptr<TreeModel> model, std::shared_ptr<TreeExpansionModel> expansion)
    ...
    void TreeWidget::connect_models() {
        if (model_ && model_connection_ == 0) {
            model_connection_ =
                model_->changed().connect([this](TreeModel&, const TreeChange& change) { on_model_changed(change); });
        }
    ...
    void TreeWidget::set_expansion_model(std::shared_ptr<TreeExpansionModel> expansion) {
        disconnect_models();
        clear_drag_state();
        clear_toggle_press();
        expansion_ = expansion ? std::move(expansion) : std::make_shared<TreeExpansionModel>();
    ...
    void TreeWidget::on_model_changed(const TreeChange&) {
        ...
        if (pressed_ != kInvalidTreeNodeId && !model_->contains(pressed_))
            clear_drag_state();
        if (pressed_toggle_node_ != kInvalidTreeNodeId && !model_->contains(pressed_toggle_node_))
            clear_toggle_press();
        rebuild_visible();
        clamp_scroll();
        mark_dirty(TC_WIDGET_DIRTY_LAYOUT | TC_WIDGET_DIRTY_STATE | TC_WIDGET_DIRTY_PAINT);
    ...
    void TreeWidget::set_scroll_y(float offset) {
        if (!std::isfinite(offset)) {
            tc_log_error("[termin-gui-native] TreeWidget rejected non-finite scroll offset");
    ...
    std::pair<size_t, size_t> TreeWidget::visible_range() {
        ...
        const size_t first =
            std::min(visible_.size(), static_cast<size_t>(std::max(0.0f, std::floor(scroll_y_ / stride))));
        const size_t last =
            std::min(visible_.size(), static_cast<size_t>(std::ceil((scroll_y_ + bounds().height) / stride)) + 1);
        return {first, last};
    }
    ...
    tc_ui_size TreeWidget::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        sync_models();
            ...
            for (size_t index = first; index < last; ++index) {
                ...
                if (dragging_ && drag_target_ == node.id) {
                    ...
                    if (drag_position_ == TreeDropPosition::Inside) {
                        ...
                        const float line_y =
                            drag_position_ == TreeDropPosition::Before ? row.y + 1.0f : row.y + row.height - 1.0f;
                        tc_ui_painter_draw_line(context,
                                                tc_ui_point{row.x, line_y},
                ...
                const auto draw_toggle = [&](bool checked, bool secondary) {
                    ...
                    if (checked) {
                        ...
                        tc_ui_painter_draw_line(context,
                                                tc_ui_point{box.x + box.width * 0.43f, box.y + box.height * 0.74f},
                                                tc_ui_point{box.x + box.width * 0.80f, box.y + box.height * 0.27f},
                                                mark,
                                                1.8f);
                    }
    ...
    size_t TreeWidget::row_index_at(float x, float y) const {
        ...
        if (index >= visible_.size() || local - static_cast<float>(index) * stride > row_height_) {
            return SIZE_MAX;
        }
        return index;
    }
    ...
    float TreeWidget::row_toggle_left(const TreeVisibleRow& row) const {
        return bounds().x + static_cast<float>(row.depth) * indent_size_ + toggle_size_ + item_padding_;
    }

    int TreeWidget::row_toggle_at(const TreeVisibleRow& row, const CollectionItem& item, float x) const {
    ...
    TreeDropPosition TreeWidget::drop_position_at(size_t index, float y) const {
        ...
        const float stride = row_height_ + row_spacing_;
        const float row_y = bounds().y + static_cast<float>(index) * stride - scroll_y_;
    ...
    tc_ui_event_result TreeWidget::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
            return TC_UI_EVENT_IGNORED;
        sync_models();
        if (event->type == TC_UI_POINTER_CANCEL) {
            const bool active =
                pressed_ != kInvalidTreeNodeId || pressed_toggle_node_ != kInvalidTreeNodeId || dragging_;
            clear_drag_state();
            clear_toggle_press();
        ...
        if (event->type == TC_UI_POINTER_MOVE && pressed_ != kInvalidTreeNodeId && captured) {
            ...
            if (dragging_) {
                ...
                drag_target_ = index == SIZE_MAX ? kInvalidTreeNodeId : visible_[index].node;
                drag_position_ = drop_position_at(index, event->y);
                mark_dirty(TC_WIDGET_DIRTY_STATE | TC_WIDGET_DIRTY_PAINT);
                return TC_UI_EVENT_HANDLED;
            }
        }
        if (event->type == TC_UI_POINTER_UP && pressed_toggle_node_ != kInvalidTreeNodeId && captured) {
            ...
            clear_toggle_press();
            const size_t index = row_index_at(event->x, event->y);
            if (index != SIZE_MAX && visible_[index].node == node_id && model_->contains(node_id)) {
                const TreeVisibleRow row = visible_[index];
                const TreeNode& node = model_->node(node_id);
                if (node.item.enabled && row_toggle_at(row, node.item, event->x) == toggle_index) {
                    toggle_activated_.emit(*this, node_id, toggle_index, node.item);
                }
            }
            return TC_UI_EVENT_HANDLED;
        }
        if (event->type == TC_UI_POINTER_UP && pressed_ != kInvalidTreeNodeId && captured) {
        ...
        if (event->type == TC_UI_POINTER_MOVE) {
            const TreeNodeId next = index == SIZE_MAX ? kInvalidTreeNodeId : visible_[index].node;
            if (next != hovered_) {
                hovered_ = next;
                mark_dirty(TC_WIDGET_DIRTY_STATE | TC_WIDGET_DIRTY_PAINT);
            }
        ...
        if (event->button == tcbase::mouse_button_value(tcbase::MouseButton::LEFT) && toggle_index >= 0) {
            activation_clicks_.clear();
    ...
    tc_ui_event_result TreeWidget::key_event(tc_ui_document_handle, const tc_ui_key_event* event) {
        ...
        if (event->key == TC_UI_KEY_SPACE && selected.item.enabled) {
            ...
            const bool present = secondary ? selected.item.secondary_toggle : selected.item.primary_toggle;
            if (present) {
