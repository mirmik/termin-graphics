...
namespace termin::gui_native {
    ...
    namespace {
        ...
        void allocate_grid_axis(GridAxisLayout& axis, float available_extent) {
            const float base_extent = axis_total_extent(axis.extents, 0.0f);
            if (available_extent > base_extent) {
                distribute_grow(axis.extents, axis.max_extents, axis.grow_weights, available_extent - base_extent);
            } else if (available_extent < base_extent) {
                distribute_shrink(axis.extents, axis.min_extents, axis.shrink_weights, base_extent - available_extent);
            }
        }
    ...
    void
    GridLayout::add_child(tc_widget_handle handle, size_t row, size_t column, size_t row_span, size_t column_span) {
    ...
    bool GridLayout::set_child_placement(
        tc_widget_handle handle, size_t row, size_t column, size_t row_span, size_t column_span) {
        if (row_span == 0 || column_span == 0 || row + row_span > rows_.size() ||
            column + column_span > columns_.size()) {
            tc_log_error("[termin-gui-native] GridLayout child placement exceeds declared tracks");
            return false;
        }
        ...
        tc_log_error("[termin-gui-native] cannot update placement for a non-child GridLayout widget");
        return false;
    }
    ...
    tc_ui_size GridLayout::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        GridAxisLayout columns = build_grid_axis(document,
                                                 c_widget(),
                                                 columns_,
                                                 items_,
                                                 true,
        ...
        const float column_spacing_total =
            column_spacing_ * static_cast<float>(columns.extents.empty() ? 0 : columns.extents.size() - 1);
        if (width_definite) {
            allocate_grid_axis(columns, std::max(0.0f, parent_extent.width - column_spacing_total));
        }
        GridAxisLayout rows = build_grid_axis(document,
                                              c_widget(),
                                              rows_,
                                              items_,
                                              false,
        ...
        const float row_spacing_total =
            row_spacing_ * static_cast<float>(rows.extents.empty() ? 0 : rows.extents.size() - 1);
        if (height_definite) {
            allocate_grid_axis(rows, std::max(0.0f, parent_extent.height - row_spacing_total));
        }
        tc_ui_size measured{axis_total_extent(columns.extents, column_spacing_) + padding_extent.width,
    ...
    void GridLayout::layout(tc_ui_document_handle document, tc_ui_rect rect) {
        ...
        GridAxisLayout columns =
            build_grid_axis(document, c_widget(), columns_, items_, true, column_spacing_, parent_extent, true, true);
        ...
        for (const GridItem& item : items_) {
            ...
            tc_ui_rect child_rect{column_offsets[item.column], row_offsets[item.row], 0.0f, 0.0f};
            ...
            child_rect.width = std::max(0.0f, child_rect.width - margin.left - margin.right);
            child_rect.height = std::max(0.0f, child_rect.height - margin.top - margin.bottom);
            tc_ui_constraints child_constraints{tc_ui_size{0.0f, 0.0f},
                                                tc_ui_size{child_rect.width, child_rect.height}};
            const tc_ui_widget_layout_spec spec = tc_widget_layout_spec(child);
            if (spec.width.mode == TC_UI_LENGTH_AUTO || spec.width.mode == TC_UI_LENGTH_FILL) {
                child_constraints.min_size.width = child_rect.width;
            }
            if (spec.height.mode == TC_UI_LENGTH_AUTO || spec.height.mode == TC_UI_LENGTH_FILL) {
                child_constraints.min_size.height = child_rect.height;
            }
            const tc_ui_size child_size = measure_widget(child, document, child_constraints, parent_extent, true, true);
