...
namespace termin::gui_native {
    namespace {
        ...
        bool style_equal(const RichTextStyle& lhs, const RichTextStyle& rhs) {
            if (lhs.bold != rhs.bold || lhs.italic != rhs.italic || lhs.color.has_value() != rhs.color.has_value()) {
                return false;
            }
            return !lhs.color || color_equal(*lhs.color, *rhs.color);
        }

        bool is_wrap_space(std::string_view codepoint) {
    ...
    tc_ui_rect RichTextView::content_rect(tc_ui_document_handle document) const {
        ...
        return tc_ui_rect{bounds().x + style.padding_left,
    ...
    float RichTextView::measure_width(tc_ui_document_handle document, std::string_view text, float font_size) const {
        if (text.empty())
            return 0.0f;
        tc_ui_text_metrics metrics{};
    ...
    void RichTextView::append_run(VisualRow& row,
                                  std::string text,
    ...
    void RichTextView::ensure_visual_rows(tc_ui_document_handle document, float width, float font_size) {
        ...
        if (cached_revision_ == model_->revision() && cached_width_ == width && cached_font_size_ == font_size) {
            return;
        }
        rows_.clear();
        ...
        for (size_t line_index = 0; line_index < model_->lines().size(); ++line_index) {
            ...
            for (const RichTextSegment& segment : line) {
                ...
                while (local_offset < segment.text.size()) {
                    ...
                    const std::string codepoint = segment.text.substr(local_offset, next - local_offset);
                    units.push_back(TextUnit{codepoint,
                                             segment.style,
                                             global_offset + local_offset,
                                             global_offset + next,
                                             measure_width(document, codepoint, font_size),
                                             is_wrap_space(codepoint)});
                    local_offset = next;
                }
            ...
            if (units.empty()) {
                ...
                for (const TextUnit& unit : units) {
                    append_run(current, unit.text, unit.style, unit.source_start, unit.source_end, unit.width);
                }
                flush();
            } else {
    ...
    tc_ui_rect RichTextView::prepare_layout(tc_ui_document_handle document, const tc_ui_style& style) {
        ...
        if (show_scrollbar_ && content_height() > content.height) {
            ensure_visual_rows(document, std::max(0.0f, content.width - scrollbar_width_), style.font_size);
    ...
    RichTextView::ScrollbarGeometry RichTextView::scrollbar_geometry(tc_ui_rect content) const {
        ...
        if (!result.visible)
            return result;
        result.track = tc_ui_rect{content.x + content.width, content.y, scrollbar_width_, content.height};
        const float ratio = std::min(1.0f, content.height / content_height());
        const float thumb_height = std::min(content.height, std::max(minimum_thumb_height_, content.height * ratio));
        const float travel = content.height - thumb_height;
        result.thumb =
            tc_ui_rect{result.track.x,
    ...
    float RichTextView::row_x_for_offset(tc_ui_document_handle document,
                                         const VisualRow& row,
                                         size_t offset,
    ...
    size_t
    RichTextView::source_offset_from_point(tc_ui_document_handle document, tc_ui_rect content, float x, float y) const {
        if (rows_.empty())
            return 0;
        const float visual_y = y - content.y + scroll_y_;
        const size_t row_index =
            std::min(rows_.size() - 1, static_cast<size_t>(std::max(0.0f, std::floor(visual_y / cached_line_height_))));
        ...
        for (const VisualRun& run : row.runs) {
            ...
            while (local < run.text.size()) {
                ...
                const float width =
                    measure_width(document, std::string_view(run.text).substr(local, next - local), cached_font_size_);
    ...
    void RichTextView::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        if (style.border_width > 0.0f) {
            tc_ui_painter_stroke_rounded_rect(context, bounds(), 3.0f, style.border, style.border_width);
        }
        ...
        if (model_->text().empty() && !placeholder_.empty()) {
            ...
            const size_t first = std::min(
                rows_.size(), static_cast<size_t>(std::max(0.0f, std::floor(scroll_y_ / cached_line_height_))));
            const size_t last = std::min(
                rows_.size(), static_cast<size_t>(std::ceil((scroll_y_ + content.height) / cached_line_height_)) + 1);
            for (size_t row_index = first; row_index < last; ++row_index) {
                const VisualRow& row = rows_[row_index];
                const float row_top = content.y + static_cast<float>(row_index) * cached_line_height_ - scroll_y_;
                if (has_selection()) {
                ...
                for (const VisualRun& run : row.runs) {
                    ...
                    tc_ui_painter_draw_text(context,
                                            run.text.c_str(),
                                            tc_ui_point{draw_x + italic_offset, row_top + style.font_size},
                                            style.font_size,
                                            color);
                    if (run.style.bold) {
                        tc_ui_painter_draw_text(context,
    ...
    tc_ui_event_result RichTextView::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
        ...
        if (event->type == TC_UI_POINTER_MOVE && dragging_scrollbar_) {
            ...
            if (travel > 0.0f) {
                set_scroll_y(drag_start_scroll_ + (event->y - drag_start_y_) * scrollbar.max_scroll / travel);
                clamp_scroll(content.height);
            }
            return TC_UI_EVENT_HANDLED;
        }
