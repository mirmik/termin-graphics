...
void TreeModel::validate_item(const CollectionItem &item) {
  if (!valid_utf8(item.stable_id) || !valid_utf8(item.text) ||
      !valid_utf8(item.subtitle)) {
...
const std::vector<TreeNodeId> &
TreeModel::sibling_list(TreeNodeId parent) const {
  if (parent == kInvalidTreeNodeId)
    return roots_;
...
TreeNodeId TreeModel::insert_child(TreeNodeId parent, size_t index,
                                   CollectionItem item) {
  validate_item(item);
...
void TreeModel::move(TreeNodeId id, TreeNodeId new_parent, size_t index) {
  ...
  if (found == nodes_.end())
    throw std::out_of_range("tree node id is not live");
  if (new_parent == id ||
      (new_parent != kInvalidTreeNodeId && is_descendant(new_parent, id))) {
    tc_log_error(
        "[termin-gui-native] tree model rejected a move that creates a cycle");
  ...
  destination.insert(destination.begin() + static_cast<std::ptrdiff_t>(index),
                     id);
...
void TreeModel::clear() {
  ...
  nodes_.clear();
  roots_.clear();
  notify(TreeChange{TreeChangeKind::Reset, kInvalidTreeNodeId,
                    kInvalidTreeNodeId, 0, count});
}
...
bool TreeExpansionModel::set_expanded(TreeNodeId id, bool value) {
  if (id == kInvalidTreeNodeId)
    return false;
  const bool changed =
      value ? expanded_.insert(id).second : expanded_.erase(id) != 0;
  if (!changed)
    return false;
  ++revision_;
  if (revision_ == 0)
...
bool TreeExpansionModel::toggle(TreeNodeId id) {
  return set_expanded(id, !expanded(id));
}
...
bool TreeExpansionModel::reconcile(const TreeTableModel &model) {
  bool changed = false;
  for (auto it = expanded_.begin(); it != expanded_.end();) {
    if (!model.contains(*it)) {
      it = expanded_.erase(it);
