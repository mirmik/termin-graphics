...
namespace termin::gui_native {
    ...
    tgfx::BackendType default_offscreen_backend() {
        ...
        if (platform_default == tgfx::BackendType::Vulkan || platform_default == tgfx::BackendType::D3D11) {
    ...
    namespace {
        ...
        fs::path module_path() {
            ...
            if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                                    ...
                                    reinterpret_cast<LPCWSTR>(&offscreen_module_anchor),
                                    &module)) {
        ...
        fs::path resolve_sdk_root(const std::string& configured) {
            ...
            if (const char* environment = std::getenv("TERMIN_SDK"); environment && environment[0] != '\0') {
                return fs::absolute(environment);
            }
            return module_path().parent_path().parent_path();
        }

        fs::path resolve_file(const std::string& configured,
            ...
            if (!configured.empty()) {
                result = configured;
            } else if (const char* environment = std::getenv(environment_name); environment && environment[0] != '\0') {
                result = environment;
            } else {
                result = fallback;
            }
            ...
            if (!is_file(result)) {
                offscreen_error(std::string("could not find ") + label + " at " + result.string());
            }
            return result;
        }
        ...
        fs::path find_on_path(const char* executable) {
            ...
            while (begin <= paths.size()) {
                ...
                if (!directory.empty()) {
                    const fs::path candidate = directory / executable_path(executable);
        ...
        void create_directory(const fs::path& path, const char* label) {
            ...
            if (error) {
                offscreen_error(std::string("could not create ") + label + " at " + path.string() + ": " +
                                error.message());
            }
        }
        ...
        termin::ShaderArtifactResolver resolve_config(OffscreenGuiCompositionConfig& config) {
            if (config.width <= 0 || config.height <= 0) {
                offscreen_error("framebuffer dimensions must be positive");
            }
            if (config.backend == tgfx::BackendType::OpenGL) {
                offscreen_error("OpenGL requires a window-system context; use Vulkan or D3D11");
            }
            if (config.backend == tgfx::BackendType::Null || !tgfx::backend_is_compiled(config.backend)) {
                offscreen_error(std::string("backend is unavailable: ") + tgfx::backend_name(config.backend));
            }

            const fs::path sdk_root = resolve_sdk_root(config.sdk_root);
            config.sdk_root = sdk_root.string();
            config.renderer.font_path = resolve_file(config.renderer.font_path,
                                                     "TERMIN_UI_FONT",
        ...
        public:
            OffscreenFrameSink(int width, int height)
                : width_(width),
                  height_(height) {}
            ...
            tgfx::TextureHandle latest_texture() const {
                return latest_texture_;
            }
    ...
    bool InMemoryDocumentPlatformServices::set_clipboard_text(const std::string& text) {
        const std::lock_guard<std::mutex> lock(impl_->mutex);
    ...
    struct OffscreenGuiComposition::Impl {
        ...
        explicit Impl(OffscreenGuiCompositionConfig composition_config)
            ...
            graphics = config.application_graphics_domain ? tgfx::GraphicsHost::create_application(config.backend)
            ...
            try {
                renderer = std::make_unique<DocumentRenderer>(*graphics, document, config.renderer, *sink, platform);
            } catch (...) {
        ...
        void require_open(const char* operation) const {
            if (closed || !graphics || graphics->is_closed() || !document.valid() || !renderer ||
                !renderer->is_open()) {
                offscreen_error(std::string("OffscreenGuiComposition::") + operation + " called after close");
            }
        }

        void prepare_frame() {
            if (sink->latest_texture() && sink->latest_frame_size() != sink->framebuffer_size()) {
    ...
    OffscreenGuiComposition::OffscreenGuiComposition(OffscreenGuiCompositionConfig config)
    ...
    OffscreenGuiComposition::OffscreenGuiComposition(OffscreenGuiComposition&&) noexcept = default;
    OffscreenGuiComposition& OffscreenGuiComposition::operator=(OffscreenGuiComposition&&) noexcept = default;
    ...
    InMemoryDocumentPlatformServices& OffscreenGuiComposition::platform_services() {
    ...
    bool OffscreenGuiComposition::tick() {
        ...
        if (should_close())
            return false;
        if (impl_->config.continuous_rendering || impl_->renderer->repaint_requested()) {
            render_frame();
        }
        return true;
    }
    ...
    std::vector<float> OffscreenGuiComposition::read_frame_rgba_float() {
        ...
        if (!texture)
            offscreen_error("no published frame is available to read");
    ...
    bool OffscreenGuiComposition::should_close() const {
        return !is_open() || impl_->close_requested.load(std::memory_order_acquire);
    }
