...
namespace termin::gui_native {
    ...
    namespace {
        ...
        ViewportPixelGeometry pixel_aligned_geometry(tc_ui_rect layout_rect) {
            if (!std::isfinite(layout_rect.x) || !std::isfinite(layout_rect.y) || !std::isfinite(layout_rect.width) ||
                !std::isfinite(layout_rect.height) || layout_rect.width <= 0.0f || layout_rect.height <= 0.0f) {
                return {};
            }
            ...
            const double right =
                std::round(static_cast<double>(layout_rect.x) + static_cast<double>(layout_rect.width));
            const double bottom =
                std::round(static_cast<double>(layout_rect.y) + static_cast<double>(layout_rect.height));
            ...
            return {
                tc_ui_rect{static_cast<float>(left),
    ...
    bool Viewport3D::surface_valid() const {
        ...
        try {
            ...
            tc_log_error("[termin-gui-native] Viewport3D surface validity check failed: %s", error.what());
        } catch (...) {
    ...
    bool Viewport3D::sync_surface_size() {
        ...
        try {
            ...
            tc_log_error("[termin-gui-native] Viewport3D size lookup failed during resize: %s", error.what());
            return false;
        } catch (...) {
        ...
        try {
            ...
            tc_log_error("[termin-gui-native] Viewport3D before-resize callback failed: %s", error.what());
            return false;
        } catch (...) {
    ...
    bool Viewport3D::dispatch_external_drag(const ViewportExternalDragEvent& event) {
        ...
        try {
            ...
            tc_log_error("[termin-gui-native] Viewport3D external drag handler failed: %s", error.what());
        } catch (...) {
    ...
    bool Viewport3D::sync_pointer_position(const tc_ui_pointer_event& event) {
        ...
        try {
            ...
            const double x = static_cast<double>(event.x - geometry.rect.x) * static_cast<double>(pixels.width) /
                             geometry.rect.width;
            const double y = static_cast<double>(event.y - geometry.rect.y) * static_cast<double>(pixels.height) /
        ...
        if (event->type == TC_UI_POINTER_DOWN || event->type == TC_UI_POINTER_UP) {
            ...
            if (surface_valid()) {
                try {
                    ...
                    surface_host_->pointer_button(
                        event->button, action, event->modifiers, std::max(1u, event->click_count));
                } catch (const std::exception& error) {
                    tc_log_error("[termin-gui-native] Viewport3D pointer button failed: %s", error.what());
                } catch (...) {
