...
namespace termin::gui_native {
    ...
    namespace {
        ...
        std::string normalize_single_line(std::string_view text) {
            ...
            for (size_t index = 0; index < text.size(); ++index) {
                ...
                if (current == '\r' && index + 1 < text.size() && text[index + 1] == '\n') {
                    ++index;
                }
    ...
    tc_ui_size TextInput::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        if (measure_text(document, displayed, style.font_size, metrics)) {
            measured.width = std::max(
                style.min_width, metrics.width + style.padding_left + style.padding_right + style.border_width * 2.0f);
            const float line_height = metrics.line_height > 0.0f ? metrics.line_height : metrics.height;
            measured.height = std::max(
                style.min_height, line_height + style.padding_top + style.padding_bottom + style.border_width * 2.0f);
        }
    ...
    void TextInput::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        const float line_height = has_metrics && metrics.line_height > 0.0f ? metrics.line_height : style.font_size;
        ...
        if (has_selection()) {
            ...
            measure_text(document,
                         std::string_view(text_).substr(selection_start(), selection_end() - selection_start()),
                         style.font_size,
                         selection_metrics);
        ...
        tc_ui_painter_draw_text(context, displayed.c_str(), tc_ui_point{text_x, baseline}, style.font_size, text_color);
        if (focused) {
    ...
    bool
    TextInput::measure_prefix(tc_ui_document_handle document, size_t byte_offset, float font_size, float& width) const {
        tc_ui_text_metrics metrics{};
    ...
    void TextInput::ensure_caret_visible(tc_ui_document_handle document) {
        ...
        if (viewport_width <= 0.0f || !measure_prefix(document, caret_, style.font_size, caret_width) ||
