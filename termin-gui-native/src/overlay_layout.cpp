...
namespace termin::gui_native {
    ...
    bool OverlayLayout::add_child(tc_widget_handle handle, OverlayAnchor anchor, tc_ui_point offset) {
        ...
        placements_.erase(
            std::remove_if(placements_.begin(),
                           placements_.end(),
                           [handle](const OverlayPlacement& item) { return same_handle(item.handle, handle); }),
            placements_.end());
    ...
    bool OverlayLayout::set_placement(tc_widget_handle handle, OverlayAnchor anchor, tc_ui_point offset) {
    ...
    tc_ui_size OverlayLayout::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
    ...
    void OverlayLayout::layout(tc_ui_document_handle document, tc_ui_rect rect) {
        ...
        for (size_t index = 0; index < child_count(); ++index) {
            ...
            if (item.anchor == OverlayAnchor::TopRight || item.anchor == OverlayAnchor::BottomRight) {
                x = rect.x + rect.width - size.width + item.offset.x;
            }
            if (item.anchor == OverlayAnchor::BottomLeft || item.anchor == OverlayAnchor::BottomRight) {
    ...
    void OverlayLayout::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        tc_ui_painter_push_clip(context, bounds());
        const tc_ui_style_override local_style = style_override();
        if ((local_style.fields & TC_UI_STYLE_BACKGROUND) != 0 && local_style.value.background.a > 0.0f) {
    ...
    tc_widget_handle OverlayLayout::hit_test(tc_ui_document_handle document, float x, float y) {
