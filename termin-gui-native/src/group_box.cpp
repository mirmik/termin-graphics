...
namespace termin::gui_native {
    ...
    GroupBox& GroupBox::set_padding(EdgeInsets padding) {
        ...
        style_override.fields |=
            TC_UI_STYLE_PADDING_LEFT | TC_UI_STYLE_PADDING_TOP | TC_UI_STYLE_PADDING_RIGHT | TC_UI_STYLE_PADDING_BOTTOM;
    ...
    GroupBox& GroupBox::set_background(SrgbColor color) {
        set_style_color(*this, TC_UI_STYLE_BACKGROUND, to_tc_ui_srgb(color));
        return *this;
    }

    GroupBox& GroupBox::set_border(SrgbColor color, float thickness) {
    ...
    tc_ui_size GroupBox::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        if (measure_text(document, title_, style.font_size, title_metrics)) {
            measured.width = std::max(measured.width, title_metrics.width + style.padding_left + style.padding_right);
        }
        if (!tc_widget_handle_is_invalid(this->content())) {
            if (tc_widget* content = resolve_child(document, c_widget(), this->content(), "GroupBox::measure")) {
                ...
                measured.width =
                    std::max(measured.width, content_size.width + style.padding_left + style.padding_right);
                measured.height = std::max(
                    measured.height, content_size.height + header_height_ + style.padding_top + style.padding_bottom);
            }
        }
    ...
    void GroupBox::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        if (!title_.empty()) {
            tc_ui_rect title_clip{bounds().x + style.padding_left,
                                  bounds().y,
    ...
    tc_ui_rect GroupBox::content_rect(tc_ui_document_handle document) const {
        const tc_ui_style style = computed_style(document);
        tc_ui_rect rect{bounds().x + style.padding_left,
