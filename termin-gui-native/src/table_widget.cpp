...
namespace termin::gui_native {
    ...
    TableWidget::TableWidget(std::shared_ptr<TableModel> model, std::shared_ptr<TableColumnModel> columns)
    ...
    void TableWidget::connect_models() {
        if (model_ && model_connection_ == 0) {
            model_connection_ =
                model_->changed().connect([this](TableModel&, const TableChange& change) { on_rows_changed(change); });
        }
        if (columns_ && column_connection_ == 0) {
            column_connection_ = columns_->changed().connect(
                [this](TableColumnModel&, const TableColumnChange& change) { on_columns_changed(change); });
        }
    }
    ...
    float TableWidget::body_height() const {
    ...
    std::pair<size_t, size_t> TableWidget::visible_range() const {
        if (model_->empty() || body_height() <= 0.0f)
            return {0, 0};
        const size_t first =
            std::min(model_->size(), static_cast<size_t>(std::max(0.0f, std::floor(scroll_y_ / row_height_))));
        const size_t last =
            std::min(model_->size(), static_cast<size_t>(std::ceil((scroll_y_ + body_height()) / row_height_)) + 1);
        return {first, last};
    }
        ...
        if (model_->empty()) {
            ...
            for (size_t row_index = first; row_index < last; ++row_index) {
                ...
                for (size_t column_index = 0; column_index < cell_count; ++column_index) {
                    const TableColumnLayout layout = column_layout_[column_index];
                    const tc_ui_rect cell{row_rect.x + layout.x, row_rect.y, layout.width, row_rect.height};
                    tc_ui_painter_push_clip(context, cell);
                    tc_ui_painter_draw_text(context,
                                            row.data.cells[column_index].c_str(),
                                            tc_ui_point{cell.x + cell_padding_, cell.y + cell.height * 0.66f},
                                            style.font_size,
                                            foreground);
                    tc_ui_painter_pop_clip(context);
    ...
    tc_ui_event_result TableWidget::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
        ...
        if (event->button == tcbase::mouse_button_value(tcbase::MouseButton::RIGHT)) {
            ...
            context_menu_requested_.emit(*this, row == SIZE_MAX ? -1 : static_cast<int64_t>(row), event->x, event->y);
            return TC_UI_EVENT_HANDLED;
        }
