...
namespace termin::gui_native {
    ...
    void TabView::add_page(std::string title, tc_widget_handle handle) {
        ...
        const auto duplicate = std::find_if(pages_.begin(), pages_.end(), [handle](const TabPage& page) {
    ...
    bool TabView::set_page_title(size_t index, std::string title) {
        if (index >= pages_.size()) {
            tc_log_error("[termin-gui-native] TabView set_page_title index out of range");
            return false;
        }
    ...
    void TabView::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        for (size_t i = 0; i < child_count(); ++i) {
            ...
            tc_ui_painter_fill_rounded_rect(
                context, tab, style.corner_radius, selected ? selected_style.background : style.background);
            if (selected) {
            ...
            tc_ui_painter_draw_text(
                ...
                tc_ui_point{
                    ...
                    centered_text_baseline(
                        document, page ? std::string_view(page->title) : std::string_view{}, style.font_size, tab)},
                style.font_size,
                style.foreground);
    ...
    tc_ui_event_result TabView::key_event(tc_ui_document_handle document, const tc_ui_key_event* event) {
    ...
    tc_widget_handle TabView::hit_test(tc_ui_document_handle document, float x, float y) {
        c_widget()->cursor_intent =
            y >= bounds().y && y < bounds().y + header_height_ ? TC_UI_CURSOR_HAND : TC_UI_CURSOR_INHERIT;
    ...
    tc_ui_rect TabView::page_rect() const {
        return tc_ui_rect{
            bounds().x, bounds().y + header_height_, bounds().width, std::max(0.0f, bounds().height - header_height_)};
    }
    ...
    float TabView::tab_width(tc_ui_document_handle document, size_t index) const {
        ...
        return std::max(min_tab_width_, metrics.width + style.padding_left + style.padding_right + 12.0f);
    }
