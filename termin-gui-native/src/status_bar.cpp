...
namespace termin::gui_native {
    ...
    tc_ui_size StatusBar::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        return clamp_size(tc_ui_size{std::max(preferred_size().width, metrics.width + padding_x_ * 2.0f),
    ...
    void StatusBar::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        tc_ui_painter_draw_line(context,
                                tc_ui_point{bounds().x, bounds().y},
                                tc_ui_point{bounds().x + bounds().width, bounds().y},
                                style.border,
                                style.border_width);
        if (!displayed_text().empty()) {
            ...
            tc_ui_painter_draw_text(context,
                                    ...
                                    tc_ui_point{bounds().x + padding_x_, bounds().y + bounds().height * 0.68f},
                                    font_size,
                                    foreground);
        }
    }
