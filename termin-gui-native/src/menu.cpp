...
namespace termin::gui_native {
    ...
    Menu::Menu(std::shared_ptr<CommandModel> model, Menu* parent, std::unordered_set<const CommandModel*> ancestors)
        : NativeWidget("Menu"),
    ...
    void Menu::layout(tc_ui_document_handle document, tc_ui_rect rect) {
        ...
        scroll_offset_ = clamp_float(scroll_offset_, 0.0f, std::max(0.0f, content_height_ - rect.height));
    }

    bool
    Menu::show(tc_ui_document_handle document, tc_ui_point position, tc_ui_rect viewport, bool dismiss_on_outside) {
        ...
        open_ = tc_ui_document_show_overlay_with_layout(document, handle(), flags, &overlay_layout, viewport);
        if (open_)
    ...
    size_t Menu::next_selectable(size_t from, int direction) const {
        ...
        for (size_t count = 0; count < model_->size(); ++count) {
            if (index == SIZE_MAX)
                ...
                index = direction > 0 ? (index + 1) % model_->size() : (index + model_->size() - 1) % model_->size();
    ...
    void Menu::ensure_current_visible() {
        ...
        scroll_offset_ = clamp_float(scroll_offset_, 0.0f, std::max(0.0f, content_height_ - bounds().height));
    }
    ...
    bool Menu::open_submenu(tc_ui_document_handle document, size_t index, bool select_first) {
        ...
        if (child_index_ == index && !tc_widget_handle_is_invalid(child_handle_)) {
            ...
            auto* existing = existing_widget ? dynamic_cast<Menu*>(native_widget_body(existing_widget)) : nullptr;
        ...
        const tc_widget_handle handle =
            tc_ui_document_adopt_widget(document, child->c_widget(), &Widget::delete_owned_widget);
        ...
        if (!child_ptr->show(document, tc_ui_point{bounds().x + bounds().width, item_y}, viewport, false)) {
    ...
    bool Menu::activate_current(tc_ui_document_handle document) {
        return activate_index(document, current_);
    }
        ...
        for (size_t index = 0; index < model_->size(); ++index) {
            ...
            if (data.kind == CommandKind::Separator) {
                tc_ui_painter_draw_line(context,
                                        tc_ui_point{bounds().x + padding_x_, y + height / 2},
                                        tc_ui_point{bounds().x + bounds().width - padding_x_, y + height / 2},
                                        style.border,
                                        1.0f);
                continue;
            }
            ...
            if (has_check) {
                if (data.checkable && data.checked)
                    tc_ui_painter_draw_text(context, "✓", tc_ui_point{prefix_x, baseline}, style.font_size, foreground);
            ...
            tc_ui_painter_draw_text(
                context, data.label.c_str(), tc_ui_point{label_x, baseline}, style.font_size, foreground);
            float right = bounds().x + bounds().width - padding_x_;
            if (has_submenu) {
                if (data.submenu)
                    tc_ui_painter_draw_text(
                        context, ">", tc_ui_point{right - 7.0f, baseline}, style.font_size, foreground);
                right -= column_gap_;
            }
    ...
    tc_ui_event_result Menu::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        ...
        if (event->type == TC_UI_POINTER_WHEEL && rect_contains(bounds(), event->x, event->y)) {
            scroll_offset_ = clamp_float(scroll_offset_ - event->wheel_y * item_height_ * 2.0f,
                                         0.0f,
        ...
        if (event->type == TC_UI_POINTER_MOVE) {
            ...
            if (index != SIZE_MAX && model_->command_at(index).data.submenu)
                ...
                close_submenu(document);
            return rect_contains(bounds(), event->x, event->y) ? TC_UI_EVENT_HANDLED : TC_UI_EVENT_IGNORED;
        }
        if (event->type == TC_UI_POINTER_DOWN && rect_contains(bounds(), event->x, event->y))
            return TC_UI_EVENT_HANDLED;
        if (event->type == TC_UI_POINTER_UP && event->button == tcbase::mouse_button_value(tcbase::MouseButton::LEFT)) {
            const size_t index = index_at(event->x, event->y);
