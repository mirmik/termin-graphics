...
namespace termin::gui_native {
    ...
    void Checkbox::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        const tc_ui_style style = computed_style(
            document, (checked_ ? TC_UI_STYLE_STATE_CHECKED : 0) | (keyboard_pressed_ ? TC_UI_STYLE_STATE_PRESSED : 0));
        const float side = std::min(bounds().width, bounds().height);
        const tc_ui_rect box{
            bounds().x + (bounds().width - side) * 0.5f, bounds().y + (bounds().height - side) * 0.5f, side, side};
        ...
        if (style.border_width > 0.0f && color_visible(style.border)) {
            tc_ui_painter_stroke_rounded_rect(context, box, style.corner_radius, style.border, style.border_width);
        }
        if (checked_) {
    ...
    tc_ui_event_result Checkbox::key_event(tc_ui_document_handle, const tc_ui_key_event* event) {
        if (!event) {
            return TC_UI_EVENT_IGNORED;
        }
