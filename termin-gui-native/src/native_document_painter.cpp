...
bool contains_document(
    std::span<const UiDocumentSubmission> submissions,
    tc_ui_document_handle document
) {
    return std::any_of(
        submissions.begin(), submissions.end(),
        [document](const UiDocumentSubmission& submission) {
            return tc_ui_document_handle_eq(
                submission.document.handle(), document);
        });
}
...
struct NativeDocumentPainter::Impl {
    ...
    void require_open(const char* operation) const {
        if (closed) {
            painter_error(
                std::string("NativeDocumentPainter::") + operation +
                " called after close");
        }
    }

    void synchronize_text_measurers(
        std::span<const UiDocumentSubmission> submissions
    ) {
        measured_documents.erase(
            std::remove_if(
                measured_documents.begin(), measured_documents.end(),
        ...
        for (const UiDocumentSubmission& submission : submissions) {
            ...
            const bool already_bound = std::any_of(
                measured_documents.begin(), measured_documents.end(),
                [document](tc_ui_document_handle candidate) {
...
NativeDocumentPainter::NativeDocumentPainter(
    NativeDocumentPainterConfig config
) : NativeDocumentPainter() {
    if (!config.font_path.empty() &&
        !set_default_font_path(config.font_path, config.font_size)) {
        painter_error(
            "failed to load UI font: " + config.font_path);
    }
}
...
NativeDocumentPainter::~NativeDocumentPainter() {
    ...
    try {
        ...
        tc_log_error(
            "[gui-native-document-painter] destructor shutdown failed: %s",
            error.what());
    } catch (...) {
        tc_log_error(
            "[gui-native-document-painter] destructor shutdown failed with unknown exception");
    }
}

bool NativeDocumentPainter::set_default_font_path(
    ...
    if (path.empty() || default_size_px <= 0) {
        tc_log_error(
            "[gui-native-document-painter] invalid font configuration");
        return false;
    }
    return impl_->renderer.set_default_font_path(path, default_size_px);
}

std::size_t NativeDocumentPainter::paint_documents(
    ...
    if (width <= 0 || height <= 0) {
        painter_error(
            "cannot paint into invalid viewport " +
    ...
    for (const UiDocumentSubmission& submission : documents) {
        if (!submission.document.valid()) {
            tc_log_error(
                "[gui-native-document-painter] skipping invalid document "
    ...
    std::sort(
        ...
        [](const UiDocumentSubmission& left,
           const UiDocumentSubmission& right) {
...
void NativeDocumentPainter::release_color_picker_surfaces(
    ColorPicker& picker
) {
