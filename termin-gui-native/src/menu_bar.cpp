...
namespace termin::gui_native {
    ...
    namespace {
        ...
        bool shortcut_matches(std::string_view descriptor, int32_t key, int32_t modifiers) {
            ...
            if (key_token.size() == 1)
                ...
                return false;
            constexpr int32_t known_modifiers = TC_UI_MOD_CTRL | TC_UI_MOD_SHIFT | TC_UI_MOD_ALT | TC_UI_MOD_SUPER;
    ...
    tc_ui_size MenuBar::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        return clamp_size(tc_ui_size{std::max(width, style.min_width), std::max(item_height_, style.min_height)},
                          constraints);
    }
    ...
    bool MenuBar::open_menu(tc_ui_document_handle document, size_t index, bool select_first) {
        ...
        const tc_widget_handle handle =
            tc_ui_document_adopt_widget(document, popup->c_widget(), &Widget::delete_owned_widget);
        ...
        popup_activated_connection_ =
            popup_ptr->activated().connect([this, index](Menu&, size_t, CommandId id, const CommandData& command) {
                activated_.emit(*this, index, id, command);
            });
        popup_dismissed_connection_ = popup_ptr->dismissed().connect([this](Menu&, tc_ui_overlay_dismiss_reason) {
    ...
    void MenuBar::switch_menu(tc_ui_document_handle document, int direction) {
        ...
        if (index == SIZE_MAX)
            ...
            index = direction > 0 ? (index + 1) % entries_.size() : (index + entries_.size() - 1) % entries_.size();
        open_menu(document, index, true);
    }

    tc_ui_event_result MenuBar::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
    ...
    bool MenuBar::dispatch_shortcut_in(const std::shared_ptr<CommandModel>& model,
                                       int32_t key,
                                       int32_t modifiers,
        ...
        for (size_t index = 0; index < model->size(); ++index) {
            ...
            if (command.data.kind != CommandKind::Action || !command.data.enabled)
                continue;
            if (command.data.submenu && dispatch_shortcut_in(command.data.submenu, key, modifiers, visited, menu_index))
                return true;
    ...
    void MenuBar::on_destroy(tc_ui_document_handle document) {
        close_menu(document);
    }
