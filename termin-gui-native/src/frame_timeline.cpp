...
namespace termin::gui_native {
    ...
    namespace {
        bool valid_sample(const FrameTimelineSample& sample) {
            return sample.stable_id >= 0 && std::isfinite(sample.interval_ms) && sample.interval_ms >= 0.0f &&
                   std::isfinite(sample.active_ms) && sample.active_ms >= 0.0f && std::isfinite(sample.lateness_ms) &&
                   sample.lateness_ms >= 0.0f && std::isfinite(sample.target_ms) && sample.target_ms >= 0.0f;
        }
    ...
    void FrameTimelineModel::append_samples(std::vector<FrameTimelineSample> samples, size_t max_samples) {
        ...
        samples_.insert(
            samples_.end(), std::make_move_iterator(samples.begin()), std::make_move_iterator(samples.end()));
    ...
    FrameTimelineWidget::~FrameTimelineWidget() {
        disconnect_model();
    }

    void FrameTimelineWidget::connect_model() {
        if (model_ && model_connection_ == 0) {
            model_connection_ = model_->changed().connect([this](FrameTimelineModel&) { on_model_changed(); });
        }
    }
    ...
    size_t FrameTimelineWidget::index_for_id(int64_t stable_id) const {
        ...
        const auto found = std::find_if(samples.begin(), samples.end(), [stable_id](const FrameTimelineSample& sample) {
    ...
    bool FrameTimelineWidget::select_index(size_t index, bool disable_follow) {
        if (index >= model_->samples().size())
            return false;
        const int64_t stable_id = model_->samples()[index].stable_id;
        if (disable_follow)
            follow_latest_ = false;
        if (selected_id_ && *selected_id_ == stable_id)
            return false;
    ...
    void FrameTimelineWidget::on_model_changed() {
        ...
        if (hovered_index_ >= model_->samples().size())
    ...
    void FrameTimelineWidget::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        const tc_ui_rect graph{rect.x, rect.y + header_height, rect.width, std::max(0.0f, rect.height - header_height)};
        float largest = 0.0f;
        float target = 0.0f;
        ...
        for (size_t index = begin; index < end; ++index) {
            ...
            tc_ui_painter_fill_rect(
                context, {x, graph.y + graph.height - interval_height, bar_width, interval_height}, interval_color);
            ...
            if (selected_id_ && sample.stable_id == *selected_id_) {
                tc_ui_painter_stroke_rect(
                    context, {x, graph.y, std::max(slot_width, 2.0f), graph.height}, style.accent, 2.0f);
            }
        }

        const FrameTimelineSample* detail = nullptr;
        if (hovered_index_ >= begin && hovered_index_ < end)
            detail = &model_->samples()[hovered_index_];
        else if (selected_id_) {
            const size_t selected = index_for_id(*selected_id_);
            if (selected != SIZE_MAX)
                detail = &model_->samples()[selected];
        }
        char label[160]{};
        if (detail) {
            std::snprintf(label,
                          sizeof(label),
                          "#%lld  interval %.2f ms  active %.2f ms  late %.2f ms",
                          static_cast<long long>(detail->stable_id),
                          detail->interval_ms,
                          detail->active_ms,
                          detail->lateness_ms);
        } else {
        ...
        tc_ui_painter_draw_text(
            context, label, {rect.x + 6.0f, rect.y + 19.0f}, std::max(9.0f, style.font_size - 1.0f), style.foreground);
        tc_ui_painter_stroke_rect(context, rect, style.border, style.border_width);
    }

    size_t FrameTimelineWidget::index_at(float x, float y) const {
        if (!detail::rect_contains(bounds(), x, y) || y < bounds().y + 28.0f)
            return SIZE_MAX;
        const auto [begin, end] = visible_range();
        if (begin == end || bounds().width <= 0.0f)
            return SIZE_MAX;
    ...
    tc_ui_event_result FrameTimelineWidget::pointer_event(tc_ui_document_handle document,
        ...
        if (!detail::rect_contains(bounds(), event->x, event->y))
            return TC_UI_EVENT_IGNORED;
        ...
        if (event->type == TC_UI_POINTER_DOWN &&
            ...
            return select_index(index_at(event->x, event->y)) ? TC_UI_EVENT_HANDLED : TC_UI_EVENT_IGNORED;
        }
        return TC_UI_EVENT_IGNORED;
    }
