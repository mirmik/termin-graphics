...
namespace termin::gui_native {
    ...
    bool SelectionModel::reconcile(size_t item_count) {
        ...
        selected_.erase(std::remove_if(selected_.begin(),
                                       selected_.end(),
                                       [item_count](size_t index) { return index >= item_count; }),
                        selected_.end());
        if (current_ >= item_count)
            current_ = item_count == 0 ? npos : item_count - 1;
        if (anchor_ >= item_count)
            anchor_ = selected_.empty() ? npos : selected_.front();
        return !same_selection(before, selected_);
    }

    bool SelectionModel::clear() {
        if (selected_.empty() && current_ == npos && anchor_ == npos)
            return false;
        selected_.clear();
        current_ = npos;
        anchor_ = npos;
    ...
    bool SelectionModel::extend_to(size_t index, bool additive) {
        ...
        if (mode_ == SelectionMode::Single)
            return select_only(index);
        if (anchor_ == npos)
            anchor_ = current_ == npos ? index : current_;
        const std::vector<size_t> before = selected_;
        if (!additive)
            selected_.clear();
    ...
    bool SelectionModel::select_all(size_t item_count) {
        if (mode_ != SelectionMode::Multiple)
            return false;
        const std::vector<size_t> before = selected_;
        selected_.resize(item_count);
        for (size_t index = 0; index < item_count; ++index)
            selected_[index] = index;
        current_ = item_count == 0 ? npos : item_count - 1;
    ...
    bool SelectionModel::items_inserted(size_t index, size_t count) {
        ...
        if (current_ != npos && current_ >= index)
            current_ += count;
        if (anchor_ != npos && anchor_ >= index)
            anchor_ += count;
    ...
    bool SelectionModel::items_erased(size_t index, size_t count, size_t remaining_count) {
        if (count == 0)
            return false;
        const size_t end =
            index > std::numeric_limits<size_t>::max() - count ? std::numeric_limits<size_t>::max() : index + count;
        const std::vector<size_t> before = selected_;
        selected_.erase(std::remove_if(selected_.begin(),
                                       selected_.end(),
                                       [index, end](size_t selected) { return selected >= index && selected < end; }),
                        selected_.end());
        for (size_t& selected : selected_) {
            if (selected >= end)
                selected -= count;
        ...
        const auto shift_position = [index, end, count, remaining_count](size_t position) {
            ...
            if (position >= index)
                return remaining_count == 0 ? npos : std::min(index, remaining_count - 1);
            return position;
        };
