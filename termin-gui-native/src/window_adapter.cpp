...
namespace termin::gui_native {
    ...
    namespace {
        ...
        public:
            explicit BorrowedWindowEndpoint(BackendWindow& window)
                : window_(&window) {}
        ...
        public:
            explicit BorrowedWindowPlatformServices(BackendWindow& window)
                : window_(&window) {}
    ...
    struct GuiWindowAdapter::Impl {
        ...
        Impl(tgfx::GraphicsHost& graphics_ref,
             TcDocument document_ref,
             DocumentRendererConfig config,
             BackendWindow& window_ref)
            ...
            if (&window->graphics_host() != graphics) {
                adapter_error("GuiWindowAdapter requires the window and renderer to share one GraphicsHost");
            }
            renderer = std::make_unique<DocumentRenderer>(*graphics, document, std::move(config), endpoint, platform);
            renderer->sync_presentation_metrics();
        }

        void require_open(const char* operation) const {
            if (closed || !renderer || !renderer->is_open()) {
                adapter_error(std::string("GuiWindowAdapter::") + operation + " called after close");
            }
        }
    ...
    GuiWindowAdapter::GuiWindowAdapter(tgfx::GraphicsHost& graphics,
                                       TcDocument document,
                                       DocumentRendererConfig config,
                                       BackendWindow& window)
        : impl_(std::make_unique<Impl>(graphics, document, std::move(config), window)) {}
    ...
    size_t GuiWindowAdapter::consume_events(std::span<const WindowEvent> events) {
        ...
        for (const WindowEvent& event : events) {
            if (event.type == WindowEventType::Resized || event.type == WindowEventType::DisplayScaleChanged) {
