...
namespace termin::gui_native {
    ...
    TableRowId TableModel::append(TableRowData row) {
        return insert(rows_.size(), std::move(row));
    }
    ...
    void TableColumnModel::validate_column(const TableColumn& column) {
        ...
        if (!std::isfinite(column.width) || column.width < 0.0f || !std::isfinite(column.min_width) ||
            ...
            (column.max_width > 0.0f && column.max_width < column.min_width) || !std::isfinite(column.stretch) ||
            column.stretch <= 0.0f || (column.policy == TableColumnPolicy::Fixed && column.width <= 0.0f)) {
    ...
    void TableColumnModel::append(TableColumn column) {
        insert(columns_.size(), std::move(column));
    }
