...
namespace termin::gui_native {
    ...
    namespace {
        ...
        void require_byte_count(std::span<const uint8_t> pixels, uint32_t width, uint32_t height) {
            ...
            if (pixels.size() != expected) {
                lease_argument_error("RGBA8 pixel buffer has " + std::to_string(pixels.size()) + " bytes; expected " +
    ...
    namespace {
        ...
        std::shared_ptr<DocumentRendererLeaseState> require_active(const std::shared_ptr<DynamicTextureRecord>& record,
            ...
            if (!state) {
                lease_error(std::string("DynamicTextureLease::") + operation + " called after host destruction");
            }
            {
                const std::lock_guard<std::mutex> lock(state->mutex);
                if (!state->open || !state->request_repaint || !state->graphics || !state->document.valid()) {
                    lease_error(std::string("DynamicTextureLease::") + operation +
                                " called after host or document shutdown");
                }
            }
            if (state->graphics->is_closed()) {
                lease_error(std::string("DynamicTextureLease::") + operation + " called after GraphicsHost shutdown");
            }
            return state;
        }

        Canvas* resolve_canvas(const DocumentRendererLeaseState& state,
        ...
        void apply_binding(
            Canvas& canvas, CanvasTextureLayer layer, tgfx::TextureHandle texture, uint32_t width, uint32_t height) {
            if (layer == CanvasTextureLayer::Image) {
                if (texture) {
                    canvas.set_texture(texture.id, tc_ui_size{static_cast<float>(width), static_cast<float>(height)});
                } else {
        ...
        bool clear_canvas_bindings(const DocumentRendererLeaseState& state,
        ...
        void release_record(const std::shared_ptr<DynamicTextureRecord>& record) noexcept {
            ...
            try {
                ...
                {
                    const std::lock_guard<std::mutex> lock(state->mutex);
                    open = state->open && state->request_repaint && state->graphics && state->document.valid();
                }
    ...
    void DocumentRendererLeaseState::register_record(const std::shared_ptr<DynamicTextureRecord>& record) {
    ...
    DynamicTextureLease::DynamicTextureLease(std::shared_ptr<DocumentRendererLeaseState> state)
    ...
    void DynamicTextureLease::update_region_rgba8(
        uint32_t x, uint32_t y, uint32_t width, uint32_t height, std::span<const uint8_t> pixels) {
        ...
        require_byte_count(pixels, width, height);
        if (x > record.width || y > record.height || width > record.width - x || height > record.height - y) {
    ...
    void DynamicTextureLease::bind_canvas(Canvas& canvas, CanvasTextureLayer layer) {
        ...
        const auto duplicate =
            std::find_if(impl_->record->bindings.begin(),
                         impl_->record->bindings.end(),
        ...
        apply_binding(canvas, layer, impl_->record->texture, impl_->record->width, impl_->record->height);
        state->request_repaint();
    }
