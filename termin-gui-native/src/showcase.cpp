...
namespace termin::gui_native {
    ...
    ShowcaseRefs build_showcase(TcDocument document) {
        ...
        root.set_padding(EdgeInsets{18.0f, 18.0f, 18.0f, 18.0f})
            .set_spacing(14.0f)
            .set_background(SrgbColor{0.055f, 0.060f, 0.070f, 1.0f});

        auto recent_menu = std::make_shared<CommandModel>();
        recent_menu->set_commands({
            CommandData{"recent-scene", "Showcase.termin", {}, {}, {}, CommandKind::Action, true, false, false, 0, {}},
        });
        auto file_menu = std::make_shared<CommandModel>();
        file_menu->set_commands({
            CommandData{"new", "New Scene", {}, "Ctrl+N", {}, CommandKind::Action, true, false, false, 0, {}},
            CommandData{"recent",
                        "Open Recent",
                        {},
                        {},
                        ...
                        false,
                        0,
                        std::move(recent_menu)},
            CommandData{"file-separator", {}, {}, {}, {}, CommandKind::Separator, true, false, false, 0, {}},
            CommandData{"quit", "Quit", {}, "Ctrl+Q", {}, CommandKind::Action, true, false, false, 0, {}},
        });
        auto view_menu = std::make_shared<CommandModel>();
        view_menu->set_commands({
            CommandData{"grid", "Grid", {}, {}, {}, CommandKind::Action, true, true, true, 0, {}},
        });
        ...
        navigation.set_padding(EdgeInsets{12.0f, 12.0f, 12.0f, 12.0f})
            .set_spacing(8.0f)
            .set_background(SrgbColor{0.10f, 0.11f, 0.13f, 1.0f})
            .set_border(SrgbColor{0.28f, 0.30f, 0.34f, 1.0f}, 1.0f);
        top.add_child(navigation);
        navigation.add_child(ui.make<Label>("Native UI", 18.0f, SrgbColor{0.92f, 0.95f, 1.0f, 1.0f}));
        navigation.add_child(ui.make<Button>("Scene", SrgbColor{0.18f, 0.32f, 0.54f, 1.0f}));
        navigation.add_child(ui.make<Button>("Assets", SrgbColor{0.16f, 0.42f, 0.32f, 1.0f}));
        navigation.add_child(ui.make<Button>("Build", SrgbColor{0.48f, 0.28f, 0.20f, 1.0f}));
        ...
        content.set_padding(EdgeInsets{14.0f, 14.0f, 14.0f, 14.0f})
            .set_spacing(10.0f)
            .set_background(SrgbColor{0.13f, 0.14f, 0.16f, 1.0f})
            .set_border(SrgbColor{0.36f, 0.39f, 0.44f, 1.0f}, 1.0f);
        ...
        preview_split.set_first(ui.make<Panel>("preview-a").set_fill(SrgbColor{0.20f, 0.42f, 0.62f, 1.0f}));
        preview_split.set_second(ui.make<Panel>("preview-b").set_fill(SrgbColor{0.26f, 0.48f, 0.34f, 1.0f}));
        ...
        controls.add_child(ui.make<Label>("Live", 14.0f, SrgbColor{0.78f, 0.84f, 0.92f, 1.0f}));
        ...
        refs.slider->changed().connect(
            [progress = refs.progress](Slider&, float value) { progress->set_value(value); });

        refs.text_area = &ui.make<TextArea>("Native TextArea\nUTF-8 selection and host "
        ...
        const TreeNodeId scene = tree_model->append_root(CollectionItem{"scene-root", "Scene", {}, true});
        const TreeNodeId camera = tree_model->append_child(scene, CollectionItem{"camera", "Camera", {}, true});
        tree_model->append_child(scene, CollectionItem{"light", "Key Light", {}, true});
        auto tree_expansion = std::make_shared<TreeExpansionModel>();
        tree_expansion->set_expanded(scene, true);
        refs.tree = &ui.make<TreeWidget>(std::move(tree_model), std::move(tree_expansion));
        ...
        table_columns->set_columns({
            TableColumn{"name", "Name", TableColumnPolicy::Stretch, 0.0f, 100.0f, 0.0f, 2.0f, true},
            TableColumn{"source", "Source", TableColumnPolicy::Stretch, 0.0f, 72.0f, 0.0f, 1.0f, true},
            TableColumn{"state", "State", TableColumnPolicy::Fixed, 88.0f, 64.0f, 120.0f, 1.0f, true},
        });
        refs.table = &ui.make<TableWidget>(std::move(table_model), std::move(table_columns));
        ...
        palette.set_padding(EdgeInsets{8.0f, 8.0f, 8.0f, 8.0f})
            ...
            .set_background(SrgbColor{0.10f, 0.11f, 0.13f, 1.0f})
            .set_border(SrgbColor{0.30f, 0.32f, 0.36f, 1.0f}, 1.0f);
        ...
        palette.add_child(ui.make<Swatch>(SrgbColor{0.90f, 0.22f, 0.24f, 1.0f}), 0, 0);
        palette.add_child(ui.make<Swatch>(SrgbColor{0.95f, 0.72f, 0.28f, 1.0f}), 0, 1);
        palette.add_child(ui.make<Swatch>(SrgbColor{0.24f, 0.68f, 0.48f, 1.0f}), 0, 2);
        palette.add_child(ui.make<Swatch>(SrgbColor{0.28f, 0.52f, 0.92f, 1.0f}), 0, 3);

        refs.tabs = &ui.make<TabView>("bottom-tabs");
        root.add_child(*refs.tabs);
        refs.tabs->add_page("Status", ui.make<Panel>("status-a").set_fill(SrgbColor{0.12f, 0.20f, 0.26f, 1.0f}));
        refs.tabs->add_page("Output", ui.make<Panel>("status-b").set_fill(SrgbColor{0.22f, 0.17f, 0.28f, 1.0f}));

        refs.status_bar = &ui.make<StatusBar>("Ready | Native UI");
        root.add_fixed_child(*refs.status_bar, 24.0f);

        refs.message_box =
            &ui.make<MessageBox>("Native Dialog", "Modal focus and result delivery", MessageBoxKind::Information);
