...
namespace termin::gui_native {
    ...
    void ToolBar::connect_model() {
        if (!model_ || model_connection_ != 0)
            return;
        model_connection_ =
            model_->changed().connect([this](CommandModel&, const CommandChange& change) { on_model_changed(change); });
    }
    ...
    void ToolBar::on_model_changed(const CommandChange& change) {
        ...
        if (keyboard_ >= model_->size() ||
            (keyboard_ != SIZE_MAX && (model_->command_at(keyboard_).data.kind != CommandKind::Action ||
    ...
    tc_ui_size ToolBar::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        return clamp_size(
            tc_ui_size{std::max(width, style.min_width), std::max(item_height_ + padding_ * 2.0f, style.min_height)},
            constraints);
    }
        ...
        for (size_t index = 0; index < model_->size(); ++index) {
            ...
            if (command.kind == CommandKind::Separator) {
                ...
                tc_ui_painter_draw_line(context,
                                        tc_ui_point{x, bounds().y + padding_ * 2.0f},
            ...
            if (has_icon) {
                const tc_ui_rect icon_rect{
                    rect.x + padding_, rect.y + (rect.height - icon_extent) * 0.5f, icon_extent, icon_extent};
    ...
    size_t ToolBar::index_at(float x, float y) const {
        for (size_t index = 0; index < item_rects_.size(); ++index) {
            if (model_->command_at(index).data.kind == CommandKind::Action && rect_contains(item_rects_[index], x, y)) {
                return index;
            }
        }
        return SIZE_MAX;
    ...
    size_t ToolBar::first_enabled(int direction) const {
        if (model_->empty())
            return SIZE_MAX;
        std::ptrdiff_t index = direction > 0 ? 0 : static_cast<std::ptrdiff_t>(model_->size()) - 1;
        while (index >= 0 && index < static_cast<std::ptrdiff_t>(model_->size())) {
            const CommandData& command = model_->command_at(static_cast<size_t>(index)).data;
    ...
    tc_ui_event_result ToolBar::pointer_event(tc_ui_document_handle document, const tc_ui_pointer_event* event) {
        if (!event)
    ...
    tc_ui_event_result ToolBar::key_event(tc_ui_document_handle, const tc_ui_key_event* event) {
