...
namespace termin::gui_native
{
    ...
    namespace
    {
        ...
        WidgetLifetimeToken lifetime_token(const SliderEdit& widget)
        {
            return WidgetLifetimeToken{widget.c_widget()->document,
                                       widget.handle()};
        }
    ...
    void SliderEdit::set_value(float value)
    {
        ...
        {
            tc_log_error(
                "[termin-gui-native] SliderEdit rejected non-finite value");
            return;
        }
        const float next = clamp_float(value, min_value_, max_value_);
        if (std::fabs(next - value_) <= 0.000001f)
            return;
        value_ = next;
    ...
    void SliderEdit::set_range(float min_value, float max_value)
    {
        ...
        {
            tc_log_error(
                "[termin-gui-native] SliderEdit rejected invalid range");
            return;
        }
    ...
    void SliderEdit::set_step(float step)
    {
        ...
        {
            tc_log_error(
                "[termin-gui-native] SliderEdit rejected invalid step");
            return;
        }
        step_ = step;
        if (!syncing_ && !tc_ui_document_handle_is_invalid(document()) &&
    ...
    void SliderEdit::set_decimals(int decimals)
    {
        ...
        {
            tc_log_error(
                "[termin-gui-native] SliderEdit rejected invalid decimals");
            return;
        }
        decimals_ = decimals;
        if (!syncing_ && !tc_ui_document_handle_is_invalid(document()) &&
    ...
    {
        if (!tc_widget_handle_is_invalid(slider_handle_) &&
            !tc_widget_handle_is_invalid(spin_box_handle_))
        {
            return true;
        }
        ...
        if (!tc_widget_append_child(c_widget(), slider_widget) ||
            !tc_widget_append_child(c_widget(), spin_widget))
        {
            tc_log_error("[termin-gui-native] SliderEdit failed to attach "
                         "numeric children");
            return false;
        }
        auto* slider_body = static_cast<Slider*>(slider_widget->body);
        auto* spin_body = static_cast<SpinBox*>(spin_widget->body);
        slider_connection_ = slider_body->changed().connect(
            [this, spin_body](Slider&, float value)
            {
                if (syncing_)
                    return;
                const WidgetLifetimeToken token = lifetime_token(*this);
        ...
        spin_box_connection_ = spin_body->changed().connect(
            [this, slider_body](SpinBox&, float value)
            {
                if (syncing_)
                    return;
                const WidgetLifetimeToken token = lifetime_token(*this);
    ...
    void SliderEdit::sync_children(tc_ui_document_handle document)
    {
        auto* slider = static_cast<Slider*>(
            tc_ui_document_resolve_widget(document, slider_handle_)->body);
        auto* spin_box = static_cast<SpinBox*>(
            tc_ui_document_resolve_widget(document, spin_box_handle_)->body);
        syncing_ = true;
    ...
    tc_ui_size SliderEdit::measure(tc_ui_document_handle,
    ...
    void SliderEdit::layout(tc_ui_document_handle document, tc_ui_rect rect)
    {
        ...
        const float slider_width =
            std::max(0.0f, rect.width - spin_box_width_ - spacing_);
        tc_widget_set_bounds(
            tc_ui_document_resolve_widget(document, slider_handle_),
            tc_ui_rect{rect.x, content_y, slider_width, content_height});
        tc_widget_set_bounds(
            tc_ui_document_resolve_widget(document, spin_box_handle_),
            tc_ui_rect{rect.x + slider_width + spacing_,
                       content_y,
                       spin_box_width_,
                       content_height});
    }

    void SliderEdit::paint(tc_ui_document_handle document,
                           tc_ui_paint_context* context)
    {
        ...
        paint_widget(tc_ui_document_resolve_widget(document, slider_handle_),
                     document,
                     context);
        paint_widget(tc_ui_document_resolve_widget(document, spin_box_handle_),
                     document,
                     context);
    ...
    tc_widget_handle
    SliderEdit::hit_test(tc_ui_document_handle document, float x, float y)
    {
    ...
    {
        if (tc_ui_document_is_alive(document, slider_handle_))
        {
            tc_widget* widget =
                tc_ui_document_resolve_widget(document, slider_handle_);
            static_cast<Slider*>(widget->body)
                ->changed()
                .disconnect(slider_connection_);
        }
        if (tc_ui_document_is_alive(document, spin_box_handle_))
        {
            tc_widget* widget =
                tc_ui_document_resolve_widget(document, spin_box_handle_);
            static_cast<SpinBox*>(widget->body)
                ->changed()
                .disconnect(spin_box_connection_);
        }
    }
