...
namespace termin::gui_native {
    ...
    bool SliderEdit::ensure_children(tc_ui_document_handle document) {
        ...
        slider_handle_ = tc_ui_document_adopt_widget(document, slider->c_widget(), &Widget::delete_owned_widget);
        if (tc_widget_handle_is_invalid(slider_handle_))
            return false;
        slider.release();
        spin_box_handle_ = tc_ui_document_adopt_widget(document, spin_box->c_widget(), &Widget::delete_owned_widget);
    ...
    void SliderEdit::layout(tc_ui_document_handle document, tc_ui_rect rect) {
        ...
        tc_widget_set_bounds(tc_ui_document_resolve_widget(document, slider_handle_),
                             tc_ui_rect{rect.x, content_y, slider_width, content_height});
        tc_widget_set_bounds(tc_ui_document_resolve_widget(document, spin_box_handle_),
    ...
    void SliderEdit::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        if (!label_.empty()) {
            ...
            tc_ui_painter_draw_text(
                context, label_.c_str(), tc_ui_point{bounds().x, bounds().y + 13.0f}, 11.0f, style.foreground);
        }
        tc_ui_painter_push_clip(context, bounds());
        paint_widget(tc_ui_document_resolve_widget(document, slider_handle_), document, context);
        paint_widget(tc_ui_document_resolve_widget(document, spin_box_handle_), document, context);
        tc_ui_painter_pop_clip(context);
    }

    tc_widget_handle SliderEdit::hit_test(tc_ui_document_handle document, float x, float y) {
    ...
    void SliderEdit::on_destroy(tc_ui_document_handle document) {
        if (tc_ui_document_is_alive(document, slider_handle_)) {
            tc_widget* widget = tc_ui_document_resolve_widget(document, slider_handle_);
            static_cast<Slider*>(widget->body)->changed().disconnect(slider_connection_);
        }
        if (tc_ui_document_is_alive(document, spin_box_handle_)) {
            tc_widget* widget = tc_ui_document_resolve_widget(document, spin_box_handle_);
