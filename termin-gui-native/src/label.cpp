...
namespace termin::gui_native {
    ...
    namespace {
        ...
        float text_width(tc_ui_document_handle document, std::string_view text, float font_size) {
        ...
        std::string ellipsize(tc_ui_document_handle document, std::string text, float font_size, float width) {
            ...
            while (!text.empty() && text_width(document, text, font_size) + ellipsis_width > width) {
        ...
        size_t
        best_character_break(tc_ui_document_handle document, std::string_view text, float font_size, float width) {
            size_t offset = 0;
            size_t best = 0;
        ...
        void add_line(LabelTextLayout& result,
            ...
            if ((force_overflow || measured > width) && std::isfinite(width) && overflow == TextOverflow::Ellipsis) {
                text = ellipsize(document, std::move(text), font_size, width);
                measured = text_width(document, text, font_size);
                result.truncated = true;
            }
            result.width = std::max(result.width, std::isfinite(width) ? std::min(measured, width) : measured);
            result.lines.push_back(LabelLine{std::move(text), measured});
        }

        void wrap_paragraph(LabelTextLayout& result,
                            tc_ui_document_handle document,
            ...
            if (wrap == TextWrapMode::None || !std::isfinite(width)) {
                add_line(result, document, std::string(paragraph), font_size, width, overflow);
                return;
            }
            if (paragraph.empty()) {
            ...
            while (!paragraph.empty()) {
                ...
                size_t split = best_character_break(document, paragraph, font_size, width);
                if (wrap == TextWrapMode::Word) {
                    size_t whitespace = std::string_view::npos;
                    size_t offset = 0;
                    while (offset < split) {
                        const unsigned char byte = static_cast<unsigned char>(paragraph[offset]);
                    ...
                    if (whitespace != std::string_view::npos && whitespace > 0) {
                        ...
                        if (word_end > split) {
                            add_line(result,
                                     document,
                                     std::string(paragraph.substr(0, word_end)),
                                     font_size,
                                     width,
                                     overflow,
                                     true);
                ...
                add_line(result, document, std::move(line), font_size, width, overflow);
                paragraph.remove_prefix(split);
                trim_leading_space(paragraph);
            }
        }

        LabelTextLayout layout_text(tc_ui_document_handle document,
            ...
            if (measure_text(document, "Mg", font_size, metrics)) {
                result.line_height = metrics.line_height > 0.0f ? metrics.line_height : metrics.height;
                result.ascent = metrics.ascent > 0.0f ? metrics.ascent : result.line_height;
            } else {
            ...
            do {
                ...
                wrap_paragraph(result, document, text.substr(start, end - start), font_size, width, wrap, overflow);
            ...
            if (line_limit > 0 && result.lines.size() > line_limit) {
                ...
                if (overflow == TextOverflow::Ellipsis && !result.lines.empty()) {
                    ...
                    last.text = ellipsize(document, std::move(last.text), font_size, width);
                    last.width = text_width(document, last.text, font_size);
                }
            }
            result.width = 0.0f;
            for (const LabelLine& line : result.lines) {
                result.width = std::max(result.width, std::isfinite(width) ? std::min(line.width, width) : line.width);
            }
            return result;
        }
    ...
    tc_ui_size Label::measure(tc_ui_document_handle document, tc_ui_constraints constraints) {
        ...
        const bool width_definite = constraints.max_size.width > 0.0f && constraints.max_size.width < kHuge;
        const float width = width_definite ? constraints.max_size.width : std::numeric_limits<float>::infinity();
        const LabelTextLayout text_layout =
            layout_text(document, text_, style.font_size, width, wrap_mode_, overflow_, max_lines_);
        tc_ui_size measured{text_layout.width, text_layout.line_height * static_cast<float>(text_layout.lines.size())};
        measured.width = std::max(measured.width, min_size().width);
        measured.height = std::max(measured.height, min_size().height);
        return clamp_size(measured, constraints);
    }
    ...
    void Label::paint(tc_ui_document_handle document, tc_ui_paint_context* context) {
        ...
        if (measure_text(document, "Mg", style.font_size, metrics)) {
            line_height = metrics.line_height > 0.0f ? metrics.line_height : metrics.height;
        }
        const size_t height_lines =
            line_height > 0.0f ? static_cast<size_t>(std::floor(bounds().height / line_height)) : 0;
        if (height_lines == 0) {
            return;
        }
        const size_t line_limit = max_lines_ == 0 ? height_lines : std::min(max_lines_, height_lines);
        const LabelTextLayout text_layout =
            layout_text(document, text_, style.font_size, bounds().width, wrap_mode_, overflow_, line_limit);
        tc_ui_painter_push_clip(context, bounds());
        for (size_t index = 0; index < text_layout.lines.size(); ++index) {
            const float baseline =
                bounds().y + static_cast<float>(index) * text_layout.line_height + text_layout.ascent;
            tc_ui_painter_draw_text(context,
                                    text_layout.lines[index].text.c_str(),
    ...
    void Label::update_unmeasured_size() {
        ...
        const float font_size =
            (style_override.fields & TC_UI_STYLE_FONT_SIZE) != 0 ? style_override.value.font_size : 15.0f;
