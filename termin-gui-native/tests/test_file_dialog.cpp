...
namespace {
    ...
    bool measure_text(void*, const char*, size_t byte_length, float font_size, tc_ui_text_metrics* metrics) {
        if (!metrics)
            return false;
    ...
    public:
        ...
        void fail_listing(std::string path) {
            failing_directories_.insert(std::move(path));
        }
    ...
    void test_modes_creation_and_failed_navigation_are_transactional() {
        ...
        assert(save.confirm().path == native_path_string(std::filesystem::path("/root") / "output.bin"));
    ...
    void test_overlay_vetoes_invalid_accept_and_emits_one_typed_result() {
        ...
        dialog.path_finished().connect(
            [&results](FileDialogOverlay&, const std::optional<std::string>& result) { results.push_back(result); });
