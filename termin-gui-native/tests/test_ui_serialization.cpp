...
namespace {
    ...
    void test_document_round_trip_preserves_structure_common_and_type_state() {
        ...
        assert(tc_widget_registry_register("test.ui.SerializableWidget", "test.module", nullptr, &factory));

        tc_ui_document_handle source = tc_ui_document_create();
        const tc_widget_handle root = tc_ui_document_create_registered_widget(source, "test.ui.SerializableWidget");
        const tc_widget_handle child = tc_ui_document_create_registered_widget(source, "test.ui.SerializableWidget");
        const tc_widget_handle overlay = tc_ui_document_create_registered_widget(source, "test.ui.SerializableWidget");
        ...
        assert(std::strcmp(tc_widget_type_name(restored_root_widget), "test.ui.SerializableWidget") == 0);
        ...
        const tc_ui_widget_layout_spec restored_layout = tc_widget_layout_spec(restored_child_widget);
    ...
    void test_editable_text_round_trip_preserves_persistent_state() {
        ...
        assert(std::strcmp(tc_value_dict_get(input_record, "type")->data.s, "termin.gui.TextInput") == 0);
        assert(std::strcmp(tc_value_dict_get(input_state, "text")->data.s, "Scene 01") == 0);
        assert(std::strcmp(tc_value_dict_get(input_state, "placeholder")->data.s, "Enter scene name") == 0);
        ...
        tc_value* restored_records = tc_value_dict_get(const_cast<tc_value*>(reserialized.raw()), "widgets");
        tc_value* area_record = tc_value_list_get(restored_records, 1);
        tc_value* area_state = tc_value_dict_get(area_record, "state");
        assert(std::strcmp(tc_value_dict_get(area_record, "type")->data.s, "termin.gui.TextArea") == 0);
        assert(std::strcmp(tc_value_dict_get(area_state, "text")->data.s, "Prompt\nsecond line") == 0);
        assert(std::strcmp(tc_value_dict_get(area_state, "placeholder")->data.s, "Enter prompt") == 0);
