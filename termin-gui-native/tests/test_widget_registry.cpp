...
namespace {
    ...
    tc_widget_factory_descriptor descriptor(FactoryState& state) {
        return tc_widget_factory_descriptor{
            ...
            TC_LANGUAGE_CXX,
            &create_factory_widget,
            &after_adopt_factory_widget,
            &destroy_factory_userdata,
            &state,
            &serialize_factory_widget,
            &deserialize_factory_widget,
        };
    }
    ...
    void test_owned_factory_identity_and_unload_invalidation() {
        ...
        assert(tc_widget_registry_register("test.ui.OwnedWidget", "test.module", "termin.gui.Widget", &factory));
        ...
        assert(std::strcmp(tc_runtime_type_registry_get_owner("test.ui.OwnedWidget"), "test.module") == 0);
        assert(std::strcmp(tc_runtime_type_registry_get_parent("test.ui.OwnedWidget"), "termin.gui.Widget") == 0);

        tc_ui_document_handle document = tc_ui_document_create();
        assert(tc_ui_document_is_valid(document));
        const tc_widget_handle parent = tc_ui_document_create_registered_widget(document, "test.ui.OwnedWidget");
        const tc_widget_handle child = tc_ui_document_create_registered_widget(document, "test.ui.OwnedWidget");
        ...
        assert(
            !tc_widget_registry_register("test.ui.OwnedWidget", "test.module", "termin.gui.Widget", &rejected_factory));
    ...
    void test_descriptor_validation_leaves_no_partial_widget_type() {
        ...
        assert(!tc_widget_registry_register(
            "test.ui.MissingParent", "test.module", "test.ui.DoesNotExist", &missing_parent));
        assert(!tc_runtime_type_registry_has_type("test.ui.MissingParent"));
        assert(!tc_widget_registry_register("test.ui.SelfParent", "test.module", "test.ui.SelfParent", &self_parent));
    ...
    void test_idle_factory_replacement_uses_a_fresh_descriptor() {
        ...
        tc_ui_document_handle document = tc_ui_document_create();
        const tc_widget_handle handle = tc_ui_document_create_registered_widget(document, "test.ui.Replaceable");
    ...
    void test_borrowed_factory_is_explicit_and_not_deleted() {
        ...
        tc_ui_document_handle document = tc_ui_document_create();
        const tc_widget_handle handle = tc_ui_document_create_registered_widget(document, "test.ui.BorrowedWidget");
        assert(!tc_widget_handle_is_invalid(handle));
    ...
    void test_after_adopt_failure_rolls_back_handle_and_owned_body() {
        ...
        tc_ui_document_handle document = tc_ui_document_create();
        const tc_widget_handle handle = tc_ui_document_create_registered_widget(document, "test.ui.FailingWidget");
        assert(tc_widget_handle_is_invalid(handle));
    ...
    void test_registered_state_hooks_round_trip_strict_dict_state() {
        ...
        tc_ui_document_handle document = tc_ui_document_create();
        const tc_widget_handle handle = tc_ui_document_create_registered_widget(document, "test.ui.StatefulWidget");
        tc_widget* widget = tc_ui_document_resolve_widget(document, handle);
        assert(widget);
    ...
    void test_owner_reload_invalidates_documents_nested_trees_and_factory_userdata() {
        ...
        assert(tc_widget_registry_register("test.ui.ReloadParent", "test.module.reload", nullptr, &parent_factory));
        assert(tc_widget_registry_register("test.ui.ReloadSibling", "test.module.reload", nullptr, &sibling_factory));
        assert(tc_widget_registry_register("test.ui.ForeignChild", "test.module.foreign", nullptr, &foreign_factory));
        ...
        const tc_widget_handle parent = tc_ui_document_create_registered_widget(first_document, "test.ui.ReloadParent");
        ...
        assert(tc_widget_registry_unregister_owner("test.module.reload", TC_WIDGET_OWNER_RELOAD_INVALIDATE) == 2);
        ...
        const tc_widget_factory_descriptor replacement_parent_factory = descriptor(replacement_parent_state);
        assert(tc_widget_registry_register(
            "test.ui.ReloadParent", "test.module.reload", nullptr, &replacement_parent_factory));
        ...
        assert(tc_widget_registry_unregister_owner("test.module.reload", TC_WIDGET_OWNER_RELOAD_INVALIDATE) == 1);
        assert(replacement_parent_state.userdata_destroys == 1);
        assert(tc_widget_registry_unregister_owner("test.module.foreign", TC_WIDGET_OWNER_RELOAD_INVALIDATE) == 1);
