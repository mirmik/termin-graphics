...
namespace termin_gui_native_test {
    ...
    inline size_t count_commands(const tc_ui_draw_list* draw_list, tc_ui_draw_command_type type) {
        size_t count = 0;
    ...
    inline bool
    test_text_measure(void*, const char* text, size_t byte_length, float font_size, tc_ui_text_metrics* out_metrics) {
    ...
    inline void install_test_text_measurer(TcDocument document) {
        const auto metrics = tc_ui_presentation_metrics_identity(tc_ui_size{1920.0f, 1080.0f});
    ...
    public:
        ...
        int up_count = 0;
        int cancel_count = 0;
        tc_ui_pointer_cancel_reason last_cancel_reason = TC_UI_POINTER_CANCEL_EXPLICIT;
    ...
    public:
        ...
        tc_ui_event_result key_event(tc_ui_document_handle, const tc_ui_key_event* event) override {
            if (!event) {
                return TC_UI_EVENT_IGNORED;
            }
            key_count += 1;
        ...
        tc_ui_event_result text_event(tc_ui_document_handle, const tc_ui_text_event* event) override {
            if (!event || !event->text) {
                return TC_UI_EVENT_IGNORED;
            }
            text_count += 1;
