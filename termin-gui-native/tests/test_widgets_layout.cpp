...
namespace termin_gui_native_test
{
    ...
    void test_controls_handle_pointer_events()
    {
        ...
        assert(
            tc_widget_handle_eq(document.pointer_capture(), checkbox.handle()));
        ...
        assert(document.dispatch_pointer_event(slider_event) ==
               TC_UI_EVENT_HANDLED);
        assert(
            tc_widget_handle_eq(document.pointer_capture(), slider.handle()));
    ...
    void test_text_widgets_clip_text_paint()
    {
        ...
        DocumentBuilder ui(document);
        auto& root = ui.make_root<BoxLayout>(Orientation::Vertical, "root");
        auto& label = ui.make<Label>(
            "Long label text that must stay inside its widget", 14.0f);
        auto& input = ui.make<TextInput>(
            "Long input text that must stay inside the edit box");
    ...
    void test_text_measurement_uses_proportional_metrics()
    {
        ...
        const tc_ui_size narrow_size =
            narrow.measure(document.get(), constraints);
    ...
    void test_wrapped_label_and_wrap_layout_reflow()
    {
        ...
        const tc_ui_constraints narrow_constraints{tc_ui_size{0.0f, 0.0f},
                                                   tc_ui_size{25.0f, 200.0f}};
        const tc_ui_constraints wide_constraints{tc_ui_size{0.0f, 0.0f},
                                                 tc_ui_size{100.0f, 200.0f}};
    ...
    void test_text_input_scrolls_to_keep_caret_inside_clip()
    {
             ...
             ++index)
        {
            ...
            if (command->type == TC_UI_DRAW_LINE &&
                near(command->p0.x, command->p1.x) &&
    ...
    void test_text_area_multiline_utf8_editing_navigation_and_scroll()
    {
        ...
        int changed = 0;
        area.changed().connect([&changed](TextArea&, const std::string&)
                               { ++changed; });
    ...
    void test_slider_edit_owns_canonical_children_and_syncs_values()
    {
        ...
        tc_widget* spin_widget = tc_ui_document_resolve_widget(
            document.get(), edit.spin_box_handle());
        ...
        assert(tc_widget_handle_eq(
            document.hit_test(slider_bounds.x + slider_bounds.width * 0.5f,
                              slider_bounds.y + slider_bounds.height * 0.5f),
            edit.slider_handle()));
        const tc_ui_rect spin_bounds = tc_widget_bounds(spin_widget);
        assert(tc_widget_handle_eq(
            document.hit_test(spin_bounds.x + spin_bounds.width * 0.5f,
        ...
        assert(
            tc_ui_document_set_focus(document.get(), edit.spin_box_handle()));
        tc_ui_key_event key{};
        key.type = TC_UI_KEY_DOWN;
        key.key = TC_UI_KEY_DOWN_ARROW;
        assert(document.dispatch_key_event(key) == TC_UI_EVENT_HANDLED);
        ...
        tc_ui_document_handle destructive_document_handle =
        ...
        destructive_document.layout_roots(
            tc_ui_rect{0.0f, 0.0f, 300.0f, 34.0f});

        const tc_widget_handle destroying_edit_handle =
            destroying_edit.handle();
        tc_widget* destroying_slider_widget = tc_ui_document_resolve_widget(
            destructive_document.get(), destroying_edit.slider_handle());
        assert(destroying_slider_widget);
        auto* destroying_slider =
            static_cast<Slider*>(destroying_slider_widget->body);
        destroying_edit.changed().connect(
            [&destructive_document, destroying_edit_handle](SliderEdit&, float)
            {
                assert(tc_ui_document_destroy_widget_recursive(
                    destructive_document.get(), destroying_edit_handle));
            });

        destroying_slider->set_value(0.75f);
        assert(!tc_ui_document_is_alive(destructive_document.get(),
                                        destroying_edit_handle));
        assert(tc_ui_document_live_widget_count(destructive_document.get()) ==
               0);
    ...
    void test_combo_box_overlay_selection_and_destruction()
    {
        ...
        int changes = 0;
        combo.changed().connect([&changes](ComboBox&, int, const std::string&)
                                { ++changes; });
    ...
    void test_icon_image_and_canvas_media_contracts()
    {
        ...
        assert(canvas.pointer_event(document.get(), &wheel) ==
               TC_UI_EVENT_HANDLED);
        ...
        canvas.layout(document.get(),
                      tc_ui_rect{canvas.bounds().x,
                                 canvas.bounds().y,
        ...
        assert(icon.pointer_event(document.get(), &click) ==
               TC_UI_EVENT_HANDLED);
             ...
             ++index)
        {
            ...
            if (!command || command->type != TC_UI_DRAW_TEXTURE)
                continue;
            linear_textures +=
                command->texture_sampling == TC_UI_TEXTURE_SAMPLING_LINEAR;
            nearest_textures +=
                command->texture_sampling == TC_UI_TEXTURE_SAMPLING_NEAREST;
        }
