...
[[noreturn]] void fail(const MatrixCase &test_case, const char *check) {
  std::fprintf(stderr,
               ...
               test_case.name, test_case.density, test_case.font_scale,
               test_case.logical_extent.width, test_case.logical_extent.height,
               check);
  std::abort();
}
...
tc_widget *widget(LoadedUiScript &loaded, const char *name) {
  return tc_ui_document_resolve_widget(loaded.document().handle(),
                                       loaded.named(name).handle);
}

float run_case(LoadedUiScript &loaded, BoxLayout &hud, IconButton &action,
  ...
  require(document.set_presentation_metrics(metrics), test_case,
          "presentation metrics rejected");

  tc_ui_rect layout_rect{};
  require(document.presentation_layout_rect(layout_rect), test_case,
          "safe layout rect unavailable");
  ...
  require(
      near(root_bounds.x, test_case.logical_safe_insets.left) &&
          near(root_bounds.y, test_case.logical_safe_insets.top) &&
          near(root_bounds.width, test_case.logical_extent.width -
                                      test_case.logical_safe_insets.left -
                                      test_case.logical_safe_insets.right) &&
          near(root_bounds.height, test_case.logical_extent.height -
                                       test_case.logical_safe_insets.top -
                                       test_case.logical_safe_insets.bottom),
      test_case, "root does not match logical safe rect");
  require(hud.orientation() == test_case.expected_orientation, test_case,
          "responsive orientation mismatch");
  require(near(hud_bounds.width, test_case.expected_hud_width), test_case,
          "responsive HUD width mismatch");
  require(hud_bounds.x >= root_bounds.x && hud_bounds.y >= root_bounds.y &&
              hud_bounds.x + hud_bounds.width <=
                  root_bounds.x + root_bounds.width &&
              hud_bounds.y + hud_bounds.height <=
                  root_bounds.y + root_bounds.height,
          test_case, "HUD escaped safe bounds");
  require(button_bounds.width >= 48.0f && button_bounds.height >= 48.0f,
          test_case, "minimum touch target was lost");
  require(hint_bounds.x >= hud_bounds.x && hint_bounds.y >= hud_bounds.y &&
              hint_bounds.x + hint_bounds.width <=
                  hud_bounds.x + hud_bounds.width &&
              hint_bounds.y + hint_bounds.height <=
                  hud_bounds.y + hud_bounds.height,
          test_case, "wrapped hint escaped HUD bounds");
  ...
  require(tc_ui_presentation_metrics_physical_to_logical_point(
              &metrics,
              tc_ui_point{
              ...
              &converted) &&
              near(converted.x, logical_button_center.x) &&
              near(converted.y, logical_button_center.y),
          test_case, "fractional physical/logical pointer conversion diverged");
  ...
  require(document.dispatch_pointer_event(pointer) == TC_UI_EVENT_HANDLED,
          test_case, "button did not consume converted pointer");
  pointer.type = TC_UI_POINTER_UP;
  require(document.dispatch_pointer_event(pointer) == TC_UI_EVENT_HANDLED,
          test_case, "button did not release converted pointer");
  ...
  require(document.dispatch_pointer_event(pointer) == TC_UI_EVENT_IGNORED,
          test_case, "uncovered scene pointer was consumed");
...
int main() {
  ...
  if (!hud_widget || !action_widget) {
    std::fprintf(stderr,
                 "adaptive layout matrix: showcase HUD is incomplete\n");
    return 1;
  }
  ...
  for (const float font_scale : {1.0f, 1.3f, 1.5f}) {
    ...
    require(height >= previous_height, test_case,
            "font scale reduced laid-out HUD height");
