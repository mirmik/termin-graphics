...
namespace {
    ...
    public:
        explicit TestSurfaceHost(std::shared_ptr<HostTrace> trace)
        ...
        uint32_t texture_id() const override {
            return trace_->texture;
        }
        ViewportSurfaceSize framebuffer_size() const override {
            return trace_->size;
        }
    ...
    void test_surface_resize_paint_input_and_drag_contract() {
        ...
        viewport->before_resize().connect([trace](Viewport3D&, ViewportSurfaceSize previous, ViewportSurfaceSize next) {
        ...
        ViewportExternalDragEvent drag{
            ViewportExternalDragPhase::Drop, "text/uri-list", "file:///tmp/scene.tscene", 12.0f, 14.0f};
    ...
    void test_detach_destroy_and_stale_surface_are_safe() {
        ...
        assert(document.dispatch_pointer_event(
                   tc_ui_pointer_event{TC_UI_POINTER_MOVE, 5.0f, 6.0f, 0, 0, 0, 0.0f, 0.0f}) == TC_UI_EVENT_HANDLED);
    ...
    void test_pointer_coordinates_scale_to_display_pixels() {
        ...
        assert(document.dispatch_pointer_event(
                   tc_ui_pointer_event{TC_UI_POINTER_MOVE, 25.0f, 10.0f, 0, 0, 0, 0.0f, 0.0f}) == TC_UI_EVENT_HANDLED);
