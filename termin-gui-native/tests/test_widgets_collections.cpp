...
namespace termin_gui_native_test {
    ...
    void test_cpp_theme_style_facade_inheritance_and_state() {
        ...
        assert(button.has_dirty_flags(TC_WIDGET_DIRTY_LAYOUT | TC_WIDGET_DIRTY_PAINT | TC_WIDGET_DIRTY_STATE));
    ...
    void test_list_widget_pointer_keyboard_and_multi_selection() {
        ...
        list.selection_changed().connect(
            [&changes](ListWidget&, const std::vector<size_t>& selected) { changes.push_back(selected); });
        size_t activated = SelectionModel::npos;
        list.activated().connect([&activated](ListWidget&, size_t index, const CollectionItem&) { activated = index; });
        ...
        list.context_menu_requested().connect([&context_requests](ListWidget&, int64_t index, float x, float y) {
    ...
    void test_tree_model_stable_ids_move_and_expansion_reconcile() {
        TreeModel model;
        const TreeNodeId root_a = model.append_root(CollectionItem{"a", "A", {}, true});
        const TreeNodeId root_b = model.append_root(CollectionItem{"b", "B", {}, true});
        const TreeNodeId child = model.append_child(root_a, CollectionItem{"child", "Child", {}, true});
        const TreeNodeId grandchild = model.append_child(child, CollectionItem{"grandchild", "Grandchild", {}, true});
        assert(model.size() == 4);
    ...
    void test_tree_widget_virtualizes_large_expanded_model() {
        ...
        for (size_t root_index = 0; root_index < 100; ++root_index) {
            const TreeNodeId root = model->append_root(
                CollectionItem{"root-" + std::to_string(root_index), "Root " + std::to_string(root_index), {}, true});
            expansion->set_expanded(root, true);
            for (size_t child_index = 0; child_index < 100; ++child_index) {
                model->append_child(
                    root,
                    CollectionItem{"node-" + std::to_string(root_index) + "-" + std::to_string(child_index),
    ...
    void test_tree_widget_pointer_keyboard_signals_and_lifetime() {
        ...
        DocumentBuilder ui(document);
        auto model = std::make_shared<TreeModel>();
        const TreeNodeId root = model->append_root(CollectionItem{"root", "Root", {}, true});
        ...
        const TreeNodeId disabled = model->append_child(root, disabled_item);
        const TreeNodeId last = model->append_child(root, CollectionItem{"last", "Last", {}, true});
        ...
        tree.selection_changed().connect([&selections](TreeWidget&, TreeNodeId node) { selections.push_back(node); });
        tree.expansion_changed().connect(
            [&expansions](TreeWidget&, TreeNodeId node, bool value) { expansions.emplace_back(node, value); });
        tree.activated().connect(
            [&activated](TreeWidget&, TreeNodeId node, const CollectionItem&) { activated = node; });
        tree.toggle_activated().connect(
            [&toggles](TreeWidget&, TreeNodeId node, int toggle_index, const CollectionItem&) {
                toggles.emplace_back(node, toggle_index);
            });
        tree.delete_requested().connect(
            [&delete_requested](TreeWidget&, TreeNodeId node, const CollectionItem&) { delete_requested = node; });
        tree.context_menu_requested().connect(
            [&context_requested](TreeWidget&, TreeNodeId node, float, float) { context_requested = node; });
        ...
        const TreeNodeId destroying_root =
            destroying_model->append_root(CollectionItem{"destroying-root", "Root", {}, true});
        const TreeNodeId destroying_child =
            destroying_model->append_child(destroying_root, CollectionItem{"destroying-child", "Child", {}, true});
        ...
        destroying_tree.selection_changed().connect([&document, destroying_handle](TreeWidget&, TreeNodeId) {
            assert(tc_ui_document_destroy_widget(document.get(), destroying_handle));
        });
    ...
    void test_tree_widget_drag_drop_positions_and_capture() {
        ...
        DocumentBuilder ui(document);
        auto model = std::make_shared<TreeModel>();
        const TreeNodeId first = model->append_root(CollectionItem{"first", "First", {}, true});
        const TreeNodeId second = model->append_root(CollectionItem{"second", "Second", {}, true});
        auto& tree = ui.make_root<TreeWidget>(model);
        tree.set_row_height(30.0f);
        ...
        tree.drop_requested().connect(
            [&](TreeWidget&, TreeNodeId dragged, TreeNodeId drop_target, TreeDropPosition drop_position) {
                dropped = dragged;
                target = drop_target;
    ...
    void test_table_models_preserve_row_ids_and_validate_columns() {
        TableModel model;
        const TableRowId first = model.append(TableRowData{"first", {"First", "1"}, true});
        const TableRowId last = model.append(TableRowData{"last", {"Last", "2"}, true});
        const TableRowId middle = model.insert(1, TableRowData{"middle", {"Middle", "3"}, false});
        assert(model.size() == 3);
        ...
        columns.set_columns({
            TableColumn{"name", "Name", TableColumnPolicy::Fixed, 80.0f, 50.0f, 120.0f, 1.0f, true},
            TableColumn{"value", "Value", TableColumnPolicy::Stretch, 0.0f, 40.0f, 0.0f, 1.0f, true},
        });
        ...
        try {
            columns.append(TableColumn{"name", "Duplicate", TableColumnPolicy::Stretch, 0.0f, 40.0f, 0.0f, 1.0f, true});
        } catch (const std::invalid_argument&) {
    ...
    void test_tree_table_model_preserves_identity_and_validates_hierarchy() {
        ...
        assert(model.roots() == std::vector<TreeTableNodeId>({render, model.find("events")}));
        assert(model.node(render).children == std::vector<TreeTableNodeId>({compose}));
        ...
        rejected = false;
        try {
            model.set_rows({TreeTableRowData{"a", "b", {"A"}, true}, TreeTableRowData{"b", "a", {"B"}, true}});
        } catch (const std::invalid_argument&) {
            rejected = true;
        }
        assert(rejected);
    ...
    void test_tree_table_widget_expansion_navigation_and_columns() {
        ...
        model->set_rows({
            TreeTableRowData{"render", "", {"Render", "12.0"}, true},
            TreeTableRowData{"render/compose", "render", {"Compose", "8.0"}, true},
            TreeTableRowData{"render/compose/pass", "render/compose", {"Pass", "5.0"}, true},
            TreeTableRowData{"events", "", {"Events", "2.0"}, true},
        });
        auto columns = std::make_shared<TableColumnModel>();
        columns->set_columns({
            TableColumn{"section", "Section", TableColumnPolicy::Stretch, 0.0f, 100.0f},
    ...
    void test_file_grid_widget_virtualizes_large_model_and_responsive_layout() {
        ...
        for (size_t index = 0; index < 10000; ++index) {
            items.push_back(
                CollectionItem{"file-" + std::to_string(index),
                               index == 0 ? "A very long UTF-8 filename пример.txt" : "File " + std::to_string(index),
                               ".txt",
                               true,
                               index == 0 ? 77u : 0u});
        }
    ...
    CommandData
    test_command(std::string id, std::string label, bool enabled = true, bool checkable = false, bool checked = false) {
        return CommandData{
            std::move(id), std::move(label), {}, {}, {}, CommandKind::Action, enabled, checkable, checked, 0, {}};
    }

    void test_command_model_stable_ids_validation_and_mutation() {
        CommandModel model;
        const CommandId first = model.append(test_command("first", "First"));
        const CommandId checked = model.append(test_command("checked", "Checked", true, true));
        model.insert(1, CommandData{"separator", {}, {}, {}, {}, CommandKind::Separator, true, false, false, 0, {}});
        assert(model.size() == 3);
        assert(model.index_of(first) == 0);
    ...
    void test_tool_bar_layout_activation_capture_and_model_lifetime() {
        ...
        toolbar.activated().connect(
            [&activated](ToolBar&, size_t, CommandId id, const CommandData&) { activated.push_back(id); });
        tc_ui_pointer_event pointer{};
        pointer.type = TC_UI_POINTER_MOVE;
    ...
    void test_menu_overlay_navigation_submenus_scrolling_and_dismissal() {
        ...
        const CommandId recent_scene = recent->append(
            CommandData{"recent-scene", "Scene.termin", {}, {}, {}, CommandKind::Action, true, false, false, 0, {}});
        auto model = std::make_shared<CommandModel>();
        model->append(test_command("disabled", "Disabled", false));
        model->append(CommandData{"separator", {}, {}, {}, {}, CommandKind::Separator, true, false, false, 0, {}});
        model->append(CommandData{"recent", "Recent", {}, {}, {}, CommandKind::Action, true, false, false, 0, recent});
        model->append(CommandData{"snap", "Snap", {}, {}, {}, CommandKind::Action, true, true, false, 0, {}});
        auto& menu = ui.make<Menu>(model);
        menu.set_max_visible_height(64.0f);
        std::string activated;
        menu.activated().connect(
            [&activated](Menu&, size_t, CommandId, const CommandData& command) { activated = command.stable_id; });
        assert(menu.show(document.get(), tc_ui_point{390.0f, 290.0f}, tc_ui_rect{0.0f, 0.0f, 400.0f, 300.0f}));
        ...
        assert(menu.show(document.get(), tc_ui_point{10.0f, 10.0f}, tc_ui_rect{0.0f, 0.0f, 400.0f, 300.0f}));
        tc_ui_pointer_event wheel{};
        wheel.type = TC_UI_POINTER_WHEEL;
        wheel.x = 20.0f;
        wheel.y = 20.0f;
    ...
    void test_menu_bar_adjacent_switching_shortcuts_and_overlay_lifetime() {
        ...
        file->append(CommandData{"save", "Save", {}, "Ctrl+S", {}, CommandKind::Action, true, true, false, 0, {}});
        auto edit = std::make_shared<CommandModel>();
        const CommandId undo =
            edit->append(CommandData{"undo", "Undo", {}, "Ctrl+Z", {}, CommandKind::Action, true, false, false, 0, {}});
        ...
        bar.activated().connect([&activated](MenuBar&, size_t, CommandId, const CommandData& command) {
        ...
        assert(document.overlay_count() == 1);
        assert((tc_ui_document_overlay_flags_at(document.get(), 0) & TC_UI_OVERLAY_ALLOW_ROOT_HIT) != 0);
    ...
    void test_dialog_modal_stack_focus_actions_and_exactly_once_results() {
        ...
        dialog.finished().connect([&results](Dialog&, const DialogResult& result) { results.push_back(result); });
        assert(dialog.show(document.get(), tc_ui_rect{0.0f, 0.0f, 800.0f, 600.0f}));
        assert(document.overlay_count() == 1);
        assert(dialog.button_handles().size() == 2);
        assert(tc_widget_handle_eq(document.focused_widget(), dialog.button_handles()[0]));
        ...
        assert(tc_widget_handle_eq(document.focused_widget(), dialog.button_handles()[1]));
        ...
        nested.finished().connect(
            [&nested_results](Dialog&, const DialogResult& result) { nested_results.push_back(result); });
        ...
        tc_widget* default_button_widget = tc_ui_document_resolve_widget(document.get(), dialog.button_handles()[0]);
        auto* default_button = dynamic_cast<Button*>(static_cast<Widget*>(default_button_widget->body));
        ...
        assert(dialog.show(document.get(), tc_ui_rect{0.0f, 0.0f, 800.0f, 600.0f}));
        tc_widget* apply_widget = tc_ui_document_resolve_widget(document.get(), dialog.button_handles()[0]);
        assert(apply_widget);
        ...
        assert(tc_widget_handle_eq(document.hit_test(click.x, click.y), dialog.button_handles()[0]));
    ...
    void test_message_box_and_input_dialog_share_modal_result_contract() {
        ...
        install_test_text_measurer(document);
        DocumentBuilder ui(document);
        auto& message = ui.make<MessageBox>("Delete", "Delete selected entity?", MessageBoxKind::Question);
        std::vector<DialogResult> message_results;
        message.finished().connect(
            [&message_results](Dialog&, const DialogResult& result) { message_results.push_back(result); });
        ...
        input.value_finished().connect(
            [&values](InputDialog&, const std::optional<std::string>& value) { values.push_back(value); });
        ...
        enter.key = TC_UI_KEY_ENTER;
        assert(document.dispatch_key_event(enter) == TC_UI_EVENT_HANDLED);
        assert(values.size() == 1 && values[0] == std::optional<std::string>{"New name"});
        assert(!input.open());
    ...
    void test_file_grid_widget_input_scrollbar_signals_and_lifetime() {
        ...
        for (size_t index = 0; index < 20; ++index) {
            model->append(
                CollectionItem{"file-" + std::to_string(index), "File " + std::to_string(index), ".txt", index != 3});
        }
        std::weak_ptr<CollectionModel> weak_model = model;
        auto& grid = ui.make_root<FileGridWidget>(model);
        ...
        grid.activated().connect(
            [&activated](FileGridWidget&, size_t index, const CollectionItem&) { activated.push_back(index); });
        grid.delete_requested().connect(
            [&deleted](FileGridWidget&, size_t index, const CollectionItem&) { deleted.push_back(index); });
        grid.context_menu_requested().connect(
            [&contexts](FileGridWidget&, int64_t index, float, float) { contexts.push_back(index); });
        ...
        grid.drag_requested().connect([&drags](FileGridWidget&, size_t index, float x, float y, int32_t modifiers) {
        ...
        pointer.type = TC_UI_POINTER_UP;
        assert(document.dispatch_pointer_event(pointer) == TC_UI_EVENT_HANDLED);
        assert((drags == std::vector<std::tuple<size_t, float, float, int32_t>>{{0, 160.0f, 140.0f, TC_UI_MOD_CTRL}}));
        ...
        destroying_grid.selection_changed().connect(
            [&document, destroying_handle](FileGridWidget&, const std::vector<size_t>&) {
                assert(tc_ui_document_destroy_widget(document.get(), destroying_handle));
            });
        destroying_model->erase(0);
        assert(!tc_ui_document_is_alive(document.get(), destroying_handle));
    ...
    void test_table_widget_virtualizes_large_model_and_lays_out_columns() {
        ...
        auto columns = std::make_shared<TableColumnModel>();
        columns->set_columns({
            TableColumn{"name", "Name", TableColumnPolicy::Fixed, 80.0f, 60.0f, 0.0f, 1.0f, true},
            TableColumn{"value", "Value", TableColumnPolicy::Stretch, 0.0f, 40.0f, 0.0f, 1.0f, true},
            TableColumn{"state", "State", TableColumnPolicy::Stretch, 0.0f, 40.0f, 160.0f, 2.0f, true},
        });
    ...
    void test_table_widget_pointer_keyboard_resize_signals_and_lifetime() {
        ...
        auto model = std::make_shared<TableModel>();
        const TableRowId first = model->append(TableRowData{"first", {"First", "1"}, true});
        const TableRowId disabled = model->append(TableRowData{"disabled", {"Disabled", "2"}, false});
        const TableRowId last = model->append(TableRowData{"last", {"Last", "3"}, true});
        auto columns = std::make_shared<TableColumnModel>();
        columns->set_columns({
            TableColumn{"name", "Name", TableColumnPolicy::Fixed, 100.0f, 60.0f, 180.0f, 1.0f, true},
            TableColumn{"value", "Value", TableColumnPolicy::Stretch, 0.0f, 40.0f, 0.0f, 1.0f, true},
        });
        ...
        table.header_clicked().connect(
            [&clicked_column](TableWidget&, size_t index, const TableColumn&) { clicked_column = index; });
        table.column_resized().connect([&resized_column, &resized_width](TableWidget&, size_t index, float width) {
            resized_column = index;
            resized_width = width;
        });
        table.activated().connect(
            [&activated](TableWidget&, size_t, TableRowId id, const TableRowData&) { activated = id; });
        table.context_menu_requested().connect(
            [&contexts](TableWidget&, int64_t index, float, float) { contexts.push_back(index); });
        ...
        const TableRowId destroying_first = destroying_model->append(TableRowData{"destroying-first", {"First"}, true});
        ...
        destroying_table.selection_changed().connect(
            [&document, destroying_handle](TableWidget&, const std::vector<size_t>&) {
                assert(tc_ui_document_destroy_widget(document.get(), destroying_handle));
            });
    ...
    void test_host_click_count_drives_collection_activation() {
        {
            ...
            list.activated().connect([&activations](ListWidget&, size_t, const CollectionItem&) { ++activations; });
            tc_ui_pointer_event pointer{};
            pointer.type = TC_UI_POINTER_DOWN;
            pointer.button = 0;
            pointer.x = 10.0f;
        ...
        }
        {
            ...
            TreeNodeId activated = kInvalidTreeNodeId;
            tree.activated().connect(
                [&activated](TreeWidget&, TreeNodeId id, const CollectionItem&) { activated = id; });
            tc_ui_pointer_event pointer{};
            pointer.type = TC_UI_POINTER_DOWN;
            pointer.button = 0;
            pointer.click_count = 1;
        ...
        }
        {
            ...
            auto columns = std::make_shared<TableColumnModel>();
            columns->set_columns({TableColumn{"value", "Value", TableColumnPolicy::Stretch, 0.0f, 20.0f}});
            auto& table = ui.make_root<TableWidget>(model, columns);
        ...
        }
        {
            ...
            size_t activations = 0;
            grid.activated().connect([&activations](FileGridWidget&, size_t, const CollectionItem&) { ++activations; });
            tc_ui_pointer_event pointer{};
            pointer.type = TC_UI_POINTER_DOWN;
            pointer.button = 0;
            pointer.click_count = 1;
