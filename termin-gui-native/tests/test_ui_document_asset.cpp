...
root:
  ...
  variants:
    ...
    void test_compiled_round_trip_and_independent_instances() {
        ...
        TcUiDocumentAsset asset = TcUiDocumentAsset::declare_source("ui-test", "Test UI", "ui/test.uiscript", SOURCE);
        assert(asset.valid());
        ...
        TcUiDocumentAsset::clear_registry_for_tests();
        TcUiDocumentAsset restored = TcUiDocumentAsset::declare_compiled_json(compiled, "ui-test");
    ...
    void test_reload_is_transactional_for_recipe_and_instance() {
        TcUiDocumentAsset::clear_registry_for_tests();
        TcUiDocumentAsset asset =
            TcUiDocumentAsset::declare_source("ui-reload", "Reload UI", "ui/reload.uiscript", SOURCE);
        assert(asset.valid());
        ...
        assert(!asset.reload_source("uiscript: 2\nroot:\n  type: termin.gui.Missing\n"));
        assert(asset.revision() == initial_revision);
        assert(tc_ui_document_resolve_widget(loaded.document().handle(), old_root));
        ...
        replacement.replace(name, std::string("name: action").size(), "name: replacement");
        ...
        assert(!tc_ui_document_resolve_widget(reloaded.document().handle(), old_root));
        assert(reloaded.named("replacement").type_name == "termin.gui.IconButton");
    }

    void test_generation_handle_and_compiled_validation() {
        TcUiDocumentAsset::clear_registry_for_tests();
        TcUiDocumentAsset asset =
            TcUiDocumentAsset::declare_source("ui-generation", "Generation UI", "ui/generation.uiscript", SOURCE);
        assert(asset.valid());
        ...
        compiled.replace(dependency, std::string("termin.gui.BoxLayout").size(), "termin.gui.Missing");
        assert(!TcUiDocumentAsset::declare_compiled_json(compiled, "ui-generation").valid());
    }

    void test_grid_scroll_round_trip_and_transactional_reload() {
        TcUiDocumentAsset::clear_registry_for_tests();
        TcUiDocumentAsset asset = TcUiDocumentAsset::declare_source(
            "ui-grid-scroll", "Grid Scroll", "ui/grid-scroll.uiscript", GRID_SCROLL_SOURCE);
        assert(asset.valid());
        ...
        TcUiDocumentAsset::clear_registry_for_tests();
        asset = TcUiDocumentAsset::declare_compiled_json(compiled, "ui-grid-scroll");
        assert(asset.valid());
        assert(asset.resolve()->compiled_json() == compiled);
        LoadedUiScript restored = asset.instantiate();
        tc_widget* scroll_widget = tc_ui_document_resolve_widget(restored.document().handle(), restored.root().handle);
        tc_widget* grid_widget =
            tc_ui_document_resolve_widget(restored.document().handle(), restored.named("grid").handle);
        ...
        assert(tc_ui_document_resolve_widget(live.document().handle(), live_root));
        assert(tc_ui_document_resolve_widget(instance.document().handle(), old_root));
    }

    void test_wrapped_label_package_round_trip() {
        TcUiDocumentAsset::clear_registry_for_tests();
        TcUiDocumentAsset asset =
            TcUiDocumentAsset::declare_source("ui-wrap", "Wrapped UI", "ui/wrap.uiscript", WRAP_SOURCE);
        assert(asset.valid());
        const std::string compiled = asset.resolve()->compiled_json();
        TcUiDocumentAsset::clear_registry_for_tests();
        ...
        assert(asset.resolve()->compiled_json() == compiled);
        LoadedUiScript restored = asset.instantiate();
        tc_widget* flow_widget = tc_ui_document_resolve_widget(restored.document().handle(), restored.root().handle);
        tc_widget* label_widget =
            tc_ui_document_resolve_widget(restored.document().handle(), restored.named("text").handle);
    ...
    void test_responsive_variant_package_round_trip() {
        TcUiDocumentAsset::clear_registry_for_tests();
        TcUiDocumentAsset asset = TcUiDocumentAsset::declare_source(
            "ui-responsive", "Responsive UI", "ui/responsive.uiscript", RESPONSIVE_SOURCE);
        assert(asset.valid());
        const std::string compiled = asset.resolve()->compiled_json();
        assert(compiled.find("\"variants\"") != std::string::npos);
        TcUiDocumentAsset::clear_registry_for_tests();
        asset = TcUiDocumentAsset::declare_compiled_json(compiled, "ui-responsive");
        assert(asset.valid());
        assert(asset.resolve()->compiled_json() == compiled);
        LoadedUiScript restored = asset.instantiate();
        restored.document().layout_roots({0.0f, 0.0f, 800.0f, 600.0f});
        tc_widget* root = tc_ui_document_resolve_widget(restored.document().handle(), restored.root().handle);
        assert(root);
        const auto* box = static_cast<const BoxLayout*>(root->body);
        assert(box->orientation() == Orientation::Horizontal);
        assert(box->spacing() == 12.0f);
        assert(restored.document().root_layout_policy() == TC_UI_ROOT_LAYOUT_SAFE_AREA);
    }
