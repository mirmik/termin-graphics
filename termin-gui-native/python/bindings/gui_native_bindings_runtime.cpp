...
tc_widget* WidgetRef::resolve() const {
    return state && state->document ? tc_ui_document_resolve_widget(state->document, handle)
                                    : nullptr;
}
...
bool PythonViewportSurfaceHost::pointer_button(
    int button, int action, int modifiers, uint32_t click_count) {
...
PythonWidget::PythonWidget(
    nb::object object_, std::string debug_name_, std::shared_ptr<DocumentState> state_)
...
void PythonWidget::capture_exception(const char* operation) {
    ...
    tc_log_error(
        "[termin-gui-native/python] Python widget %s failed for '%s'", operation,
        tc_widget_debug_name(&widget) ? tc_widget_debug_name(&widget) : "<unnamed>");
}

tc_ui_size PythonWidget::measure(
    tc_widget* widget, tc_ui_document*, tc_ui_constraints constraints) {
    PythonWidget* self = from_widget(widget);
    if (!self) {
...
void PythonWidget::layout(tc_widget* widget, tc_ui_document*, tc_ui_rect rect) {
    PythonWidget* self = from_widget(widget);
    if (!self) {
...
void PythonWidget::paint(tc_widget* widget, tc_ui_document*, tc_ui_paint_context* context) {
    PythonWidget* self = from_widget(widget);
    if (!self) {
...
tc_ui_event_result PythonWidget::pointer_event(
    tc_widget* widget, tc_ui_document*, const tc_ui_pointer_event* event) {
    PythonWidget* self = from_widget(widget);
    if (!self || !event) {
        tc_log_error(
            "[termin-gui-native/python] cannot route pointer event to invalid Python widget shim");
...
tc_widget_handle PythonWidget::hit_test(
    tc_widget* widget, tc_ui_document* document, float x, float y) {
    PythonWidget* self = from_widget(widget);
    if (!self) {
...
tc_ui_event_result PythonWidget::key_event(
    tc_widget* widget, tc_ui_document*, const tc_ui_key_event* event) {
    PythonWidget* self = from_widget(widget);
    if (!self || !event) {
        tc_log_error(
            "[termin-gui-native/python] cannot route key event to invalid Python widget shim");
        return TC_UI_EVENT_IGNORED;
    }
    try {
        nb::gil_scoped_acquire gil;
...
tc_ui_event_result PythonWidget::text_event(
    tc_widget* widget, tc_ui_document*, const tc_ui_text_event* event) {
    PythonWidget* self = from_widget(widget);
    if (!self || !event) {
        tc_log_error(
            "[termin-gui-native/python] cannot route text event to invalid Python widget shim");
        return TC_UI_EVENT_IGNORED;
    }
    try {
        nb::gil_scoped_acquire gil;
...
void PythonWidget::focus_event(tc_widget* widget, tc_ui_document*, bool focused) {
    PythonWidget* self = from_widget(widget);
    if (!self) {
        tc_log_error(
            "[termin-gui-native/python] cannot route focus event to invalid Python widget shim");
        return;
    }
    try {
        nb::gil_scoped_acquire gil;
...
void PythonWidget::overlay_dismissed(
    tc_widget* widget, tc_ui_document*, tc_ui_overlay_dismiss_reason reason) {
    PythonWidget* self = from_widget(widget);
    if (!self) {
        tc_log_error(
            "[termin-gui-native/python] cannot notify invalid dismissed overlay shim");
        return;
    }
    try {
        nb::gil_scoped_acquire gil;
...
bool create_python_registered_widget(
    tc_ui_document* document, void* userdata, tc_widget_factory_result* result) {
...
bool bind_python_registered_widget(
    tc_ui_document*, tc_widget* widget, tc_widget_handle handle, void*) {
...
bool serialize_python_registered_widget(
    const tc_widget* widget, void* userdata, tc_value* out_state) {
    auto* factory = static_cast<PythonWidgetFactory*>(userdata);
...
bool deserialize_python_registered_widget(
    tc_widget* widget, const tc_value* state, void* userdata) {
    auto* factory = static_cast<PythonWidgetFactory*>(userdata);
    PythonWidget* python_widget = PythonWidget::from_widget(widget);
...
tc_ui_document* Document::get() const {
...
WidgetHandle Document::adopt(nb::object object, const std::string& debug_name) {
    ...
    tc_widget_handle handle =
        tc_ui_document_adopt_widget(get(), &widget->widget, &PythonWidget::delete_widget);
...
bool Document::clipboard_set(void* user_data, const char* text, size_t byte_length) {
    auto* self = static_cast<Document*>(user_data);
    try {
        nb::gil_scoped_acquire gil;
