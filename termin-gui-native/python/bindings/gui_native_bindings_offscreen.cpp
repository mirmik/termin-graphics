...
namespace {
    ...
    Rgba8View rgba8_view(const Rgba8Array& array, const char* operation) {
        ...
        if (array.shape(0) == 0 || array.shape(1) == 0 || array.shape(0) > std::numeric_limits<uint32_t>::max() ||
            array.shape(1) > std::numeric_limits<uint32_t>::max()) {
            throw std::invalid_argument(std::string(operation) + " requires positive uint32-sized dimensions");
        }
        ...
        return Rgba8View{
            width,
            height,
            std::span<const uint8_t>(array.data(), static_cast<size_t>(width) * static_cast<size_t>(height) * 4),
        };
    }
    ...
    public:
        ...
        ~PythonOffscreenGuiComposition() {
            try {
                if (composition_) {
                    tc_log_error("[termin-gui-native/python] OffscreenGuiComposition reached "
                                 "finalization while open; closing it");
                }
                close();
            } catch (const std::exception& error) {
                tc_log_error("[termin-gui-native/python] offscreen finalization failed: %s", error.what());
            } catch (...) {
                tc_log_error("[termin-gui-native/python] offscreen finalization failed with "
                             "unknown exception");
            }
        }
        ...
        bool is_open() const {
            return composition_ && composition_->is_open();
        }
    ...
    tgfx::BackendType offscreen_backend(const std::string& name) {
        ...
        if (backend == tgfx::BackendType::Null) {
            throw std::invalid_argument("backend must name a supported offscreen backend such as "
                                        "'vulkan' or 'd3d11'");
        }
        return backend;
    }

    nb::ndarray<nb::numpy, float> read_offscreen_frame(PythonOffscreenGuiComposition& self) {
    ...
    void set_unhandled_key_handler(termin::gui_native::DocumentRenderer& renderer, nb::object callback) {
        if (!callback.is_none() && !nb::isinstance<nb::callable>(callback)) {
            throw std::invalid_argument("unhandled key handler must be callable or None");
        }
        if (callback.is_none()) {
            renderer.set_unhandled_key_handler({});
            return;
        }
        renderer.set_unhandled_key_handler([callback = std::move(callback)](const tc_ui_key_event& event) {
...
void bind_gui_native_offscreen(nb::module_& m) {
    ...
    nb::class_<PythonOffscreenGuiComposition>(m, "OffscreenGuiComposition")
        ...
        .def("pump_events", [](PythonOffscreenGuiComposition& self) { return self.get().pump_input(); })
        .def("render_frame", [](PythonOffscreenGuiComposition& self) { return self.get().render_frame(); })
        .def("tick", [](PythonOffscreenGuiComposition& self) { return self.get().tick(); })
        .def("resize",
             [](PythonOffscreenGuiComposition& self, int width, int height) { self.get().resize(width, height); })
        .def("read_frame_rgba_float", &read_offscreen_frame)
        .def(
            "push_pointer_move",
            [](PythonOffscreenGuiComposition& self, float x, float y, uint32_t modifiers) {
        ...
        .def(
            "push_key",
            [](PythonOffscreenGuiComposition& self, int32_t key, bool pressed, uint32_t modifiers, bool repeat) {
            ...
            nb::arg("repeat") = false)
        .def("push_text",
             [](PythonOffscreenGuiComposition& self, const std::string& text) { self.get().push_text(text); })
        .def("request_repaint", [](PythonOffscreenGuiComposition& self) { self.get().request_repaint(); })
        .def_prop_ro("repaint_requested",
                     [](PythonOffscreenGuiComposition& self) { return self.get().repaint_requested(); })
        .def_prop_ro("should_close", [](PythonOffscreenGuiComposition& self) { return self.get().should_close(); })
        .def("request_close", [](PythonOffscreenGuiComposition& self) { self.get().request_close(); })
        .def("wait_idle", [](PythonOffscreenGuiComposition& self) { self.get().wait_idle(); })
        ...
        .def_prop_ro("frame_generation",
                     [](PythonOffscreenGuiComposition& self) { return self.get().frame_generation(); })
        .def_prop_ro("latest_frame_texture",
                     [](PythonOffscreenGuiComposition& self) { return self.get().latest_frame_texture(); })
        ...
        .def_prop_ro(
            "clipboard_text",
            [](PythonOffscreenGuiComposition& self) { return self.get().platform_services().clipboard_text(); })
        .def_prop_ro(
            "text_input_enabled",
            [](PythonOffscreenGuiComposition& self) { return self.get().platform_services().text_input_enabled(); })
        .def("close", &PythonOffscreenGuiComposition::close)
        .def_prop_ro("closed", [](const PythonOffscreenGuiComposition& self) { return !self.is_open(); })
        .def(
            "__enter__",
            [](PythonOffscreenGuiComposition& self) -> PythonOffscreenGuiComposition& { return self; },
            nb::rv_policy::reference_internal)
        .def("__exit__", [](PythonOffscreenGuiComposition& self, nb::handle, nb::handle, nb::handle) {
            self.close();
            return false;
        });
    ...
    nb::class_<DynamicTextureLease>(m, "DynamicTextureLease")
        .def(
            "__init__",
            [](DynamicTextureLease* self, PythonOffscreenGuiComposition& composition) {
        ...
        .def("update_region_rgba8",
             [](DynamicTextureLease& self, uint32_t x, uint32_t y, const Rgba8Array& data) {
                 ...
                 self.update_region_rgba8(x, y, view.width, view.height, view.pixels);
             })
        .def("borrow", &DynamicTextureLease::borrow)
        .def(
            "bind_canvas",
            [](DynamicTextureLease& self, const CanvasRef& canvas, CanvasTextureLayer layer) {
