...
inline tc_value python_to_tc_value(nb::object value) {
    if (value.is_none())
        return tc_value_nil();
...
inline nb::object tc_value_to_python(const tc_value* value) {
    if (!value)
        return nb::none();
    switch (value->type) {
    case TC_VALUE_NIL:
...
inline std::mutex g_document_states_mutex;
inline std::unordered_map<tc_ui_document*, std::weak_ptr<DocumentState>> g_document_states;

inline void register_document_state(const std::shared_ptr<DocumentState>& state) {
    std::lock_guard<std::mutex> lock(g_document_states_mutex);
    g_document_states[state->document] = state;
}

inline void unregister_document_state(tc_ui_document* document) {
    std::lock_guard<std::mutex> lock(g_document_states_mutex);
    g_document_states.erase(document);
}

inline std::shared_ptr<DocumentState> find_document_state(tc_ui_document* document) {
    std::lock_guard<std::mutex> lock(g_document_states_mutex);
...
inline const tc_widget_vtable PythonWidget::VTABLE {
    "PythonWidget",
...
inline bool create_python_registered_widget(tc_ui_document* document, void* userdata,
...
inline bool bind_python_registered_widget(tc_ui_document*, tc_widget* widget,
...
inline void destroy_python_widget_factory(void* userdata) {
    if (!userdata)
        return;
    nb::gil_scoped_acquire gil;
...
inline bool serialize_python_registered_widget(const tc_widget* widget, void* userdata,
                                        tc_value* out_state) {
    auto* factory = static_cast<PythonWidgetFactory*>(userdata);
...
inline bool deserialize_python_registered_widget(tc_widget* widget, const tc_value* state,
                                          void* userdata) {
    auto* factory = static_cast<PythonWidgetFactory*>(userdata);
    PythonWidget* python_widget = PythonWidget::from_widget(widget);
...
inline nb::object snapshot_handle_or_none(tc_widget_handle handle) {
    return tc_widget_handle_is_invalid(handle) ? nb::none()
                                               : nb::cast(WidgetHandle {handle});
}

inline nb::dict document_snapshot_to_python(const Document& document) {
...
inline DrawCommand command_at_checked(const DrawList& draw_list, size_t index) {
