...
namespace termin::gui_native::python_bindings {
    ...
    struct WidgetRef {
        ...
        WidgetRef() = default;
        WidgetRef(std::shared_ptr<DocumentState> state_value, tc_widget_handle handle_value)
    ...
    template <typename T> T& native_widget_checked(const WidgetRef& ref, const char* expected_type) {
...
#define TERMIN_GUI_NATIVE_DERIVED_WIDGET_REF(Name, Type)                                                               \
    ...
    TERMIN_GUI_NATIVE_DERIVED_WIDGET_REF(WrapLayoutRef, WrapLayout);
...
#undef TERMIN_GUI_NATIVE_WIDGET_REF
    ...
    struct PythonWidget {
        ...
        static tc_ui_size measure(tc_widget* widget, tc_ui_document_handle, tc_ui_constraints constraints);
        ...
        pointer_event(tc_widget* widget, tc_ui_document_handle, const tc_ui_pointer_event* event);
        static tc_widget_handle hit_test(tc_widget* widget, tc_ui_document_handle document, float x, float y);
        static tc_ui_event_result key_event(tc_widget* widget, tc_ui_document_handle, const tc_ui_key_event* event);
        static tc_ui_event_result text_event(tc_widget* widget, tc_ui_document_handle, const tc_ui_text_event* event);
        static void focus_event(tc_widget* widget, tc_ui_document_handle, bool focused);
        static void overlay_dismissed(tc_widget* widget, tc_ui_document_handle, tc_ui_overlay_dismiss_reason reason);
    ...
    void set_document_clipboard_handlers(termin::gui_native::TcDocument document, nb::object getter, nb::object setter);
    ...
    WidgetRef document_make_native(termin::gui_native::TcDocument document, Args&&... args) {
        ...
        if constexpr (termin::gui_native::NativeWidgetRuntimeType<T>::name) {
            ...
            tc_widget_factory_result result{
                widget.release()->c_widget(), &termin::gui_native::Widget::delete_owned_widget, TC_WIDGET_OWNED};
            if (!tc_ui_document_adopt_registered_widget(checked_document_handle(document),
                                                        termin::gui_native::NativeWidgetRuntimeType<T>::name,
                                                        &result,
                                                        &handle)) {
                throw std::runtime_error("failed to adopt registered native widget");
            }
        } else {
            handle = tc_ui_document_adopt_widget(checked_document_handle(document),
                                                 widget->c_widget(),
