...
namespace {
    ...
    ColorPicker& require_color_picker(GuiWindowAdapter& adapter, const nb::object& picker) {
        WidgetHandleRef handle = nb::cast<WidgetHandleRef>(picker.attr("handle"));
        tc_widget* widget = tc_ui_document_resolve_widget(adapter.document().handle(), handle.handle);
        if (!widget || !widget->body) {
            throw std::runtime_error("color picker reference is stale");
        }
        auto* color_picker = dynamic_cast<ColorPicker*>(static_cast<termin::gui_native::Widget*>(widget->body));
        if (!color_picker) {
    ...
    nb::dict python_window_event(const termin::WindowEvent& event) {
        ...
        case termin::WindowEventType::KeyReleased:
            result["type"] = event.type == termin::WindowEventType::KeyPressed ? "key_down" : "key_up";
...
void bind_gui_native_window_adapter(nb::module_& m) {
    nb::class_<GuiWindowAdapter>(m, "GuiWindowAdapter")
        ...
        .def(
            ...
            [](GuiWindowAdapter& self, WindowManager& manager, WindowHandle handle, nb::object interceptor) {
                if (!interceptor.is_none() && !nb::isinstance<nb::callable>(interceptor)) {
                    throw std::invalid_argument("event interceptor must be callable or None");
                }
                if (&manager.window(handle) != &self.window()) {
                    throw std::invalid_argument("window handle does not identify this adapter window");
                }
                std::vector<termin::WindowEvent> events = manager.take_events(handle);
                std::vector<termin::WindowEvent> unconsumed;
                for (const termin::WindowEvent& event : events) {
                    const bool consumed =
                        !interceptor.is_none() && nb::cast<bool>(interceptor(python_window_event(event)));
                    if (!consumed)
        ...
        .def(
            ...
            [](GuiWindowAdapter& self, nb::object callback) {
                set_unhandled_key_handler(self.renderer(), std::move(callback));
            },
        ...
        .def(
            ...
            [](GuiWindowAdapter& self, nb::object callback) {
                if (!callback.is_none() && !nb::isinstance<nb::callable>(callback)) {
                    throw std::invalid_argument("before-frame callback must be callable or None");
                }
                if (callback.is_none()) {
                    self.renderer().set_before_frame_callback({});
                    return;
                }
                self.renderer().set_before_frame_callback([callback = std::move(callback)](tgfx::RenderContext2&) {
                    nb::gil_scoped_acquire gil;
                    callback();
                });
            },
        ...
        .def("register_color_picker",
             [](GuiWindowAdapter& self, const nb::object& picker) {
                 self.renderer().register_color_picker(require_color_picker(self, picker));
             })
        .def("unregister_color_picker",
             [](GuiWindowAdapter& self, const nb::object& picker) {
                 self.renderer().unregister_color_picker(require_color_picker(self, picker));
             })
        ...
        .def_prop_ro("closed", [](GuiWindowAdapter& self) { return !self.is_open(); })
        .def_prop_ro("rendered_frame_count",
                     [](GuiWindowAdapter& self) { return self.renderer().rendered_frame_count(); })
        .def_prop_ro("framebuffer_size",
                     [](GuiWindowAdapter& self) {
                         const auto [width, height] = self.renderer().framebuffer_size();
                         return std::array<int, 2>{width, height};
                     })
        .def_prop_ro("color_target", [](GuiWindowAdapter& self) { return self.renderer().color_target(); })
        .def_prop_ro(
            "window",
            [](GuiWindowAdapter& self) -> termin::BackendWindow& { return self.window(); },
            nb::rv_policy::reference_internal)
        .def_prop_ro(
            "graphics",
            [](GuiWindowAdapter& self) -> tgfx::GraphicsHost& { return self.renderer().graphics(); },
            nb::rv_policy::reference_internal);

    m.def(
        "dynamic_texture_lease",
        [](GuiWindowAdapter& adapter) -> DynamicTextureLease* { return new DynamicTextureLease(adapter.renderer()); },
        nb::arg("adapter"),
